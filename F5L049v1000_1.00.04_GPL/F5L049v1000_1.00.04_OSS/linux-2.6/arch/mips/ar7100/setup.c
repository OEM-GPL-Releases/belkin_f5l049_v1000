...
#ifdef CONFIG_SXLEDCTL
#include "cfg_led.h"
#include "led_ctrl.h"
#endif /* CONFIG_SXLEDCTL */
...
extern int sxflash_eeprom_need_factory_default;
...
enable_wmac_led()
{
    sx_led_on(LED_WIRELESS, LED_OPT_FORCE);
}
...
disable_wmac_led()
{
    sx_led_off(LED_WIRELESS, LED_OPT_FORCE);
}
...
{
    uint32_t pll, pll_div, ahb_div, ddr_div, freq;
...
static int sx_check_factory_default(void)
{
	int sw_pushed = 0;

	while((ar7100_reg_rd(AR7100_GPIO_IN) & 0x200000) == 0) {
		/* Turn ON ALL LEDs */
		/* GPIO2 and GPIO17 must set bit, because this bit use USB power control
		 * for the USB HUB errata */
		ar7100_reg_wr_nf(AR7100_GPIO_OUT, 0x20004);
		sw_pushed = 1;
	}
	if(sw_pushed) {
		/* Turn off LEDs except Power LED */
		ar7100_reg_wr_nf(AR7100_GPIO_OUT, 0x205ff);
	}
...
#if 0
	/* The boot loader initial GPIO OUTE register, Linux donot initial again. */
	ar7100_reg_wr_nf(AR7100_GPIO_OE, 0xDFB);
#endif
	/* Early setup GPIO Output enable */
	ar7100_reg_wr_nf(AR7100_GPIO_OUT, 0x205ff);

	/* The specification of silex factory default setting */
	if(sx_check_factory_default()) {
		sxflash_eeprom_need_factory_default = 1;
	}
	else
		sxflash_eeprom_need_factory_default = 0;
...
serial_setup(void)
{
	...
	p.flags     = UPF_BOOT_AUTOCONF | UPF_SKIP_TEST | UPF_AUTO_IRQ;
	p.iotype    = UPIO_MEM32;
