...
static void send_repl_message(uint32 low_serial)
{
        ...
        if (!tdb) {
                DEBUG3( ("send_repl_message(): failed to open connections "
                          "database\n"));
                return;
        }

        DEBUG3( ("sending replication message, serial = 0x%04x\n", 
                  low_serial));
                          ...
                          const char *mailslot)
{
	...
	if (!lp_domain_logons()) {
		DEBUG5(("process_logon_packet: Logon packet received from IP %s and domain \
logons are not enabled.\n", inet_ntoa(p->ip) ));
	...
	DEBUG4(("process_logon_packet: Logon from %s: code = 0x%x\n", inet_ntoa(p->ip), code));
		...
		case 0:    
			{
				...
				if (!user || PTR_DIFF(user, buf) >= len) {
					DEBUG0(("process_logon_packet: bad packet\n"));
					return;
				}
				...
				DEBUG5(("process_logon_packet: Domain login request from %s at IP %s user=%s token=%x\n",
		...
		case QUERYFORPDC:
			{
				...
				DEBUG5(("process_logon_packet: GETDC request from %s at IP %s, \
reporting %s domain %s 0x%x ntversion=%x lm_nt token=%x lm_20 token=%x\n",
		...
		case SAMLOGON:

			{
				...
				DEBUG5(("process_logon_packet: SAMLOGON sidsize %d, len = %d\n", domainsidsize, len));
				...
				DEBUG5(("process_logon_packet: len = %d PTR_DIFF(q, buf) = %ld\n", len, (unsigned long)PTR_DIFF(q, buf) ));
				...
				DEBUG3(("process_logon_packet: SAMLOGON sidsize %d ntv %d\n", domainsidsize, ntversion));
				...
				DEBUG5(("process_logon_packet: SAMLOGON user %s\n", ascuser));

				fstrcpy(reply_name, "\\\\"); /* Here it wants \\LOGONSERVER. */
				fstrcat(reply_name, my_name);

				DEBUG5(("process_logon_packet: SAMLOGON request from %s(%s) for %s, returning logon svr %s domain %s code %x token=%x\n",
				...
				if ((ntversion < 11) || (SEC_ADS != lp_security()) || (ROLE_DOMAIN_PDC != lp_server_role())) {
					...
					if (False == secrets_fetch_domain_guid(domain, &domain_guid)) {
						DEBUG2( ("Could not fetch DomainGUID for %s\n", domain));
						return;
					}
		...
		case SAM_UAS_CHANGE:
			{
				...
				if (db_info == NULL) {
					DEBUG3( ("out of memory allocating info for %d databases\n", db_count));
					return;
				}
		...
		default:
			DEBUG3(("process_logon_packet: Unknown domain request %d\n",code));
			return;
	}
}
