					 ...
					 struct in_addr mask_ip)
{
	...
	if (!subrec) {
		DEBUG0(("make_subnet: malloc fail !\n"));
		close(nmb_sock);
		close(dgram_sock);
		return(NULL);
	}
	...
	if((subrec->subnet_name = SMB_STRDUP(name)) == NULL) {
		DEBUG0(("make_subnet: malloc fail for subnet name !\n"));
		close(nmb_sock);
		close(dgram_sock);
		ZERO_STRUCTP(subrec);
		SAFE_FREE(subrec);
	...
	DEBUG2( ("making subnet name:%s ", name ));
	DEBUG2( ("Broadcast address:%s ", inet_ntoa(bcast_ip)));
	DEBUG2( ("Subnet mask:%s\n", inet_ntoa(mask_ip)));
...
{    
  ...
  try_interfaces_again:
	...
	if (iface_count_nl() == 0) {
		DEBUG0(("create_subnets: No non-loopback local interfaces !\n"));
		DEBUG0(("create_subnets: Waiting for an interface to appear ...\n"));
	}
	...
	for (i = 0 ; i < num_interfaces; i++) {
		...
		if (!iface) {
			DEBUG2(("create_subnets: can't get interface %d.\n", i ));
			continue;
		}
		...
		if (ip_equal(iface->ip, loopback_ip)) {
			DEBUG2(("create_subnets: Ignoring loopback interface.\n" ));
			continue;
		}
	...
	if (subnetlist == NULL) {
		...
		DEBUG0(("create_subnets: Unable to create any subnet from "
