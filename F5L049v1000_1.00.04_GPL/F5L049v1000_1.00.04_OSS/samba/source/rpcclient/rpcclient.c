...
static void fetch_machine_sid(struct cli_state *cli)
{
	...
	if (!(mem_ctx=talloc_init("fetch_machine_sid"))) {
		DEBUG0(("fetch_machine_sid: talloc_init returned NULL!\n"));
		goto error;
	}
	...
	if (cmd_entry->pipe_idx != -1 && cmd_entry->rpc_pipe == NULL) {
		switch (pipe_default_auth_type) {
			...
			case PIPE_AUTH_TYPE_SCHANNEL:
				...
				break;
			default:
				DEBUG0( ("Could not initialise %s. Invalid auth type %u\n",
					cli_get_pipe_name(cmd_entry->pipe_idx),
					pipe_default_auth_type ));
				return NT_STATUS_UNSUCCESSFUL;
		}
		if (!cmd_entry->rpc_pipe) {
			DEBUG0( ("Could not initialise %s. Error was %s\n",
				cli_get_pipe_name(cmd_entry->pipe_idx),
				nt_errstr(ntresult) ));
			return ntresult;
		}
		...
		if (cmd_entry->pipe_idx == PI_NETLOGON) {
			...
			if (!NT_STATUS_IS_OK(ntresult)) {
				DEBUG0( ("Could not initialise credentials for %s.\n",
					cli_get_pipe_name(cmd_entry->pipe_idx)));
				return ntresult;
			}
		}
 ...
 int main(int argc, char *argv[])
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("Cannot connect to server.  Error was %s\n", nt_errstr(nt_status)));
		return 1;
	}
