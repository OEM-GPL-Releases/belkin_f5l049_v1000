...
void cmd_wks_query_info(struct client_info *info)
{
	...
	DEBUG4(("cmd_wks_query_info: server:%s info level: %d\n",
				dest_wks, info_level));

	DEBUG5( ("cmd_wks_query_info: smb_cli->fd:%d\n", smb_cli->fd));
	...
	if (res)
	{
		DEBUG5(("cmd_wks_query_info: query succeeded\n"));
	...
	{
		DEBUG5(("cmd_wks_query_info: query failed\n"));
	}
}
