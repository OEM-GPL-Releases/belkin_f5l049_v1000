...
void set_profile_level(int level, struct process_id src)
{
	...
	case 0:		/* turn off profiling */
		...
		DEBUG1(("INFO: Profiling turned OFF from pid %d\n",
			 (int)procid_to_pid(&src)));
		break;
	case 1:		/* turn on counter profiling only */
		do_profile_flag = True;
		do_profile_times = False;
		DEBUG1(("INFO: Profiling counts turned ON from pid %d\n",
			 (int)procid_to_pid(&src)));
		break;
	case 2:		/* turn on complete profiling */
		...
		if (!have_profiling_clock) {
			do_profile_flag = True;
			do_profile_times = False;
			DEBUG1(("INFO: Profiling counts turned ON from "
				"pid %d\n", (int)procid_to_pid(&src)));
		...
		DEBUG1(("INFO: Full profiling turned ON from pid %d\n",
			 (int)procid_to_pid(&src)));
		break;
	case 3:		/* reset profile values */
		memset((char *)profile_p, 0, sizeof(*profile_p));
		DEBUG1(("INFO: Profiling values cleared from pid %d\n",
			 (int)procid_to_pid(&src)));
		break;
	}
#else /* WITH_PROFILE */
	DEBUG1(("INFO: Profiling support unavailable in this build.\n"));
#endif /* WITH_PROFILE */
}
			...
			void *buf, size_t len, void *private_data)
{
	...
	level = 0;
#endif
	DEBUG1(("INFO: Received REQ_PROFILELEVEL message from PID %u\n",
...
static void init_clock_gettime(void)
{
	...
	if (!this_is_smp()) {
	    if (clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &ts) == 0) {
		    DEBUG10( ("Using CLOCK_PROCESS_CPUTIME_ID "
				"for profile_clock\n"));
	...
	if (!have_profiling_clock &&
		...
		DEBUG10( ("Using CLOCK_MONOTONIC for profile_clock\n"));
...
BOOL profile_setup(BOOL rdonly)
{
 ...
 again:
	...
	if (shm_id == -1) {
		DEBUG0(("Can't create or use IPC area. Error was %s\n", 
			 strerror(errno)));
		return False;
	}   
	...
	if ((long)profile_p == -1) {
		DEBUG0(("Can't attach to IPC area. Error was %s\n", 
			 strerror(errno)));
		return False;
	}
	...
	if (shmctl(shm_id, IPC_STAT, &shm_ds) != 0) {
		DEBUG0(("ERROR shmctl : can't IPC_STAT. Error was %s\n", 
			 strerror(errno)));
		return False;
	}
	...
	if (shm_ds.shm_perm.cuid != sec_initial_uid() ||
		...
		DEBUG0(("ERROR: we did not create the shmem "
	...
	if (shm_ds.shm_segsz != sizeof(*profile_h)) {
		DEBUG0(("WARNING: profile size is %d (expected %lu). Deleting\n",
	...
	if (!read_only && (shm_ds.shm_nattch == 1)) {
		...
		DEBUG3(("Initialised profile area\n"));
	}
