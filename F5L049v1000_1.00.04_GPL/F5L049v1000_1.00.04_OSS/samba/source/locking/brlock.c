...
{
	DEBUG10(("[%u]: smbpid = %u, tid = %u, pid = %u, ",
			i,
			(unsigned int)pls->context.smbpid,
			(unsigned int)pls->context.tid,
			(unsigned int)procid_to_pid(&pls->context.pid) ));
	
	DEBUG10(("start = %.0f, size = %.0f, fnum = %d, %s %s\n",
		(double)pls->start,
		(double)pls->size,
		pls->fnum,
...
void brl_init(int read_only)
{
	...
	if (!tdb) {
		DEBUG0(("Failed to open byte range locking database %s\n",
	...
	if (signal_pending_read) {
		...
		for (i=0; i < br_lck->num_locks; i++) {
			...
			if (pend_lock->lock_type == PENDING_READ_LOCK &&
				...
				DEBUG10(("brl_lock_posix: sending unlock message to pid %s\n",
...
#if !ZERO_ZERO
	if (start == 0 && size == 0) {
		DEBUG0(("client sent 0/0 lock - please report this\n"));
	}
#endif
  ...
  unlock_continue:
	...
	for (j=0; j < br_lck->num_locks; j++) {
		...
		if (brl_pending_overlap(plock, pend_lock)) {
			DEBUG10(("brl_unlock: sending unlock message to pid %s\n",
				procid_str_static(&pend_lock->context.pid )));
...
static BOOL brl_unlock_posix(struct byte_range_lock *br_lck, const struct lock_struct *plock)
{
	...
	if (plock->start + plock->size < plock->start ||
			plock->start + plock->size < plock->size) {
		DEBUG10(("brl_unlock_posix: lock wrap\n"));
		return False;
	}
	...
	if (!tp) {
		DEBUG10(("brl_unlock_posix: malloc fail\n"));
		return False;
	}
	...
	if (!overlap_found) {
		...
		SAFE_FREE(tp);
		DEBUG10(("brl_unlock_posix: No overlap - unlocked.\n"));
		return True;
	}
	...
	if (count) {
		tp = (struct lock_struct *)SMB_REALLOC(tp, count * sizeof(*locks));
		if (!tp) {
			DEBUG10(("brl_unlock_posix: realloc fail\n"));
			return False;
		}
	} else {
		...
		enum brl_flavour lock_flav)
{
	...
	if(lp_posix_locking(fsp->conn->params) && (lock_flav == WINDOWS_LOCK)) {
		...
		DEBUG10(("brl_locktest: posix start=%.0f len=%.0f %s for fnum %d file %s\n",
		...
		enum brl_flavour lock_flav)
{
	...
	if(lp_posix_locking(fsp->conn->params)) {
		...
		DEBUG10(("brl_lockquery: posix start=%.0f len=%.0f %s for fnum %d file %s\n",
					...
					files_struct *fsp, BOOL read_only)
{
	...
	if (read_only) {
		...
		if (tdb_chainlock(tdb, key) != 0) {
			DEBUG3( ("Could not lock byte range lock entry\n"));
			TALLOC_FREE(br_lck);
			return NULL;
		}
	...
	if (DEBUGLEVEL >= 10) {
		unsigned int i;
		struct lock_struct *locks = (struct lock_struct *)br_lck->lock_data;
		DEBUG10(("brl_get_locks_internal: %u current locks on dev=%.0f, inode=%.0f\n",
			br_lck->num_locks,
