		...
		enum brl_type lock_type)
{
	...
	if (strict_locking == Auto) {
		if  (EXCLUSIVE_OPLOCK_TYPE(fsp->oplock_type) && (lock_type == READ_LOCK || lock_type == WRITE_LOCK)) {
			DEBUG10(("is_locked: optimisation - exclusive oplock on file %s\n", fsp->fsp_name ));
			ret = False;
		} else if ((fsp->oplock_type == LEVEL_II_OPLOCK) &&
			   (lock_type == READ_LOCK)) {
			DEBUG10(("is_locked: optimisation - level II oplock on file %s\n", fsp->fsp_name ));
			ret = False;
		} else {
	...
	DEBUG10(("is_locked: flavour = %s brl start=%.0f len=%.0f %s for fnum %d file %s\n",
			...
			uint32 *plock_pid)
{
	...
	DEBUG10(("do_lock: lock flavour %s lock type %s start=%.0f len=%.0f requested for fnum %d file %s\n",
			...
			enum brl_flavour lock_flav)
{
	...
	DEBUG10(("do_unlock: unlock start=%.0f len=%.0f requested for fnum %d file %s\n",
		  (double)offset, (double)count, fsp->fnum, fsp->fsp_name ));
	...
	if (!ok) {
		DEBUG10(("do_unlock: returning ERRlock.\n" ));
			...
			enum brl_flavour lock_flav)
{
	...
	DEBUG10(("do_lock_cancel: cancel start=%.0f len=%.0f requested for fnum %d file %s\n",
		  (double)offset, (double)count, fsp->fnum, fsp->fsp_name ));
	...
	if (!ok) {
		DEBUG10(("do_lock_cancel: returning ERRcancelviolation.\n" ));
		return NT_STATUS_DOS(ERRDOS, ERRcancelviolation);
	}
...
BOOL locking_init(int read_only)
{
	...
	if (!tdb) {
		DEBUG0(("ERROR: Failed to initialise locking database\n"));
		return False;
	}
...
static void print_share_mode_table(struct locking_data *data)
{
	...
	for (i = 0; i < num_share_modes; i++) {
		...
		DEBUG10(("print_share_mode_table: %s\n",
...
static BOOL parse_share_modes(TDB_DATA dbuf, struct share_mode_lock *lck)
{
	...
	DEBUG10( ("parse_share_modes: delete_on_close: %d, "
		   "num_share_modes: %d\n",
		lck->delete_on_close,
		lck->num_share_modes));

	if ((lck->num_share_modes < 0) || (lck->num_share_modes > 1000000)) {
		DEBUG0( ("invalid number of share modes: %d\n",
			  lck->num_share_modes));
	...
	if (data->u.s.delete_token_size) {
		...
		if ((data->u.s.delete_token_size < sizeof(uid_t) + sizeof(gid_t)) ||
			...
			DEBUG0( ("parse_share_modes: invalid token size %d\n",
	...
	for (i = 0; i < lck->num_share_modes; i++) {
		...
		DEBUG10(("parse_share_modes: %s\n",
			  share_mode_str(i, entry_p) ));
		if (!process_exists(entry_p->pid)) {
			DEBUG10(("parse_share_modes: deleted %s\n",
				  share_mode_str(i, entry_p) ));
...
static TDB_DATA unparse_share_modes(struct share_mode_lock *lck)
{
	...
	DEBUG10( ("unparse_share_modes: del: %d, tok = %u, num: %d\n",
						...
						const char *fname)
{
	...
	if (tdb_chainlock(tdb, key) != 0) {
		DEBUG3( ("Could not lock share entry\n"));
		TALLOC_FREE(lck);
		return NULL;
	}
	...
	if (lck->fresh) {
		...
		if (!parse_share_modes(data, lck)) {
			DEBUG0( ("Could not parse share modes\n"));
			TALLOC_FREE(lck);
			SAFE_FREE(data.dptr);
			return NULL;
		}
			...
			const char *newname)
{
	...
	DEBUG10( ("rename_share_filename: servicepath %s newname %s\n",
		servicepath, newname));
	...
	if (lck->filename == NULL || lck->servicepath == NULL) {
		DEBUG0( ("rename_share_filename: talloc failed\n"));
		return False;
	}
	lck->modified = True;
	...
	DEBUG10(("rename_share_filename: msg_len = %u\n", (unsigned int)msg_len ));
	...
	for (i=0; i<lck->num_share_modes; i++) {
		...
		DEBUG10(("rename_share_filename: sending rename message to pid %s "
				 ...
				 uint32 dosmode)
{
	...
	if ((dosmode & aRONLY) &&
		...
		DEBUG10(("can_set_delete_on_close: file %s delete on close "
	...
	if (!CAN_WRITE(fsp->conn)) {
		DEBUG10(("can_set_delete_on_close: file %s delete on "
...
BOOL set_delete_on_close(files_struct *fsp, BOOL delete_on_close, UNIX_USER_TOKEN *tok)
{
	...
	DEBUG10(("set_delete_on_close: %s delete on close flag for "
		  "fnum = %d, file %s\n",
