					 ...
					 const char *snapin_guid)
{
	...
	DEBUG10(("process_extension_with_snapin: no snapin handler for extension %s (%s) found\n", 
		extension_guid, snapin_guid));
...
ADS_STATUS gpo_snapin_handler_none(ADS_STRUCT *ads, 
				   ...
				   const char *snapin_guid)
{
	DEBUG10(("gpo_snapin_handler_none\n"));
...
ADS_STATUS gpo_snapin_handler_security_settings(ADS_STRUCT *ads, 
						...
						const char *snapin_guid)
{
	DEBUG10(("gpo_snapin_handler_security_settings\n"));
			   ...
			   struct cli_state **cli_out)
{
	...
	if (!NT_STATUS_IS_OK(result) && 
		...
		DEBUG10(("check_refresh_gpo: failed to get local gpt version: %s\n", 
			nt_errstr(result)));
		goto out;
	}
	...
	while (gpo->version > sysvol_gpt_version) {
		...
		DEBUG1(("check_refresh_gpo: need to refresh GPO\n"));
		...
		if (*cli_out == NULL) {
			...
			result = cli_full_connection(&cli, global_myname(), 
						     ...
						     Undefined, NULL);
			if (!NT_STATUS_IS_OK(result)) {
				DEBUG10(("check_refresh_gpo: failed to connect: %s\n", nt_errstr(result)));
				goto out;
			}
