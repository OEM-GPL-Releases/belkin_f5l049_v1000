...
static BOOL gpo_sync_files(struct sync_context *ctx)
{
	...
	if (cli_list(ctx->cli, ctx->mask, ctx->attribute, gpo_sync_func, ctx) == -1) {
		DEBUG1(("listing [%s] failed with error: %s\n", 
			  ...
			  void *state)
{
	...
	DEBUG5(("gpo_sync_func: got mask: [%s], name: [%s]\n", 
		mask, info->name));
