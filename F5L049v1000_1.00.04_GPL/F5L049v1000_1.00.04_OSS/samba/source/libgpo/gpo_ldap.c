			    ...
			    struct GP_LINK *gp_link)
{
	...
	DEBUG10(("ads_parse_gplink: gPLink: %s\n", gp_link_raw));
			    ...
			    struct GP_LINK *gp_link_struct)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG10(("ads_get_gpo_link: search failed with %s\n", ads_errstr(status)));
		return status;
	}

	if (ads_count_replies(ads, res) != 1) {
		DEBUG10(("ads_get_gpo_link: no result\n"));
		ads_msgfree(ads, res);
	...
	if (gp_link == NULL) {
		DEBUG10(("ads_get_gpo_link: no 'gPLink' attribute found\n"));
		ads_msgfree(ads, res);
	...
	if (!ads_pull_uint32(ads, res, "gPOptions", &gp_options)) {
		DEBUG10(("ads_get_gpo_link: no 'gPOptions' attribute found\n"));
		gp_options = 0;
	}
			    ...
			    uint32 gpo_opt)
{
	...
	status = ads_search_dn(ads, &res, link_dn, attrs);
	if (!ADS_ERR_OK(status)) {
		DEBUG10(("ads_add_gpo_link: search failed with %s\n", ads_errstr(status)));
		return status;
	}

	if (ads_count_replies(ads, res) != 1) {
		DEBUG10(("ads_add_gpo_link: no result\n"));
		ads_msgfree(ads, res);
		return ADS_ERROR(LDAP_NO_SUCH_OBJECT);
	}
			       ...
			       const char *gpo_dn)
{
	...
	if (gpo_dn[0] != '[') {
		DEBUG10(("ads_delete_gpo_link: first char not: [\n"));
		return ADS_ERROR(LDAP_INVALID_DN_SYNTAX);
	}
		
	if (gpo_dn[strlen(gpo_dn)] != ']') {
		DEBUG10(("ads_delete_gpo_link: last char not: ]\n"));
		return ADS_ERROR(LDAP_INVALID_DN_SYNTAX);
	}

	status = ads_search_dn(ads, &res, link_dn, attrs);
	if (!ADS_ERR_OK(status)) {
		DEBUG10(("ads_delete_gpo_link: search failed with %s\n", ads_errstr(status)));
		return status;
	}

	if (ads_count_replies(ads, res) != 1) {
		DEBUG10(("ads_delete_gpo_link: no result\n"));
		ads_msgfree(ads, res);
		return ADS_ERROR(LDAP_NO_SUCH_OBJECT);
	}
		       ...
		       struct GROUP_POLICY_OBJECT *gpo)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG10(("ads_get_gpo: search failed with %s\n", ads_errstr(status)));
		return status;
	}

	if (ads_count_replies(ads, res) != 1) {
		DEBUG10(("ads_get_gpo: no result\n"));
		ads_msgfree(ads, res);
		return ADS_ERROR(LDAP_NO_SUCH_OBJECT);
	}
	...
	for (i = 0; i < gp_link->num_links; i++) {
		...
		if (only_add_forced_gpos) {
			...
			if (! (gp_link->link_opts[i] & GPO_LINK_OPT_ENFORCED)) {
				DEBUG10(("skipping nonenforced GPO link because GPOPTIONS_BLOCK_INHERITANCE has been set\n"));
				continue;
			} else {
				DEBUG10(("adding enforced GPO link although the GPOPTIONS_BLOCK_INHERITANCE has been set\n"));
			}
		}
		...
		DEBUG10(("add_gplink_to_gplist: added GPLINK #%d %s to GPO list\n", 
			    ...
			    struct GROUP_POLICY_OBJECT **gpo_list)
{
	...
	DEBUG10(("ads_get_gpo_list: getting GPO list for [%s]\n", dn));
	...
	if (flags & GPO_LIST_FLAG_MACHINE) {
		...
		DEBUG10(("ads_get_gpo_list: query SITE: [%s] for GPOs\n", site_dn));
	...
	while ( (parent_dn = ads_parent_dn(tmp_dn)) && 
		...
		if (strncmp(parent_dn, "DC=", strlen("DC=")) == 0) {
			...
			DEBUG10(("ads_get_gpo_list: query DC: [%s] for GPOs\n", parent_dn));
	...
	while ( (parent_dn = ads_parent_dn(tmp_dn)) && 
		...
		if (strncmp(parent_dn, "OU=", strlen("OU=")) == 0) {
			...
			DEBUG10(("ads_get_gpo_list: query OU: [%s] for GPOs\n", parent_dn));
