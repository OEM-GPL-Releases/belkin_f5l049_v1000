...
static void display_finfo(file_info *finfo)
{
	if (do_this_one(finfo)) {
		...
		if (!showacls) {
			...
			if (fnum == -1) {
				DEBUG0( ("display_finfo() Failed to open %s: %s\n",
					afname,
				...
				if (!sd) {
					DEBUG0( ("display_finfo() failed to "
...
static void adjust_do_list_queue(void)
{
	...
	if (do_list_queue_start == do_list_queue_end) {
		...
		*do_list_queue = '\0';
		...
		DEBUG4(("sliding do_list_queue backward\n"));
...
static void add_to_do_list_queue(const char* entry)
{
	...
	while (new_end > do_list_queue_size) {
		...
		DEBUG4(("enlarging do_list_queue to %d\n",
			 (int)do_list_queue_size));
	...
	if (do_list_queue) {
		...
		DEBUG4(("added %s to do_list_queue (start=%d, end=%d)\n",
...
static void remove_do_list_queue_head(void)
{
	if (do_list_queue_end > do_list_queue_start) {
		...
		DEBUG4(("removed head of do_list_queue (start=%d, end=%d)\n",
...
{  
	...
	DEBUG1(("getting file %s of size %.0f as %s ", 
...

	{
		...
		DEBUG1(("(%3.1f kb/s) (average %3.1f kb/s)\n",
...
static int do_put(char *rname, char *lname, BOOL reput)
{
	...
	DEBUG1(("putting file %s as %s ",lname,
		 rname));
...
{
	lowercase = !lowercase;
	DEBUG2(("filename lowercasing is now %s\n",lowercase?"on":"off"));
...
static int cmd_setcase(void)
{
	...
	DEBUG2(("filename case sensitivity is now %s\n",!orig_case_sensitive ?
		"on":"off"));
...
{
	showacls = !showacls;
	DEBUG2(("showacls is now %s\n",showacls?"on":"off"));

	if (!ctx && showacls)
		ctx = talloc_init("smbclient:showacls");
		if (!ctx) {
			DEBUG0( ("cmd_showacls() out of memory.  talloc_init() failed.\n"));
	}
...
{
	recurse = !recurse;
	DEBUG2(("directory recursion is now %s\n",recurse?"on":"off"));
...
static int cmd_translate(void)
{
	...
	DEBUG2(("CR/LF<->LF and print text translation now %s\n",
		 translation?"on":"off"));
...
static int cmd_lcd(void)
{
	...
	if (next_token_nr(NULL,buf,NULL,sizeof(buf)))
		chdir(buf);
	DEBUG2(("the local directory is now %s\n",sys_getwd(d)));
...
static BOOL browse_host_rpc(BOOL sort)
{
	...
	if (pipe_hnd == NULL) {
		DEBUG10( ("Could not connect to srvsvc pipe: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(mem_ctx);
		return False;
	}
	...
	*/
	if (FD_ISSET(cli->fd,&fds)) {
		if (!receive_smb(cli->fd,cli->inbuf,cli->bufsize,0)) {
			DEBUG0( ("Read from server failed, maybe it closed the "
				"connection\n"));
			return;
		}
		goto again;
 ...
 int main(int argc,char *argv[])
{
	...
	*base_directory = 0;
	...
	DEBUG3(("Client started (version %s).\n", SAMBA_VERSION_STRING));
