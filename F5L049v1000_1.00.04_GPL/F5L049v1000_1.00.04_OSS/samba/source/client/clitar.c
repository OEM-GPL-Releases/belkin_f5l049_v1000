...
static char *string_create_s(int size)
{
	...
	if (tmp == NULL) {
		DEBUG0( ("Out of memory in string_create_s\n"));
	}
			   ...
			   const char *amode, unsigned char ftype)
{
	...
	DEBUG5( ("WriteTarHdr, Type = %c, Size= %.0f, Name = %s\n", ftype, (double)size, aname));
	...
	if (l+2 >= NAMSIZ) {
		...
		i = strlen(b)+1;
		DEBUG5( ("File name in tar file: %s, size=%d, \n", b, (int)strlen(b)));
...
static long readtarheader(union hblock *hb, file_info2 *finfo, char *prefix)
{
	...
	if ((hb->dbuf.linkflag != '0') && (hb -> dbuf.linkflag != '5')) {
		if (hb->dbuf.linkflag == 0) {
			DEBUG6( ("Warning: NULL link flag (gnu tar archive ?) %s\n",
				finfo->name));
		} else { 
			if (hb -> dbuf.linkflag == 'L') { /* We have a longlink */
				...
				DEBUG0( ("this tar file appears to contain some kind \
...
static void dotareof(int f)
{
	...
	if (sys_fstat(f, &stbuf) == -1) {
		DEBUG0( ("Couldn't stat file handle\n"));
		return;
	}
	...
	while (p) {
		...
		if (!cli_chkpath(cli, partpath)) {
			if (!cli_mkdir(cli, partpath)) {
				DEBUG0( ("Error mkdirhiering\n"));
				return False;
			} else {
				DEBUG3( ("mkdirhiering %s\n", partpath));
			}
		}
...
static void do_atar(char *rname,char *lname,file_info *finfo1)
{
	...
	if (dry_run) {
		DEBUG3(("skipping file %s of size %12.0f bytes\n", finfo.name,
				(double)finfo.size));
		shallitime=0;
	...
	if (fnum == -1) {
		DEBUG0(("%s opening remote file %s (%s)\n",
	...
	if (finfo.name == NULL) {
		DEBUG0( ("Unable to allocate space for finfo.name in do_atar\n"));
		return;
	}
	...
	DEBUG3(("file %s attrib 0x%X\n",finfo.name,finfo.mode));

	if (tar_inc && !(finfo.mode & aARCH)) {
		DEBUG4( ("skipping %s - archive bit not set\n", finfo.name));
		shallitime=0;
	} else if (!tar_system && (finfo.mode & aSYSTEM)) {
		DEBUG4( ("skipping %s - system bit is set\n", finfo.name));
		shallitime=0;
	} else if (!tar_hidden && (finfo.mode & aHIDDEN)) {
		DEBUG4( ("skipping %s - hidden bit is set\n", finfo.name));
		shallitime=0;
	} else {
		BOOL wrote_tar_header = False;

		DEBUG3(("getting file %s of size %.0f bytes as a tar file %s",
		...
		do {
			...
			*/
			...
			if (nread > finfo.size) {
				...
				DEBUG0(("File size change - truncating %s to %.0f bytes\n",
			...
			* write out in 512 byte intervals */
			...
			if (dotarbuf(tarhandle,data,datalen) != datalen) {
				DEBUG0(("Error writing to tar file - %s\n", strerror(errno)));
				break;
			}
	      
			if ( (datalen == 0) && (finfo.size != 0) ) {
				DEBUG0(("Error reading file %s. Got 0 bytes\n", rname));
				break;
			}
		...
		if (wrote_tar_header) {
			...
			if (nread < finfo.size) {
				DEBUG0( ("Didn't get entire file. size=%.0f, nread=%d\n",
							(double)finfo.size, (int)nread));
				if (padit(data, (SMB_BIG_UINT)sizeof(data), finfo.size - nread))
					DEBUG0(("Error writing tar file - %s\n", strerror(errno)));
			}
			...
			DEBUG4( ("skipping %s - initial read failed (file was locked ?)\n", finfo.name));
			shallitime=0;
		}
	}
...
static void do_tar(file_info *finfo)
{
	...
	if (finfo->mode & aDIR) {
		...
		DEBUG5( ("Sizeof(cur_dir)=%d, strlen(cur_dir)=%d, \
strlen(finfo->name)=%d\nname=%s,cur_dir=%s\n",
		...
		pstrcat(mtar_mask,"*");
		DEBUG5( ("Doing list with mtar_mask: %s\n", mtar_mask));
...
static void unfixtarname(char *tptr, char *fp, int l, BOOL first)
{
	...
	DEBUG5( ("firstb=%lX, secondb=%lX, len=%i\n", (long)tptr, (long)fp, l));
...
static int next_block(char *ltarbuf, char **bufferp, int bufsiz)
{
	...
	DEBUG5( ("Advancing to next block: %0lx\n", (unsigned long)*bufferp));
	*bufferp += TBLOCK;
	total = TBLOCK;

	if (*bufferp >= (ltarbuf + bufsiz)) {

		DEBUG5( ("Reading more data into ltarbuf ...\n"));
...
static int skip_file(int skipsize)
{
	...
	DEBUG5( ("Skiping file. Size = %i\n", skipsize));

	/* FIXME, we should skip more than one block at a time */

	while (dsize > 0) {
		if (next_block(tarbuf, &buffer_p, tbufsiz) <= 0) {
			DEBUG0( ("Empty file, short tar file, or read error: %s\n", strerror(errno)));
			return(False);
		}
		dsize -= TBLOCK;
	}
...
static int get_file(file_info2 finfo)
{
	...
	while (rsize > 0) {
		...
		DEBUG5( ("writing %i bytes, bpos = %i ...\n", dsize, bpos));

		if (cli_write(cli, fnum, 0, buffer_p + bpos, pos, dsize) != dsize) {
			DEBUG0( ("Error writing remote file\n"));
			return 0;
		}
	...
	if (!cli_close(cli, fnum)) {
		DEBUG0( ("Error closing remote file\n"));
		return(False);
	}

	/* Now we update the creation date ... */
	DEBUG5( ("Updating creation date on %s\n", finfo.name));

	if (!cli_setatr(cli, finfo.name, finfo.mode, finfo.mtime_ts.tv_sec)) {
		if (tar_real_noisy) {
			DEBUG0( ("Could not set time on file: %s\n", finfo.name));
			/*return(False); */ /* Ignore, as Win95 does not allow changes */
		}
	}

	ntarf++;
	DEBUG0( ("restore tar file %s of size %.0f bytes\n", finfo.name, (double)finfo.size));
	return(True);
}
...
{
	DEBUG0( ("restore directory %s\n", finfo.name));

	if (!ensurepath(finfo.name)) {
		DEBUG0( ("Problems creating directory\n"));
		return(False);
	}
	ntarf++;
	return(True);
...
static char *get_longfilename(file_info2 finfo)
{
	...
	DEBUG5( ("Restoring a long file name: %s\n", finfo.name));
	DEBUG5( ("Len = %.0f\n", (double)finfo.size));

	if (longname == NULL) {
		DEBUG0( ("could not allocate buffer of size %d for longname\n", namesize));
		return(NULL);
	}
...
static void do_tarput(void)
{
	...
	while (True) {
		...
		DEBUG5( ("Reading the next header ...\n"));
		...
		switch (readtarheader((union hblock *) buffer_p, &finfo, cur_dir)) {
			case -2:    /* Hmm, not good, but not fatal */
				...
				if ((next_block(tarbuf, &buffer_p, tbufsiz) <= 0) && !skip_file(finfo.size)) {
					DEBUG0( ("Short file, bailing out...\n"));
					return;
				}
				break;

			case -1:
				DEBUG0( ("abandoning restore, -1 from read tar header\n"));
				return;

			case 0: /* chksum is zero - looks like an EOF */
				DEBUG0( ("tar: restored %d files and directories\n", ntarf));
		...
		DEBUG5( ("Skip = %i, cliplist=%s, file=%s\n", skip, (cliplist?cliplist[0]:NULL), finfo.name));
		if (skip) {
...
int cmd_block(void)
{
	...
	if (block < 0 || block > 65535) {
		DEBUG0( ("blocksize out of range"));
		return 1;
	}

	blocksize=block;
	DEBUG2(("blocksize is now %d\n", blocksize));
...
int cmd_tarmode(void)
{
	...
	while (next_token_nr(NULL,buf,NULL,sizeof(buf))) {
		if (strequal(buf, "full"))
			...
			DEBUG0( ("tarmode: unrecognised option %s\n", buf));
	}

	DEBUG0( ("tarmode is now %s, %s, %s, %s, %s\n",
		...
		while(*q) {
			switch (*q++) {
				...
				case 'a':
					...
					break;
				default:
					DEBUG0( ("setmode <filename> <perm=[+|-]rsha>\n"));
					return 1;
			}
		}
	}
...
int cmd_tar(void)
{
	...
	if (!next_token_nr(NULL,buf,NULL,sizeof(buf))) {
		DEBUG0(("tar <c|x>[IXbgan] <filename>\n"));
		return 1;
	}
	...
	switch(tar_type) {
		...
		case 'c':
			if (clipn && tar_excl) {
				...
				for (i=0; i<clipn; i++) {
					...
					if (strrchr_m(cliplist[i], '\\')) {
						...
						*(strrchr_m(cur_dir, '\\')+1)='\0';
						...
						DEBUG5( ("process_tar, do_list with tarmac: %s\n", tarmac));
				...
				DEBUG5( ("process_tar, do_list with mask: %s\n", mask));
				pstrcat(mask,"\\*");
			...
			DEBUG0( ("tar: dumped %d files and directories\n", ntarf));
			DEBUG0( ("Total bytes written: %.0f\n", (double)ttarf));
			break;
	}
...
static int read_inclusion_file(char *filename)
{
	...
	if ((inclusion = x_fopen(filename, O_RDONLY, 0)) == NULL) {
		 ...
		 */
		DEBUG0(("Unable to open inclusion file %s\n", filename));
		return 0;
	}
	...
	while ((! error) && (x_fgets(buf, sizeof(buf)-1, inclusion))) {
		if (inclusion_buffer == NULL) {
			...
			if ((inclusion_buffer = (char *)SMB_MALLOC(inclusion_buffer_size)) == NULL) {
				DEBUG0(("failure allocating buffer to read inclusion file\n"));
				error = 1;
				break;
			}
		}
		...
		if ((strlen(buf) + 1 + inclusion_buffer_sofar) >= inclusion_buffer_size) {
			...
			if (!inclusion_buffer) {
				DEBUG0(("failure enlarging inclusion buffer to %d bytes\n",
	...
	if (! error) {
		...
		if (cliplist == NULL) {
			DEBUG0(("failure allocating memory for cliplist\n"));
			error = 1;
		} else {
			cliplist[clipn] = NULL;
			p = inclusion_buffer;
			for (i = 0; (! error) && (i < clipn); i++) {
				...
				if ((tmpstr = (char *)SMB_MALLOC(strlen(p)+1)) == NULL) {
					DEBUG0( ("Could not allocate space for a cliplist item, # %i\n", i));
					error = 1;
				} else {
					unfixtarname(tmpstr, p, strlen(p) + 1, True);
					cliplist[i] = tmpstr;
					if ((p = strchr_m(p, '\000')) == NULL) {
						DEBUG0(("INTERNAL ERROR: inclusion_buffer is of unexpected contents.\n"));
						abort();
					}
				}
				++p;
...
int tar_parseargs(int argc, char *argv[], const char *Optarg, int Optind)
{
		...
		switch(*Optarg++) {
			...
			case 'b':
				if (Optind>=argc || !(blocksize=atoi(argv[Optind]))) {
					DEBUG0(("Option b must be followed by valid blocksize\n"));
					return 0;
				} else {
					Optind++;
					newOptind++;
			...
			case 'N':
				if (Optind>=argc) {
					DEBUG0(("Option N must be followed by valid file name\n"));
					return 0;
				} else {
					SMB_STRUCT_STAT stbuf;
					...
					if (sys_stat(argv[Optind], &stbuf) == 0) {
						...
						DEBUG1(("Getting files newer than %s",
							time_to_asc(newer_than)));
						newOptind++;
						Optind++;
					} else {
						DEBUG0(("Error setting newer-than time\n"));
						return 0;
					}
				}
				break;
			...
			case 'I':
				if (tar_clipfl) {
					DEBUG0(("Only one of I,X,F must be specified\n"));
					return 0;
				}
				tar_clipfl='I';
				break;
			...
			case 'n':
				if (tar_type == 'c') {
					...
					DEBUG0( ("n is only meaningful when creating a tar-file\n"));
					return 0;
				}
				break;
			default:
				DEBUG0(("Unknown tar option\n"));
				return 0;
		}
	}
	...
	if (tar_clipfl=='F') {
		if (argc-Optind-1 != 1) {
			DEBUG0(("Option F must be followed by exactly one filename.\n"));
			return 0;
		}
		newOptind++;
		...
		if ((tmplist=SMB_MALLOC_ARRAY(char *,clipn)) == NULL) {
			DEBUG0( ("Could not allocate space to process cliplist, count = %i\n", clipn));
			return 0;
		}

		for (clipcount = 0; clipcount < clipn; clipcount++) {

			DEBUG5( ("Processing an item, %s\n", cliplist[clipcount]));
			...
			DEBUG5( ("Processed an item, %s\n", tmpstr));

			DEBUG5( ("Cliplist is: %s\n", cliplist[0]));
		}
	...
	if (Optind>=argc || !strcmp(argv[Optind], "-")) {
		...
		if (!argv[Optind]) {
			DEBUG0(("Must specify tar filename\n"));
			return 0;
		}
		...
		if (tar_type=='c' && dry_run) {
			...
			DEBUG0(("Error opening local file %s - %s\n", argv[Optind], strerror(errno)));
			return(0);
		}
		newOptind++;
	}
