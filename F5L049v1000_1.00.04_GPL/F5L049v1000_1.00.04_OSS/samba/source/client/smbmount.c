...
static struct cli_state *do_connection(char *the_service)
{
 ...
 again:
	...
	if (!(c=cli_initialise()) || (cli_set_port(c, smb_port) != smb_port) ||
		...
		DEBUG0(("%d: Connection to %s failed\n", sys_getpid(), server_n));
		if (c) {
			cli_shutdown(c);
		}
		return NULL;
	...
	if (!cli_session_request(c, &calling, &called)) {
		char *p;
		DEBUG0(("%d: session request to %s failed (%s)\n", 
	...
	DEBUG4(("%d: session request ok\n", sys_getpid()));

	if (!cli_negprot(c)) {
		DEBUG0(("%d: protocol negotiation failed\n", sys_getpid()));
		cli_shutdown(c);
		return NULL;
	}
	...
	if (!NT_STATUS_IS_OK(cli_session_setup(c, username, 
		...
		if (password[0] || !username[0] ||
			...
			DEBUG0(("%d: session setup failed: %s\n",
				sys_getpid(), cli_errstr(c)));
			cli_shutdown(c);
			return NULL;
		}
		DEBUG0(("Anonymous login successful\n"));
	}

	DEBUG4(("%d: session setup ok\n", sys_getpid()));

	if (!cli_send_tconX(c, share, "?????",
			    password, strlen(password)+1)) {
		DEBUG0(("%d: tree connect failed: %s\n",
			 sys_getpid(), cli_errstr(c)));
		cli_shutdown(c);
		return NULL;
	}
...
static void smb_umount(char *mount_point)
{
	...
	*/
        if (umount(mount_point) != 0) {
                DEBUG0(("%d: Could not umount %s: %s\n",
			 sys_getpid(), mount_point, strerror(errno)));
                return;
        }

        if ((fd = open(MOUNTED"~", O_RDWR|O_CREAT|O_EXCL, 0600)) == -1) {
                DEBUG0(("%d: Can't get "MOUNTED"~ lock file", sys_getpid()));
                return;
        }
...
#define MOUNTED_TMP MOUNTED".tmp"
        ...
        if ((new_mtab = setmntent(MOUNTED_TMP, "w")) == NULL) {
                DEBUG0(("%d: Can't open " MOUNTED_TMP ": %s\n",
			 sys_getpid(), strerror(errno)));
                endmntent(mtab);
                return;
        }
        ...
        if (fchmod (fileno (new_mtab), S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH) < 0) {
                DEBUG0(("%d: Error changing mode of %s: %s\n",
        ...
        if (rename(MOUNTED_TMP, MOUNTED) < 0) {
                DEBUG0(("%d: Cannot rename %s to %s: %s\n",
			 sys_getpid(), MOUNTED, MOUNTED_TMP, strerror(errno)));
                return;
        }

        if (unlink(MOUNTED"~") == -1) {
                DEBUG0(("%d: Can't remove "MOUNTED"~", sys_getpid()));
                return;
        }
}
...
static void send_fs_socket(char *the_service, char *mount_point, struct cli_state *c)
{
	...
	while (1) {
		if ((fd = open(mount_point, O_RDONLY)) < 0) {
			DEBUG0(("mount.smbfs[%d]: can't open %s\n",
		...
		if (res != 0) {
			DEBUG0(("mount.smbfs[%d]: ioctl failed, res=%d\n",
				 sys_getpid(), res));
			close(fd);
			break;
		}
		...
		if (!closed) {
			...
			if ( (fd = open("/dev/null", O_WRONLY)) < 0)
				DEBUG2(("mount.smbfs: can't open /dev/null\n"));
			close_our_files(fd);
			if (fd >= 0) {
			...
			reopen_logs();
			DEBUG0( ("mount.smbfs: entering daemon mode for service %s, pid=%d\n", the_service, sys_getpid()));
		...
		while (!c) {
			...
			pause();
			DEBUG2(("mount.smbfs[%d]: got signal, getting new socket\n", sys_getpid()));
			c = do_connection(the_service);
		}
	}

	smb_umount(mount_point);
	DEBUG2(("mount.smbfs[%d]: exit\n", sys_getpid()));
	exit(1);
}
...
static void read_credentials_file(char *filename)
{
	...
	if ((auth=sys_fopen(filename, "r")) == NULL)
	{
		...
		DEBUG0(("ERROR: Unable to open credentials file!\n"));
		exit (-1);
	}
 ...
 int main(int argc,char *argv[])
{
	...
	DEBUG3(("mount.smbfs started (version %s)\n", SAMBA_VERSION_STRING));
