				       ...
				       const char *servername)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG1( ("ads_find_printer_on_server: cannot find host %s in ads\n",
			  servername));
		return status;
	}
	...
	for (i=0; map[i].valname; i++) {
		if (StrCaseCmp(map[i].valname, value->valuename) == 0) {
			if (!map[i].fn(ctx, mods, value)) {
				DEBUG5( ("Add of value %s to modlist failed\n", value->valuename));
			} else {
				DEBUG7( ("Mapped value %s\n", value->valuename));
			}
					  ...
					  const char *printer)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG3( ("Unable to open printer %s, error is %s.\n",
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG3( ("Unable to do enumdataex on %s, error is %s.\n",
			  printername, dos_errstr(result)));
	} else {
