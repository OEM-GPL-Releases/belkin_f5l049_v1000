					       ...
					       LDAPMessage **res)
{
	...
	*res = NULL;
	...
	if (ADS_ERR_OK(status)) {
               DEBUG5(("Search for %s in <%s> gave %d replies\n",
	...
	while (--count) {
		...
		*res = NULL;
		...
		DEBUG3(("Reopening ads connection to realm '%s' after error %s\n", 
		...
		if (!ADS_ERR_OK(status)) {
			DEBUG1(("ads_search_retry: failed to reconnect (%s)\n",
				 ads_errstr(status)));
			ads_destroy(&ads);
			SAFE_FREE(bp);
			return status;
		...
		*res = NULL;
		...
		if (ADS_ERR_OK(status)) {
			DEBUG5(("Search for filter: %s, base: %s gave %d replies\n",
				 expr, bp, ads_count_replies(ads, *res)));
			SAFE_FREE(bp);
			return status;
		}
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG1(("ads reopen failed after error %s\n", 
			 ads_errstr(status)));
	}
	return status;
}
