						    ...
						    void *cbdata)
{
	...
	while (count) {
		...
		DEBUG10(("smb_krb5_locator_lookup: got ret: %s (%d)\n", 
	...
	if (ret) {
		DEBUG10(("smb_krb5_locator_lookup: failed to call callback: %s (%d)\n", 
				      ...
				      void **private_data)
{
	...
	DEBUG10(("smb_krb5_locator_init: called\n"));
...
{
	DEBUG10(("smb_krb5_locator_close: called\n"));
					...
					void *cbdata)
{
	...
	DEBUG10(("smb_krb5_locator_lookup: called for\n"));
	...
	if (ret) {
		DEBUG10(("smb_krb5_locator_lookup: returning ret: %s (%d)\n", 
			error_message(ret), ret));
		return ret;
	}
	...
	if (!saf_name || strlen(saf_name) == 0) {
		DEBUG10(("smb_krb5_locator_lookup: no SAF name stored for %s\n", 
			realm));
		goto find_kdc;
	}

	DEBUG10(("smb_krb5_locator_lookup: got %s for %s from SAF cache\n", 
		saf_name, realm));
 ...
 find_kdc:
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG10(("smb_krb5_locator_lookup: got %s (%s)\n",
			nt_errstr(status), 
