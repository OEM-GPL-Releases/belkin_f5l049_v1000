				...
				time_t renewable_time)
{
	...
	DEBUG10(("kerberos_kinit_password: using [%s] as ccache and config [%s]\n",
...
int ads_kinit_password(ADS_STRUCT *ads)
{
	...
	if (ret) {
		DEBUG0(("kerberos_kinit_password %s failed: %s\n", 
			 s, error_message(ret)));
	}
	SAFE_FREE(s);
	return ret;
...
int ads_kdestroy(const char *cc_name)
{
	...
	initialize_krb5_error_table();
	if ((code = krb5_init_context (&ctx))) {
		DEBUG3( ("ads_kdestroy: kdb5_init_context failed: %s\n", 
			error_message(code)));
		return code;
	}
	...
	if (!cc_name) {
		...
		if ((code = krb5_cc_resolve(ctx, cc_name, &cc))) {
			DEBUG3( ("ads_kdestroy: krb5_cc_resolve failed: %s\n",
				  error_message(code)));
			krb5_free_context(ctx);
			return code;
		}
	...
	if ((code = krb5_cc_destroy (ctx, cc))) {
		DEBUG3( ("ads_kdestroy: krb5_cc_destroy failed: %s\n", 
			error_message(code)));
	}
...
BOOL kerberos_secrets_store_des_salt( const char* salt )
{
	...
	if ( (key = des_salt_key()) == NULL ) {
		DEBUG0(("kerberos_secrets_store_des_salt: failed to generate key!\n"));
		return False;
	}

	if ( !salt ) {
		DEBUG8(("kerberos_secrets_store_des_salt: deleting salt\n"));
		secrets_delete( key );
		return True;
	}

	DEBUG3(("kerberos_secrets_store_des_salt: Storing salt \"%s\"\n", salt));
...
char* kerberos_secrets_fetch_des_salt( void )
{
	...
	if ( (key = des_salt_key()) == NULL ) {
		DEBUG0(("kerberos_secrets_fetch_des_salt: failed to generate key!\n"));
		return False;
	}
...
char *kerberos_get_default_realm_from_ccache( void )
{
	...
	DEBUG5(("kerberos_get_default_realm_from_ccache: "
		"Trying to read krb5 cache: %s\n",
		krb5_cc_default_name(ctx)));
	if (krb5_cc_default(ctx, &cc)) {
		DEBUG0(("kerberos_get_default_realm_from_ccache: "
...
static char *get_kdc_ip_string(char *mem_ctx, const char *realm, const char *sitename, struct in_addr primary_ip)
{
	...
	DEBUG10(("get_kdc_ip_string: Returning %s\n",
		kdc_str ));
					...
					const char *sitename, struct in_addr ip)
{
	...
	if ((mkdir(dname, 0755)==-1) && (errno != EEXIST)) {
		DEBUG0(("create_local_private_krb5_conf_for_domain: "
	...
	DEBUG10(("create_local_private_krb5_conf_for_domain: fname = %s, realm = %s, domain = %s\n",
		fname, realm, domain ));
	...
	if (fd == -1) {
		DEBUG0(("create_local_private_krb5_conf_for_domain: smb_mkstemp failed,"
			" for file %s. Errno %s\n",
			tmpname, strerror(errno) ));
	}

	if (fchmod(fd, 0644)==-1) {
		DEBUG0(("create_local_private_krb5_conf_for_domain: fchmod failed for %s."
			" Errno %s\n",
			tmpname, strerror(errno) ));
		unlink(tmpname);
		close(fd);
	...
	if (flen != ret) {
		DEBUG0(("create_local_private_krb5_conf_for_domain: write failed,"
	...
	if (close(fd)==-1) {
		DEBUG0(("create_local_private_krb5_conf_for_domain: close failed."
			" Errno %s\n", strerror(errno) ));
		unlink(tmpname);
		TALLOC_FREE(dname);
		return False;
	...
	if (rename(tmpname, fname) == -1) {
		DEBUG0(("create_local_private_krb5_conf_for_domain: rename "
	...
	DEBUG5(("create_local_private_krb5_conf_for_domain: wrote "
...
#define SYSTEM_KRB5_CONF_PATH "/etc/krb5.conf"
	...
	if (strequal(realm, lp_realm())) {
		...
		if (symlink(fname, SYSTEM_KRB5_CONF_PATH) == -1) {
			if (errno != EEXIST) {
				DEBUG0(("create_local_private_krb5_conf_for_domain: symlink "
					"of %s to %s failed. Errno %s\n",
