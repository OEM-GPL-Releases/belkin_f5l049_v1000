 ...
 LDAP *ldap_open_with_timeout(const char *server, int port, unsigned int to)
{
	...
	if (ldp == NULL) {
		DEBUG2(("Could not open LDAP connection to %s:%d: %s\n",
...
BOOL ads_sitename_match(ADS_STRUCT *ads)
{
	if (ads->config.server_site_name == NULL &&
		...
		DEBUG10(("ads_sitename_match: both null\n"));
		return True;
	}
	if (ads->config.server_site_name &&
	    ...
	    strequal(ads->config.server_site_name,
		     ads->config.client_site_name)) {
		DEBUG10(("ads_sitename_match: name %s match\n", ads->config.server_site_name));
		return True;
	}
	DEBUG10(("ads_sitename_match: no match between server: %s and client: %s\n",
...
BOOL ads_closest_dc(ADS_STRUCT *ads)
{
	if (ads->config.flags & ADS_CLOSEST) {
		DEBUG10(("ads_closest_dc: ADS_CLOSEST flag set\n"));
		return True;
	}

	/* not sure if this can ever happen */
	if (ads_sitename_match(ads)) {
		DEBUG10(("ads_closest_dc: ADS_CLOSEST flag not set but sites match\n"));
		return True;
	}

	if (ads->config.client_site_name == NULL) {
		DEBUG10(("ads_closest_dc: client belongs to no site\n"));
		return True;
	}

	DEBUG10(("ads_closest_dc: %s is not the closest DC\n", 
...
BOOL ads_try_connect(ADS_STRUCT *ads, const char *server )
{
	...
	DEBUG5(("ads_try_connect: sending CLDAP request to %s (realm: %s)\n", 
	...
	if ( !ads_cldap_netlogon( srv, ads->server.realm, &cldap_reply ) ) {
		DEBUG3(("ads_try_connect: CLDAP request %s failed.\n", srv));
		SAFE_FREE( srv );
		return False;
	}
	...
	if ( !(cldap_reply.flags & ADS_LDAP) ) {
		DEBUG1(("ads_try_connect: %s's CLDAP reply says it is not an LDAP server!\n",
			srv));
		SAFE_FREE( srv );
		return False;
	}
...
static NTSTATUS ads_find_dc(ADS_STRUCT *ads)
{
	...
	if ( !c_realm || !*c_realm ) {
		DEBUG0(("ads_find_dc: no realm or workgroup!  Don't know what to do\n"));
	...
	if (!(ads->auth.flags & ADS_AUTH_NO_BIND)) {
		...
		DEBUG6(("ads_find_dc: (ldap) looking for %s '%s'\n",
 ...
 again:
	...
	DEBUG6(("ads_find_dc: (cldap) looking for %s '%s'\n",
		(got_realm ? "realm" : "domain"), realm));
	...
	if (sitename) {
		DEBUG1(("ads_find_dc: failed to find a valid DC on our site (%s), "
...
got_connection:
	DEBUG3(("Connected to LDAP server %s\n", inet_ntoa(ads->ldap_ip)));
					   ...
					   int *count, struct berval **cookie)
{
	...
	*/
	...
	if (rc) {
		DEBUG3(("ads_do_paged_search_args: ldap_search_with_timeout(%s) -> %s\n", expr,
			 ldap_err2string(rc)));
		goto done;
	}
...
#else
	DEBUG0( ("no ldap_add_result_entry() support in LDAP libs!\n"));
			  ...
			  const char **attrs, LDAPMessage **res)
{
	...
	*res = NULL;
	if (!(ctx = talloc_init("ads_do_search"))) {
		DEBUG1(("ads_do_search: talloc_init() failed!"));
		return ADS_ERROR(LDAP_NO_MEMORY);
	}
	...
	if ((push_utf8_talloc(ctx, &utf8_expr, expr) == (size_t)-1) ||
	    (push_utf8_talloc(ctx, &utf8_path, bind_path) == (size_t)-1)) {
		DEBUG1(("ads_do_search: push_utf8_talloc() failed!"));
		rc = LDAP_NO_MEMORY;
		goto done;
	}
	...
	if (!attrs || !(*attrs))
		...
		if (!(str_list_copy(&search_attrs, attrs)))
		{
			DEBUG1(("ads_do_search: str_list_copy() failed!"));
			rc = LDAP_NO_MEMORY;
			goto done;
		}
	}
	...
	if (rc == LDAP_SIZELIMIT_EXCEEDED) {
		DEBUG3(("Warning! sizelimit exceeded in ldap. Truncating.\n"));
		rc = 0;
	}
 ...
 char *ads_get_dn(ADS_STRUCT *ads, LDAPMessage *msg)
{
	...
	if (pull_utf8_allocate(&unix_dn, utf8_dn) == (size_t)-1) {
		DEBUG0(("ads_get_dn: string conversion failure utf8 [%s]\n",
			utf8_dn ));
		return NULL;
	}
...
ADS_STATUS ads_gen_add(ADS_STRUCT *ads, const char *new_dn, ADS_MODLIST mods)
{
	...
	if (push_utf8_allocate(&utf8_dn, new_dn) == -1) {
		DEBUG1( ("ads_gen_add: push_utf8_allocate failed!"));
		return ADS_ERROR_NT(NT_STATUS_NO_MEMORY);
	}
...
ADS_STATUS ads_del_dn(ADS_STRUCT *ads, char *del_dn)
{
	...
	if (push_utf8_allocate(&utf8_dn, del_dn) == -1) {
		DEBUG1( ("ads_del_dn: push_utf8_allocate failed!"));
		return ADS_ERROR_NT(NT_STATUS_NO_MEMORY);
	}
...
char *ads_default_ou_string(ADS_STRUCT *ads, const char *wknguid)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG1(("Failed while searching for: %s\n", base));
		goto out;
	}
...
uint32 ads_get_kvno(ADS_STRUCT *ads, const char *machine_name)
{
	...
	DEBUG5(("ads_get_kvno: Searching for host %s\n", machine_name));
	...
	if (!ADS_ERR_OK(ret) && ads_count_replies(ads, res)) {
		DEBUG1(("ads_get_kvno: Computer Account For %s not found.\n", machine_name));
		ads_msgfree(ads, res);
		return kvno;
	}
	...
	if (!dn_string) {
		DEBUG0(("ads_get_kvno: out of memory.\n"));
		ads_msgfree(ads, res);
		return kvno;
	}
	DEBUG5(("ads_get_kvno: Using: %s\n", dn_string));
	...
	if (!ads_pull_uint32(ads, res, "msDS-KeyVersionNumber", &kvno)) {
		DEBUG3(("ads_get_kvno: Error Determining KVNO!\n"));
		DEBUG3(("ads_get_kvno: Windows 2000 does not support KVNO's, so this may be normal.\n"));
		ads_msgfree(ads, res);
		return kvno;
	}

	/* Success */
	DEBUG5(("ads_get_kvno: Looked Up KVNO of: %d\n", kvno));
	ads_msgfree(ads, res);
	return kvno;
}
...
ADS_STATUS ads_clear_service_principal_names(ADS_STRUCT *ads, const char *machine_name)
{
	...
	if (!ADS_ERR_OK(ret) || ads_count_replies(ads, res) != 1) {
		DEBUG5(("ads_clear_service_principal_names: WARNING: Host Account for %s not found... skipping operation.\n", machine_name));
		DEBUG5(("ads_clear_service_principal_names: WARNING: Service Principals for %s have NOT been cleared.\n", machine_name));
		ads_msgfree(ads, res);
		return ADS_ERROR(LDAP_NO_SUCH_OBJECT);
	}

	DEBUG5(("ads_clear_service_principal_names: Host account for %s found\n", machine_name));
	...
	if (!ADS_ERR_OK(ret)) {
		DEBUG1(("ads_clear_service_principal_names: Error creating strlist.\n"));
		ads_msgfree(ads, res);
		talloc_destroy(ctx);
		return ret;
	}
	...
	ads_memfree(ads,dn_string);
	if (!ADS_ERR_OK(ret)) {
		DEBUG1(("ads_clear_service_principal_names: Error: Updating Service Principals for machine %s in LDAP\n",
			machine_name));
		ads_msgfree(ads, res);
		talloc_destroy(ctx);
		return ret;
                                          ...
                                          const char *my_fqdn, const char *spn)
{
	...
	ret = ads_find_machine_acct(ads, &res, machine_name);
	if (!ADS_ERR_OK(ret) || ads_count_replies(ads, res) != 1) {
		DEBUG1(("ads_add_service_principal_name: WARNING: Host Account for %s not found... skipping operation.\n",
			machine_name));
		DEBUG1(("ads_add_service_principal_name: WARNING: Service Principal '%s/%s@%s' has NOT been added.\n",
			spn, machine_name, ads->config.realm));
		ads_msgfree(ads, res);
		return ADS_ERROR(LDAP_NO_SUCH_OBJECT);
	}

	DEBUG1(("ads_add_service_principal_name: Host account for %s found\n", machine_name));
	...
	DEBUG5(("ads_add_service_principal_name: INFO: Adding %s to host %s\n", 
		psp1, machine_name));
	...
	if (!ADS_ERR_OK(ret)) {
		DEBUG1(("ads_add_service_principal_name: Error: Updating Service Principals in LDAP\n"));
		goto out;
	}
			       ...
			       BOOL *more_strings)
{
		...
		*more_strings = True;
		...
		if (sscanf(&range_attr[strlen(expected_range_attrib)], "%lu-*", 
			...
			*more_strings = False;
			...
			DEBUG1( ("ads_pull_strings_range:  Cannot parse Range attriubte (%s)\n", 
				  range_attr));
	...
	if ((*num_strings) != range_start) {
		DEBUG1( ("ads_pull_strings_range: Range attribute (%s) doesn't start at %u, but at %lu"
	...
	if (*more_strings && ((*num_strings + num_new_strings) != (range_end + 1))) {
		DEBUG1( ("ads_pull_strings_range: Range attribute (%s) tells us we have %lu "
	...
	if (*more_strings) {
		*next_attribute = talloc_asprintf(mem_ctx,
		...
		if (!*next_attribute) {
			DEBUG1( ("talloc_asprintf for next attribute failed!\n"));
			ldap_memfree(range_attr);
			*more_strings = False;
			return NULL;
		}
...
ADS_STATUS ads_current_time(ADS_STRUCT *ads)
{
	...
	if (ads->config.current_time != 0) {
		...
		DEBUG4(("time offset is %d seconds\n", ads->auth.time_offset));
	}
...
ADS_STATUS ads_domain_func_level(ADS_STRUCT *ads, uint32 *val)
{
	...
	*val = DS_DOMAIN_FUNCTION_2000;
	...
	if ( !ads_pull_uint32(ads_s, res, "domainFunctionality", val) ) {
		DEBUG5(("ads_domain_func_level: Failed to pull the domainFunctionality attribute.\n"));
	}
	DEBUG3(("ads_domain_func_level: %d\n", *val));
	...
	*q = '\0';
	...
	case ADS_EXTENDED_DN_HEX_STRING: {
		...
		if (!sid_parse(buf, buf_len, sid)) {
			DEBUG10(("failed to parse sid\n"));
			return False;
		}
		break;
		}
	default:
		DEBUG10(("unknown extended dn format\n"));
		return False;
	}
...
char* ads_get_dnshostname( ADS_STRUCT *ads, TALLOC_CTX *ctx, const char *machine_name )
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG0(("ads_get_dnshostname: Failed to find account for %s\n",
			global_myname()));
		goto out;
	}
		
	if ( (count = ads_count_replies(ads, res)) != 1 ) {
		DEBUG1(("ads_get_dnshostname: %d entries returned!\n", count));
		goto out;
	}
		
	if ( (name = ads_pull_string(ads, ctx, res, "dNSHostName")) == NULL ) {
		DEBUG0(("ads_get_dnshostname: No dNSHostName attribute!\n"));
	}
...
char* ads_get_upn( ADS_STRUCT *ads, TALLOC_CTX *ctx, const char *machine_name )
{
	...
	status = ads_find_machine_acct(ads, &res, global_myname());
	if (!ADS_ERR_OK(status)) {
		DEBUG0(("ads_get_upn: Failed to find account for %s\n",
			global_myname()));
		goto out;
	}
		
	if ( (count = ads_count_replies(ads, res)) != 1 ) {
		DEBUG1(("ads_get_upn: %d entries returned!\n", count));
		goto out;
	}
		
	if ( (name = ads_pull_string(ads, ctx, res, "userPrincipalName")) == NULL ) {
		DEBUG2(("ads_get_upn: No userPrincipalName attribute!\n"));
	}
...
char* ads_get_samaccountname( ADS_STRUCT *ads, TALLOC_CTX *ctx, const char *machine_name )
{
	...
	if ( (name = ads_pull_string(ads, ctx, res, "sAMAccountName")) == NULL ) {
		DEBUG0(("ads_get_dnshostname: No sAMAccountName attribute!\n"));
	}
			...
			uint32 account_type, const char *org_unit)
{
	...
	if (ADS_ERR_OK(status) && ads_count_replies(ads, res) == 1) {
		DEBUG0( ("Host account for %s already exists - deleting old account\n", machine));
		status = ads_leave_realm(ads, machine);
		if (!ADS_ERR_OK(status)) {
			DEBUG0( ("Failed to delete host '%s' from the '%s' realm.\n",
	...
	*/
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG0( ("ads_join_realm: ads_add_machine_acct failed (%s): %s\n", machine, ads_errstr(status)));
		SAFE_FREE(machine);
		return status;
	}
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG0( ("ads_join_realm: Host account test failed for machine %s\n", machine));
		SAFE_FREE(machine);
		return status;
	}
...
ADS_STATUS ads_leave_realm(ADS_STRUCT *ads, const char *hostname)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG0( ("Host account for %s does not exist.\n", host));
		SAFE_FREE(host);
		return status;
	}
	...
	if (rc) {
		DEBUG3(("ldap_delete_ext_s failed with error code %d\n", rc));
	}else {
		DEBUG3(("ldap_delete_ext_s succeeded with error code %d\n", rc));
	}
	...
	if (rc != LDAP_SUCCESS) {
		...
		for (msg_sub = ads_first_entry(ads, res); msg_sub;
			...
			if (!ADS_ERR_OK(status)) {
				DEBUG3(("failed to delete dn %s: %s\n", dn, ads_errstr(status)));
				SAFE_FREE(host);
				ads_memfree(ads, dn);
				ads_memfree(ads, hostnameDN);
				return status;
	...
	status = ads_find_machine_acct(ads, &res, host);
	if (ADS_ERR_OK(status) && ads_count_replies(ads, res) == 1) {
		DEBUG3( ("Failed to remove host account.\n"));
		SAFE_FREE(host);
		return status;
	}
