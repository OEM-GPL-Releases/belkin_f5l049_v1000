					...
					krb5_error_code *perr)
{
	...
	*perr = 0;
	...
	if (ret) {
		DEBUG1( ("ads_keytab_verify_ticket: krb5_kt_default failed (%s)\n", error_message(ret)));
		goto out;
	}
	...
	if (ret) {
		DEBUG1( ("ads_keytab_verify_ticket: krb5_kt_start_seq_get failed (%s)\n", error_message(ret)));
		goto out;
	}
	...
	while (!auth_ok && (krb5_kt_next_entry(context, keytab, &kt_entry, &kt_cursor) == 0)) {
		...
		if (ret) {
			DEBUG1( ("ads_keytab_verify_ticket: smb_krb5_unparse_name failed (%s)\n",
				error_message(ret)));
			goto out;
		}
		...
		for (i = 0; i < ARRAY_SIZE(valid_princ_formats); i++) {
			...
			*pp_tkt = NULL;
			...
			if (ret) {
				DEBUG10(("ads_keytab_verify_ticket: "
				...
				* entries - Guenther */
				...
				DEBUG3(("ads_keytab_verify_ticket: "
  ...
  out:
	...
	if (!auth_ok) {
		if (!number_matched_principals) {
			DEBUG3( ("ads_keytab_verify_ticket: no keytab principals matched expected file service name.\n"));
		} else {
			DEBUG3( ("ads_keytab_verify_ticket: krb5_rd_req failed for all %d matched keytab principals\n",
				number_matched_principals));
		}
	}
						...
						krb5_error_code *perr)
{
	...
	*perr = 0;
	...
	if (!secrets_init()) {
		DEBUG1(("ads_secrets_verify_ticket: secrets_init failed\n"));
	...
	if (!password_s) {
		DEBUG1(("ads_secrets_verify_ticket: failed to fetch machine password\n"));
	...
	for (i=0;enctypes[i];i++) {
		...
		if (!(ret = krb5_rd_req(context, &auth_context, &packet, 
					NULL,
					NULL, NULL, pp_tkt))) {
			DEBUG10(("ads_secrets_verify_ticket: enc type [%u] decrypted message !\n",
			   ...
			   DATA_BLOB *session_key)
{
	...
	*session_key = data_blob(NULL,0);
	...
	if (ret) {
		DEBUG1(("ads_verify_ticket: krb5_init_context failed (%s)\n", error_message(ret)));
	...
	if (ret) {
		DEBUG1(("ads_verify_ticket: krb5_set_default_realm failed (%s)\n", error_message(ret)));
		goto out;
	}
	...
	if (ret) {
		DEBUG1(("ads_verify_ticket: krb5_auth_con_init failed (%s)\n", error_message(ret)));
		goto out;
	}
	...
	if (ret) {
		DEBUG1(("ads_verify_ticket: smb_krb5_parse_name(%s) failed (%s)\n",
	...
	if (!grab_server_mutex("replay cache mutex")) {
		DEBUG1(("ads_verify_ticket: unable to protect replay cache with mutex.\n"));
		ret = KRB5_CC_IO;
		goto out;
	}
	...
	if (ret) {
		DEBUG1(("ads_verify_ticket: krb5_get_server_rcache failed (%s)\n", error_message(ret)));
		goto out;
	}

	ret = krb5_auth_con_setrcache(context, auth_context, rcache);
	if (ret) {
		DEBUG1(("ads_verify_ticket: krb5_auth_con_setrcache failed (%s)\n", error_message(ret)));
		goto out;
	}
	...
	if (!auth_ok) {
		DEBUG3(("ads_verify_ticket: krb5_rd_req with auth failed (%s)\n", 
			 error_message(ret)));
		...
		DEBUG10(("ads_verify_ticket: returning error %s\n",
			nt_errstr(sret) ));
		goto out;
	} 
	...
	if (ret) {
		DEBUG3(("ads_verify_ticket: Failed to generate mutual authentication reply (%s)\n",
			error_message(ret)));
		goto out;
	}
	...
	*ap_rep = data_blob(packet.data, packet.length);
	...
	if (!got_auth_data) {
		DEBUG3(("ads_verify_ticket: did not retrieve auth data. continuing without PAC\n"));
	}
	...
	if (got_auth_data && pac_data != NULL) {
		...
		if (!NT_STATUS_IS_OK(pac_ret)) {
			DEBUG3(("ads_verify_ticket: failed to decode PAC_DATA: %s\n", nt_errstr(pac_ret)));
			*pac_data = NULL;
		}
	...
	if ((ret = smb_krb5_unparse_name(context, client_principal, principal))) {
		DEBUG3(("ads_verify_ticket: smb_krb5_unparse_name failed (%s)\n", 
			 error_message(ret)));
		sret = NT_STATUS_LOGON_FAILURE;
		goto out;
	}
