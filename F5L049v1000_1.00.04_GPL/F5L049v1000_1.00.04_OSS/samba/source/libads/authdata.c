				  ...
				  prs_struct *ps, int depth)
{
	...
	if (info->ptr_res_groups) {
		...
		if (!(info->user_flgs & LOGON_RESOURCE_GROUPS)) {
			DEBUG0(("user_flgs attribute does not have LOGON_RESOURCE_GROUPS\n"));
			/* return False; */
		}
				    ...
				    prs_struct *ps, int depth)
{
	...
	if (hdr->offset != prs_offset(ps)) {
		DEBUG5(("offset in header(x%x) and data(x%x) do not match, correcting\n",
	...
	case PAC_TYPE_SERVER_CHECKSUM:
		DEBUG5( ("PAC_TYPE_SERVER_CHECKSUM\n"));
		if (UNMARSHALLING(ps))
	...
	case PAC_TYPE_PRIVSVR_CHECKSUM:
		DEBUG5( ("PAC_TYPE_PRIVSVR_CHECKSUM\n"));
		if (UNMARSHALLING(ps))
...
#error UNKNOWN_KRB5_KEYUSAGE
	...
	if (ret) {
		DEBUG2(("check_pac_checksum: PAC Verification failed: %s (%d)\n", 
			 ...
			 
{
	...
	*pac_data = NULL;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("decode_pac_data: failed to parse PAC\n"));
		return nt_status;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("decode_pac_data: failed to verify PAC server signature\n"));
		goto out;
	}
	...
	if (!nt_time_equals(&tgs_authtime_nttime, &logon_name->logon_time)) {
		...
		DEBUG2(("decode_pac_data: Logon time mismatch between ticket and PAC!\n"));
	...
	if (!logon_name->len) {
		DEBUG2(("decode_pac_data: No Logon Name available\n"));
		nt_status = NT_STATUS_INVALID_PARAMETER;
		goto out;
	}
	...
	if (ret) {
		DEBUG2(("decode_pac_data: Could not parse name from incoming PAC: [%s]: %s\n", 
	...
	if (!smb_krb5_principal_compare_any_realm(context, client_principal, client_principal_pac)) {
		DEBUG2(("decode_pac_data: Name in PAC [%s] does not match principal name in ticket\n", 
			username));
		nt_status = NT_STATUS_ACCESS_DENIED;
		goto out;
	}

	DEBUG10(("Successfully validated Kerberos PAC\n"));
