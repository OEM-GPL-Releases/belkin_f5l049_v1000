                       ...
                       uint8 **ptr, struct dns_rr_srv *srv )
{
	...
	if ( !ads_dns_parse_rr( ctx, start, end, ptr, &rr ) ) {
		DEBUG1(("ads_dns_parse_rr_srv: Failed to parse RR record\n"));
		return False;
	}

	if ( rr.type != T_SRV ) {
		DEBUG1(("ads_dns_parse_rr_srv: Bad answer type (%d)\n", rr.type));
		return False;
	}
	...
	if ( namelen < 0 ) {
		DEBUG1(("ads_dns_parse_rr_srv: Failed to uncompress name!\n"));
		return False;
	}

	srv->hostname = talloc_strdup( ctx, dcname );

	DEBUG10(("ads_dns_parse_rr_srv: Parsed %s [%u, %u, %u]\n", 
		  srv->hostname, 
		  srv->priority,
		  srv->weight,
		  srv->port));
                       ...
                       uint8 **ptr, struct dns_rr_ns *nsrec )
{
	...
	if ( !ads_dns_parse_rr( ctx, start, end, ptr, &rr ) ) {
		DEBUG1(("ads_dns_parse_rr_ns: Failed to parse RR record\n"));
		return False;
	}

	if ( rr.type != T_NS ) {
		DEBUG1(("ads_dns_parse_rr_ns: Bad answer type (%d)\n", rr.type));
		return False;
	}
	...
	if ( namelen < 0 ) {
		DEBUG1(("ads_dns_parse_rr_ns: Failed to uncompress name!\n"));
		return False;
	}
	...
	do {
		...
		if (buf_len) {
			if ( (buffer = TALLOC_ARRAY(ctx, uint8, buf_len)) == NULL ) {
				DEBUG0(("ads_dns_lookup_srv: talloc() failed!\n"));
		...
		if ( (resp_len = res_query(name, C_IN, q_type, buffer, buf_len)) < 0 ) {
			DEBUG3(("ads_dns_lookup_srv: Failed to resolve %s (%s)\n", name, strerror(errno)));
			TALLOC_FREE( buffer );
			if (errno == ETIMEDOUT) {
		...
		if (buf_len == resp_len) {
			if (resp_len == MAX_DNS_PACKET_SIZE) {
				DEBUG1(("dns_send_req: DNS reply too large when resolving %s\n",
					name));
				TALLOC_FREE( buffer );
...
static NTSTATUS ads_dns_lookup_srv( TALLOC_CTX *ctx, const char *name, struct dns_rr_srv **dclist, int *numdcs )
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG3(("ads_dns_lookup_srv: Failed to send DNS query (%s)\n",
			nt_errstr(status)));
		return status;
	}
	p = buffer;
	...
	DEBUG4(("ads_dns_lookup_srv: %d records returned in the answer section.\n", 
		answer_count));
		
	if (answer_count) {
		if ( (dcs = TALLOC_ZERO_ARRAY(ctx, struct dns_rr_srv, answer_count)) == NULL ) {
			DEBUG0(("ads_dns_lookup_srv: talloc() failure for %d char*'s\n", 
				answer_count));
			return NT_STATUS_NO_MEMORY;
		}
	} else {
	...
	for ( rrnum=0; rrnum<additional_count; rrnum++ ) {
		...
		if (!ads_dns_parse_rr(ctx, buffer, buffer+resp_len,
					&p, &rr)) {
			DEBUG1(("ads_dns_lookup_srv: Failed "
...
NTSTATUS ads_dns_lookup_ns( TALLOC_CTX *ctx, const char *dnsdomain, struct dns_rr_ns **nslist, int *numns )
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG3(("ads_dns_lookup_ns: Failed to send DNS query (%s)\n",
			nt_errstr(status)));
		return status;
	}
	p = buffer;
	...
	DEBUG4(("ads_dns_lookup_ns: %d records returned in the answer section.\n", 
		answer_count));
		
	if (answer_count) {
		if ( (nsarray = TALLOC_ARRAY(ctx, struct dns_rr_ns, answer_count)) == NULL ) {
			DEBUG0(("ads_dns_lookup_ns: talloc() failure for %d char*'s\n", 
				answer_count));
			return NT_STATUS_NO_MEMORY;
		}
	} else {
	...
	for ( rrnum=0; rrnum<query_count; rrnum++ ) {
		...
		if ( !ads_dns_parse_query( ctx, buffer, buffer+resp_len, &p, &q ) ) {
			DEBUG1(("ads_dns_lookup_ns: Failed to parse query record!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	}
	...
	for ( rrnum=0; rrnum<answer_count; rrnum++ ) {
		if ( !ads_dns_parse_rr_ns( ctx, buffer, buffer+resp_len, &p, &nsarray[rrnum] ) ) {
			DEBUG1(("ads_dns_lookup_ns: Failed to parse answer record!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}		
	}
	idx = rrnum;
	...
	for ( rrnum=0; rrnum<auth_count; rrnum++ ) {
		...
		if ( !ads_dns_parse_rr( ctx, buffer, buffer+resp_len, &p, &rr ) ) {
			DEBUG1(("ads_dns_lookup_ns: Failed to parse authority record!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	}
	...
	for ( rrnum=0; rrnum<additional_count; rrnum++ ) {
		...
		if ( !ads_dns_parse_rr( ctx, buffer, buffer+resp_len, &p, &rr ) ) {
			DEBUG1(("ads_dns_lookup_ns: Failed to parse additional records section!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
...
BOOL sitename_store(const char *realm, const char *sitename)
{
	...
	if (!realm || (strlen(realm) == 0)) {
		DEBUG0(("sitename_store: no realm\n"));
		return False;
	}
	...
	if (!sitename || (sitename && !*sitename)) {
		DEBUG5(("sitename_store: deleting empty sitename!\n"));
	...
	DEBUG10(("sitename_store: realm = [%s], sitename = [%s], expire = [%u]\n",
...
char *sitename_fetch(const char *realm)
{
	...
	SAFE_FREE(key);
	if ( !ret ) {
		DEBUG5(("sitename_fetch: No stored sitename for %s\n",
			query_realm));
	} else {
		DEBUG5(("sitename_fetch: Returning sitename for %s: \"%s\"\n",
...
BOOL stored_sitename_changed(const char *realm, const char *sitename)
{
	...
	if (!realm || (strlen(realm) == 0)) {
		DEBUG0(("stored_sitename_changed: no realm\n"));
		return False;
	}
