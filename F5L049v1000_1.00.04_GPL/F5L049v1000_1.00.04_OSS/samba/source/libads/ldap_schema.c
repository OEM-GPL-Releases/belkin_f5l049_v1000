...
failed:
	DEBUG0(("ads_get_attrname_by_oid: failed to retrieve name for oid: %s\n", 
		OID));
					  ...
					  struct posix_schema **s ) 
{
	...
	DEBUG10(("ads_check_posix_schema_mapping\n"));
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG3(("ads_check_posix_mapping: Unable to retrieve schema DN!\n"));
		goto done;
	}
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG3(("ads_check_posix_schema_mapping: failed %s\n", 
			ads_errstr(status)));
		goto done;
	}
