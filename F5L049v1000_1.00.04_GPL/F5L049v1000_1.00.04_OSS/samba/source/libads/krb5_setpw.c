					   ...
					   krb5_data *packet)
{
	...
	if (ret) {
		DEBUG1(("krb5_auth_con_setflags failed (%s)\n",
			 error_message(ret)));
		return ret;
	}
	...
	*/
	...
	if (use_tcp) {
		...
		if (RIVAL(p, 0) != msg_length) {
			DEBUG1(("Bad TCP packet length (%d/%d) from kpasswd server\n",
	...
	if (RSVAL(p, 0) != msg_length) {
		DEBUG1(("Bad packet length (%d/%d) from kpasswd server\n",
	...
	if (vnum != KRB5_KPASSWD_VERS_SETPW && 
		...
		DEBUG1(("Bad vnum (%d) from kpasswd server\n", vnum));
	...
	if (p + ap_rep.length >= (char *)packet->data + packet->length) {
		DEBUG1(("ptr beyond end of packet from kpasswd server\n"));
		return KRB5KRB_AP_ERR_MODIFIED;
	}
	
	if (ap_rep.length == 0) {
		DEBUG1(("got unencrypted setpw result?!\n"));
		return KRB5KRB_AP_ERR_MODIFIED;
	}
	...
	if (ret) {
		DEBUG1(("failed to rd setpw reply (%s)\n", error_message(ret)));
		return KRB5KRB_AP_ERR_MODIFIED;
	}
	...
	ret = krb5_rd_priv(context, auth_context, &cipherresult, &clearresult,
			   &replay);
	if (ret) {
		DEBUG1(("failed to decrypt setpw reply (%s)\n", error_message(ret)));
		return KRB5KRB_AP_ERR_MODIFIED;
	}
	...
	if (res_code == KRB5_KPASSWD_SUCCESS) {
		...
		DEBUG1( ("Error changing password: %s (%d)\n", errstr, res_code));
					  ...
					  const char *newpw)
{
	...
	ret = krb5_mk_req_extended(context, &auth_context, AP_OPTS_USE_SUBKEY,
				   NULL, credsp, &ap_req);
	if (ret) {
		DEBUG1(("krb5_mk_req_extended failed (%s)\n", error_message(ret)));
	...
	do {
		...
		if (sock == -1) {
			...
			DEBUG1(("failed to open kpasswd socket to %s (%s)\n", 
		...
		if (ret) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG1(("krb5_auth_con_setaddrs failed (%s)\n", error_message(ret)));
			return ADS_ERROR_KRB5(ret);
		}
		...
		if (ret) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG1(("build_setpw_request failed (%s)\n", error_message(ret)));
			return ADS_ERROR_KRB5(ret);
		}
		...
		if (ret < 0) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG1(("recv of chpw reply failed (%s)\n", strerror(errno)));
			return ADS_ERROR_SYSTEM(errno);
		}
		...
		if (ret) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG1(("krb5_auth_con_setaddrs on reply failed (%s)\n", 
				 error_message(ret)));
			return ADS_ERROR_KRB5(ret);
		}
		...
		if (ret) {
			...
			SAFE_FREE(ap_req.data);
			krb5_auth_con_free(context, auth_context);
			DEBUG1(("parse_setpw_reply failed (%s)\n", 
				 error_message(ret)));
			return ADS_ERROR_KRB5(ret);
		}
				 ...
				 const char *newpw, int time_offset)
{
	...
	if (ret) {
		DEBUG1(("Failed to init krb5 context (%s)\n", error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
	...
	if (ret) {
		...
		DEBUG1(("Failed to get default creds (%s)\n", error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
	...
	if (ret) {
		krb5_cc_close(context, ccache);
                krb5_free_context(context);
		DEBUG1(("Failed to parse kadmin/changepw (%s)\n", error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
	...
	if (ret) {
		...
		krb5_princ_set_realm(context, creds.server, &orig_realm);
	        ...
	        krb5_free_principal(context, principal);
                krb5_free_context(context);
		DEBUG1(("Failed to get principal from ccache (%s)\n", 
			 error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
	...
	if (ret) {
		...
		krb5_princ_set_realm(context, creds.server, &orig_realm);
	        ...
	        krb5_free_principal(context, principal);
	        krb5_free_context(context);
		DEBUG1(("krb5_get_credentials failed (%s)\n", error_message(ret)));
		return ADS_ERROR_KRB5(ret);
	}
					...
					int time_offset)
{
	...
	DEBUG1(("Password incorrect while getting initial ticket"));
      else
	DEBUG1(("krb5_get_init_creds_password failed (%s)\n", error_message(ret)));
				 ...
				 int time_offset)
{
	...
	DEBUG1(("Failed kinit for principal %s (%s)\n", auth_principal, error_message(ret)));
	return ADS_ERROR_KRB5(ret);
    }
