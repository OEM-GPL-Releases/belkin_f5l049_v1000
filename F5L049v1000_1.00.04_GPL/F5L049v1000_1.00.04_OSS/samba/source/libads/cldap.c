	...
	do {
		if ((*ptr & 0xc0) == 0xc0) {
			...
			if ((pret - ret + len + 1) >= MAX_DNS_LABEL) {
				DEBUG1(("DC returning too long DNS name\n"));
				return 0;
			}
			       ...
			       const char *hostname, unsigned ntversion)
{
	...
	if (data.has_error) {
		DEBUG2(("Failed to build cldap netlogon at offset %d\n", (int)data.ofs));
		asn1_free(&data);
		return -1;
	}

	if (write(sock, data.data, data.length) != (ssize_t)data.length) {
		DEBUG2(("failed to send cldap query (%s)\n", strerror(errno)));
		asn1_free(&data);
		return -1;
	}
...
static int recv_cldap_netlogon(int sock, struct cldap_netlogon_reply *reply)
{
	...
	if (ret <= 0) {
		DEBUG1(("no reply received to cldap netlogon\n"));
		data_blob_free(&blob);
		return -1;
	}
	blob.length = ret;
	...
	if (data.has_error) {
		...
		data_blob_free(&os3);
		asn1_free(&data);
		DEBUG1(("Failed to parse cldap reply\n"));
		return -1;
	}
...
BOOL ads_cldap_netlogon(const char *server, const char *realm,  struct cldap_netlogon_reply *reply)
{
	...
	if (sock == -1) {
		DEBUG2(("ads_cldap_netlogon: Failed to open udp socket to %s\n", 
			 server));
		return False;
	}
