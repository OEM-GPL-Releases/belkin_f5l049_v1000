...
static ADS_STATUS ads_sasl_spnego_bind(ADS_STRUCT *ads)
{
	...
	for (i=0;OIDs[i];i++) {
		DEBUG3(("ads_sasl_spnego_bind: got OID=%s\n", OIDs[i]));
#ifdef HAVE_KRB5
	...
	DEBUG3(("ads_sasl_spnego_bind: got server principal name = %s\n", given_principal));
	    ...
	    got_kerberos_mechanism) 
	{
		...
		DEBUG10(("ads_sasl_spnego_krb5_bind failed with: %s, "
			  "calling kinit\n", ads_errstr(status)));
