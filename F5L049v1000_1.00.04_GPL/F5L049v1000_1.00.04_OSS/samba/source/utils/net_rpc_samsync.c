				...
				const char **argv) 
{
	...
	if (!secrets_fetch_trust_account_password(domain_name,
		...
		DEBUG0(("Could not fetch trust account password\n"));
		goto fail;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("Error connecting to NETLOGON pipe\n"));
		goto fail;
	}
#endif
...
static NTSTATUS fetch_account_info(uint32 rid, SAM_ACCOUNT_INFO *delta)
{
	...
	DEBUG3( ("Attempting to find SID %s for user %s in the passdb\n", sid_to_string(sid_string, &user_sid), account));
	if (!pdb_getsampwsid(sam_account, &user_sid)) {
		sam_account_from_delta(sam_account, delta);
		DEBUG3( ("Attempting to add user SID %s for user %s in the passdb\n", 
			  sid_to_string(sid_string, &user_sid), pdb_get_username(sam_account)));
		if (!NT_STATUS_IS_OK(pdb_add_sam_account(sam_account))) {
			DEBUG1( ("SAM Account for %s failed to be added to the passdb!\n",
				  account));
			return NT_STATUS_ACCESS_DENIED; 
		}
	} else {
		sam_account_from_delta(sam_account, delta);
		DEBUG3( ("Attempting to update user SID %s for user %s in the passdb\n", 
			  sid_to_string(sid_string, &user_sid), pdb_get_username(sam_account)));
		if (!NT_STATUS_IS_OK(pdb_update_sam_account(sam_account))) {
			DEBUG1( ("SAM Account for %s failed to be updated in the passdb!\n",
				  account));
	...
	if (!pdb_getgrsid(&map, group_sid)) {
		DEBUG0( ("Primary group of %s has no mapping!\n",
			  pdb_get_username(sam_account)));
	} else {
		if (map.gid != passwd->pw_gid) {
			if (!(grp = getgrgid(map.gid))) {
				DEBUG0( ("Could not find unix group %lu for user %s (group SID=%s)\n", 
	...
	if ( !passwd ) {
		DEBUG1( ("No unix user for this account (%s), cannot adjust mappings\n", 
			pdb_get_username(sam_account)));
	}
...
static NTSTATUS fetch_group_mem_info(uint32 rid, SAM_GROUP_MEM_INFO *delta)
{
	...
	if (!get_domain_group_from_sid(group_sid, &map)) {
		DEBUG0( ("Could not find global group %d\n", rid));
		return NT_STATUS_NO_SUCH_GROUP;
	}

	if (!(grp = getgrgid(map.gid))) {
		DEBUG0( ("Could not find unix group %lu\n", (unsigned long)map.gid));
		return NT_STATUS_NO_SUCH_GROUP;
	}
	...
	for (i=0; i<delta->num_members; i++) {
		...
		if (!pdb_getsampwsid(member, &member_sid)) {
			DEBUG1( ("Found bogus group member: %d (member_sid=%s group=%s)\n",
					   ...
					   int alloced)
{
	...
	if (i == alloced){
		DEBUG1( ("Could not find rid %d in groupmap array\n", 
			  group_rid));
		return NT_STATUS_UNSUCCESSFUL;
	}
					    ...
					    FILE *mod_fd, int alloced)
{
	...
	if (delta->grp_mem_info.num_members > 0) {
		...
		for (i=0; i<delta->grp_mem_info.num_members; i++) {
			...
			if (k == alloced){
				DEBUG1( ("Could not find rid %d in "
					  "accountmap array\n", rid));
				return NT_STATUS_UNSUCCESSFUL;
			}
				       ...
				       const char *user_file)
{
	...
	if (suffix == NULL || strcmp(suffix, "") == 0) {
		DEBUG0(("ldap suffix missing from smb.conf--exiting\n"));
		exit(1);
	}

	/* Get other smb.conf data */
	if (!(lp_workgroup()) || !*(lp_workgroup())) {
		DEBUG0(("workgroup missing from smb.conf--exiting\n"));
		exit(1);
	}
	...
	if ((add_name != NULL) &&
		...
		DEBUG1(("unlink(%s) failed, error was (%s)\n",
