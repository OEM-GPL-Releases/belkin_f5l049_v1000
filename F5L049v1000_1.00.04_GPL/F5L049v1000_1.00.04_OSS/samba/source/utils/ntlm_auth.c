...
const char *get_winbind_domain(void)
{
	...
	if (winbindd_request_response(WINBINDD_DOMAIN_NAME, NULL, &response) !=
	    NSS_STATUS_SUCCESS) {
		DEBUG0( ("could not obtain winbind domain name!\n"));
		return lp_workgroup();
	}
...
const char *get_winbind_netbios_name(void)
{
	...
	if (winbindd_request_response(WINBINDD_NETBIOS_NAME, NULL, &response) !=
	    NSS_STATUS_SUCCESS) {
		DEBUG0( ("could not obtain winbind netbios name!\n"));
...
static BOOL get_require_membership_sid(void) {
	...
	if (!parse_ntlm_auth_domain_user(require_membership_of, 
		...
		DEBUG0( ("Could not parse %s into seperate domain/name parts!\n", 
	...
	if (winbindd_request_response(WINBINDD_LOOKUPNAME, &request, &response) !=
	    NSS_STATUS_SUCCESS) {
		DEBUG0( ("Winbindd lookupname failed to resolve %s into a SID!\n", 
			  require_membership_of));
		return False;
	}
				 ...
				 BOOL stdout_diagnostics)
{
	...
	if (stdout_diagnostics) {
		...
		if ((result != NSS_STATUS_SUCCESS) && (response.data.auth.nt_status == 0)) {
			DEBUG1( ("Reading winbind reply failed! (0x01)\n"));
		}
...
static NTSTATUS local_pw_check(struct ntlmssp_state *ntlmssp_state, DATA_BLOB *user_session_key, DATA_BLOB *lm_session_key) 
{
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		DEBUG3( ("Login for user [%s]\\[%s]@[%s] failed due to [%s]\n", 
...
static NTSTATUS ntlm_auth_start_ntlmssp_client(NTLMSSP_STATE **client_ntlmssp_state) 
{
	...
	if ( (opt_username == NULL) || (opt_domain == NULL) ) {
		...
		DEBUG1( ("Need username and domain for NTLMSSP\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG1( ("Could not start NTLMSSP client: %s\n",
			  nt_errstr(status)));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG1( ("Could not set username: %s\n",
			  nt_errstr(status)));
		ntlmssp_end(client_ntlmssp_state);
		return status;
	}
	...
	if (opt_password) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG1( ("Could not set password: %s\n",
				  nt_errstr(status)));
			ntlmssp_end(client_ntlmssp_state);
			return status;
		}
...
static NTSTATUS ntlm_auth_start_ntlmssp_server(NTLMSSP_STATE **ntlmssp_state) 
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG1( ("Could not start NTLMSSP server: %s\n",
			  nt_errstr(status)));
		return status;
	}
					 ...
					 char *buf, int length) 
{
	...
	if (strlen(buf) < 2) {
		DEBUG1( ("NTLMSSP query [%s] invalid", buf));
	...
	if (strlen(buf) > 3) {
		if(strncmp(buf, "SF ", 3) == 0){
			DEBUG10( ("Setting flags to negotioate\n"));
					 ...
					 char *buf, int length) 
{
	...
	if (strlen(buf) > 3) {
		if(strncmp(buf, "SF ", 3) == 0) {
			DEBUG10( ("Looking for flags to negotiate\n"));
			SAFE_FREE(want_feature_list);
			want_feature_list = SMB_STRNDUP(buf+3, strlen(buf)-3);
			x_fprintf(x_stdout, "OK\n");
			return;
				       ...
				       char *buf, int length) 
{
	...
	if (!pass) {
		DEBUG2( ("Password not found. Denying access\n"));
...
static void offer_gss_spnego_mechs(void) {
	...
	if (len == -1) {
		DEBUG1( ("Could not write SPNEGO data blob\n"));
	...
	DEBUG10( ("sent SPNEGO negTokenInit\n"));
	return;
}
				      ...
				      char *buf, int length) 
{
	...
	if (strlen(buf) < 2) {
		DEBUG1( ("SPENGO query [%s] invalid", buf));
	...
	if (strlen(buf) <= 3) {
		DEBUG1( ("GSS-SPNEGO query [%s] invalid\n", buf));
	...
	if (len == -1) {
		DEBUG1( ("GSS-SPNEGO query [%s] invalid", buf));
		x_fprintf(x_stdout, "BH GSS-SPNEGO query invalid\n");
		return;
	}
	...
	if (request.type == SPNEGO_NEG_TOKEN_INIT) {
		...
		if ( (request.negTokenInit.mechTypes == NULL) ||
			...
			DEBUG1( ("Client did not offer any mechanism"));
		...
		if (strcmp(request.negTokenInit.mechTypes[0], OID_NTLMSSP) == 0) {
			...
			if ( request.negTokenInit.mechToken.data == NULL ) {
				DEBUG1( ("Client did not provide NTLMSSP data\n"));
				x_fprintf(x_stdout, "BH Client did not provide NTLMSSP data\n");
				return;
			}

			if ( ntlmssp_state != NULL ) {
				DEBUG1( ("Client wants a new NTLMSSP challenge, but "
					  "already got one\n"));
		...
		if (strcmp(request.negTokenInit.mechTypes[0], OID_KERBEROS5_OLD) == 0) {
			...
			if ( request.negTokenInit.mechToken.data == NULL ) {
				DEBUG1( ("Client did not provide Kerberos data\n"));
			...
			if (NT_STATUS_IS_OK(status)) {
				...
				if (domain == NULL) {
					DEBUG1( ("Did not get a valid principal "
		...
		if ( (request.negTokenTarg.supportedMech == NULL) ||
			...
			DEBUG1( ("Got a negTokenTarg for something non-NTLMSSP: %s\n",
		...
		if (request.negTokenTarg.responseToken.data == NULL) {
			DEBUG1( ("Got a negTokenTarg without a responseToken!\n"));
...
static BOOL manage_client_ntlmssp_init(SPNEGO_DATA spnego)
{
	...
	DEBUG10( ("Got spnego negTokenInit with NTLMSSP\n"));

	if (client_ntlmssp_state != NULL) {
		DEBUG1( ("Request for initial SPNEGO request where "
	...
	if ( !(NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED) ||
		...
		DEBUG1( ("Expected OK or MORE_PROCESSING_REQUIRED, got: %s\n",
			  nt_errstr(status)));
		ntlmssp_end(&client_ntlmssp_state);
		return False;
	}
...
static void manage_client_ntlmssp_targ(SPNEGO_DATA spnego)
{
	...
	DEBUG10( ("Got spnego negTokenTarg with NTLMSSP\n"));

	if (client_ntlmssp_state == NULL) {
		DEBUG1( ("Got NTLMSSP tArg without a client state\n"));
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED)) {
		DEBUG1( ("Expected MORE_PROCESSING_REQUIRED from "
...
static BOOL manage_client_krb5_init(SPNEGO_DATA spnego)
{
	...
	if ( (spnego.negTokenInit.mechListMIC.data == NULL) ||
		...
		DEBUG1( ("Did not get a principal for krb5\n"));
		return False;
	}
	...
	if (principal == NULL) {
		DEBUG1( ("Could not malloc principal\n"));
		return False;
	}
	...
	if (retval) {
		...
		if ((retval = kerberos_kinit_password(user, opt_password, 0, NULL))) {
			DEBUG10( ("Requesting TGT failed: %s\n", error_message(retval)));
			return False;
		}
		...
		if (retval) {
			DEBUG10( ("Kinit suceeded, but getting a ticket failed: %s\n", error_message(retval)));
			return False;
		}
	}
	...
	SAFE_FREE(reply_base64);
	data_blob_free(&to_server);
	DEBUG10( ("sent GSS-SPNEGO KERBEROS5 negTokenInit\n"));
	return True;
}
...
static void manage_client_krb5_targ(SPNEGO_DATA spnego)
{
	...
	case SPNEGO_ACCEPT_INCOMPLETE:
		DEBUG1( ("Got a Kerberos negTokenTarg with ACCEPT_INCOMPLETE\n"));
		x_fprintf(x_stdout, "BH Got a Kerberos negTokenTarg with ACCEPT_INCOMPLETE\n");
		break;
	case SPNEGO_ACCEPT_COMPLETED:
		DEBUG10( ("Accept completed\n"));
		x_fprintf(x_stdout, "AF\n");
		break;
	case SPNEGO_REJECT:
		DEBUG10( ("Rejected\n"));
		...
		break;
	default:
		DEBUG1( ("Got an invalid negTokenTarg\n"));
		x_fprintf(x_stdout, "AF\n");
	}
}
					     ...
					     char *buf, int length) 
{
	...
	if (strlen(buf) <= 3) {
		DEBUG1( ("SPNEGO query [%s] too short\n", buf));
	...
	if ( (strncmp(buf, "TT ", 3) != 0) &&
	     (strncmp(buf, "AF ", 3) != 0) &&
	     (strncmp(buf, "NA ", 3) != 0) ) {
		DEBUG1( ("SPNEGO request [%s] invalid\n", buf));
	...
	if (len == -1) {
		DEBUG1( ("Could not read SPNEGO data for [%s]\n", buf));
	...
	if (spnego.type == SPNEGO_NEG_TOKEN_INIT) {
		...
		DEBUG1( ("Server offered no compatible mechanism\n"));
	...
	if (spnego.type == SPNEGO_NEG_TOKEN_TARG) {
		...
		if (spnego.negTokenTarg.supportedMech == NULL) {
			...
			case SPNEGO_REJECT:
				...
				break;
			default:
				DEBUG1( ("Got a negTokenTarg with no mech and an "
					  "unknown negResult: %d\n",
	...
	DEBUG1( ("Got an SPNEGO token I could not handle [%s]!\n", buf));
...
static void manage_squid_request(enum stdio_helper_mode helper_mode, stdio_helper_function fn) 
{
	...
	if (fgets(buf, sizeof(buf)-1, stdin) == NULL) {
		if (ferror(stdin)) {
			DEBUG1( ("fgets() failed! dying..... errno=%d (%s)\n", ferror(stdin),
	...
	if (err) {
		DEBUG2( ("Oversized message\n"));
		x_fprintf(x_stderr, "ERR\n");
		err = 0;
		return;
	}

	DEBUG10( ("Got '%s' from squid (length: %d).\n",buf,length));

	if (buf[0] == '\0') {
		DEBUG2( ("Invalid Request\n"));
		x_fprintf(x_stderr, "ERR\n");
		return;
	}
