			...
			struct in_addr *server_ip, const char *server_name)
{
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		DEBUG1(("Cannot connect to server (anonymously).  Error was %s\n", nt_errstr(nt_status)));
		return nt_status;
	}
}
			...
			struct in_addr *server_ip, const char *server_name)
{
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		DEBUG1(("Cannot connect to server using kerberos.  Error was %s\n", nt_errstr(nt_status)));
		return nt_status;
	}
}
...
NTSTATUS connect_dst_pipe(struct cli_state **cli_dst, struct rpc_pipe_client **pp_pipe_hnd, int pipe_num)
{
	...
	if (!pipe_hnd) {
		DEBUG0( ("couldn't not initialize pipe\n"));
...
BOOL net_find_server(const char *domain, unsigned flags, struct in_addr *server_ip, char **server_name)
{
	...
	if (opt_have_ip) {
		*server_ip = opt_dest_ip;
		...
		if (!resolve_name(*server_name, server_ip, 0x20))  {
			DEBUG1(("Unable to resolve server name\n"));
			return False;
		}
		...
		if (!resolve_name(d, &msbrow_ip, 0x1B))  {
			DEBUG1(("Unable to resolve domain browser via name lookup\n"));
			return False;
		} else {
			*server_ip = msbrow_ip;
		}
		*server_name = SMB_STRDUP(inet_ntoa(opt_dest_ip));
		...
		if (!resolve_name(d, &brow_ips, 0x1D))  {
			...
			DEBUG1(("Unable to resolve master browser via name lookup\n"));
			return False;
		} else {
			*server_ip = brow_ips;
		}
	...
	if (!server_name || !*server_name) {
		DEBUG1(("no server to connect to\n"));
		return False;
	}
...
static int net_getlocalsid(int argc, const char **argv)
{
	...
	if(!initialize_password_db(False)) {
		DEBUG0( ("WARNING: Could not open passdb - local sid may not reflect passdb\n"
	...
	if (!secrets_fetch_domain_sid(name, &sid)) {
		DEBUG0( ("Can't fetch domain SID for name: %s\n", name));
		return 1;
	}
...
static int net_setlocalsid(int argc, const char **argv)
{
	...
	if (!secrets_store_domain_sid(global_myname(), &sid)) {
		DEBUG0(("Can't store domain SID as a pdc/bdc.\n"));
		return 1;
	}
...
static int net_getdomainsid(int argc, const char **argv)
{
	...
	if(!initialize_password_db(False)) {
		DEBUG0( ("WARNING: Could not open passdb - domain sid may not reflect passdb\n"
			  "backend knowlege (such as the sid stored in LDAP)\n"));
	}
...
static int net_maxrid(int argc, const char **argv)
{
	...
	if (argc != 0) {
	        DEBUG0( ("usage: net maxrid\n"));
		return 1;
	}

	if ((rid = get_maxrid()) == 0) {
		DEBUG0( ("can't get current maximum rid\n"));
		return 1;
	}
