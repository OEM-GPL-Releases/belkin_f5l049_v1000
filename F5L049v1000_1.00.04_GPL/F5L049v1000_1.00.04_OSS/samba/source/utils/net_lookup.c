...
static int net_lookup_ldap(int argc, const char **argv)
{
	...
	DEBUG9( ("Lookup up ldap for domain %s\n", domain));
	...
	if ( NT_STATUS_IS_OK(status) && numdcs ) {
		...
		return 0;
     	...
     	DEBUG9( ("Looking up DC for domain %s\n", domain));
	...
	DEBUG9( ("Found DC with DNS name %s\n", hostent->h_name));
	...
	DEBUG9( ("Looking up ldap for domain %s\n", domain));
...
static int net_lookup_kdc(int argc, const char **argv)
{
	...
	if (rc) {
		DEBUG1(("krb5_init_context failed (%s)\n", 
			 error_message(rc)));
		return -1;
	}
	...
	if (argc>0) {
		...
		if (rc) {
			DEBUG1(("krb5_gethost_realm failed (%s)\n",
				 error_message(rc)));
			return -1;
		}
	...
	if (rc) {
		DEBUG1( ("smb_krb5_locate_kdc failed (%s)\n", error_message(rc)));
		return -1;
	}
