                            ...
                            const char *username) 
{
	...
	DEBUG3( ("export_database: username=\"%s\"\n", username ? username : "(NULL)"));
			  ...
			  const BOOL badpw, const BOOL hours)
{
	...
	if (!pdb_update_autolock_flag(sam_pwent, &updated_autolock)) {
		DEBUG2(("pdb_update_autolock_flag failed.\n"));
	}

	if (!pdb_update_bad_password_count(sam_pwent, &updated_badpw)) {
		DEBUG2(("pdb_update_bad_password_count failed.\n"));
	}
			...
			const char *profile, char *user_sid, BOOL stdin_get)
{
	...
	if ( !(pwd = getpwnam_alloc( NULL, username )) ) {
		DEBUG0(("Cannot locate Unix account for %s\n", username));
		return -1;
	}

	if ( (sam_pwent = samu_new( NULL )) == NULL ) {
		DEBUG0( ("Memory allocation failure!\n"));
		return -1;
	}
	...
	if (!NT_STATUS_IS_OK(samu_alloc_rid_unix(sam_pwent, pwd ))) {
		TALLOC_FREE( sam_pwent );
		TALLOC_FREE( pwd );
		DEBUG0( ("could not create account to add new user %s\n", username));
		return -1;
	}
