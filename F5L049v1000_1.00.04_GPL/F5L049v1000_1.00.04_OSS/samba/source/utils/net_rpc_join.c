...
int net_rpc_join_ok(const char *domain, const char *server, struct in_addr *ip )
{
	...
	/* We return NT_STATUS_INVALID_NETWORK_RESPONSE if the server is refusing
	   to negotiate schannel, but the creds were set up ok. That'll have to do. */
		...
		if (NT_STATUS_EQUAL(ntret, NT_STATUS_INVALID_NETWORK_RESPONSE)) {
			...
			DEBUG0(("net_rpc_join_ok: failed to get schannel session "
	...
	if (!pipe_hnd) {
		DEBUG0(("net_rpc_join_ok: failed to open schannel session "
...
int net_rpc_join_newstyle(int argc, const char **argv) 
{
	...
	if (!(mem_ctx = talloc_init("net_rpc_join_newstyle"))) {
		DEBUG0( ("Could not initialise talloc context\n"));
		goto done;
	}
	...
	if (!pipe_hnd) {
		DEBUG0( ("Error connecting to LSA pipe. Error was %s\n",
			nt_errstr(result) ));
		goto done;
	}
	...
	if (!domain) {
		DEBUG0( ("Could not get domain name.\n"));
		goto done;
	}
	...
	if (!pipe_hnd) {
		DEBUG0( ("Error connecting to SAM pipe. Error was %s\n",
			nt_errstr(result) ));
		goto done;
	}
	...
	if (name_types[0] != SID_NAME_USER) {
		DEBUG0( ("%s is not a user account (type=%d)\n", acct_name, name_types[0]));
		goto done;
	}
	...
	if (!pipe_hnd) {
		DEBUG0(("Error connecting to NETLOGON pipe. Error was %s\n",
			nt_errstr(result) ));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG0( ("Error in domain join verification (credential setup failed): %s\n\n",
			  nt_errstr(result)));
	...
	if (lp_client_schannel() && (neg_flags & NETLOGON_NEG_SCHANNEL)) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG0( ("Error in domain join verification (schannel setup failed): %s\n\n",
				  nt_errstr(result)));
	...
	if (!secrets_store_machine_password(clear_trust_password, domain, sec_channel_type)) {
		DEBUG0( ("error storing plaintext domain secrets for %s\n", domain));
	}
