			...
			const char **argv) 
{
	...
	if (!(conn_flags & NET_FLAGS_NO_PIPE)) {
		if (lp_client_schannel() && (pipe_idx == PI_NETLOGON)) {
			...
			pipe_hnd = cli_rpc_pipe_open_schannel(cli, pipe_idx,
							...
							&nt_status);
			if (!pipe_hnd) {
				DEBUG0( ("Could not initialise schannel netlogon pipe. Error was %s\n",
					nt_errstr(nt_status) ));
				cli_shutdown(cli);
				return -1;
			}
			...
			if (!pipe_hnd) {
				DEBUG0( ("Could not initialise pipe %s. Error was %s\n",
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG1( ("rpc command function failed! (%s)\n", nt_errstr(nt_status)));
	} else {
		DEBUG5( ("rpc command function succedded\n"));
	}
					...
					const char **argv)
{
	...
	if (!pipe_hnd) {
		DEBUG0(("rpc_oldjoin_internals: netlogon pipe open to machine %s failed. "
			"error was %s\n",
			cli->desthost,
			nt_errstr(result) ));
		return result;
	...
	*/
	...
	if (!secrets_store_domain_sid(opt_target_workgroup, domain_sid)) {
		DEBUG0( ("error storing domain sid for %s\n", opt_target_workgroup));
				...
				int argc, const char **argv)
{
	...
	acb_info = ACB_NORMAL;
        acct_flags = SAMR_GENERIC_READ | SAMR_GENERIC_WRITE |
                ...
                SAMR_USER_SETATTR;
	DEBUG10( ("Creating account with flags: %d\n",acct_flags));
...
static int rpc_share_add(int argc, const char **argv)
{
	if ((argc < 1) || !strchr(argv[0], '=')) {
		DEBUG1(("Sharename or path not specified on add\n"));
...
{
	if (argc < 1) {
		DEBUG1(("Sharename not specified on delete\n"));
		return rpc_share_usage(argc, argv);
	}
	return run_rpc_command(NULL, PI_SRVSVC, 0, 
...
BOOL sync_files(struct copy_clistate *cp_clistate, pstring mask)
{
	...
	DEBUG3(("calling cli_list with mask: %s\n", mask));
				...
				const char *sharename)
{
	...
	switch (net_mode_share) {
	case NET_MODE_SHARE_MIGRATE:
		DEBUG3(("calling net_copy_fileattr for '.' directory in share %s\n", sharename));
				...
				const char **argv)
{
	...
	for (i=0; i<num_server_aliases; i++) {
		...
		DEBUG1( ("%s\\%s %d: ", domains[0], names[0], types[0]));
...
static BOOL get_user_sids(const char *domain, const char *user, NT_USER_TOKEN *token)
{
	...
	if (result != NSS_STATUS_SUCCESS) {
		DEBUG1( ("winbind could not find %s\n", full_name));
		return False;
	}

	if (response.data.sid.type != SID_NAME_USER) {
		DEBUG1( ("%s is not a user\n", full_name));
		return False;
	}
	...
	if (result != NSS_STATUS_SUCCESS) {
		DEBUG1( ("winbind could not get groups of %s\n", full_name));
		return False;
	}
	...
	for (i = 0; i < response.data.num_entries; i++) {
		...
		if (result != NSS_STATUS_SUCCESS) {
			DEBUG1( ("winbind could not find SID of gid %d\n",
				  gid));
			return False;
		}
...
static BOOL get_user_tokens(int *num_tokens, struct user_token **user_tokens)
{
	...
	*num_tokens = 0;
	...
	if (result == NULL) {
		DEBUG1( ("Could not malloc sid array\n"));
		return False;
	}
				      ...
				      struct user_token **tokens)
{
	...
	while (!feof(f)) {
		...
		if (line[0] == ' ') {
			...
			if (token == NULL) {
				DEBUG0( ("File does not begin with username"));
				return False;
			}
		...
		*tokens = SMB_REALLOC_ARRAY(*tokens, struct user_token, *num_tokens);
		if (*tokens == NULL) {
			DEBUG0( ("Could not realloc tokens\n"));
			return False;
		}
			...
			struct user_token *tokens)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG1( ("Coult not query secdesc for share %s\n",
			  netname));
		return;
	}
	...
	if (share_sd == NULL) {
		DEBUG1( ("Got no secdesc for share %s\n",
			  netname));
	}
	...
	for (i=0; i<num_tokens; i++) {
		...
		if (share_sd != NULL) {
			if (!se_access_check(share_sd, &tokens[i].token,
				...
				DEBUG1( ("Could not check share_sd for "
					  "user %s\n",
					  tokens[i].name));
				continue;
			}
		...
		if (!se_access_check(root_sd, &tokens[i].token,
				     1, &acc_granted, &status)) {
			DEBUG1( ("Could not check root_sd for user %s\n",
				  tokens[i].name));
			continue;
		}
						...
						const char **argv)
{
	...
	if (f == NULL) {
		DEBUG0( ("Could not open userlist: %s\n", strerror(errno)));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (!r) {
		DEBUG0( ("Could not read users from file\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (ret == -1) {
		DEBUG0( ("Error returning browse list: %s\n",
			  cli_errstr(cli)));
		goto done;
	}
...
int net_usersidlist(int argc, const char **argv)
{
	...
	if (!get_user_tokens(&num_tokens, &tokens)) {
		DEBUG0( ("Could not get the user/sid list\n"));
		return 0;
	}
...
{
	if (argc < 1) {
		DEBUG1( ("No fileid given on close\n"));
					...
					const char **argv) 
{
	...
	if (NT_STATUS_IS_OK(result)) {
		...
		DEBUG5(("cmd_shutdown_abort: query succeeded\n"));
	} else
		DEBUG5(("cmd_shutdown_abort: query failed\n"));
						...
						const char **argv) 
{
	...
	if (NT_STATUS_IS_OK(result)) {
		d_printf("\nShutdown successfully aborted\n");
		DEBUG5(("cmd_reg_abort_shutdown: query succeeded\n"));
	} else
		DEBUG5(("cmd_reg_abort_shutdown: query failed\n"));
...
static int rpc_shutdown_abort(int argc, const char **argv) 
{
	...
	DEBUG1( ("initshutdown pipe didn't work, trying winreg pipe\n"));
						...
						const char **argv) 
{
	...
	if (NT_STATUS_IS_OK(result)) {
		...
		DEBUG5(("Shutdown of remote machine succeeded\n"));
	} else {
		DEBUG1(("Shutdown of remote machine failed!\n"));
	}
	return result;
}
...
static int rpc_shutdown(int argc, const char **argv) 
{
	...
	if (rc) {
		DEBUG1( ("initshutdown pipe failed, trying winreg pipe\n"));
						...
						const char **argv)
{
...

	{
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG0(("Could not set trust account password: %s\n",
				 nt_errstr(result)));
			goto done;
		}
	}
...
static int rpc_trustdom_establish(int argc, const char **argv)
{
	...
	if (!net_find_pdc(&server_ip, pdc_name, domain_name)) {
		DEBUG0( ("Couldn't find domain controller for domain %s\n", domain_name));
		return -1;
	}
	...
	if (!NT_STATUS_EQUAL(nt_status, NT_STATUS_NOLOGON_INTERDOMAIN_TRUST_ACCOUNT)) {
		...
		DEBUG0( ("Couldn't verify trusting domain account. Error was %s\n",
			nt_errstr(nt_status)));
		return -1;
	}
	...
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG0( ("Couldn't connect to domain %s controller. Error was %s.\n",
	...
	if (!cli_get_pdc_name(cli, domain_name, (char*)pdc_name)) {
		DEBUG0( ("NetServerEnum2 error: Couldn't find primary domain controller\
			 for domain %s\n", domain_name));
		cli_shutdown(cli);
		return -1;
	}
	...
	if (!pipe_hnd) {
		DEBUG0( ("Could not initialise lsa pipe. Error was %s\n", nt_errstr(nt_status) ));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	}
	...
	nt_status = rpccli_lsa_open_policy2(pipe_hnd, mem_ctx, True, SEC_RIGHTS_QUERY_VALUE,
	                                 &connect_hnd);
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG0( ("Couldn't open policy handle. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	...
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG0( ("LSA Query Info failed. Returned error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	...
	if (!secrets_store_trusted_domain_password(domain_name,
						   opt_password,
						   domain_sid)) {
		DEBUG0( ("Storing password for trusted domain failed.\n"));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	}
	...
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG0( ("Couldn't close LSA pipe. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
...
static int rpc_trustdom_revoke(int argc, const char **argv)
{
	...
	if (!trusted_domain_password_delete(domain_name)) {
		DEBUG0( ("Failed to revoke relationship to the trusted domain %s\n",
			  domain_name));
		return -1;
	};
				      ...
				      const char *trusted_dom_name)
{
	...
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG0(("Could not query trusted domain info. Error was %s\n",
		nt_errstr(nt_status)));
		goto done;
	}
	...
	if (cleartextpwd == NULL) {
		DEBUG0(("retrieved NULL password\n"));
		nt_status = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
...
static int rpc_trustdom_vampire(int argc, const char **argv)
{
	...
	if (!(cli = net_make_ipc_connection(NET_FLAGS_PDC))) {
		DEBUG0( ("Couldn't connect to domain controller\n"));
		talloc_destroy(mem_ctx);
		return -1;
	};
	...
	do {
		...
		if (NT_STATUS_IS_ERR(nt_status)) {
			DEBUG0( ("Couldn't enumerate trusted domains. Error was %s\n",
				nt_errstr(nt_status)));
			cli_shutdown(cli);
			talloc_destroy(mem_ctx);
			return -1;
	...
	nt_status = rpccli_lsa_close(pipe_hnd, mem_ctx, &connect_hnd);
	if (NT_STATUS_IS_ERR(nt_status)) {
		DEBUG0( ("Couldn't properly close lsa policy handle. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
...
static int rpc_trustdom_list(int argc, const char **argv)
{
	...
	if (!pipe_hnd) {
		DEBUG0( ("Could not initialise samr pipe. Error was %s\n", nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	};
	...
	nt_status = rpccli_samr_connect(pipe_hnd, mem_ctx, SA_RIGHT_SAM_OPEN_DOMAIN,
								 &connect_hnd);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0( ("Couldn't open SAMR policy handle. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0( ("Couldn't open domain object. Error was %s\n",
			nt_errstr(nt_status)));
		cli_shutdown(cli);
		talloc_destroy(mem_ctx);
		return -1;
	...
	do {
		...
		nt_status = rpccli_samr_enum_dom_users(pipe_hnd, mem_ctx, &domain_hnd,
		                                    ...
		                                    &num_domains);
		if (NT_STATUS_IS_ERR(nt_status)) {
			DEBUG0( ("Couldn't enumerate accounts. Error was: %s\n",
				nt_errstr(nt_status)));
			cli_shutdown(cli);
			talloc_destroy(mem_ctx);
			return -1;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0( ("Couldn't properly close domain policy handle for domain %s\n", domain_name));
	};
	
	nt_status = rpccli_samr_close(pipe_hnd, mem_ctx, &connect_hnd);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0( ("Couldn't properly close samr policy handle for domain %s\n", domain_name));
	};
