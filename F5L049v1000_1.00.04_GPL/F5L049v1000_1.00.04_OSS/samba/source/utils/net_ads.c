...
static int net_ads_leave(int argc, const char **argv)
{
	...
	if (!secrets_init()) {
		DEBUG1(("Failed to initialise secrets database\n"));
		return -1;
	}
...
static ADS_STATUS net_set_machine_spn(TALLOC_CTX *ctx, ADS_STRUCT *ads_s )
{
	...
	if ( (count = ads_count_replies(ads_s, res)) != 1 ) {
		DEBUG1(("net_set_machine_spn: %d entries returned!\n", count));
		return ADS_ERROR(LDAP_NO_MEMORY);	
	}
	
	if ( (dn_string = ads_get_dn(ads_s, res)) == NULL ) {
		DEBUG1( ("ads_add_machine_acct: ads_get_dn returned NULL (malloc failure?)\n"));
		goto done;
	}
...
static BOOL net_derive_salting_principal( TALLOC_CTX *ctx, ADS_STRUCT *ads )
{
	...
	if ( !ADS_ERR_OK(status) ) {
		DEBUG2(("Failed to determine domain functional level!\n"));
		return False;
	}
	...
	if ( !NT_STATUS_IS_OK(status) || (ns_count == 0)) {
		...
		if ( !ads->ld ) {
			...
			if ( !ADS_ERR_OK(ads_status) ) {
				DEBUG0(("net_update_dns_internal: Failed to connect to our DC!\n"));
				goto done;				
			}			
		}
		...
		if ( !NT_STATUS_IS_OK(status) || (ns_count == 0)) {			
		DEBUG3(("net_ads_join: Failed to find name server for the %s "
			 "realm\n", ads->config.realm));
		goto done;
	}
...
static NTSTATUS net_update_dns(TALLOC_CTX *mem_ctx, ADS_STRUCT *ads)
{
	...
	if ( num_addrs <= 0 ) {
		DEBUG4(("net_ads_join: Failed to find my non-loopback IP "
			 "addresses!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
int net_ads_join(int argc, const char **argv)
{
	...
	if  ( create_in_ou ) {
		...
		if ( !ADS_ERR_OK(status) ) {
			d_fprintf( stderr, "Failed to pre-create the machine object "
				"in OU %s.\n", argv[0]);
			DEBUG1( ("error calling net_precreate_machine_acct: %s\n", 
				  ads_errstr(status)));
			nt_status = ads_ntstatus(status);
			goto fail;
		}
	...
	if ( !NT_STATUS_IS_OK(nt_status) ) {
		DEBUG1( ("call of net_join_domain failed: %s\n", 
	...
	if ( !net_derive_salting_principal( ctx, ads ) ) {
		DEBUG1(("Failed to determine salting principal\n"));
		goto fail;
	}
	...
	if (lp_use_kerberos_keytab() && ads_keytab_create_default(ads)) {
		DEBUG1(("Error creating host keytab!\n"));
	}
