		  ...
		  BOOL copy_timestamps, BOOL is_file)
{
	...
	if (copy_acls) {
		...
		if (!sd) {
			DEBUG0(("failed to get security descriptor: %s\n",
	...
	if (copy_attrs || copy_timestamps) {
		...
		if (!cli_getattrE(cli_share_src, fnum_src, &attr, NULL, 
			...
			DEBUG0(("failed to get file-attrs: %s\n", 
				cli_errstr(cli_share_src)));
			nt_status = cli_nt_error(cli_share_src);
			goto out;
		}
	...
	if (fnum_dst == -1) {
		DEBUG0(("failed to open %s on the destination server: %s: %s\n",
	...
	if (copy_timestamps) {
		...
		if (!cli_setattrE(cli_share_dst, fnum_dst, f_ctime, f_atime, f_mtime)) {
			DEBUG0(("failed to set file-attrs (timestamps): %s\n",
	...
	if (copy_acls) {
		...
		if (!cli_set_secdesc(cli_share_dst, fnum_dst, sd)) {
			DEBUG0(("could not set secdesc on %s: %s\n",
	...
	if (copy_attrs) {
		...
		if (!cli_setatr(cli_share_dst, dst_name, attr, 0)) {
			DEBUG0(("failed to set file-attrs: %s\n",
				cli_errstr(cli_share_dst)));
			nt_status = cli_nt_error(cli_share_dst);
			goto out;
		}
		       ...
		       BOOL copy_timestamps, BOOL is_file)
{
	...
	if (!is_file && !cli_chkpath(cli_share_dst, dst_name)) {
		...
		if (!cli_mkdir(cli_share_dst, dst_name)) {
			DEBUG0(("cannot create directory %s: %s\n",
				dst_name, cli_errstr(cli_share_dst)));
...
static NTSTATUS check_arch_dir(struct cli_state *cli_share, const char *short_archi)
{
	...
	DEBUG10(("creating print-driver dir for architecture: %s\n", 
		short_archi));

	if (!cli_mkdir(cli_share, dir)) {
                DEBUG1(("cannot create directory %s: %s\n",
					...
					POLICY_HND *hnd)
{
	...
	DEBUG10(("connecting to: %s as %s for %s and access: %x\n", 
	...
	DEBUG2(("got printer handle for printer: %s, server: %s\n", 
			     ...
			     PRINTER_DRIVER_CTR *ctr)
{
	...
	if (!W_ERROR_IS_OK(result)) {
		DEBUG1(("cannot get driver (for architecture: %s): %s\n", 
						...
						const char **argv)
{
	...
	for (i = 0; i < num_printers; i++) {
		...
		DEBUG1(("got %d forms for printer\n", num_forms));
						...
						const char **argv)
{
	...
	DEBUG3(("copying printer-drivers\n"));
						...
						const char **argv)
{
	...
	for (i = 0; i < num_printers; i++) {
		...
		if (!net_spoolss_open_printer_ex(pipe_hnd_dst, mem_ctx, sharename, 
			...
			DEBUG1(("could not open printer: %s\n", sharename));
		} else {
			got_hnd_dst = True;
		}
		...
		if (!net_spoolss_getprinter(pipe_hnd_dst, mem_ctx, &hnd_dst, level, &ctr_dst)) {
			...
			DEBUG1(("printer already exists: %s\n", sharename));
						...
						const char **argv)
{
	...
	DEBUG3(("copying printer settings\n"));
