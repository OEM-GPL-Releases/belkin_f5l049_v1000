	...
	switch (info->switch_value)
	{
		...
		case 2:
			...
			*/
			...
			break;
		default:
			DEBUG0(("Something is obviously wrong somewhere !\n"));
			break;
	}
...
static BOOL smb_io_notify_option_type_data(const char *desc, SPOOL_NOTIFY_OPTION_TYPE *type, prs_struct *ps, int depth)
{
	...
	if (type->count2 != type->count)
		DEBUG4(("What a mess, count was %x now is %x !\n", type->count, type->count2));
...
static BOOL smb_io_notify_info_data(const char *desc,SPOOL_NOTIFY_INFO_DATA *data, prs_struct *ps, int depth)
{
	...
	default:
		DEBUG3( ("invalid enc_type %d for smb_io_notify_info_data\n",
			  data->enc_type));
		break;
	}
                                     ...
                                     prs_struct *ps, int depth)
{
	...
	default:
		DEBUG3( ("invalid enc_type %d for smb_io_notify_info_data_strings\n",
			  data->enc_type));
		break;
	}
...
BOOL spoolss_io_devmode(const char *desc, prs_struct *ps, int depth, DEVICEMODE *devmode)
{
	...
	if ( available_space && ((available_space % sizeof(uint32)) != 0) ) {
		DEBUG0(("spoolss_io_devmode: available_space [%d] no in multiple of 4 bytes (size = %d)!\n",
			available_space, devmode->size));
		DEBUG0(("spoolss_io_devmode: please report to samba-technical@samba.org!\n"));
		return False;
	}
	...
	if (available_space) {
		DEBUG0(("spoolss_io_devmode: I've parsed all I know and there is still stuff left|\n"));
		DEBUG0(("spoolss_io_devmode: available_space = [%d], devmode_size = [%d]!\n",
			available_space, devmode->size));
		DEBUG0(("spoolss_io_devmode: please report to samba-technical@samba.org!\n"));
		return False;
	}
	...
	if (devmode->driverextra!=0) {
		if (UNMARSHALLING(ps)) {
			...
			if(devmode->dev_private == NULL)
				return False;
			DEBUG7(("spoolss_io_devmode: allocated memory [%d] for dev_private\n",devmode->driverextra)); 
		}
			
		DEBUG7(("spoolss_io_devmode: parsing [%d] bytes of dev_private\n",devmode->driverextra));
...
static BOOL spoolss_io_devmode_cont(const char *desc, DEVMODE_CTR *dm_c, prs_struct *ps, int depth)
{
	...
	if (UNMARSHALLING(ps)) {
		DEBUG9(("Allocating memory for spoolss_io_devmode\n"));
...
{
	DEBUG5(("make_spoolss_q_open_printer_ex\n"));
...
{
	DEBUG5(("make_spoolss_q_addprinterex\n"));
	...
	switch (level) {
		case 2:
			...
			if (!make_spoolss_printer_info_2(mem_ctx, &q_u->info.info_2, ctr->printers_2)) {
				DEBUG0(("make_spoolss_q_addprinterex: Unable to fill SPOOL_Q_ADDPRINTEREX struct!\n"));
				return False;
			}
			break;
		default :
				...
				PRINTER_INFO_2 *info)
{
	...
	if (!(inf=TALLOC_P(mem_ctx, SPOOL_PRINTER_INFO_LEVEL_2))) {
		DEBUG0(("make_spoolss_printer_info_2: Unable to allocate SPOOL_PRINTER_INFO_LEVEL_2 sruct!\n"));
		return False;
	}
				...
				PRINTER_INFO_3 *info)
{
	...
	if (!(inf=TALLOC_P(mem_ctx, SPOOL_PRINTER_INFO_LEVEL_3))) {
		DEBUG0(("make_spoolss_printer_info_3: Unable to allocate SPOOL_PRINTER_INFO_LEVEL_3 sruct!\n"));
		return False;
	}
				...
				PRINTER_INFO_7 *info)
{
	...
	if (!(inf=TALLOC_P(mem_ctx, SPOOL_PRINTER_INFO_LEVEL_7))) {
		DEBUG0(("make_spoolss_printer_info_7: Unable to allocate SPOOL_PRINTER_INFO_LEVEL_7 struct!\n"));
		return False;
	}
...
BOOL make_spoolss_q_deleteprinterdriverex( TALLOC_CTX *mem_ctx,
                                           ...
                                           int version)
{
	DEBUG5(("make_spoolss_q_deleteprinterdriverex\n"));
...
{
	DEBUG5(("make_spoolss_q_deleteprinterdriver\n"));
				   ...
				   const char *valuename, uint32 size)
{
        ...
        DEBUG5(("make_spoolss_q_getprinterdata\n"));
				     ...
				     const char *valuename, uint32 size)
{
        ...
        DEBUG5(("make_spoolss_q_getprinterdataex\n"));
...
BOOL make_spoolss_q_closeprinter(SPOOL_Q_CLOSEPRINTER *q_u, POLICY_HND *hnd)
{
	...
	DEBUG5(("make_spoolss_q_closeprinter\n"));
	...
	if (MARSHALLING(ps)) {
		...
		if (*devmode == NULL) {
			...
			if (!prs_uint32("offset", ps, depth, &relative_offset))
				return False;
			DEBUG8( ("boing, the devmode was NULL\n"));
				...
				uint32 command)
{
	...
	default: 
		DEBUG0(("make_spoolss_q_setprinter: Unknown info level [%d]\n", level));
			break;
	}
...
BOOL spoolss_io_q_setprinter(const char *desc, SPOOL_Q_SETPRINTER *q_u, prs_struct *ps, int depth)
{
	...
	switch ( q_u->level ) {
		...
		case 7:
			...
			break;
		default:
			DEBUG0(("spoolss_io_q_setprinter: unsupported printer info level [%d]\n", 
				q_u->level));
			return True;
	}
...
{
	DEBUG5(("make_spoolss_q_addprinterdriver\n"));
	...
	default:
		DEBUG0(("make_spoolss_q_addprinterdriver: Unknown info level [%d]\n", level));
		break;
	}
	...
	if (src) {
		if (len) {
			if((buf5->buffer=(uint16*)TALLOC_MEMDUP(mem_ctx, src, sizeof(uint16)*len)) == NULL) {
				DEBUG0(("make_spoolss_buffer5: Unable to malloc memory for buffer!\n"));
				return False;
			}
		} else {
			buf5->buffer = NULL;
                                ...
                                NT_PRINTER_DRIVER_INFO_LEVEL_3 **asc)
{
	...
	DEBUG7(("uni_2_asc_printer_driver_3: Converting from UNICODE to ASCII\n"));
                                ...
                                NT_PRINTER_DRIVER_INFO_LEVEL_6 **asc)
{
	...
	DEBUG7(("uni_2_asc_printer_driver_6: Converting from UNICODE to ASCII\n"));
...
{
	DEBUG7(("Converting from UNICODE to ASCII\n"));
...
{	
	...
	if (UNMARSHALLING(ps) && r_u->valuesize) {
		...
		if (!r_u->value) {
			DEBUG0( ("spoolss_io_r_enumprinterdata: out of memory for printerdata value\n"));
			return False;
		}
	}
	...
	if (UNMARSHALLING(ps) && r_u->datasize) {
		...
		if (!r_u->data) {
			DEBUG0( ("spoolss_io_r_enumprinterdata: out of memory for printerdata data\n"));
			return False;
		}
	}
				...
				SPOOL_NOTIFY_INFO_DATA *src, int n)
{
	...
	for (i=0; i<n; i++) {
		...
		if (s == NULL) {
			DEBUG0(("copy_spool_notify_info_data: malloc() failed!\n"));
			return False;
		}
...
{
	if (!dst) {
		DEBUG0(("copy_spool_notify_info: NULL destination pointer!\n"));
		return False;
	}
	...
	if (dst->count) 
	{
		...
		DEBUG10(("copy_spool_notify_info: allocating space for [%d] PRINTER_NOTIFY_INFO_DATA entries\n",
			dst->count));

		if (dst->data == NULL) {
			DEBUG0(("copy_spool_notify_info: malloc() failed for [%d] entries!\n", 
				dst->count));
			return False;
		}
...
{      
	...
	if (info->count) {
		DEBUG10(("make_spoolss_q_reply_rrpcn: [%d] PRINTER_NOTIFY_INFO_DATA\n",
			info->count));
...
BOOL make_spoolss_q_enumprinterkey(SPOOL_Q_ENUMPRINTERKEY *q_u, 
				   ...
				   uint32 size)
{
	DEBUG5(("make_spoolss_q_enumprinterkey\n"));
...
{
	DEBUG5(("make_spoolss_q_deleteprinterkey\n"));
...
{
	DEBUG5(("make_spoolss_q_getprintprocessordirectory\n"));
	...
	case 2:
		/* DOC_INFO_2 is only used by Windows 9x and since it
	           doesn't do printing over RPC we don't have to worry
  	           about it. */
	default:
		DEBUG3( ("unsupported info level %d\n", level));
		return False;
	}
...
BOOL convert_port_data_1( NT_PORT_DATA_1 *port1, RPC_BUFFER *buf ) 
{
	...
	case 2:
		...
		break;
	default:
		DEBUG3(("convert_port_data_1: unknown protocol [%d]!\n", 
			spdata_1.protocol));
		return False;
	}
