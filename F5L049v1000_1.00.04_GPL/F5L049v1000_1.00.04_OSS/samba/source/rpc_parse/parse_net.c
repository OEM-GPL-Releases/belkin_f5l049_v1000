...
BOOL net_io_q_logon_ctrl2(const char *desc, NET_Q_LOGON_CTRL2 *q_l, prs_struct *ps, int depth)
{
	...
	switch ( q_l->function_code ) {
		...
		default:
			DEBUG0(("net_io_q_logon_ctrl2: unknown function_code [%d]\n",
				q_l->function_code));
			return False;
	}
			    ...
			    const char *trusted_domain_name)
{
	...
	case 3:
		...
		break;
	default:
		DEBUG2(("init_r_logon_ctrl2: unsupported switch value %d\n",
			r_l->switch_value));
...
BOOL net_io_r_logon_ctrl2(const char *desc, NET_R_LOGON_CTRL2 *r_l, prs_struct *ps, int depth)
{
	...
	if (r_l->ptr != 0) {
		...
		case 3:
			...
			break;
		default:
			DEBUG2(("net_io_r_logon_ctrl2: unsupported switch value %d\n",
				r_l->switch_value));
			break;
		}
	}
			   ...
			   uint32 flags, uint32 pdc_status)
{
	...
	case 1:
		...
		break;
	default:
		DEBUG2(("init_r_logon_ctrl: unsupported switch value %d\n",
			r_l->switch_value));
		r_l->ptr = 0; /* undocumented pointer */
			 ...
			 int depth)
{
	...
	if (r_l->ptr != 0) {
		...
		case 1:
			...
			break;
		default:
			DEBUG2(("net_io_r_logon_ctrl: unsupported switch value %d\n",
				r_l->switch_value));
			break;
		}
	}
...
static BOOL net_io_id_info_ctr(const char *desc, NET_ID_INFO_CTR **pp_ctr, prs_struct *ps, int depth)
{
	...
	default:
		/* PANIC! */
		DEBUG4(("smb_io_sam_info_ctr: unknown switch_value!\n"));
		break;
	}
		       ...
		       int depth, uint16 validation_level, BOOL kerb_validation_level)
{
	...
	if (usr->num_groups != usr->num_groups2) {
		DEBUG3(("net_io_user_info3: num_groups mismatch! (%d != %d)\n", 
	...
	if (validation_level == 3 && usr->buffer_other_sids) {
		...
		if (!(usr->user_flgs & LOGON_EXTRA_SIDS)) {
			DEBUG10(("net_io_user_info3: user_flgs attribute does not have LOGON_EXTRA_SIDS\n"));
			/* return False; */
		}
	...
	if (info->ptr_rids != 0)
	{
		...
		if (UNMARSHALLING(ps)) {
			if (info->num_members2) {
				...
				if (info->rids == NULL) {
					DEBUG0( ("out of memory allocating %d rids\n",
						info->num_members2));
					return False;
				}
			} else {
		...
		if (UNMARSHALLING(ps)) {
			if (info->num_members3) {
				...
				if (info->attribs == NULL) {
					DEBUG0( ("out of memory allocating %d attribs\n",
						info->num_members3));
					return False;
				}
			} else {
	...
	if (info->ptr_members != 0)
	{
		...
		if (UNMARSHALLING(ps)) {
			if (info->num_sids) {
				...
				if (info->ptr_sids == NULL) {
					DEBUG0( ("out of memory allocating %d ptr_sids\n",
						info->num_sids));
					return False;
				}
			} else {
		...
		if (UNMARSHALLING(ps)) {
			if (info->num_sids) {
				...
				if (info->sids == NULL) {
					DEBUG0( ("error allocating %d sids\n",
						info->num_sids));
					return False;
				}
			} else {
				 ...
				 prs_struct *ps, int depth)
{
	...
	switch (type) {
		...
		default:
			DEBUG0( ("Replication error: Unknown delta type 0x%x\n", type));
			break;
	}
		...
		if (r_s->ptr_deltas2 != 0)
		{
			...
			if (UNMARSHALLING(ps)) {
					...
					if (r_s->hdr_deltas == NULL) {
						DEBUG0( ("error tallocating memory "
							"for %d delta headers\n", 
							r_s->num_deltas2));
						return False;
					}
...
void init_net_q_dsr_getdcname(NET_Q_DSR_GETDCNAME *r_t, const char *server_unc,
			      ...
			      uint32_t flags)
{
	DEBUG5( ("init_net_q_dsr_getdcname\n"));
...
void init_net_q_dsr_getdcnameex(NET_Q_DSR_GETDCNAMEEX *r_t, const char *server_unc,
				...
				uint32_t flags)
{
	DEBUG5( ("init_net_q_dsr_getdcnameex\n"));
...
void init_net_q_dsr_getdcnameex2(NET_Q_DSR_GETDCNAMEEX2 *r_t, const char *server_unc,
				 ...
				 uint32_t flags)
{
	DEBUG5( ("init_net_q_dsr_getdcnameex2\n"));
...
{
	DEBUG5( ("init_net_q_dsr_getsitename\n"));
