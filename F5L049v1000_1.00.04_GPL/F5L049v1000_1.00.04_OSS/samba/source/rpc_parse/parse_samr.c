...
{
	DEBUG5( ("init_samr_q_lookup_domain\n"));
...
{
	DEBUG5( ("init_samr_r_lookup_domain\n"));
...
{
	DEBUG5( ("samr_init_samr_q_remove_sid_foreign_domain\n"));
...
void init_samr_q_open_domain(SAMR_Q_OPEN_DOMAIN * q_u,
			     ...
			     const DOM_SID *sid)
{
	DEBUG5( ("samr_init_samr_q_open_domain\n"));
...
void init_samr_q_get_usrdom_pwinfo(SAMR_Q_GET_USRDOM_PWINFO * q_u,
				   POLICY_HND *user_pol)
{
	DEBUG5( ("samr_init_samr_q_get_usrdom_pwinfo\n"));
...
{
	DEBUG5( ("init_samr_r_get_usrdom_pwinfo\n"));
...
{
	DEBUG5( ("samr_init_samr_q_query_sec_obj\n"));
...
{
	DEBUG5( ("samr_init_samr_q_query_domain_info\n"));
...
void init_samr_r_query_domain_info(SAMR_R_QUERY_DOMAIN_INFO * r_u,
				   ...
				   NTSTATUS status)
{
	DEBUG5( ("init_samr_r_query_domain_info\n"));
				 ...
				 prs_struct *ps, int depth)
{
	...
	if (r_u->ptr_0 != 0 && r_u->ctr != NULL) {
		...
		case 0x01:
			...
			break;
		default:
			DEBUG0( ("samr_io_r_query_domain_info: unknown switch level 0x%x\n",
				r_u->switch_value));
...
{
	DEBUG5( ("samr_init_samr_q_set_sec_obj\n"));
...
{
	DEBUG5( ("init_samr_q_enum_dom_users\n"));
...
{
	DEBUG5( ("init_samr_r_enum_dom_users\n"));
			      ...
			      prs_struct *ps, int depth)
{
	...
	if (r_u->ptr_entries1 != 0) {
		...
		if ((r_u->sam == NULL || r_u->uni_acct_name == NULL) && r_u->num_entries2 != 0) {
			DEBUG0(("NULL pointers in SAMR_R_ENUM_DOM_USERS\n"));
			r_u->num_entries4 = 0;
...
{
	DEBUG5( ("init_samr_q_query_dispinfo\n"));
			     ...
			     struct samr_displayentry *entries)
{
	...
	DEBUG10( ("init_sam_dispinfo_1: num_entries: %d\n", num_entries));
				  ...
				  prs_struct *ps, int depth)
{
	...
	if (UNMARSHALLING(ps) && num_entries > 0) {
		...
		if ((sam->sam = PRS_ALLOC_MEM(ps, SAM_ENTRY1, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_ENTRY1\n"));
			return False;
		}

		if ((sam->str = PRS_ALLOC_MEM(ps, SAM_STR1, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_STR1\n"));
			return False;
		}
	}
			     ...
			     struct samr_displayentry *entries)
{
	...
	DEBUG10( ("init_sam_dispinfo_2: num_entries: %d\n", num_entries));
				  ...
				  prs_struct *ps, int depth)
{
	...
	if (UNMARSHALLING(ps) && num_entries > 0) {
		...
		if ((sam->sam = PRS_ALLOC_MEM(ps, SAM_ENTRY2, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_ENTRY2\n"));
			return False;
		}

		if ((sam->str = PRS_ALLOC_MEM(ps, SAM_STR2, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_STR2\n"));
			return False;
		}
	}
			     ...
			     struct samr_displayentry *entries)
{
	...
	DEBUG5( ("init_sam_dispinfo_3: num_entries: %d\n", num_entries));
				  ...
				  prs_struct *ps, int depth)
{
	...
	if (UNMARSHALLING(ps) && num_entries > 0) {
		...
		if ((sam->sam = PRS_ALLOC_MEM(ps, SAM_ENTRY3, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_ENTRY3\n"));
			return False;
		}

		if ((sam->str = PRS_ALLOC_MEM(ps, SAM_STR3, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_STR3\n"));
			return False;
		}
	}
			     ...
			     struct samr_displayentry *entries)
{
	...
	DEBUG5( ("init_sam_dispinfo_4: num_entries: %d\n", num_entries));
				  ...
				  prs_struct *ps, int depth)
{
	...
	if (UNMARSHALLING(ps) && num_entries > 0) {
		...
		if ((sam->sam = PRS_ALLOC_MEM(ps, SAM_ENTRY4, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_ENTRY4\n"));
			return False;
		}

		if ((sam->str = PRS_ALLOC_MEM(ps, SAM_STR4, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_STR4\n"));
			return False;
		}
	}
			     ...
			     struct samr_displayentry *entries)
{
	...
	DEBUG5( ("init_sam_dispinfo_5: num_entries: %d\n", num_entries));
				  ...
				  prs_struct *ps, int depth)
{
	...
	if (UNMARSHALLING(ps) && num_entries > 0) {
		...
		if ((sam->sam = PRS_ALLOC_MEM(ps, SAM_ENTRY5, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_ENTRY5\n"));
			return False;
		}

		if ((sam->str = PRS_ALLOC_MEM(ps, SAM_STR5, num_entries)) == NULL) {
			DEBUG0( ("out of memory allocating SAM_STR5\n"));
			return False;
		}
	}
...
void init_samr_r_query_dispinfo(SAMR_R_QUERY_DISPINFO * r_u,
				...
				NTSTATUS status)
{
	DEBUG5( ("init_samr_r_query_dispinfo: level %d\n", switch_level));
			      ...
			      prs_struct *ps, int depth)
{
	...
	case 0x5:
		...
		break;
	default:
		DEBUG0(("samr_io_r_query_dispinfo: unknown switch value\n"));
		break;
	}
...
{
	DEBUG5( ("init_samr_q_get_dispenum_index\n"));
...
{
	DEBUG5( ("init_samr_r_get_dispenum_index\n"));
				...
				prs_struct *ps, int depth)
{
	...
	case 5:
		...
		break;
	default:
		DEBUG0(("samr_group_info_ctr: unsupported switch level\n"));
		break;
	}
...
void init_samr_q_create_dom_group(SAMR_Q_CREATE_DOM_GROUP * q_e,
				  ...
				  uint32 access_mask)
{
	DEBUG5( ("init_samr_q_create_dom_group\n"));
...
void init_samr_q_delete_dom_group(SAMR_Q_DELETE_DOM_GROUP * q_c,
				  POLICY_HND *hnd)
{
	DEBUG5( ("init_samr_q_delete_dom_group\n"));
...
{
	DEBUG5( ("init_samr_q_del_groupmem\n"));
...
void init_samr_r_del_groupmem(SAMR_R_DEL_GROUPMEM * r_u, POLICY_HND *pol,
			      NTSTATUS status)
{
	DEBUG5( ("init_samr_r_del_groupmem\n"));
...
void init_samr_q_add_groupmem(SAMR_Q_ADD_GROUPMEM * q_e,
			      POLICY_HND *pol, uint32 rid)
{
	DEBUG5( ("init_samr_q_add_groupmem\n"));
...
void init_samr_r_add_groupmem(SAMR_R_ADD_GROUPMEM * r_u, POLICY_HND *pol,
			      NTSTATUS status)
{
	DEBUG5( ("init_samr_r_add_groupmem\n"));
...
{
	DEBUG5( ("init_samr_q_set_groupinfo\n"));
...
{
	DEBUG5( ("init_samr_r_set_groupinfo\n"));
...
{
	DEBUG5( ("init_samr_q_query_groupinfo\n"));
...
{
	DEBUG5( ("init_samr_r_query_groupinfo\n"));
...
{
	DEBUG5( ("init_samr_q_query_groupmem\n"));
...
{
	DEBUG5( ("init_samr_r_query_groupmem\n"));
...
void init_samr_q_query_usergroups(SAMR_Q_QUERY_USERGROUPS * q_u,
				  POLICY_HND *hnd)
{
	DEBUG5( ("init_samr_q_query_usergroups\n"));
...
void init_samr_r_query_usergroups(SAMR_R_QUERY_USERGROUPS * r_u,
				  ...
				  NTSTATUS status)
{
	DEBUG5( ("init_samr_r_query_usergroups\n"));
...
{
	DEBUG5( ("init_samr_q_enum_domains\n"));
...
void init_samr_r_enum_domains(SAMR_R_ENUM_DOMAINS * r_u,
			      uint32 next_idx, uint32 num_sam_entries)
{
	DEBUG5( ("init_samr_r_enum_domains\n"));
			    ...
			    prs_struct *ps, int depth)
{
	...
	if (r_u->ptr_entries1 != 0) {
		...
		if ((r_u->sam == NULL || r_u->uni_dom_name == NULL) && r_u->num_entries2 != 0) {
			DEBUG0( ("NULL pointers in SAMR_R_ENUM_DOMAINS\n"));
			r_u->num_entries4 = 0;
			r_u->status = NT_STATUS_MEMORY_NOT_ALLOCATED;
			return False;
		}
...
void init_samr_q_enum_dom_groups(SAMR_Q_ENUM_DOM_GROUPS * q_e,
				 ...
				 uint32 start_idx, uint32 size)
{
	DEBUG5( ("init_samr_q_enum_dom_groups\n"));
...
void init_samr_r_enum_dom_groups(SAMR_R_ENUM_DOM_GROUPS * r_u,
				 uint32 next_idx, uint32 num_sam_entries)
{
	DEBUG5( ("init_samr_r_enum_dom_groups\n"));
...
void init_samr_q_enum_dom_aliases(SAMR_Q_ENUM_DOM_ALIASES * q_e,
				  ...
				  uint32 size)
{
	DEBUG5( ("init_samr_q_enum_dom_aliases\n"));
...
{
	DEBUG5( ("init_samr_r_enum_dom_aliases\n"));
				...
				prs_struct *ps, int depth)
{
	...
	if (r_u->ptr_entries1 != 0) {
		...
		if (r_u->num_entries2 != 0 && 
			...
			DEBUG0(("NULL pointers in SAMR_R_ENUM_DOM_ALIASES\n"));
			r_u->num_entries4 = 0;
			r_u->status = NT_STATUS_MEMORY_NOT_ALLOCATED;
			return False;
		}
...
BOOL samr_alias_info_ctr(const char *desc, prs_struct *ps, int depth, ALIAS_INFO_CTR * ctr)
{
	...
	case 3: 
		...
		break;
	default:
		DEBUG0(("samr_alias_info_ctr: unsupported switch level\n"));
		break;
	}
...
{
	DEBUG5( ("init_samr_q_query_aliasinfo\n"));
...
{
	DEBUG5( ("init_samr_r_query_aliasinfo\n"));
...
{
	DEBUG5( ("init_samr_q_set_aliasinfo\n"));
...
{
	DEBUG5( ("init_samr_q_query_useraliases\n"));
...
void init_samr_r_query_useraliases(SAMR_R_QUERY_USERALIASES * r_u,
				   ...
				   NTSTATUS status)
{
	DEBUG5( ("init_samr_r_query_useraliases\n"));
...
void init_samr_q_lookup_rids(TALLOC_CTX *ctx, SAMR_Q_LOOKUP_RIDS * q_u,
			     ...
			     uint32 num_rids, uint32 *rid)
{
	DEBUG5( ("init_samr_q_lookup_rids\n"));
...
{
	DEBUG5( ("init_samr_r_lookup_rids\n"));
...
{
	DEBUG5( ("init_samr_q_delete_alias\n"));
...
{
	DEBUG5( ("init_samr_q_create_dom_alias\n"));
...
void init_samr_q_add_aliasmem(SAMR_Q_ADD_ALIASMEM * q_u, POLICY_HND *hnd,
			      DOM_SID *sid)
{
	DEBUG5( ("init_samr_q_add_aliasmem\n"));
...
void init_samr_q_del_aliasmem(SAMR_Q_DEL_ALIASMEM * q_u, POLICY_HND *hnd,
			      DOM_SID *sid)
{
	DEBUG5( ("init_samr_q_del_aliasmem\n"));
...
void init_samr_q_delete_dom_alias(SAMR_Q_DELETE_DOM_ALIAS * q_c,
				  POLICY_HND *hnd)
{
	DEBUG5( ("init_samr_q_delete_dom_alias\n"));
...
void init_samr_r_delete_dom_alias(SAMR_R_DELETE_DOM_ALIAS * r_u,
				  NTSTATUS status)
{
	DEBUG5( ("init_samr_r_delete_dom_alias\n"));
...
void init_samr_q_query_aliasmem(SAMR_Q_QUERY_ALIASMEM * q_c,
				POLICY_HND *hnd)
{
	DEBUG5( ("init_samr_q_query_aliasmem\n"));
...
void init_samr_r_query_aliasmem(SAMR_R_QUERY_ALIASMEM * r_u,
				...
				NTSTATUS status)
{
	DEBUG5( ("init_samr_r_query_aliasmem\n"));
			      ...
			      uint32 num_names, const char **name)
{
	...
	DEBUG5( ("init_samr_q_lookup_names\n"));
...
NTSTATUS init_samr_r_lookup_names(TALLOC_CTX *ctx, SAMR_R_LOOKUP_NAMES * r_u,
			      ...
			      NTSTATUS status)
{
	DEBUG5( ("init_samr_r_lookup_names\n"));
			    ...
			    prs_struct *ps, int depth)
{
	...
	if (r_u->ptr_rids != 0)	{
		...
		if (UNMARSHALLING(ps) && r_u->num_rids2) {
			...
			if (!r_u->rids) {
				DEBUG0( ("NULL rids in samr_io_r_lookup_names\n"));
				return False;
			}
		}
	...
	if (r_u->ptr_types != 0) {
		...
		if (UNMARSHALLING(ps) && r_u->num_types2) {
			...
			if (!r_u->types) {
				DEBUG0( ("NULL types in samr_io_r_lookup_names\n"));
				return False;
			}
		}
...
void init_samr_q_delete_dom_user(SAMR_Q_DELETE_DOM_USER * q_c,
				 POLICY_HND *hnd)
{
	DEBUG5( ("init_samr_q_delete_dom_user\n"));
...
void init_samr_q_open_user(SAMR_Q_OPEN_USER * q_u,
			   ...
			   uint32 access_mask, uint32 rid)
{
	DEBUG5( ("samr_init_samr_q_open_user\n"));
...
{
	DEBUG5( ("samr_init_samr_q_create_user\n"));
...
{
	DEBUG5( ("init_samr_q_query_userinfo\n"));
			     ...
			     prs_struct *ps, int depth)
{
	...
	if (hrs->len > sizeof(hrs->hours)) {
		DEBUG3( ("sam_io_logon_hrs: truncating length from %d\n", hrs->len));
...
NTSTATUS init_sam_user_info21A(SAM_USER_INFO_21 *usr, struct samu *pw, DOM_SID *domain_sid)
{
	...
	if (!sid_peek_check_rid(domain_sid, user_sid, &user_rid)) {
		...
		DEBUG0( ("init_sam_user_info_21A: User %s has SID %s, \nwhich conflicts with "
	...
	if (!sid_peek_check_rid(domain_sid, group_sid, &group_rid)) {
		...
		fstring domain_sid_string;
		DEBUG0( ("init_sam_user_info_21A: User %s has Primary Group SID %s, \n"
...
{
	DEBUG5( ("make_samr_userinfo_ctr_usr21\n"));
...
/* whoops - got this wrong.  i think.  or don't understand what's happening. */
	...
	default:
		DEBUG4(("make_samr_userinfo_ctr: unsupported info\n"));
				   ...
				   uint16 switch_value, void *info)
{
	...
	case 0x07:
		break;
	default:
		DEBUG4(("init_samr_userinfo_ctr: unsupported switch level: %d\n", switch_value));
	}
}
				 ...
				 prs_struct *ps, int depth)
{
	...
	case 7:
		...
		if (ctr->info.id7 == NULL) {
			DEBUG2(("samr_io_userinfo_ctr: info pointer not initialised\n"));
			return False;
		}
	...
	case 26:
		...
		break;
	default:
		DEBUG2( ("samr_io_userinfo_ctr: unknown switch level 0x%x\n", ctr->switch_value));
		ret = False;
		break;
	}
...
{
	DEBUG5( ("init_samr_r_query_userinfo\n"));
...
void init_samr_q_set_userinfo(SAMR_Q_SET_USERINFO * q_u,
			      ...
			      uint16 switch_value, void *info)
{
	DEBUG5( ("init_samr_q_set_userinfo\n"));
...
{
	DEBUG5( ("init_samr_r_set_userinfo\n"));
...
{
	DEBUG5( ("init_samr_q_set_userinfo2\n"));
...
{
	DEBUG5( ("init_samr_r_set_userinfo2\n"));
...
{
	DEBUG5( ("init_samr_q_connect_anon\n"));
...
void init_samr_q_get_dom_pwinfo(SAMR_Q_GET_DOM_PWINFO * q_u,
				char *srv_name)
{
	DEBUG5( ("init_samr_q_get_dom_pwinfo\n"));
...
{
	DEBUG5( ("init_samr_q_chgpasswd_user\n"));
...
{
	DEBUG5( ("init_samr_r_chgpasswd_user\n"));
...
void init_samr_q_chgpasswd_user3(SAMR_Q_CHGPASSWD_USER3 * q_u,
				 ...
				 const uchar lm_oldhash[16])
{
	DEBUG5( ("init_samr_q_chgpasswd_user3\n"));
...
{
	DEBUG5( ("init_samr_r_chgpasswd_user3\n"));
...
void init_samr_q_query_domain_info2(SAMR_Q_QUERY_DOMAIN_INFO2 *q_u,
				POLICY_HND *domain_pol, uint16 switch_value)
{
	DEBUG5( ("init_samr_q_query_domain_info2\n"));
...
void init_samr_r_query_domain_info2(SAMR_R_QUERY_DOMAIN_INFO2 * r_u,
				    ...
				    NTSTATUS status)
{
	DEBUG5( ("init_samr_r_query_domain_info2\n"));
				  ...
				  prs_struct *ps, int depth)
{
	...
	if (r_u->ptr_0 != 0 && r_u->ctr != NULL) {
		...
		case 0x01:
			...
			break;
		default:
			DEBUG0( ("samr_io_r_query_domain_info2: unknown switch level 0x%x\n",
				r_u->switch_value));
			r_u->status = NT_STATUS_INVALID_INFO_CLASS;
			return False;
		}
...
{
	DEBUG5( ("init_samr_q_set_domain_info\n"));
			      ...
			      prs_struct *ps, int depth)
{
	...
	case 0x01:
		...
		break;
	default:
		DEBUG0( ("samr_io_r_samr_unknown_2e: unknown switch level 0x%x\n",
			q_u->switch_value));
		return False;
	}
...
{
	DEBUG5( ("init_samr_r_set_domain_info\n"));
