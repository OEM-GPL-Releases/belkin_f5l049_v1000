		     ...
		     int from_off, int to_off)
{
	...
	if (fd != -1) {
		...
		if ( (sz != to_off-from_off) || (i != 0) ) {
			DEBUG0(("Error writing/closing %s: %ld!=%ld %d\n", fname, (unsigned long)sz, (unsigned long)to_off-from_off, i ));
		} else {
			DEBUG0(("created %s\n", fname));
		}
	}
...
BOOL prs_init(prs_struct *ps, uint32 size, TALLOC_CTX *ctx, BOOL io)
{
	...
	if (size != 0) {
		...
		if((ps->data_p = (char *)SMB_MALLOC((size_t)size)) == NULL) {
			DEBUG0(("prs_init: malloc fail for %u bytes.\n", (unsigned int)size));
			return False;
		}
...
BOOL prs_set_buffer_size(prs_struct *ps, uint32 newsize)
{
	...
	if (newsize < ps->buffer_size) {
		...
		if (newsize == 0) {
			...
			if (ps->data_p == NULL) {
				DEBUG0(("prs_set_buffer_size: Realloc failure for size %u.\n",
					(unsigned int)newsize));
				DEBUG0(("prs_set_buffer_size: Reason %s\n",strerror(errno)));
				return False;
			}
		}
	}
...
BOOL prs_grow(prs_struct *ps, uint32 extra_space)
{
	...
	if(UNMARSHALLING(ps) || !ps->is_dynamic) {
		DEBUG0(("prs_grow: Buffer overflow - unable to expand buffer by %u bytes.\n",
	...
	if(ps->buffer_size == 0) {
		...
		if((ps->data_p = (char *)SMB_MALLOC(new_size)) == NULL) {
			DEBUG0(("prs_grow: Malloc failure for size %u.\n", (unsigned int)new_size));
			return False;
		}
		...
		if ((ps->data_p = (char *)SMB_REALLOC(ps->data_p, new_size)) == NULL) {
			DEBUG0(("prs_grow: Realloc failure for size %u.\n",
				(unsigned int)new_size));
			return False;
		}
...
BOOL prs_force_grow(prs_struct *ps, uint32 extra_space)
{
	...
	if(!UNMARSHALLING(ps) || !ps->is_dynamic) {
		DEBUG0(("prs_force_grow: Buffer overflow - unable to expand buffer by %u bytes.\n",
				(unsigned int)extra_space));
		return False;
	}

	if((ps->data_p = (char *)SMB_REALLOC(ps->data_p, new_size)) == NULL) {
		DEBUG0(("prs_force_grow: Realloc failure for size %u.\n",
			(unsigned int)new_size));
		return False;
	}
...
char *prs_mem_get(prs_struct *ps, uint32 extra_size)
{
	if(UNMARSHALLING(ps)) {
		...
		if (ps->data_offset + extra_size > ps->buffer_size) {
			DEBUG0(("prs_mem_get: reading data of size %u would overrun "
				"buffer by %u bytes.\n",
...
BOOL prs_uint8s(BOOL charmode, const char *name, prs_struct *ps, int depth, uint8 *data8s, int len)
{
	...
	DEBUG5(("%s%04x %s: ", tab_depth(depth), ps->data_offset ,name));
	if (charmode)
		   ...
		   char *data, size_t data_len)
{
	...
	DEBUG10(("SCHANNEL: schannel_encode seq_num=%d data_len=%lu\n", a->seq_num, (unsigned long)data_len));
		   ...
		   RPC_AUTH_SCHANNEL_CHK * verf, char *data, size_t data_len)
{
	...
	DEBUG10(("SCHANNEL: schannel_decode seq_num=%d data_len=%lu\n", a->seq_num, (unsigned long)data_len));
	...
	if (memcmp(verf->seq_num, seq_num, sizeof(seq_num))) {
		...
		*/
		DEBUG2( ("schannel_decode: FAILED: packet sequence number:\n"));
	...
	if (memcmp(verf->sig, schannel_sig, sizeof(verf->sig))) {
		...
		DEBUG2( ("schannel_decode: FAILED: packet header:\n"));
