 ...
 BOOL init_group_mapping(void)
{
	...
	if (!tdb) {
		DEBUG0(("Failed to open group mapping database\n"));
		return False;
	}
 ...
 BOOL get_group_map_from_sid(DOM_SID sid, GROUP_MAP *map)
{
	...
	if ( ret == -1 ) {
		DEBUG3(("get_group_map_from_sid: tdb_unpack failure\n"));
		return False;
	}
 ...
 BOOL get_group_map_from_gid(gid_t gid, GROUP_MAP *map)
{
	...
	for (kbuf = tdb_firstkey(tdb); 
		...
		if ( ret == -1 ) {
			DEBUG3(("get_group_map_from_gid: tdb_unpack failure\n"));
			return False;
		}
 ...
 BOOL get_group_map_from_ntname(const char *name, GROUP_MAP *map)
{
	...
	if(!init_group_mapping()) {
		DEBUG0(("get_group_map_from_ntname:failed to initialize group mapping\n"));
		return(False);
	}
	...
	for (kbuf = tdb_firstkey(tdb); 
		...
		if ( ret == -1 ) {
			DEBUG3(("get_group_map_from_ntname: tdb_unpack failure\n"));
			return False;
		}
			...
			size_t *p_num_entries, BOOL unix_only)
{
	...
	*pp_rmap=NULL;
	...
	for (kbuf = tdb_firstkey(tdb); 
		...
		if ( ret == -1 ) {
			DEBUG3(("enum_group_mapping: tdb_unpack failure\n"));
			continue;
		}
		...
		if (!(*pp_rmap)) {
			DEBUG0(("enum_group_mapping: Unable to enlarge group map!\n"));
			return False;
		}
