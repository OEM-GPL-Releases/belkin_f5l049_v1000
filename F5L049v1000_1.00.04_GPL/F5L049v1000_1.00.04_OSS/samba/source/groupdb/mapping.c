...
NTSTATUS add_initial_entry(gid_t gid, const char *sid, enum lsa_SidType sid_name_use, const char *nt_name, const char *comment)
{
	...
	if(!init_group_mapping()) {
		DEBUG0(("failed to initialize group mapping\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	
	map.gid=gid;
	if (!string_to_sid(&map.sid, sid)) {
		DEBUG0( ("string_to_sid failed: %s", sid));
		return NT_STATUS_UNSUCCESSFUL;
	}
...
NTSTATUS map_unix_group(const struct group *grp, GROUP_MAP *pmap)
{
	...
	if (lookup_name(tmp_talloc_ctx(), grpname, LOOKUP_NAME_ISOLATED,
		...
		DEBUG5( ("%s exists as %s\\%s, retrying as \"%s\"\n",
	...
	if (pdb_rid_algorithm()) {
		...
		if (!pdb_new_rid(&rid)) {
			DEBUG3( ("Could not get a new RID for %s\n",
				  grp->gr_name));
...
BOOL get_domain_group_from_sid(DOM_SID sid, GROUP_MAP *map)
{
	...
	DEBUG10( ("get_domain_group_from_sid\n"));
	...
	DEBUG10( ("get_domain_group_from_sid: SID found in the TDB\n"));
	...
	DEBUG10( ("get_domain_group_from_sid: SID is a domain group\n"));
 	
	if (map->gid==-1) {
		return False;
	}

	DEBUG10( ("get_domain_group_from_sid: SID is mapped to gid:%lu\n",(unsigned long)map->gid));
	
	grp = getgrgid(map->gid);
	if ( !grp ) {
		DEBUG10( ("get_domain_group_from_sid: gid DOESN'T exist in UNIX security\n"));
		return False;
	}

	DEBUG10( ("get_domain_group_from_sid: gid exists in UNIX security\n"));
				  ...
				  const char *name, uint32 *rid)
{
	...
	DEBUG10( ("Trying to create alias %s\n", name));
	...
	if (!winbind_allocate_gid(&gid)) {
		DEBUG3( ("Could not get a gid out of winbind\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	if (!pdb_new_rid(&new_rid)) {
		DEBUG0( ("Could not allocate a RID -- wasted a gid :-(\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	DEBUG10( ("Creating alias %s with gid %d and rid %d\n",
		   name, gid, new_rid));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG0( ("Could not add group mapping entry for alias %s "
				   ...
				   struct acct_info *info)
{
	...
	if ((map.sid_name_use != SID_NAME_ALIAS) &&
		...
		DEBUG2( ("%s is a %s, expected an alias\n",
...
NTSTATUS pdb_create_builtin_alias(uint32 rid)
{
	...
	DEBUG10( ("Trying to create builtin alias %d\n", rid));
	...
	if (!winbind_allocate_gid(&gid)) {
		DEBUG3( ("pdb_create_builtin_alias: Could not get a gid out of winbind\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	DEBUG10(("Creating alias %s with gid %d\n", groupname, gid));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG0( ("pdb_create_builtin_alias: Could not add group mapping entry for alias %d "
