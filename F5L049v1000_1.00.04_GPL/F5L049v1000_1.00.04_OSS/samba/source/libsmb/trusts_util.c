					 ...
					 uint32 sec_channel_type)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG3(("just_change_the_password: unable to setup creds (%s)!\n",
			 nt_errstr(result)));
		return result;
	}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG0(("just_change_the_password: unable to change password (%s)!\n",
			 nt_errstr(result)));
	}
	return result;
}
				      ...
				      uint32 sec_channel_type)
{
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		DEBUG3(("%s : trust_pw_change_and_store_it: Changed password.\n", 
					   ...
					   const char *domain) 
{
	...
	if (!secrets_fetch_trust_account_password(domain,
		...
		DEBUG0( ("could not fetch domain secrets for domain %s!\n", domain));
				     ...
				     DOM_SID **sids )
{
	...
	*sids = NULL;
	...
	if ( !get_dc_name(domain, NULL, dc_name, &dc_ip) ) {
		DEBUG3(("enumerate_domain_trusts: can't locate a DC for domain %s\n",
			domain));
		return False;
	}
	...
	/* cleanup */
	if (cli) {
		DEBUG10(("enumerate_domain_trusts: shutting down connection...\n"));
		cli_shutdown( cli );
	}
