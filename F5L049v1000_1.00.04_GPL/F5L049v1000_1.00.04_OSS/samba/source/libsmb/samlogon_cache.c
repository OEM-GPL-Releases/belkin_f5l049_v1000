...
void netsamlogon_clear_cached_user(TDB_CONTEXT *tdb, NET_USER_INFO_3 *user)
{
	...
	if (!tdb) {
		...
		if (!tdb) {
			DEBUG5( ("netsamlogon_clear_cached_user: failed to open cache\n"));
			return;
		}
		got_tdb = True;
	}
	...
	DEBUG10( ("netsamlogon_clear_cached_user: clearing %s\n", key_str));
...
BOOL netsamlogon_cache_store( const char *username, NET_USER_INFO_3 *user )
{
	...
	if (!netsamlogon_cache_init()) {
		DEBUG0(("netsamlogon_cache_store: cannot open %s for write!\n", NETSAMLOGON_TDB));
		return False;
	}
	...
	DEBUG10(("netsamlogon_cache_store: SID [%s]\n", keystr));
	...
	if ( !(mem_ctx = TALLOC_P( NULL, int )) ) {
		DEBUG0(("netsamlogon_cache_store: talloc() failed!\n"));
		return False;
	}
...
NET_USER_INFO_3* netsamlogon_cache_get( TALLOC_CTX *mem_ctx, const DOM_SID *user_sid)
{
	...
	if (!netsamlogon_cache_init()) {
		DEBUG0(("netsamlogon_cache_get: cannot open %s for write!\n", NETSAMLOGON_TDB));
		return False;
	}

	/* Prepare key as DOMAIN-SID/USER-RID string */
	slprintf(keystr, sizeof(keystr), "%s", sid_string_static(user_sid));
	DEBUG10(("netsamlogon_cache_get: SID [%s]\n", keystr));
	key.dptr = keystr;
	   ...
	   --jerry */
	{
		...
		if ( (time_diff < 0 ) || (time_diff > lp_winbind_cache_time()) ) {
			DEBUG10(("netsamlogon_cache_get: cache entry expired \n"));
