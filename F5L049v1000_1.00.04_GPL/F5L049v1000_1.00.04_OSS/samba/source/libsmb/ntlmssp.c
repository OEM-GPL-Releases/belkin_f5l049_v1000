...
{
	DEBUG3(("Got NTLMSSP neg_flags=0x%08x\n", neg_flags));
			...
			const DATA_BLOB in, DATA_BLOB *out) 
{
	...
	if (ntlmssp_state->expected_state == NTLMSSP_DONE) {
		...
		DEBUG1( ("Called NTLMSSP after state machine was 'done'\n"));
	...
	*out = data_blob(NULL, 0);
	...
	if (!input.length) {
		...
		if (!msrpc_parse(&input, "Cd",
				 "NTLMSSP",
				 &ntlmssp_command)) {
			DEBUG1( ("Failed to parse NTLMSSP packet, could not extract NTLMSSP command\n"));
	...
	if (ntlmssp_command != ntlmssp_state->expected_state) {
		DEBUG1( ("got NTLMSSP command %u, expected %u\n", ntlmssp_command, ntlmssp_state->expected_state));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	DEBUG1( ("failed to find NTLMSSP callback for NTLMSSP mode %u, command %u\n", 
					 ...
					 const DATA_BLOB request, DATA_BLOB *reply) 
{
	...
	if (request.length) {
		if ((request.length < 16) || !msrpc_parse(&request, "Cdd",
							...
							&ntlmssp_command,
							&neg_flags)) {
			DEBUG1( ("ntlmssp_server_negotiate: failed to parse NTLMSSP Negotiate of length %u\n",
	...
	*reply = data_blob(NULL, 0);
	...
	if (!msrpc_parse(&request, parse_string,
		...
		if (!msrpc_parse(&request, parse_string,
				 ...
				 &user, 
				 &workstation)) {
			DEBUG1( ("ntlmssp_server_auth: failed to parse NTLMSSP (tried both formats):\n"));
			dump_data(2, (const char *)request.data, request.length);
			SAFE_FREE(domain);
			SAFE_FREE(user);
			SAFE_FREE(workstation);
	...
	DEBUG3(("Got user=[%s] domain=[%s] workstation=[%s] len1=%lu len2=%lu\n",
	...
	*/
	...
	if (doing_ntlm2) {
		if (user_session_key.data && user_session_key.length == 16) {
			...
			DEBUG10(("ntlmssp_server_auth: Created NTLM2 session key.\n"));
			dump_data_pw("NTLM2 session key:\n", session_key.data, session_key.length);
			
		} else {
			DEBUG10(("ntlmssp_server_auth: Failed to create NTLM2 session key.\n"));
		...
		if (lm_session_key.data && lm_session_key.length >= 8) {
			if (ntlmssp_state->lm_resp.data && ntlmssp_state->lm_resp.length == 24) {
				...
				SMBsesskeygen_lm_sess_key(lm_session_key.data, ntlmssp_state->lm_resp.data, 
							  session_key.data);
				DEBUG10(("ntlmssp_server_auth: Created NTLM session key.\n"));
			} else {
			...
			DEBUG10(("ntlmssp_server_auth: Failed to create NTLM session key.\n"));
			session_key = data_blob(NULL, 0);
		}
	} else if (user_session_key.data) {
		session_key = user_session_key;
		DEBUG10(("ntlmssp_server_auth: Using unmodified nt session key.\n"));
		dump_data_pw("unmodified session key:\n", session_key.data, session_key.length);
	} else if (lm_session_key.data) {
		session_key = lm_session_key;
		DEBUG10(("ntlmssp_server_auth: Using unmodified lm session key.\n"));
		dump_data_pw("unmodified session key:\n", session_key.data, session_key.length);
	} else {
		DEBUG10(("ntlmssp_server_auth: Failed to create unmodified session key.\n"));
		session_key = data_blob(NULL, 0);
	}
	...
	if (ntlmssp_state->neg_flags & NTLMSSP_NEGOTIATE_KEY_EXCH) {
		if (!encrypted_session_key.data || encrypted_session_key.length != 16) {
			data_blob_free(&encrypted_session_key);
			DEBUG1( ("Client-supplied KEY_EXCH session key was of invalid length (%u)!\n", 
				  (unsigned int)encrypted_session_key.length));
			return NT_STATUS_INVALID_PARAMETER;
		} else if (!session_key.data || session_key.length != 16) {
			DEBUG5( ("server session key is invalid (len == %u), cannot do KEY_EXCH!\n", 
...
NTSTATUS ntlmssp_server_start(NTLMSSP_STATE **ntlmssp_state)
{
	...
	*ntlmssp_state = TALLOC_ZERO_P(mem_ctx, NTLMSSP_STATE);
	if (!*ntlmssp_state) {
		DEBUG0(("ntlmssp_server_start: talloc failed!\n"));
		talloc_destroy(mem_ctx);
		return NT_STATUS_NO_MEMORY;
	}
					 ...
					 const DATA_BLOB reply, DATA_BLOB *next_request) 
{
	...
	if (!msrpc_parse(&reply, "CdBd",
			 ...
			 &server_domain_blob,
			 &chal_flags)) {
		DEBUG1( ("Failed to parse the NTLMSSP Challenge: (#1)\n"));
	...
	DEBUG3( ("NTLMSSP: Set final flags:\n"));
	...
	if (!msrpc_parse(&reply, chal_parse_string,
			 ...
			 &unkn1, &unkn2,
			 &struct_blob)) {
		DEBUG1( ("Failed to parse the NTLMSSP Challenge: (#2)\n"));
		dump_data(2, (const char *)reply.data, reply.length);
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (!ntlmssp_state->nt_hash || !ntlmssp_state->lm_hash) {
		...
		if (!struct_blob.length) {
			...
			DEBUG1( ("Server did not provide 'target information', required for NTLMv2\n"));
			return NT_STATUS_INVALID_PARAMETER;
		}
		...
		DEBUG5( ("NTLMSSP challenge set by NTLM2\n"));
		DEBUG5( ("challenge is: \n"));
	...
	if (!NT_STATUS_IS_OK(nt_status = ntlmssp_sign_init(ntlmssp_state))) {
		DEBUG1( ("Could not setup NTLMSSP signing/sealing system (error was: %s)\n", nt_errstr(nt_status)));
	}
...
NTSTATUS ntlmssp_client_start(NTLMSSP_STATE **ntlmssp_state)
{
	...
	*ntlmssp_state = TALLOC_ZERO_P(mem_ctx, NTLMSSP_STATE);
	if (!*ntlmssp_state) {
		DEBUG0(("ntlmssp_client_start: talloc failed!\n"));
		talloc_destroy(mem_ctx);
		return NT_STATUS_NO_MEMORY;
	}
