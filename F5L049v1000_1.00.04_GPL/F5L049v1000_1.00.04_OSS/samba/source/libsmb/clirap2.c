...
int cli_NetGroupDelete(struct cli_state *cli, const char *group_name )
{
  ...
  if (cli_api(cli, 
	...
	/* nothing to do */		
          ...
          DEBUG1( ("Access Denied\n"));
      }
      else if (res == 2220) {
         DEBUG (1, ("Group does not exist\n"));
      }
      else {
	DEBUG4(("NetGroupDelete res=%d\n", res));
      }      
    } else {
      res = -1;
      DEBUG4(("NetGroupDelete failed\n"));
    }
...
int cli_NetGroupAdd(struct cli_state *cli, RAP_GROUP_INFO_1 * grinfo )
{
  ...
  if (cli_api(cli, 
	...
	/* nothing to do */		
    	...
    	DEBUG4(("NetGroupAdd res=%d\n", res));
      }
    } else {
      res = -1;
      DEBUG4(("NetGroupAdd failed\n"));
    }
...
int cli_RNetGroupEnum(struct cli_state *cli, void (*fn)(const char *, const char *, void *), void *state)
{
  ...
  if (cli_api(cli,
    ...
    if(cli->rap_error == 234) {
        DEBUG1(("Not all group names were returned (such as those longer than 21 characters)\n"));}
    else if (cli->rap_error != 0) {
      DEBUG1(("NetGroupEnum gave error %d\n", cli->rap_error));
    }
  }
  ...
  if (rdata) {
    if (res == 0 || res == ERRmoredata) {
      ...
      DEBUG4(("NetGroupEnum res=%d\n", res));
    }
  } else {
    DEBUG4(("NetGroupEnum no data returned\n"));
  }
...
int cli_NetGroupDelUser(struct cli_state * cli, const char *group_name, const char *user_name)
{
  ...
  if (cli_api(cli, 
	      ...
	      &rdata, &rdrcnt))   /* return data, length */
    {
	  ...
	  break;
        case 50:
	  DEBUG1( ("Not supported by server\n"));
	  break;
        case 2220:
	  DEBUG1( ("Group does not exist\n"));
	  break;
        case 2221:
	  DEBUG1( ("User does not exist\n"));
	  break;
        case 2237:
	  DEBUG1( ("User is not in group\n"));
	  break;
        default:
	  DEBUG4(("NetGroupDelUser res=%d\n", res));
      }
    } else {
      res = -1;
      DEBUG4(("NetGroupDelUser failed\n"));
    }
...
int cli_NetGroupAddUser(struct cli_state * cli, const char *group_name, const char *user_name)
{
  ...
  if (cli_api(cli, 
	      ...
	      &rdata, &rdrcnt))   /* return data, length */
    {
	  ...
	  break;
        default:
	  DEBUG4(("NetGroupAddUser res=%d\n", res));
      }
    } else {
      res = -1;
      DEBUG4(("NetGroupAddUser failed\n"));
    }
...
int cli_NetGroupGetUsers(struct cli_state * cli, const char *group_name, void (*fn)(const char *, void *), void *state )
{
  ...
  if (cli_api(cli,
    ...
    cli->rap_error = res;
    if (res != 0) {
      DEBUG1(("NetGroupGetUsers gave error %d\n", res));
    }
  }
  if (rdata) {
    if (res == 0 || res == ERRmoredata) {
	...
	fn(username, state);
      ...
      DEBUG4(("NetGroupGetUsers res=%d\n", res));
    }
  } else {
    DEBUG4(("NetGroupGetUsers no data returned\n"));
  }
  SAFE_FREE(rdata);
  SAFE_FREE(rparam);
  return res;
...
int cli_NetUserGetGroups(struct cli_state * cli, const char *user_name, void (*fn)(const char *, void *), void *state )
{
  ...
  if (cli_api(cli,
    ...
    cli->rap_error = res;
    if (res != 0) {
      DEBUG1(("NetUserGetGroups gave error %d\n", res));
    }
  }
  if (rdata) {
    if (res == 0 || res == ERRmoredata) {
      ...
      DEBUG4(("NetUserGetGroups res=%d\n", res));
    }
  } else {
    DEBUG4(("NetUserGetGroups no data returned\n"));
  }
  SAFE_FREE(rdata);
  SAFE_FREE(rparam);
  return res;
  ...
  if (cli_api(cli, 
	...
	/* nothing to do */		
         ...
         DEBUG (1, ("User does not exist\n"));
          ...
          DEBUG4(("NetUserDelete res=%d\n", res));
      }      
    } else {
      res = -1;
      DEBUG4(("NetUserDelete failed\n"));
    }
...
int cli_NetUserAdd(struct cli_state *cli, RAP_USER_INFO_1 * userinfo )
{
  ...
  if (cli_api(cli, 
	...
	/* nothing to do */		
	    ...
	    DEBUG4(("NetUserAdd res=%d\n", res));
      }
    } else {
      res = -1;
      DEBUG4(("NetUserAdd failed\n"));
    }
...
/* BB Fix handling of large numbers of users to be returned */
  if (cli_api(cli,
    ...
    cli->rap_error = res;
    if (cli->rap_error != 0) {
      DEBUG1(("NetUserEnum gave error %d\n", cli->rap_error));
    }
  }
  if (rdata) {
    if (res == 0 || res == ERRmoredata) {
      ...
      DEBUG4(("NetUserEnum res=%d\n", res));
    }
  } else {
    DEBUG4(("NetUserEnum no data returned\n"));
  }
...
int cli_NetFileClose(struct cli_state *cli, uint32 file_id )
{
  ...
  if (cli_api(cli, 
	...
	/* nothing to do */		
         ...
         DEBUG1( ("NetFileClose2 - attempt to close non-existant file open instance\n"));
      } else {
	DEBUG4(("NetFileClose2 res=%d\n", res));
      }      
    } else {
      res = -1;
      DEBUG4(("NetFileClose2 failed\n"));
    }
...
int cli_NetFileGetInfo(struct cli_state *cli, uint32 file_id, void (*fn)(const char *, const char *, uint16, uint16, uint32))
{
  ...
  if (cli_api(cli, 
	...
	DEBUG4(("NetFileGetInfo2 res=%d\n", res));
      }      
    } else {
      res = -1;
      DEBUG4(("NetFileGetInfo2 failed\n"));
    }
...
int cli_NetFileEnum(struct cli_state *cli, char * user, char * base_path, void (*fn)(const char *, const char *, uint16, uint16, uint32))
{
  ...
  if (cli_api(cli, 
	...
	DEBUG4(("NetFileEnum2 res=%d\n", res));
      }      
    } else {
      DEBUG4(("NetFileEnum2 failed\n"));
    }
...
int cli_NetShareAdd(struct cli_state *cli, RAP_SHARE_INFO_2 * sinfo )
{
  ...
  if (cli_api(cli, 
	...
	DEBUG4(("NetShareAdd res=%d\n", res));
      }      
    } else {
      res = -1;
      DEBUG4(("NetShareAdd failed\n"));
    }
...
int cli_NetShareDelete(struct cli_state *cli, const char * share_name )
{
  ...
  if (cli_api(cli, 
	...
	DEBUG4(("NetShareDelete res=%d\n", res));
      }      
    } else {
      res = -1;
      DEBUG4(("NetShareDelete failed\n"));
    }
...
BOOL cli_get_pdc_name(struct cli_state *cli, char *workgroup, char *pdc_name)
{
  ...
  *pdc_name = '\0';
  ...
  if (cli_api(cli, 
	...
	DEBUG4(("cli_get_pdc_name: machine %s failed the NetServerEnum call. "
...
BOOL cli_ns_check_server_type(struct cli_state *cli, char *workgroup, uint32 stype)
{
  ...
  if (cli_api(cli, 
	...
	if (strequal(ret_server, cli->desthost)) {
      ...
      DEBUG4(("cli_ns_check_server_type: machine %s failed the NetServerEnum call. "
	       "Error was : %s.\n", cli->desthost, cli_errstr(cli) ));
    }
  }
...
BOOL cli_NetWkstaUserLogoff(struct cli_state *cli,char *user, char *workstation)
{
  ...
  if (cli_api(cli,
    ...
    if (cli->rap_error != 0) {
      DEBUG4(("NetwkstaUserLogoff gave error %d\n", cli->rap_error));
    }
  }
		...
		void (*jfn)(uint16,const char*,const char*,const char*,const char*,uint16,uint16,const char*,uint,uint,const char*))
{
  ...
  if (cli_api(cli,
    ...
    cli->rap_error = res;
    if (res != 0) {
      DEBUG1(("NetPrintQEnum gave error %d\n", res));
    }
  }
  ...
  if (rdata) {
	...
	if (jobcount) {
      ...
      DEBUG4(("NetPrintQEnum res=%d\n", res));
    }
  } else {
    DEBUG4(("NetPrintQEnum no data returned\n"));
  }
	...
	void (*jfn)(uint16,const char*,const char*,const char*,const char*,uint16,uint16,const char*,uint,uint,const char*))
{
  ...
  if (cli_api(cli,
    ...
    cli->rap_error = res;
    if (res != 0) {
      DEBUG1(("NetPrintQGetInfo gave error %d\n", res));
    }
  }
  ...
  if (rdata) {
	...
	for (j=0;(j<jobcount)&&(PTR_DIFF(p,rdata)< rsize);j++) {
      ...
      DEBUG4(("NetPrintQGetInfo res=%d\n", res));
    }
  } else {
    DEBUG4(("NetPrintQGetInfo no data returned\n"));
  }
...
int cli_RNetServiceEnum(struct cli_state *cli, void (*fn)(const char *, const char *, void *), void *state)
{
  ...
  if (cli_api(cli,
    ...
    cli->rap_error = res;
    if(cli->rap_error == 234) {
        DEBUG1(("Not all service names were returned (such as those longer than 15 characters)\n"));}
    else if (cli->rap_error != 0) {
      DEBUG1(("NetServiceEnum gave error %d\n", cli->rap_error));
    }
  }
  ...
  if (rdata) {
    if (res == 0 || res == ERRmoredata) {
      ...
      DEBUG4(("NetServiceEnum res=%d\n", res));
    }
  } else {
    DEBUG4(("NetServiceEnum no data returned\n"));
  }
...
int cli_NetSessionEnum(struct cli_state *cli, void (*fn)(char *, char *, uint16, uint16, uint16, uint, uint, uint, char *))
{
  ...
  if (cli_api(cli,
    ...
    cli->rap_error = res;
    if (res != 0) {
      DEBUG1(("NetSessionEnum gave error %d\n", res));
    }
  }
  ...
  if (rdata) {
	...
	fn(wsname, username, num_conns, num_opens, num_users, sess_time,
      ...
      DEBUG4(("NetSessionEnum res=%d\n", res));
    }
  } else {
    DEBUG4(("NetSesssionEnum no data returned\n"));
  }
...
int cli_NetSessionGetInfo(struct cli_state *cli, const char *workstation, void (*fn)(const char *, const char *, uint16, uint16, uint16, uint, uint, uint, const char *))
{
  ...
  if (cli_api(cli,
    ...
    if (cli->rap_error != 0) {
      DEBUG1(("NetSessionGetInfo gave error %d\n", cli->rap_error));
    }
  }
  ...
  if (rdata) {
    ...
    if (res == 0 || res == ERRmoredata) {
	 ...
	 idle_time, user_flags, clitype_name);
      ...
      DEBUG4(("NetSessionGetInfo res=%d\n", res));
    }
  } else {
    DEBUG4(("NetSessionGetInfo no data returned\n"));
  }
...
int cli_NetConnectionEnum(struct cli_state *cli, const char *qualifier, void (*fn)(uint16 conid, uint16 contype, uint16 numopens, uint16 numusers, uint32 contime, const char *username, const char *netname))
{
  ...
  if (cli_api(cli,
    ...
    cli->rap_error = res;
    if (res != 0) {
      DEBUG1(("NetConnectionEnum gave error %d\n", res));
    }
  }
  if (rdata) {
    if (res == 0 || res == ERRmoredata) {
	...
	fn(conn_id, conn_type, num_opens, num_users, conn_time,
      ...
      DEBUG4(("NetConnectionEnum res=%d\n", res));
    }
  } else {
    DEBUG4(("NetConnectionEnum no data returned\n"));
  }
  SAFE_FREE(rdata);
  SAFE_FREE(rparam);
  return res;
