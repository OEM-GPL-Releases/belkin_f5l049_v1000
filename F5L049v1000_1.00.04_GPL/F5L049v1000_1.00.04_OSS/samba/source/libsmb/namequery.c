...
BOOL saf_store( const char *domain, const char *servername )
{
	...
	if ( !domain || !servername ) {
		DEBUG2(("saf_store: Refusing to store empty domain or servername!\n"));
		return False;
	}

	if ( (strlen(domain) == 0) || (strlen(servername) == 0) ) {
		DEBUG0(("saf_store: refusing to store 0 length domain or servername!\n"));
		return False;
	}
	...
	DEBUG10(("saf_store: domain = [%s], server = [%s], expire = [%u]\n",
...
BOOL saf_join_store( const char *domain, const char *servername )
{
	...
	if ( !domain || !servername ) {
		DEBUG2(("saf_join_store: Refusing to store empty domain or servername!\n"));
		return False;
	}

	if ( (strlen(domain) == 0) || (strlen(servername) == 0) ) {
		DEBUG0(("saf_join_store: refusing to store 0 length domain or servername!\n"));
		return False;
	}
	...
	DEBUG10(("saf_join_store: domain = [%s], server = [%s], expire = [%u]\n",
		domain, servername, (unsigned int)expire ));
...
BOOL saf_delete( const char *domain )
{
	...
	if ( !domain ) {
		DEBUG2(("saf_delete: Refusing to delete empty domain\n"));		
		return False;
	}
	...
	if (ret) {
		DEBUG10(("saf_delete[join]: domain = [%s]\n", domain ));
	}
...
char *saf_fetch( const char *domain )
{
	...
	if ( !domain || strlen(domain) == 0) {
		DEBUG2(("saf_fetch: Empty domain name!\n"));
		return NULL;
	}
	...
	if ( ret ) {
		DEBUG5(("saf_fetch[join]: Returning \"%s\" for \"%s\" domain\n",
			server, domain ));
		return server;
	}
	...
	if ( !ret ) {
		DEBUG5(("saf_fetch: failed to find server for \"%s\" domain\n", domain ));
	} else {
		DEBUG5(("saf_fetch: Returning \"%s\" for \"%s\" domain\n", 
			server, domain ));
	}
...
BOOL name_status_find(const char *q_name, int q_type, int type, struct in_addr to_ip, fstring name)
{
	...
	if (lp_disable_netbios()) {
		DEBUG5(("name_status_find(%s#%02x): netbios is disabled\n", q_name, q_type));
		return False;
	}

	DEBUG10( ("name_status_find: looking up %s#%02x at %s\n", q_name, 
	...
	DEBUG10( ("name_status_find: name %sfound", result ? "" : "not "));
...
static int remove_duplicate_addrs2( struct ip_service *iplist, int count )
{
	...
	DEBUG10(("remove_duplicate_addrs2: looking for duplicate address/port pairs\n"));
			   ...
			   BOOL *timed_out)
{
	...
	if (lp_disable_netbios()) {
		DEBUG5(("name_query(%s#%02x): netbios is disabled\n", name, name_type));
		return NULL;
	}
	...
	while (1) {
		...
		if ((p2=receive_nmb_packet(fd,90,nmb->header.name_trn_id))) {     
			...
			if (!ip_list) {
				DEBUG0(("name_query: Realloc failed.\n"));
				free_packet(p2);
				return( NULL );
			}
			
			DEBUG2(("Got a positive name query response from %s ( ", inet_ntoa(p2->ip)));
...
XFILE *startlmhosts(char *fname)
{
	...
	if (!fp) {
		DEBUG4(("startlmhosts: Can't open lmhosts file %s. Error was %s\n",
...
BOOL getlmhostsent( XFILE *fp, pstring name, int *name_type, struct in_addr *ipaddr)
{
	...
	while(!x_feof(fp) && !x_ferror(fp)) {
		...
		*name_type = -1;
		...
		if (count > 0 && count < 2) {
			DEBUG0(("getlmhostsent: Ill formed hosts line [%s]\n",line));
			continue;
		}

		if (count >= 4) {
			DEBUG0(("getlmhostsent: too many columns in lmhosts file (obsolete syntax)\n"));
			continue;
		}

		DEBUG4( ("getlmhostsent: lmhost entry: %s %s %s\n", ip, name, flags));

		if (strchr_m(flags,'G') || strchr_m(flags,'S')) {
			DEBUG0(("getlmhostsent: group flag in lmhosts ignored (obsolete)\n"));
			continue;
		}
		...
		*ipaddr = *interpret_addr2(ip);
		...
		if((ptr1 = strchr_m(name, '#')) != NULL) {
			...
			*name_type = (int)strtol(ptr1, &endptr, 16);
			if(!*ptr1 || (endptr == ptr1)) {
				DEBUG0(("getlmhostsent: invalid name %s containing '#'.\n", name));
				continue;
			}
...
static BOOL convert_ip2service( struct ip_service **return_iplist, struct in_addr *ip_list, int count )
{
	...
	if ( (*return_iplist = SMB_MALLOC_ARRAY(struct ip_service, count)) == NULL ) {
		DEBUG0(("convert_ip2service: malloc failed for %d enetries!\n", count ));
		return False;
	}
			...
			struct ip_service **return_iplist, int *return_count)
{
	...
	if (lp_disable_netbios()) {
		DEBUG5(("name_resolve_bcast(%s#%02x): netbios is disabled\n", name, name_type));
		return False;
	}
	...
	*return_count = 0;
	...
	DEBUG3(("name_resolve_bcast: Attempting broadcast lookup for name %s<0x%x>\n", name, name_type));
		  ...
		  struct ip_service **return_iplist, int *return_count)
{
	...
	if (lp_disable_netbios()) {
		DEBUG5(("resolve_wins(%s#%02x): netbios is disabled\n", name, name_type));
		return False;
	}
	...
	*return_count = 0;
	...
	DEBUG3(("resolve_wins: Attempting wins lookup for name %s<0x%x>\n", name, name_type));

	if (wins_srv_count() < 1) {
		DEBUG3(("resolve_wins: WINS server resolution selected and no WINS servers listed.\n"));
		return False;
	}
	...
	for (t=0; wins_tags && wins_tags[t]; t++) {
		...
		for (i=0; i<srv_count; i++) {
			...
			DEBUG3(("resolve_wins: using WINS server %s and tag '%s'\n", inet_ntoa(wins_ip), wins_tags[t]));
                         ...
                         struct ip_service **return_iplist, int *return_count)
{
	...
	*return_count = 0;
	...
	DEBUG3(("resolve_lmhosts: Attempting lmhosts lookup for name %s<0x%x>\n", name, name_type));
	...
	while (getlmhostsent(fp, lmhost_name, &name_type2, &return_ip)) 
	{
		...
		*return_iplist = SMB_REALLOC_ARRAY((*return_iplist), struct ip_service,
		...
		if ((*return_iplist) == NULL) {
			endlmhosts(fp);
			DEBUG3(("resolve_lmhosts: malloc fail !\n"));
			return False;
		}
                         ...
                         struct ip_service **return_iplist, int *return_count)
{
	...
	if ( name_type != 0x20 && name_type != 0x0) {
		DEBUG5( ("resolve_hosts: not appropriate for name type <0x%x>\n", name_type));
		return False;
	}
	...
	*return_count = 0;
	...
	DEBUG3(("resolve_hosts: Attempting host lookup for name %s<0x%x>\n", name, name_type));
	...
	if (((hp = sys_gethostbyname(name)) != NULL) && (hp->h_addr != NULL)) {
		...
		*return_iplist = SMB_MALLOC_P(struct ip_service);
		if(*return_iplist == NULL) {
			DEBUG3(("resolve_hosts: malloc fail !\n"));
			return False;
		}
                         ...
                         struct ip_service **return_iplist, int *return_count)
{
	...
	DEBUG5(("resolve_ads: Attempting to resolve DC's for %s using DNS\n",
		name));
			
	if ( (ctx = talloc_init("resolve_ads")) == NULL ) {
		DEBUG0(("resolve_ads: talloc_init() failed!\n"));
		return False;
	}
	...
	if ( (*return_iplist = SMB_MALLOC_ARRAY(struct ip_service, numaddrs)) == NULL ) {
		DEBUG0(("resolve_ads: malloc failed for %d entries\n", numaddrs ));
		talloc_destroy(ctx);
		return False;
	}
			   ...
			   int *return_count, const char *resolve_order)
{
	...
	*return_count = 0;
	...
	DEBUG10( ("internal_resolve_name: looking up %s#%x (sitename %s)\n",
			name, name_type, sitename ? sitename : NULL));

	if (allzeros || allones || is_address) {
  
		if ( (*return_iplist = SMB_MALLOC_P(struct ip_service)) == NULL ) {
			DEBUG0(("internal_resolve_name: malloc fail !\n"));
			return False;
		}
		...
		if(is_address) { 
			...
			if (((*return_iplist)->ip.s_addr = inet_addr(name)) == 0xFFFFFFFF ){
				DEBUG1(("internal_resolve_name: inet_addr failed on %s\n", name));
	...
	if ( strcmp( resolve_order, "NULL") == 0 ) {
		DEBUG8(("internal_resolve_name: all lookups disabled\n"));
		return False;
	}
	...
	while (next_token(&ptr, tok, LIST_SEP, sizeof(tok))) {
		if((strequal(tok, "host") || strequal(tok, "hosts"))) {
			...
			DEBUG0(("resolve_name: unknown name switch type %s\n", tok));
		}
	}
	...
	*return_count = 0;
	...
	if ( DEBUGLEVEL >= 10 ) {
		DEBUG10( ("internal_resolve_name: returning %d addresses: ", *return_count));
...
BOOL find_master_ip(const char *group, struct in_addr *master_ip)
{
	...
	if (lp_disable_netbios()) {
		DEBUG5(("find_master_ip(%s): netbios is disabled\n", group));
		return False;
	}
...
BOOL get_pdc_ip(const char *domain, struct in_addr *ip)
{
	...
	if ( count > 1 ) {
		DEBUG6(("get_pdc_ip: PDC has %d IP addresses!\n", count));		
                            ...
                            int *count, enum dc_lookup_type lookup_type, int *ordered)
{
	...
	*ordered = False;
	...
	if ( !*pserver ) {
		DEBUG10(("get_dc_list: no preferred domain controllers.\n"));
	...
	DEBUG3(("get_dc_list: preferred server list: \"%s\"\n", pserver ));
	...
	while (next_token(&p,name,LIST_SEP,sizeof(name))) {
		if (!done_auto_lookup && strequal(name, "*")) {
			...
			DEBUG8(("Adding %d DC's from auto lookup\n", auto_count));
		} else  {
			num_addresses++;
		}
	}
	...
	if ( (num_addresses == 0) ) {
		if ( done_auto_lookup ) {
			DEBUG4(("get_dc_list: no servers found\n")); 
			status = NT_STATUS_NO_LOGON_SERVERS;
			goto out;
		}
		if (internal_resolve_name(domain, 0x1C, sitename, ip_list, count,
	...
	if ( (return_iplist = SMB_MALLOC_ARRAY(struct ip_service, num_addresses)) == NULL ) {
		DEBUG3(("get_dc_list: malloc fail !\n"));
		...
		if ( strequal(name, "*") ) {
			for ( j=0; j<auto_count; j++ ) {
				...
				if(!NT_STATUS_IS_OK(check_negative_conn_cache(domain, 
					...
					DEBUG5(("get_dc_list: negative entry %s removed from DC list\n",
						inet_ntoa(auto_ip_list[j].ip) ));
					continue;
				}
	...
	if ( DEBUGLEVEL >= 4 ) {
		DEBUG4(("get_dc_list: returning %d ip addresses in an %sordered list\n", local_count, 
			*ordered ? "":"un"));
		DEBUG4(("get_dc_list: "));
...
NTSTATUS get_sorted_dc_list( const char *domain, const char *sitename, struct ip_service **ip_list, int *count, BOOL ads_only )
{
	...
	DEBUG8(("get_sorted_dc_list: attempting lookup for name %s (sitename %s) "
		"using [%s]\n",
		domain,
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_NO_LOGON_SERVERS) && sitename) {
		DEBUG3(("get_sorted_dc_list: no server for name %s available"
