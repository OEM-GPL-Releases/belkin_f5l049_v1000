					...
					uint32 *p_resume_key, DATA_BLOB *p_last_name_raw, uint32 *p_last_name_raw_len)
{
	...
	DEBUG1(("Unknown long filename format %d\n",level));
	return (size_t)IVAL(base,0);
}
		 ...
		 void (*fn)(const char *, file_info *, const char *, void *), void *state)
{
	...
	while (ff_eos == 0) {
		loop_count++;
		if (loop_count > 200) {
			DEBUG0(("Error: Looping in FIND_NEXT??\n"));
			break;
		}
		...
		for (p2=p,i=0;i<ff_searchcount;i++) {
			...
			if (!First && *mask && strcsequal(finfo.name, mask)) {
				DEBUG0(("Error: Looping in FIND_NEXT as name %s has already been seen?\n",
					finfo.name));
				ff_eos = 1;
				break;
			}
		...
		if (!dirlist) {
			DEBUG0(("cli_list_new: Failed to expand dirlist\n"));
			SAFE_FREE(rdata);
			SAFE_FREE(rparam);
			break;
		}
		...
		DEBUG3(("received %d entries (eos=%d)\n",
		 ...
		 void (*fn)(const char *, file_info *, const char *, void *), void *state)
{
	...
	while (1) {
		...
		*p++ = 5;
		...
		if (!dirlist) {
			DEBUG0(("cli_list_old: failed to expand dirlist"));
			return 0;
		}
