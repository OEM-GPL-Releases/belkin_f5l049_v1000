	...
	if (cli_api(cli, 
		    ...
		    &rdata, &rdrcnt))                /* return data, length */
		{
			...
			if (res == 0 || res == ERRmoredata) {
				...
				DEBUG4(("NetShareEnum res=%d\n", res));
			}      
		} else {
			DEBUG4(("NetShareEnum failed\n"));
		}
                             ...
                             const char *old_password)
{
	...
	if (strlen(user) >= sizeof(fstring)-1) {
		DEBUG0(("cli_oem_change_password: user name %s is too long.\n", user));
		return False;
	}
	...
	if (cli_send_trans(cli,SMBtrans,
		...
		DEBUG0(("cli_oem_change_password: Failed to send password change for user %s\n",
			user ));
		return False;
	}
	...
	if (!cli_receive_trans(cli,SMBtrans,
                       &rparam, &rprcnt,
                       &rdata, &rdrcnt)) {
		DEBUG0(("cli_oem_change_password: Failed to recieve reply to password change for user %s\n",
			user ));
		return False;
	}
