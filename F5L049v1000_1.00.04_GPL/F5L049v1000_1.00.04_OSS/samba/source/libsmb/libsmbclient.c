                ...
                char *options, int options_len)
{
	...
	if (strncmp(p, "//", 2) && strncmp(p, "\\\\", 2)) {
                ...
                DEBUG1( ("Invalid path (does not begin with smb://"));
		return -1;
...
smbc_check_options(char *server,
                   ...
                   char *options)
{
        DEBUG4( ("smbc_check_options(): server='%s' share='%s' "
                  "path='%s' options='%s'\n",
                          ...
                          SMBCSRV * srv)
{
	...
	for (file = context->internal->_files; file; file=file->next) {
		if (file->srv == srv) {
			/* Still used */
			DEBUG3( ("smbc_remove_usused_server: "
                                  "%p still used by %p.\n", 
				  srv, file));
			return 1;
		}
	...
	DEBUG3( ("smbc_remove_usused_server: %p removed.\n", srv));
            ...
            fstring password)
{
	...
	DEBUG4(("smbc_server: server_n=[%s] server=[%s]\n", server_n, server));
  
	DEBUG4((" -> server_n=[%s] server=[%s]\n", server_n, server));
 ...
 again:
		...
		if (strcmp(called.name, "*SMBSERVER")) {
			...
			if (is_ipaddress(server) && !tried_reverse) {
				...
				if ((rem_ip.s_addr=inet_addr(server)) == INADDR_NONE) {
					DEBUG4( ("Could not convert IP address "
						"%s to struct in_addr\n", server));
					errno = ETIMEDOUT;
					return NULL;
				}
	...
	if ((context->callbacks.add_cached_srv_fn)(context, srv,
		...
		DEBUG3( (" Failed to add server to cache\n"));
		errno = saved_errno;
		if (errno == 0) {
			errno = ENOMEM;
		}
	...
	DEBUG2( ("Server connect ok: //%s/%s: %p\n", 
		  server, share, srv));
	...
	SMBCSRV *ipc_srv=NULL;
        ...
        if (!ipc_srv) {
                ...
                if (! NT_STATUS_IS_OK(nt_status)) {
                        DEBUG1(("cli_full_connection failed! (%s)\n",
                                 nt_errstr(nt_status)));
                        errno = ENOTSUP;
                        return NULL;
                }
                ...
                if (pol) {
                        pipe_hnd = cli_rpc_pipe_open_noauth(ipc_srv->cli,
                                                            ...
                                                            &nt_status);
                        if (!pipe_hnd) {
                                DEBUG1( ("cli_nt_session_open fail!\n"));
                                errno = ENOTSUP;
               ...
               SMBCFILE *file)
{
	...
	if (!cli_close(targetcli, file->cli_fd)) {
		...
		DEBUG3( ("cli_close failed on %s. purging server.\n", 
			  file->fname));
            ...
            SMB_INO_T *ino)
{
	...
	DEBUG4(("smbc_getatr: sending qpathinfo\n"));
                   ...
                   void *state)
{
	...
	struct rpc_pipe_client *pipe_hnd;
        ...
        if (!pipe_hnd) {
                DEBUG1( ("net_share_enum_rpc pipe open fail!\n"));
                return -1;
        }
        ...
        if (mem_ctx == NULL) {
                DEBUG0( ("out of memory for net_share_enum_rpc!\n"));
                 ...
                 const char *fname)
{
	...
	if (smbc_parse_path(context, fname,
	        ...
	        DEBUG4( ("no valid path\n"));
		errno = EINVAL + 8194;
		return NULL;
	}

	DEBUG4( ("parsed path: fname='%s' server='%s' share='%s' "
                  "path='%s' options='%s'\n",
                  fname, server, share, path, options));

        /* Ensure the options are valid */
        if (smbc_check_options(server, share, path, options)) {
                DEBUG4( ("unacceptable options (%s)\n", options));
                errno = EINVAL + 8195;
                return NULL;
        }
                 ...
                 SMBCFILE *dir)
{
	...
	if (!context || !context->internal ||
		...
		errno = EINVAL;
                DEBUG0( ("Invalid context in smbc_readdir_ctx()\n"));
		return NULL;
	...
	if (!dir || !DLIST_CONTAINS(context->internal->_files, dir)) {
		...
		errno = EBADF;
                DEBUG0( ("Invalid dir in smbc_readdir_ctx()\n"));
		return NULL;
	...
	if (dir->file != False) { /* FIXME, should be dir, perhaps */
		...
		errno = ENOTDIR;
                DEBUG0( ("Found file vs directory in smbc_readdir_ctx()\n"));
		return NULL;
               ...
               char *str)
{
	...
	while (next_token(&p, tok, "\t,\r\n", sizeof(tok))) {
		...
		if (StrnCaseCmp(tok,"OWNER:", 6) == 0) {
			if (owner_sid) {
				DEBUG5( ("OWNER specified more than once!\n"));
				goto done;
			}
			...
			if (!owner_sid ||
			    !convert_string_to_sid(ipc_cli, pol,
                                                   numeric,
                                                   owner_sid, tok+6)) {
				DEBUG5( ("Failed to parse owner sid\n"));
				goto done;
			}
			continue;
		}
		...
		if (StrnCaseCmp(tok,"GROUP:", 6) == 0) {
			if (group_sid) {
				DEBUG5( ("GROUP specified more than once!\n"));
				goto done;
			}
			...
			if (!group_sid ||
			    !convert_string_to_sid(ipc_cli, pol,
                                                   numeric,
                                                   group_sid, tok+6)) {
				DEBUG5( ("Failed to parse group sid\n"));
				goto done;
			}
			continue;
		}
		...
		if (StrnCaseCmp(tok,"ACL:", 4) == 0) {
			...
			if (!parse_ace(ipc_cli, pol, &ace, numeric, tok+4)) {
				DEBUG5( ("Failed to parse ACL %s\n", tok));
				goto done;
			}
			if(!add_ace(&dacl, &ace, ctx)) {
				DEBUG5( ("Failed to add ACL %s\n", tok));
				goto done;
			}
			continue;
		}
		...
		DEBUG5( ("Failed to parse security descriptor\n"));
		goto done;
	}
               ...
               SMBCSRV *srv)
{
	...
	SMB_INO_T inode = 0;
        ...
        if (!smbc_getatr(context, srv, CONST_DISCARD(char *, filename),
                ...
                errno = smbc_errno(context, srv->cli);
                DEBUG5( ("dos_attr_query Failed to query old attributes\n"));
                return NULL;
	...
	SMB_INO_T ino = 0;
        ...
        if (all || all_nt || all_dos) {
                ...
                if (StrCaseCmp(pExclude, "nt_sec_desc.revision") == 0) {
                    ...
                    DEBUG5( ("cacl_get received unknown exclusion: %s\n",
                              pExclude));
                    errno = ENOATTR;
                    return -1;
                }
				      ...
				      &targetcli, targetpath))
		{
			...
			return -1;
                ...
                if (fnum == -1) {
                        DEBUG5( ("cacl_get failed to open %s: %s\n",
         ...
         int flags)
{
	...
	if (fnum == -1) {
                DEBUG5( ("cacl_set failed to open %s: %s\n",
                          targetpath, cli_errstr(targetcli)));
                errno = 0;
		return -1;
	}
	...
	if (!old) {
                DEBUG5( ("cacl_set Failed to query old descriptor\n"));
                errno = 0;
		return -1;
	}
	...
	if (!cli_set_secdesc(targetcli, fnum, sd)) {
		DEBUG5( ("ERROR: secdesc set failed: %s\n", cli_errstr(targetcli)));
		ret = -1;
	}
                        ...
                        const char *fname)
{
        ...
        DEBUG4( ("smbc_open_print_job_ctx(%s)\n", fname));
                          ...
                          int id)
{
        ...
        DEBUG4( ("smbc_unlink_print_job(%s)\n", fname));
                  ...
                  int shutdown_ctx)
{
        ...
        if (shutdown_ctx) {
                SMBCFILE * f;
                DEBUG1(("Performing aggressive shutdown.\n"));
                ...
                if (context->callbacks.purge_cached_fn(context)) {
                        SMBCSRV * s;
                        SMBCSRV * next;
                        DEBUG1( ("Could not purge all servers, "
                                  "Nice way shutdown failed.\n"));
                        s = context->internal->_servers;
                        while (s) {
                                DEBUG1( ("Forced shutdown: %p (fd=%d)\n",
                                          s, s->cli->fd));
                                cli_shutdown(s->cli);
                                (context->callbacks.remove_cached_srv_fn)(context,
                                                                          s);
                ...
                if (context->internal->_servers) {
                        DEBUG1( ("Active servers in context, "
                                  "free_context failed.\n"));
                        errno = EBUSY;
                        return 1;
                }
                if (context->internal->_files) {
                        DEBUG1( ("Active files in context, "
                                  "free_context failed.\n"));
                        errno = EBUSY;
                        return 1;
                }               
        ...
        DEBUG3( ("Context %p successfully freed\n", context));
		...
		if (home) {
			...
			if (lp_load(conf, True, False, False, True)) {
				conf_loaded = True;
                                ...
                                DEBUG5( ("Could not load config file: %s\n",
                                          conf));
			}
               	}
