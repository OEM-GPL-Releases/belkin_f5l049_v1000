...
BOOL namecache_enable(void)
{
	...
	if (lp_name_cache_timeout() == 0) {
		DEBUG5( ("namecache_enable: disabling netbios name cache\n"));
		return False;
	}
	...
	if (!gencache_init()) {
		DEBUG2( ("namecache_enable: Couldn't initialise namecache on top of gencache.\n"));
		return False;
	}

	/* I leave it for now, though I don't think we really need this (mimir, 27.09.2002) */
	DEBUG5( ("namecache_enable: enabling netbios namecache, timeout %d "
...
BOOL namecache_shutdown(void)
{
	if (!gencache_shutdown()) {
		DEBUG2( ("namecache_shutdown: Couldn't close namecache on top of gencache.\n"));
		return False;
	}
	
	DEBUG5( ("namecache_shutdown: netbios namecache closed successfully.\n"));
	return True;
}
                     ...
                     int num_names, struct ip_service *ip_list)
{
	...
	if ( DEBUGLEVEL >= 5 ) {
		DEBUG5( ("namecache_store: storing %d address%s for %s#%02x: ",
                     ...
                     int *num_names)
{
	...
	*num_names = 0;
	...
	if (!gencache_get(key, &value, &timeout)) {
		DEBUG5( ("no entry for %s#%02X found.\n", name, name_type));
		SAFE_FREE(key);
		return False;
	} else {
		DEBUG5( ("name %s#%02X found.\n", name, name_type));
	}
		...
		const char *srvname)
{
	...
	if (ret){
		DEBUG5( ("namecache_status_store: entry %s -> %s\n", key, srvname ));}
	else
		DEBUG5( ("namecache_status_store: entry %s store failed.\n", key ));
			...
			int name_type, struct in_addr keyip, char *srvname_out)
{
	...
	if (!gencache_get(key, &value, &timeout)) {
		DEBUG5( ("namecache_status_fetch: no entry for %s found.\n", key));
		SAFE_FREE(key);
		return False;
	} else {
		DEBUG5( ("namecache_status_fetch: key %s -> %s\n", key, value ));
	}
