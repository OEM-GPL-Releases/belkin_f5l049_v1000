...
{  
	...
	if(( pkt_copy = SMB_MALLOC_P(struct packet_struct)) == NULL) {
		DEBUG0(("copy_nmb_packet: malloc fail.\n"));
		return NULL;
	}
 ...
 free_and_exit:
	...
	DEBUG0(("copy_nmb_packet: malloc fail in resource records.\n"));
	return NULL;
}
...
{ 
	...
	if(( pkt_copy = SMB_MALLOC_P(struct packet_struct)) == NULL) {
		DEBUG0(("copy_dgram_packet: malloc fail.\n"));
		return NULL;
	}
...
struct packet_struct *read_packet(int fd,enum packet_type packet_type)
{
	...
	DEBUG5(("Received a packet of len %d from (%s) port %d\n",
...
static BOOL send_udp(int fd,char *buf,int len,struct in_addr ip,int port)
{
	...
	DEBUG5( ( "Sending a packet of len %d to (%s) on port %d\n",
	...
	if (!ret)
		DEBUG0(("Packet send failed to %s(%d) ERRNO=%s\n",
...
struct packet_struct *receive_packet(int fd,enum packet_type type,int t)
{
	...
	if ((ret = sys_select_intr(fd+1,&fds,NULL,NULL,&timeout)) == -1) {
		...
		DEBUG0(("select returned -1, errno = %s (%d)\n", strerror(errno), errno));
		return NULL;
	}
...
static char *name_ptr(char *buf,int ofs)
{
	...
	if ((c & 0xC0) == 0xC0) {
		...
		DEBUG5(("name ptr to pos %d from %d is %s\n",l,ofs,buf+l));
		return(buf + l);
	} else {
		return(buf+ofs);
	}
