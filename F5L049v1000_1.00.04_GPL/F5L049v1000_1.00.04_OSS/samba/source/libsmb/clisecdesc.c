			    ...
			    TALLOC_CTX *mem_ctx)
{
	...
	if (!cli_send_nt_trans(cli, 
			       ...
			       param, 8, 4,
			       NULL, 0, 0x10000)) {
		DEBUG1(("Failed to send NT_TRANSACT_QUERY_SECURITY_DESC\n"));
		goto cleanup;
	}
	...
	if (!cli_receive_nt_trans(cli, 
				  &rparam, &rparam_count,
				  &rdata, &rdata_count)) {
		DEBUG1(("Failed to recv NT_TRANSACT_QUERY_SECURITY_DESC\n"));
		goto cleanup;
	}
	...
	if (!sec_io_desc("sd data", &psd, &pd, 1)) {
		DEBUG1(("Failed to parse secdesc\n"));
		goto cleanup;
	}
...
BOOL cli_set_secdesc(struct cli_state *cli, int fnum, SEC_DESC *sd)
{
	...
	if (!sec_io_desc("sd data", &sd, &pd, 1)) {
		DEBUG1(("Failed to marshall secdesc\n"));
		goto cleanup;
	}
	...
	if (!cli_send_nt_trans(cli, 
		...
		DEBUG1(("Failed to send NT_TRANSACT_SET_SECURITY_DESC\n"));
		goto cleanup;
	}
	...
	if (!cli_receive_nt_trans(cli, 
				  &rparam, &rparam_count,
				  &rdata, &rdata_count)) {
		DEBUG1(("NT_TRANSACT_SET_SECURITY_DESC failed\n"));
		goto cleanup;
	}
