	...
	for (fcc = failed_connection_cache; fcc; fcc = fcc->next) {
		...
		if (failed_cache_timeout != (unsigned int)-1) {
			if (failed_cache_timeout == 0 ||
				...
				DEBUG10( ("check_negative_conn_cache: cache entry expired for %s, %s\n", 
					domain, server ));
		...
		DEBUG10( ("check_negative_conn_cache: returning negative entry for %s, %s\n", 
			domain, server ));
...
void add_failed_connection_entry(const char *domain, const char *server, NTSTATUS result) 
{
	...
	for (fcc = failed_connection_cache; fcc; fcc = fcc->next) {			
		if ( strequal(fcc->domain_name, domain) && strequal(fcc->controller, server) ) {
			DEBUG10( ("add_failed_connection_entry: domain %s (%s) already tried and failed\n",
				   domain, server ));
	...
	if ( !(fcc = SMB_MALLOC_P(struct failed_connection_cache)) ) {
		DEBUG0( ("malloc failed in add_failed_connection_entry!\n"));
		return;
	}
	...
	DEBUG10(("add_failed_connection_entry: added domain %s (%s) to failed conn cache\n",
		domain, server ));
...
void flush_negative_conn_cache_for_domain(const char *domain)
{
	...
	while (fcc) {
		...
		if (strequal(fcc->domain_name, domain)) {
			DEBUG10(("flush_negative_conn_cache_for_domain: removed server %s "
