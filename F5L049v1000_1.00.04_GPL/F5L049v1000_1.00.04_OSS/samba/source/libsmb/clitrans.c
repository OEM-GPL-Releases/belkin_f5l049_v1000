                              ...
                              char **data, unsigned int *data_len)
{
	...
	*data_len = *param_len = 0;
	...
	if (CVAL(cli->inbuf,smb_com) != trans) {
		DEBUG0(("Expected %s response, got command 0x%02x\n",
	...
	if (total_data!=0) {
		*data = (char *)SMB_REALLOC(*data,total_data);
		if (!(*data)) {
			DEBUG0(("cli_receive_trans: failed to enlarge data buffer\n"));
			goto out;
		}
	}
	...
	if (total_param!=0) {
		*param = (char *)SMB_REALLOC(*param,total_param);
		if (!(*param)) {
			DEBUG0(("cli_receive_trans: failed to enlarge param buffer\n"));
			goto out;
		}
	}
	...
	for (;;)  {
		...
		if (this_data + *data_len > total_data ||
			...
			DEBUG1(("Data overflow in cli_receive_trans\n"));
			goto out;
		}
		...
		if (this_param) {
			...
			if (param_offset_out > total_param ||
				...
				DEBUG1(("Param overflow in cli_receive_trans\n"));
				goto out;
			}
			  ...
			  char **data, unsigned int *data_len)
{
	...
	*data_len = *param_len = 0;
	...
	if (CVAL(cli->inbuf,smb_com) != SMBnttrans) {
		DEBUG0(("Expected SMBnttrans response, got command 0x%02x\n",
			 CVAL(cli->inbuf,smb_com)));
		return(False);
	}
	...
	if (total_data) {
		*data = (char *)SMB_REALLOC(*data,total_data);
		if (!(*data)) {
			DEBUG0(("cli_receive_nt_trans: failed to enlarge data buffer to %d\n",total_data));
			goto out;
		}
	}
	...
	if (total_param) {
		*param = (char *)SMB_REALLOC(*param,total_param);
		if (!(*param)) {
			DEBUG0(("cli_receive_nt_trans: failed to enlarge param buffer to %d\n", total_param));
			goto out;
		}
	}
	...
	while (1)  {
		...
		if (this_data + *data_len > total_data ||
		    this_param + *param_len > total_param) {
			DEBUG1(("Data overflow in cli_receive_nt_trans\n"));
			goto out;
		}
		...
		if (this_param) {
			...
			if (param_offset_out > total_param ||
					...
					param_offset_out + this_param < param_offset_out ||
					param_offset_out + this_param < this_param) {
				DEBUG1(("Param overflow in cli_receive_nt_trans\n"));
				goto out;
			}
			if (param_offset_in > cli->bufsize ||
					param_offset_in + this_param >  cli->bufsize ||
