				  ...
				  const char *principal)
{
	...
	if (data.has_error) {
		DEBUG1(("Failed to build negTokenInit at offset %d\n", (int)data.ofs));
		asn1_free(&data);
	}
...
DATA_BLOB gen_negTokenTarg(const char *OIDs[], DATA_BLOB blob)
{
	...
	if (data.has_error) {
		DEBUG1(("Failed to build negTokenTarg at offset %d\n", (int)data.ofs));
		asn1_free(&data);
	}
...
BOOL parse_negTokenTarg(DATA_BLOB blob, char *OIDs[ASN1_MAX_OIDS], DATA_BLOB *secblob)
{
	...
	if (data.has_error) {
		...
		DEBUG1(("Failed to parse negTokenTarg at offset %d\n", (int)data.ofs));
		asn1_free(&data);
		return False;
	}
...
DATA_BLOB spnego_gen_krb5_wrap(const DATA_BLOB ticket, const uint8 tok_id[2])
{
	...
	if (data.has_error) {
		DEBUG1(("Failed to build krb5 wrapper at offset %d\n", (int)data.ofs));
		asn1_free(&data);
	}
...
BOOL spnego_parse_auth(DATA_BLOB blob, DATA_BLOB *auth)
{
	...
	if (data.has_error) {
		DEBUG3(("spnego_parse_auth failed at %d\n", (int)data.ofs));
		data_blob_free(auth);
		asn1_free(&data);
		return False;
	}
				...
				DATA_BLOB *auth)
{
	...
	if (data.has_error) {
		DEBUG3(("spnego_parse_auth_response failed at %d\n", (int)data.ofs));
		asn1_free(&data);
		data_blob_free(auth);
		return False;
	}
