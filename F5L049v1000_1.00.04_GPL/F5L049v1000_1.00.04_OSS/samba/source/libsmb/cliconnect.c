					  ...
					  const char *workgroup)
{
	...
	if (passlen > 0 && (cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) && passlen != 24) {
		...
		if (!SMBencrypt(pass, cli->secblob.data,(uchar *)lm_response.data)) {
			DEBUG1( ("Password is > 14 chars in length, and is therefore incompatible with Lanman authentication\n"));
...
static BOOL cli_session_setup_blob(struct cli_state *cli, DATA_BLOB blob)
{
	...
	if (cli->max_xmit < BASE_SESSSETUP_BLOB_PACKET_SIZE + 1) {
		DEBUG0(("cli_session_setup_blob: cli->max_xmit too small "
	...
	while ( remaining > 0) {
		...
		DEBUG10( ("cli_session_setup_blob: Remaining (%u) sending (%u) current (%u)\n", 
			(unsigned int)remaining,
			(unsigned int)send_blob.length,
			(unsigned int)cur ));

		if (!cli_session_setup_blob_send(cli, send_blob)) {
			DEBUG0( ("cli_session_setup_blob: send failed\n"));
			return False;
		}
		...
		if (cli_is_error(cli) &&
				!NT_STATUS_EQUAL( cli_get_nt_error(cli), 
					NT_STATUS_MORE_PROCESSING_REQUIRED)) {
			DEBUG0( ("cli_session_setup_blob: recieve failed (%s)\n",
...
static ADS_STATUS cli_session_setup_kerberos(struct cli_state *cli, const char *principal, const char *workgroup)
{
	...
	DEBUG2(("Doing kerberos session setup\n"));

	/* generate the encapsulated kerberos5 ticket */
	rc = spnego_gen_negTokenTarg(principal, 0, &negTokenTarg, &session_key_krb5, 0, NULL);

	if (rc) {
		DEBUG1( ("cli_session_setup_kerberos: spnego_gen_negTokenTarg failed: %s\n",
			error_message(rc)));
	...
	do {
		...
		if (NT_STATUS_EQUAL(nt_status, NT_STATUS_MORE_PROCESSING_REQUIRED) || NT_STATUS_IS_OK(nt_status)) {
			...
			if (!cli_session_setup_blob_send(cli, msg1)) {
				DEBUG3( ("Failed to send NTLMSSP/SPNEGO blob to server!\n"));
				nt_status = NT_STATUS_UNSUCCESSFUL;
			} else {
		...
		if (!blob.length) {
			...
			if (!spnego_parse_challenge(blob, &blob_in, 
						    &tmp_blob)) {
				DEBUG3(("Failed to parse challenges\n"));
			...
			if (!spnego_parse_auth_response(blob, nt_status, 
							&blob_in)) {
				DEBUG3(("Failed to parse auth response\n"));
				if (NT_STATUS_IS_OK(nt_status) 
				    || NT_STATUS_EQUAL(nt_status, NT_STATUS_MORE_PROCESSING_REQUIRED)) 
					nt_status = NT_STATUS_INVALID_PARAMETER;
			}
			      ...
			      const char * dest_realm)
{
	...
	DEBUG3(("Doing spnego session setup (blob length=%lu)\n", (unsigned long)cli->secblob.length));

	/* the server might not even do spnego */
	if (cli->secblob.length <= 16) {
		DEBUG3(("server didn't supply a full spnego negprot\n"));
		goto ntlmssp;
	}
	...
	if (got_kerberos_mechanism && cli->use_kerberos) {
		...
		if (principal == NULL &&
			...
			char *machine = NULL;
			char *host = NULL;
			DEBUG3(("cli_session_setup_spnego: got a "
			...
			if (realm && *realm) {
				...
				DEBUG3(("cli_session_setup_spnego: guessed "
					"server principal=%s\n",
					principal ? principal : "<null>"));
			}
			SAFE_FREE(machine);
			   ...
			   const char *workgroup)
{
	...
	if (cli->protocol < PROTOCOL_NT1) {
		if (!lp_client_lanman_auth() && passlen != 24 && (*pass)) {
			DEBUG1( ("Server requested LM password but 'client lanman auth'"
				  " is disabled\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
		...
		if ((cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) == 0 &&
			...
			DEBUG1( ("Server requested plaintext password but 'client use plaintext auth'"
				  " is disabled\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	...
	if (cli->capabilities & CAP_EXTENDED_SECURITY) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG3(("cli_session_setup: NT1 session setup "
				 "failed: %s\n", nt_errstr(status)));
			return status;
		}
	}
		    ...
		    const char *share, const char *dev, const char *pass, int passlen)
{
	...
	if (cli->sec_mode & NEGOTIATE_SECURITY_USER_LEVEL) {
		...
		DEBUG1( ("Server not using user level security and no password supplied.\n"));
		return False;
	}
	...
	if ((cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) &&
	    *pass && passlen != 24) {
		if (!lp_client_lanman_auth()) {
			DEBUG1( ("Server requested LANMAN password (share-level security) but 'client use lanman auth'"
				  " is disabled\n"));
			return False;
		}
...
BOOL cli_negprot(struct cli_state *cli)
{
	...
	if ((cli->protocol < PROTOCOL_NT1) && cli->sign_info.mandatory_signing) {
		DEBUG0(("cli_negprot: SMB signing is mandatory and the selected protocol level doesn't support it.\n"));
		return False;
	}
	...
	if (cli->protocol >= PROTOCOL_NT1) {    
		...
		if (cli->sec_mode & NEGOTIATE_SECURITY_SIGNATURES_REQUIRED) {
			...
			if (!cli->sign_info.allow_smb_signing) {
				DEBUG0(("cli_negprot: SMB signing is mandatory and we have disabled it.\n"));
				return False;
			}
			...
			if (!(cli->sec_mode & NEGOTIATE_SECURITY_SIGNATURES_ENABLED)) {
				DEBUG1(("cli_negprot: SMB signing is mandatory and the server doesn't support it.\n"));
				return False;
			}
			cli->sign_info.negotiated_smb_signing = True;
			cli->sign_info.mandatory_signing = True;
	...
	if (CVAL(cli->inbuf,0) == 0x84) {
		...
		*/
		...
		/* Try again */
		{
			...
			BOOL ret;
			if (depth > 4) {
				DEBUG0(("Retarget recursion - failing\n"));
				return False;
			}
			depth++;
...
NTSTATUS cli_connect(struct cli_state *cli, const char *host, struct in_addr *ip)
{
	...
	if (cli->fd == -1) {
		DEBUG1(("Error connecting to %s (%s)\n",
...
again:
	...
	DEBUG3(("Connecting to host=%s\n", dest_host));
	
	nt_status = cli_connect(cli, dest_host, &ip);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG1(("cli_start_connection: failed to connect to %s (%s). Error %s\n",
	...
	if (!cli_session_request(cli, &calling, &called)) {
		char *p;
		DEBUG1(("session request to %s failed (%s)\n", 
	...
	*output_cli = NULL;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		...
		if (!(flags & CLI_FULL_CONNECTION_ANONYMOUS_FALLBACK)) {
			DEBUG1(("failed session setup with %s\n",
				 nt_errstr(nt_status)));
			cli_shutdown(cli);
			return nt_status;
		}
		...
		if (!NT_STATUS_IS_OK(nt_status)) {
			DEBUG1(("anonymous failed session setup with %s\n",
				 nt_errstr(nt_status)));
			cli_shutdown(cli);
			return nt_status;
		}
                                     ...
                                     struct in_addr *pdest_ip)
{
	...
	if (!cli_session_request(*ppcli, &calling, &called)) {
		...
		if (nmb_name_equal(&called, &smbservername)) {
			...
			DEBUG0(("attempt_netbios_session_request: %s rejected the session for name *SMBSERVER \
with error %s.\n", desthost, cli_errstr(*ppcli) ));
		...
		*ppcli = cli_initialise();
		...
		if (!NT_STATUS_IS_OK(status) ||
			...
			DEBUG0(("attempt_netbios_session_request: %s rejected the session for \
...
struct cli_state *get_ipc_connect_master_ip(struct ip_service * mb_ip, pstring workgroup, struct user_auth_info *user_info)
{
	...
	struct in_addr server_ip; 
        ...
        if (!find_master_ip(name, &server_ip)) {
                ...
                DEBUG4( ("found master browser %s, %s\n", 
