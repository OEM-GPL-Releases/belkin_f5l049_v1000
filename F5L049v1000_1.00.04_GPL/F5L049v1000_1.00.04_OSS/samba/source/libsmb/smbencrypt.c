		  ...
		  uchar kr_buf[16])
{
	...
	if (user_byte_len == (size_t)-1) {
		DEBUG0( ("push_uss2_allocate() for user returned -1 (probably malloc() failure)\n"));
		return False;
	}

	domain_byte_len = push_ucs2_allocate(&domain, domain_in);
	if (domain_byte_len == (size_t)-1) {
		DEBUG0( ("push_uss2_allocate() for domain returned -1 (probably malloc() failure)\n"));
		SAFE_FREE(user);
		return False;
	}
		      ...
		      int string_flags)
{
	...
	*/
	...
	if ( (byte_len < 0) || (byte_len > 512)) {
		DEBUG0( ("decode_pw_buffer: incorrect password length (%d).\n", byte_len));
		DEBUG0( ("decode_pw_buffer: check that 'encrypt passwords = yes'\n"));
		return False;
	}
...
char *decrypt_trustdom_secret(const char *pass, DATA_BLOB *data_in)
{
	...
	if (length > data_in->length - 8) {
		DEBUG0(("decrypt_trustdom_secret: invalid length (%d)\n", length));
		return NULL;
	}
	
	if (version != 1) {
		DEBUG0(("decrypt_trustdom_secret: unknown version number (%d)\n", version));
		return NULL;
	}
