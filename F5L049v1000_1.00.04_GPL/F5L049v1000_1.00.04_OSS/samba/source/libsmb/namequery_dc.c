			...
			fstring srv_name)
{
	...
	if (i == 3) {
		DEBUG1(("ads_dc_name: sitename (now \"%s\") keeps changing ???\n",
			sitename ? sitename : ""));
		SAFE_FREE(sitename);
		return False;
	}
	...
	*dc_ip = ads->ldap_ip;
	...
	DEBUG4(("ads_dc_name: using server='%s' IP=%s\n",
...
static BOOL rpc_dc_name(const char *domain, fstring srv_name, struct in_addr *ip_out)
{
	...
	if (!NT_STATUS_IS_OK(get_sorted_dc_list(domain, NULL, &ip_list, &count,
						False))) {
		DEBUG3( ("Could not look up dc's for domain %s\n", domain));
		return False;
	}
	...
	DEBUG3( ("rpc_dc_name: Returning DC %s (%s) for domain %s\n", srv_name,
