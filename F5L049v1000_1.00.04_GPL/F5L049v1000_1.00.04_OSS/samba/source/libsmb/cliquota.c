...
BOOL cli_get_user_quota(struct cli_state *cli, int quota_fnum, SMB_NTQUOTA_STRUCT *pqt)
{
	...
	if (!cli_send_nt_trans(cli, 
			       ...
			       params, 16, 4,
			       data, data_len, 112)) {
		DEBUG1(("Failed to send NT_TRANSACT_GET_USER_QUOTA\n"));
		goto cleanup;
	}
	...
	if (!cli_receive_nt_trans(cli,
				  &rparam, &rparam_count,
				  &rdata, &rdata_count)) {
		DEBUG1(("Failed to recv NT_TRANSACT_GET_USER_QUOTA\n"));
		goto cleanup;
	}
	...
	if ((rparam&&rdata)&&(rparam_count>=4&&rdata_count>=8)) {
		...
		DEBUG0(("Got INVALID NT_TRANSACT_GET_USER_QUOTA reply.\n"));
		ret = False; 
	}
...
BOOL cli_set_user_quota(struct cli_state *cli, int quota_fnum, SMB_NTQUOTA_STRUCT *pqt)
{
	...
	if (!cli_send_nt_trans(cli, 
			       ...
			       params, 2, 0,
			       data, 112, 0)) {
		DEBUG1(("Failed to send NT_TRANSACT_SET_USER_QUOTA\n"));
		goto cleanup;
	}
	...
	if (!cli_receive_nt_trans(cli, 
				  &rparam, &rparam_count,
				  &rdata, &rdata_count)) {
		DEBUG1(("NT_TRANSACT_SET_USER_QUOTA failed\n"));
		goto cleanup;
	}
...
BOOL cli_list_user_quota(struct cli_state *cli, int quota_fnum, SMB_NTQUOTA_LIST **pqt_list)
{
	...
	for (curdata=rdata,curdata_count=rdata_count;
		...
		if (!parse_user_quota_record(curdata, curdata_count, &offset, &qt)) {
			DEBUG1(("Failed to parse the quota record\n"));
			goto cleanup;
		}
