...
static BOOL client_receive_smb(int fd,char *buffer, size_t bufsize, unsigned int timeout)
{
	...
	for(;;) {
		...
		if (!ret) {
			DEBUG10(("client_receive_smb failed\n"));
			show_msg(buffer);
			return ret;
		}
...
BOOL cli_receive_smb(struct cli_state *cli)
{
 ...
 again:
	...
	if (!ret) {
                DEBUG0( ("Receiving SMB: Server stopped responding\n"));
	...
	if (!cli_check_sign_mac(cli)) {
		...
		DEBUG0( ("SMB Signature verification failed on incoming packet!\n"));
		cli->smb_rw_error = READ_BAD_SIG;
		close(cli->fd);
		cli->fd = -1;
		return False;
...
static ssize_t write_socket(int fd, const char *buf, size_t len)
{
        ...
        DEBUG6(("write_socket(%d,%d)\n",fd,(int)len));
        ret = write_data(fd,buf,len);
                                                                                                                                            
        DEBUG6(("write_socket(%d,%d) wrote %d\n",fd,(int)len,(int)ret));
        if(ret <= 0)
                DEBUG0(("write_socket: Error writing %d bytes to socket %d: ERRNO = %s\n",
...
void cli_init_creds(struct cli_state *cli, const char *username, const char *domain, const char *password)
{
	...
	if (!*username) {
        ...
        DEBUG10(("cli_init_creds: user %s domain %s\n", cli->user_name, cli->domain));
}
...
struct cli_state *cli_initialise(void)
{
	...
	if (is_setuid_root()) {
		DEBUG0(("libsmb based programs must *NOT* be setuid root.\n"));
		return NULL;
	}
...
BOOL cli_rpc_pipe_close(struct rpc_pipe_client *cli)
{
	...
	if (!ret) {
		DEBUG1(("cli_rpc_pipe_close: cli_close failed on pipe %s, "
                         "fnum 0x%x "
...
{
        if (cli->fd == -1) {
                DEBUG3( ("cli_send_keepalive: fd == -1\n"));
                return False;
        }
        if (!send_keepalive(cli->fd)) {
                close(cli->fd);
                cli->fd = -1;
                DEBUG0(("Error sending keepalive packet to client.\n"));
                return False;
        }
        return True;
}
