					...
					krb5_enctype enctype)
{
	...
	if (ret) {
		DEBUG1(("krb5_principal2salt failed (%s)\n", error_message(ret)));
		return ret;
	}
					...
					krb5_enctype enctype)
{
	...
	if (ret) {
		DEBUG1(("krb5_get_pw_salt failed (%s)\n", error_message(ret)));
		return ret;
	}
...
BOOL unwrap_pac(TALLOC_CTX *mem_ctx, DATA_BLOB *auth_data, DATA_BLOB *unwrapped_pac_data)
{
	...
	if (data_type != KRB5_AUTHDATA_WIN2K_PAC ) {
		DEBUG10(("authorization data is not a Windows PAC (type: %d)\n", data_type));
		asn1_free(&data);
		return False;
	}
	    ...
	    tkt->enc_part2->authorization_data[0]->length)
	{
		for (i = 0; tkt->enc_part2->authorization_data[i] != NULL; i++) {
			...
			if (tkt->enc_part2->authorization_data[i]->ad_type != 
				...
				DEBUG10(("get_auth_data_from_tkt: ad_type is %d\n", 
 ...
 krb5_error_code smb_krb5_locate_kdc(krb5_context ctx, const krb5_data *realm, struct sockaddr **addr_pp, int *naddrs, int get_masters)
{
	...
	*naddrs = 0;
	...
	if (rc) {
		DEBUG0( ("smb_krb5_locate_kdc: krb5_krbhst_init failed (%s)\n", error_message(rc)));
		return rc;
	}
	...
	if (!num_kdcs) {
		DEBUG0( ("smb_krb5_locate_kdc: zero kdcs found !\n"));
	...
	if (!sa) {
		DEBUG0( ("smb_krb5_locate_kdc: malloc failed\n"));
		krb5_krbhst_free(ctx, hnd);
		naddrs = 0;
		return -1;
	}
	...
	for (i = 0; i < num_kdcs && (rc = krb5_krbhst_next(ctx, hnd, &hinfo) == 0); i++) {
		...
		if (rc) {
			DEBUG0(("krb5_krbhst_get_addrinfo failed: %s\n", error_message(rc)));
			continue;
		}
#endif
...
{
	DEBUG0( ("unable to explicitly locate the KDC on this platform\n"));
				      ...
				      krb5_creds  *credsp)
{
	...
	DEBUG3( ("ads_cleanup_expired_creds: Ticket in ccache[%s:%s] expiration %s\n",
	...
	*/
	if (strequal(cc_type, "FILE")) {
		DEBUG5( ("ads_cleanup_expired_creds: We do not remove creds from a %s ccache\n", cc_type));
		return False;
	}

	retval = krb5_cc_remove_cred(context, ccache, 0, credsp);
	if (retval) {
		DEBUG1( ("ads_cleanup_expired_creds: krb5_cc_remove_cred failed, err %s\n",
				       ...
				       time_t *expire_time)
{
	...
	if (retval) {
		DEBUG1(("ads_krb5_mk_req: Failed to parse principal %s\n", principal));
		return retval;
	}
	...
	if ((retval = krb5_copy_principal(context, server, &creds.server))) {
		DEBUG1(("ads_krb5_mk_req: krb5_copy_principal failed (%s)\n", 
			 error_message(retval)));
		goto cleanup_princ;
	}
	...
	if ((retval = krb5_cc_get_principal(context, ccache, &creds.client))) {
		...
		DEBUG3(("ads_krb5_mk_req: krb5_cc_get_principal failed (%s)\n", 
			 error_message(retval)));
		goto cleanup_creds;
	}
	...
	while (!creds_ready && (i < maxtries)) {
		...
		if ((retval = krb5_get_credentials(context, 0, ccache, 
						   &creds, &credsp))) {
			DEBUG1(("ads_krb5_mk_req: krb5_get_credentials failed for %s (%s)\n",
		...
		if ((unsigned)credsp->times.starttime > time(NULL)) {
			...
			DEBUG4(("ads_krb5_mk_req: Advancing clock by %d seconds to cope with clock skew\n", time_offset));
	...
	DEBUG10(("ads_krb5_mk_req: Ticket (%s) in ccache (%s:%s) is valid until: (%s - %u)\n",
	...
	if (retval) {
		DEBUG1(("ads_krb5_mk_req: krb5_mk_req_extended failed (%s)\n", 
			 error_message(retval)));
	}
...

{
	...
	if (retval) {
		DEBUG1(("cli_krb5_get_ticket: krb5_init_context failed (%s)\n", 
			 error_message(retval)));
		goto failed;
	}
	...
	if ((retval = krb5_cc_resolve(context, ccname ?
		...
		DEBUG1(("cli_krb5_get_ticket: krb5_cc_default failed (%s)\n",
			 error_message(retval)));
		goto failed;
	}

	if ((retval = krb5_set_default_tgs_ktypes(context, enc_types))) {
		DEBUG1(("cli_krb5_get_ticket: krb5_set_default_tgs_ktypes failed (%s)\n",
			 error_message(retval)));
		goto failed;
	}
...
failed:
 ...
 BOOL get_krb5_smb_session_key(krb5_context context, krb5_auth_context auth_context, DATA_BLOB *session_key, BOOL remote)
 {
	...
	if (err == 0 && skey != NULL) {
		DEBUG10( ("Got KRB5 session key of length %d\n",  (int)KRB5_KEY_LENGTH(skey)));
		*session_key = data_blob(KRB5_KEY_DATA(skey), KRB5_KEY_LENGTH(skey));
		...
		DEBUG10( ("KRB5 error getting session key %d\n", err));
	}
					 ...
					 size_t length)
{
...
#if defined(HAVE_KRB5_C_VERIFY_CHECKSUM)
	{
		...
		ret = krb5_c_verify_checksum(context, 
					     ...
					     &checksum_valid);
		if (ret) {
			DEBUG3(("smb_krb5_verify_checksum: krb5_c_verify_checksum() failed: %s\n", 
				error_message(ret)));
			return ret;
		}
...

	{
		...
		ret = krb5_crypto_init(context,
				       ...
				       &crypto);
		if (ret) {
			DEBUG0(("smb_krb5_verify_checksum: krb5_crypto_init() failed: %s\n", 
				error_message(ret)));
			return ret;
		}
		    ...
		    krb5_keyblock **out_key)
{
	...
	if (ret) {
		DEBUG0(("get_key_from_keytab: failed to open keytab: %s\n", error_message(ret)));
		return ret;
	}

	if ( DEBUGLEVEL >= 10 ) {
		if (smb_krb5_unparse_name(context, server, &name) == 0) {
			DEBUG10(("get_key_from_keytab: will look for kvno %d, enctype %d and name: %s\n", 
				kvno, enctype, name));
			SAFE_FREE(name);
		}
	}
	...
	if (ret) {
		DEBUG0(("get_key_from_keytab: failed to retrieve key: %s\n", error_message(ret)));
		goto out;
	}
...
#error UNKNOWN_KRB5_KEYTAB_ENTRY_FORMAT
	...
	if (ret) {
		DEBUG0(("get_key_from_keytab: failed to copy key: %s\n", error_message(ret)));
		goto out;
	}
							...
							krb5_keyblock **keyblock)
{
	...
	ret = get_key_from_keytab(context, 
				  ...
				  &local_keyblock);
	if (ret) {
		DEBUG0(("krb5_rd_req_return_keyblock_from_keytab: failed to call get_key_from_keytab\n"));
		goto out;
	}
				       ...
				       time_t *expire_time)
{
	...
	DEBUG10(("smb_krb5_renew_ticket: using %s as ccache\n", ccache_string));
...
#ifdef HAVE_KRB5_GET_RENEWED_CREDS	/* MIT */
	{
		...
		if (ret) {
			DEBUG10(("smb_krb5_renew_ticket: krb5_get_kdc_cred failed: %s\n", error_message(ret)));
			goto done;
		}
					...
					krb5_data *packet)
{
	...
	DEBUG10(("handle_krberror_packet: got error packet\n"));
...
#ifdef HAVE_E_DATA_POINTER_IN_KRB5_ERROR /* Heimdal */
	{
		...
		if ((ret = krb5_rd_error(context, packet, &krberror))) {
			DEBUG10(("handle_krberror_packet: krb5_rd_error failed with: %s\n", 
				error_message(ret)));
			return ret;
		}
...
#endif
	if (got_error_code) {
		DEBUG5(("handle_krberror_packet: got KERBERR from kpasswd: %s (%d)\n", 
