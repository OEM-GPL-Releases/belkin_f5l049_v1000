				     ...
				     uint16 mid, uint32 reply_seq_num)
{
	...
	DLIST_ADD(*list, t);
	DEBUG10(("store_sequence_for_reply: stored seq = %u mid = %u\n",
	...
	for (t = *list; t; t = t->next) {
		if (t->mid == mid) {
			*reply_seq_num = t->reply_seq_num;
			DEBUG10(("get_sequence_for_reply: found seq = %u mid = %u\n",
...
static BOOL set_smb_signing_real_common(struct smb_sign_info *si)
{
	if (si->mandatory_signing) {
		DEBUG5( ("Mandatory SMB signing enabled!\n"));
	}

	si->doing_signing = True;
	DEBUG5( ("SMB signing enabled!\n"));
...
static BOOL signing_good(char *inbuf, struct smb_sign_info *si, BOOL good, uint32 seq, BOOL must_be_ok) 
{
	if (good) {
		...
		if (!si->mandatory_signing && !si->seen_valid) {
			...
			DEBUG5( ("srv_check_incoming_message: signing negotiated but not required and peer\n"
			...
			if (seq)
				DEBUG0( ("signing_good: BAD SIG: seq %u\n", (unsigned int)seq));
			return False;
		}
	}
	return True;
				    ...
				    unsigned char calc_md5_mac[16])
{
	...
	DEBUG10(("simple_packet_signature: sequence number %u\n", seq_number ));
...
static void client_sign_outgoing_message(char *outbuf, struct smb_sign_info *si)
{
	...
	if (smb_len(outbuf) < (smb_ss_field + 8 - 4)) {
		DEBUG1( ("client_sign_outgoing_message: Logic error. Can't check signature on short packet! smb_len = %u\n",
					smb_len(outbuf) ));
		abort();
	}
	...
	DEBUG10( ("client_sign_outgoing_message: sent SMB signature of\n"));
...
static BOOL client_check_incoming_message(char *inbuf, struct smb_sign_info *si, BOOL must_be_ok)
{
	...
	if (smb_len(inbuf) < (smb_ss_field + 8 - 4)) {
		DEBUG1( ("client_check_incoming_message: Can't check signature on short packet! smb_len = %u\n", smb_len(inbuf)));
		return False;
	}

	if (!get_sequence_for_reply(&data->outstanding_packet_list, SVAL(inbuf, smb_mid), &reply_seq_number)) {
		DEBUG1( ("client_check_incoming_message: received message "
	...
	if (!good) {
		DEBUG5( ("client_check_incoming_message: BAD SIG: wanted SMB signature of\n"));
		dump_data(5, (const char *)calc_md5_mac, 8);
		
		DEBUG5( ("client_check_incoming_message: BAD SIG: got SMB signature of\n"));
...
#if 1 /* JRATEST */
		{
			...
			for (i = -5; i < 5; i++) {
				...
				if (memcmp(server_sent_mac, calc_md5_mac, 8) == 0) {
					DEBUG0(("client_check_incoming_message: out of seq. seq num %u matches. \
We were expecting seq %u\n", reply_seq_number+i, reply_seq_number ));
		...
		DEBUG10( ("client_check_incoming_message: seq %u: got good SMB signature of\n", (unsigned int)reply_seq_number));
			    ...
			    const DATA_BLOB response)
{
	...
	DEBUG10( ("cli_simple_set_signing: user_session_key\n"));
	...
	if (response.length) {
		...
		DEBUG10( ("cli_simple_set_signing: response_data\n"));
		dump_data(10, (const char *)response.data, response.length);
	} else {
		DEBUG10( ("cli_simple_set_signing: NULL response_data\n"));
	}
...
static void srv_sign_outgoing_message(char *outbuf, struct smb_sign_info *si)
{
	...
	/* JRA Paranioa test - we should be able to get rid of this... */
	if (smb_len(outbuf) < (smb_ss_field + 8 - 4)) {
		DEBUG1( ("srv_sign_outgoing_message: Logic error. Can't send signature on short packet! smb_len = %u\n",
					smb_len(outbuf) ));
		abort();
	}
	...
	DEBUG10( ("srv_sign_outgoing_message: seq %u: sent SMB signature of\n", (unsigned int)send_seq_number));
	dump_data(10, (const char *)calc_md5_mac, 8);
...
static BOOL srv_check_incoming_message(char *inbuf, struct smb_sign_info *si, BOOL must_be_ok)
{
	...
	if (smb_len(inbuf) < (smb_ss_field + 8 - 4)) {
		DEBUG1( ("srv_check_incoming_message: Can't check signature on short packet! smb_len = %u\n", smb_len(inbuf)));
		return False;
	}
	...
	if (!good) {
		...
		if (saved_seq) {
			DEBUG0( ("srv_check_incoming_message: BAD SIG: seq %u wanted SMB signature of\n",
					(unsigned int)saved_seq));
			dump_data(5, (const char *)calc_md5_mac, 8);

			DEBUG0( ("srv_check_incoming_message: BAD SIG: seq %u got SMB signature of\n",
						(unsigned int)reply_seq_number));
			dump_data(5, (const char *)server_sent_mac, 8);
		}
...
#if 1 /* JRATEST */
		{
			...
			for (i = 0; i < 10; i++, reply_seq_number++) {
				simple_packet_signature(data, (const unsigned char *)inbuf, reply_seq_number, calc_md5_mac);
				if (memcmp(server_sent_mac, calc_md5_mac, 8) == 0) {
					DEBUG0(("srv_check_incoming_message: out of seq. seq num %u matches. \
We were expecting seq %u\n", reply_seq_number, saved_seq ));
		...
		DEBUG10( ("srv_check_incoming_message: seq %u: (current is %u) got good SMB signature of\n", (unsigned int)reply_seq_number, (unsigned int)data->send_seq_num));
		dump_data(10, (const char *)server_sent_mac, 8);
	}
...
void srv_cancel_sign_response(uint16 mid)
{
	...
	DEBUG10(("srv_cancel_sign_response: for mid %u\n", (unsigned int)mid ));
...
void srv_set_signing(const DATA_BLOB user_session_key, const DATA_BLOB response)
{
	...
	if (!srv_sign_info.negotiated_smb_signing && !srv_sign_info.mandatory_signing) {
		DEBUG5(("srv_set_signing: signing negotiated = %u, mandatory_signing = %u. Not allowing smb signing.\n",
	...
	DEBUG3(("srv_set_signing: turning on SMB signing: signing negotiated = %s, mandatory_signing = %s.\n",
