						...
						BOOL *retry)
{
	...
	*retry = True;
	...
	if(!netlogon_pipe) {
		DEBUG0(("connect_to_domain_password_server: unable to open the domain client session to \
machine %s. Error was : %s.\n", dc_name, nt_errstr(result)));
	...
	if (!lp_client_schannel()) {
		...
		if (!get_trust_pw_hash(domain, machine_pwd, &account_name,
				       &sec_chan_type))
		{
			DEBUG0( ("connect_to_domain_password_server: could not fetch "
...

{
	...
	if ( !NT_STATUS_IS_OK(nt_status) ) {
		DEBUG0(("domain_client_validate: Domain password server not available.\n"));
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("domain_client_validate: unable to validate password "
					...
					auth_serversupplied_info **server_info)
{
	...
	if ( lp_server_role() != ROLE_DOMAIN_MEMBER ) {
		DEBUG0(("check_ntdomain_security: Configuration error!  Cannot use "
			"ntdomain auth method when not a member of a domain.\n"));
		return NT_STATUS_NOT_IMPLEMENTED;
	}

	if (!user_info || !server_info || !auth_context) {
		DEBUG1(("check_ntdomain_security: Critical variables not present.  Failing.\n"));
	...
	if(strequal(get_global_sam_name(), user_info->domain)) {
		DEBUG3(("check_ntdomain_security: Requested domain was for this machine.\n"));
		return NT_STATUS_NOT_IMPLEMENTED;
	}
	...
	if ( !get_dc_name(domain, NULL, dc_name, &dc_ip) ) {
		DEBUG5(("check_ntdomain_security: unable to locate a DC for domain %s\n",
			user_info->domain));
		return NT_STATUS_NO_LOGON_SERVERS;
	}
					   ...
					   auth_serversupplied_info **server_info)
{
	...
	if (!user_info || !server_info || !auth_context) {
		DEBUG1(("check_trustdomain_security: Critical variables not present.  Failing.\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if( strequal(get_global_sam_name(), user_info->domain)) {
		DEBUG3(("check_trustdomain_security: Requested domain [%s] was for this machine.\n",
			user_info->domain));
		return NT_STATUS_NOT_IMPLEMENTED;
	}
	...
	if (!secrets_fetch_trusted_domain_password(user_info->domain, &trust_password,
		...
		DEBUG0( ("check_trustdomain_security: could not fetch trust "
	...
	if ( !get_dc_name(user_info->domain, NULL, dc_name, &dc_ip) ) {
		DEBUG5(("check_trustdomain_security: unable to locate a DC for domain %s\n",
			user_info->domain));
		return NT_STATUS_NO_LOGON_SERVERS;
	}
