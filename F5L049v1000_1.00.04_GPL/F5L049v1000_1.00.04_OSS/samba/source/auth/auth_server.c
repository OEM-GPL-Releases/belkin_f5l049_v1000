...
static struct cli_state *server_cryptkey(TALLOC_CTX *mem_ctx)
{
	...
	p = pserver;
		...
		if(!resolve_name( desthost, &dest_ip, 0x20)) {
			DEBUG1(("server_cryptkey: Can't resolve address for %s\n",desthost));
			continue;
		}

		if (ismyip(dest_ip)) {
			DEBUG1(("Password server loop - disabling password server %s\n",desthost));
			continue;
		}
		...
		if (NT_STATUS_IS_OK(status)) {
			DEBUG3(("connected to password server %s\n",desthost));
			connected_ok = True;
			break;
		}
		DEBUG10(("server_cryptkey: failed to connect to server %s. Error %s\n",
			desthost, nt_errstr(status) ));
	}

	if (!connected_ok) {
		release_server_mutex();
		DEBUG0(("password server not available\n"));
		cli_shutdown(cli);
		return NULL;
	}
	...
	if (!attempt_netbios_session_request(&cli, global_myname(), 
					     desthost, &dest_ip)) {
		release_server_mutex();
		DEBUG1(("password server fails session request\n"));
		cli_shutdown(cli);
		return NULL;
	}
	...
	if (cli->protocol < PROTOCOL_LANMAN2 ||
		...
		DEBUG1(("%s isn't in user level security mode\n",desthost));
		release_server_mutex();
		cli_shutdown(cli);
		return NULL;
	}
	...
	*/
	...
	if (!NT_STATUS_IS_OK(cli_session_setup(cli, "", "", 0, "", 0,
					       ""))) {
		DEBUG0(("%s rejected the initial session setup (%s)\n",
...
static void free_server_private_data(void **private_data_pointer) 
{
	...
	if (*cli && (*cli)->initialised) {
		DEBUG10( ("Shutting down smbserver connection\n"));
		cli_shutdown(*cli);
	}
	...
	if (private_data_pointer) {
		...
		if (cli && cli->initialised) {
			if (!send_keepalive(cli->fd)) {
				DEBUG2( ( "send_server_keepalive: password server keepalive failed.\n"));
				cli_shutdown(cli);
				*private_data_pointer = NULL;
			}
		}
					   ...
					   TALLOC_CTX *mem_ctx)
{
	...
	if (cli) {
		DEBUG3(("using password server validation\n"));

		if ((cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) == 0) {
			/* We can't work with unencrypted password servers
			   unless 'encrypt passwords = no' */
			DEBUG5(("make_auth_info_server: Server is unencrypted, no challenge available..\n"));
			...
			*my_private_data = (void *)cli;
			...
			DEBUG2(("make_auth_info_server: Didn't receive a full challenge from server\n"));
			cli_shutdown(cli);
			return data_blob(NULL, 0);
		}
					 ...
					 auth_serversupplied_info **server_info)
{
	...
	if (!cli || !cli->initialised) {
		DEBUG1(("password server is not connected (cli not initilised)\n"));
		return NT_STATUS_LOGON_FAILURE;
	}  
	
	if ((cli->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) == 0) {
		if (user_info->encrypted) {
			DEBUG1(("password server %s is plaintext, but we are encrypted. This just can't work :-(\n", cli->desthost));
			return NT_STATUS_LOGON_FAILURE;		
		}
	} else {
		if (memcmp(cli->secblob.data, auth_context->challenge.data, 8) != 0) {
			DEBUG1(("the challenge that the password server (%s) supplied us is not the one we gave our client. This just can't work :-(\n", cli->desthost));
			return NT_STATUS_LOGON_FAILURE;		
		}
	}
	...
	if ((!tested_password_server) && (lp_paranoid_server_security())) {
		if (NT_STATUS_IS_OK(cli_session_setup(cli, baduser,
			...
			if ((SVAL(cli->inbuf,smb_vwv2) & 1) == 0) {
				DEBUG0(("server_validate: password server %s allows users as non-guest \
with a bad password.\n", cli->desthost));
				DEBUG0(("server_validate: This is broken (and insecure) behaviour. Please do not \
use this machine as the password server.\n"));
		...
		if(bad_password_server) {
			DEBUG0(("server_validate: [1] password server %s allows users as non-guest \
with a bad password.\n", cli->desthost));
			DEBUG0(("server_validate: [1] This is broken (and insecure) behaviour. Please do not \
use this machine as the password server.\n"));
			return NT_STATUS_LOGON_FAILURE;
		}
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG1(("password server %s rejected the password: %s\n",
			 cli->desthost, nt_errstr(nt_status)));
	}

	/* if logged in as guest then reject */
	if ((SVAL(cli->inbuf,smb_vwv2) & 1) != 0) {
		DEBUG1(("password server %s gave us guest only\n", cli->desthost));
