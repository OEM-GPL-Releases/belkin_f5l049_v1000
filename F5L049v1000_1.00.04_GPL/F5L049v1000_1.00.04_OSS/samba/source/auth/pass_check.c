...
static BOOL dfs_auth(char *user, char *password)
{
	...
	if (err != error_status_ok)
	{
		...
		DEBUG0( ("DCE can't get current context. %s\n", dce_errstr));
	...
	{
		dce_error_inq_text(err, dce_errstr, &err2);
		DEBUG0( ("DCE can't get expiration. %s\n", dce_errstr));
	...
	if (expire_time < (current_time + 60))
	{
		...
		{
			dce_error_inq_text(err, dce_errstr, &err2);
			DEBUG0( ("DCE can't refresh identity. %s\n",
				  dce_errstr));
		...
		{
			dce_error_inq_text(err, dce_errstr, &err2);
			DEBUG0( ("DCE can't get key for %s. %s\n",
	...
	{
		dce_error_inq_text(err, dce_errstr, &err2);
		DEBUG0( ("DCE Setup Identity for %s failed: %s\n",
			  user, dce_errstr));
		return (False);
	}
	...
	{
		dce_error_inq_text(err, dce_errstr, &err2);
		DEBUG0( ("DCE can't purge context. %s\n", dce_errstr));
	...
	{
		dce_error_inq_text(err, dce_errstr, &err2);
		DEBUG0( ("DCE certify identity failed: %s\n", dce_errstr));
		goto err;
	}

	if (auth_src != sec_login_auth_src_network)
	{
		DEBUG0( ("DCE context has no network credentials.\n"));
	}
	...
	DEBUG0( ("DCE login succeeded for principal %s on pid %d\n",
		  user, sys_getpid()));

	DEBUG3( ("DCE principal: %s\n"
		  "          uid: %d\n"
		  "          gid: %d\n",
	...
	if (t) {
		...
		if (asct) {
			DEBUG0(("DCE context expires: %s", asct));
		}
	}
...
#ifndef HAVE_CRYPT
	DEBUG1( ("Warning - no crypt available\n"));
		    ...
		    int pwlen, BOOL (*fn) (const char *, const char *), BOOL run_cracker)
{
	...
	DEBUG4( ("pass_check: Checking (PAM) password for user %s (l=%d)\n", user, pwlen));

#else /* Not using PAM */

	DEBUG4( ("pass_check: Checking password for user %s (l=%d)\n", user, pwlen));

	if (!pass) {
		DEBUG3( ("Couldn't find user %s\n", user));
...
#ifdef OSF1_ENH_SEC
	{
		...
		DEBUG5( ("Checking password for user %s in OSF1_ENH_SEC\n",
			  user));
	...
	if (!*this_crypted) {
		if (!lp_null_passwords()) {
			DEBUG2( ("Disallowing %s with null password\n",
				  this_user));
			return NT_STATUS_LOGON_FAILURE;
		}
		if (!*password) {
