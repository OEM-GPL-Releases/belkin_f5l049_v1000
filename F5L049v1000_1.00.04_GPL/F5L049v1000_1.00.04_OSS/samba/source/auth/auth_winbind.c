...
static NTSTATUS get_info3_from_ndr(TALLOC_CTX *mem_ctx, struct winbindd_response *response, NET_USER_INFO_3 *info3)
{
	...
	if (len > 0) {
		...
		if (!net_io_user_info3("", info3, &ps, 1, 3, False)) {
			DEBUG2( ("get_info3_from_ndr: could not parse info3 struct!\n"));
		...
		DEBUG2( ("get_info3_from_ndr: No info3 struct found!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
}
				       ...
				       auth_serversupplied_info **server_info)
{
	...
	if (!auth_context) {
		DEBUG3(("Password for user %s cannot be checked because we have no auth_info to get the challenge from.\n", 
			 user_info->internal_username));
		return NT_STATUS_INVALID_PARAMETER;
	}		

	if (strequal(user_info->domain, get_global_sam_name())) {
		DEBUG3(("check_winbind_security: Not using winbind, requested domain [%s] was for this SAM.\n",
			user_info->domain));
	...
	if ( result == NSS_STATUS_UNAVAIL )  {
		...
		if ( auth_method )
			...
			DEBUG0(("check_winbind_security: ERROR!  my_private_data == NULL!\n"));
	}
