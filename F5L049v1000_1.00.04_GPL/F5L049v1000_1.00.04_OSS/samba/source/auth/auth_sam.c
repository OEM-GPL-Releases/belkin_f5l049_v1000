				...
				DATA_BLOB *lm_sess_key)
{
	...
	if (acct_ctrl & ACB_PWNOTREQ) {
		if (lp_null_passwords()) {
			DEBUG3(("Account for user '%s' has no password and null passwords are allowed.\n", username));
			return NT_STATUS_OK;
		} else {
			DEBUG3(("Account for user '%s' has no password and null passwords are NOT allowed.\n", username));
...
static BOOL logon_hours_ok(struct samu *sampass)
{
	...
	if (!hours) {
		DEBUG5(("logon_hours_ok: No hours restrictions for user %s\n",pdb_get_username(sampass)));
		return True;
	}
	...
	if (!utctime) {
		DEBUG1( ("logon_hours_ok: failed to get gmtime. Failing logon for user %s\n",
			pdb_get_username(sampass) ));
		return False;
	}
	...
	if (! (hours[bitpos/8] & bitmask)) {
		...
		DEBUG1( ("logon_hours_ok: Account for user %s not allowed to "
	...
	DEBUG5(("logon_hours_ok: user %s allowed to logon at this time (%s)\n",
			       ...
			       const auth_usersupplied_info *user_info)
{
	...
	DEBUG4(("sam_account_ok: Checking SMB password for user %s\n",pdb_get_username(sampass)));

	/* Quit if the account was disabled. */
	if (acct_ctrl & ACB_DISABLED) {
		DEBUG1(("sam_account_ok: Account for user '%s' was disabled.\n", pdb_get_username(sampass)));
		return NT_STATUS_ACCOUNT_DISABLED;
	}

	/* Quit if the account was locked out. */
	if (acct_ctrl & ACB_AUTOLOCK) {
		DEBUG1(("sam_account_ok: Account for user %s was locked out.\n", pdb_get_username(sampass)));
	...
	if (kickoff_time != 0 && time(NULL) > kickoff_time) {
		DEBUG1(("sam_account_ok: Account for user '%s' has expired.\n", pdb_get_username(sampass)));
		DEBUG3(("sam_account_ok: Account expired at '%ld' unix time.\n", (long)kickoff_time));
	...
	if (!(pdb_get_acct_ctrl(sampass) & ACB_PWNOEXP) && !(pdb_get_acct_ctrl(sampass) & ACB_PWNOTREQ)) {
		...
		if (((acct_ctrl & (ACB_WSTRUST|ACB_SVRTRUST)) == 0) && (last_set_time == 0)) {
			DEBUG1(("sam_account_ok: Account for user '%s' password must change!.\n", pdb_get_username(sampass)));
			return NT_STATUS_PASSWORD_MUST_CHANGE;
		}

		/* check for expired password */
		if (must_change_time < time(NULL) && must_change_time != 0) {
			DEBUG1(("sam_account_ok: Account for user '%s' password expired!.\n", pdb_get_username(sampass)));
			DEBUG1(("sam_account_ok: Password expired at '%s' (%ld) unix time.\n", http_timestring(must_change_time), (long)must_change_time));
	...
	if (*workstation_list) {
		...
		while (next_token(&s, tok, ",", sizeof(tok))) {
			DEBUG10(("sam_account_ok: checking for workstation match %s and %s\n",
			...
			if (tok[0] == '+') {
				DEBUG10(("sam_account_ok: checking for workstation %s in group: %s\n", 
					machine_name, tok + 1));
	...
	if (acct_ctrl & ACB_DOMTRUST) {
		DEBUG2(("sam_account_ok: Domain trust account %s denied by server\n", pdb_get_username(sampass)));
		return NT_STATUS_NOLOGON_INTERDOMAIN_TRUST_ACCOUNT;
	}
	
	if (acct_ctrl & ACB_SVRTRUST) {
		if (!(user_info->logon_parameters & MSV1_0_ALLOW_SERVER_TRUST_ACCOUNT)) {
			DEBUG2(("sam_account_ok: Server trust account %s denied by server\n", pdb_get_username(sampass)));
	...
	if (acct_ctrl & ACB_WSTRUST) {
		if (!(user_info->logon_parameters & MSV1_0_ALLOW_WORKSTATION_TRUST_ACCOUNT)) {
			DEBUG2(("sam_account_ok: Wksta trust account %s denied by server\n", pdb_get_username(sampass)));
				   ...
				   auth_serversupplied_info **server_info)
{
	...
	if (ret == False) {
		DEBUG3(("check_sam_security: Couldn't find user '%s' in "
	...
	if (pdb_get_acct_ctrl(sampass) & ACB_AUTOLOCK) {
		DEBUG3(("check_sam_security: Account for user %s was locked out.\n", pdb_get_username(sampass)));
		return NT_STATUS_ACCOUNT_LOCKED_OUT;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("check_sam_security: make_server_info_sam() failed with '%s'\n", nt_errstr(nt_status)));
		TALLOC_FREE(sampass);
		data_blob_free(&user_sess_key);
		data_blob_free(&lm_sess_key);
		return nt_status;
					 ...
					 auth_serversupplied_info **server_info)
{
	...
	switch ( lp_server_role() ) {
		...
		case ROLE_DOMAIN_MEMBER:
			if ( !is_local_name ) {
				DEBUG6(("check_samstrict_security: %s is not one of my local names (%s)\n",
		...
		case ROLE_DOMAIN_BDC:
			if ( !is_local_name && !is_my_domain ) {
				DEBUG6(("check_samstrict_security: %s is not one of my local names or domain name (DC)\n",
					user_info->domain));
				return NT_STATUS_NOT_IMPLEMENTED;
			}
		default: /* name is ok */
