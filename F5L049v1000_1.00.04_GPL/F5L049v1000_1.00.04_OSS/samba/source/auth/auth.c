...
NTSTATUS smb_register_auth(int version, const char *name, auth_init_function init)
{
	...
	if (version != AUTH_INTERFACE_VERSION) {
		DEBUG0(("Can't register auth_method!\n"
	...
	DEBUG5(("Attempting to register auth backend %s\n", name));

	if (auth_find_backend_entry(name)) {
		DEBUG0(("There already is an auth method registered with the name %s!\n", name));
	...
	DEBUG5(("Successfully added auth method '%s'\n", name));
	return NT_STATUS_OK;
}
...
static const uint8 *get_ntlm_challenge(struct auth_context *auth_context) 
{
	...
	if (auth_context->challenge.length) {
		DEBUG5( ("get_ntlm_challenge (auth subsystem): returning previous challenge by module %s (normal)\n", 
	...
	for (auth_method = auth_context->auth_method_list; auth_method; auth_method = auth_method->next) {
		if (auth_method->get_chal == NULL) {
			DEBUG5( ("auth_get_challenge: module %s did not want to specify a challenge\n", auth_method->name));
			continue;
		}

		DEBUG5( ("auth_get_challenge: getting challenge from module %s\n", auth_method->name));
		if (challenge_set_by != NULL) {
			DEBUG1( ("auth_get_challenge: CONFIGURATION ERROR: authentication method %s has already specified a challenge.  Challenge by %s ignored.\n", 
		...
		if (!challenge.length) {
			DEBUG3( ("auth_get_challenge: getting challenge from authentication method %s FAILED.\n", 
				  auth_method->name));
		} else {
			DEBUG5( ("auth_get_challenge: successfully got challenge from module %s\n", auth_method->name));
	...
	DEBUG5( ("auth_context challenge created by %s\n", challenge_set_by));
	DEBUG5( ("challenge is: \n"));
...
static BOOL check_domain_match(const char *user, const char *domain) 
{
	...
	if (!lp_allow_trusted_domains() &&
	    !(strequal("", domain) || 
	      ...
	      is_myname(domain))) {
		DEBUG1( ("check_domain_match: Attempt to connect as user %s from domain %s denied.\n", user, domain));
		return False;
	} else {
		return True;
	}
				    ...
				    struct auth_serversupplied_info **server_info)
{
	...
	DEBUG3( ("check_ntlm_password:  Checking password for unmapped user [%s]\\[%s]@[%s] with the new password interface\n", 
		  user_info->client_domain, user_info->smb_name, user_info->wksta_name));

	DEBUG3( ("check_ntlm_password:  mapped user is: [%s]\\[%s]@[%s]\n", 
		  user_info->domain, user_info->internal_username, user_info->wksta_name));

	if (auth_context->challenge.length != 8) {
		DEBUG0( ("check_ntlm_password:  Invalid challenge stored for this auth context - cannot continue\n"));
		return NT_STATUS_LOGON_FAILURE;
	}

	if (auth_context->challenge_set_by)
		DEBUG10( ("check_ntlm_password: auth_context challenge created by %s\n",
					auth_context->challenge_set_by));

	DEBUG10( ("challenge is: \n"));
	dump_data(5, (const char *)auth_context->challenge.data, auth_context->challenge.length);
	...
	for (auth_method = auth_context->auth_method_list;auth_method; auth_method = auth_method->next) {
		...
		if ( NT_STATUS_V(result) == NT_STATUS_V(NT_STATUS_NOT_IMPLEMENTED) ) {
			DEBUG10(("check_ntlm_password: %s had nothing to say\n", auth_method->name));
			talloc_destroy(mem_ctx);
			continue;
		}
		...
		if (NT_STATUS_IS_OK(nt_status)) {
			DEBUG3( ("check_ntlm_password: %s authentication for user [%s] succeeded\n", 
				  auth_method->name, user_info->smb_name));
		} else {
			DEBUG5( ("check_ntlm_password: %s authentication for user [%s] FAILED with error %s\n", 
	...
	if (NT_STATUS_IS_OK(nt_status)) {
		...
		if (!(*server_info)->guest) {
			...
			if (NT_STATUS_IS_OK(nt_status)) {
				DEBUG5( ("check_ntlm_password:  PAM Account for user [%s] succeeded\n", 
					  unix_username));
			} else {
				DEBUG3( ("check_ntlm_password:  PAM Account for user [%s] FAILED with error %s\n", 
	...
	DEBUG2( ("check_ntlm_password:  Authentication for user [%s] -> [%s] FAILED with error %s\n", 
...
static NTSTATUS make_auth_context(struct auth_context **auth_context) 
{
	...
	*auth_context = TALLOC_P(mem_ctx, struct auth_context);
	if (!*auth_context) {
		DEBUG0(("make_auth_context: talloc failed!\n"));
		talloc_destroy(mem_ctx);
		      ...
		      const char *module, auth_methods **ret) 
{
	...
	DEBUG5(("load_auth_module: Attempting to find an auth method to match %s\n",
		 module));
	...
	if (entry != NULL) {
		if (!NT_STATUS_IS_OK(entry->init(auth_context, module_params, ret))) {
			DEBUG0(("load_auth_module: auth method %s did not correctly init\n",
				 module_name));
		} else {
			DEBUG5(("load_auth_module: auth method %s has a valid init\n",
				 module_name));
			good = True;
		}
	} else {
		DEBUG0(("load_auth_module: can't find auth method %s!\n", module_name));
	}
...
static NTSTATUS make_auth_context_text_list(struct auth_context **auth_context, char **text_list) 
{
	...
	if (!text_list) {
		DEBUG2(("make_auth_context_text_list: No auth method list!?\n"));
...
NTSTATUS make_auth_context_subsystem(struct auth_context **auth_context) 
{
	...
	if (auth_method_list == NULL) {
		...
		{
		case SEC_DOMAIN:
			DEBUG5(("Making default auth method list for security=domain\n"));
			auth_method_list = str_list_make("guest sam winbind:ntdomain", NULL);
			break;
		case SEC_SERVER:
			DEBUG5(("Making default auth method list for security=server\n"));
			auth_method_list = str_list_make("guest sam smbserver", NULL);
			break;
		case SEC_USER:
			if (lp_encrypted_passwords()) {	
				if ((lp_server_role() == ROLE_DOMAIN_PDC) || (lp_server_role() == ROLE_DOMAIN_BDC)) {
					DEBUG5(("Making default auth method list for DC, security=user, encrypt passwords = yes\n"));
					auth_method_list = str_list_make("guest sam winbind:trustdomain", NULL);
				} else {
					DEBUG5(("Making default auth method list for standalone security=user, encrypt passwords = yes\n"));
					auth_method_list = str_list_make("guest sam", NULL);
				}
			} else {
				DEBUG5(("Making default auth method list for security=user, encrypt passwords = no\n"));
				auth_method_list = str_list_make("guest unix", NULL);
			}
			break;
		case SEC_SHARE:
			if (lp_encrypted_passwords()) {
				DEBUG5(("Making default auth method list for security=share, encrypt passwords = yes\n"));
				auth_method_list = str_list_make("guest sam", NULL);
			} else {
				DEBUG5(("Making default auth method list for security=share, encrypt passwords = no\n"));
				auth_method_list = str_list_make("guest unix", NULL);
			}
			break;
		case SEC_ADS:
			DEBUG5(("Making default auth method list for security=ADS\n"));
			auth_method_list = str_list_make("guest sam winbind:ntdomain", NULL);
			break;
		default:
			DEBUG5(("Unknown auth method!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
	} else {
		DEBUG5(("Using specified auth order\n"));
	}
