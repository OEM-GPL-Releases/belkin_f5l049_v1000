                               ...
                               BOOL encrypted)
{
	...
	DEBUG5(("attempting to make a user_info for %s (%s)\n", internal_username, smb_name));

	*user_info = SMB_MALLOC_P(auth_usersupplied_info);
	if (*user_info == NULL) {
		DEBUG0(("malloc failed for user_info (size %lu)\n", (unsigned long)sizeof(*user_info)));
		return NT_STATUS_NO_MEMORY;
	}

	ZERO_STRUCTP(*user_info);

	DEBUG5(("making strings for %s's user_info struct\n", internal_username));
	...
	DEBUG5(("making blobs for %s's user_info struct\n", internal_username));
	...
	DEBUG10(("made an %sencrypted user_info for %s (%s)\n", encrypted ? "":"un" , internal_username, smb_name));
			    ...
			    BOOL encrypted)
{
	...
	DEBUG5( ("make_user_info_map: Mapping user [%s]\\[%s] from workstation [%s]\n",
			      ...
			      DATA_BLOB plaintext_password)
{
	...
	DEBUG5(("make_user_info_for_reply: User passwords not in encrypted "
		 "format.\n"));
...
#ifdef DEBUG_PASSWORD
		DEBUG10(("Unencrypted password (len %d):\n",
...
NT_USER_TOKEN *get_root_nt_token( void )
{
	...
	if ( !(pw = sys_getpwnam( "root" )) ) {
		DEBUG0(("get_root_nt_token: getpwnam\"root\") failed!\n"));
		return NULL;
	}
			      ...
			      struct samu *sampass)
{
	...
	if ( !(pwd = getpwnam_alloc(NULL, username)) ) {
		DEBUG1( ("User %s in passdb, but getpwnam() fails!\n",
			  username));
	...
	if (IS_DC && is_our_machine_account(username)) {
		...
		DEBUG10( ("make_server_info_sam: our machine account %s "
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG10( ("pdb_enum_group_memberships failed: %s\n",
				   nt_errstr(status)));
	...
	for ( i=0; i<num_gids; i++ ) {
		if ( !gid_to_unix_groups_sid( gids[i], &unix_group_sid ) ) {
			DEBUG1(("make_server_info_sam: Failed to create SID "
				"for gid %d!\n", gids[i]));
			continue;
		}
	...
	DEBUG5(("make_server_info_sam: made server info for user %s -> %s\n",
			    ...
			    struct nt_user_token *token)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG10( ("pdb_enum_alias_memberships failed: %s\n",
			   nt_errstr(status)));
		TALLOC_FREE(tmp_ctx);
		return status;
	}
	...
	for (i=0; i<num_aliases; i++) {
		...
		if (!add_sid_to_array_unique(token, &alias_sid,
					&token->user_sids,
					&token->num_sids)) {
			DEBUG0( ("add_sid_to_array failed\n"));
			TALLOC_FREE(tmp_ctx);
			return NT_STATUS_NO_MEMORY;
		}
	}
...
static NTSTATUS log_nt_token(TALLOC_CTX *tmp_ctx, NT_USER_TOKEN *token)
{
	...
	DEBUG8( ("running command: [%s]\n", command));
	if (smbrun(command, NULL) != 0) {
		DEBUG0( ("Could not log NT token\n"));
...
static NTSTATUS create_builtin_users( void )
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG0(("create_builtin_users: Failed to create Users\n"));
		return status;
	}
		...
		&& secrets_fetch_domain_sid(lp_workgroup(), &dom_users))
	{
		...
		if ( !NT_STATUS_IS_OK(status) ) {
			DEBUG0(("create_builtin_users: Failed to add Domain Users to"
				" Users\n"));
			return status;
		}
	}
...
static NTSTATUS create_builtin_administrators( void )
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG0(("create_builtin_administrators: Failed to create Administrators\n"));
		return status;
	}
		...
		&& secrets_fetch_domain_sid(lp_workgroup(), &dom_admins))
	{
		...
		if ( !NT_STATUS_IS_OK(status) ) {
			DEBUG0(("create_builtin_administrators: Failed to add Domain Admins"
				" Administrators\n"));
			return status;
		}
	}
	...
	if ( ret ) {
		...
		if ( !NT_STATUS_IS_OK(status) ) {
			DEBUG0(("create_builtin_administrators: Failed to add root"
				" Administrators\n"));
			return status;
		}
	}
						   ...
						   const DOM_SID *groupsids)
{
	...
	DEBUG10( ("Create local NT token for %s\n", sid_string_static(user_sid)));
	...
	if ( !sid_to_gid( &global_sid_Builtin_Administrators, &gid ) ) {
		...
		if ( lp_winbind_nested_groups() && winbind_ping() ) {
			...
			if ( !NT_STATUS_IS_OK(status) ) {
				DEBUG2(("create_local_nt_token: Failed to create BUILTIN\\Administrators group!\n"));
			...
			if ( !NT_STATUS_IS_OK(status) ) {
				...
				DEBUG3(("create_local_nt_token: failed to check for local Administrators"
					" membership (%s)\n", nt_errstr(status)));
			}			
		}		
	}
	...
	if ( !sid_to_gid( &global_sid_Builtin_Users, &gid ) ) {
		...
		if ( lp_winbind_nested_groups() && winbind_ping() ) {
			...
			if ( !NT_STATUS_IS_OK(status) ) {
				DEBUG2(("create_local_nt_token: Failed to create BUILTIN\\Users group!\n"));
				/* don't fail, just log the message */
			}
			unbecome_root();
		}
...
NTSTATUS create_local_token(auth_serversupplied_info *server_info)
{
	...
	for (i=1; i<server_info->ptok->num_sids; i++) {
		...
		if (!sid_to_gid(sid, &gid)) {
			DEBUG10( ("Could not convert SID %s to gid, "
				    ...
				    struct nt_user_token **token)
{
	...
	if (!lookup_name_smbconf(tmp_ctx, username, LOOKUP_NAME_ALL,
		...
		DEBUG1( ("lookup_name_smbconf for %s failed\n", username));
		goto done;
	}

	if (type != SID_NAME_USER) {
		DEBUG1( ("%s is a %s, not a user\n", username,
			  sid_type_lookup(type)));
		goto done;
	}

	if (!sid_to_uid(&user_sid, uid)) {
		DEBUG1( ("sid_to_uid for %s (%s) failed\n",
	...
	if (sid_check_is_in_our_domain(&user_sid)) {
		...
		if (!ret) {
			DEBUG1( ("pdb_getsampwsid(%s) for user %s failed\n",
		...
		result = pdb_enum_group_memberships(tmp_ctx, sam_acct,
						    ...
						    &num_group_sids);
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG10( ("enum_group_memberships failed for %s\n",
				   username));
			DEBUGADD(1, ("Fall back to unix user %s\n", username));
			goto unix_user;
		}
	...
	unix_user:
		...
		if (pass == NULL) {
			DEBUG1( ("getpwuid(%d) for user %s failed\n",
				  *uid, username));
			goto done;
		}
		...
		if (!getgroups_unix_user(tmp_ctx, username, pass->pw_gid,
					 &gids, &num_group_sids)) {
			DEBUG1( ("getgroups_unix_user for user %s failed\n",
				  username));
			goto done;
		}
	...
	for ( i=0; i<num_gids; i++ ) {
		...
		if ( !gid_to_unix_groups_sid( gids[i], &unix_group_sid ) ) {
			DEBUG1(("create_token_from_username: Failed to create SID "
				"for gid %d!\n", gids[i]));
			continue;
		}
...
BOOL user_in_group_sid(const char *username, const DOM_SID *group_sid)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG10( ("could not create token for %s\n", username));
		return False;
	}
...
BOOL user_in_group(const char *username, const char *groupname)
{
	...
	if (!ret) {
		DEBUG10( ("lookup_name for (%s) failed.\n", groupname));
		return False;
	}
...
static NTSTATUS make_new_server_info_guest(auth_serversupplied_info **server_info)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG10( ("create_local_token failed: %s\n",
			   nt_errstr(status)));
		return status;
	}
				 ...
				 BOOL *username_was_mapped)
{
	...
	*found_username = talloc_strdup( mem_ctx, real_username );
	...
	DEBUG5(("fill_sam_account: located username was [%s]\n", *found_username));
				...
				NET_USER_INFO_3 *info3) 
{
	...
	*/
	...
	result = make_server_info(NULL);
	if (result == NULL) {
		DEBUG4( ("make_server_info failed!\n"));
		TALLOC_FREE(sam_account);
		return NT_STATUS_NO_MEMORY;
	}
	...
	for (i = 0; i < info3->num_groups2; i++) {
		...
		if (!sid_compose(&sid, &info3->dom_sid.sid,
				 info3->gids[i].g_rid)) {
			DEBUG3(("could not append additional group rid "
				 "0x%x\n", info3->gids[i].g_rid));
			TALLOC_FREE(result);
			return NT_STATUS_INVALID_PARAMETER;
		}
...
{
	DEBUG5(("attempting to free (and zero) a user_info structure\n"));
	if (*user_info != NULL) {
		if ((*user_info)->smb_name) {
			DEBUG10(("structure was created for %s\n",
				  (*user_info)->smb_name));
		}
...
BOOL make_auth_methods(struct auth_context *auth_context, auth_methods **auth_method) 
{
	...
	*auth_method = TALLOC_P(auth_context->mem_ctx, auth_methods);
	if (!*auth_method) {
		DEBUG0(("make_auth_method: malloc failed!\n"));
		return False;
	}
...
NT_USER_TOKEN *dup_nt_token(TALLOC_CTX *mem_ctx, const NT_USER_TOKEN *ptoken)
{
	...
	if ( !se_priv_copy( &token->privileges, &ptoken->privileges ) ) {
		DEBUG0(("dup_nt_token: Failure to copy SE_PRIV!.  "
...
BOOL nt_token_check_domain_rid( NT_USER_TOKEN *token, uint32 rid ) 
{
	...
	if ( lp_server_role() == ROLE_DOMAIN_MEMBER ) {
		if ( !secrets_fetch_domain_sid( lp_workgroup(),
						&domain_sid ) ) {
			DEBUG1(("nt_token_check_domain_rid: Cannot lookup "
				 "SID for domain [%s]\n", lp_workgroup()));
			return False;
		}
	} 
