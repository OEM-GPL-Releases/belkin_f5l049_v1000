	...
	int32_t len;
		...
		if(type->u64 == ACL_NFS4) {
			...
			if(type->u64 == ACL_AIXC) {
				...
				DEBUG0(("aixacl2_getlen:unknown type:%d\n",type->u64));
                		return False;
			}	
		}		
		DEBUG10(("aixacl2_getlen:%d\n",len));
	return len;
}
...
static AIXJFS2_ACL_T *aixjfs2_getacl_alloc(const char *fname, acl_type_t *type)
{
	...
	if ((ret != 0) && (errno == ENOSPC)) {
		...
		DEBUG10(("aixjfs2_getacl_alloc - acl_len:%d\n",len));
	...
	SMB4ACL_T **ppacl, BOOL *pretryPosix)
{
	...
	DEBUG10(("jfs2 get_nt_acl invoked for %s\n", fsp->fsp_name));
	...
	pacl = aixjfs2_getacl_alloc(fsp->fsp_name, &type);
        if (pacl == NULL) {
		DEBUG9( ("aixjfs2_getacl_alloc failed for %s with %s\n",
	...
	DEBUG10( ("len: %d, version: %d, nace: %d, type: 0x%x\n",
	...
	*ppacl = smb_create_smb4acl();
	...
	for (i=0; i<jfs2_acl->aclEntryN; i++) {
		...
		DEBUG10( ("type: %d, iflags: %x, flags: %x, mask: %x, "
	...
	DEBUG10(("jfs2 get_nt_acl finished successfully\n"));
	...
	uint32 security_info, SEC_DESC **ppdesc)
{
	...
	*ppdesc = NULL;
	...
	if (retryPosix)
	{
		DEBUG10( ("retrying with posix acl...\n"));
...
static SMB_ACL_T aixjfs2_get_posix_acl(const char *path, acl_type_t type)
{
	...
	int ret;
        ...
        if (acl == NULL) {
                DEBUG10( ("aixjfs2_getacl failed for %s with %s\n",
                           path, strerror(errno)));
                if (errno == 0) {
                        errno = EINVAL;
                }
                                    ...
                                    SMB_ACL_TYPE_T type)
{
        ...
        case SMB_ACL_TYPE_DEFAULT:
                DEBUG0( ("Got AIX JFS2 unsupported type: %d\n", type));
                return NULL;
        default:
                DEBUG0( ("Got invalid type: %d\n", type));
                smb_panic("exiting");
        }
...
)
{
	...
	if (aclx_gettypes(filepath, &acl_type_list, &acl_type_list_len)) {
		DEBUG2( ("aclx_gettypes failed with error %s for %s\n",
	...
	for(i=0; i<acl_type_list.num_entries; i++) {
		if (acl_type_list.entries[i].u64==aclType) {
			...
			DEBUG10( ("found %s ACL support for %s\n",
...
static BOOL aixjfs2_process_smbacl(files_struct *fsp, SMB4ACL_T *smbacl)
{
	...
	DEBUG10( ("jfs2_process_smbacl invoked on %s\n", fsp->fsp_name));
	...
	if (rc) {
		DEBUG8( ("aclx_put failed with %s\n", strerror(errno)));
		return False;
	}

	DEBUG10( ("jfs2_process_smbacl succeeded.\n"));
	return True;
}
			      ...
			      SMB_ACL_T theacl)
{
	...
	DEBUG10( ("aixjfs2_sys_acl_set_file invoked for %s", name));

	rc = aixjfs2_query_acl_support((char *)name, ACL_AIXC, &acl_type_info);
	if (rc) {
		DEBUG8( ("jfs2_set_nt_acl: AIXC support not found\n"));
		return -1;
	}
	...
	if (rc) {
		DEBUG2( ("aclx_put failed with %s for %s\n",
			strerror(errno), name));
		return -1;
	}
			    ...
			    int fd, SMB_ACL_T theacl)
{
	...
	DEBUG10( ("aixjfs2_sys_acl_set_fd invoked for %s", fsp->fsp_name));
	...
	if (rc) {
		DEBUG2( ("aclx_fput failed with %s for %s\n",
