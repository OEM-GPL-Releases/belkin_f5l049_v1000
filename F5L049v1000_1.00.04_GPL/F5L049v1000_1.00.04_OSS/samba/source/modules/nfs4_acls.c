...
static int smbacl4_GetFileOwner(files_struct *fsp, SMB_STRUCT_STAT *psbuf)
{
	...
	if (fsp->is_directory || fsp->fh->fd == -1) {
		...
		{
			DEBUG8( ("SMB_VFS_STAT failed with error %s\n",
				strerror(errno)));
			return -1;
		}
	} else {
		...
		{
			DEBUG8( ("SMB_VFS_FSTAT failed with error %s\n",
				strerror(errno)));
			return -1;
		}
	}
...
)
{
	...
	DEBUG10( ("smbacl_nfs42win entered"));
	...
	for (aceint=aclint->first; aceint!=NULL; aceint=(SMB_ACE4_INT_T *)aceint->next) {
		...
		DEBUG10( ("magic: 0x%x, type: %d, iflags: %x, flags: %x, mask: %x, "
		...
		if (ace->flags & SMB_ACE4_ID_SPECIAL) {
			...
			case SMB_ACE4_WHO_EVERYONE:
				...
				break;
			default:
				DEBUG8( ("invalid special who id %d "
					"ignored\n", ace->who.special_id));
			}
		} else {
	...
	SEC_DESC **ppdesc, SMB4ACL_T *acl)
{
	...
	DEBUG10( ("smb_get_nt_acl_nfs4 invoked for %s\n", fsp->fsp_name));
	...
	if (smbacl4_nfs42win(acl,
		...
		DEBUG8(("smbacl4_nfs42win failed\n"));
		return 0;
	}
	...
	if (psa == NULL) {
		DEBUG2(("make_sec_acl failed\n"));
		return 0;
	}

	DEBUG10(("after make sec_acl\n"));
	*ppdesc = make_sec_desc(mem_ctx, SEC_DESC_REVISION,
		...
		NULL, psa, &sd_size);
	if (*ppdesc==NULL) {
		DEBUG2(("make_sec_desc failed\n"));
		return 0;
	}

	DEBUG10( ("smb_get_nt_acl_nfs4 successfully exited with sd_size %d\n", sd_size));
	return sd_size;
}
...
)
{
	...
	DEBUG10( ("mode:%s, do_chown:%s, acedup: %s\n",
...
)
{
	...
	if (ace_v4->aceFlags!=ace_nt->flags)
		DEBUG9( ("ace_v4->aceFlags(0x%x)!=ace_nt->flags(0x%x)\n",
			ace_v4->aceFlags, ace_nt->flags));

	if (ace_v4->aceMask!=ace_nt->access_mask)
		DEBUG9( ("ace_v4->aceMask(0x%x)!=ace_nt->access_mask(0x%x)\n",
			ace_v4->aceMask, ace_nt->access_mask));
	...
	if (sid_equal(&ace_nt->trustee, &global_sid_World)) {
		...
		if (type == SID_NAME_USER) {
			if (!sid_to_uid(&ace_nt->trustee, &uid)) {
				DEBUG2( ("Could not convert %s to uid\n",
					sid_string_static(&ace_nt->trustee)));
				return -1;
			}
			...
			if (!sid_to_gid(&ace_nt->trustee, &gid)) {
				DEBUG2( ("Could not convert %s to gid\n",
					sid_string_static(&ace_nt->trustee)));
				return -1;
			}
...
)
{
	...
	if (ace4found)
	{
		...
		case e_reject: /* do an error */
			DEBUG8( ("ACL rejected by duplicate nt ace#%d\n", i));
...
)
{
	...
	DEBUG10( ("smbacl4_win2nfs4 invoked\n"));
	...
	set_nfs4acl_native_fn_t set_nfs4_native)
{
	...
	DEBUG10( ("smb_set_nt_acl_nfs4 invoked for %s\n", fsp->fsp_name));

	if ((security_info_sent & (DACL_SECURITY_INFORMATION |
		GROUP_SECURITY_INFORMATION | OWNER_SECURITY_INFORMATION)) == 0)
	{
		DEBUG9( ("security_info_sent (0x%x) ignored\n",
			security_info_sent));
	...
	if (params.do_chown) {
		...
		{
			DEBUG8( ("unpack_nt_owners failed"));
			return False;
		}
	...
	if ((security_info_sent & DACL_SECURITY_INFORMATION)!=0 && psd->dacl!=NULL)
	{
		...
		if (result!=True)
		{
			DEBUG10( ("set_nfs4_native failed with %s\n", strerror(errno)));
			return False;
		}
	} else
		DEBUG10( ("no dacl found; security_info_sent = 0x%x\n", security_info_sent));

	/* Any chown pending? */
	if (need_chown) {
		DEBUG3(("chown#2 %s. uid = %u, gid = %u.\n",
			fsp->fsp_name, (unsigned int)newUID, (unsigned int)newGID));
		if (try_chown(fsp->conn, fsp->fsp_name, newUID, newGID)) {
			DEBUG2(("chown#2 %s, %u, %u failed. Error = %s.\n",
				fsp->fsp_name, (unsigned int)newUID, (unsigned int)newGID,
				strerror(errno)));
			return False;
		}
		DEBUG10(("chown#2 %s, %u, %u succeeded.\n",
			fsp->fsp_name, (unsigned int)newUID, (unsigned int)newGID));
	}

	DEBUG10( ("smb_set_nt_acl_nfs4 succeeded\n"));
	return True;
}
