...
static BOOL read_target_host(const char *mapfile, pstring targethost)
{
	...
	if (f == NULL) {
		DEBUG0(("can't open IP map %s. Error %s\n",
			 mapfile, strerror(errno) ));
		return False;
	}

	DEBUG10( ("Scanning mapfile [%s]\n", mapfile));
	...
	while (x_fgets(buf, sizeof(buf), f) != NULL) {
		...
		DEBUG10( ("Scanning line [%s]\n", buf));

		space = strchr_m(buf, ' ');

		if (space == NULL) {
			DEBUG0( ("Ignoring invalid line %s\n", buf));
			continue;
		}
...
static BOOL expand_msdfs_target(connection_struct* conn, pstring target)
{
	...
	if (filename_start == NULL) {
		DEBUG10( ("No filename start in %s\n", target));
		return False;
	}
	...
	DEBUG10( ("Expanding from table [%s]\n", mapfilename));

	if (!read_target_host(mapfilename, targethost)) {
		DEBUG1( ("Could not expand target host from file %s\n",
			  mapfilename));
		return False;
	}
	...
	DEBUG10( ("Expanded targethost to %s\n", targethost));
