...
static SMB_STRUCT_DIR *shadow_copy_opendir(vfs_handle_struct *handle, const char *fname, const char *mask, uint32 attr)
{
	...
	if (!p) {
		DEBUG0(("shadow_copy_opendir: SMB_VFS_NEXT_OPENDIR() failed for [%s]\n",fname));
		return NULL;
	}

	dirp = SMB_MALLOC_P(shadow_copy_Dir);
	if (!dirp) {
		DEBUG0(("shadow_copy_opendir: Out of memory\n"));
	...
	while (True) {
		...
		if (shadow_copy_match_name(d->d_name)) {
			DEBUG8(("shadow_copy_opendir: hide [%s]\n",d->d_name));
			continue;
		}

		DEBUG10(("shadow_copy_opendir: not hide [%s]\n",d->d_name));
...
static int shadow_copy_get_shadow_copy_data(vfs_handle_struct *handle, files_struct *fsp, SHADOW_COPY_DATA *shadow_copy_data, BOOL labels)
{
	...
	if (!p) {
		DEBUG0(("shadow_copy_get_shadow_copy_data: SMB_VFS_NEXT_OPENDIR() failed for [%s]\n",fsp->conn->connectpath));
		return -1;
	}
	...
	while (True) {
		...
		/* */
		if (!shadow_copy_match_name(d->d_name)) {
			DEBUG10(("shadow_copy_get_shadow_copy_data: ignore [%s]\n",d->d_name));
			continue;
		}

		DEBUG7(("shadow_copy_get_shadow_copy_data: not ignore [%s]\n",d->d_name));
		...
		if (tlabels == NULL) {
			DEBUG0(("shadow_copy_get_shadow_copy_data: Out of memory\n"));
			SMB_VFS_NEXT_CLOSEDIR(handle,p);
			return -1;
		}
...
NTSTATUS vfs_shadow_copy_init(void)
{
	...
	if (vfs_shadow_copy_debug_level == -1) {
		...
		DEBUG0( ("%s: Couldn't register custom debugging class!\n",
			"vfs_shadow_copy_init"));
	} else {
		DEBUG10( ("%s: Debug class number of '%s': %d\n", 
