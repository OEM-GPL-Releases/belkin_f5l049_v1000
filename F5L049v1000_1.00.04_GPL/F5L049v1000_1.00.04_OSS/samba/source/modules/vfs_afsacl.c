...
static BOOL init_afs_acl(struct afs_acl *acl)
{
	...
	if (acl->ctx == NULL) {
		DEBUG10( ("Could not init afs_acl"));
		return False;
	}
	return True;
}
				   ...
				   const char *name, uint32 rights)
{
	...
	if (strcmp(name, "system:administrators") == 0) {
		...
		if (!lookup_name(tmp_talloc_ctx(), name, LOOKUP_NAME_ALL,
			...
			DEBUG10( ("Could not find AFS user %s\n", name));
	...
	if (result == NULL) {
		DEBUG0( ("Could not talloc AFS ace\n"));
		return NULL;
	}

	result->name = talloc_strdup(mem_ctx, name);
	if (result->name == NULL) {
		DEBUG0( ("Could not talloc AFS ace name\n"));
		return NULL;
	}
			...
			const char *name, uint32 rights)
{
	...
	DEBUG10( ("add_afs_ace: Added %s entry for %s with rights %d\n",
...
static BOOL parse_afs_acl(struct afs_acl *acl, const char *acl_str)
{
	...
	DEBUG10( ("Found %d positive entries\n", nplus));
	...
	DEBUG10( ("Found %d negative entries\n", nminus));
...
static uint32 nt_to_afs_dir_rights(const char *filename, const SEC_ACE *ace)
{
	...
	DEBUG1( ("AFSACL FALLBACK: 0x%X 0x%X 0x%X %s %X\n",
			    ...
			    struct security_descriptor **ppdesc)
{
	...
	while (afs_ace != NULL) {
		...
		if (afs_ace->type == SID_NAME_UNKNOWN) {
			DEBUG10( ("Ignoring unknown name %s\n",
				   afs_ace->name));
			  ...
			  struct afs_acl *afs_acl)
{
	...
	for (i = 0; i < dacl->num_aces; i++) {
		...
		if (!mappable_sid(&ace->trustee)) {
			DEBUG10( ("Ignoring unmappable SID %s\n",
		...
		if (sid_compare(&ace->trustee,
			...
			if (!lookup_sid(tmp_talloc_ctx(), &ace->trustee,
				...
				DEBUG1( ("AFSACL: Could not lookup SID %s on file %s\n",
...
static BOOL afs_get_afs_acl(char *filename, struct afs_acl *acl)
{
	...
	if (ret) {
		DEBUG1( ("got error from PIOCTL: %d\n", ret));
		return False;
	}
	...
	if (!parse_afs_acl(acl, space)) {
		DEBUG1( ("Could not parse AFS acl\n"));
		free_afs_acl(acl);
		return False;
	}
...
static void merge_unknown_aces(struct afs_acl *src, struct afs_acl *dst)
{
	...
	for (ace = src->acelist; ace != NULL; ace = ace->next)
	{
		...
		if (ace->type != SID_NAME_UNKNOWN) {
			DEBUG10( ("Not merging known ACE for %s\n",
				   ace->name));
			continue;
		}

		DEBUG10( ("Merging unknown ACE for %s\n", ace->name));

		copy = clone_afs_ace(dst->ctx, ace);

		if (copy == NULL) {
			DEBUG0( ("Could not clone ACE for %s\n", ace->name));
			continue;
		}
			   ...
			   struct security_descriptor *psd)
{
	...
	if (!afs_get_afs_acl(name, &old_afs_acl)) {
		DEBUG3( ("Could not get old ACL of %s\n", fsp->fsp_name));
		goto done;
	}
	...
	DEBUG10( ("trying to set acl '%s' on file %s\n", iob.in, name));

	ret = afs_syscall(AFSCALL_PIOCTL, name, VIOCSETAL, (char *)&iob, 0);

	if (ret != 0) {
		DEBUG10( ("VIOCSETAL returned %d\n", ret));
	}
