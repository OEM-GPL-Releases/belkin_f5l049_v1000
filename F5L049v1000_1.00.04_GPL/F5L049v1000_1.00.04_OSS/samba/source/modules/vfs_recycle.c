...
#define ALLOC_CHECK(ptr, label) do { if ((ptr) == NULL) { DEBUG0( ("recycle.bin: out of memory!\n")); errno = ENOMEM; goto label; } } while(0)
...
static int recycle_connect(vfs_handle_struct *handle, const char *service, const char *user)
{
	DEBUG10(("recycle_connect() connect to service[%s] as user[%s].\n",
		service,user));
...
static void recycle_disconnect(vfs_handle_struct *handle)
{
	DEBUG10(("recycle_disconnect() connect to service[%s].\n",
...
static BOOL recycle_keep_dir_tree(vfs_handle_struct *handle)
{
	...
	DEBUG10( ("recycle_bin: keeptree = %s\n", ret?"True":"False"));
...
static mode_t recycle_directory_mode(vfs_handle_struct *handle)
{
	...
	DEBUG10( ("recycle: directory_mode = %o\n", dirmode));
...
static SMB_OFF_T recycle_get_file_size(vfs_handle_struct *handle, const char *fname)
{
	...
	if (SMB_VFS_NEXT_STAT(handle, fname, &st) != 0) {
		DEBUG0(("recycle: stat for %s returned %s\n", fname, strerror(errno)));
		return (SMB_OFF_T)0;
	}
	...
	*new_dir = '\0';
	...
	for(token = strtok(tok_str, "/"); token; token = strtok(NULL, "/")) {
		...
		if (recycle_directory_exist(handle, new_dir)){
			DEBUG10( ("recycle: dir %s already exists\n", new_dir));}
		else {
			DEBUG5( ("recycle: creating new dir %s\n", new_dir));
			if (SMB_VFS_NEXT_MKDIR(handle, new_dir, mode) != 0) {
				DEBUG1(("recycle: mkdir failed for %s with error: %s\n", new_dir, strerror(errno)));
				ret = False;
				goto done;
			}
		}
...
static void recycle_do_touch(vfs_handle_struct *handle, const char *fname, BOOL touch_mtime)
{
	...
	if (SMB_VFS_NEXT_NTIMES(handle, fname, ts) == -1 ) {
		DEBUG0( ("recycle: touching %s failed, reason = %s\n", fname, strerror(errno)));
	}
}
...
static int recycle_unlink(vfs_handle_struct *handle, const char *file_name)
{
	...
	if(!repository || *(repository) == '\0') {
		DEBUG3( ("recycle: repository path not set, purging %s...\n", file_name));
	...
	if (strncmp(file_name, repository, strlen(repository)) == 0) {
		DEBUG3( ("recycle: File is within recycling bin, unlinking ...\n"));
		rc = SMB_VFS_NEXT_UNLINK(handle, file_name);
		goto done;
	}
	 ...
	 *
	if(fsize == 0) {
		DEBUG3( ("recycle: File %s is empty, purging...\n", file_name));
		rc = SMB_VFS_NEXT_UNLINK(handle,file_name);
		goto done;
	}
	 */
	...
	if(maxsize > 0 && file_size > maxsize) {
		DEBUG3( ("recycle: File %s exceeds maximum recycle size, purging... \n", file_name));
		rc = SMB_VFS_NEXT_UNLINK(handle, file_name);
		goto done;
	}
	minsize = recycle_minsize(handle);
	if(minsize > 0 && file_size < minsize) {
		DEBUG3( ("recycle: File %s lowers minimum recycle size, purging... \n", file_name));
		rc = SMB_VFS_NEXT_UNLINK(handle, file_name);
		goto done;
	}
	...
	DEBUG5( ("space_avail = %Lu, file_size = %Lu\n", space_avail, file_size));
	if(space_avail < file_size) {
		DEBUG3( ("recycle: Not enough diskspace, purging file %s\n", file_name));
		rc = SMB_VFS_NEXT_UNLINK(handle, file_name);
		goto done;
	}
	 */
	...
	if (exist) {
		DEBUG10( ("recycle: Directory already exists\n"));
	} else {
		DEBUG10( ("recycle: Creating directory %s\n", temp_name));
		if (recycle_create_dir(handle, temp_name) == False) {
			DEBUG3( ("recycle: Could not create directory, purging %s...\n", file_name));
			rc = SMB_VFS_NEXT_UNLINK(handle, file_name);
			goto done;
		}
	}
	...
	DEBUG10( ("recycle: recycled file name: %s\n", final_name));		/* new filename with path */
	...
	if (recycle_file_exist(handle, final_name)) {
		if (recycle_versions(handle) == False || matchparam(recycle_noversions(handle), base) == True) {
			DEBUG3( ("recycle: Removing old file %s from recycle bin\n", final_name));
			if (SMB_VFS_NEXT_UNLINK(handle, final_name) != 0) {
				DEBUG1( ("recycle: Error deleting old file: %s\n", strerror(errno)));
			}
		}
	}
	...
	DEBUG10( ("recycle: Moving %s to %s\n", file_name, final_name));
	rc = SMB_VFS_NEXT_RENAME(handle, file_name, final_name);
	if (rc != 0) {
		DEBUG3( ("recycle: Move error %d (%s), purging file %s (%s)\n", errno, strerror(errno), file_name, final_name));
		rc = SMB_VFS_NEXT_UNLINK(handle, file_name);
		goto done;
	}
...
NTSTATUS vfs_recycle_init(void)
{
	...
	if (vfs_recycle_debug_level == -1) {
		...
		DEBUG0( ("vfs_recycle: Couldn't register custom debugging class!\n"));
	} else {
		DEBUG10( ("vfs_recycle: Debug class number of 'recycle': %d\n", vfs_recycle_debug_level));
	}
