				    ...
				    SMB_ACL_TYPE_T type)
{
	...
	DEBUG10(("Entering AIX sys_acl_get_file\n"));
	DEBUG10(("path_p is %s\n",path_p));
	...
	if(file_acl == NULL) {
		errno=ENOMEM;
		DEBUG0(("Error in AIX sys_acl_get_file: %d\n",errno));
		return(NULL);
	}
	...
	if( (rc == -1) && (errno == ENOSPC)) {
		...
		if( rc == -1) {
			DEBUG0(("statacl returned %d with errno %d\n",rc,errno));
			SAFE_FREE(file_acl);
			return(NULL);
		}
	}

	DEBUG10(("Got facl and returned it\n"));
				  ...
				  int fd)
{
	...
	DEBUG10(("Entering AIX sys_acl_get_fd\n"));
	DEBUG10(("fd is %d\n",fd));
	file_acl = (struct acl *)SMB_MALLOC(BUFSIZ);

	if(file_acl == NULL) {
		errno=ENOMEM;
		DEBUG0(("Error in AIX sys_acl_get_fd is %d\n",errno));
		return(NULL);
	}
	...
	if( (rc == -1) && (errno == ENOSPC)) {
		...
		if( rc == -1) {
			DEBUG0(("fstatacl returned %d with errno %d\n",rc,errno));
			SAFE_FREE(file_acl);
			return(NULL);
		}
	}
			      ...
			      SMB_ACL_T theacl)
{
	...
	DEBUG10(("errno is %d\n",errno));
	DEBUG10(("return code is %d\n",rc));
	SAFE_FREE(file_acl);
	DEBUG10(("Exiting the aixacl_sys_acl_set_file\n"));
			    ...
			    int fd, SMB_ACL_T theacl)
{
	...
	DEBUG10(("return code is %d\n",rc));
	SAFE_FREE(file_acl);
	DEBUG10(("Exiting aixacl_sys_acl_set_fd\n"));
