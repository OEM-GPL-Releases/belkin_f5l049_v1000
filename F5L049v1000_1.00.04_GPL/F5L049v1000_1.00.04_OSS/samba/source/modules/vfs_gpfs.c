...
static struct gpfs_acl *gpfs_getacl_alloc(const char *fname, gpfs_aclType_t type)
{
	...
	if (ret != 0)
	{
		DEBUG8( ("smbd_gpfs_getacl failed with %s\n",strerror(errno)));
		return NULL;
	}
...
static BOOL gpfs_get_nfs4_acl(struct files_struct *fsp, SMB4ACL_T **ppacl, BOOL *pretryPosix)
{
	...
	*ppacl = smb_create_smb4acl();
	...
	DEBUG10( ("len: %d, level: %d, version: %d, nace: %d\n",
	...
	for (i=0; i<gacl->acl_nace; i++) {
		...
		if (gace->aceIFlags & ACE4_IFLAG_SPECIAL_ID) {
			...
			case ACE4_SPECIAL_EVERYONE:
				...
				break;
			default:
				DEBUG8( ("invalid special gpfs id %d "
					  "ignored\n", gace->aceWho));
...
static BOOL gpfsacl_process_smbacl(files_struct *fsp, SMB4ACL_T *smbacl)
{
	...
	for (smbace=smb_first_ace4(smbacl); smbace!=NULL; smbace = smb_next_ace4(smbace)) {
		...
		if (aceprop->flags&SMB_ACE4_ID_SPECIAL)
		{
			...
			case SMB_ACE4_WHO_GROUP:
				...
				break;
			default:
				DEBUG8( ("unsupported special_id %d\n", aceprop->who.special_id));
				continue; /* don't add it !!! */
			}
		} else {
	...
	if (ret != 0) {
		DEBUG8( ("gpfs_putacl failed with %s\n", strerror(errno)));
		gpfs_dumpacl(8, gacl);
		return False;
	}

	DEBUG10( ("gpfs_putacl succeeded\n"));
	return True;
}
...
static SMB_ACL_T gpfs2smb_acl(const struct gpfs_acl *pacl)
{
	...
	for (i=0; i<pacl->acl_nace; i++) {
		...
		DEBUG10( ("Converting type %d id %lu perm %x\n",
		...
		case GPFS_ACL_MASK:
			...
			break;
		default:
			DEBUG10( ("Got invalid ace_type: %d\n",
				   g_ace->ace_type));
			errno = EINVAL;
			SAFE_FREE(result);
			return NULL;
...
static SMB_ACL_T gpfsacl_get_posix_acl(const char *path, gpfs_aclType_t type)
{
	...
	if (pacl == NULL) {
		DEBUG10( ("gpfs_getacl failed for %s with %s\n",
			   path, strerror(errno)));
		if (errno == 0) {
			errno = EINVAL;
		}
	...
	if (pacl->acl_version != GPFS_ACL_VERSION_POSIX) {
		DEBUG10( ("Got acl version %d, expected %d\n",
				     ...
				     SMB_ACL_TYPE_T type)
{
	...
	DEBUG10( ("smb2gpfs_acl: Got ACL with %d entries\n", pacl->count));
	...
	for (i=0; i<pacl->count; i++) {
		...
		DEBUG10( ("Converting type %d perm %x\n",
