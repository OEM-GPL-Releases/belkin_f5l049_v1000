...
static void init_bitmap(struct bitmap **bm, const char **ops)
{
	...
	*bm = bitmap_allocate(SMB_VFS_OP_LAST);
	...
	while (*ops != NULL) {
		...
		if (!found) {
			DEBUG0( ("Could not find opname %s, logging all\n",
				  *ops));
			log_all = True;
			break;
		}
...
NTSTATUS vfs_full_audit_init(void)
{
	...
	if (vfs_full_audit_debug_level == -1) {
		...
		DEBUG0( ("vfs_full_audit: Couldn't register custom debugging "
			  "class!\n"));
	} else {
		DEBUG10( ("vfs_full_audit: Debug class number of "
