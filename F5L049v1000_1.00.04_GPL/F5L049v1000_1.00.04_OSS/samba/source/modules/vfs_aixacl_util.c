...
SMB_ACL_T aixacl_to_smbacl(struct acl *file_acl)
{
	...
	DEBUG10(("acl_entry is %d\n",acl_entry));
	DEBUG10(("acl_last(file_acl) id %d\n",acl_last(file_acl)));
	...
	if(file_acl->acl_mode & S_IXACL){
		...
		while(acl_entry < acl_last(file_acl)) {
			...
			DEBUG10(("case ACEID_USER ace->uid is %d\n",ace->uid));
			...
			DEBUG10(("case ACEID_GROUP ace->gid is %d\n",ace->gid));
	...
	for( i = 1; i < 4; i++) {
		DEBUG10(("i is %d\n",i));
			...
			if (result == NULL) {
				DEBUG0( ("SMB_REALLOC failed\n"));
				errno = ENOMEM;
				DEBUG0(("Error in AIX sys_acl_get_file is %d\n",errno));
				return NULL;
			}
		...
		result->count++;
		DEBUG10(("ace->a_perm = %d\n",ace->a_perm));
		DEBUG10(("ace->a_type = %d\n",ace->a_type));
	}
...
struct acl *aixacl_smb_to_aixacl(SMB_ACL_TYPE_T acltype, SMB_ACL_T theacl)
{
	...
	DEBUG10(("Entering aixacl_smb_to_aixacl\n"));
	...
	if(file_acl == NULL) {
		errno = ENOMEM;
		DEBUG0(("Error in aixacl_smb_to_aixacl is %d\n",errno));
		return NULL;
	}
	...
	for(i=0; i<theacl->count; i++ ) {
		...
		switch(id_type) {
			...
			default: /* abnormal case */
				DEBUG10(("The id_type is unknown !\n"));
				continue;
		}
