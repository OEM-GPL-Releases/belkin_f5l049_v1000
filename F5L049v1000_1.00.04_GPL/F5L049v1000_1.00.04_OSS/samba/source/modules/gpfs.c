			...
			uint32 share_access)
{
	...
	if (allow == GPFS_SHARE_NONE) {
		DEBUG10( ("special case am=no_access:%x\n",access_mask));
	}
	else {	
	...
	DEBUG10( ("am=%x, allow=%d, sa=%x, deny=%d\n",
		   access_mask, allow, share_access, deny));

	result = gpfs_set_share_fn(fsp->fh->fd, allow, deny);
	if (result != 0) {
		if (errno == ENOSYS) {
			DEBUG5( ("VFS module vfs_gpfs loaded, but no gpfs "
				  "support has been compiled into Samba. Allowing access\n"));
			return True;
		} else {
			DEBUG10( ("gpfs_set_share failed: %s\n",
				   strerror(errno)));
		}
	}
...
void init_gpfs(void)
{
	...
	if (libgpfs_handle == NULL) {
		DEBUG10( ("sys_dlopen for libgpfs_gpl failed: %s\n",
			   strerror(errno)));
		return;
	}

	DEBUG10( ("libgpfs_gpl.so loaded\n"));

	gpfs_set_share_fn = sys_dlsym(libgpfs_handle, "gpfs_set_share");
	if (gpfs_set_share_fn == NULL) {
		DEBUG3( ("libgpfs_gpl.so does not contain the symbol "
			  "'gpfs_set_share'\n"));
...
{
	DEBUG0( ("'VFS module smbgpfs loaded, without gpfs support compiled\n"));
...
BOOL set_gpfs_sharemode(files_struct *fsp, uint32 access_mask,
			uint32 share_access)
{
	DEBUG0( ("VFS module - smbgpfs.so loaded, without gpfs support compiled\n"));
