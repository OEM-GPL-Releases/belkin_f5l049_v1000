		 ...
		 DOM_SID *ret_sid, enum lsa_SidType *ret_type)
{
	...
	DEBUG10(("lookup_name: %s => %s (domain), %s (name)\n",
		full_name, domain, name));
	DEBUG10( ("lookup_name: flags = 0x0%x\n", flags));
	...
	{
		if (!secrets_fetch_domain_sid(name, &sid)) {
			DEBUG3( ("Could not fetch the domain SID\n"));
			TALLOC_FREE(tmp_ctx);
			return False;
		}
		/* Swap domain and name */
	...
	if (IS_DC && winbind_lookup_name("", name, &sid, &type)) {
		...
		if (!winbind_lookup_sid(tmp_ctx, &dom_sid, &domain, NULL,
			...
			DEBUG2( ("winbind could not find the domain's name "
		...
		enum lsa_SidType *ret_type)
{
	...
	if (ret) {
		...
		DEBUG10( ("failed to lookup sid %s\n",
...
static BOOL fetch_sid_from_uid_cache(DOM_SID *psid, uid_t uid)
{
	...
	for (pc = uid_sid_cache_head; pc; pc = pc->next) {
		if (pc->uid == uid) {
			*psid = pc->sid;
			DEBUG3(("fetch sid from uid cache %u -> %s\n",
...
static BOOL fetch_uid_from_cache( uid_t *puid, const DOM_SID *psid )
{
	...
	for (pc = uid_sid_cache_head; pc; pc = pc->next) {
		if (sid_compare(&pc->sid, psid) == 0) {
			*puid = pc->uid;
			DEBUG3(("fetch uid from cache %u -> %s\n",
...
static BOOL fetch_sid_from_gid_cache(DOM_SID *psid, gid_t gid)
{
	...
	for (pc = gid_sid_cache_head; pc; pc = pc->next) {
		if (pc->gid == gid) {
			*psid = pc->sid;
			DEBUG3(("fetch sid from gid cache %u -> %s\n",
...
static BOOL fetch_gid_from_cache(gid_t *pgid, const DOM_SID *psid)
{
	...
	for (pc = gid_sid_cache_head; pc; pc = pc->next) {
		if (sid_compare(&pc->sid, psid) == 0) {
			*pgid = pc->gid;
			DEBUG3(("fetch gid from cache %u -> %s\n",
...
void store_gid_sid_cache(const DOM_SID *psid, gid_t gid)
{
	...
	DEBUG3(("store_gid_sid_cache: gid %u in cache -> %s\n", (unsigned int)gid,
...
static BOOL legacy_sid_to_uid(const DOM_SID *psid, uid_t *puid)
{
	...
	if (sid_peek_check_rid(get_global_sam_sid(), psid, &rid)) {
		...
		if (ret) {
			if (type != SID_NAME_USER) {
				DEBUG5( ("sid %s is a %s, expected a user\n",
					  sid_string_static(psid),
	...
	DEBUG10(("LEGACY: mapping failed for sid %s\n", sid_string_static(psid)));
	return False;

done:
	DEBUG10(("LEGACY: sid %s -> uid %u\n", sid_string_static(psid),
		(unsigned int)*puid ));
	...
	if (sid_peek_check_rid(get_global_sam_sid(), psid, &rid)) {
		...
		if (ret) {
			if ((type != SID_NAME_DOM_GRP) &&
			    (type != SID_NAME_ALIAS)) {
				DEBUG5( ("LEGACY: sid %s is a %s, expected a group\n",
					  sid_string_static(psid),
					  sid_type_lookup(type)));
				return False;
			}
...
void uid_to_sid(DOM_SID *psid, uid_t uid)
{
	...
	if (!winbind_uid_to_sid(psid, uid)) {
		...
		DEBUG5( ("uid_to_sid: winbind failed to find a sid for uid %u\n",
			uid));
		return;
	}
...
void gid_to_sid(DOM_SID *psid, gid_t gid)
{
	...
	if (!winbind_gid_to_sid(psid, gid)) {
		...
		DEBUG5( ("gid_to_sid: winbind failed to find a sid for gid %u\n",
			gid));
		return;
	}
...
BOOL sid_to_uid(const DOM_SID *psid, uid_t *puid)
{
	...
	if (!winbind_sid_to_uid(puid, psid)) {
		...
		DEBUG5( ("winbind failed to find a uid for sid %s\n",
			  sid_string_static(psid)));
		return False;
	}
...
BOOL sid_to_gid(const DOM_SID *psid, gid_t *pgid)
{
	...
	if ( !winbind_sid_to_gid(pgid, psid) ) {
		...
		DEBUG10(("winbind failed to find a gid for sid %s\n",
					sid_string_static(psid)));
		return False;
	}
