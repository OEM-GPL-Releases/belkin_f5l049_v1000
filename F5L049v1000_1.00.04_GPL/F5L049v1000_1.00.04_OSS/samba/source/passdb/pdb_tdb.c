...
static BOOL init_sam_from_buffer_v0(struct samu *sampass, uint8 *buf, uint32 buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG0( ("init_sam_from_buffer_v0: NULL parameters found!\n"));
		return False;
	}
...
static BOOL init_sam_from_buffer_v1(struct samu *sampass, uint8 *buf, uint32 buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG0( ("init_sam_from_buffer_v1: NULL parameters found!\n"));
		return False;
	}
...
BOOL init_sam_from_buffer_v2(struct samu *sampass, uint8 *buf, uint32 buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG0( ("init_sam_from_buffer_v2: NULL parameters found!\n"));
		return False;
	}
...
static BOOL tdbsam_convert(int32 from) 
{
	...
	while (key.dptr) {
		...
		if (key.dptr) {
			...
			if (!data.dptr) {
				DEBUG0(("tdbsam_convert: database entry not found: %s.\n",key.dptr));
				return False;
			}
	
			/* unpack the buffer from the former format */
			if ( !(user = samu_new( NULL )) ) {
				DEBUG0(("tdbsam_convert: samu_new() failed!\n"));
				SAFE_FREE( data.dptr );
				return False;
			}
			DEBUG10(("tdbsam_convert: Try unpacking a record with (key:%s) (version:%d)\n", key.dptr, from));
			switch (from) {
				case 0:
			...
			if (!ret) {
				DEBUG0(("tdbsam_convert: Bad struct samu entry returned from TDB (key:%s) (version:%d)\n", key.dptr, from));
				SAFE_FREE(data.dptr);
				TALLOC_FREE(user );
				return False;
			}
			...
			DEBUG10(("tdbsam_convert: Try packing a record (key:%s) (version:%d)\n", key.dptr, from));
			data.dsize = init_buffer_from_sam (&buf, user, False);
			TALLOC_FREE(user );
			
			if ( data.dsize == -1 ) {
				DEBUG0(("tdbsam_convert: cannot pack the struct samu into the new format\n"));
				return False;
			}
			data.dptr = (char *)buf;
			...
			if (tdb_store(tdbsam, key, data, TDB_MODIFY) != TDB_SUCCESS) {
				DEBUG0(("tdbsam_convert: cannot store the struct samu (key:%s) in new format\n",key.dptr));
				SAFE_FREE(data.dptr);
				return False;
			}
...
static BOOL tdbsam_open( const char *name )
{
	...
	if ( tdbsam ) {
		ref_count++;
		DEBUG8(("tdbsam_open: Incrementing open reference count.  Ref count is now %d\n", 
			ref_count));
		return True;
	}
	...
	if (!(tdbsam = tdb_open_log(name, 0, TDB_DEFAULT, O_CREAT|O_RDWR, 0600))) {
		DEBUG0( ("tdbsam_open: Failed to open/create TDB passwd [%s]\n", name));
		return False;
	}
	...
	if (version > TDBSAM_VERSION) {
		...
		DEBUG0( ("tdbsam_open: unknown version => %d\n", version));
		tdb_close( tdbsam );
		ref_count = 0;
		return False;
	} 
	...
	if ( version < TDBSAM_VERSION ) {	
		DEBUG1( ("tdbsam_open: Converting version %d database to version %d.\n", 
			version, TDBSAM_VERSION));
		
		if ( !tdbsam_convert(version) ) {
			DEBUG0( ("tdbsam_open: Error when trying to convert tdbsam [%s]\n",name));
			tdb_close(tdbsam);
			ref_count = 0;
			return False;
		}
			
		DEBUG3( ("TDBSAM converted successfully.\n"));
	}
	
	DEBUG4(("tdbsam_open: successfully opened %s\n", name ));	
...
void tdbsam_close( void )
{
	...
	DEBUG8(("tdbsam_close: Reference count is now %d.\n", ref_count));
...
static int tdbsam_traverse_setpwent(TDB_CONTEXT *t, TDB_DATA key, TDB_DATA data, void *state)
{
	...
	if ( strncmp(key.dptr, prefix, prefixlen) == 0 ) {
		if ( !(ptr=SMB_MALLOC_P(struct pwent_list)) ) {
			DEBUG0(("tdbsam_traverse_setpwent: Failed to malloc new entry for list\n"));
		...
		if (!ptr->key.dptr) {
			DEBUG0(("tdbsam_traverse_setpwent: memdup failed\n"));
			/* just return 0 and let the traversal continue */
			SAFE_FREE(ptr);
			return 0;
		}
...
static NTSTATUS tdbsam_setsampwent(struct pdb_methods *my_methods, BOOL update, uint32 acb_mask)
{
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG0(("tdbsam_getsampwnam: failed to open %s!\n", tdbsam_filename));
...
static void tdbsam_endsampwent(struct pdb_methods *my_methods)
{
	...
	if ( pwent_initialized ) {
		DEBUG7( ("endtdbpwent: closed sam database.\n"));
		tdbsam_close();
	}
...
static NTSTATUS tdbsam_getsampwent(struct pdb_methods *my_methods, struct samu *user)
{
	...
	if ( !user ) {
		DEBUG0(("tdbsam_getsampwent: struct samu is NULL.\n"));
		return nt_status;
	}

	if ( !tdbsam_pwent_list ) {
		DEBUG4(("tdbsam_getsampwent: end of list\n"));
		return nt_status;
	}
	...
	if ( !data.dptr ) {
		DEBUG5(("pdb_getsampwent: database entry not found.  Was the user deleted?\n"));
		return nt_status;
	}
  
	if ( !init_sam_from_buffer(user, (unsigned char *)data.dptr, data.dsize) ) {
		DEBUG0(("pdb_getsampwent: Bad struct samu entry returned from TDB!\n"));
	}
...
static NTSTATUS tdbsam_getsampwnam (struct pdb_methods *my_methods, struct samu *user, const char *sname)
{
	...
	if ( !user ) {
		DEBUG0(("pdb_getsampwnam: struct samu is NULL.\n"));
	...
	data = tdb_fetch(tdbsam, key);
	if (!data.dptr) {
		DEBUG5(("pdb_getsampwnam (TDB): error fetching database.\n"));
		DEBUGADD(5, (" Error: %s\n", tdb_errorstr(tdbsam)));
		DEBUGADD(5, (" Key: %s\n", keystr));
		tdbsam_close();
...
static NTSTATUS tdbsam_getsampwrid (struct pdb_methods *my_methods, struct samu *user, uint32 rid)
{
	...
	if ( !user ) {
		DEBUG0(("pdb_getsampwrid: struct samu is NULL.\n"));
		return nt_status;
	}
	...
	data = tdb_fetch (tdbsam, key);
	if (!data.dptr) {
		DEBUG5(("pdb_getsampwrid (TDB): error looking up RID %d by key %s.\n", rid, keystr));
		DEBUGADD(5, (" Error: %s\n", tdb_errorstr(tdbsam)));
...
static BOOL tdb_delete_samacct_only( struct samu *sam_pass )
{
	...
	if (tdb_delete(tdbsam, key) != TDB_SUCCESS) {
		DEBUG5( ("Error deleting entry from tdb passwd database!\n"));
		DEBUGADD(5, (" Error: %s\n", tdb_errorstr(tdbsam)));
		return False;
	}
...
static NTSTATUS tdbsam_delete_sam_account(struct pdb_methods *my_methods, struct samu *sam_pass)
{
	...
	if ( !tdbsam_open( tdbsam_filename ) ) {
		DEBUG0(("tdbsam_delete_sam_account: failed to open %s!\n",
			 tdbsam_filename));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	if ( tdb_delete(tdbsam, key) != TDB_SUCCESS ) {
		DEBUG5( ("Error deleting entry from tdb rid database!\n"));
		DEBUGADD(5, (" Error: %s\n", tdb_errorstr(tdbsam)));
		nt_status = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
...
static BOOL tdb_update_samacct_only( struct samu* newpwd, int flag )
{
	...
	if ( (data.dsize=init_buffer_from_sam (&buf, newpwd, False)) == -1 ) {
		DEBUG0(("tdb_update_sam: ERROR - Unable to copy struct samu info BYTE buffer!\n"));
		ret = False;
		goto done;
	}
	data.dptr = (char *)buf;
	...
	DEBUG5( ("Storing %saccount %s with RID %d\n", 
	...
	if ( tdb_store(tdbsam, key, data, flag) != TDB_SUCCESS ) {
		DEBUG0( ("Unable to modify passwd TDB!"));
...
static BOOL tdb_update_ridrec_only( struct samu* newpwd, int flag )
{
	...
	/* add the reference */
	if (tdb_store(tdbsam, key, data, flag) != TDB_SUCCESS) {
		DEBUG0( ("Unable to modify TDB passwd !"));
		DEBUGADD(0, (" Error: %s\n", tdb_errorstr(tdbsam)));
...
static BOOL tdb_update_sam(struct pdb_methods *my_methods, struct samu* newpwd, int flag)
{
	...
	if (!pdb_get_user_rid(newpwd)) {
		DEBUG0(("tdb_update_sam: struct samu (%s) with no RID!\n", pdb_get_username(newpwd)));
		return False;
	}
...
static BOOL init_idmap_tdb(TDB_CONTEXT *tdb)
{
	...
	if (tdb_lock_bystring(tdb, "IDMAP_VERSION") != 0) {
		DEBUG0( ("Could not lock IDMAP_VERSION\n"));
		return False;
	}
	...
	if (version == -1) {
		...
		if (tdb_store_int32(tdb, "IDMAP_VERSION",
				    IDMAP_VERSION) != 0) {
			DEBUG0( ("Could not store IDMAP_VERSION\n"));
	...
	if (version != IDMAP_VERSION) {
		DEBUG0( ("Expected IDMAP_VERSION=%d, found %d. Please "
...
static BOOL tdbsam_new_rid(struct pdb_methods *methods, uint32 *prid)
{
	...
	if (tdb == NULL) {
		DEBUG1( ("Could not open idmap: %s\n", strerror(errno)));
		goto done;
	}

	if (!init_idmap_tdb(tdb)) {
		DEBUG1( ("Could not init idmap\n"));
		goto done;
	}
	...
	if (!tdb_change_uint32_atomic(tdb, "RID_COUNTER", &rid, 1)) {
		DEBUG3( ("tdbsam_new_rid: Failed to increase RID_COUNTER\n"));
		goto done;
	}
