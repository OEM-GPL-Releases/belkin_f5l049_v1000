...
struct samu *samu_new( TALLOC_CTX *ctx )
{
	...
	if ( !(user = TALLOC_ZERO_P( ctx, struct samu )) ) {
		DEBUG0(("samuser_new: Talloc failed!\n"));
		return NULL;
	}
...
static NTSTATUS samu_set_unix_internal(struct samu *user, const struct passwd *pwd, BOOL create)
{
	...
	if (pwd->pw_name[strlen(pwd->pw_name)-1] == '$') {
		...
		if (!pdb_set_acct_ctrl(user, ACB_WSTRUST, PDB_DEFAULT)) {
			DEBUG1( ("Failed to set 'workstation account' flags for user %s.\n", 
				pwd->pw_name));
		...
		if (!pdb_set_acct_ctrl(user, ACB_NORMAL, PDB_DEFAULT)) {
			DEBUG1( ("Failed to set 'normal account' flags for user %s.\n", 
				pwd->pw_name));
	...
	if ( create && !pdb_rid_algorithm() ) {
		...
		if ( !pdb_new_rid( &user_rid ) ) {
			DEBUG3( ("Could not allocate a new RID\n"));
		...
		if ( !pdb_set_user_sid(user, &user_sid, PDB_SET) ) {
			DEBUG3( ("pdb_set_user_sid failed\n"));
...
int algorithmic_rid_base(void)
{
	...
	if (rid_offset < BASE_RID) {  
		...
		DEBUG0( ("'algorithmic rid base' must be equal to or above %ld\n", BASE_RID));
		rid_offset = BASE_RID;
	}
	if (rid_offset & 1) {
		DEBUG0( ("algorithmic rid base must be even\n"));
		rid_offset += 1;
	}
	return rid_offset;
}
			    ...
			    enum lsa_SidType *type)
{
	...
	if ((flags & LOOKUP_NAME_GROUP) == 0) {
		...
		if (ret) {
			if (!sid_check_is_in_our_domain(&user_sid)) {
				DEBUG0( ("User %s with invalid SID %s in passdb\n",
	...
	if (!sid_check_is_in_our_domain(&map.sid)) {
		DEBUG10( ("Found group %s (%s) not in our domain -- "
			   "ignoring.", user,
...
BOOL init_sam_from_buffer_v3(struct samu *sampass, uint8 *buf, uint32 buflen)
{
	...
	if(sampass == NULL || buf == NULL) {
		DEBUG0( ("init_sam_from_buffer_v3: NULL parameters found!\n"));
		return False;
	}
...
/* TDB_FORMAT_STRING_V3       "dddddddBBBBBBBBBBBBddBBBdwdBwwd" */
	...
	if ( (*buf=(uint8*)SMB_MALLOC(len)) == NULL) {
		DEBUG0(("init_buffer_from_sam_v3: Unable to malloc() memory for buffer!\n"));
		return (-1);
	}
	...
	if (buflen != len) {
		DEBUG0( ("init_buffer_from_sam_v3: somthing odd is going on here: bufflen (%lu) != len (%lu) in tdb_pack operations!\n", 
...
BOOL pdb_update_bad_password_count(struct samu *sampass, BOOL *updated)
{
	...
	if (!BadPasswordCount) {
		DEBUG9( ("No bad password attempts.\n"));
		return True;
	}
	...
	if (!res) {
		DEBUG0( ("pdb_update_bad_password_count: pdb_get_account_policy failed.\n"));
		return False;
	}

	/* First, check if there is a reset time to compare */
	if ((resettime == (uint32) -1) || (resettime == 0)) {
		DEBUG9( ("No reset time, can't reset bad pw count\n"));
		return True;
	}

	LastBadPassword = pdb_get_bad_password_time(sampass);
	DEBUG7( ("LastBadPassword=%d, resettime=%d, current time=%d.\n", 
...
BOOL pdb_update_autolock_flag(struct samu *sampass, BOOL *updated)
{
	...
	if (!(pdb_get_acct_ctrl(sampass) & ACB_AUTOLOCK)) {
		DEBUG9( ("pdb_update_autolock_flag: Account %s not autolocked, no check needed\n",
	...
	if (!res) {
		DEBUG0( ("pdb_update_autolock_flag: pdb_get_account_policy failed.\n"));
		return False;
	}

	/* First, check if there is a duration to compare */
	if ((duration == (uint32) -1)  || (duration == 0)) {
		DEBUG9( ("pdb_update_autolock_flag: No reset duration, can't reset autolock\n"));
		return True;
	}
		      
	LastBadPassword = pdb_get_bad_password_time(sampass);
	DEBUG7( ("pdb_update_autolock_flag: Account %s, LastBadPassword=%d, duration=%d, current time =%d.\n",
		  pdb_get_username(sampass), (uint32)LastBadPassword, duration*60, (uint32)time(NULL)));

	if (LastBadPassword == (time_t)0) {
		DEBUG1(("pdb_update_autolock_flag: Account %s administratively locked out with no \
...
BOOL pdb_increment_bad_password_count(struct samu *sampass)
{
	...
	unbecome_root();
	if ( !ret ) {
		DEBUG0( ("pdb_increment_bad_password_count: pdb_get_account_policy failed.\n"));
		return False;
	}

	/* If there is no policy, we don't need to continue checking */
	if (!account_policy_lockout) {
		DEBUG9( ("No lockout policy, don't track bad passwords\n"));
		return True;
	}
	...
	*/
	...
	if (!pdb_set_acct_ctrl(sampass,
			       pdb_get_acct_ctrl(sampass) | ACB_AUTOLOCK,
			       PDB_CHANGED)) {
		DEBUG1( ("pdb_increment_bad_password_count:failed to set 'autolock' flag. \n")); 
		return False;
	}
