...
static const char* get_userattr_key2string( int schema_ver, int key )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG0(("get_userattr_key2string: unknown schema version specified\n"));
			break;
	}
	return NULL;
}
...
const char** get_userattr_list( TALLOC_CTX *mem_ctx, int schema_ver )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG0(("get_userattr_list: unknown schema version specified!\n"));
			break;
	}
					      ...
					      int schema_ver )
{
	switch ( schema_ver ) {
		...
		default:
			DEBUG0(("get_userattr_delete_list: unknown schema version specified!\n"));
			break;
	}
...
static const char* get_objclass_filter( int schema_ver )
{
	...
	switch( schema_ver ) {
		...
		case SCHEMAVER_SAMBASAMACCOUNT:
			...
			break;
		default:
			DEBUG0(("get_objclass_filter: Invalid schema version specified!\n"));
			break;
	}
...
static NTSTATUS ldapsam_get_seq_num(struct pdb_methods *my_methods, time_t *seq_num)
{
	...
	if (!seq_num) {
		DEBUG3(("ldapsam_get_seq_num: no sequence_number\n"));
		return ntstatus;
	}

	if (!smbldap_has_naming_context(ldap_state->smbldap_state->ldap_struct, lp_ldap_suffix())) {
		DEBUG3(("ldapsam_get_seq_num: DIT not configured to hold %s "
	...
	if (num_result != 1) {
		DEBUG3(("ldapsam_get_seq_num: Expected one entry, got %d\n", num_result));
		goto done;
	}

	entry = ldap_first_entry(ldap_state->smbldap_state->ldap_struct, msg);
	if (entry == NULL) {
		DEBUG3(("ldapsam_get_seq_num: Could not retrieve entry\n"));
		goto done;
	}

	values = ldap_get_values(ldap_state->smbldap_state->ldap_struct, entry, attrs[0]);
	if (values == NULL) {
		DEBUG3(("ldapsam_get_seq_num: no values\n"));
		goto done;
	}

	num_values = ldap_count_values(values);
	if (num_values == 0) {
		DEBUG3(("ldapsam_get_seq_num: not a single value\n"));
		goto done;
	}

	p = values[0];
	if (!next_token(&p, tok, "#", sizeof(tok))) {
		DEBUG0(("ldapsam_get_seq_num: failed to parse sequence number\n"));
		goto done;
	}
	...
	DEBUG10(("ldapsam_get_seq_num: got %s: %s\n", attrs[0], p));

	*seq_num = generalized_to_unix_time(p);

	/* very basic sanity check */
	if (*seq_num <= 0) {
		DEBUG3(("ldapsam_get_seq_num: invalid sequence number: %d\n", 
			(int)*seq_num));
		goto done;
	}
	...
	for (name = ldap_first_attribute(priv2ld(priv), entry, &ptr);
		...
		for (attrib = attrs; *attrib != NULL; attrib++) {
			if (strequal(*attrib, name)) {
				DEBUG10( ("ldapsam_delete_entry: deleting "
					   "attribute %s\n", name));
...
static time_t ldapsam_get_entry_timestamp( struct ldapsam_privates *ldap_state, LDAPMessage * entry)
{
	...
	if ( !strptime(temp, "%Y%m%d%H%M%SZ", &tm)) {
		DEBUG2(("ldapsam_get_entry_timestamp: strptime failed on: %s\n",
			(char*)temp));
		return (time_t) 0;
	}
	tzset();
				...
				LDAPMessage * entry)
{
	...
	if (sampass == NULL || ldap_state == NULL || entry == NULL) {
		DEBUG0( ("init_sam_from_ldap: NULL parameters found!\n"));
		return False;
	}

	if (priv2ld(ldap_state) == NULL) {
		DEBUG0( ("init_sam_from_ldap: ldap_state->smbldap_state->"
			  "ldap_struct is NULL!\n"));
		return False;
	}
	...
	if (!smbldap_get_single_pstring(priv2ld(ldap_state), entry, "uid",
					username)) {
		DEBUG1( ("init_sam_from_ldap: No uid attribute found for "
			  "this user!\n"));
		return False;
	}

	DEBUG2( ("init_sam_from_ldap: Entry found for user: %s\n", username));
	...
	if (pdb_get_init_flags(sampass,PDB_USERSID) == PDB_DEFAULT) {
		DEBUG1( ("init_sam_from_ldap: no %s or %s attribute found for this user %s\n", 
			get_userattr_key2string(ldap_state->schema_ver, LDAP_ATTR_USER_SID),
	...
	if (ldap_state->is_nds_ldap) {
		...
		if (user_dn != NULL) {
			DEBUG3( ("init_sam_from_ldap: smbldap_get_dn(%s) returned '%s'\n", username, user_dn));
			...
			DEBUG0( ("init_sam_from_ldap: failed to get user_dn for '%s'\n", username));
		}
	}
	...
	if (pwHistLen > 0){
		...
		if ((pwhist = SMB_MALLOC_ARRAY(uint8, pwHistLen * PW_HISTORY_ENTRY_LEN)) == NULL){
			DEBUG0( ("init_sam_from_ldap: malloc failed!\n"));
			return False;
		}
		...
		if (!smbldap_get_single_attribute(ldap_state->smbldap_state->ldap_struct, entry,
			...
			if (hex_failed) {
				DEBUG0(("init_sam_from_ldap: Failed to get password history for user %s\n",
					username));
				memset(pwhist, '\0', pwHistLen * PW_HISTORY_ENTRY_LEN);
			}
		}
	...
	DEBUG7( ("ldap time is %u, cache time is %u, bad time = %u\n", 
						    ...
						    enum pdb_elements))
{
	...
	if (mods == NULL || sampass == NULL) {
		DEBUG0( ("init_ldap_from_sam: NULL parameters found!\n"));
		return False;
	}
	...
	*mods = NULL;
	...
	DEBUG2( ("init_ldap_from_sam: Setting entry for user: %s\n", pdb_get_username(sampass)));
		...
		switch ( ldap_state->schema_ver ) {
			case SCHEMAVER_SAMBAACCOUNT:
				if (!sid_peek_check_rid(&ldap_state->domain_sid, user_sid, &rid)) {
					DEBUG1( ("init_ldap_from_sam: User's SID (%s) is not for this domain (%s), cannot add to LDAP!\n", 
			...
			default:
				DEBUG0(("init_ldap_from_sam: unknown schema version specified\n"));
				break;
		}		
	}
		...
		switch ( ldap_state->schema_ver ) {
			case SCHEMAVER_SAMBAACCOUNT:
				if (!sid_peek_check_rid(&ldap_state->domain_sid, group_sid, &rid)) {
					DEBUG1( ("init_ldap_from_sam: User's Primary Group SID (%s) is not for this domain (%s), cannot add to LDAP!\n",
	...
	if (need_update(sampass, PDB_BAD_PASSWORD_COUNT))  /* &&
		...
		DEBUG3( ("updating bad password fields, policy=%u, count=%u, time=%u\n",
			(unsigned int)pol, (unsigned int)badcount, (unsigned int)badtime));

		if ((badcount >= pol) || (badcount == 0)) {
			DEBUG7( ("making mods to update ldap, count=%u, time=%u\n",
		...
		if (badcount == 0) {
			DEBUG7( ("bad password count is reset, deleting login cache entry for %s\n", pdb_get_nt_username(sampass)));
			login_cache_delentry(sampass);
		} else {
			...
			DEBUG7( ("Updating bad password count and time in login cache\n"));
...
static NTSTATUS ldapsam_setsampwent(struct pdb_methods *my_methods, BOOL update, uint32 acb_mask)
{
	...
	DEBUG10(("ldapsam_setsampwent: LDAP Query for acb_mask 0x%x will use suffix %s\n", 
		acb_mask, suffix));
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG0( ("ldapsam_setsampwent: LDAP search failed: %s\n", ldap_err2string(rc)));
		DEBUG3( ("ldapsam_setsampwent: Query was: %s, %s\n", suffix, filter));
		ldap_msgfree(ldap_state->result);
		ldap_state->result = NULL;
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG2( ("ldapsam_setsampwent: %d entries in the base %s\n",
...
static NTSTATUS ldapsam_getsampwnam(struct pdb_methods *my_methods, struct samu *user, const char *sname)
{
	...
	if (count < 1) {
		DEBUG4( ("ldapsam_getsampwnam: Unable to locate user [%s] count=%d\n", sname, count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	} else if (count > 1) {
		DEBUG1( ("ldapsam_getsampwnam: Duplicate entries for this user [%s] Failing. count=%d\n", sname, count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	}
	...
	if (entry) {
		if (!init_sam_from_ldap(ldap_state, user, entry)) {
			DEBUG1(("ldapsam_getsampwnam: init_sam_from_ldap failed for user '%s'!\n", sname));
			ldap_msgfree(result);
			return NT_STATUS_NO_SUCH_USER;
		}
...
static NTSTATUS ldapsam_getsampwsid(struct pdb_methods *my_methods, struct samu * user, const DOM_SID *sid)
{
	...
	if (count < 1) {
		DEBUG4( ("ldapsam_getsampwsid: Unable to locate SID [%s] count=%d\n", sid_to_string(sid_string, sid),
		       count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	}  else if (count > 1) {
		DEBUG1( ("ldapsam_getsampwsid: More than one user with SID [%s]. Failing. count=%d\n", sid_to_string(sid_string, sid),
		       count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	}
	...
	if (!init_sam_from_ldap(ldap_state, user, entry)) {
		DEBUG1(("ldapsam_getsampwsid: init_sam_from_ldap failed!\n"));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_USER;
	}
				     ...
				     BOOL (*need_update)(const struct samu *, enum pdb_elements))
{
	...
	if (!mods) {
		DEBUG5(("ldapsam_modify_entry: mods is empty: nothing to modify\n"));
		...
		switch(ldap_op) {
			...
			case LDAP_MOD_REPLACE: 
				...
				break;
			default: 	
				DEBUG0(("ldapsam_modify_entry: Wrong LDAP operation type: %d!\n", 
					 ldap_op));
				return NT_STATUS_INVALID_PARAMETER;
		}
	...
	if (!(pdb_get_acct_ctrl(newpwd)&(ACB_WSTRUST|ACB_SVRTRUST|ACB_DOMTRUST)) &&
		...
		if (!ldap_state->is_nds_ldap) {
			...
			if (!smbldap_has_extension(ldap_state->smbldap_state->ldap_struct, 
				...
				DEBUG2( ("ldap password change requested, but LDAP "
		...
		if ((ber = ber_alloc_t(LBER_USE_DER))==NULL) {
			DEBUG0(("ber_alloc_t returns NULL\n"));
			SAFE_FREE(utf8_password);
			SAFE_FREE(utf8_dn);
			return NT_STATUS_UNSUCCESSFUL;
		}
		...
		ber_printf (ber, "ts", LDAP_TAG_EXOP_MODIFY_PASSWD_ID, utf8_dn);
			...
			DEBUG0(("ldapsam_modify_entry: ber_flatten returns a value <0\n"));
			ber_free(ber,1);
			SAFE_FREE(utf8_dn);
			SAFE_FREE(utf8_password);
			return NT_STATUS_UNSUCCESSFUL;
		...
		if (rc != LDAP_SUCCESS) {
			...
			if (rc == LDAP_OBJECT_CLASS_VIOLATION) {
				DEBUG3( ("Could not set userPassword "
			...
			ldap_get_option(ldap_state->smbldap_state->ldap_struct, LDAP_OPT_ERROR_STRING,
					&ld_error);
			DEBUG0(("ldapsam_modify_entry: LDAP Password could not be changed for user %s: %s\n\t%s\n",
			...
			DEBUG3(("ldapsam_modify_entry: LDAP Password changed for user %s\n",pdb_get_username(newpwd)));
					   ...
					   struct samu * sam_acct)
{
	...
	if (!sam_acct) {
		DEBUG0( ("ldapsam_delete_sam_account: sam_acct was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	sname = pdb_get_username(sam_acct);

	DEBUG3( ("ldapsam_delete_sam_account: Deleting user %s from "
		  "LDAP.\n", sname));

	mem_ctx = talloc_new(NULL);
	if (mem_ctx == NULL) {
		DEBUG0( ("talloc_new failed\n"));
		goto done;
	}
...
static NTSTATUS ldapsam_update_sam_account(struct pdb_methods *my_methods, struct samu * newpwd)
{
	...
	if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) == 0) {
		DEBUG0( ("ldapsam_update_sam_account: No user to modify!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	DEBUG4( ("ldapsam_update_sam_account: user %s to be modified has dn: %s\n", pdb_get_username(newpwd), dn));

	if (!init_ldap_from_sam(ldap_state, entry, &mods, newpwd,
				element_is_changed)) {
		DEBUG0( ("ldapsam_update_sam_account: init_ldap_from_sam failed!\n"));
		SAFE_FREE(dn);
		if (mods != NULL)
	...
	if ((lp_ldap_passwd_sync() != LDAP_PASSWD_SYNC_ONLY)
	    && (mods == NULL)) {
		DEBUG4(("ldapsam_update_sam_account: mods is empty: nothing to update for user: %s\n",
			 pdb_get_username(newpwd)));
		SAFE_FREE(dn);
		return NT_STATUS_OK;
	}
	...
	DEBUG2( ("ldapsam_update_sam_account: successfully modified uid = %s in the LDAP database\n",
		  pdb_get_username(newpwd)));
	return NT_STATUS_OK;
}
					   ...
					   const char *newname)
{
	...
	if (!old_acct) {
		DEBUG0( ("ldapsam_rename_sam_account: old_acct was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	if (!newname) {
		DEBUG0( ("ldapsam_rename_sam_account: newname was NULL!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
static NTSTATUS ldapsam_add_sam_account(struct pdb_methods *my_methods, struct samu * newpwd)
{
	...
	if (!username || !*username) {
		DEBUG0( ("ldapsam_add_sam_account: Cannot add user without a username!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) != 0) {
		DEBUG0(("ldapsam_add_sam_account: User '%s' already in the base, with samba attributes\n", 
			 username));
		ldap_msgfree(result);
		TALLOC_FREE( attr_list );
		return NT_STATUS_UNSUCCESSFUL;
	...
	if (element_is_set_or_changed(newpwd, PDB_USERSID)) {
		...
		if (rc == LDAP_SUCCESS) {
			if (ldap_count_entries(ldap_state->smbldap_state->ldap_struct, result) != 0) {
				DEBUG0(("ldapsam_add_sam_account: SID '%s' already in the base, with samba attributes\n", 
					 sid_to_string(sid_string, sid)));
				TALLOC_FREE( attr_list );
				ldap_msgfree(result);
				return NT_STATUS_UNSUCCESSFUL;
	...
	if (num_result == 1) {
		...
		DEBUG3(("ldapsam_add_sam_account: User exists without samba attributes: adding them\n"));
	...
	if (num_result == 0) {
		...
		DEBUG3(("ldapsam_add_sam_account: Adding new user\n"));
		ldap_op = LDAP_MOD_ADD;

		escape_username = escape_rdn_val_string_alloc(username);
		if (!escape_username) {
			DEBUG0( ("Out of memory!\n"));
			ldap_msgfree(result);
			return NT_STATUS_NO_MEMORY;
		}
	...
	if (!init_ldap_from_sam(ldap_state, entry, &mods, newpwd,
		...
		DEBUG0( ("ldapsam_add_sam_account: init_ldap_from_sam failed!\n"));
		ldap_msgfree(result);
		if (mods != NULL)
			ldap_mods_free(mods,True);
		return NT_STATUS_UNSUCCESSFUL;		
	...
	if (mods == NULL) {
		DEBUG0(("ldapsam_add_sam_account: mods is empty: nothing to add for user: %s\n",pdb_get_username(newpwd)));
		return NT_STATUS_UNSUCCESSFUL;
	}
	switch ( ldap_state->schema_ver ) {
		case SCHEMAVER_SAMBAACCOUNT:
		...
		case SCHEMAVER_SAMBASAMACCOUNT:
			...
			break;
		default:
			DEBUG0(("ldapsam_add_sam_account: invalid schema version specified\n"));
			break;
	}

	ret = ldapsam_modify_entry(my_methods,newpwd,dn,mods,ldap_op, element_is_set_or_changed);
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG0(("ldapsam_add_sam_account: failed to modify/add user with uid = %s (dn = %s)\n",
			 pdb_get_username(newpwd),dn));
		ldap_mods_free(mods, True);
		return ret;
	}

	DEBUG2(("ldapsam_add_sam_account: added: uid == %s in the LDAP database\n", pdb_get_username(newpwd)));
	ldap_mods_free(mods, True);
				 ...
				 GROUP_MAP *map, LDAPMessage *entry)
{
	...
	if (ldap_state == NULL || map == NULL || entry == NULL ||
		...
		DEBUG0( ("init_group_from_ldap: NULL parameters found!\n"));
		return False;
	}

	if (!smbldap_get_single_pstring(ldap_state->smbldap_state->ldap_struct, entry, 
			get_attr_key2string(groupmap_attr_list, LDAP_ATTR_GIDNUMBER), temp)) {
		DEBUG0( ("init_group_from_ldap: Mandatory attribute %s not found\n", 
			get_attr_key2string( groupmap_attr_list, LDAP_ATTR_GIDNUMBER)));
		return False;
	}
	DEBUG2( ("init_group_from_ldap: Entry found for group: %s\n", temp));
	...
	if (!string_to_sid(&map->sid, temp)) {
		DEBUG1( ("SID string [%s] could not be read as a valid SID\n", temp));
		return False;
	}
	...
	if ((map->sid_name_use < SID_NAME_USER) ||
		...
		DEBUG0( ("init_group_from_ldap: Unknown Group type: %d\n", map->sid_name_use));
		return False;
	}
	...
	if (!smbldap_get_single_pstring(ldap_state->smbldap_state->ldap_struct, entry, 
		...
		{
			DEBUG0( ("init_group_from_ldap: Attributes cn not found either \
				 ...
				 GROUP_MAP *map)
{
	...
	if (count < 1) {
		DEBUG4( ("ldapsam_getgroup: Did not find group\n"));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_GROUP;
	}

	if (count > 1) {
		DEBUG1( ("ldapsam_getgroup: Duplicate entries for filter %s: "
			  "count=%d\n", filter, count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_GROUP;
	}
	...
	if (!init_group_from_ldap(ldap_state, map, entry)) {
		DEBUG1( ("ldapsam_getgroup: init_group_from_ldap failed for "
			  "group filter %s\n", filter));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_GROUP;
	}
					   ...
					   uint32 *rid)
{
	...
	if (!smbldap_get_single_attribute(ldap_struct, entry, "sambaSID",
					  str, sizeof(str)-1)) {
		DEBUG10( ("Could not find sambaSID attribute\n"));
		return False;
	}

	if (!string_to_sid(&sid, str)) {
		DEBUG10( ("Could not convert string %s to sid\n", str));
		return False;
	}

	if (sid_compare_domain(&sid, domain_sid) != 0) {
		DEBUG10( ("SID %s is not in expected domain %s\n",
			   str, sid_string_static(domain_sid)));
		return False;
	}

	if (!sid_peek_rid(&sid, rid)) {
		DEBUG10( ("Could not peek into RID\n"));
		return False;
	}
					   ...
					   size_t *p_num_members)
{
	...
	*p_num_members = 0;
	...
	if (count > 1) {
		DEBUG1( ("Found more than one groupmap entry for %s\n",
			  sid_string_static(group)));
	...
	if (values) {
		...
		count = ldap_count_entries(conn->ldap_struct, result);
		DEBUG10(("ldapsam_enum_group_members: found %d accounts\n", count));
		     ...
		     entry = ldap_next_entry(conn->ldap_struct, entry))
		{
			...
			sidstr = smbldap_talloc_single_attribute(conn->ldap_struct,
								 ...
								 mem_ctx);
			if (!sidstr) {
				DEBUG0( ("Severe DB error, sambaSamAccount can't miss "
			...
			if (!sid_check_is_in_our_domain(&sid)) {
				DEBUG0( ("Inconsistent SAM -- group member uid not "
					  "in our domain\n"));
				ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
				goto done;
			}
					       ...
					       size_t *p_num_groups)
{
	...
	*pp_sids = NULL;
	...
	switch (count) {
	case 0:	
		DEBUG1( ("User account [%s] not found!\n", pdb_get_username(user)));
	...
	case 1:
		...
		break;
	default:
		DEBUG1( ("found more than one account with the same user name ?!\n"));
		ret = NT_STATUS_INTERNAL_DB_CORRUPTION;
		goto done;
	}
	...
	*pp_sids = NULL;
	...
	if (sid_compare(&global_sid_NULL, &(*pp_sids)[0]) == 0) {
		DEBUG3( ("primary group of [%s] not found\n",
			  pdb_get_username(user)));
		goto done;
	}
						...
						GROUP_MAP *map)
{
	...
	if ((rc == LDAP_SUCCESS) &&
		...
		DEBUG3( ("SID %s already present in LDAP, refusing to add "
			  "group mapping entry\n",
			  sid_string_static(&map->sid)));
	...
	case SID_NAME_ALIAS:
		...
		{
			DEBUG3( ("Refusing to map sid %s as an alias, not in our domain\n",
				  sid_string_static(&map->sid)));
	...
	default:
		DEBUG3( ("Got invalid use '%s' for mapping\n",
	...
	if (map->gid == -1) {
		DEBUG10( ("Refusing to map gid==-1\n"));
		result = NT_STATUS_INVALID_PARAMETER;
		goto done;
	}

	if (pdb_gid_to_sid(map->gid, &sid)) {
		DEBUG3( ("Gid %d is already mapped to SID %s, refusing to "
						   ...
						   GROUP_MAP *map)
{
	...
	if (mods == NULL) {
		DEBUG4( ("ldapsam_update_group_mapping_entry: mods is empty: "
			  "nothing to do\n"));
		result = NT_STATUS_OK;
		goto done;
	}
	...
	DEBUG2( ("ldapsam_update_group_mapping_entry: successfully modified "
		  "group %lu in LDAP\n", (unsigned long)map->gid));
				    ...
				    BOOL update)
{
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG0( ("ldapsam_setsamgrent: LDAP search failed: %s\n",
			  ldap_err2string(rc)));
		DEBUG3( ("ldapsam_setsamgrent: Query was: %s, %s\n",
	...
	DEBUG2( ("ldapsam_setsamgrent: %d entries in the base!\n",
		  ldap_count_entries(ldap_state->smbldap_state->ldap_struct,
				     ldap_state->result)));
					   ...
					   BOOL unix_only)
{
	...
	*pp_rmap = NULL;
	...
	if (!NT_STATUS_IS_OK(ldapsam_setsamgrent(methods, False))) {
		DEBUG0( ("ldapsam_enum_group_mapping: Unable to open "
			  "passdb\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	while (NT_STATUS_IS_OK(ldapsam_getsamgrent(methods, &map))) {
		...
		if (!(*pp_rmap)) {
			DEBUG0(("ldapsam_enum_group_mapping: Unable to "
				 "enlarge group map!\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
					...
					int modop)
{
	...
	if (type == SID_NAME_USE_NONE) {
		DEBUG5( ("SID %s is neither in builtin nor in our domain!\n",
	...
	if (count < 1) {
		DEBUG4( ("ldapsam_modify_aliasmem: Did not find alias\n"));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_ALIAS;
	}

	if (count > 1) {
		DEBUG1( ("ldapsam_modify_aliasmem: Duplicate entries for "
			  "filter %s: count=%d\n", filter, count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_ALIAS;
	}
				      ...
				      size_t *p_num_members)
{
	...
	*p_num_members = 0;
	...
	if (count < 1) {
		DEBUG4( ("ldapsam_enum_aliasmem: Did not find alias\n"));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_ALIAS;
	}

	if (count > 1) {
		DEBUG1( ("ldapsam_enum_aliasmem: Duplicate entries for "
			  "filter %s: count=%d\n", filter, count));
		ldap_msgfree(result);
		return NT_STATUS_NO_SUCH_ALIAS;
	}
					  ...
					  size_t *p_num_alias_rids)
{
	...
	if (type == SID_NAME_USE_NONE) {
		DEBUG5( ("SID %s is neither builtin nor domain!\n",
						   ...
						   uint32 value)
{
	...
	DEBUG10(("ldapsam_set_account_policy_in_ldap\n"));
	...
	if (policy_attr == NULL) {
		DEBUG0(("ldapsam_set_account_policy_in_ldap: invalid "
			 "policy\n"));
		return ntstatus;
	}
	...
	if (!cache_account_policy_set(policy_index, value)) {
		DEBUG0(("ldapsam_set_account_policy_in_ldap: failed to "
						     ...
						     uint32 *value)
{
	...
	DEBUG10(("ldapsam_get_account_policy_from_ldap\n"));
	...
	policy_attr = get_account_policy_attr(policy_index);
	if (!policy_attr) {
		DEBUG0(("ldapsam_get_account_policy_from_ldap: invalid "
			 "policy index: %d\n", policy_index));
		return ntstatus;
	}
					   ...
					   int policy_index, uint32 *value)
{
	...
	DEBUG10(("ldapsam_get_account_policy: failed to retrieve from "
		  "ldap\n"));
	...
	DEBUG10(("ldapsam_get_account_policy: no tdb for %d, trying "
		  "default\n", policy_index));
#endif
...
/* update_ldap: */
 ...
 update_cache:
	...
	if (!cache_account_policy_set(policy_index, *value)) {
		DEBUG0(("ldapsam_get_account_policy: failed to update local "
			 "tdb as a cache\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
				    ...
				    enum lsa_SidType *attrs)
{
	...
	for (entry = ldap_first_entry(ld, msg);
		...
		if (!ldapsam_extract_rid_from_entry(ld, entry, domain_sid,
						    &rid)) {
			DEBUG2( ("Could not find sid from ldap entry\n"));
			continue;
		}
		...
		name = smbldap_talloc_single_attribute(ld, entry, "uid",
						       names);
		if (name == NULL) {
			DEBUG2( ("Could not retrieve uid attribute\n"));
			continue;
		}
		...
		if (rid_index == num_rids) {
			DEBUG2( ("Got a RID not asked for: %d\n", rid));
			continue;
		}
	     ...
	     entry = ldap_next_entry(ld, entry))
	{
		...
		attr = smbldap_talloc_single_attribute(ld, entry, "sambaGroupType",
						       mem_ctx);
		if (attr == NULL) {
			DEBUG2( ("Could not extract type from ldap entry %s\n",
				  dn));
			continue;
		}
		...
		if ((is_builtin && (type != SID_NAME_ALIAS)) ||
			...
			DEBUG2( ("Rejecting invalid group mapping entry %s\n", dn));
		}

		if (!ldapsam_extract_rid_from_entry(ld, entry, domain_sid,
						    &rid)) {
			DEBUG2( ("Could not find sid from ldap entry %s\n", dn));
			continue;
		}
		...
		if (attr == NULL) {
			DEBUG10( ("Could not retrieve 'displayName' attribute from %s\n",
				   dn));
			attr = smbldap_talloc_single_attribute(ld, entry, "cn", names);
		}

		if (attr == NULL) {
			DEBUG2( ("Could not retrieve naming attribute from %s\n",
				  dn));
			continue;
		}
...
static BOOL ldapsam_search_firstpage(struct pdb_search *search)
{
	...
	if ((rc != LDAP_SUCCESS) || (state->entries == NULL)) {
        ...
        if ( ld == NULL) {
                DEBUG5( ("Don't have an LDAP connection right after a "
			  "search\n"));
                return False;
        }
...
static void ldapsam_search_end(struct pdb_search *search)
{
	...
	if (rc != LDAP_SUCCESS)
		DEBUG5( ("Could not end search properly\n"));
				  ...
				  struct samr_displayentry *result)
{
	...
	if ((vals == NULL) || (vals[0] == NULL)) {
		DEBUG5( ("\"sambaAcctFlags\" not found\n"));
		return False;
	}
	...
	if ((vals == NULL) || (vals[0] == NULL)) {
		DEBUG0( ("\"objectSid\" not found\n"));
		return False;
	}

	if (!string_to_sid(&sid, vals[0])) {
		DEBUG0( ("Could not convert %s to SID\n", vals[0]));
		ldap_value_free(vals);
		return False;
	}
	ldap_value_free(vals);
	...
	if (!sid_peek_check_rid(get_global_sam_sid(), &sid, &result->rid)) {
		DEBUG0( ("sid %s does not belong to our domain\n",
			  sid_string_static(&sid)));
		return False;
	}
				   ...
				   struct samr_displayentry *result)
{
	...
	if ((vals == NULL) || (vals[0] == NULL)) {
		DEBUG5( ("\"sambaGroupType\" not found\n"));
		if (vals != NULL) {
			ldap_value_free(vals);
		}
		return False;
				    ...
				    uint32 *rid)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG3( ("Could not get domain info: %s\n",
			  nt_errstr(status)));
		goto done;
	}
	...
	if (entry == NULL) {
		DEBUG0( ("Could not get domain info entry\n"));
			      ...
			      union unid_t *id, enum lsa_SidType *type)
{
	...
	if (ldap_count_entries(priv2ld(priv), result) != 1) {
		DEBUG10( ("Got %d entries, expected one\n",
	...
	if (value != NULL) {
		...
		if (gid_str == NULL) {
			DEBUG1( ("%s has sambaGroupType but no gidNumber\n",
	...
	value = smbldap_talloc_single_attribute(priv2ld(priv), entry,
						"uidNumber", mem_ctx);
	if (value == NULL) {
		DEBUG1( ("Could not find uidNumber in %s\n",
				    ...
				    uint32 acb_info, uint32 *rid)
{
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG0(("ldapsam_create_user: ldap search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (num_result == 1) {
		...
		if (!dn) {
			DEBUG0(("ldapsam_create_user: Out of memory!\n"));
			return NT_STATUS_NO_MEMORY;
		}
	}
	...
	if (!NT_STATUS_IS_OK((ret = ldapsam_new_rid_internal(my_methods, rid)))) {
		DEBUG1( ("ldapsam_create_user: Could not allocate a new RID\n"));
		return ret;
	}
	...
	if (!user) {
		DEBUG1(("ldapsam_create_user: Unable to allocate user struct\n"));
		return NT_STATUS_NO_MEMORY;
	}

	if (!pdb_set_username(user, name, PDB_SET)) {
		DEBUG1(("ldapsam_create_user: Unable to fill user structs\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (ldap_state->schema_ver != SCHEMAVER_SAMBASAMACCOUNT) {
		DEBUG1(("ldapsam_create_user: Unsupported schema version\n"));
	}
	smbldap_set_mod(&mods, LDAP_MOD_ADD, "objectClass", LDAP_OBJ_SAMBASAMACCOUNT);
	...
	if (add_posix) {
		...
		DEBUG3(("ldapsam_create_user: Creating new posix user\n"));
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG0(("ldapsam_create_user: failed to create a new user [%s] (dn = %s)\n", name ,dn));
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG2(("ldapsam_create_user: added account [%s] in the LDAP database\n", name));
...
static NTSTATUS ldapsam_delete_user(struct pdb_methods *my_methods, TALLOC_CTX *tmp_ctx, struct samu *sam_acct)
{
	...
	DEBUG0(("ldapsam_delete_user: Attempt to delete user [%s]\n", pdb_get_username(sam_acct)));
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG0(("ldapsam_delete_user: user search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG0(("ldapsam_delete_user: user not found!\n"));
		return NT_STATUS_NO_SUCH_USER;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG0(("ldapsam_delete_user: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
					 ...
					 uint32 *rid)
{
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG0(("ldapsam_create_group: ldap search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 1) {
		...
		dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
		if (!dn) {
			DEBUG0(("ldapsam_create_group: Out of memory!\n"));
			return NT_STATUS_NO_MEMORY;
		}
	}
	...
	if (num_result == 0) {
		...
		DEBUG3(("ldapsam_create_user: Creating new posix group\n"));
	...
	if (!NT_STATUS_IS_OK((ret = ldapsam_new_rid_internal(my_methods, rid)))) {
		DEBUG1( ("ldapsam_create_group: Could not allocate a new RID\n"));
		return ret;
	}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG0(("ldapsam_create_group: failed to create a new group [%s] (dn = %s)\n", name ,dn));
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG2(("ldapsam_create_group: added group [%s] in the LDAP database\n", name));
...
static NTSTATUS ldapsam_delete_dom_group(struct pdb_methods *my_methods, TALLOC_CTX *tmp_ctx, uint32 rid)
{
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG1(("ldapsam_delete_dom_group: group search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG1(("ldapsam_delete_dom_group: group not found!\n"));
		return NT_STATUS_NO_SUCH_GROUP;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG0(("ldapsam_delete_dom_group: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG1(("ldapsam_delete_dom_group: accounts search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result != 0) {
		DEBUG3(("ldapsam_delete_dom_group: Can't delete group, it is a primary group for %d users\n", num_result));
					...
					int modop)
{
	...
	switch (modop) {
	case LDAP_MOD_ADD:
		DEBUG1(("ldapsam_change_groupmem: add new member(rid=%d) to a domain group(rid=%d)", member_rid, group_rid));
		break;
	case LDAP_MOD_DELETE:
		DEBUG1(("ldapsam_change_groupmem: delete member(rid=%d) from a domain group(rid=%d)", member_rid, group_rid));
		break;
	default:
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG1(("ldapsam_change_groupmem: member search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG1(("ldapsam_change_groupmem: member not found!\n"));
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG1(("ldapsam_change_groupmem: group search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(tmp_ctx, result);
	...
	if (num_result == 0) {
		DEBUG1(("ldapsam_change_groupmem: group not found!\n"));
		return NT_STATUS_NO_SUCH_GROUP;
	}
	...
	dn = smbldap_talloc_dn(tmp_ctx, priv2ld(ldap_state), entry);
	if (!dn) {
		DEBUG0(("ldapsam_change_groupmem: Out of memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if (rc != LDAP_SUCCESS) {
		if (rc == LDAP_TYPE_OR_VALUE_EXISTS && modop == LDAP_MOD_ADD) {
			DEBUG1(("ldapsam_change_groupmem: member is already in group, add failed!\n"));
			return NT_STATUS_MEMBER_IN_GROUP;
		}
		if (rc == LDAP_NO_SUCH_ATTRIBUTE && modop == LDAP_MOD_DELETE) {
			DEBUG1(("ldapsam_change_groupmem: member is not in group, delete failed!\n"));
					  ...
					  struct samu *sampass)
{
	...
	DEBUG0(("ldapsam_set_primary_group: Attempt to set primary group for user [%s]\n", pdb_get_username(sampass)));

	if (!sid_to_gid(pdb_get_group_sid(sampass), &gid)) {
		DEBUG0(("ldapsam_set_primary_group: failed to retieve gid from user's group SID!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	gidstr = talloc_asprintf(mem_ctx, "%d", gid);
	if (!gidstr) {
		DEBUG0(("ldapsam_set_primary_group: Out of Memory!\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	rc = smbldap_search_suffix(ldap_state->smbldap_state, filter, NULL, &result);
	if (rc != LDAP_SUCCESS) {
		DEBUG0(("ldapsam_set_primary_group: user search failed!\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	talloc_autofree_ldapmsg(mem_ctx, result);
	...
	if (num_result == 0) {
		DEBUG0(("ldapsam_set_primary_group: user not found!\n"));
		return NT_STATUS_NO_SUCH_USER;
	}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG0(("ldapsam_set_primary_group: failed to modify [%s] primary group to [%s]\n",
...
static NTSTATUS pdb_init_ldapsam_common(struct pdb_methods **pdb_method, const char *location)
{
	...
	if ( !(ldap_state = TALLOC_ZERO_P(*pdb_method, struct ldapsam_privates)) ) {
		DEBUG0( ("pdb_init_ldapsam_common: talloc() failed for ldapsam private_data!\n"));
		return NT_STATUS_NO_MEMORY;
	}
...
NTSTATUS pdb_init_ldapsam(struct pdb_methods **pdb_method, const char *location)
{
	...
	if ( !NT_STATUS_IS_OK(nt_status) ) {
		DEBUG2( ("pdb_init_ldapsam: WARNING: Could not get domain "
	...
	entry = ldap_first_entry(ldap_state->smbldap_state->ldap_struct,
				 result);
	if (!entry) {
		DEBUG0( ("pdb_init_ldapsam: Could not get domain info "
			  "entry\n"));
		ldap_msgfree(result);
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if (smbldap_get_single_pstring(
		...
		if (!string_to_sid(&ldap_domain_sid, domain_sid_string)) {
			DEBUG1( ("pdb_init_ldapsam: SID [%s] could not be "
				  "read as a valid SID\n", domain_sid_string));
			ldap_msgfree(result);
			return NT_STATUS_INVALID_PARAMETER;
		}
		...
		if (!found_sid || !sid_equal(&secrets_domain_sid,
			...
			DEBUG1( ("pdb_init_ldapsam: Resetting SID for domain "
	...
	if (smbldap_get_single_pstring(
		...
		if (alg_rid_base != algorithmic_rid_base()) {
			DEBUG0( ("The value of 'algorithmic RID base' has "
