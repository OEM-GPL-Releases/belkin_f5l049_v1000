...
NTSTATUS smb_register_passdb(int version, const char *name, pdb_init_function init) 
{
	...
	if(version != PASSDB_INTERFACE_VERSION) {
		DEBUG0(("Can't register passdb backend!\n"
	...
	DEBUG5(("Attempting to register passdb backend %s\n", name));

	/* Check for duplicates */
	if (pdb_find_backend_entry(name)) {
		DEBUG0(("There already is a passdb backend registered with the name %s!\n", name));
	...
	DEBUG5(("Successfully added passdb backend '%s'\n", name));
	return NT_STATUS_OK;
}
...
NTSTATUS make_pdb_method_name(struct pdb_methods **methods, const char *selected)
{
	...
	DEBUG5(("Attempting to find an passdb backend to match %s (%s)\n", selected, module_name));

	entry = pdb_find_backend_entry(module_name);
	
	/* Try to find a module that contains this module */
	if (!entry) { 
		DEBUG2(("No builtin backend found, trying to load plugin\n"));
		if(NT_STATUS_IS_OK(smb_probe_module("pdb", module_name)) && !(entry = pdb_find_backend_entry(module_name))) {
			DEBUG0(("Plugin is available, but doesn't register passdb backend %s\n", module_name));
	...
	if(!entry) { 
		DEBUG0(("No builtin nor plugin backend for %s found\n", module_name));
		SAFE_FREE(module_name);
		return NT_STATUS_INVALID_PARAMETER;
	}

	DEBUG5(("Found pdb backend %s\n", module_name));

	if ( !NT_STATUS_IS_OK( nt_status = entry->init(methods, module_location) ) ) {
		DEBUG0(("pdb backend %s did not correctly init (error was %s)\n", 
	...
	DEBUG5(("pdb backend %s has a valid init\n", selected));
...
BOOL guest_user_info( struct samu *user )
{
	...
	if ( !(pwd = getpwnam_alloc( NULL, guestname ) ) ) {
		DEBUG0(("guest_user_info: Unable to locate guest account [%s]!\n", 
			guestname));
		return False;
	}
...
BOOL pdb_getsampwsid(struct samu *sam_acct, const DOM_SID *sid) 
{
	...
	if ( rid == DOMAIN_USER_RID_GUEST ) {
		DEBUG6(("pdb_getsampwsid: Building guest account\n"));
					...
					uint32 acb_info, uint32 *rid)
{
	...
	if ( !(pwd = Get_Pwnam_alloc(tmp_ctx, name)) ) {
		...
		if (add_script[0] == '\0') {
			DEBUG3( ("Could not find user %s and no add script "
				  "defined\n", name));
		...
		if (add_ret != 0) {
			DEBUG1( ("Creating a faked user %s for build farm "
				  "purposes\n", name));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG3( ("pdb_default_create_user: failed to create a new user structure: %s\n", nt_errstr(status)));
		return status;
	}

	if (!sid_peek_check_rid(get_global_sam_sid(),
				pdb_get_user_sid(sam_pass), rid)) {
		DEBUG0( ("Could not get RID of fresh user\n"));
...
static int smb_delete_user(const char *unix_user)
{
	...
	if ( strequal( unix_user, "root" ) ) {
		DEBUG0(("smb_delete_user: Refusing to delete local system root account!\n"));
		return -1;
	}
					     ...
					     uint32 rid)
{
	...
	if (!get_domain_group_from_sid(group_sid, &map)) {
		DEBUG10( ("Could not find group for rid %d\n", rid));
...
BOOL pdb_new_rid(uint32 *rid)
{
	...
	if (pdb_rid_algorithm()) {
		DEBUG0( ("Trying to allocate a RID when algorithmic RIDs "
			  "are active\n"));
		return False;
	}

	if (algorithmic_rid_base() != BASE_RID) {
		DEBUG0( ("'algorithmic rid base' is set but a passdb backend "
	...
	if ( (ctx = talloc_init("pdb_new_rid")) == NULL ) {
		DEBUG0(("pdb_new_rid: Talloc initialization failure\n"));
		return False;
	}
	...
	if ( allocated_rid == 0 ) {
		DEBUG0(("pdb_new_rid: Failed to find unused RID\n"));
		return False;
	}
				   ...
				   DOM_SID *sid)
{
	...
	if ( !unix_pw ) {
		DEBUG4(("pdb_default_uid_to_rid: host has no idea of uid "
			 "%lu\n", (unsigned long)uid));
		return False;
	}
	
	if ( !(sampw = samu_new( NULL )) ) {
		DEBUG0(("pdb_default_uid_to_rid: samu_new() failed!\n"));
		return False;
	}
	...
	if (!ret) {
		DEBUG5( ("pdb_default_uid_to_rid: Did not find user "
			  "%s (%d)\n", unix_pw->pw_name, uid));
		TALLOC_FREE(sampw);
		return False;
	}
				   ...
				   uint32 *rid)
{
	...
	if (!ret) {
		DEBUG1( ("Could not peek rid out of sid %s\n",
				  ...
				  union unid_t *id, enum lsa_SidType *type)
{
	...
	if (sid_peek_check_rid(&global_sid_Builtin, sid, &rid)) {
		...
		if (!NT_STATUS_IS_OK(methods->getgrsid(methods, &map, *sid))) {
			DEBUG10( ("Could not find map for sid %s\n",
				   sid_string_static(sid)));
			goto done;
		}
		if ((map.sid_name_use != SID_NAME_ALIAS) &&
		    (map.sid_name_use != SID_NAME_WKN_GRP)) {
			DEBUG10( ("Map for sid %s is a %s, expected an "
				   "alias\n", sid_string_static(sid),
	...
	DEBUG5( ("Sid %s is neither ours, a Unix SID, nor builtin\n",
		  sid_string_static(sid)));
	...
	*pp_member_rids = TALLOC_ZERO_ARRAY(mem_ctx, uint32, num_uids);
	...
	for (i=0; i<num_uids; i++) {
		...
		if (!sid_check_is_in_our_domain(&sid)) {
			DEBUG5( ("Inconsistent SAM -- group member uid not "
				  "in our domain\n"));
			continue;
		}
				  ...
				  union unid_t *unix_id)
{
	...
	*psid_name_use = SID_NAME_UNKNOWN;
	...
	DEBUG5(("lookup_global_sam_rid: looking up RID %u.\n",
		 (unsigned int)rid));
	...
	if ( unix_id ) {
		DEBUG5( ("Can't find a unix id for an unmapped group\n"));
		return False;
	}
			     ...
			     struct samr_displayentry *entry)
{
 ...
 next:
	if ( !(user = samu_new( NULL )) ) {
		DEBUG0( ("next_entry_users: samu_new() failed!\n"));
		return False;
	}
				     ...
				     uint32 acct_flags)
{
	...
	if (user_search_in_progress) {
		DEBUG1( ("user search in progress\n"));
		return False;
	}

	if (!pdb_setsampwent(False, acct_flags)) {
		DEBUG5( ("Could not start search\n"));
		return False;
	}
