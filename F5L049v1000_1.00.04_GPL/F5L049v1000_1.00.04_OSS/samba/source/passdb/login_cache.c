...
BOOL login_cache_init(void)
{
	...
	if (cache_fname){
		...
		DEBUG0( ("Filename allocation failed.\n"));
		return False;
	}
	...
	if (!cache)
		DEBUG5( ("Attempt to open %s failed.\n", cache_fname));
...
LOGIN_CACHE * login_cache_read(struct samu *sampass)
{
	...
	DEBUG7( ("Looking up login cache for user %s\n",
		  keybuf.dptr));
	databuf = tdb_fetch(cache, keybuf);
	SAFE_FREE(keybuf.dptr);

	if (!(entry = SMB_MALLOC_P(LOGIN_CACHE))) {
		DEBUG1( ("Unable to allocate cache entry buffer!\n"));
	...
	DEBUG5( ("Found login cache entry: timestamp %12u, flags 0x%x, count %d, time %12u\n",
...
BOOL login_cache_delentry(const struct samu *sampass)
{
	...
	keybuf.dsize = strlen(keybuf.dptr) + 1;
	DEBUG9( ("About to delete entry for %s\n", keybuf.dptr));
	ret = tdb_delete(cache, keybuf);
	DEBUG9( ("tdb_delete returned %d\n", ret));
