						 ...
						 uint32 *channel)
{
	...
	if (!(pass = (struct machine_acct_pass *)secrets_fetch(
		...
		DEBUG5( ("secrets_fetch failed!\n"));
		return False;
	}
	
	if (size != sizeof(*pass)) {
		DEBUG0( ("secrets were of incorrect size!\n"));
		return False;
	}
					  ...
					  uint32 *channel)
{
	...
	plaintext = secrets_fetch_machine_password(domain, pass_last_set_time,
						   channel);
	if (plaintext) {
		DEBUG4(("Using cleartext machine password\n"));
                                           ...
                                           DOM_SID *sid, time_t *pass_last_set_time)
{
	...
	if (pass_len != size) {
		DEBUG5( ("Invalid secrets size. Unpacked data doesn't match trusted_dom_pass structure.\n"));
		return False;
	}
                                           ...
                                           const DOM_SID *sid)
{
	...
	if (push_ucs2_allocate(&uni_dom_name, domain) == (size_t)-1) {
		DEBUG0( ("Could not convert domain name %s to unicode\n",
			  domain));
		return False;
	}
			...
			const char **account_name, uint32 *channel)
{
	...
	if (is_trusted_domain_situation(domain)) {
		if (!secrets_fetch_trusted_domain_password(domain, ret_pwd,
							   NULL, &last_set_time))
		{
			DEBUG0( ("get_trust_pw: could not fetch trust "
	...
	DEBUG5( ("get_trust_pw_clear: could not fetch clear text trust "
		       ...
		       const char **account_name, uint32 *channel)
{
	...
	DEBUG5( ("get_trust_pw_hash: could not fetch trust account "
...
BOOL secrets_store_ldap_pw(const char* dn, char* pw)
{
	...
	if (asprintf(&key, "%s/%s", SECRETS_LDAP_BIND_PW, dn) < 0) {
		DEBUG0( ("secrets_store_ldap_pw: asprintf failed!\n"));
		return False;
	}
...
BOOL fetch_ldap_pw(char **dn, char** pw)
{
	...
	*dn = smb_xstrdup(lp_ldap_admin_dn());
	...
	if (asprintf(&key, "%s/%s", SECRETS_LDAP_BIND_PW, *dn) < 0) {
		SAFE_FREE(*dn);
		DEBUG0( ("fetch_ldap_pw: asprintf failed!\n"));
	}
	...
	*pw=(char *)secrets_fetch(key, &size);
	...
	if (!size) {
		...
		if (!old_style_key) {
			DEBUG0( ("fetch_ldap_pw: strdup failed!\n"));
			return False;
		}
		...
		if (!size && size < sizeof(old_style_pw)) {
			DEBUG0(("fetch_ldap_pw: neither ldap secret retrieved!\n"));
		...
		if (!secrets_store_ldap_pw(*dn, old_style_pw)) {
			DEBUG0(("fetch_ldap_pw: ldap secret could not be upgraded!\n"));
			SAFE_FREE(old_style_key);
			SAFE_FREE(*dn);
			return False;			
		}
		if (!secrets_delete(old_style_key)) {
			DEBUG0(("fetch_ldap_pw: old ldap secret could not be deleted!\n"));
		}
				 ...
				 struct trustdom_info ***domains)
{
	...
	if (pattern == NULL) {
		DEBUG0( ("secrets_trusted_domains: talloc_asprintf() "
			  "failed!\n"));
		TALLOC_FREE(tmp_ctx);
		return NT_STATUS_NO_MEMORY;
	}
	...
	*num_domains = 0;
	...
	for (k = keys; k; k = k->next) {
		...
		if (size != packed_size) {
			DEBUG2( ("Secrets record %s is invalid!\n",
				  secrets_key));
			continue;
		}

		if (pass.domain_sid.num_auths != 4) {
			DEBUG0( ("SID %s is not a domain sid, has %d "
				  "auths instead of 4\n",
		...
		if (pull_ucs2_talloc(dom_info, &dom_info->name,
			...
			DEBUG2( ("pull_ucs2_talloc failed\n"));
			tdb_search_list_free(keys);
			TALLOC_FREE(tmp_ctx);
			return NT_STATUS_NO_MEMORY;
		}
	...
	DEBUG5( ("secrets_get_trusted_domains: got %d domains\n",
		  *num_domains));
...
BOOL secrets_named_mutex(const char *name, unsigned int timeout)
{
	...
	if (ret == 0)
		DEBUG10(("secrets_named_mutex: got mutex for %s\n", name ));
...
void secrets_named_mutex_release(const char *name)
{
	...
	DEBUG10(("secrets_named_mutex: released mutex for %s\n", name ));
}
...
void secrets_fetch_ipc_userpass(char **username, char **domain, char **password)
{
	...
	*password = (char *)secrets_fetch(SECRETS_AUTH_PASSWORD, NULL);
	...
	if (*username && **username) {
		...
		DEBUG3( ("IPC$ connections done by user %s\\%s\n", 
			  *domain, *username));

	} else {
		DEBUG3( ("IPC$ connections done anonymously\n"));
...
static TDB_CONTEXT *open_schannel_session_store(TALLOC_CTX *mem_ctx)
{
	...
	if (!fname) {
        ...
        if (!tdb_sc) {
                DEBUG0(("open_schannel_session_store: Failed to open %s\n", fname));
		TALLOC_FREE(fname);
                return NULL;
        }
	...
	if (vers.dptr == NULL) {
		...
		if (ver != 1) {
			tdb_close(tdb_sc);
			tdb_sc = NULL;
			DEBUG0(("open_schannel_session_store: wrong version number %d in %s\n",
				(int)ver, fname ));
		}
	} else {
		tdb_close(tdb_sc);
		tdb_sc = NULL;
		DEBUG0(("open_schannel_session_store: wrong version number size %d in %s\n",
				...
				const struct dcinfo *pdc)
{
	...
	DEBUG3(("secrets_store_schannel_session_info: stored schannel info with key %s\n",
		keystr ));
				...
				struct dcinfo **ppdc)
{
	...
	*ppdc = NULL;
	...
	if (!value.dptr) {
		DEBUG0(("secrets_restore_schannel_session_info: Failed to find entry with key %s\n",
			keystr ));
		tdb_close(tdb_sc);
		return False;
	}
	...
	DEBUG3(("secrets_restore_schannel_session_info: restored schannel info key %s\n",
		keystr ));
