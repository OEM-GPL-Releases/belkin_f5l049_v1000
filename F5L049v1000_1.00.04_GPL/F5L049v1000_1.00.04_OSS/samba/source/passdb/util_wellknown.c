			  ...
			  const char **domain, const char **name)
{
	...
	if (!sid_split_rid(&dom_sid, &rid)) {
		DEBUG2( ("Could not split rid from SID\n"));
		return False;
	}
	...
	if (users == NULL) {
		DEBUG10( ("SID %s is no special sid\n",
	...
	DEBUG10( ("RID of special SID %s not found\n",
		   sid_string_static(sid)));
			   ...
			   DOM_SID *sid, const char **domain)
{
	...
	DEBUG10(("map_name_to_wellknown_sid: looking up %s\n", name));
