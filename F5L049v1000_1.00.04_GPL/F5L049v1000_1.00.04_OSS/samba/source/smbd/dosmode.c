...
#endif
#define ATTR_SPARSE_FILE    0x0200 /* sparse file */
#define ATTR_REPARSE_POINT  0x0400 /* reparse point */
#define ATTR_COMPRESSED     0x0800 /* compressed */
#define ATTR_ENCRYPT        0x4000 /* encryption */
		 ...
		 const char *inherit_from_dir)
{
	...
	if (fname && (inherit_from_dir != NULL)
		...
		DEBUG2( ("unix_mode(%s) inheriting from %s\n", fname,
			  inherit_from_dir));
		if (SMB_VFS_STAT(conn, inherit_from_dir, &sbuf) != 0) {
			DEBUG4(("unix_mode(%s) failed, [dir %s]: %s\n", fname,
		...
		DEBUG2(("unix_mode(%s) inherit mode %o\n",fname,(int)dir_mode));
		/* Clear "result" */
		result = 0;
	} 
	...
	DEBUG3(("unix_mode(%s) returning 0%o\n",fname,(int)result ));
	return(result);
}
...
static uint32 dos_mode_from_sbuf(connection_struct *conn, const char *path, SMB_STRUCT_STAT *sbuf)
{
	...
	DEBUG8(("dos_mode_from_sbuf returning "));

	if (result & aHIDDEN) DEBUG8( ("h"));
	if (result & aRONLY ) DEBUG8( ("r"));
	if (result & aSYSTEM) DEBUG8( ("s"));
	if (result & aDIR   ) DEBUG8( ("d"));
	if (result & aARCH  ) DEBUG8( ("a"));
...
static BOOL get_ea_dos_attribute(connection_struct *conn, const char *path,SMB_STRUCT_STAT *sbuf, uint32 *pattr)
{
	...
	if (sizeret == -1) {
		...
		if ((errno != ENOTSUP) && (errno != ENOATTR) && (errno != EACCES) && (errno != EPERM)) {
			DEBUG1(("get_ea_dos_attributes: Cannot get attribute from EA on file %s: Error = %s\n",
				path, strerror(errno) ));
	...
	attrstr[sizeret] = 0;
	DEBUG10(("get_ea_dos_attribute: %s attrstr = %s\n", path, attrstr));

	if (sizeret < 2 || attrstr[0] != '0' || attrstr[1] != 'x' ||
			sscanf(attrstr, "%x", &dosattr) != 1) {
		DEBUG1(("get_ea_dos_attributes: Badly formed DOSATTRIB on file %s - %s\n", path, attrstr));
                return False;
        }
	...
	*pattr = (uint32)(dosattr & SAMBA_ATTRIBUTES_MASK);
	...
	DEBUG8(("get_ea_dos_attribute returning (0x%x)", dosattr));

	if (dosattr & aHIDDEN) DEBUG8( ("h"));
	if (dosattr & aRONLY ) DEBUG8( ("r"));
	if (dosattr & aSYSTEM) DEBUG8( ("s"));
	if (dosattr & aDIR   ) DEBUG8( ("d"));
	if (dosattr & aARCH  ) DEBUG8( ("a"));
...
static BOOL set_ea_dos_attribute(connection_struct *conn, const char *path, SMB_STRUCT_STAT *sbuf, uint32 dosmode)
{
	...
	DEBUG10(("set_ea_dos_attribute: set EA %s on file %s\n", attrstr, path));
	return True;
}
...
uint32 dos_mode_msdfs(connection_struct *conn, const char *path,SMB_STRUCT_STAT *sbuf)
{
	...
	if(!re && (sxfs.f_type==MSDOS_SUPER_MAGIC)) {
		...
		if(!(re=ioctl(fd,FAT_IOCTL_GET_ATTRIBUTES,&attr))) {
			if(attr & ATTR_RO)    result |= aRONLY;
			if(attr & ATTR_HIDDEN)result |= aHIDDEN;
			if(attr & ATTR_SYS)   result |= aSYSTEM;
			if(attr & ATTR_DIR)   result |= aDIR;
			if(attr & ATTR_ARCH)  result |= aARCH;
		}
		if(result&aDIR) {
			if(result&aRONLY&&!conn->read_only) {
				become_root();
				chmod(path,0777);
				unbecome_root();
			}
		...
		DEBUG10(("attr=0x%x(%d)path=%s",attr,re,path));
...
#endif /* _SX_FATATTR */
#ifdef _SX_NTFSATTR
	if(!re && ((sxfs.f_type==FUSE_SUPER_MAGIC) || (sxfs.f_type==NTFS_SB_MAGIC)) ) {
		uint32 attr=0;

		re = getxattr(path,"/attribute/",&attr,sizeof(attr));
		if(re>=0) {
			if(attr & ATTR_RO)    result |= aRONLY;
			if(attr & ATTR_HIDDEN)result |= aHIDDEN;
			if(attr & ATTR_SYS)   result |= aSYSTEM;
			if(attr & ATTR_DIR)   result |= aDIR;
			if(attr & ATTR_ARCH)  result |= aARCH;
			if(attr & ATTR_SPARSE_FILE)   result |= ATTR_SPARSE_FILE;
			if(attr & ATTR_REPARSE_POINT) result |= ATTR_REPARSE_POINT;
			if(attr & ATTR_COMPRESSED)    result |= ATTR_COMPRESSED;
			if(attr & ATTR_ENCRYPT)       result |= ATTR_ENCRYPT;
		}
	...
	/* Optimization : Only call is_hidden_path if it's not already
	   hidden. */
	if(IS_HIDDEN_PATH(conn,path)) {
		result |= aHIDDEN;
	}

	DEBUG8(("dos_mode_msdfs returning "));
	...
	if (result & aDIR   ) DEBUG8( ("d"));
	if (result & aARCH  ) DEBUG8( ("a"));
	if (result & FILE_ATTRIBUTE_SPARSE ) DEBUG8( ("[sparse]"));
		     ...
		     const char *parent_dir)
{
	...
	DEBUG10(("file_set_dosmode: setting dos mode 0x%x on file %s\n", dosmode, fname));
	...
	if(!re && (sxfs.f_type==MSDOS_SUPER_MAGIC)) {
		...
		re=ioctl(fd,FAT_IOCTL_GET_ATTRIBUTES,&attr);

		dosmode &= SAMBA_ATTRIBUTES_MASK;

		if(dosmode & FILE_ATTRIBUTE_READONLY)  attr |=  ATTR_RO;
		else                                   attr &= ~ATTR_RO;
		if(dosmode & FILE_ATTRIBUTE_HIDDEN)    attr |=  ATTR_HIDDEN;
		else                                   attr &= ~ATTR_HIDDEN;
		if(dosmode & FILE_ATTRIBUTE_ARCHIVE)   attr |=  ATTR_ARCH;
		else                                   attr &= ~ATTR_ARCH;
		...
		DEBUG10(("attr=0x%x (getattr ok?=%d:setattr ok?=%d)", attr, re, ret));
	...
	if(!re && ((sxfs.f_type==FUSE_SUPER_MAGIC) || (sxfs.f_type==NTFS_SB_MAGIC)) ) {
		...
		re = getxattr(fname,"/attribute/",&attr,sizeof(attr));
		...
		become_root();
		re = setxattr(fname,"/attribute/",&attr,sizeof(attr),0);
		unbecome_root();
		return 0;
	}
#endif /* _SX_NTFSATTR */
		...
		const struct timespec mtime)
{
	...
	if (file_ntimes(conn, fname, ts)) {
		DEBUG4(("set_filetime(%s) failed: %s\n",fname,strerror(errno)));
		return False;
	}
