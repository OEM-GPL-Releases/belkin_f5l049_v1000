...
static void msg_deliver(void)
{
	...
	if (! (*lp_msg_command())) {
		DEBUG1(("no messaging command specified\n"));
		msgpos = 0;
		return;
	}
	...
	if (fd == -1) {
		DEBUG1(("can't open message file %s\n",name));
		return;
	}
	...
	if ((len = (int)convert_string_allocate(NULL,CH_DOS, CH_UNIX, msgbuf, msgpos, (void **)(void *)&msg, True)) < 0 || !msg) {
		DEBUG3(("Conversion failed, delivering message in DOS codepage format\n"));
		for (i = 0; i < msgpos;) {
			...
			if ( sz != 1 ) {
				DEBUG0(("Write error to fd %d: %ld(%d)\n",fd, (long)sz, errno ));
			}
		}
	} else {
		for (i = 0; i < len;) {
...
int reply_sendstrt(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	DEBUG3( ( "SMBsendstrt (from %s to %s)\n", msgfrom, msgto ) );
