...
#if (HAVE_SYS_STATFS_H) && (LINUX)
#define SX_USE_FAT_TRUNCATE 0
#else
#define SX_USE_FAT_TRUNCATE 1
#endif
				...
				const char *fname, char *ea_name, struct ea_struct *pea)
{
 ...
 again:
	...
	DEBUG10(("get_ea_value: EA %s is of length %u: ", ea_name, (unsigned int)sizeret));
					...
					const char *fname, size_t *pea_total_len)
{
	...
	*pea_total_len = 0;
	...
	DEBUG10(("get_ea_list_from_file: ea_namelist size = %u\n", (unsigned int)sizeret ));
		...
		for (p = ea_namelist; p - ea_namelist < sizeret; p += strlen(p) + 1) {
...

			{
				...
				*pea_total_len += 4 + strlen(dos_ea_name) + 1 + listp->ea.value.length;
				DEBUG10(("get_ea_list_from_file: total_len = %u, %s, val len = %u\n",
	...
	DEBUG10(("get_ea_list_from_file: total_len = %u\n", (unsigned int)*pea_total_len));
	return ea_list_head;
}
	...
	connection_struct *conn, struct ea_list *ea_list)
{
	...
	DEBUG10(("fill_ea_buffer: data_size = %u\n", ret_data_size ));
...
static void canonicalize_ea_name(connection_struct *conn, files_struct *fsp, const char *fname, fstring unix_ea_name)
{
	...
	for (; ea_list; ea_list = ea_list->next) {
		if (strequal(&unix_ea_name[5], ea_list->ea.name)) {
			DEBUG10(("canonicalize_ea_name: %s -> %s\n",
...
NTSTATUS set_ea(connection_struct *conn, files_struct *fsp, const char *fname, struct ea_list *ea_list)
{
	...
	for (;ea_list; ea_list = ea_list->next) {
		...
		DEBUG10(("set_ea: ea_name %s ealen = %u\n", unix_ea_name, (unsigned int)ea_list->ea.value.length));

		if (samba_private_attr_name(unix_ea_name)) {
			DEBUG10(("set_ea: ea name %s is a private Samba name.\n", unix_ea_name));
		...
		if (ea_list->ea.value.length == 0) {
			...
			if (fsp && (fsp->fh->fd != -1)) {
				DEBUG10(("set_ea: deleting ea name %s on file %s by file descriptor.\n",
					unix_ea_name, fsp->fsp_name));
				ret = SMB_VFS_FREMOVEXATTR(fsp, fsp->fh->fd, unix_ea_name);
			} else {
				DEBUG10(("set_ea: deleting ea name %s on file %s.\n",
					unix_ea_name, fname));
			...
			if (ret == -1 && errno == ENOATTR) {
				DEBUG10(("set_ea: deleting ea name %s didn't exist - succeeding by default.\n",
						unix_ea_name));
				ret = 0;
			}
#endif
			...
			if (fsp && (fsp->fh->fd != -1)) {
				DEBUG10(("set_ea: setting ea name %s on file %s by file descriptor.\n",
					unix_ea_name, fsp->fsp_name));
				ret = SMB_VFS_FSETXATTR(fsp, fsp->fh->fd, unix_ea_name,
							ea_list->ea.value.data, ea_list->ea.value.length, 0);
			} else {
				DEBUG10(("set_ea: setting ea name %s on file %s.\n",
					unix_ea_name, fname));
...
static struct ea_list *read_ea_name_list(TALLOC_CTX *ctx, const char *pdata, size_t data_size)
{
	...
	while (offset + 2 < data_size) {
		...
		DEBUG10(("read_ea_name_list: read ea name %s\n", eal->ea.name));
	}
...
struct ea_list *read_ea_list_entry(TALLOC_CTX *ctx, const char *pdata, size_t data_size, size_t *pbytes_used)
{
	...
	DEBUG10(("read_ea_list_entry: read ea name %s\n", eal->ea.name));
			...
			int max_data_bytes)
{
	...
	if (max_data_bytes > 0 && datasize > max_data_bytes) {
		DEBUG5(("send_trans2_replies: max_data_bytes %d exceeded by data %d\n",
	...
	while (params_to_send || data_to_send) {
		...
		DEBUG9(("t2_rep: params_sent_thistime = %d, data_sent_thistime = %d, useable_space = %d\n",
			params_sent_thistime, data_sent_thistime, useable_space));
		DEBUG9(("t2_rep: params_to_send = %d, data_to_send = %d, paramsize = %d, datasize = %d\n",
		...
		/* Sanity check */
		if(params_to_send < 0 || data_to_send < 0) {
			DEBUG0(("send_trans2_replies failed sanity check pts = %d, dts = %d\n!!!",
				...
				unsigned int max_data_bytes)
{
	...
	DEBUG3(("call_trans2open %s deny_mode=0x%x attr=%d ofun=0x%x size=%d\n",
	...
	if (total_data != 4) {
		...
		if (IVAL(pdata,0) > total_data) {
			DEBUG10(("call_trans2open: bad total data size (%u) > %u\n",
...
static uint32 unix_filetype(mode_t mode)
{
	...
	DEBUG0(("unix_filetype: unknown filetype %u", (unsigned)mode));
				 ...
				 int *last_entry_off, struct ea_list *name_list, TALLOC_CTX *ea_ctx)
{
	...
	*got_exact_match = False;
	...
	while (!found) {
		...
		DEBUG8(("get_lanman2_dir_entry:readdir on dirptr 0x%lx now at offset %ld\n",
		...
		if(got_match) {
			...
			if (INFO_LEVEL_IS_UNIX(info_level)) {
				if (SMB_VFS_LSTAT(conn,pathreal,&sbuf) != 0) {
					DEBUG5(("get_lanman2_dir_entry:Couldn't lstat [%s] (%s)\n",
				...
				if(lp_host_msdfs() && 
				   ...
				   ((ms_dfs_link = is_msdfs_link(conn, pathreal, link_target, &sbuf)) == True)) {
					DEBUG5(("get_lanman2_dir_entry: Masquerading msdfs link %s "
						"as a directory\n",
						pathreal));
					...
					DEBUG5(("get_lanman2_dir_entry:Couldn't stat [%s] (%s)\n",
						pathreal,strerror(errno)));
					continue;
				}
			}
			...
			if (!dir_check_ftype(conn,mode,dirtype)) {
				DEBUG5(("[%s] attribs didn't match %x\n",fname,dirtype));
				continue;
			}
			...
			DEBUG5(("get_lanman2_dir_entry found %s fname=%s\n",pathreal,fname));
	...
	switch (info_level) {
		case SMB_FIND_INFO_STANDARD:
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_INFO_STANDARD\n"));
		...
		case SMB_FIND_EA_SIZE:
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_EA_SIZE\n"));
			if(requires_resume_key) {
				SIVAL(p,0,reskey);
				p += 4;
			}
		...
		case SMB_FIND_EA_LIST:
		{
			...
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_EA_LIST\n"));
			if (!name_list) {
				return False;
			}
			if(requires_resume_key) {
			...
			if (PTR_DIFF(p + 255 + ea_len,pdata) > space_remaining) {
				...
				*out_of_space = True;
				DEBUG9(("get_lanman2_dir_entry: out of space\n"));
		...
		case SMB_FIND_FILE_BOTH_DIRECTORY_INFO:
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_FILE_BOTH_DIRECTORY_INFO\n"));
		...
		case SMB_FIND_FILE_DIRECTORY_INFO:
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_FILE_DIRECTORY_INFO\n"));
			p += 4;
			SIVAL(p,0,reskey); p += 4;
			put_long_date_timespec(p,create_date_ts); p += 8;
			put_long_date_timespec(p,adate_ts); p += 8;
		...
		case SMB_FIND_FILE_FULL_DIRECTORY_INFO:
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_FILE_FULL_DIRECTORY_INFO\n"));
			p += 4;
			SIVAL(p,0,reskey); p += 4;
			put_long_date_timespec(p,create_date_ts); p += 8;
			put_long_date_timespec(p,adate_ts); p += 8;
		...
		case SMB_FIND_FILE_NAMES_INFO:
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_FILE_NAMES_INFO\n"));
			p += 4;
			SIVAL(p,0,reskey); p += 4;
			p += 4;
		...
		case SMB_FIND_ID_FULL_DIRECTORY_INFO:
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_ID_FULL_DIRECTORY_INFO\n"));
			p += 4;
			SIVAL(p,0,reskey); p += 4;
			put_long_date_timespec(p,create_date_ts); p += 8;
			put_long_date_timespec(p,adate_ts); p += 8;
		...
		case SMB_FIND_ID_BOTH_DIRECTORY_INFO:
			DEBUG10(("get_lanman2_dir_entry: SMB_FIND_ID_BOTH_DIRECTORY_INFO\n"));
			was_8_3 = mangle_is_8_3(fname, True, conn->params);
			p += 4;
			SIVAL(p,0,reskey); p += 4;
			put_long_date_timespec(p,create_date_ts); p += 8;
		...
		case SMB_FIND_FILE_UNIX_INFO2:
			...
			if (info_level == SMB_FIND_FILE_UNIX) {
				DEBUG10(("get_lanman2_dir_entry: SMB_FIND_FILE_UNIX\n"));
				p = store_file_unix_basic(conn, p,
							NULL, &sbuf);
				len = srvstr_push(outbuf, p, fname, PTR_DIFF(end_data, p), STR_TERMINATE);
			} else {
				DEBUG10(("get_lanman2_dir_entry: SMB_FIND_FILE_UNIX_INFO2\n"));
				...
				unsigned int max_data_bytes)
{
	...
	*directory = *mask = 0;
	...
	DEBUG3(("call_trans2findfirst: dirtype = %x, maxentries = %d, close_after_first=%d, \
close_if_end = %d requires_resume_key = %d level = 0x%x, max_data_bytes = %d\n",
	...
	if (info_level == SMB_FIND_EA_LIST) {
		...
		if (ea_size != total_data) {
			DEBUG4(("call_trans2findfirst: Rejecting EA request with incorrect \
total_data=%u (should be %u)\n", (unsigned int)total_data, (unsigned int)IVAL(pdata,0) ));
	...
	*pparams = (char *)SMB_REALLOC(*pparams, 10);
	...
	DEBUG4(("dptr_num is %d, wcard = %s, attr = %d\n", dptr_num, mask, dirtype));

	/* We don't need to check for VOL here as this is returned by 
		a different TRANS2 call. */
  
	DEBUG8(("dirpath=<%s> dontdescend=<%s>\n", conn->dirpath,lp_dontdescend(SNUM(conn))));
	...
	if(close_after_first || (finished && close_if_end)) {
		DEBUG5(("call_trans2findfirst - (2) closing dptr_num %d\n", dptr_num));
		dptr_close(&dptr_num);
	}
	...
	DEBUG4( ( "%s mask=%s directory=%s dirtype=%d numentries=%d\n",
					...
					unsigned int max_data_bytes)
{
	...
	*mask = *directory = *resume_name = 0;
	...
	DEBUG3(("call_trans2findnext: dirhandle = %d, max_data_bytes = %d, maxentries = %d, \
...
resume_key = %d resume name = %s continue=%d level = %d\n",
	...
	if (info_level == SMB_FIND_EA_LIST) {
		...
		ea_size = IVAL(pdata,0);
		if (ea_size != total_data) {
			DEBUG4(("call_trans2findnext: Rejecting EA request with incorrect \
total_data=%u (should be %u)\n", (unsigned int)total_data, (unsigned int)IVAL(pdata,0) ));
			return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
		}
	...
	*pparams = (char *)SMB_REALLOC(*pparams, 6*SIZEOFWORD);
	...
	if((p = dptr_wcard(dptr_num))== NULL) {
		DEBUG2(("dptr_num %d has no wildcard\n", dptr_num));
		talloc_destroy(ea_ctx);
		return ERROR_DOS(ERRDOS,ERRnofiles);
	}
	...
	DEBUG3(("dptr_num is %d, mask = %s, attr = %x, dirptr=(0x%lX,%ld)\n",
		dptr_num, mask, dirtype, 
		(long)conn->dirptr,
	...
	if(close_after_request || (finished && close_if_end)) {
		DEBUG5(("call_trans2findnext: closing dptr_num = %d\n", dptr_num));
	...
	DEBUG3( ( "%s mask=%s directory=%s dirtype=%d numentries=%d\n",
		smb_fn_name(CVAL(inbuf,smb_com)), 
		mask, directory, dirtype, numentries ) );
					...
					unsigned int max_data_bytes)
{
	...
	DEBUG3(("call_trans2qfsinfo: level = %d\n", info_level));

	if(SMB_VFS_STAT(conn,".",&st)!=0) {
		DEBUG2(("call_trans2qfsinfo: stat of . failed (%s)\n", strerror(errno)));
	...
	*ppdata = (char *)SMB_REALLOC(
	...
	switch (info_level) {
		case SMB_INFO_ALLOCATION:
		{
			...
			DEBUG5(("call_trans2qfsinfo : SMB_INFO_ALLOCATION id=%x, bsize=%u, cSectorUnit=%u, \
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)st.st_dev, (unsigned int)bsize, (unsigned int)sectors_per_unit,
		...
		case SMB_INFO_VOLUME:
			...
			DEBUG5(("call_trans2qfsinfo : time = %x, namelen = %d, name = %s\n",
		...
		case SMB_FS_ATTRIBUTE_INFORMATION:
		{
			...
			re = statfs(conn->connectpath,&sxfs);
			if(!re) {
				if((sxfs.f_type==MSDOS_SUPER_MAGIC)) {
					fstype = fat;
				}
				if((sxfs.f_type==NTFS_SB_MAGIC||sxfs.f_type==FUSE_SUPER_MAGIC)) {
			...
			/* FS ATTRIBUTES */
			SIVAL(pdata,0,FILE_CASE_PRESERVED_NAMES|FILE_UNICODE_ON_DISK|quota_flag|fsattr); 
		...
		case SMB_FS_VOLUME_INFORMATION:
			...
			DEBUG5(("call_trans2qfsinfo : SMB_QUERY_FS_VOLUME_INFO namelen = %d, vol=%s serv=%s\n", 
		...
		case SMB_FS_SIZE_INFORMATION:
		{
			...
			bytes_per_sector = 512;
			sectors_per_unit = bsize/bytes_per_sector;
			DEBUG5(("call_trans2qfsinfo : SMB_QUERY_FS_SIZE_INFO bsize=%u, cSectorUnit=%u, \
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)bsize, (unsigned int)sectors_per_unit,
		...
		case SMB_FS_FULL_SIZE_INFORMATION:
		{
			...
			bytes_per_sector = 512;
			sectors_per_unit = bsize/bytes_per_sector;
			DEBUG5(("call_trans2qfsinfo : SMB_QUERY_FS_FULL_SIZE_INFO bsize=%u, cSectorUnit=%u, \
cBytesSector=%u, cUnitTotal=%u, cUnitAvail=%d\n", (unsigned int)bsize, (unsigned int)sectors_per_unit,
				(unsigned int)bytes_per_sector, (unsigned int)dsize, (unsigned int)dfree));
		 ...
		 */
		{
			...
			if (current_user.ut.uid != 0) {
				DEBUG0(("set_user_quota: access_denied service [%s] user [%s]\n",
					lp_servicename(SNUM(conn)),conn->user));
				return ERROR_DOS(ERRDOS,ERRnoaccess);
			}
			
			if (vfs_get_ntquota(&fsp, SMB_USER_FS_QUOTA_TYPE, NULL, &quotas)!=0) {
				DEBUG0(("vfs_get_ntquota() failed for service [%s]\n",lp_servicename(SNUM(conn))));
				return ERROR_DOS(ERRSRV,ERRerror);
			}

			data_len = 48;

			DEBUG10(("SMB_FS_QUOTA_INFORMATION: for service [%s]\n",lp_servicename(SNUM(conn))));		
		...
		case SMB_QUERY_POSIX_FS_INFO:
		{
			...
			if (!rc) {
				...
				DEBUG5(("call_trans2qfsinfo : SMB_QUERY_POSIX_FS_INFO succsessful\n"));
#ifdef EOPNOTSUPP
			} else if (rc == EOPNOTSUPP) {
				return ERROR_NT(NT_STATUS_INVALID_LEVEL);
#endif /* EOPNOTSUPP */
			} else {
				DEBUG0(("vfs_statvfs() failed for service [%s]\n",lp_servicename(SNUM(conn))));
				return ERROR_DOS(ERRSRV,ERRerror);
			}
			break;
		}
					...
					unsigned int max_data_bytes)
{
	...
	DEBUG10(("call_trans2setfsinfo: for service [%s]\n",lp_servicename(SNUM(conn))));

	/*  */
	if (total_params < 4) {
		DEBUG0(("call_trans2setfsinfo: requires total_params(%d) >= 4 bytes!\n",
			total_params));
		return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
	}
		...
		case SMB_SET_CIFS_UNIX_INFO:
			{
				...
				DEBUG10(("call_trans2setfsinfo: set unix info. major = %u, minor = %u \
cap_low = 0x%x, cap_high = 0x%x\n",
		...
		case SMB_FS_QUOTA_INFORMATION:
			{
				...
				if (!CHECK_NTQUOTA_HANDLE_OK(fsp,conn)) {
					DEBUG3(("TRANSACT_GET_USER_QUOTA: no valid QUOTA HANDLE\n"));
					return ERROR_NT(NT_STATUS_INVALID_HANDLE);
				}

				if (total_data < 42) {
					DEBUG0(("call_trans2setfsinfo: SET_FS_QUOTA: requires total_data(%d) >= 42 bytes!\n",
						total_data));
					return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
				}
				...
				if (vfs_set_ntquota(fsp, SMB_USER_FS_QUOTA_TYPE, NULL, &quotas)!=0) {
					DEBUG0(("vfs_set_ntquota() failed for service [%s]\n",lp_servicename(SNUM(conn))));
					return ERROR_DOS(ERRSRV,ERRerror);
				}
		...
		default:
			DEBUG3(("call_trans2setfsinfo: unknown level (0x%X) not implemented yet.\n",
				info_level));
			return ERROR_NT(NT_STATUS_INVALID_LEVEL);
			break;
	}
...
static BOOL marshall_posix_acl(connection_struct *conn, char *pdata, SMB_STRUCT_STAT *pst, SMB_ACL_T posix_acl)
{
	...
	while ( posix_acl && (SMB_VFS_SYS_ACL_GET_ENTRY(conn, posix_acl, entry_id, &entry) == 1)) {
		...
		if (SMB_VFS_SYS_ACL_GET_TAG_TYPE(conn, entry, &tagtype) == -1) {
			DEBUG0(("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_TAG_TYPE failed.\n"));
			return False;
		}

		if (SMB_VFS_SYS_ACL_GET_PERMSET(conn, entry, &permset) == -1) {
			DEBUG0(("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_PERMSET failed.\n"));
			return False;
		}
		...
		switch (tagtype) {
			...
			case SMB_ACL_USER:
				{
					...
					if (!puid) {
						DEBUG0(("marshall_posix_acl: SMB_VFS_SYS_ACL_GET_QUALIFIER failed.\n"));
					}
			...
			case SMB_ACL_OTHER:
				...
				break;
			default:
				DEBUG0(("marshall_posix_acl: unknown tagtype.\n"));
				return False;
		}
...
static char *store_file_unix_basic(connection_struct *conn,
				...
				const SMB_STRUCT_STAT *psbuf)
{
	DEBUG10(("store_file_unix_basic: SMB_QUERY_FILE_UNIX_BASIC\n"));
	DEBUG4(("store_file_unix_basic: st_mode=%o\n",(int)psbuf->st_mode));
					...
					unsigned int max_data_bytes)
{
	...
	if (tran_call == TRANSACT2_QFILEINFO) {
		...
		DEBUG3(("call_trans2qfilepathinfo: TRANSACT2_QFILEINFO: level = %d\n", info_level));
		...
		if(fsp && (fsp->fake_file_handle)) {
			...
			if (INFO_LEVEL_IS_UNIX(info_level)) {
				...
				if (SMB_VFS_LSTAT(conn,fname,&sbuf)) {
					DEBUG3(("call_trans2qfilepathinfo: SMB_VFS_LSTAT of %s failed (%s)\n",fname,strerror(errno)));
					return UNIXERROR(ERRDOS,ERRbadpath);
				}
			} else if (SMB_VFS_STAT(conn,fname,&sbuf)) {
				DEBUG3(("call_trans2qfilepathinfo: SMB_VFS_STAT of %s failed (%s)\n",fname,strerror(errno)));
				return UNIXERROR(ERRDOS,ERRbadpath);
			}
			...
			if (SMB_VFS_FSTAT(fsp,fsp->fh->fd,&sbuf) != 0) {
				DEBUG3(("fstat of fnum %d failed (%s)\n", fsp->fnum, strerror(errno)));
		...
		DEBUG3(("call_trans2qfilepathinfo: TRANSACT2_QPATHINFO: level = %d\n", info_level));
		...
		status = check_name(conn, fname);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG3(("call_trans2qfilepathinfo: fileinfo of %s failed (%s)\n",fname,nt_errstr(status)));
			return ERROR_NT(status);
		}
	...
	DEBUG3(("call_trans2qfilepathinfo %s (fnum = %d) level=%d call=%d total_data=%d\n",
	...
	switch (info_level) {
		case SMB_INFO_QUERY_EAS_FROM_LIST:
		{
			...
			if (total_data > 0 && ea_size != total_data) {
				DEBUG4(("call_trans2qfilepathinfo: Rejecting EA request with incorrect \
total_data=%u (should be %u)\n", (unsigned int)total_data, (unsigned int)IVAL(pdata,0) ));
				return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
			}
	...
	*ppdata = (char *)SMB_REALLOC(*ppdata, data_size); 
	...
	switch (info_level) {
		case SMB_INFO_STANDARD:
			DEBUG10(("call_trans2qfilepathinfo: SMB_INFO_STANDARD\n"));
			data_size = 22;
		...
		case SMB_INFO_QUERY_EA_SIZE:
		{
			...
			DEBUG10(("call_trans2qfilepathinfo: SMB_INFO_QUERY_EA_SIZE\n"));
			data_size = 26;
		...
		case SMB_INFO_IS_NAME_VALID:
			DEBUG10(("call_trans2qfilepathinfo: SMB_INFO_IS_NAME_VALID\n"));
			if (tran_call == TRANSACT2_QFILEINFO) {
		...
		case SMB_INFO_QUERY_EAS_FROM_LIST:
		{
			...
			DEBUG10(("call_trans2qfilepathinfo: SMB_INFO_QUERY_EAS_FROM_LIST\n"));
		...
		case SMB_INFO_QUERY_ALL_EAS:
		{
			...
			DEBUG10(("call_trans2qfilepathinfo: SMB_INFO_QUERY_ALL_EAS\n"));
		...
		case SMB_QUERY_FILE_BASIC_INFO:
			...
			if (info_level == SMB_QUERY_FILE_BASIC_INFO) {
				DEBUG10(("call_trans2qfilepathinfo: SMB_QUERY_FILE_BASIC_INFO\n"));
				data_size = 36; /* w95 returns 40 bytes not 36 - why ?. */
			} else {
				DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_BASIC_INFORMATION\n"));
				data_size = 40;
				SIVAL(pdata,36,0);
			}
			...
			DEBUG5(("SMB_QFBI - "));
			DEBUG5(("create: %s ", ctime(&create_time)));
			DEBUG5(("access: %s ", ctime(&atime)));
			DEBUG5(("write: %s ", ctime(&mtime)));
			DEBUG5(("change: %s ", ctime(&mtime)));
			DEBUG5(("mode: %x\n", mode));
			break;
		...
		case SMB_QUERY_FILE_STANDARD_INFO:
			...
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_STANDARD_INFORMATION\n"));
			data_size = 24;
		...
		{
			unsigned int ea_size = estimate_ea_size(conn, fsp, fname);
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_EA_INFORMATION\n"));
			data_size = 4;
		...
		case SMB_FILE_ALTERNATE_NAME_INFORMATION:
		{
			...
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_ALTERNATE_NAME_INFORMATION\n"));
		...
		case SMB_QUERY_FILE_NAME_INFO:
			...
			DEBUG10(("call_trans2qfilepathinfo: SMB_QUERY_FILE_NAME_INFO\n"));
			data_size = 4 + len;
			SIVAL(pdata,0,len);
			break;
		...
		case SMB_QUERY_FILE_ALLOCATION_INFO:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_ALLOCATION_INFORMATION\n"));
			data_size = 8;
			SOFF_T(pdata,0,allocation_size);
			break;
		...
		case SMB_QUERY_FILE_END_OF_FILEINFO:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_END_OF_FILE_INFORMATION\n"));
			data_size = 8;
		...
		{
			unsigned int ea_size = estimate_ea_size(conn, fsp, fname);
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_ALL_INFORMATION\n"));
			put_long_date_timespec(pdata,create_time_ts);
			put_long_date_timespec(pdata+8,atime_ts);
			put_long_date_timespec(pdata+16,mtime_ts); /* write time */
			put_long_date_timespec(pdata+24,mtime_ts); /* change time */
		...
		case SMB_FILE_INTERNAL_INFORMATION:
			...
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_INTERNAL_INFORMATION\n"));
		...
		case SMB_FILE_ACCESS_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_ACCESS_INFORMATION\n"));
		...
		case SMB_FILE_NAME_INFORMATION:
			/* Pathname with leading '\'. */
			{
				...
				DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_NAME_INFORMATION\n"));
		...
		case SMB_FILE_DISPOSITION_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_DISPOSITION_INFORMATION\n"));
			data_size = 1;
			SCVAL(pdata,0,delete_pending);
			break;

		case SMB_FILE_POSITION_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_POSITION_INFORMATION\n"));
			data_size = 8;
			SOFF_T(pdata,0,pos);
			break;

		case SMB_FILE_MODE_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_MODE_INFORMATION\n"));
			SIVAL(pdata,0,mode);
			data_size = 4;
			break;

		case SMB_FILE_ALIGNMENT_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_ALIGNMENT_INFORMATION\n"));
		...
		case SMB_FILE_STREAM_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_STREAM_INFORMATION\n"));
			if (mode & aDIR) {
				data_size = 0;
			} else {
		...
		case SMB_FILE_COMPRESSION_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_COMPRESSION_INFORMATION\n"));
			SOFF_T(pdata,0,file_size);
			SIVAL(pdata,8,0); /* ??? */
			SIVAL(pdata,12,0); /* ??? */
			data_size = 16;
		...
		case SMB_FILE_NETWORK_OPEN_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_NETWORK_OPEN_INFORMATION\n"));
			put_long_date_timespec(pdata,create_time_ts);
			put_long_date_timespec(pdata+8,atime_ts);
			put_long_date_timespec(pdata+16,mtime_ts); /* write time */
			put_long_date_timespec(pdata+24,mtime_ts); /* change time */
		...
		case SMB_FILE_ATTRIBUTE_TAG_INFORMATION:
			DEBUG10(("call_trans2qfilepathinfo: SMB_FILE_ATTRIBUTE_TAG_INFORMATION\n"));
			SIVAL(pdata,0,mode);
			SIVAL(pdata,4,0);
			data_size = 8;
			break;
		...
		case SMB_QUERY_FILE_UNIX_BASIC:
			...
			{
				int i;
				DEBUG4(("call_trans2qfilepathinfo: SMB_QUERY_FILE_UNIX_BASIC "));

				for (i=0; i<100; i++)
					DEBUG4(("%d=%x, ",i, (*ppdata)[i]));
				DEBUG4(("\n"));
			}
		...
		case SMB_QUERY_FILE_UNIX_INFO2:
			...
			{
				int i;
				DEBUG4(("call_trans2qfilepathinfo: SMB_QUERY_FILE_UNIX_INFO2 "));
		...
		case SMB_QUERY_FILE_UNIX_LINK:
			{
				...
				DEBUG10(("call_trans2qfilepathinfo: SMB_QUERY_FILE_UNIX_LINK\n"));
#ifdef S_ISLNK
		...
		case SMB_QUERY_POSIX_ACL:
			{
				...
				if (file_acl == NULL && no_acl_syscall_error(errno)) {
					DEBUG5(("call_trans2qfilepathinfo: ACLs not implemented on filesystem containing %s\n",
						fname ));
				...
				if ( data_size < (num_file_acls + num_def_acls)*SMB_POSIX_ACL_ENTRY_SIZE + SMB_POSIX_ACL_HEADER_SIZE) {
					DEBUG5(("call_trans2qfilepathinfo: data_size too small (%u) need %u\n",
						data_size,
...
NTSTATUS hardlink_internals(connection_struct *conn, pstring oldname, pstring newname)
{
	...
	DEBUG10(("hardlink_internals: doing hard link %s -> %s\n", newname, oldname ));

	if (SMB_VFS_LINK(conn,oldname,newname) != 0) {
		status = map_nt_error_from_unix(errno);
		DEBUG3(("hardlink_internals: Error %s hard link %s -> %s\n",
				...
				struct timespec ts[2])
{
	...
	DEBUG6(("smb_set_file_time: actime: %s " , time_to_asc(convert_timespec_to_time_t(ts[0])) ));
	DEBUG6(("smb_set_file_time: modtime: %s ", time_to_asc(convert_timespec_to_time_t(ts[1])) ));
	...
	if(fsp != NULL) {
		...
		if (!null_timespec(ts[1])) {
			DEBUG10(("smb_set_file_time: setting pending modtime to %s\n",
				time_to_asc(convert_timespec_to_time_t(ts[1])) ));
			fsp_set_pending_modtime(fsp, ts[1]);
		}

	}
	DEBUG10(("smb_set_file_time: setting utimes to modified values.\n"));
				...
				uint32 dosmode)
{
	...
	DEBUG6(("smb_set_file_dosmode: dosmode: 0x%x\n", (unsigned int)dosmode));

	/* check the mode isn't different, before changing it */
	if ((dosmode != 0) && (dosmode != dos_mode(conn, fname, psbuf))) {

		DEBUG10(("smb_set_file_dosmode: file %s : setting dos mode 0x%x\n",
					fname, (unsigned int)dosmode ));

		if(file_set_dosmode(conn, fname, dosmode, psbuf, False)) {
			DEBUG2(("smb_set_file_dosmode: file_set_dosmode of %s failed (%s)\n",
						fname, strerror(errno)));
			return map_nt_error_from_unix(errno);
		}
	}
				...
				SMB_OFF_T size)
{
	...
	DEBUG6(("smb_set_file_size: size: %.0f ", (double)size));

	if (size == get_file_size(*psbuf)) {
		return NT_STATUS_OK;
	}

	DEBUG10(("smb_set_file_size: file %s : setting new size to %.0f\n",
		fname, (double)size ));

	if (fsp && fsp->fh->fd != -1) {
		/* Handle based call. */
#if (SX_USE_FAT_TRUNCATE)
		if (vfs_set_filelen(fsp, size) == -1) {
#else
		if (vfs_set_filelen2(conn,fsp, size,get_file_size(*psbuf)) == -1) {
#endif
			return map_nt_error_from_unix(errno);
		}
		return NT_STATUS_OK;
				...
				const char *fname)
{
	...
	if (IVAL(pdata,0) > total_data) {
		DEBUG10(("smb_info_set_ea: bad total data size (%u) > %u\n",
			IVAL(pdata,0), (unsigned int)total_data));
		return NT_STATUS_INVALID_PARAMETER;
	}
				...
				SMB_STRUCT_STAT *psbuf)
{
	...
	DEBUG10(("smb_set_file_disposition_info: file %s, dosmode = %u, "
		"delete_on_close = %u\n",
		fsp->fsp_name,
		(unsigned int)dosmode,
				...
				files_struct *fsp)
{
	...
	DEBUG10(("smb_file_position_information: Set file position information for file %s to %.0f\n",
				...
				const char *fname)
{
	...
	DEBUG10(("smb_set_file_unix_link: SMB_SET_FILE_UNIX_LINK doing symlink %s -> %s\n",
			newname, link_target ));
				...
				pstring fname)
{
	...
	DEBUG10(("smb_set_file_unix_hlink: SMB_SET_FILE_UNIX_LINK doing hard link %s -> %s\n",
		fname, oldname));
				...
				pstring fname)
{
	...
	if (fsp) {
		DEBUG10(("smb_file_rename_information: SMB_FILE_RENAME_INFORMATION (fnum %d) %s -> %s\n",
			fsp->fnum, fsp->fsp_name, base_name ));
		status = rename_internals_fsp(conn, fsp, base_name, 0, overwrite);
	} else {
		DEBUG10(("smb_file_rename_information: SMB_FILE_RENAME_INFORMATION %s -> %s\n",
			fname, newname ));
				...
				SMB_STRUCT_STAT *psbuf)
{
	...
	DEBUG10(("smb_set_posix_acl: file %s num_file_acls = %u, num_def_acls = %u\n",
				...
				files_struct *fsp)
{
	...
	DEBUG10(("smb_set_posix_lock: file %s, lock_type = %u,"
					...
					const SMB_STRUCT_STAT *psbuf)
{
	...
	DEBUG10(("smb_set_info_standard: file %s\n",
		fname ? fname : fsp->fsp_name ));
					...
					SMB_STRUCT_STAT *psbuf)
{
	...
	DEBUG10(("smb_set_file_basic_info: file %s\n",
		fname ? fname : fsp->fsp_name ));
					...
					SMB_STRUCT_STAT *psbuf)
{
	...
	DEBUG10(("smb_set_file_allocation_info: Set file allocation info for file %s to %.0f\n",
	...
	DEBUG10(("smb_set_file_allocation_info: file %s : setting new allocation size to %.0f\n",
			fname, (double)allocation_size ));
...
#endif /* LARGE_SMB_OFF_T */
	DEBUG10(("smb_set_file_end_of_file_info: Set end of file info for "
		"file %s to %.0f\n", fname, (double)size ));
					...
					SMB_STRUCT_STAT *psbuf)
{
	...
	DEBUG10(("smb_unix_mknod: SMB_SET_FILE_UNIX_BASIC doing mknod dev %.0f mode \
					...
					SMB_STRUCT_STAT *psbuf)
{
	...
	if(fname){
		struct statfs fs;
		if(!statfs(fname,&fs)) {
			if(fs.f_type==NTFS_SB_MAGIC||fs.f_type==MSDOS_SUPER_MAGIC||
			   fs.f_type==FUSE_SUPER_MAGIC) {
				ch_ignore = True;
			}
		}
	}
	...
	DEBUG10(("smb_set_file_unix_basic: SMB_SET_FILE_UNIX_BASIC: name = %s \
size = %.0f, uid = %u, gid = %u, raw perms = 0%o\n",
	...
	if (raw_unixmode != SMB_MODE_NO_CHANGE) {
		DEBUG10(("smb_set_file_unix_basic: SMB_SET_FILE_UNIX_BASIC setting mode 0%o for file %s\n",
			(unsigned int)unixmode, fname ));
		if (SMB_VFS_CHMOD(conn, fname, unixmode) != 0 && ch_ignore==False) {
			return map_nt_error_from_unix(errno);
		}
	}
	...
	if ((set_owner != (uid_t)SMB_UID_NO_CHANGE) && (psbuf->st_uid != set_owner)) {
		DEBUG10(("smb_set_file_unix_basic: SMB_SET_FILE_UNIX_BASIC changing owner %u for file %s\n",
			(unsigned int)set_owner, fname ));
		if (SMB_VFS_CHOWN(conn, fname, set_owner, (gid_t)-1) != 0 && ch_ignore==False) {
			status = map_nt_error_from_unix(errno);
			if (delete_on_fail) {
				SMB_VFS_UNLINK(conn,fname);
			}
	...
	if ((set_grp != (uid_t)SMB_GID_NO_CHANGE) && (psbuf->st_gid != set_grp)) {
		DEBUG10(("smb_set_file_unix_basic: SMB_SET_FILE_UNIX_BASIC changing group %u for file %s\n",
			(unsigned int)set_owner, fname ));
		if (SMB_VFS_CHOWN(conn, fname, (uid_t)-1, set_grp) != 0 && ch_ignore==False) {
			status = map_nt_error_from_unix(errno);
			if (delete_on_fail) {
				SMB_VFS_UNLINK(conn,fname);
			}
				...
				int *pdata_return_size)
{
	...
	DEBUG10(("smb_posix_mkdir: file %s, mode 0%o\n",
				...
				int *pdata_return_size)
{
	...
	switch (wire_open_mode & SMB_ACCMODE) {
		...
		case SMB_O_RDWR:
			...
			break;
		default:
			DEBUG5(("smb_posix_open: invalid open mode 0x%x\n",
	...
	if((wire_open_mode & (SMB_O_CREAT | SMB_O_EXCL)) == (SMB_O_CREAT | SMB_O_EXCL)) {
		...
		DEBUG5(("smb_posix_open: invalid create mode 0x%x\n",
			(unsigned int)wire_open_mode ));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	DEBUG10(("smb_posix_open: file %s, smb_posix_flags = %u, mode 0%o\n",
		fname,
		(unsigned int)wire_open_mode,
				...
				SMB_STRUCT_STAT *psbuf)
{
	...
	if (lck == NULL) {
		DEBUG0( ("smb_posix_unlink: Could not get share mode "
			"lock for file %s\n", fsp->fsp_name));
		close_file(fsp, NORMAL_CLOSE);
		return NT_STATUS_INVALID_PARAMETER;
	}
					...
					unsigned int max_data_bytes)
{
	...
	if (tran_call == TRANSACT2_SETFILEINFO) {
		...
		if(fsp && (fsp->is_directory || fsp->fh->fd == -1)) {
			...
			if (INFO_LEVEL_IS_UNIX(info_level)) {
				/* Always do lstat for UNIX calls. */
				if (SMB_VFS_LSTAT(conn,fname,&sbuf)) {
					DEBUG3(("call_trans2setfilepathinfo: SMB_VFS_LSTAT of %s failed (%s)\n",fname,strerror(errno)));
					return UNIXERROR(ERRDOS,ERRbadpath);
				}
			} else {
				if (SMB_VFS_STAT(conn,fname,&sbuf) != 0) {
					DEBUG3(("call_trans2setfilepathinfo: fileinfo of %s failed (%s)\n",fname,strerror(errno)));
					return UNIXERROR(ERRDOS,ERRbadpath);
				}
			}
			...
			if ((info_level == SMB_SET_FILE_DISPOSITION_INFO) && CVAL(pdata,0)) {
				...
				DEBUG3(("call_trans2setfilepathinfo: Cancelling print job (%s)\n", fsp->fsp_name ));
			...
			if (SMB_VFS_FSTAT(fsp, fsp->fh->fd, &sbuf) != 0) {
				DEBUG3(("call_trans2setfilepathinfo: fstat of fnum %d failed (%s)\n",fsp->fnum, strerror(errno)));
				return(UNIXERROR(ERRDOS,ERRbadfid));
			}
		}
	} else {
		...
		if (INFO_LEVEL_IS_UNIX(info_level)) {
			...
			DEBUG3(("call_trans2setfilepathinfo: SMB_VFS_STAT of %s failed (%s)\n",fname,strerror(errno)));
			return UNIXERROR(ERRDOS,ERRbadpath);
		}
	}
	...
	DEBUG3(("call_trans2setfilepathinfo(%d) %s (fnum %d) info_level=%d totdata=%d\n",
					...
					unsigned int max_data_bytes)
{
	...
	DEBUG3(("call_trans2mkdir : name = %s\n", directory));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG5(("call_trans2mkdir error (%s)\n", nt_errstr(status)));
		return ERROR_NT(status);
	}
	...
	if (total_data != 4) {
		...
		if (IVAL(pdata,0) > total_data) {
			DEBUG10(("call_trans2mkdir: bad total data size (%u) > %u\n",
				IVAL(pdata,0), (unsigned int)total_data));
			return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
		}
					...
					unsigned int max_data_bytes)
{
	...
	info_level = SVAL(params,4);
	DEBUG3(("call_trans2findnotifyfirst - info_level %d\n", info_level));
					...
					unsigned int max_data_bytes)
{
	...
	DEBUG3(("call_trans2findnotifynext\n"));
					...
					unsigned int max_data_bytes)
{
	...
	DEBUG10(("call_trans2getdfsreferral\n"));
		    ...
		    char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	DEBUG3(("reply_findclose, dptr_num = %d\n", dptr_num));

	dptr_close(&dptr_num);

	outsize = set_message(outbuf,0,0,False);

	DEBUG3(("SMBfindclose dptr_num = %d\n", dptr_num));
		     ...
		     char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	DEBUG3(("reply_findnclose, dptr_num = %d\n", dptr_num));
	...
	DEBUG3(("SMB_findnclose dptr_num = %d\n", dptr_num));
		  ...
		  char *inbuf, char *outbuf, int size, int bufsize)
{
	...
	default:
		/* Error in request */
		DEBUG2(("Unknown request %d in trans2 call\n", state->call));
		 ...
		 int size, int bufsize)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG2( ("Got invalid trans2 request: %s\n",
			  nt_errstr(result)));
	...
	if (state->setup_count != 1) {
		...
		if ( (state->setup_count == 4) && (tran_call == TRANSACT2_IOCTL) &&
				...
				(SVAL(inbuf,(smb_setup+6)) == LMFUNC_GETJOBID)) {
			DEBUG2(("Got Trans2 DevIOctl jobid\n"));
		} else {
			DEBUG2(("Invalid smb_sucnt in trans2 call(%u)\n",state->setup_count));
			DEBUG2(("Transaction is %d\n",tran_call));
			TALLOC_FREE(state);
			END_PROFILE(SMBtrans2);
			return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
		}
	...
	if (state->total_data) {
		...
		if (state->data == NULL) {
			DEBUG0(("reply_trans2: data malloc fail for %u "
	...
	if (state->total_param) {
		...
		if (state->param == NULL) {
			DEBUG0(("reply_trans: param malloc fail for %u "
  ...
  bad_param:
	...
	DEBUG0(("reply_trans2: invalid trans parameters\n"));
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
	END_PROFILE(SMBtrans2);
		  ...
		  char *inbuf,char *outbuf,int size,int bufsize)
{
  ...
  bad_param:
	...
	DEBUG0(("reply_transs2: invalid trans parameters\n"));
	DLIST_REMOVE(conn->pending_trans, state);
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
