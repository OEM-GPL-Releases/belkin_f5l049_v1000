...
NTSTATUS change_trust_account_password( const char *domain, const char *remote_machine)
{
	...
	DEBUG5(("change_trust_account_password: Attempting to change trust account password in domain %s....\n",
		domain));
	...
	if (remote_machine == NULL || !strcmp(remote_machine, "*")) {
		...
		if ( !get_pdc_ip(domain, &pdc_ip) ) {
			DEBUG0(("Can't get IP for PDC for domain %s\n", domain));
			goto failed;
		}
	...
	if (!NT_STATUS_IS_OK(cli_full_connection(&cli, global_myname(), dc_name, 
					   ...
					   "", "",
					   "", 0, Undefined, NULL))) {
		DEBUG0(("modify_trust_password: Connection to %s failed!\n", dc_name));
	...
	if (!netlogon_pipe) {
		DEBUG0(("modify_trust_password: unable to open the domain client session to machine %s. Error was : %s.\n", 
...
failed:
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("%s : change_trust_account_password: Failed to change password for domain %s.\n", 
			current_timestring(False), domain));
	}
	else
		DEBUG5(("change_trust_account_password: sucess!\n"));
