			  ...
			  char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	if (rmode == 2) {
		DEBUG4(("Resetting open result to open from create.\n"));
		rmode = 1;
	}
...
int reply_pipe_write(char *inbuf,char *outbuf,int length,int dum_bufsize)
{
	...
	DEBUG3(("write-IPC pnum=%04x nwritten=%d\n", p->pnum, nwritten));
...
int reply_pipe_write_and_X(char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	if (numtowrite == 0) {
		...
		if(pipe_start_message_raw) {
			 ...
			 */
	 	       if(numtowrite < 2) {
				DEBUG0(("reply_pipe_write_and_X: start of message set and not enough data sent.(%u)\n",
	...
	DEBUG3(("writeX-IPC pnum=%04x nwritten=%d\n", p->pnum, nwritten));
...
int reply_pipe_read_and_X(char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	DEBUG3(("readX-IPC pnum=%04x min=%d max=%d nread=%d\n",
...
int reply_pipe_close(connection_struct *conn, char *inbuf,char *outbuf)
{
	...
	DEBUG5(("reply_pipe_close: pnum:%x\n", p->pnum));
