...
static BOOL become_uid(uid_t uid)
{
	...
	if (uid == (uid_t)-1 || 
		...
		if (!done) {
			DEBUG1(("WARNING: using uid %d is a security risk\n",
				 (int)uid));
			done = 1;
		}
	}
...
static BOOL become_gid(gid_t gid)
{
	...
	if (gid == (gid_t)-1 || ((sizeof(gid_t) == 2) && 
		...
		if (!done) {
			DEBUG1(("WARNING: using gid %d is a security risk\n",
				 (int)gid));  
			done = 1;
		}
	}
...
static int get_current_groups(gid_t gid, int *p_ngroups, gid_t **p_groups)
{
	...
	if((groups = SMB_MALLOC_ARRAY(gid_t, ngroups+1)) == NULL) {
		DEBUG0(("setup_groups malloc fail !\n"));
		goto fail;
	}
	...
	DEBUG3( ( "get_current_groups: user is in %u groups: ", ngroups));
	for (i = 0; i < ngroups; i++ ) {
		DEBUG3( ( "%s%d", (i ? ", " : ""), (int)groups[i] ) );
	}
	DEBUG3( ( "\n" ) );
...
BOOL push_sec_ctx(void)
{
	...
	if (sec_ctx_stack_ndx == MAX_SEC_CTX_DEPTH) {
		DEBUG0( ("Security context stack overflow!\n"));
	...
	ctx_p->ut.gid = getegid();
 	...
 	DEBUG3( ("push_sec_ctx(%u, %u) : sec_ctx_stack_ndx = %d\n", 
	...
	if (ctx_p->ut.ngroups != 0) {
		if (!(ctx_p->ut.groups = SMB_MALLOC_ARRAY(gid_t, ctx_p->ut.ngroups))) {
			DEBUG0( ("Out of memory in push_sec_ctx()\n"));
...
void set_sec_ctx(uid_t uid, gid_t gid, int ngroups, gid_t *groups, NT_USER_TOKEN *token)
{
	...
	DEBUG3( ("setting sec ctx (%u, %u) - sec_ctx_stack_ndx = %d\n", 
...
BOOL pop_sec_ctx(void)
{
	...
	if (sec_ctx_stack_ndx == 0) {
		DEBUG0( ("Security context stack underflow!\n"));
	...
	DEBUG3( ("pop_sec_ctx (%u, %u) - sec_ctx_stack_ndx = %d\n", 
