...
find_again:
	...
	if (i == -1) {
                ...
                if (newsz <= oldsz) {
                        /* Integer wrap. */
		        DEBUG0(("ERROR! Out of connection structures\n"));
                        return NULL;
                }

		DEBUG4(("resizing connections bitmap from %d to %d\n",
                        oldsz, newsz));

                nbmap = bitmap_allocate(newsz);
		if (!nbmap) {
			DEBUG0(("ERROR! malloc fail.\n"));
			return NULL;
		}
	 ...
	 */
	if (i > 65534) {
		DEBUG0( ("Maximum connection limit reached\n"));
		return NULL;
	}
		    ...
		    void *private_data)
{
	...
	if (strcmp(sharename, "*") == 0) {
		DEBUG1(("Forcing close of all shares\n"));
		conn_close_all();
		return;
	}
	...
	for (conn=Connections;conn;conn=next) {
		...
		if (strequal(lp_servicename(SNUM(conn)), sharename)) {
			DEBUG1(("Forcing close of share %s cnum=%d\n",
				 sharename, conn->cnum));
