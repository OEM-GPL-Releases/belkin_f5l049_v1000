		   ...
		   const struct share_params *p)
{
	...
	if (size == (size_t)-1) {
		DEBUG0(("is_8_3: internal error push_ucs2_allocate() failed!\n"));
		goto done;
	}
...
static void cache_mangled_name( const char mangled_name[13], char *raw_name )
{
	...
	if (tdb_store_bystring(tdb_mangled_cache, mangled_name_key, data_val, TDB_REPLACE) != 0) {
		DEBUG0(("cache_mangled_name: Error storing entry %s -> %s\n", mangled_name_key, raw_name));
	} else {
		DEBUG5(("cache_mangled_name: Stored entry %s -> %s\n", mangled_name_key, raw_name));
	}
}
		     ...
		     int default_case, const struct share_params *p)
{
	...
	DEBUG5(("name_map( %s, need83 = %s, cache83 = %s)\n", OutName,
