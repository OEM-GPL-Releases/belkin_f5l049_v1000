...
static BOOL check_user_ok(connection_struct *conn, user_struct *vuser,int snum)
{
	...
	if (!readonly_share &&
		...
		readonly_share = True;
		DEBUG5(("falling back to read-only access-evaluation due to "
			 "security descriptor\n"));
	}
...
BOOL change_to_user(connection_struct *conn, uint16 vuid)
{
	...
	if (!conn) {
		DEBUG2(("change_to_user: Connection not open\n"));
		return(False);
	}
	...
	if((sec == SEC_SHARE) && (current_user.conn == conn) &&
	   (current_user.ut.uid == conn->uid)) {
		DEBUG4(("change_to_user: Skipping user change - already "
			 "user\n"));
		return(True);
	...
	if ((vuser) && !check_user_ok(conn, vuser, snum)) {
		DEBUG2(("change_to_user: SMB user %s (unix user %s, vuid %d) "
		...
		return False;
			...
			conn->user, snum)) {
		DEBUG2(("change_to_user: unix user %s "
			 "not permitted access to share %s.\n",
			 conn->user,
			 lp_servicename(snum)));
		return false;
	...
	if (conn->force_user) /* security = share sets this too */ {
		...
		DEBUG2(("change_to_user: Invalid vuid used %d in accessing "
	...
	DEBUG5(("change_to_user uid=(%d,%d) gid=(%d,%d)\n",
...
BOOL change_to_root_user(void)
{
	...
	DEBUG5(("change_to_root_user: now uid=(%d,%d) gid=(%d,%d)\n",
		(int)getuid(),(int)geteuid(),(int)getgid(),(int)getegid()));
...
static void push_conn_ctx(void)
{
	...
	if (conn_ctx_stack_ndx == MAX_SEC_CTX_DEPTH) {
		DEBUG0( ("Connection context stack overflow!\n"));
	...
	DEBUG3( ("push_conn_ctx(%u) : conn_ctx_stack_ndx = %d\n",
...
static void pop_conn_ctx(void)
{
	...
	if (conn_ctx_stack_ndx == 0) {
		DEBUG0( ("Connection context stack underflow!\n"));
