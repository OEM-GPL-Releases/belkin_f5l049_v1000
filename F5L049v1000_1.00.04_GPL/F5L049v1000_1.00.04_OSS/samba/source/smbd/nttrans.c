		    ...
		    char *params, int paramsize, char *pdata, int datasize)
{
	...
	while (params_to_send || data_to_send) {
		...
		DEBUG9(("nt_rep: params_sent_thistime = %d, data_sent_thistime = %d, useable_space = %d\n",
			params_sent_thistime, data_sent_thistime, useable_space));
		DEBUG9(("nt_rep: params_to_send = %d, data_to_send = %d, paramsize = %d, datasize = %d\n",
		...
		if(params_to_send < 0 || data_to_send < 0) {
			DEBUG0(("send_nt_replies failed sanity check pts = %d, dts = %d\n!!!",
			...
			char *inbuf, char *outbuf, int *ppnum)
{
	...
	DEBUG4(("nt_open_pipe: Opening pipe %s.\n", fname));
	...
	DEBUG3(("nt_open_pipe: Known pipe %s opening.\n", fname));
	...
	if ( !store_pipe_opendb( p ) ) {
		DEBUG3(("nt_open_pipe: failed to store %s pipe open.\n", fname));
	}
			 ...
			 char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	DEBUG5(("do_ntcreate_pipe_open: open pipe = %s\n", fname));
				...
				const char *fname)
{
	...
	DEBUG5(("reply_ntcreate_and_X_quota: fnum = %d, open name = %s\n", fsp->fnum, fsp->fsp_name));
...
{  
	...
	DEBUG10(("reply_ntcreate_and_X: flags = 0x%x, access_mask = 0x%x "
	...
	DEBUG5(("reply_ntcreate_and_X: fnum = %d, open name = %s\n", fsp->fnum, fsp->fsp_name));
				  ...
				  char **ppdata, uint32 data_count)
{
	...
	if(parameter_count < 54) {
		DEBUG0(("do_nt_transact_create_pipe - insufficient parameters (%u)\n", (unsigned int)parameter_count));
		return ERROR_DOS(ERRDOS,ERRnoaccess);
	}
	...
	DEBUG5(("do_nt_transact_create_pipe: open name = %s\n", fname));
...
static NTSTATUS set_sd(files_struct *fsp, char *data, uint32 sd_len, uint32 security_info_sent)
{
	...
	if ((mem_ctx = talloc_init("set_sd")) == NULL) {
		DEBUG0(("set_sd: talloc_init failed.\n"));
	...
	if(!sec_io_desc( "sd data", &psd, &pd, 1)) {
		DEBUG0(("set_sd: Error in unmarshalling security descriptor.\n"));
		/*
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	DEBUG5(("call_nt_transact_create\n"));
	...
	if(parameter_count < 54) {
		DEBUG0(("call_nt_transact_create - insufficient parameters (%u)\n", (unsigned int)parameter_count));
		return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
	}
	...
	if ((ea_len + sd_len > data_count) ||
		...
		DEBUG10(("call_nt_transact_create - ea_len = %u, sd_len = %u, data_count = %u\n",
	...
	if (ea_len) {
		if (!lp_ea_support(SNUM(conn))) {
			DEBUG10(("call_nt_transact_create - ea_len = %u but EA's not supported.\n",
				(unsigned int)ea_len ));
			return ERROR_NT(NT_STATUS_EAS_NOT_SUPPORTED);
		}

		if (ea_len < 10) {
			DEBUG10(("call_nt_transact_create - ea_len = %u - too small (should be more than 10)\n",
				(unsigned int)ea_len ));
			return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
		}
	}
	...
	DEBUG5(("call_nt_transact_create: open name = %s\n", fname));
		   ...
		   char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	DEBUG3(("reply_ntcancel: cancel called on mid = %d.\n", mid));
...
static NTSTATUS copy_internals(connection_struct *conn, char *oldname, char *newname, uint32 attrs)
{
	...
	DEBUG10(("copy_internals: doing file copy %s to %s\n", oldname, newname));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG3(("copy_internals: Error %s copy file %s to %s\n",
					  ...
					  uint32 max_param_count)
{
	...
	DEBUG3(("call_nt_transact_notify_change\n"));
...

	{
		...
		DEBUG3(("call_nt_transact_notify_change: notify change "
	...
	if (fsp->notify == NULL) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG10( ("change_notify_create returned %s\n",
				   nt_errstr(status)));
			return ERROR_NT(status);
		}
	}
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	DEBUG3(("nt transact rename from = %s, to = %s succeeded.\n", 
...
static size_t get_null_nt_acl(TALLOC_CTX *mem_ctx, SEC_DESC **ppsd)
{
	...
	*ppsd = make_standard_sec_desc( mem_ctx, &global_sid_World, &global_sid_World, NULL, &sd_size);
	if(!*ppsd) {
		DEBUG0(("get_null_nt_acl: Unable to malloc space for security descriptor.\n"));
		sd_size = 0;
	}
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	DEBUG3(("call_nt_transact_query_security_desc: file = %s, info_wanted = 0x%x\n", fsp->fsp_name,
	...
	if ((mem_ctx = talloc_init("call_nt_transact_query_security_desc")) == NULL) {
		DEBUG0(("call_nt_transact_query_security_desc: talloc_init failed.\n"));
		return ERROR_DOS(ERRDOS,ERRnomem);
	}
	...
	DEBUG3(("call_nt_transact_query_security_desc: sd_size = %lu.\n",(unsigned long)sd_size));
	...
	if(!sec_io_desc( "sd data", &psd, &pd, 1)) {
		DEBUG0(("call_nt_transact_query_security_desc: Error in marshalling \
security descriptor.\n"));
		/*
		 * Return access denied for want of a better error message..
		 */ 
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	DEBUG3(("call_nt_transact_set_security_desc: file = %s, sent 0x%x\n", fsp->fsp_name,
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	if (setup_count != 8) {
		DEBUG3(("call_nt_transact_ioctl: invalid setup count %d\n", setup_count));
		return ERROR_NT(NT_STATUS_NOT_SUPPORTED);
	}
	...
	DEBUG10(("call_nt_transact_ioctl: function[0x%08X] FID[0x%04X] isFSctl[0x%02X] compfilter[0x%02X]\n", 
	...
	case FSCTL_SET_SPARSE:
		...
		DEBUG10(("FSCTL_SET_SPARSE: called on FID[0x%04X](but not implemented)\n", fidnum));
	...
	case FSCTL_0x000900C0:
		...
		DEBUG10(("FSCTL_0x000900C0: called on FID[0x%04X](but not implemented)\n",fidnum));
		send_nt_replies(outbuf, bufsize, NT_STATUS_OK, NULL, 0, NULL,
				0);
		return -1;
	...
	case FSCTL_GET_REPARSE_POINT:
		...
		DEBUG10(("FSCTL_GET_REPARSE_POINT: called on FID[0x%04X](but not implemented)\n",fidnum));
	...
	case FSCTL_SET_REPARSE_POINT:
		...
		DEBUG10(("FSCTL_SET_REPARSE_POINT: called on FID[0x%04X](but not implemented)\n",fidnum));
		send_nt_replies(outbuf, bufsize, NT_STATUS_NOT_A_REPARSE_POINT,
				NULL, 0, NULL, 0);
		return -1;
	...
	case FSCTL_GET_SHADOW_COPY_DATA: /* don't know if this name is right...*/
	{
		...
		if (max_data_count < 16) {
			DEBUG0(("FSCTL_GET_SHADOW_COPY_DATA: max_data_count(%u) < 16 is invalid!\n",
				max_data_count));
			return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
		}
		...
		if (shadow_mem_ctx == NULL) {
			DEBUG0(("talloc_init(SHADOW_COPY_DATA) failed!\n"));
			return ERROR_NT(NT_STATUS_NO_MEMORY);
		}

		shadow_data = TALLOC_ZERO_P(shadow_mem_ctx,SHADOW_COPY_DATA);
		if (shadow_data == NULL) {
			DEBUG0(("TALLOC_ZERO() failed!\n"));
		...
		if (SMB_VFS_GET_SHADOW_COPY_DATA(fsp, shadow_data, labels)!=0) {
			...
			if (errno == ENOSYS) {
				DEBUG5(("FSCTL_GET_SHADOW_COPY_DATA: connectpath %s, not supported.\n", 
					conn->connectpath));
				return ERROR_NT(NT_STATUS_NOT_SUPPORTED);
			} else {
				DEBUG0(("FSCTL_GET_SHADOW_COPY_DATA: connectpath %s, failed.\n", 
					conn->connectpath));
		...
		if (max_data_count<data_count) {
			DEBUG0(("FSCTL_GET_SHADOW_COPY_DATA: max_data_count(%u) too small (%u) bytes needed!\n",
		...
		DEBUG10(("FSCTL_GET_SHADOW_COPY_DATA: %u volumes for path[%s].\n",
	...
	case FSCTL_FIND_FILES_BY_SID: /* I hope this name is right */
	{
		...
		DEBUG10(("FSCTL_FIND_FILES_BY_SID: called on FID[0x%04X]\n",fidnum));
		...
		if (!sid_to_uid(&sid, &uid)) {
			DEBUG0(("sid_to_uid: failed, sid[%s] sid_len[%lu]\n",
	...
	default:
		if (!logged_message) {
			...
			DEBUG0(("call_nt_transact_ioctl(0x%x): Currently not implemented.\n",
				 function));
		}
	}
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	if (current_user.ut.uid != 0) {
		DEBUG1(("get_user_quota: access_denied service [%s] user [%s]\n",
	...
	if (parameter_count < 4) {
		DEBUG0(("TRANSACT_GET_USER_QUOTA: requires %d >= 4 bytes parameters\n",parameter_count));
	...
	switch (level) {
		...
		case TRANSACT_GET_USER_QUOTA_FOR_SID:
			...
			if (data_count < 8) {
				DEBUG0(("TRANSACT_GET_USER_QUOTA_FOR_SID: requires %d >= %d bytes data\n",data_count,8));
			...
			if (data_count < 8+sid_len) {
				DEBUG0(("TRANSACT_GET_USER_QUOTA_FOR_SID: requires %d >= %lu bytes data\n",data_count,(unsigned long)(8+sid_len)));
				return ERROR_DOS(ERRDOS,ERRunknownlevel);				
			}
			...
			if (max_data_count < data_len) {
				DEBUG0(("TRANSACT_GET_USER_QUOTA_FOR_SID: max_data_count(%d) < data_len(%d)\n",
		...
		default:
			DEBUG0(("do_nt_transact_get_user_quota: fnum %d unknown level 0x%04hX\n",fsp->fnum,level));
			return ERROR_DOS(ERRSRV,ERRerror);
			break;
	}
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	/* access check */
	if (current_user.ut.uid != 0) {
		DEBUG1(("set_user_quota: access_denied service [%s] user [%s]\n",
			lp_servicename(SNUM(conn)),conn->user));
		return ERROR_DOS(ERRDOS,ERRnoaccess);
	}
	...
	if (parameter_count < 2) {
		DEBUG0(("TRANSACT_SET_USER_QUOTA: requires %d >= 2 bytes parameters\n",parameter_count));
		return ERROR_DOS(ERRDOS,ERRinvalidparam);
	}
	...
	if (data_count < 40) {
		DEBUG0(("TRANSACT_SET_USER_QUOTA: requires %d >= %d bytes data\n",data_count,40));
		return ERROR_DOS(ERRDOS,ERRunknownlevel);		
	}
	...
	if (data_count < 40+sid_len) {
		DEBUG0(("TRANSACT_SET_USER_QUOTA: requires %d >= %lu bytes data\n",data_count,(unsigned long)40+sid_len));
		return ERROR_DOS(ERRDOS,ERRunknownlevel);		
	}
			  ...
			  char *inbuf, char *outbuf, int size, int bufsize)
{
	...
	switch(state->call) {
		...
		default:
			/* Error in request */
			DEBUG0(("reply_nttrans: Unknown request %d in nttrans call\n",
				 state->call));
			return ERROR_DOS(ERRSRV,ERRerror);
	}
	return outsize;
			...
			char *inbuf,char *outbuf,int size,int bufsize)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG2( ("Got invalid nttrans request: %s\n", nt_errstr(result)));
		END_PROFILE(SMBnttrans);
	...
	if(CVAL(inbuf, smb_wct) != 19 + (state->setup_count/2)) {
		DEBUG2(("Invalid smb_wct %d in nttrans call (should be %d)\n",
	...
	if (state->total_data)  {
		...
		if ((state->data = (char *)SMB_MALLOC(state->total_data)) == NULL) {
			DEBUG0(("reply_nttrans: data malloc fail for %u "
	...
	if (state->total_param) {
		...
		if ((state->param = (char *)SMB_MALLOC(state->total_param)) == NULL) {
			DEBUG0(("reply_nttrans: param malloc fail for %u "
	...
	if(state->setup_count > 0) {
		DEBUG10(("reply_nttrans: state->setup_count = %d\n",
			  state->setup_count));
		state->setup = (uint16 *)TALLOC(state, state->setup_count);
		if (state->setup == NULL) {
			DEBUG0(("reply_nttrans : Out of memory\n"));
			SAFE_FREE(state->data);
  ...
  bad_param:
	...
	DEBUG0(("reply_nttrans: invalid trans parameters\n"));
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
	END_PROFILE(SMBnttrans);
		   ...
		   int size,int bufsize)
{
  ...
  bad_param:
	...
	DEBUG0(("reply_nttranss: invalid trans parameters\n"));
	DLIST_REMOVE(conn->pending_trans, state);
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
