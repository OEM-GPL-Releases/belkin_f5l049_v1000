...
static BOOL is_mangled_component(const char *name, size_t len)
{
	...
	DEBUG10(("is_mangled_component %s (len %lu) ?\n", name, (unsigned long)len));
	...
	DEBUG10(("is_mangled_component %s (len %lu) -> yes\n", name, (unsigned long)len));
...
static BOOL check_cache(char *name, size_t maxlen, const struct share_params *p)
{
	...
	if (!is_mangled(name, p)) {
		DEBUG10(("check_cache: %s -> not mangled\n", name));
		return False;
	}
	...
	if (!prefix) {
		DEBUG10(("check_cache: %s -> %08X -> not found\n", name, hash));
		return False;
	}
	...
	if (extension[0]) {
		DEBUG10(("check_cache: %s -> %s.%s\n", name, prefix, extension));
		slprintf(name, maxlen, "%s.%s", prefix, extension);
	} else {
		DEBUG10(("check_cache: %s -> %s\n", name, prefix));
...
static void name_map(fstring name, BOOL need83, BOOL cache83, int default_case, const struct share_params *p)
{
	...
	*/
	...
	DEBUG10(("name_map: %s -> %08X -> %s (cache=%d)\n", 
