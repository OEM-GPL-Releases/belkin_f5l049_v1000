...
void initialize_async_io_handler(void)
{
	...
	if (sigaction(RT_SIGNAL_AIO, &act, NULL) != 0) {
                DEBUG0(("Failed to setup RT_SIGNAL_AIO handler\n"));
        }
			     ...
			     size_t smb_maxcnt)
{
	...
	if (!min_aio_read_size || (smb_maxcnt < min_aio_read_size)) {
		...
		DEBUG10(("schedule_aio_read_and_X: read size (%u) too small "
	...
	if (outstanding_aio_calls >= AIO_PENDING_SIZE) {
		DEBUG10(("schedule_aio_read_and_X: Already have %d aio "
	...
	if ((aio_ex = create_aio_ex_read(fsp, bufsize,
		...
		DEBUG10(("schedule_aio_read_and_X: malloc fail.\n"));
		return False;
	}
	...
	if (SMB_VFS_AIO_READ(fsp,a) == -1) {
		DEBUG0(("schedule_aio_read_and_X: aio_read failed. "
			 "Error %s\n", strerror(errno) ));
	...
	DEBUG10(("schedule_aio_read_and_X: scheduled aio_read for file %s, "
				...
				size_t numtowrite)
{
	...
	if (!min_aio_write_size || (numtowrite < min_aio_write_size)) {
		...
		DEBUG10(("schedule_aio_write_and_X: write size (%u) too "
	...
	if (outstanding_aio_calls >= AIO_PENDING_SIZE) {
		DEBUG3(("schedule_aio_write_and_X: Already have %d aio "
			 "activities outstanding.\n",
			  outstanding_aio_calls ));
		DEBUG10(("schedule_aio_write_and_X: failed to schedule "
	...
	if (!(aio_ex = create_aio_ex_write(fsp, inbufsize, outbufsize,
					   SVAL(inbuf,smb_mid)))) {
		DEBUG0(("schedule_aio_write_and_X: malloc fail.\n"));
		return False;
	}
	...
	if (SMB_VFS_AIO_WRITE(fsp,a) == -1) {
		DEBUG3(("schedule_aio_wrote_and_X: aio_write failed. "
			 "Error %s\n", strerror(errno) ));
		delete_aio_ex(aio_ex);
		unbecome_root();
		return False;
	...
	if (!write_through && !lp_syncalways(SNUM(fsp->conn))
		...
		DEBUG10(("schedule_aio_write_and_X: scheduled aio_write "
			  "behind for file %s\n", fsp->fsp_name ));
	} else {
		srv_defer_sign_response(aio_ex->mid);
	}
	...
	DEBUG10(("schedule_aio_write_and_X: scheduled aio_write for file "
...
static int handle_aio_read_complete(struct aio_extra *aio_ex)
{
	...
	if (nread < 0) {
		...
		DEBUG3(( "handle_aio_read_complete: file %s nread == -1. "
			   "Error = %s\n",
		...
		DEBUG3( ( "handle_aio_read_complete file %s max=%d "
			    "nread=%d\n",
			    aio_ex->fsp->fsp_name,
	...
	DEBUG10(("handle_aio_read_complete: scheduled aio_read completed "
...
static int handle_aio_write_complete(struct aio_extra *aio_ex)
{
	...
	if (fsp->aio_write_behind) {
		if (nwritten != numtowrite) {
			if (nwritten == -1) {
				DEBUG5(("handle_aio_write_complete: "
					 "aio_write_behind failed ! File %s "
					 "is corrupt ! Error %s\n",
					 fsp->fsp_name, strerror(errno) ));
				ret = errno;
			} else {
				DEBUG0(("handle_aio_write_complete: "
					 "aio_write_behind failed ! File %s "
			...
			DEBUG10(("handle_aio_write_complete: "
	...
	if(nwritten == -1) {
		DEBUG3(( "handle_aio_write: file %s wanted %u bytes. "
			   "nwritten == %d. Error = %s\n",
		...
		DEBUG3(("handle_aio_write: fnum=%d num=%d wrote=%d\n",
			 fsp->fnum, (int)numtowrite, (int)nwritten));
		status = sync_file(fsp->conn,fsp, write_through);
		if (!NT_STATUS_IS_OK(status)) {
			UNIXERROR(ERRHRD,ERRdiskfull);
			ret = errno;
                	DEBUG5(("handle_aio_write: sync_file for %s returned %s\n",
	...
	DEBUG10(("handle_aio_write_complete: scheduled aio_write completed "
...
static BOOL handle_aio_completed(struct aio_extra *aio_ex, int *perr)
{
	...
	if (SMB_VFS_AIO_ERROR(aio_ex->fsp, &aio_ex->acb) == EINPROGRESS) {
		DEBUG10(( "handle_aio_completed: operation mid %u still in "
			   "process for file %s\n",
...
int process_aio_queue(void)
{
	...
	DEBUG10(("process_aio_queue: signals_received = %d\n",
		  (int)signals_received));
	DEBUG10(("process_aio_queue: outstanding_aio_calls = %d\n",
		  outstanding_aio_calls));
	...
	for (i = 0; i < signals_received; i++) {
		...
		if (!aio_ex) {
			DEBUG3(("process_aio_queue: Can't find record to "
				 "match mid %u.\n", (unsigned int)mid));
		...
		if (fsp == NULL) {
			 ...
			 * ignore. */
			DEBUG3(( "process_aio_queue: file closed whilst "
				   "aio outstanding.\n"));
			srv_cancel_sign_response(mid);
			continue;
		}
...
int wait_for_aio_completion(files_struct *fsp)
{
	...
	for (seconds_left = SMB_TIME_FOR_AIO_COMPLETE_WAIT;
		...
		DEBUG3(("wait_for_aio_completion: waiting for %d aio events "
			 "to complete.\n", aio_completion_count ));
		...
		DEBUG10(("wait_for_aio_completion: %d events, doing a wait "
			  "of %d seconds.\n",
		...
		DEBUG10(("wait_for_aio_completion: returned err = %d, "
			  "errno = %s\n", err, strerror(errno) ));
		
		if (err == -1 && errno == EAGAIN) {
			DEBUG0(("wait_for_aio_completion: aio_suspend timed "
		...
		for( i = 0; i < aio_completion_count; i++) {
			...
			if (!aio_ex) {
				DEBUG0( ("wait_for_aio_completion: mid %u "
	 ...
	 * else EIO. */
	DEBUG10(("wait_for_aio_completion: aio_suspend timed out waiting "
		  "for %d events\n",
		  aio_completion_count));
