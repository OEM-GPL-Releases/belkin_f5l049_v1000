				...
				const char *user, const char *domain)
{
	if (NT_STATUS_EQUAL(status, NT_STATUS_NO_SUCH_USER)) {
		if ((lp_map_to_guest() == MAP_TO_GUEST_ON_BAD_USER) || 
			...
			DEBUG3(("No such user %s [%s] - using guest account\n",
				 user, domain));
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_WRONG_PASSWORD)) {
		if (lp_map_to_guest() == MAP_TO_GUEST_ON_BAD_PASSWORD) {
			DEBUG3(("Registered username %s for guest access\n",user));
			status = make_server_info_guest(server_info);
		}
	}
...
static BOOL make_krb5_skew_error(DATA_BLOB *pblob_out)
{
	...
	*pblob_out = data_blob(NULL,0);
	...
	if (kerr) {
		DEBUG10(("make_krb5_skew_error: smb_krb5_parse_name failed for name %s: Error %s\n",
	...
	if (kerr) {
		DEBUG10(("make_krb5_skew_error: smb_krb5_mk_error failed: Error %s\n",
			error_message(kerr) ));
		goto out;
	}
				 ...
				 BOOL *p_invalidate_vuid)
{
...
#endif
		DEBUG1(("Failed to verify incoming ticket with error %s!\n", nt_errstr(ret)));	
		talloc_destroy(mem_ctx);
		return ERROR_NT(nt_status_squash(ret));
	}

	DEBUG3(("Ticket name is [%s]\n", client));

	p = strchr_m(client, '@');
	if (!p) {
		DEBUG3(("Doesn't look like a valid principal\n"));
		data_blob_free(&ap_rep);
	...
	*p = 0;
	...
	if (!strequal(p+1, lp_realm())) {
		DEBUG3(("Ticket for foreign realm %s@%s\n", client, p+1));
	...
	if (logon_info && logon_info->info3.hdr_logon_dom.uni_str_len) {
		...
		DEBUG10( ("Mapping [%s] to short name\n", domain));
		...
		if (wb_result == NSS_STATUS_SUCCESS) {
			...
			DEBUG10( ("Mapped to [%s] (using Winbind)\n", domain));
		} else {
			DEBUG3( ("Could not find short name -- winbind "
				  "not running?\n"));
		}
	}
	...
	if (pw) {
		...
		if (  !NT_STATUS_IS_OK(ret)) {
			DEBUG1( ("PAM account restriction prevents user login\n"));
			data_blob_free(&ap_rep);
			data_blob_free(&session_key);
			TALLOC_FREE(mem_ctx);
			return ERROR_NT(nt_status_squash(ret));
	...
	if (!pw) {
		...
		if ( !pw ) {
			DEBUG1(("Username %s is invalid on this system\n", user));
			SAFE_FREE(client);
			data_blob_free(&ap_rep);
			data_blob_free(&session_key);
			TALLOC_FREE(mem_ctx);
	...
	if ( map_domainuser_to_guest ) {
		...
		if ( !NT_STATUS_IS_OK(ret) ) {
			DEBUG1(("make_server_info_info3 failed: %s!\n",
				 nt_errstr(ret)));
			SAFE_FREE(client);
			data_blob_free(&ap_rep);
			data_blob_free(&session_key);
		...
		if ( !NT_STATUS_IS_OK(ret) ) {
			DEBUG1(("make_server_info_pw failed: %s!\n",
				 nt_errstr(ret)));
			SAFE_FREE(client);
			data_blob_free(&ap_rep);
			data_blob_free(&session_key);
...
static NTSTATUS parse_spnego_mechanisms(DATA_BLOB blob_in, DATA_BLOB *pblob_out, BOOL *p_is_krb5)
{
	...
	*p_is_krb5 = False;
	...
	for (i=0;OIDs[i];i++) {
		DEBUG5(("parse_spnego_mechanisms: Got OID %s\n", OIDs[i]));
		free(OIDs[i]);
	}
	return NT_STATUS_OK;
}
				  ...
				  AUTH_NTLMSSP_STATE **auth_ntlmssp_state)
{
	...
	DEBUG3(("reply_spnego_negotiate: Got secblob of size %lu\n", (unsigned long)secblob.length));
	...
	if (got_kerberos_mechanism) {
		invalidate_intermediate_vuid(vuid);
		DEBUG3(("reply_spnego_negotiate: network "
			     ...
			     AUTH_NTLMSSP_STATE **auth_ntlmssp_state)
{
	...
	if (auth.data[0] == ASN1_APPLICATION(0)) {
		...
		if (NT_STATUS_IS_OK(status)) {
			DEBUG3(("reply_spnego_auth: Got secblob of size %lu\n", (unsigned long)secblob.length));
#ifdef HAVE_KRB5
...
static NTSTATUS check_spnego_blob_complete(uint16 smbpid, uint16 vuid, DATA_BLOB *pblob)
{
	...
	if (pblob->length == 0) {
		/* Caller can cope. */
		DEBUG2(("check_spnego_blob_complete: zero blob length !\n"));
	...
	if (pad) {
		...
		if (pad->partial_data.length + copy_len < pad->partial_data.length ||
			...
			DEBUG2(("check_spnego_blob_complete: integer wrap "
		...
		DEBUG10(("check_spnego_blob_complete: "
			"pad->partial_data.length = %u, "
			"pad->needed_len = %u, "
			"copy_len = %u, "
			"pblob->length = %u,\n",
	...
	DEBUG10(("check_spnego_blob_complete: needed_len = %u, "
		"pblob->length = %u\n",
	...
	if (needed_len > 65536) {
		DEBUG2(("check_spnego_blob_complete: needed_len too large (%u)\n",
			(unsigned int)needed_len ));
		return NT_STATUS_INVALID_PARAMETER;
	}
					...
					int length,int bufsize)
{
	...
	DEBUG3(("Doing spnego session setup\n"));
	...
	DEBUG3(("NativeOS=[%s] NativeLanMan=[%s] PrimaryDomain=[%s]\n", 
	...
	if (!vuser) {
		...
		if (pad) {
			DEBUG10(("reply_sesssetup_and_X_spnego: found pending vuid %u\n",
	...
	DEBUG1(("Unknown packet in reply_sesssetup_and_X_spnego\n"));
...
{
	DEBUG2(("setup_new_vc_session: New VC == 0, if NT4.x compatible we would close all old resources.\n"));
#if 0
	conn_close_all();
			  ...
			  int length,int bufsize)
{
	...
	if (CVAL(inbuf, smb_wct) == 12 &&
		...
		if (!global_spnego_negotiated) {
			DEBUG0(("reply_sesssetup_and_X:  Rejecting attempt at SPNEGO session setup when it was not negoitiated.\n"));
			return ERROR_NT(nt_status_squash(NT_STATUS_LOGON_FAILURE));
		}
	...
	if (Protocol < PROTOCOL_NT1) {
		...
		*domain = 0;
		...
		DEBUG3(("Domain=[%s]  NativeOS=[%s] NativeLanMan=[%s] PrimaryDomain=[%s]\n",
	...
	if (*user) {
		if (global_spnego_negotiated) {
			...
			DEBUG0(("reply_sesssetup_and_X:  Rejecting attempt at 'normal' session setup after negotiating spnego.\n"));
			return ERROR_NT(nt_status_squash(NT_STATUS_LOGON_FAILURE));
		}
	...
	if (!*user) {
		...
		if (!negprot_global_auth_context) {
			DEBUG0( ("reply_sesssetup_and_X:  Attempted encrypted session setup without negprot denied!\n"));
			return ERROR_NT(nt_status_squash(NT_STATUS_LOGON_FAILURE));
		}
