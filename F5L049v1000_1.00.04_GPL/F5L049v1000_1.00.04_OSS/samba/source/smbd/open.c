		    ...
		    mode_t mode)
{
	...
	DEBUG10(("fd_open: name %s, flags = 0%o mode = 0%o, fd = %d. %s\n",
					...
					files_struct *fsp)
{
	...
	if (ret == -1) {
		DEBUG0(("change_file_owner_to_parent: failed to stat parent "
	...
	unbecome_root();
	if (ret == -1) {
		DEBUG0(("change_file_owner_to_parent: failed to fchown "
	...
	DEBUG10(("change_file_owner_to_parent: changed new file %s to "
				       ...
				       SMB_STRUCT_STAT *psbuf)
{
	...
	ret = SMB_VFS_STAT(conn, inherit_from_dir, &parent_st);
	if (ret == -1) {
		DEBUG0(("change_dir_owner_to_parent: failed to stat parent "
			 "directory %s. Error was %s\n",
			 inherit_from_dir, strerror(errno) ));
		return;
	}
	...
	*/
	...
	if (!vfs_GetWd(conn,saved_dir)) {
		DEBUG0(("change_dir_owner_to_parent: failed to get "
	...
	if (vfs_ChDir(conn, fname) == -1) {
		DEBUG0(("change_dir_owner_to_parent: failed to change "
	...
	if (SMB_VFS_STAT(conn,".",&sbuf) == -1) {
		DEBUG0(("change_dir_owner_to_parent: failed to stat "
			 "directory '.' (%s) Error was %s\n",
			 fname, strerror(errno)));
		goto out;
	}
	...
	if (sbuf.st_dev != psbuf->st_dev ||
		...
		DEBUG0(("change_dir_owner_to_parent: "
	...
	unbecome_root();
	if (ret == -1) {
		DEBUG10(("change_dir_owner_to_parent: failed to chown "
	...
	DEBUG10(("change_dir_owner_to_parent: changed ownership of new "
		  "directory %s to parent directory uid %u.\n",
			  ...
			  uint32 open_access_mask) /* what we're actually using in the open. */
{
	...
	if (!CAN_WRITE(conn)) {
		...
		if(accmode != O_RDONLY) {
			DEBUG3(("Permission denied opening %s\n", path));
	...
	if ((accmode == O_RDONLY) && ((flags & O_TRUNC) == O_TRUNC)) {
		DEBUG10(("open_file: truncate requested on read-only open "
			  "for file %s\n", path));
	...
	if ((open_access_mask & (FILE_READ_DATA|FILE_WRITE_DATA|FILE_APPEND_DATA|FILE_EXECUTE)) ||
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG3(("Error opening file %s (%s) (local_flags=%d) "
				 "(flags=%d)\n",
	...
	if (!file_existed) {
		...
		if (fsp->fh->fd == -1) {
			...
			if (ret == -1) {
				DEBUG0(("Error doing fstat on open file %s "
					 "(%s)\n", path,strerror(errno) ));
			}
		}
	...
	DEBUG2(("%s opened file %s read=%s write=%s (numopen=%d)\n",
...
static BOOL share_conflict(struct share_mode_entry *entry,
			   ...
			   uint32 share_access)
{
	DEBUG10(("share_conflict: entry->access_mask = 0x%x, "
	...
	DEBUG10(("share_conflict: access_mask = 0x%x, share_access = 0x%x\n",
	...
	if ((entry->access_mask & (FILE_WRITE_DATA|
				   ...
				   FILE_EXECUTE|
				   DELETE_ACCESS)) == 0) {
		DEBUG10(("share_conflict: No conflict due to "
...
#define CHECK_MASK(num, am, right, sa, share) \
	DEBUG10(("share_conflict: [%d] am (0x%x) & right (0x%x) = 0x%x\n", \
		(unsigned int)(num), (unsigned int)(am), \
		(unsigned int)(right), (unsigned int)(am)&(right) )); \
	DEBUG10(("share_conflict: [%d] sa (0x%x) & share (0x%x) = 0x%x\n", \
		(unsigned int)(num), (unsigned int)(sa), \
		(unsigned int)(share), (unsigned int)(sa)&(share) )); \
	if (((am) & (right)) && !((sa) & (share))) { \
		DEBUG10(("share_conflict: check %d conflict am = 0x%x, right = 0x%x, \
...
sa = 0x%x, share = 0x%x\n", (num), (unsigned int)(am), (unsigned int)(right), (unsigned int)(sa), \
	...
	DEBUG10(("share_conflict: No conflict.\n"));
	return False;
}
				      ...
				      struct share_mode_entry *share_entry)
{
	...
	if (!fsp) {
		DEBUG0(("validate_my_share_entries: PANIC : %s\n",
			 share_mode_str(num, share_entry) ));
			      ...
			      int oplock_request)
{
	...
	DEBUG10( ("Sending break request to PID %s\n",
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG3( ("Could not send oplock break message: %s\n",
			  nt_errstr(status)));
	}
		       ...
		       struct deferred_open_record *state)
{
	...
	for (i=0; i<lck->num_share_modes; i++) {
		...
		if (procid_is_me(&e->pid) && (e->op_mid == mid)) {
			DEBUG0( ("Trying to defer an already deferred "
	...
	DEBUG10(("defer_open_sharing_error: time [%u.%06u] adding deferred "
				  ...
				  mode_t *returned_unx_mode)
{
	...
	DEBUG10(("open_match_attributes: file %s old_dos_attr = 0x%x, "
				     ...
				     uint32 create_options)
{
	...
	DEBUG5(("fcb_or_dos_open: attempting old open semantics for "
		 "file %s.\n", fname ));

	for(fsp = file_find_di_first(dev, inode); fsp;
	    fsp = file_find_di_next(fsp)) {

		DEBUG10(("fcb_or_dos_open: checking file %s, fd = %d, "
		...
		if (fsp->fh->fd != -1 &&
			...
			DEBUG10(("fcb_or_dos_open: file match\n"));
			break;
		}
	}
	...
	if (is_executable(fname) &&
	    (fsp->fh->private_options & NTCREATEX_OPTIONS_PRIVATE_DENY_DOS)) {
		DEBUG10(("fcb_or_dos_open: file fail due to is_executable.\n"));
		return NULL;
	}
				 ...
				 uint32 *pcreate_options)
{
	...
	DEBUG10(("map_open_params_to_ntcreate: fname = %s, deny_mode = 0x%x, "
		  "open_func = 0x%x\n",
	...
	switch (GET_OPENX_MODE(deny_mode)) {
		...
		case DOS_OPEN_FCB:
			...
			break;
		default:
			DEBUG10(("map_open_params_to_ntcreate: bad open mode = 0x%x\n",
	...
	switch (open_func) {
		...
		default:
			...
			DEBUG10(("map_open_params_to_ntcreate: bad "
	...
	switch (GET_DENY_MODE(deny_mode)) {
		...
		default:
			DEBUG10(("map_open_params_to_ntcreate: bad deny_mode 0x%x\n",
				(unsigned int)GET_DENY_MODE(deny_mode) ));
			return False;
	}

	DEBUG10(("map_open_params_to_ntcreate: file %s, access_mask = 0x%x, "
			    ...
			    files_struct **result)
{
	...
	if (conn->printer) {
		...
		DEBUG10( ("open_file_ntcreate: printer open fname=%s\n", fname));
	...
	DEBUG10( ("open_file_ntcreate: fname=%s, dos_attrs=0x%x "
	...
	if ((pml = get_open_deferred_message(mid)) != NULL) {
		...
		if (lck == NULL) {
			DEBUG0( ("could not get share mode lock\n"));
		} else {
	...
	if (!lp_posix_pathnames() && strstr(fname,".+,;=[].")) {
		 ...
		 * release. */
		DEBUG5(("open_file_ntcreate: OS/2 long filenames are not "
			 "supported.\n"));
		if (use_nt_status()) {
	...
	switch( create_disposition ) {
		...
		case FILE_SUPERSEDE:
			...
			if (!NT_STATUS_IS_OK(check_vetoname(pnewname))) flags2 &= ~O_CREAT;
			break;
		...
		case FILE_OPEN:
			...
			if (!file_existed) {
				DEBUG5(("open_file_ntcreate: FILE_OPEN "
		...
		case FILE_OVERWRITE:
			 ...
			 * error. */
			if (!file_existed) {
				DEBUG5(("open_file_ntcreate: FILE_OVERWRITE "
					 "requested for file %s and file "
					 "doesn't exist.\n", fname ));
				errno = ENOENT;
				return NT_STATUS_OBJECT_NAME_NOT_FOUND;
		...
		case FILE_CREATE:
			 ...
			 * create. */
			if (file_existed) {
				DEBUG5(("open_file_ntcreate: FILE_CREATE "
					 "requested for file %s and file "
					 "already exists.\n", fname ));
				if (S_ISDIR(psbuf->st_mode)) {
					errno = EISDIR;
	...
	if (!posix_open && file_existed && ((create_disposition == FILE_OVERWRITE) ||
		...
		if (!open_match_attributes(conn, fname,
			...
			DEBUG5(("open_file_ntcreate: attributes missmatch "
	...
	DEBUG10( ("open_file_ntcreate: fname=%s, after mapping "
		   "access_mask=0x%x\n", fname, access_mask ));
	...
	if (flags != O_RDONLY && file_existed &&
		...
		DEBUG5(("open_file_ntcreate: write access requested for "
	...
	DEBUG4(("calling open_file with flags=0x%X flags2=0x%X mode=0%o, "
	...
	if (!file_existed) {
		...
		if (lck == NULL) {
			DEBUG0( ("open_file_ntcreate: Could not get share "
				  "mode lock for %s\n", fname));
			fd_close(conn, fsp);
			file_free(fsp);
			return NT_STATUS_SHARING_VIOLATION;
	...
	if (!posix_open && !file_existed && !def_acl) {
...

		{
			...
			if (ret == -1 && errno == ENOSYS) {
				...
				DEBUG5( ("open_file_ntcreate: reset "
		...
		if ((ret == -1) &&
			...
			DEBUG5( ("open_file_ntcreate: failed to reset "
				  "attributes of file %s to 0%o\n",
				  fname, (unsigned int)new_unx_mode));
	}
				...
				SMB_STRUCT_STAT *psbuf)
{
	...
	if(!CAN_WRITE(conn)) {
		DEBUG5(("mkdir_internal: failing create on read-only share "
	...
	if (SMB_VFS_LSTAT(conn, name, psbuf) == -1) {
		DEBUG2( ("Could not stat directory '%s' just created: %s\n",
			  name, strerror(errno)));
		return map_nt_error_from_unix(errno);
	}

	if (!S_ISDIR(psbuf->st_mode)) {
		DEBUG0( ("Directory just '%s' created is not a directory\n",
			  name));
		return NT_STATUS_ACCESS_DENIED;
	}
			...
			files_struct **result)
{
	...
	DEBUG5(("open_directory: opening directory %s, access_mask = 0x%x, "
	...
	if (is_ntfs_stream_name(fname)) {
		DEBUG0(("open_directory: %s is a stream name!\n", fname ));
	...
	switch( create_disposition ) {
		...
		case FILE_CREATE:
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG2( ("open_directory: unable to create "
					  "%s. Error was %s\n", fname,
					  nt_errstr(status)));
				return status;
			}
		...
		case FILE_OVERWRITE_IF:
		default:
			DEBUG5(("open_directory: invalid create_disposition "
	...
	if(!S_ISDIR(psbuf->st_mode)) {
		DEBUG5(("open_directory: %s is not a directory !\n",
			 fname ));
		return NT_STATUS_NOT_A_DIRECTORY;
	}
	...
	if (lck == NULL) {
		DEBUG0( ("open_directory: Could not get share mode lock for %s\n", fname));
		file_free(fsp);
		return NT_STATUS_SHARING_VIOLATION;
	}
			...
			SMB_STRUCT_STAT *psbuf, files_struct **result)
{
	...
	DEBUG5(("open_file_stat: 'opening' file %s\n", fname));
			  ...
			  void *buf, size_t len, void *private_data)
{
	...
	if (buf == NULL || len < MSG_FILE_RENAMED_MIN_SIZE + 2) {
                DEBUG0( ("msg_file_was_renamed: Got invalid msg len %d\n", (int)len));
                return;
        }
	...
	DEBUG10(("msg_file_was_renamed: Got rename message for sharepath %s, new name %s, "
		"dev %x, inode  %.0f\n",
		sharepath, newname, (unsigned int)dev, (double)inode ));

	for(fsp = file_find_di_first(dev, inode); fsp; fsp = file_find_di_next(fsp)) {
		if (memcmp(fsp->conn->connectpath, sharepath, sp_len) == 0) {
	                DEBUG10(("msg_file_was_renamed: renaming file fnum %d from %s -> %s\n",
			...
			/* Now we have the complete path we can work out if this is
			   actually within this share and adjust newname accordingly. */
	                DEBUG10(("msg_file_was_renamed: share mismatch (sharepath %s "
				"not sharepath %s) "
				"fnum %d from %s -> %s\n",
