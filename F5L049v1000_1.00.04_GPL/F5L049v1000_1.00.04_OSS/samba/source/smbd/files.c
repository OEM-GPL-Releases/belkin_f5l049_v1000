...
NTSTATUS file_new(connection_struct *conn, files_struct **result)
{
	...
	if (i == -1) {
		DEBUG0(("ERROR! Out of file structures\n"));
	...
	DEBUG5(("allocated file structure %d, fnum = %d (%d used)\n",
...
void file_init(void)
{
	...
	if(real_max_open_files != request_max_open_files) {
		DEBUG1(("file_init: Information only: requested %d \
...
void file_dump_open_table(void)
{
	...
	for (fsp=Files;fsp;fsp=fsp->next,count++) {
		DEBUG10(("Files[%d], fnum = %d, name %s, fd = %d, fileid = %lu, dev = %x, inode = %.0f\n",
	...
	for (fsp=Files;fsp;fsp=fsp->next,count++) {
		...
		if (fsp->dev == dev && 
			...
			if ((fsp->fh->fd == -1) &&
				...
				DEBUG0(("file_find_dif: file %s dev = %x, inode = %.0f, file_id = %u \
...
void file_free(files_struct *fsp)
{
	...
	DEBUG5(("freed files structure %d (%d used)\n",
		 fsp->fnum, files_used));
