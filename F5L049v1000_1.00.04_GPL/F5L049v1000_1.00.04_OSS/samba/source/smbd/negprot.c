...
static void get_challenge(char buff[8]) 
{
	...
	if (negprot_global_auth_context) {
		DEBUG3( ("get challenge: is this a secondary negprot?  negprot_global_auth_context is non-NULL!\n"));
		(negprot_global_auth_context->free)(&negprot_global_auth_context);
	}

	DEBUG10( ("get challenge: creating negprot_global_auth_context\n"));
	if (!NT_STATUS_IS_OK(nt_status = make_auth_context_subsystem(&negprot_global_auth_context))) {
		DEBUG0( ("make_auth_context_subsystem returned %s", nt_errstr(nt_status)));
		smb_panic("cannot make_negprot_global_auth_context!\n");
	}
	DEBUG10( ("get challenge: getting challenge\n"));
...
static int reply_nt1(char *inbuf, char *outbuf)
{
	...
	if (lp_server_signing()) {
			...
			DEBUG0(("reply_nt1: smb signing is incompatible with share level security !\n"));
			if (lp_server_signing() == Required) {
		  ...
		  int dum_buffsize)
{
	...
	if(choice != -1) {
		...
		DEBUG3(("Selected protocol %s\n",supported_protocols[protocol].proto_name));
	} else {
		DEBUG0(("No protocol supported !\n"));
	}
	SSVAL(outbuf,smb_vwv0,choice);
  
	DEBUG5( ( "negprot index=%d\n", choice ) );
