			...
			SMB_STRUCT_STAT *pst)
{
	...
	*dirpath = 0;
	...
	DEBUG5( ("unix_convert called on file \"%s\"\n", name));
	...
	if (SMB_VFS_STAT(conn,name,&st) == 0) {
		...
		DEBUG5(("conversion finished %s -> %s\n",orig_path, name));
		*pst = st;
		return NT_STATUS_OK;
	}

	DEBUG5(("unix_convert begin: name = %s, dirpath = %s, start = %s\n", name, dirpath, start));
			...
			*rest = 0;
			...
			if (name_has_wildcard || 
				...
				if (end) {
					 ...
					 */
					DEBUG5(("Intermediate not found %s\n",start));
					*end = '/';
...
NTSTATUS check_name(connection_struct *conn, const pstring name)
{
	if (IS_VETO_PATH(conn, name))  {
		...
		if (!((name[0] == '.') && (!name[1] || (name[1] == '.' && !name[2])))) {
			DEBUG5(("check_name: file path name %s vetoed\n",name));
	...
	if (!lp_widelinks(SNUM(conn)) || !lp_symlinks(SNUM(conn))) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG5(("check_name: name %s failed with %s\n",name, nt_errstr(status)));
			return status;
		}
	}
...
/****************************************************************************
 silex code
****************************************************************************/
NTSTATUS check_vetoname(const pstring name)
{
#if 0
	int i,j;
	const char *p=(const char *)name;
	const char veto_name[6][5] = {"COM#","LPT#","CON","AUX","PRN","NUL"};
	const char veto_char[] = "/:*?<>|\\";

	/* check veto character */
	for( i=0; p[i]!='\0'; i++ ) {
		for( j=0; veto_char[j]!='\0'; j++ ) {
			if(p[i]==veto_char[j])
	...
	for( i=0; i<6; i++ ) {
		for( j=0; j<5; j++ ) {
			if( name[j]=='.'&&veto_name[i][j]=='\0' ) {
				return NT_STATUS_ACCESS_DENIED;
			}
			if( name[j]=='\0'&&veto_name[i][j]=='\0' ) {
				return NT_STATUS_ACCESS_DENIED;
			}
			if( name[j]==veto_name[i][j]||name[j]==(veto_name[i][j]+0x20)  ) {
				continue;
			}
			if( veto_name[i][j]=='#'&&(name[j]>='1'&&name[j]<='9') ) {
				continue;
			}
			break;
		}
...
static BOOL scan_directory(connection_struct *conn, const char *path, char *name, size_t maxlength)
{
	...
	if (!(cur_dir = OpenDir(conn, path, NULL, 0))) {
		DEBUG3(("scan dir didn't open dir [%s]\n",path));
		return(False);
	}
