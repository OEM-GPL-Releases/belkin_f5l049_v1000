                              ...
                              SMB_BIG_UINT *bsize,SMB_BIG_UINT *dfree,SMB_BIG_UINT *dsize)
{
	...
	if (dfree_command && *dfree_command) {
		...
		if (sys_fsusage(path, dfree, dsize) != 0) {
			DEBUG (0, ("disk_free: sys_fsusage() failed. Error was : %s(%s)\n",
				strerror(errno),path ));
			return (SMB_BIG_UINT)-1;
		}
	}
	...
	if (*bsize < 256) {
		DEBUG5(("disk_free:Warning: bsize == %d < 256 . Changing to assumed correct bsize = 512\n",(int)*bsize));
		*bsize = 512;
	}
	...
	if ((*dsize)<1) {
		static int done;
		if (!done) {
			DEBUG0(("WARNING: dfree is broken on this system\n"));
			done=1;
		}
