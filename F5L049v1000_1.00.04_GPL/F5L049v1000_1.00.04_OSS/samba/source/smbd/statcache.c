...
void stat_cache_add( const char *full_orig_name, const char *orig_translated_path, BOOL case_sensitive)
{
	...
	if (original_path_length != translated_path_length) {
		if (original_path_length < translated_path_length) {
			DEBUG0( ("OOPS - tried to store stat cache entry for weird length paths [%s] %lu and [%s] %lu)!\n",
	...
	if (tdb_store_bystring(tdb_stat_cache, original_path, data_val, TDB_REPLACE) != 0) {
		DEBUG0(("stat_cache_add: Error storing entry %s -> %s\n", original_path, translated_path));
	} else {
		DEBUG5(("stat_cache_add: Added entry (%lx:size%x) %s -> %s\n",
		       ...
		       char **start, SMB_STRUCT_STAT *pst)
{
	...
	*start = name;
	...
	if (conn->case_sensitive) {
		...
		if (!chk_name) {
			DEBUG0( ("stat_cache_lookup: strdup failed!\n"));
			return False;
		}
		...
		if (!chk_name) {
			DEBUG0( ("stat_cache_lookup: strdup_upper failed!\n"));
			return False;
		}
	...
	while (1) {
		...
		if(data_val.dptr == NULL || data_val.dsize == 0) {
			DEBUG10(("stat_cache_lookup: lookup failed for name [%s]\n", chk_name ));
			/*
			...
			DEBUG10(("stat_cache_lookup: lookup succeeded for name [%s] -> [%s]\n", chk_name, translated_path ));
...
void stat_cache_delete(const char *name)
{
	...
	DEBUG10(("stat_cache_delete: deleting name [%s] -> %s\n",
			lname, name ));
