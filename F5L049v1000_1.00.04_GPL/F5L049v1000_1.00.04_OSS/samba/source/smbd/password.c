		  ...
		  const char *smb_name)
{
	...
	if((vuser = SMB_MALLOC_P(user_struct)) == NULL) {
		DEBUG0(("Failed to malloc users struct!\n"));
		data_blob_free(&session_key);
		TALLOC_FREE(server_info);
		return UID_FIELD_INVALID;
	}
	...
	DEBUG10(("register_vuid: allocated vuid = %u\n",
	...
	if (vuser->n_groups) {
		if (!(vuser->groups = (gid_t *)memdup(server_info->groups,
						      sizeof(gid_t) *
						      vuser->n_groups))) {
			DEBUG0(("register_vuid: failed to memdup "
				 "vuser->groups\n"));
			data_blob_free(&session_key);
			free(vuser);
			TALLOC_FREE(server_info);
	...
	DEBUG3(("UNIX uid %d is UNIX user %s, and will be vuid %u\n",
	...
	if (!session_claim(vuser)) {
		DEBUG1( ("Failed to claim session for vuid=%d\n",
			  vuser->vuid));
	...
	if ( (!vuser->guest) && vuser->unix_homedir && *(vuser->unix_homedir)) 
	{
		...
		if ( servicenumber == -1 ) {
			DEBUG3( ("Adding homes service for user '%s' using "
				  "home directory: '%s'\n", 
			...
			DEBUG3( ("Using static (or previously created) "
...
void add_session_user(const char *user)
{
	...
	if( !session_userlist ||
		...
		if (len_session_userlist > 128 * PSTRING_LEN) {
			DEBUG3(("add_session_user: session userlist already "
				 "too large.\n"));
			return;
		}
		...
		if( newlist == NULL ) {
			DEBUG1(("Unable to resize session_userlist\n"));
			return;
		}
		if (!session_userlist) {
			*newlist = '\0';
...
BOOL user_in_netgroup(const char *user, const char *ngname)
{
	...
	if(mydomain == NULL) {
		DEBUG5(("Unable to get default yp domain, let's try without specifying it\n"));
	}

	DEBUG5(("looking for user %s of domain %s in netgroup %s\n",
		user, mydomain?mydomain:"(ANY)", ngname));

	if (innetgr(ngname, NULL, user, mydomain)) {
		DEBUG5(("user_in_netgroup: Found\n"));
		return (True);
	} else {
...
BOOL user_in_list(const char *user,const char **list)
{
	...
	DEBUG10(("user_in_list: checking user %s in list\n", user));

	while (*list) {

		DEBUG10(("user_in_list: checking user |%s| against |%s|\n",
			  user, *list));
		...
		if (gptr) {
			...
			*member_list = '\0';
			...
			while (*member) {
				...
				DEBUG10(("validate_group = member = %s\n",
					  member));
	...
	*/
	...
	if (!ok) {
		...
		for (auser=strtok(user_list,LIST_SEP); !ok && auser;
			...
			if (password_ok(user2,password)) {
				ok = True;
				fstrcpy(user,user2);
				DEBUG3(("authorise_login: ACCEPTED: session "
		...
		for (auser=strtok(user_list,LIST_SEP); auser && !ok;
			...
			if (*auser == '@') {
				...
				if (auser) {
					ok = True;
					fstrcpy(user,auser);
					DEBUG3(("authorise_login: ACCEPTED: "
	...
	if (!ok && GUEST_OK(snum)) {
		...
		if (Get_Pwnam(guestname)) {
			...
			ok = True;
			DEBUG3(("authorise_login: ACCEPTED: guest account "
				 "and guest ok (%s)\n",	user));
		} else {
			DEBUG0(("authorise_login: Invalid guest account "
				 "%s??\n",guestname));
		}
		*guest = True;
	}
	...
	if (ok && !user_ok(user, snum)) {
		DEBUG0(("authorise_login: rejected invalid user %s\n",user));
		ok = False;
	}
