			...
			switch(siz) {
				...
				case 1:
					*d++ = *s++;
					break;
				default:
					DEBUG0(("check_path_syntax_internal: character length assumptions invalid !\n"));
					*d = '\0';
	...
	*name1 = *name2 = 0;
	...
	case 0x81: /* session request */
		...
		if (name_len(inbuf+4) > 50 || 
			...
			DEBUG0(("Invalid name length in session request\n"));
			return(0);
		}
		name_extract(inbuf,4,name1);
		name_type = name_extract(inbuf,4 + name_len(inbuf + 4),name2);
		DEBUG2(("netbios connect: name1=%s name2=%s\n",
			 name1,name2));      

		set_local_machine_name(name1, True);
		set_remote_machine_name(name2, True);

		DEBUG2(("netbios connect: local=%s remote=%s, name type = %x\n",
	...
	case 0x84: /* retarget session response */
		DEBUG0(("Unexpected session response\n"));
		break;
	...
	DEBUG5(("init msg_type=0x%x msg_flags=0x%x\n",
		    msg_type, msg_flags));
...
int reply_tcon_and_X(connection_struct *conn, char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	*service = *client_devicetype = 0;
	...
	DEBUG4(("Client requested device type [%s] for share [%s]\n", client_devicetype, service));
	...
	if (Protocol < PROTOCOL_NT1) {
		...
		if(!re && (sxfs.f_type==MSDOS_SUPER_MAGIC)) {
			fstype = fat;
		}
#endif
...
int reply_unknown(char *inbuf,char *outbuf)
{
	...
	DEBUG0(("unknown command type (%s): type=%d (0x%X)\n",
		...
		char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	DEBUG4( ("Received IOCTL (code 0x%x)\n", ioctl_code));
...
int reply_checkpath(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	DEBUG3(("reply_checkpath %s mode=%d\n", name, (int)SVAL(inbuf,smb_vwv0)));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG3(("reply_checkpath: check_name of %s failed (%s)\n",name,nt_errstr(status)));
		goto path_err;
	}

	if (!VALID_STAT(sbuf) && (SMB_VFS_STAT(conn,name,&sbuf) != 0)) {
		DEBUG3(("reply_checkpath: stat of %s failed (%s)\n",name,strerror(errno)));
...
int reply_getatr(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	if (*fname == '\0') {
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG3(("reply_getatr: check_name of %s failed (%s)\n",fname,nt_errstr(status)));
			END_PROFILE(SMBgetatr);
			return ERROR_NT(status);
		}
		if (!VALID_STAT(sbuf) && (SMB_VFS_STAT(conn,fname,&sbuf) != 0)) {
			DEBUG3(("reply_getatr: stat of %s failed (%s)\n",fname,strerror(errno)));
	...
	DEBUG3(("reply_getatr: name=%s mode=%d size=%u\n", fname, mode, (unsigned int)size ) );
...
int reply_search(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
  ...
  SearchEmpty:
	...
	DEBUG4( ( "%s mask=%s path=%s dtype=%d nument=%u of %u\n",
...
int reply_open(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	if (fattr & aDIR) {
		DEBUG3(("attempt to open a directory %s\n",fname));
...
int reply_ulogoffX(connection_struct *conn, char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	if(vuser == 0)
		DEBUG3(("ulogoff, vuser id %d does not map to user.\n", vuid));
...
int reply_mknew(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	if (fattr & aVOLID) {
		DEBUG0(("Attempt to create file (%s) with volid set - please report this\n",fname));
	}
	...
	DEBUG2( ( "reply_mknew: file %s\n", fname ) );
	DEBUG3( ( "reply_mknew %s fd=%d dmode=0x%x\n", fname, fsp->fh->fd, (unsigned int)fattr ) );
...
int reply_ctemp(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	DEBUG2( ( "reply_ctemp: created temp file %s\n", fname ) );
	DEBUG3( ( "reply_ctemp %s fd=%d umode=0%o\n", fname, fsp->fh->fd,
			   ...
			   uint32 dirtype, BOOL can_defer)
{
	...
	DEBUG10(("can_delete: %s, dirtype = %d\n", fname, dirtype ));
		...
		*/
		...
		while ((dname = ReadDirName(dir_hnd, &offset))) {
			...
			if (SMB_VFS_UNLINK(conn,fname) == 0) {
				count++;
				DEBUG3(("unlink_internals: successful unlink "
					 "[%s]\n",fname));
				notify_fname(conn, NOTIFY_ACTION_REMOVED,
					     FILE_NOTIFY_CHANGE_FILE_NAME,
					     fname);
	...
	if ( (chain_size == 0) && (nread > 0) &&
		...
		if ( SMB_VFS_SENDFILE( smbd_server_fd(), fsp, fsp->fh->fd, &header, startpos, nread) == -1) {
			...
			if (errno == EINTR) {
				...
				DEBUG0(("send_file_readbraw: sendfile not available. Faking..\n"));

				if (fake_sendfile(fsp, startpos, nread, outbuf + 4, out_buffsize - 4) == -1) {
					DEBUG0(("send_file_readbraw: fake_sendfile failed for file %s (%s).\n",
			...
			DEBUG0(("send_file_readbraw: sendfile failed for file %s (%s). Terminating\n",
				fsp->fsp_name, strerror(errno) ));
...
int reply_readbraw(connection_struct *conn, char *inbuf, char *outbuf, int dum_size, int out_buffsize)
{
	...
	if (!FNUM_OK(fsp,conn) || !fsp->can_read) {
		 ...
		 */
		DEBUG3(("fnum %d not open in readbraw - cache prime?\n",(int)SVAL(inbuf,smb_vwv0)));
	...
	if(CVAL(inbuf,smb_wct) == 10) {
		...
		if(IVAL(inbuf,smb_vwv8) != 0) {
			DEBUG0(("readbraw - large offset (%x << 32) used and we don't support \
64 bit offsets.\n", (unsigned int)IVAL(inbuf,smb_vwv8) ));
		...
		if(startpos < 0) {
			DEBUG0(("readbraw - negative 64 bit readraw offset (%.0f) !\n", (double)startpos ));
			_smb_setlen(header,0);
			if (write_data(smbd_server_fd(),header,4) != 4)
				fail_readraw();
			END_PROFILE(SMBreadbraw);
	...
	DEBUG3( ( "readbraw fnum=%d start=%.0f max=%lu min=%lu nread=%lu\n", fsp->fnum, (double)startpos,
...
int reply_lockread(connection_struct *conn, char *inbuf,char *outbuf, int length, int dum_buffsiz)
{
	...
	if (numtoread > max_recv) {
		DEBUG0(("reply_lockread: requested read size (%u) is greater than maximum allowed (%u). \
Returning short read of maximum allowed for compatibility with Windows 2000.\n",
	...
	DEBUG3(("lockread fnum=%d num=%d nread=%d\n",
...
int reply_read(connection_struct *conn, char *inbuf,char *outbuf, int size, int dum_buffsize)
{
	 ...
	 */
	if (numtoread > max_recv) {
		DEBUG0(("reply_read: requested read size (%u) is greater than maximum allowed (%u). \
Returning short read of maximum allowed for compatibility with Windows 2000.\n",
			(unsigned int)numtoread, (unsigned int)max_recv ));
		numtoread = MIN(numtoread,max_recv);
	}
	...
	DEBUG3( ( "read fnum=%d num=%d nread=%d\n",
		fsp->fnum, (int)numtoread, (int)nread ) );
		...
		files_struct *fsp, SMB_OFF_T startpos, size_t smb_maxcnt)
{
	...
	if ((chain_size == 0) && (CVAL(inbuf,smb_vwv0) == 0xFF) &&
		...
		if ((nread = SMB_VFS_SENDFILE( smbd_server_fd(), fsp, fsp->fh->fd, &header, startpos, smb_maxcnt)) == -1) {
			...
			if (errno == EINTR) {
				/* Ensure we don't do this again. */
				set_use_sendfile(SNUM(conn), False);
				DEBUG0(("send_file_readX: sendfile not available. Faking..\n"));

				if ((nread = fake_sendfile(fsp, startpos, smb_maxcnt, data,
							len_outbuf - (data-outbuf))) == -1) {
					DEBUG0(("send_file_readX: fake_sendfile failed for file %s (%s).\n",
						fsp->fsp_name, strerror(errno) ));
					exit_server_cleanly("send_file_readX: fake_sendfile failed");
				}
				DEBUG3( ( "send_file_readX: fake_sendfile fnum=%d max=%d nread=%d\n",
					fsp->fnum, (int)smb_maxcnt, (int)nread ) );
				/* Returning -1 here means successful sendfile. */
				return -1;
			}

			DEBUG0(("send_file_readX: sendfile failed for file %s (%s). Terminating\n",
				fsp->fsp_name, strerror(errno) ));
			exit_server_cleanly("send_file_readX sendfile failed");
		}

		DEBUG3( ( "send_file_readX: sendfile fnum=%d max=%d nread=%d\n",
			fsp->fnum, (int)smb_maxcnt, (int)nread ) );
		/* Returning -1 here means successful sendfile. */
		return -1;
	}
  ...
  normal_read:
	...
	DEBUG3( ( "send_file_readX fnum=%d max=%d nread=%d\n",
		fsp->fnum, (int)smb_maxcnt, (int)nread ) );
...
int reply_read_and_X(connection_struct *conn, char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	if (global_client_caps & CAP_LARGE_READX) {
		...
		if (smb_maxcnt > BUFFER_SIZE) {
			DEBUG0(("reply_read_and_X - read too large (%u) for reply buffer %u\n",
	...
	if(CVAL(inbuf,smb_wct) == 12) {
		...
		if(IVAL(inbuf,smb_vwv10) != 0) {
			DEBUG0(("reply_read_and_X - large offset (%x << 32) used and we don't support \
...
int reply_writebraw(connection_struct *conn, char *inbuf,char *outbuf, int size, int dum_buffsize)
{
	...
	DEBUG3(("writebraw1 fnum=%d start=%.0f num=%d wrote=%d sync=%d\n",
	...
	if (numtowrite != 0) {
		...
		if (numtowrite > BUFFER_SIZE) {
			DEBUG0(("reply_writebraw: Oversize secondary write raw requested (%u). Terminating\n",
				(unsigned int)numtowrite ));
			exit_server_cleanly("secondary writebraw failed");
		}

		if (tcount > nwritten+numtowrite) {
			DEBUG3(("Client overestimated the write %d %d %d\n",
				(int)tcount,(int)nwritten,(int)numtowrite));
		}

		if (read_data( smbd_server_fd(), inbuf+4, numtowrite) != numtowrite ) {
			DEBUG0(("reply_writebraw: Oversize secondary write raw read failed (%s). Terminating\n",
				strerror(errno) ));
			exit_server_cleanly("secondary writebraw failed");
		}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG5(("reply_writebraw: sync_file for %s returned %s\n",
			fsp->fsp_name, nt_errstr(status) ));
		END_PROFILE(SMBwritebraw);
		return ERROR_NT(status);
	}

	DEBUG3(("writebraw2 fnum=%d start=%.0f num=%d wrote=%d\n",
		      ...
		      int size, int dum_buffsize)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		END_PROFILE(SMBwriteunlock);
		DEBUG5(("reply_writeunlock: sync_file for %s returned %s\n",
			fsp->fsp_name, nt_errstr(status) ));
		return ERROR_NT(status);
	}
	...
	DEBUG3(("writeunlock fnum=%d num=%d wrote=%d\n",
...
int reply_write(connection_struct *conn, char *inbuf,char *outbuf,int size,int dum_buffsize)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		END_PROFILE(SMBwrite);
		DEBUG5(("reply_write: sync_file for %s returned %s\n",
			fsp->fsp_name, nt_errstr(status) ));
		return ERROR_NT(status);
	}
	...
	DEBUG3(("write fnum=%d num=%d wrote=%d\n", fsp->fnum, (int)numtowrite, (int)nwritten));
...
int reply_write_and_X(connection_struct *conn, char *inbuf,char *outbuf,int length,int bufsize)
{
	...
	if(CVAL(inbuf,smb_wct) == 14) {
		...
		if(IVAL(inbuf,smb_vwv12) != 0) {
			DEBUG0(("reply_write_and_X - large offset (%x << 32) used and we don't support \
64 bit offsets.\n", (unsigned int)IVAL(inbuf,smb_vwv12) ));
	...
	DEBUG3(("writeX fnum=%d num=%d wrote=%d\n",
		fsp->fnum, (int)numtowrite, (int)nwritten));

	status = sync_file(conn, fsp, write_through);
	if (!NT_STATUS_IS_OK(status)) {
		END_PROFILE(SMBwriteX);
		DEBUG5(("reply_write_and_X: sync_file for %s returned %s\n",
			fsp->fsp_name, nt_errstr(status) ));
		return ERROR_NT(status);
	}
...
int reply_lseek(connection_struct *conn, char *inbuf,char *outbuf, int size, int dum_buffsize)
{
	...
	DEBUG3(("lseek fnum=%d ofs=%.0f newpos = %.0f mode=%d\n",
...
int reply_flush(connection_struct *conn, char *inbuf,char *outbuf, int size, int dum_buffsize)
{
	...
	if (!fsp) {
		...
		if (!NT_STATUS_IS_OK(status)) {
			...
			DEBUG5(("reply_flush: sync_file for %s returned %s\n",
				fsp->fsp_name, nt_errstr(status) ));
			return ERROR_NT(status);
		}
	}
                ...
                int dum_buffsize)
{
	...
	if(fsp->is_directory) {
		...
		DEBUG3(("close fd=%d fnum=%d (numopen=%d)\n",
			 fsp->fh->fd, fsp->fnum,
			 conn->num_files_open));
		     ...
		     char *inbuf,char *outbuf, int size, int dum_buffsize)
{
	...
	if (numtowrite) {
		DEBUG3(("reply_writeclose: zero length write doesn't close file %s\n",
			fsp->fsp_name ));
		close_status = close_file(fsp,NORMAL_CLOSE);
	}

	DEBUG3(("writeclose fnum=%d num=%d wrote=%d (numopen=%d)\n",
		 fsp->fnum, (int)numtowrite, (int)nwritten,
		 conn->num_files_open));
	       ...
	       char *inbuf,char *outbuf, int length, int dum_buffsize)
{
	...
	DEBUG3(("lock fd=%d fnum=%d offset=%.0f count=%.0f\n",
		 ...
		 int dum_buffsize)
{
	...
	DEBUG3( ( "unlock fd=%d fnum=%d offset=%.0f count=%.0f\n",
		    fsp->fh->fd, fsp->fnum, (double)offset, (double)count ) );
	       ...
	       char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	if (!conn) {
		DEBUG4(("Invalid connection in tdis\n"));
		END_PROFILE(SMBtdis);
	       ...
	       char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	if (data_len > BUFFER_SIZE) {
		DEBUG0(("reply_echo: data_len too large.\n"));
		END_PROFILE(SMBecho);
		return -1;
	}
	...
	if (smb_reverb > 100) {
		DEBUG0(("large reverb (%d)?? Setting to 100\n",smb_reverb));
		smb_reverb = 100;
	}
		     ...
		     char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	DEBUG3(("printqueue start_index=%d max_count=%d\n",
...

	{
		...
		DEBUG3(("%d entries returned in queue\n",count));
	}
...
int reply_printwrite(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	DEBUG3( ( "printwrite fnum=%d num=%d\n", fsp->fnum, numtowrite ) );
...
int reply_mkdir(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	DEBUG5( ("create_directory returned %s\n", nt_errstr(status)));
...
NTSTATUS rmdir_internals(connection_struct *conn, const char *directory)
{
  ...
  err:
	...
	if (ret != 0) {
		DEBUG3(("rmdir_internals: couldn't remove directory %s : "
...
static BOOL resolve_wildcards(const char *name1, char *name2)
{
	...
	if (ext2[0]) {
		...
		if (actual_space >= available_space - 1) {
			DEBUG1(("resolve_wildcards: can't fit resolved name into specified buffer (overrun by %d bytes)\n",
				...
				SMB_DEV_T dev, SMB_INO_T inode, const char *newname)
{
	...
	for(fsp = file_find_di_first(dev, inode); fsp; fsp = file_find_di_next(fsp)) {
		...
		DEBUG10(("rename_open_files: renaming file fnum %d (dev = %x, inode = %.0f) from %s -> %s\n",
	...
	if (!did_rename) {
		DEBUG10(("rename_open_files: no open files on dev %x, inode %.0f for %s\n",
...
NTSTATUS rename_internals_fsp(connection_struct *conn, files_struct *fsp, pstring newname, uint32 attrs, BOOL replace_if_exists)
{
...

	{
		...
		if((pnewname=strrchr_m(newname,'/')) == 0) {
			pnewname=newname;
		}
		else 
			pnewname++;

		fmode = dos_mode(conn,fsp->fsp_name,&sbuf);
		if( !(fmode&aDIR ) ) {
			status = check_vetoname(pnewname);
			if (!NT_STATUS_IS_OK(status)) {
				return status;
			}
		}
	...
	if (strcsequal(fsp->fsp_name, newname)) {
		DEBUG3(("rename_internals_fsp: identical names in rename %s - returning success\n",
			newname));
		return NT_STATUS_OK;
	}
	...
	if(!replace_if_exists && dest_exists) {
		DEBUG3(("rename_internals_fsp: dest exists doing rename %s -> %s\n",
			fsp->fsp_name,newname));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG3(("rename_internals_fsp: Error %s rename %s -> %s\n",
	...
	if(SMB_VFS_RENAME(conn,fsp->fsp_name, newname) == 0) {
		...
		DEBUG3(("rename_internals_fsp: succeeded doing rename on %s -> %s\n",
			fsp->fsp_name,newname));
				...
				BOOL dest_has_wild)
{
	...
	*directory = *mask = 0;
...

	{
		...
		fmode = dos_mode(conn,name,&sbuf1);
		if( !(fmode&aDIR ) ) {
			status = check_vetoname(pnewname);
			if (!NT_STATUS_IS_OK(status)) {
				return status;
	...
	if (!src_has_wild) {
		...
		DEBUG3( ("rename_internals: case_sensitive = %d, "
		...
		if (dest_has_wild) {
			if (!resolve_wildcards(directory,newname)) {
				DEBUG6( ("rename_internals: resolve_wildcards %s %s failed\n", 
					  directory,newname));
		...
		if (!vfs_object_exist(conn, directory, &sbuf1)) {
			DEBUG3( ("rename_internals: source doesn't exist "
				  "doing rename %s -> %s\n",
				directory,newname));
			...
			status = map_nt_error_from_unix(errno);
			DEBUG3( ("rename_internals: Error %s rename %s -> "
		...
		if (strcsequal(directory, newname)) {
			DEBUG3( ("rename_internals: identical names in "
		...
		if(!replace_if_exists && vfs_object_exist(conn,newname,NULL)) {
			DEBUG3(("rename_internals: dest exists doing "
				 "rename %s -> %s\n", directory, newname));
			return NT_STATUS_OBJECT_NAME_COLLISION;
		}
		...
		if(SMB_VFS_RENAME(conn,directory, newname) == 0) {
			DEBUG3(("rename_internals: succeeded doing rename "
				 "on %s -> %s\n", directory, newname));
		...
		DEBUG3(("rename_internals: Error %s rename %s -> %s\n",
	...
	while ((dname = ReadDirName(dir_hnd, &offset))) {
		...
		if (!vfs_object_exist(conn, fname, &sbuf1)) {
			...
			DEBUG6( ("rename %s failed. Error %s\n",
				  fname, nt_errstr(status)));
			continue;
		}
		status = can_rename(conn,fname,attrs,&sbuf1,False);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG6( ("rename %s refused\n", fname));
			continue;
		}
		pstrcpy(destname,newname);
			
		if (!resolve_wildcards(fname,destname)) {
			DEBUG6( ("resolve_wildcards %s %s failed\n", 
				  fname, destname));
			continue;
		}
		...
		if (strcsequal(fname,destname)) {
			DEBUG3(("rename_internals: identical names "
		...
		TALLOC_FREE(lck);
		DEBUG3(("rename_internals: doing rename on %s -> "
			 "%s\n",fname,destname));
	}
	CloseDir(dir_hnd);
			...
			BOOL target_is_directory)
{
	...
	if ((ofun&3) == 1) {
		if(SMB_VFS_LSEEK(fsp2,fsp2->fh->fd,0,SEEK_END) == -1) {
			DEBUG0(("copy_file: error - vfs lseek returned error %s\n", strerror(errno) ));
			/*
...
int reply_copy(connection_struct *conn, char *inbuf,char *outbuf, int dum_size, int dum_buffsize)
{
	...
	*directory = *mask = 0;
	...
	DEBUG3(("reply_copy : %s -> %s\n",name,newname));
   
	if (tid2 != conn->cnum) {
		/* can't currently handle inter share copies XXXX */
		DEBUG3(("Rejecting inter-share copy\n"));
		END_PROFILE(SMBcopy);
	...
	if (!source_has_wild) {
		...
		while ((dname = ReadDirName(dir_hnd, &offset))) {
			...
			DEBUG3(("reply_copy : doing copy on %s -> %s\n",fname, destname));
...
SMB_BIG_UINT get_lock_count( char *data, int data_offset, BOOL large_file_format)
{
	...
	if(!large_file_format) {
		...
		if(IVAL(data,SMB_LARGE_LKLEN_OFFSET_HIGH(data_offset)) != 0) {
			DEBUG3(("get_lock_count: truncating lock count (high)0x%x (low)0x%x to just low count.\n",
	...
	*err = False;
	...
	if(!large_file_format) {
		...
		if(IVAL(data,SMB_LARGE_LKOFF_OFFSET_HIGH(data_offset)) != 0) {
			...
			DEBUG3(("get_lock_offset: truncating lock offset (high)0x%x (low)0x%x to offset 0x%x.\n",
		   ...
		   int length, int bufsize)
{
	...
	*/
	if ((locktype & LOCKING_ANDX_OPLOCK_RELEASE)) {
		...
		DEBUG5(("reply_lockingX: oplock break reply (%u) from client "
		...
		if (fsp->oplock_type == 0) {
			...
			DEBUG5(("reply_lockingX: Error : oplock break from "
		...
		if (!result) {
			DEBUG0( ("reply_lockingX: error in removing "
				  "oplock on file %s\n", fsp->fsp_name));
		...
		if (num_locks == 0 && num_ulocks == 0) {
			...
			if(CVAL(inbuf,smb_vwv0) != 0xff)
				DEBUG0(("reply_lockingX: Error : pure oplock "
	...
	for(i = 0; i < (int)num_ulocks; i++) {
		...
		DEBUG10(("reply_lockingX: unlock start=%.0f, len=%.0f for "
	...
	for(i = 0; i < (int)num_locks; i++) {
		...
		DEBUG10(("reply_lockingX: lock start=%.0f, len=%.0f for pid "
			  "%u, file %s timeout = %d\n", (double)offset,
	...
	DEBUG3( ("lockingX fnum=%d type=%d num_locks=%d num_ulocks=%d\n",
...
int reply_setattrE(connection_struct *conn, char *inbuf,char *outbuf, int size, int dum_buffsize)
{
	...
	DEBUG3( ( "reply_setattrE fnum=%d actime=%u modtime=%u\n",
		fsp->fnum,
...
int reply_writebmpx(connection_struct *conn, char *inbuf,char *outbuf, int size, int dum_buffsize)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		END_PROFILE(SMBwriteBmpx);
		DEBUG5(("reply_writebmpx: sync_file for %s returned %s\n",
			fsp->fsp_name, nt_errstr(status) ));
		return ERROR_NT(status);
	}
	...
	if((ssize_t)tcount > nwritten) {
		...
		if(!wbms) {
			DEBUG0(("Out of memory in reply_readmpx\n"));
			END_PROFILE(SMBwriteBmpx);
	...
	DEBUG3( ( "writebmpx fnum=%d num=%d wrote=%d\n",
			fsp->fnum, (int)numtowrite, (int)nwritten ) );
