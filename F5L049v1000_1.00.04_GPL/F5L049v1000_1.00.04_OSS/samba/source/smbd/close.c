...
static void check_magic(files_struct *fsp,connection_struct *conn)
{
...

	{
		...
		DEBUG3(("Invoking magic command %s gave %d\n",fname,ret));
		unlink(fname);
		if (ret != 0 || tmp_fd == -1) {
			if (tmp_fd != -1)
				close(tmp_fd);
					...
					enum file_close_type close_type)
{
	...
	if (lck == NULL) {
		DEBUG0( ("close_remove_share_mode: Could not get share mode "
			  "lock for file %s\n", fsp->fsp_name));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (!del_share_mode(lck, fsp)) {
		DEBUG0( ("close_remove_share_mode: Could not delete share "
			  "entry for file %s\n", fsp->fsp_name));
	}
	...
	DEBUG5(("close_remove_share_mode: file %s. Delete on close was set "
		 "- deleting file.\n", fsp->fsp_name));
	...
	if (ret != 0) {
		DEBUG5(("close_remove_share_mode: file %s. Delete on close "
	...
	if(sbuf.st_dev != fsp->dev || sbuf.st_ino != fsp->inode) {
		DEBUG5(("close_remove_share_mode: file %s. Delete on close "
			 ...
			 fsp->fsp_name ));
		DEBUG5(("close_remove_share_mode: file %s. stored dev = %x, "
...
static NTSTATUS close_normal_file(files_struct *fsp, enum file_close_type close_type)
{
	...
	DEBUG2(("%s closed file %s (numopen=%d) %s\n",
...
static NTSTATUS close_directory(files_struct *fsp, enum file_close_type close_type)
{
	...
	if (lck == NULL) {
		DEBUG0( ("close_directory: Could not get share mode lock for %s\n", fsp->fsp_name));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (!del_share_mode(lck, fsp)) {
		DEBUG0( ("close_directory: Could not delete share entry for %s\n", fsp->fsp_name));
	}
	...
	if ((close_type == NORMAL_CLOSE || close_type == SHUTDOWN_CLOSE) &&
		...
		DEBUG5(("close_directory: %s. Delete on close was set - "
