...
BOOL yield_connection(connection_struct *conn, const char *name)
{
	...
	DEBUG3(("Yielding connection to %s\n",name));
...
static int count_fn( TDB_CONTEXT *the_tdb, TDB_DATA kbuf, TDB_DATA dbuf, void *udp)
{
	...
	if (cs->Clear && !process_exists(crec.pid) && (errno == ESRCH)) {
		...
		if (tdb_delete(the_tdb, kbuf) != 0)
			DEBUG0(("count_fn: tdb_delete failed with error %s\n", tdb_errorstr(tdb) ));
		return 0;
	}
...
int count_current_connections( const char *sharename, BOOL clear  )
{
	...
	if (tdb_traverse(tdb, count_fn, &cs) == -1) {
		DEBUG0(("claim_connection: traverse of connections.tdb failed with error %s.\n",
			tdb_errorstr(tdb) ));
		return False;
	}
...
BOOL claim_connection(connection_struct *conn, const char *name,int max_connections,BOOL Clear, uint32 msg_flags)
{
	...
	if (max_connections > 0) {
		...
		if (curr_connections >= max_connections) {
			DEBUG1(("claim_connection: Max connections (%d) exceeded for %s\n",
	...
	if (tdb_store(tdb, kbuf, dbuf, TDB_REPLACE) != 0) {
		DEBUG0(("claim_connection: tdb_store failed with error %s.\n",
			tdb_errorstr(tdb) ));
		return False;
	}
...
BOOL register_message_flags(BOOL doreg, uint32 msg_flags)
{
	...
	DEBUG10(("register_message_flags: %s flags 0x%x\n",
	...
	make_conn_key(NULL, "", &kbuf, &key);
        ...
        if (!dbuf.dptr) {
		DEBUG0(("register_message_flags: tdb_fetch failed: %s\n",
			tdb_errorstr(tdb)));
		return False;
	}
	...
	if (tdb_store(tdb, kbuf, dbuf, TDB_REPLACE) != 0) {
		DEBUG0(("register_message_flags: tdb_store failed: %s.\n",
			tdb_errorstr(tdb) ));
		SAFE_FREE(dbuf.dptr);
		return False;
	}

	DEBUG10(("register_message_flags: new flags 0x%x\n",
...
BOOL store_pipe_opendb( smb_np_struct *p )
{
	...
	if ( (prec = TALLOC_P( NULL, struct pipe_open_rec)) == NULL ) {
		DEBUG0(("store_pipe_opendb: talloc failed!\n"));
		return False;
	}
