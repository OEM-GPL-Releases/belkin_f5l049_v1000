...
static struct _FAKE_FILE_HANDLE *init_fake_file_handle(enum FAKE_FILE_TYPE type)
{
	...
	for (i=0;fake_files[i].name!=NULL;i++) {
		if (fake_files[i].type==type) {
			DEBUG5(("init_fake_file_handle: for [%s]\n",fake_files[i].name));

			if ((mem_ctx=talloc_init("fake_file_handle"))==NULL) {
				DEBUG0(("talloc_init(fake_file_handle) failed.\n"));
				return NULL;	
			}
...
enum FAKE_FILE_TYPE is_fake_file(const char *fname)
{
	...
	for (i=0;fake_files[i].name!=NULL;i++) {
		if (strncmp(fname,fake_files[i].name,strlen(fake_files[i].name))==0) {
			DEBUG5(("is_fake_file: [%s] is a fake file\n",fname));
				...
				files_struct **result)
{
	...
	if (current_user.ut.uid != 0) {
		DEBUG1(("open_fake_file_shared: access_denied to service[%s] file[%s] user[%s]\n",
	...
	DEBUG5(("open_fake_file_shared: fname = %s, FID = %d, access_mask = 0x%x\n",
