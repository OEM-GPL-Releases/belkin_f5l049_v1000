...
int linux_set_lease_sighandler(int fd)
{
        if (fcntl(fd, F_SETSIG, RT_SIGNAL_LEASE) == -1) {
                DEBUG3(("Failed to set signal handler for kernel lease\n"));
                return -1;
        }
...
static BOOL linux_set_kernel_oplock(files_struct *fsp, int oplock_type)
{
	if ( SMB_VFS_LINUX_SETLEASE(fsp,fsp->fh->fd, F_WRLCK) == -1) {
		DEBUG3(("linux_set_kernel_oplock: Refused oplock on file %s, "
	...
	DEBUG3(("linux_set_kernel_oplock: got kernel oplock on file %s, "
...
struct kernel_oplocks *linux_init_kernel_oplocks(void) 
{
	...
	if (!linux_oplocks_available()) {
		DEBUG3(("Linux kernel oplocks not available\n"));
		return NULL;
	}
	...
	if (sigaction(RT_SIGNAL_LEASE, &act, NULL) != 0) {
		DEBUG0(("Failed to setup RT_SIGNAL_LEASE handler\n"));
		return NULL;
	}
	...
	DEBUG3(("Linux kernel oplocks enabled\n"));
