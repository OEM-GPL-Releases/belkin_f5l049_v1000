...
static BOOL irix_oplocks_available(void)
{
	...
	if(pipe(pfd) != 0) {
		DEBUG0(("check_kernel_oplocks: Unable to create pipe. Error "
			 "was %s\n",
			 strerror(errno) ));
		return False;
	}
	...
	if((fd = sys_open(tmpname, O_RDWR|O_CREAT|O_EXCL|O_TRUNC, 0600)) < 0) {
		DEBUG0(("check_kernel_oplocks: Unable to open temp test file "
			 "%s. Error was %s\n",
	...
	if(sys_fcntl_long(fd, F_OPLKREG, pfd[1]) == -1) {
		DEBUG0(("check_kernel_oplocks: Kernel oplocks are not "
	...
	if(sys_fcntl_long(fd, F_OPLKACK, OP_REVOKE) < 0 ) {
		DEBUG0(("check_kernel_oplocks: Error when removing kernel "
...
static files_struct *irix_oplock_receive_message(fd_set *fds)
{
	...
	if(read(oplock_pipe_read, &dummy, 1) != 1) {
		DEBUG0(("irix_oplock_receive_message: read of kernel "
	...
	if(sys_fcntl_ptr(oplock_pipe_read, F_OPLKSTAT, &os) < 0) {
		DEBUG0(("irix_oplock_receive_message: fcntl of kernel "
			 "notification failed. Error was %s.\n",
			 strerror(errno) ));
		if(errno == EAGAIN) {
			/*
	...
	if ((fsp = file_find_di_first((SMB_DEV_T)os.os_dev,
		...
		DEBUG0(("irix_oplock_receive_message: unable to find open "
			 "file with dev = %x, inode = %.0f\n",
			 (unsigned int)os.os_dev, (double)os.os_ino ));
		return NULL;
	}
     
	DEBUG5(("irix_oplock_receive_message: kernel oplock break request "
...
static BOOL irix_set_kernel_oplock(files_struct *fsp, int oplock_type)
{
	if (sys_fcntl_long(fsp->fh->fd, F_OPLKREG, oplock_pipe_write) == -1) {
		if(errno != EAGAIN) {
			DEBUG0(("irix_set_kernel_oplock: Unable to get "
			...
			DEBUG5(("irix_set_kernel_oplock: Refused oplock on "
	...
	DEBUG10(("irix_set_kernel_oplock: got kernel oplock on file %s, dev "
...
struct kernel_oplocks *irix_init_kernel_oplocks(void) 
{
	...
	if(pipe(pfd) != 0) {
		DEBUG0(("setup_kernel_oplock_pipe: Unable to create pipe. "
			 "Error was %s\n", strerror(errno) ));
		return False;
	}
