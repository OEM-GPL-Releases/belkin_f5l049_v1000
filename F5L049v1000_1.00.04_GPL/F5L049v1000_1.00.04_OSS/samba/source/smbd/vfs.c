...
NTSTATUS smb_register_vfs(int version, const char *name, vfs_op_tuple *vfs_op_tuples)
{
	struct vfs_init_function_entry *entry = backends;
 	...
 	if ((version != SMB_VFS_INTERFACE_VERSION)) {
		DEBUG0( ("Failed to register vfs module.\n"
	...
	if (!name || !name[0] || !vfs_op_tuples) {
		DEBUG0(("smb_register_vfs() called with NULL pointer or empty name!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (vfs_find_backend_entry(name)) {
		DEBUG0(("VFS module %s already loaded!\n", name));
	...
	DEBUG5( ("Successfully added vfs backend '%s'\n", name));
	return NT_STATUS_OK;
}
...
{
	DEBUG3( ("Initialising default vfs hooks\n"));
...
BOOL vfs_init_custom(connection_struct *conn, const char *vfs_object)
{
	...
	if (!conn||!vfs_object||!vfs_object[0]) {
		DEBUG0(("vfs_init_custon() called with NULL pointer or emtpy vfs_object!\n"));
		return False;
	}
	...
	DEBUG3( ("Initialising custom vfs hooks from [%s]\n", vfs_object));
	   ...
	   (NT_STATUS_IS_OK(smb_probe_module("vfs", module_name)) && 
		...
		DEBUGADD(5,("Successfully loaded vfs module [%s] with the new modules system\n", vfs_object));
	 	...
	 	if ((ops = entry->vfs_op_tuples) == NULL) {
	 		DEBUG0( ("entry->vfs_op_tuples==NULL for [%s] failed\n", vfs_object));
	 		SAFE_FREE(module_name);
	 		return False;
	 	}
	} else {
		DEBUG0(("Can't find a vfs module [%s]\n",vfs_object));
		SAFE_FREE(module_name);
		return False;
	}
	...
	DLIST_ADD(conn->vfs_handles, handle);
 	...
 	for(i=0; ops[i].op != NULL; i++) {
		DEBUG5( ("Checking operation #%d (type %d, layer %d)\n", i, ops[i].type, ops[i].layer));
...
BOOL smbd_vfs_init(connection_struct *conn)
{
	...
	for (j=i-1; j >= 0; j--) {
		if (!vfs_init_custom(conn, vfs_objects[j])) {
			DEBUG0( ("smbd_vfs_init: vfs_init_custom failed for %s\n", vfs_objects[j]));
			return False;
		}
	}
	return True;
...
int vfs_allocate_file_space(files_struct *fsp, SMB_BIG_UINT len)
{
	...
	DEBUG10(("vfs_allocate_file_space: file %s, len %.0f\n", fsp->fsp_name, (double)len ));

	if (((SMB_OFF_T)len) < 0) {
		DEBUG0(("vfs_allocate_file_space: %s negative len requested.\n", fsp->fsp_name ));
		errno = EINVAL;
		return -1;
	}
	...
	if (len < (SMB_BIG_UINT)st.st_size) {
		...
		DEBUG10(("vfs_allocate_file_space: file %s, shrink. Current size %.0f\n",
	...
	DEBUG10(("vfs_allocate_file_space: file %s, grow. Current size %.0f, needed blocks = %.0f, space avail = %.0f\n",
...
int vfs_set_filelen(files_struct *fsp, SMB_OFF_T len)
{
	...
	release_level_2_oplocks_on_change(fsp);
	DEBUG10(("vfs_set_filelen: ftruncate %s to len %.0f\n", fsp->fsp_name, (double)len));
	flush_write_cache(fsp, SIZECHANGE_FLUSH);
...
/*
 silex sorce
*/
static int check_truncate_fs(connection_struct *conn,files_struct *fsp)
{
	int len,ret;
	char *path;
	struct statfs stfs;

	len = strlen(conn->connectpath) + strlen(fsp->fsp_name) + 2;
	if(!(path = (char*)malloc(len)))
		return -1;
	if(snprintf(path,len,"%s/%s",conn->connectpath,fsp->fsp_name)<=0){
		free(path);
		return -1;
	}
	...
	free(path);
	if(ret) return -1;
	if(stfs.f_type==MSDOS_SUPER_MAGIC) return 0; /* fat file system */

	return 1; /* can trucate */
}

/*
 silex sorce
*/
int vfs_set_filelen2(connection_struct *conn,files_struct *fsp, SMB_OFF_T len, SMB_OFF_T oldlen)
{
#if (HAVE_SYS_STATFS_H) && (LINUX)
	int ret;
	SMB_OFF_T tlen = len;
	...
	release_level_2_oplocks_on_change(fsp);
	DEBUG10(("vfs_set_filelen2: ftruncate %s to len %.0f\n", fsp->fsp_name, (double)len));
	flush_write_cache(fsp, SIZECHANGE_FLUSH);

	/*syslog(LOG_ERR,"%s:%s(%d)%llu:%llu\n",__FILE__,__FUNCTION__,__LINE__,oldlen,len);*/
	if(oldlen<len && !check_truncate_fs(conn,fsp))
		tlen = oldlen;

	if ((ret = SMB_VFS_FTRUNCATE(fsp, fsp->fh->fd, tlen)) != -1) {
		set_filelen_write_cache(fsp, len);
		notify_fname(fsp->conn, NOTIFY_ACTION_MODIFIED,
			     FILE_NOTIFY_CHANGE_SIZE
			     | FILE_NOTIFY_CHANGE_ATTRIBUTES,
...
int vfs_fill_sparse(files_struct *fsp, SMB_OFF_T len)
{
	...
	DEBUG10(("vfs_fill_sparse: write zeros in file %s from len %.0f to len %.0f (%.0f bytes)\n",
	...
	while (total < num_to_write) {
		...
		if (pwrite_ret == -1) {
			DEBUG10(("vfs_fill_sparse: SMB_VFS_PWRITE for file %s failed with error %s\n",
...
static void array_promote(char *array,int elsize,int element)
{
	...
	if (!p) {
		DEBUG5(("array_promote: malloc fail\n"));
		return;
	}
...
char *vfs_GetWd(connection_struct *conn, char *path)
{
	...
	*s = 0;
	...
	if (SMB_VFS_STAT(conn, ".",&st) == -1) {
		...
		DEBUG1(("vfs_GetWd: couldn't stat \".\" path=%s error %s (NFS problem ?)\n", path, strerror(errno) ));
		return(SMB_VFS_GETWD(conn,path));
	}
	...
	if (!SMB_VFS_GETWD(conn,s)) {
		DEBUG0(("vfs_GetWd: SMB_VFS_GETWD call failed, errno %s\n",strerror(errno)));
		return (NULL);
	}

	pstrcpy(path,s);

	DEBUG5(("vfs_GetWd %s, inode %.0f, dev %.0f\n",s,(double)st.st_ino,(double)st.st_dev));
...
NTSTATUS reduce_name(connection_struct *conn, const pstring fname)
{
	...
	if (!resolved_name) {
		switch (errno) {
			case ENOTDIR:
				DEBUG3(("reduce_name: Component not a directory in getting realpath for %s\n", fname));
			...
			case ENOENT:
			{
...
#endif
				if (!resolved_name) {
					DEBUG3(("reduce_name: couldn't get realpath for %s\n", fname));
					return map_nt_error_from_unix(errno);
				}
				...
				if (!resolved_name) {
					DEBUG0(("reduce_name: malloc fail for %s\n", tmp_fname));
			...
			default:
				DEBUG1(("reduce_name: couldn't get realpath for %s\n", fname));
				return map_nt_error_from_unix(errno);
		}
	}

	DEBUG10(("reduce_name realpath [%s] -> [%s]\n", fname, resolved_name));

	if (*resolved_name != '/') {
		DEBUG0(("reduce_name: realpath doesn't return absolute paths !\n"));
	...
	if (!lp_widelinks(SNUM(conn)) && (strncmp(conn->connectpath, resolved_name, con_path_len) != 0)) {
		DEBUG2( ("reduce_name: Bad access attempt: %s is a symlink outside the share path", fname));
		if (free_resolved_name) {
			SAFE_FREE(resolved_name);
		}
        ...
        if (!lp_symlinks(SNUM(conn))) {
			...
			if (free_resolved_name) {
				SAFE_FREE(resolved_name);
                        ...
                        DEBUG3(("reduce_name: denied: file path name %s is a symlink\n",resolved_name));
			return NT_STATUS_ACCESS_DENIED;
                }
        }
#endif

	DEBUG3(("reduce_name: %s reduced to %s\n", fname, resolved_name));
	if (free_resolved_name) {
		SAFE_FREE(resolved_name);
	}
	return NT_STATUS_OK;
