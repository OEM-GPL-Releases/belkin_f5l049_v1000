		...
		uint32 blocking_pid)
{
	...
	if(in_chained_smb() ) {
		DEBUG0(("push_blocking_lock_request: cannot queue a chained request (currently).\n"));
		return False;
	}
	...
	if((blr = SMB_MALLOC_P(blocking_lock_record)) == NULL) {
		DEBUG0(("push_blocking_lock_request: Malloc fail !\n" ));
		return False;
	}
	...
	if((blr->inbuf = (char *)SMB_MALLOC(length)) == NULL) {
		DEBUG0(("push_blocking_lock_request: Malloc fail (2)!\n" ));
		SAFE_FREE(blr);
		return False;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG0(("push_blocking_lock_request: failed to add PENDING_LOCK record.\n"));
	...
	DEBUG3(("push_blocking_lock_request: lock request length=%d blocked with "
...
static void blocking_lock_reply_error(blocking_lock_record *blr, NTSTATUS status)
{
	...
	default:
		DEBUG0(("blocking_lock_reply_error: PANIC - unknown type on blocking lock queue - exiting.!\n"));
...
static BOOL process_lockingX(blocking_lock_record *blr)
{
	...
	if(blr->lock_num == num_locks) {
		...
		DEBUG3(("process_lockingX file = %s, fnum=%d type=%d num_locks=%d\n",
	...
	DEBUG10(("process_lockingX: only got %d locks of %d needed for file %s, fnum = %d. \
Waiting....\n", 
...
static BOOL blocking_lock_record_process(blocking_lock_record *blr)
{
	switch(blr->com_type) {
		...
		default:
			DEBUG0(("blocking_lock_record_process: PANIC - unknown type on blocking lock queue - exiting.!\n"));
			exit_server("PANIC - unknown type on blocking lock queue");
	}
...
void cancel_pending_lock_requests_by_fid(files_struct *fsp, struct byte_range_lock *br_lck)
{
	...
	for(blr = blocking_lock_queue; blr; blr = next) {
		...
		if(blr->fsp->fnum == fsp->fnum) {
			...
			if (br_lck) {
				DEBUG10(("remove_pending_lock_requests_by_fid - removing request type %d for \
...
void remove_pending_lock_requests_by_mid(int mid)
{
	...
	for(blr = blocking_lock_queue; blr; blr = next) {
		...
		if(SVAL(blr->inbuf,smb_mid) == mid) {
			...
			if (br_lck) {
				DEBUG10(("remove_pending_lock_requests_by_mid - removing request type %d for \
file %s fnum = %d\n", blr->com_type, fsp->fsp_name, fsp->fnum ));
...
unsigned int blocking_locks_timeout_ms(unsigned int default_timeout_ms)
{
	...
	DEBUG10(("blocking_locks_timeout_ms: returning %u\n", timeout_ms));
...
void process_blocking_lock_queue(void)
{
	...
	for (blr = blocking_lock_queue; blr; blr = next) {
		...
		DEBUG5(("process_blocking_lock_queue: examining pending lock fnum = %d for file %s\n",
		...
		if(!change_to_user(conn,vuid)) {
			...
			DEBUG0(("process_blocking_lock_queue: Unable to become user vuid=%d.\n",
				vuid ));
		...
		if(!set_current_service(conn,SVAL(blr->inbuf,smb_flg),True)) {
			...
			DEBUG0(("process_blocking_lock_queue: Unable to become service Error was %s.\n", strerror(errno) ));
			blocking_lock_reply_error(blr,NT_STATUS_ACCESS_DENIED);
			DLIST_REMOVE(blocking_lock_queue, blr);
			free_blocking_lock_record(blr);
			change_to_root_user();
		...
		if (!timeval_is_zero(&blr->expire_time) && timeval_compare(&blr->expire_time, &tv_curr) <= 0) {
			...
			if (br_lck) {
				DEBUG5(("process_blocking_lock_queue: pending lock fnum = %d for file %s timed out.\n",
					fsp->fnum, fsp->fsp_name ));
						 ...
						 void *private_data)
{
	...
	if (len != MSG_BLOCKING_LOCK_CANCEL_SIZE) {
		DEBUG0( ("process_blocking_lock_cancel_message: "
	...
	DEBUG10(("process_blocking_lock_cancel_message: returning error %s\n",
		nt_errstr(err) ));
