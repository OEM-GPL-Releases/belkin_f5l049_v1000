...
{  
	...
	DEBUG8(("put name [%s] from [%s] into dir struct\n",buf+30, fname));
}
...
static void dptr_idleoldest(void)
{
	...
	if(!dptr) {
		DEBUG0(("No dptrs available to idle ?\n"));
		return;
	}
		...
		if(dptr->dnum == key) {
			if (!forclose && !dptr->dir_hnd) {
				if (dirhandles_open >= MAX_OPEN_DIRECTORIES)
					dptr_idleoldest();
				DEBUG4(("dptr_get: Reopening dptr key %d\n",key));
				if (!(dptr->dir_hnd = OpenDir(dptr->conn, dptr->path, dptr->wcard, dptr->attr))) {
					DEBUG4(("dptr_get: Failed to open %s (%s)\n",dptr->path,
						strerror(errno)));
					return False;
				}
			}
...
static void dptr_close_internal(struct dptr_struct *dptr)
{
	...
	if(bitmap_query( dptr_bmap, dptr->dnum - 1) != True) {
		DEBUG0(("dptr_close_internal : Error - closing dnum = %d and bitmap not set !\n",
			dptr->dnum ));
	}
...
void dptr_close(int *key)
{
	...
	if (!dptr) {
		DEBUG0(("Invalid key %d given to dptr_close\n", *key));
		return;
	}
...
static void dptr_close_oldest(BOOL old)
{
	...
	if(!dptr) {
		DEBUG0(("No old dptrs available to close oldest ?\n"));
		return;
	}
		...
		const char *wcard, BOOL wcard_has_wild, uint32 attr, struct dptr_struct **dptr_ret)
{
	...
	if(!dptr) {
		DEBUG0(("malloc fail in dptr_create.\n"));
		CloseDir(dir_hnd);
	...
	if(old_handle) {
		...
		if(dptr->dnum == -1 || dptr->dnum > 254) {
			...
			dptr->dnum = bitmap_find(dptr_bmap, 0);
			if(dptr->dnum == -1 || dptr->dnum > 254) {
				DEBUG0(("dptr_create: returned %d: Error - all old dirptrs in use ?\n", dptr->dnum));
				SAFE_FREE(dptr);
				CloseDir(dir_hnd);
		...
		if(dptr->dnum == -1 || dptr->dnum < 255) {
			...
			if(dptr->dnum == -1 || dptr->dnum < 255) {
				DEBUG0(("dptr_create: returned %d: Error - all new dirptrs in use ?\n", dptr->dnum));
				SAFE_FREE(dptr);
				CloseDir(dir_hnd);
				return NT_STATUS_TOO_MANY_OPENED_FILES;
			}
	...
	DEBUG3(("creating new dirptr %d for path %s, expect_close = %d\n",
...
BOOL dptr_fill(char *buf1,unsigned int key)
{
	...
	uint32 offset;
	if (!dptr) {
		DEBUG1(("filling null dirptr %d\n",key));
		return(False);
	}
	offset = (uint32)TellDir(dptr->dir_hnd);
	DEBUG6(("fill on key %u dirptr 0x%lx now at %d\n",key,
...
struct dptr_struct *dptr_fetch(char *buf,int *num)
{
	...
	*num = key;
	...
	DEBUG3(("fetching dirptr %d for path %s at offset %d\n",
...
struct dptr_struct *dptr_fetch_lanman2(int dptr_num)
{
	...
	DEBUG3(("fetching dirptr %d for path %s\n",dptr_num,dptr_path(dptr_num)));
	return(dptr);
}
                   ...
                   SMB_OFF_T *size,uint32 *mode,time_t *date,BOOL check_descend)
{
	...
	*path = *pathreal = *filename = 0;
	...
	while (!found) {
		...
		DEBUG6(("readdir on dirptr 0x%lx now at offset %ld\n",
		...
		if ((strcmp(mask,"*.*") == 0) ||
			...
			*path = 0;
			...
			if (!VALID_STAT(sbuf) && (SMB_VFS_STAT(conn, pathreal, &sbuf)) != 0) {
				DEBUG5(("Couldn't stat 1 [%s]. Error = %s\n",path, strerror(errno) ));
				continue;
			}
			...
			*mode = dos_mode(conn,pathreal,&sbuf);
			...
			if (!dir_check_ftype(conn,*mode,dirtype)) {
				DEBUG5(("[%s] attribs 0x%x didn't match 0x%x\n",filename,(unsigned int)*mode,(unsigned int)dirtype));
				continue;
			}
			...
			*date = sbuf.st_mtime;
			...
			DEBUG3(("get_dir_entry mask=[%s] found %s fname=%s\n",mask, pathreal,fname));
...
static BOOL user_can_read_file(connection_struct *conn, char *name, SMB_STRUCT_STAT *pst)
{
	...
	if (!VALID_STAT(*pst) && (SMB_VFS_STAT(conn, name, pst) != 0)) {
		DEBUG10(("user_can_read_file: SMB_VFS_STAT failed for file %s with error %s\n",
			name, strerror(errno) ));
		return False;
	}
...
BOOL is_visible_file(connection_struct *conn, const char *dir_path, const char *name, SMB_STRUCT_STAT *pst, BOOL use_veto)
{
	...
	if (use_veto && IS_VETO_PATH(conn, name)) {
		DEBUG10(("is_visible_file: file %s is vetoed.\n", name ));
		return False;
	}
	...
	if (hide_unreadable || hide_unwriteable || hide_special) {
		...
		if (hide_unreadable && !user_can_read_file(conn, entry, pst)) {
			DEBUG10(("is_visible_file: file %s is unreadable.\n", entry ));
			SAFE_FREE(entry);
			return False;
		}
		/* Honour _hide unwriteable_ option */
		if (hide_unwriteable && !user_can_write_file(conn, entry, pst)) {
			DEBUG10(("is_visible_file: file %s is unwritable.\n", entry ));
			SAFE_FREE(entry);
			return False;
		}
		/* Honour _hide_special_ option */
		if (hide_special && file_is_special(conn, entry, pst)) {
			DEBUG10(("is_visible_file: file %s is special.\n", entry ));
			SAFE_FREE(entry);
			return False;
		}
		SAFE_FREE(entry);
				...
				const char *dirname)
{
	...
	while ((dname = ReadDirName(dir_hnd,&dirpos))) {
		...
		DEBUG10(("can_delete_directory: got name %s - can't delete\n", dname ));
