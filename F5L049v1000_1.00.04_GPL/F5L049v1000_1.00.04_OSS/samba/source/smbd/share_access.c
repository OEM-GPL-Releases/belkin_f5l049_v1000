				...
				const char *name)
{
	...
	if ( string_to_sid( &sid, name ) ) {
		DEBUG5(("token_contains_name: Checking for SID [%s] in token\n", name));
	...
	if (!do_group_checks(&name, &prefix)) {
		if (!lookup_name_smbconf(mem_ctx, name, LOOKUP_NAME_ALL,
				 NULL, NULL, &sid, &type)) {
			DEBUG5( ("lookup_name %s failed\n", name));
			return False;
		}
		if (type != SID_NAME_USER) {
			DEBUG5( ("%s is a %s, expected a user\n",
	...
	for (/* initialized above */ ; *prefix != '\0'; prefix++) {
		if (*prefix == '+') {
			...
			if ((type != SID_NAME_DOM_GRP) &&
				...
				DEBUG5( ("%s is a %s, expected a group\n",
					  name, sid_type_lookup(type)));
				return False;
			}
...
BOOL user_ok_token(const char *username, const struct nt_user_token *token, int snum)
{
	if (lp_invalid_users(snum) != NULL) {
		if (token_contains_name_in_list(username, lp_servicename(snum),
			...
			DEBUG10( ("User %s in 'invalid users'\n", username));
			return False;
		}
	}
	...
	if (lp_valid_users(snum) != NULL) {
		if (!token_contains_name_in_list(username,
						 ...
						 lp_valid_users(snum))) {
			DEBUG10( ("User %s not in 'valid users'\n",
				   username));
			return False;
		}
	}
	...
	if (lp_onlyuser(snum)) {
		...
		if ((list[0] == NULL) || (*list[0] == '\0')) {
			DEBUG0( ("'only user = yes' and no 'username ='\n"));
			return False;
		}
	...
	DEBUG10( ("user_ok_token: share %s is ok for unix user %s\n",
				  ...
				  const struct nt_user_token *token, int snum)
{
	...
	DEBUG10(("is_share_read_only_for_user: share %s is %s for unix user "
		  "%s\n", lp_servicename(snum),
