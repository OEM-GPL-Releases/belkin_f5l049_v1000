...
void set_conn_connectpath(connection_struct *conn, const pstring connectpath)
{
	...
	*d = '\0';
	...
	DEBUG10(("set_conn_connectpath: service %s, connectpath = %s\n",
...
int find_service(fstring service)
{
	...
	if (iService < 0) {
		...
		DEBUG3(("checking for home directory %s gave %s\n",service,
	...
	if (iService < 0) {
		...
		if ((iPrinterService = lp_servicenumber(PRINTERS_NAME)) >= 0) {
			DEBUG3(("checking whether %s is a valid printer name...\n", service));
			if (pcap_printername_ok(service)) {
				DEBUG3(("%s is a valid printer name\n", service));
				DEBUG3(("adding %s as a printer service\n", service));
				lp_add_printer(service, iPrinterService);
				iService = lp_servicenumber(service);
				if (iService < 0) {
					DEBUG0(("failed to add %s as a printer service!\n", service));
				}
			} else {
				DEBUG3(("%s is not a valid printer name\n", service));
			}
		}
	}
  ...
  fail:
	...
	if (iService < 0)
		DEBUG3(("find_service() failed to find service %s\n", service));
				  ...
				  gid_t *pgid)
{
	...
	*pgid = (gid_t)-1;
	...
	if (!lookup_name_smbconf(mem_ctx, groupname,
		...
		DEBUG10( ("lookup_name_smbconf(%s) failed\n",
			   groupname));
		goto done;
	}
	...
	if ((type != SID_NAME_DOM_GRP) && (type != SID_NAME_ALIAS) &&
		...
		DEBUG10( ("%s is a %s, not a group\n", groupname,
			   sid_type_lookup(type)));
		goto done;
	}

	if (!sid_to_gid(&group_sid, &gid)) {
		DEBUG10( ("sid_to_gid(%s) for %s failed\n",
	...
	if (force_user && user_must_be_member) {
		if (user_in_group_sid(username, &group_sid)) {
			...
			*pgid = gid;
			DEBUG3(("Forced group %s for member %s\n",
				 groupname, username));
		} else {
			DEBUG0(("find_forced_group: forced user %s is not a member "
					       ...
					       NTSTATUS *status)
{
	...
	*user = 0;
	...
	conn = conn_new();
	if (!conn) {
		DEBUG0(("Couldn't find free connection.\n"));
	...
	if (lp_guest_only(snum)) {
		...
		if (!pass) {
			DEBUG0(("make_connection_snum: Invalid guest "
				 "account %s??\n",guestname));
			conn_free(conn);
		...
		TALLOC_FREE(found_username);
		TALLOC_FREE(pass);
		DEBUG3(("Guest only user %s\n",user));
	} else if (vuser) {
		if (vuser->guest) {
			if (!lp_guest_ok(snum)) {
				DEBUG2( ("guest user (from session setup) "
			...
			if (!user_ok_token(vuser->user.unix_name,
				...
				DEBUG2( ("user '%s' (from session setup) not "
		...
		if (!authorise_login(snum,user,password,&guest)) {
			DEBUG2( ( "Invalid username/password for [%s]\n", 
				    lp_servicename(snum)) );
			conn_free(conn);
		...
		DEBUG0( ("invalid VUID (vuser) but not in security=share\n"));
		conn_free(conn);
		*status = NT_STATUS_ACCESS_DENIED;
		return NULL;
	}
	...
	if (conn->nt_user_token != NULL) {
		...
		for (i=0; i<conn->nt_user_token->num_sids; i++) {
			...
			if (!add_gid_to_array_unique(conn->mem_ctx, gid, &conn->groups,
						&conn->ngroups)) {
				DEBUG0( ("add_gid_to_array_unique failed\n"));
				conn_free(conn);
				*status = NT_STATUS_NO_MEMORY;
				return NULL;
			}
...

	{
		...
		DEBUG3(("Connect path is '%s' for service [%s]\n",s,
			 lp_servicename(snum)));
	}
...

	{
		...
		if (token == NULL) {
			DEBUG0(("make_connection: connection to %s "
	...
	if (!smbd_vfs_init(conn)) {
		DEBUG0( ("vfs_init failed for service %s\n",
			  lp_servicename(snum)));
		conn_free(conn);
...
/* ROOT Activities: */	
	...
	if (!claim_connection(conn,
			      ...
			      False,0)) {
		DEBUG1(("too many connections - rejected\n"));
		conn_free(conn);
		*status = NT_STATUS_INSUFFICIENT_RESOURCES;
		return NULL;
	}  
	...
	if (*lp_rootpreexec(snum)) {
		...
		if (ret != 0 && lp_rootpreexec_close(snum)) {
			DEBUG1(("root preexec gave %d - failing "
				 "connection\n", ret));
...
/* USER Activites: */
	if (!change_to_user(conn, conn->vuid)) {
		...
		DEBUG0(("Can't become connected user!\n"));
		yield_connection(conn, lp_servicename(snum));
		conn_free(conn);
	...
	if (*lp_preexec(snum)) {
		...
		if (ret != 0 && lp_preexec_close(snum)) {
			DEBUG1(("preexec gave %d - failing connection\n",
				 ret));
			change_to_root_user();
			yield_connection(conn, lp_servicename(snum));
			conn_free(conn);
	...
	if (SMB_VFS_CONNECT(conn, lp_servicename(snum), user) < 0) {
		DEBUG0(("make_connection: VFS make connection failed!\n"));
		change_to_root_user();
		yield_connection(conn, lp_servicename(snum));
		conn_free(conn);
	...
	if ((ret = SMB_VFS_STAT(conn, conn->connectpath, &st)) != 0 ||
		...
		if (ret == 0 && !S_ISDIR(st.st_mode)) {
			DEBUG0(("'%s' is not a directory, when connecting to "
				 "[%s]\n", conn->connectpath,
				 lp_servicename(snum)));
		} else {
			DEBUG0(("'%s' does not exist or permission denied "
					      ...
					      NTSTATUS *status)
{
	...
	if ( conn && vfs_ChDir(conn,conn->connectpath) != 0 ) {
		DEBUG0(("move_driver_to_download_area: Can't change "
				   ...
				   NTSTATUS *status)
{
	...
	if (!non_root_mode() && (euid = geteuid()) != 0) {
		DEBUG0(("make_connection: PANIC ERROR. Called as nonroot "
			 "(%u)\n", (unsigned int)euid ));
	...
	if(lp_security() != SEC_SHARE) {
		...
		if (!vuser) {
			DEBUG1(("make_connection: refusing to connect with "
				 "no session setup\n"));
			*status = NT_STATUS_ACCESS_DENIED;
			return NULL;
		}
	...
	*/
	...
	if (strequal(service_in,HOMES_NAME)) {
		if(lp_security() != SEC_SHARE) {
			...
			if (vuser->homes_snum == -1) {
				DEBUG2( ("[homes] share not available for "
			...
			DEBUG5( ("making a connection to [homes] service "
			...
			if (snum != -1) {
				DEBUG5( ("making a connection to 'homes' "
					  "service %s based on "
					  "security=share\n", service_in));
		...
		DATA_BLOB no_pw = data_blob(NULL, 0);
		DEBUG5( ("making a connection to 'homes' service [%s] "
			  "created at session setup time\n", service_in));
		return make_connection_snum(vuser->homes_snum,
					    vuser, no_pw, 
					    dev, status);
	...
	if (snum < 0) {
		if (strequal(service,"IPC$") ||
			...
			DEBUG3(("refusing IPC connection to %s\n", service));
			*status = NT_STATUS_ACCESS_DENIED;
			return NULL;
		}

		DEBUG0(("%s (%s) couldn't find service %s\n",
	...
	if (lp_host_msdfs() && (*lp_msdfs_proxy(snum) != '\0'))  {
		DEBUG3( ("refusing connection to dfs proxy share '%s' "
			  "(pointing to %s)\n", 
	...
	DEBUG5( ("making a connection to 'normal' service %s\n", service));
