				...
				char *private_data, size_t private_len)
{
	...
	if(msg == NULL) {
		DEBUG0(("push_message: malloc fail (1)\n"));
		return False;
	}

	msg->buf = data_blob_talloc(msg, buf, msg_len);
	if(msg->buf.data == NULL) {
		DEBUG0(("push_message: malloc fail (2)\n"));
		TALLOC_FREE(msg);
		return False;
	}
	...
	if (private_data) {
		...
		if (msg->private_data.data == NULL) {
			DEBUG0(("push_message: malloc fail (3)\n"));
			TALLOC_FREE(msg);
			return False;
		}
	}
	...
	DEBUG10(("push_message: pushed message length %u on "
...
void remove_deferred_open_smb_message(uint16 mid)
{
	...
	for (pml = deferred_open_queue; pml; pml = pml->next) {
		if (mid == SVAL(pml->buf.data,smb_mid)) {
			DEBUG10(("remove_sharing_violation_open_smb_message: "
...
void schedule_deferred_open_smb_message(uint16 mid)
{
	...
	for (pml = deferred_open_queue; pml; pml = pml->next) {
		...
		DEBUG10(("schedule_deferred_open_smb_message: [%d] msg_mid = %u\n", i++,
			(unsigned int)msg_mid ));
		if (mid == msg_mid) {
			DEBUG10(("schedule_deferred_open_smb_message: scheduling mid %u\n",
				mid ));
	...
	DEBUG10(("schedule_deferred_open_smb_message: failed to find message mid %u\n",
		mid ));
}
			       ...
			       char *private_data, size_t priv_len)
{
	...
	DEBUG10(("push_deferred_open_smb_message: pushing message len %u mid %u "
		  "timeout time [%u.%06u]\n",
...
{
	DEBUG10(("async_processing: Doing async processing.\n"));
...
static BOOL receive_message_or_smb(char *buffer, int buffer_len, int timeout)
{
 ...
 again:
  	...
  	if(deferred_open_queue != NULL) {
		...
		if (timeval_is_zero(&msg->end_time)) {
			...
			if (tdif <= 0) {
				/* Timed out. Schedule...*/
				pop_message = True;
				DEBUG10(("receive_message_or_smb: queued message timed out.\n"));
			} else {
				/* Make a more accurate select timeout. */
				to.tv_sec = tdif / 1000000;
				to.tv_usec = tdif % 1000000;
				DEBUG10(("receive_message_or_smb: select with timeout of [%u.%06u]\n",
		...
		if (pop_message) {
			...
			DEBUG5(("receive_message_or_smb: returning deferred open smb message.\n"));
			return True;
		}
	}
	...
	if (oplock_message_waiting(&r_fds)) {
		DEBUG10(("receive_message_or_smb: oplock_message is waiting.\n"));
...
void respond_to_all_remaining_local_messages(void)
{
	...
	if(get_number_of_exclusive_open_oplocks()) {
		DEBUG0(("respond_to_all_remaining_local_messages: PANIC : we have %d exclusive oplocks.\n",
...
static void smb_dump(const char *name, int type, char *data, ssize_t len)
{
	...
	if (fd != -1) {
		...
		if (ret != len)
			DEBUG0(("smb_dump: problem: write returned %d\n", (int)ret ));
		close(fd);
		DEBUG0(("created %s len %lu\n", fname, (unsigned long)len));
	}
}
...
static int switch_message(int type,char *inbuf,char *outbuf,int size,int bufsize)
{
	...
	if ((strncmp(smb_base(inbuf),"\377SMB",4) != 0) || (size < (smb_size - 4))) {
		DEBUG2(("Non-SMB packet of length %d. Terminating server\n",smb_len(inbuf)));
	...
	if (smb_messages[type].fn == NULL) {
		...
		DEBUG3(("switch message %s (pid %d) conn 0x%lx\n",smb_fn_name(type),(int)pid,(unsigned long)conn));
...
static void process_smb(char *inbuf, char *outbuf)
{
	...
	if (trans_num == 0) {
		...
		if (!check_access(smbd_server_fd(), lp_hostsallow(-1),
			...
			DEBUG1( ( "Connection denied from %s\n", client_addr() ) );
			(void)send_smb(smbd_server_fd(),(char *)buf);
			exit_server_cleanly("connection denied");
		}
	}

	DEBUG6( ( "got message type 0x%x of len 0x%x\n", msg_type, len ) );
	DEBUG3( ( "Transaction %d of length %d\n", trans_num, nread ) );
	...
	if(nread > 0) {
		...
		if (nread != smb_len(outbuf) + 4) {
			DEBUG0(("ERROR: Invalid message response size! %d %d\n",
...
int chain_reply(char *inbuf,char *outbuf,int size,int bufsize)
{
	...
	if (new_size < 0) {
		DEBUG0(("chain_reply: chain packet size incorrect (orig size = %d, "
			"offset = %d)\n",
			size,
			(inbuf2 - inbuf) ));
...
void check_reload(time_t t)
{
	...
	{ 
		...
		{
			DEBUG3(( "Printcap cache time expired.\n"));
			reload_printers();
			last_printer_reload_time = t;
		}
	}
...
static BOOL timeout_processing(int deadtime, int *select_timeout, time_t *last_timeout_processing_time)
{
	...
	if (smb_read_error == READ_EOF) {
		DEBUG3(("timeout_processing: End of file from client (client has disconnected).\n"));
		return False;
	}

	if (smb_read_error == READ_ERROR) {
		DEBUG3(("timeout_processing: receive_smb error (%s) Exiting\n",
			strerror(errno)));
		return False;
	}

	if (smb_read_error == READ_BAD_SIG) {
		DEBUG3(("timeout_processing: receive_smb error bad smb signature. Exiting\n"));
		return False;
	}
	...
	*last_timeout_processing_time = t = time(NULL);
	...
	if (conn_num_open()==0 && (t - last_idle_closed_check) >= IDLE_CLOSED_TIMEOUT) {
		DEBUG2( ( "Closing idle connection\n" ) );
		return False;
	} else {
		last_idle_closed_check = t;
	}
	...
	if (keepalive && (t - last_keepalive_sent_time)>keepalive) {
		if (!send_keepalive(smbd_server_fd())) {
			DEBUG2( ( "Keepalive failed - exiting.\n" ) );
			return False;
		}
	...
	if (allidle && conn_num_open()>0) {
		DEBUG2(("Closing idle connection 2.\n"));
		return False;
	}
	...
	if(global_machine_password_needs_changing && 
		...
		if (secrets_lock_trust_account_password(lp_workgroup(), True) == False) {
			DEBUG0(("process: unable to lock the machine account password for \
machine %s in domain %s.\n", global_myname(), lp_workgroup() ));
			return True;
		}

		if(!secrets_fetch_trust_account_password(lp_workgroup(), trust_passwd_hash, &lct, NULL)) {
			DEBUG0(("process: unable to read the machine account password for \
machine %s in domain %s.\n", global_myname(), lp_workgroup()));
