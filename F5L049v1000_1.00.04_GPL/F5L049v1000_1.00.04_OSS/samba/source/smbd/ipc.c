				...
				char *rdata, int data_offset, int data_len)
{
	...
	DEBUG5(("copy_trans_params_and_data: params[%d..%d] data[%d..%d]\n",
				...
				BOOL buffer_too_large)
{
	...
	if (buffer_too_large)
		DEBUG5(("send_trans_reply: buffer %d too large\n", ldata ));
...
static BOOL api_rpc_trans_reply(char *outbuf, smb_np_struct *p)
{
	...
	if(rdata == NULL) {
		DEBUG0(("api_rpc_trans_reply: malloc fail.\n"));
		return False;
	}
...
static BOOL api_WNPHS(char *outbuf, smb_np_struct *p, char *param, int param_len)
{
	...
	DEBUG4(("WaitNamedPipeHandleState priority %x\n", priority));
...
static BOOL api_SNPHS(char *outbuf, smb_np_struct *p, char *param, int param_len)
{
	...
	id = SVAL(param,0);
	DEBUG4(("SetNamedPipeHandleState to code %x\n", id));
...
static BOOL api_no_reply(char *outbuf, int max_rdata_len)
{
	...
	DEBUG3(("Unsupported API fd command\n"));
		 	...
		 	int suwcnt,int tdscnt,int tpscnt,int mdrcnt,int mprcnt)
{
	...
	if (suwcnt != 2) {
		DEBUG0(("Unexpected named pipe transaction.\n"));
	...
	if(!(p = get_rpc_pipe(pnum))) {
		if (subcommand == TRANSACT_WAITNAMEDPIPEHANDLESTATE) {
			...
			DEBUG3(("Got TRANSACT_WAITNAMEDPIPEHANDLESTATE on text pipe name\n"));
			send_trans_reply(outbuf, NULL, 0, NULL, 0, False);
			return -1;
		}

		DEBUG1(("api_fd_reply: INVALID PIPE HANDLE: %x\n", pnum));
		return ERROR_NT(NT_STATUS_INVALID_HANDLE);
	}

	if (vuid != p->vuid) {
		DEBUG1( ("Got pipe request (pnum %x) using invalid VUID %d, "
			  "expected %d\n", pnum, vuid, p->vuid));
		return ERROR_NT(NT_STATUS_INVALID_HANDLE);
	}

	DEBUG3(("Got API command 0x%x on pipe \"%s\" (pnum %x)\n", subcommand, p->name, pnum));

	/* record maximum data length that can be transmitted in an SMBtrans */
	p->max_trans_reply = mdrcnt;

	DEBUG10(("api_fd_reply: p:%p max_trans_reply: %d\n", p, p->max_trans_reply));
...
{
	DEBUG3(("named pipe command on <%s> name\n", name));
	...
	{
		DEBUG4(("named pipe command from Win95 (wow!)\n"));
	...
	if (setup)
		DEBUG3(("unknown named pipe: setup 0x%X setup1=%d\n", (int)setup[0],(int)setup[1]));
			     ...
			     char *outbuf, int *outsize)
{
	...
	DEBUG3(("trans <%s> data=%u params=%u setup=%u\n",
		...
		int size, int bufsize)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG2( ("Got invalid trans request: %s\n",
			  nt_errstr(result)));
	...
	if (state->total_data)  {
		...
		if (state->data == NULL) {
			DEBUG0(("reply_trans: data malloc fail for %u "
	...
	if (state->setup_count) {
		...
		if((state->setup = TALLOC_ARRAY(
			...
			DEBUG0(("reply_trans: setup malloc fail for %u "
				 "bytes !\n", (unsigned int)
  ...
  bad_param:
	...
	DEBUG0(("reply_trans: invalid trans parameters\n"));
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
	END_PROFILE(SMBtrans);
		 ...
		 int size, int bufsize)
{
  ...
  bad_param:
	...
	DEBUG0(("reply_transs: invalid trans parameters\n"));
	DLIST_REMOVE(conn->pending_trans, state);
	SAFE_FREE(state->data);
	SAFE_FREE(state->param);
	TALLOC_FREE(state);
