...
BOOL session_init(void)
{
	...
	tdb = tdb_open_log(lock_path("sessionid.tdb"), 0, TDB_CLEAR_IF_FIRST|TDB_DEFAULT, 
		       O_RDWR | O_CREAT, 0644);
	if (!tdb) {
		DEBUG1(("session_init: failed to open sessionid tdb\n"));
		return False;
	}
...
BOOL session_claim(user_struct *vuser)
{
	...
	if (lp_utmp()) {
		...
		if (i == MAX_SESSION_ID) {
			DEBUG1(("session_claim: out of session IDs (max is %d)\n", 
				 MAX_SESSION_ID));
			return False;
		}
	...
	*/
	...
	if (!smb_pam_claim_session(sessionid.username, sessionid.id_str, sessionid.hostname)) {
		DEBUG1(("pam_session rejected the session for %s [%s]\n",
	...
	if (tdb_store(tdb, key, data, tdb_store_flag) != 0) {
		DEBUG1(("session_claim: unable to create session id record\n"));
		return False;
	}
	...
	if (!vuser->session_keystr) {
		DEBUG0( ("session_claim:  strdup() failed for session_keystr\n"));
		return False;
	}
	return True;
}
...
static int gather_sessioninfo(TDB_CONTEXT *stdb, TDB_DATA kbuf, TDB_DATA dbuf, void *state)
{
	...
	DEBUG7(("gather_sessioninfo session from %s@%s\n", 
...
int list_sessions(struct sessionid **session_list)
{
	...
	if (!session_traverse(gather_sessioninfo, (void *) &sesslist)) {
		DEBUG3( ("Session traverse failed\n"));
