		   ...
		   const char *passwordprogram, BOOL as_root)
{
	...
	{
		DEBUG3( ("More weirdness, could not open %s\n", slavedev));
		return (False);
	}
	...
	{
		DEBUG3( ("Error in ioctl call for slave pty\n"));
		/* return(False); */
	}
	...
	{
		DEBUG3( ("Could not re-direct stdin\n"));
		return (False);
	}
	if (sys_dup2(slave, STDOUT_FILENO) != STDOUT_FILENO)
	{
		DEBUG3( ("Could not re-direct stdout\n"));
		return (False);
	}
	if (sys_dup2(slave, STDERR_FILENO) != STDERR_FILENO)
	{
		DEBUG3( ("Could not re-direct stderr\n"));
		return (False);
	}
	if (slave > 2)
		close(slave);
	...
	{
		DEBUG3( ("could not set attributes of pty\n"));
		return (False);
	}
	...
	{
		DEBUG3( ("Bad status returned from %s\n", passwordprogram));
		return (False);
	}
	return (True);
}
	...
	for (attempts = 0; attempts < 2; attempts++) {
		if (!strequal(issue, ".")) {
			...
			if ((len = sys_write(master, issue, strlen(issue))) != strlen(issue)) {
				DEBUG2(("expect: (short) write returned %d\n", len ));
				return False;
			}
		}
			      ...
			      char *chatsequence, BOOL as_root)
{
	...
	if (pass == NULL) {
		DEBUG0( ("chat_with_program: user doesn't exist in the UNIX password database.\n"));
		return False;
	}

	/* allocate a pseudo-terminal device */
	if ((master = findpty(&slavedev)) < 0) {
		DEBUG3( ("chat_with_program: Cannot Allocate pty for password change: %s\n", pass->pw_name));
		return (False);
	}
	...
	if ((pid = sys_fork()) < 0) {
		DEBUG3( ("chat_with_program: Cannot fork() child for password change: %s\n", pass->pw_name));
		close(master);
		CatchChild();
		return (False);
	}
	...
	if (pid > 0) {			/* This is the parent process */
		if ((chstat = talktochild(master, chatsequence)) == False) {
			DEBUG3( ("chat_with_program: Child failed to change password: %s\n", pass->pw_name));
		...
		if (wpid < 0) {
			DEBUG3( ("chat_with_program: The process is no longer waiting!\n\n"));
			close(master);
			CatchChild();
			return (False);
		}
		...
		if (pid != wpid) {
			DEBUG3( ("chat_with_program: We were waiting for the wrong process ID\n"));
			return (False);
		}
		if (WIFEXITED(wstat) && (WEXITSTATUS(wstat) != 0)) {
			DEBUG3( ("chat_with_program: The process exited with status %d \
while we were waiting\n", WEXITSTATUS(wstat)));
			return (False);
		}
#if defined(WIFSIGNALLED) && defined(WTERMSIG)
		else if (WIFSIGNALLED(wstat)) {
                        DEBUG3( ("chat_with_program: The process was killed by signal %d \
while we were waiting\n", WTERMSIG(wstat)));
			return (False);
		}
#endif
		...
		DEBUG3( ("chat_with_program: Dochild for user %s (uid=%d,gid=%d) (as_root = %s)\n", pass->pw_name,
		...
		DEBUG0( ("chat_with_program: Error: dochild() returned %d\n", chstat));
		exit(1);
	}

	if (chstat)
		DEBUG3( ("chat_with_program: Password change %ssuccessful for user %s\n",
	       ...
	       const char *oldpass, const char *newpass, BOOL as_root)
{
	...
	DEBUG3( ("chgpasswd: Password change (as_root=%s) for user: %s\n", BOOLSTR(as_root), name));
	...
	if (strcmp(oldpass, newpass) == 0) {
		...
		DEBUG2( ("chgpasswd: Password Change: %s, New password is same as old\n", name));	/* log the attempt */
	...
	for (i = 0; i < len; i++) {
		if (iscntrl((int)oldpass[i])) {
			DEBUG0( ("chgpasswd: oldpass contains control characters (disallowed).\n"));
			return False;
		}
	}
	...
	for (i = 0; i < len; i++) {
		if (iscntrl((int)newpass[i])) {
			DEBUG0( ("chgpasswd: newpass contains control characters (disallowed).\n"));
			return False;
		}
	}
	...
	if (pass == NULL) {
		DEBUG0( ("chgpasswd: user %s doesn't exist in the UNIX password database.\n", name));
		return False;
	}
	...
	if (!*chatsequence) {
		DEBUG2( ("chgpasswd: Null chat sequence - no password changing\n"));
		return (False);
	}

	if (!*passwordprogram) {
		DEBUG2( ("chgpasswd: Null password program - no password changing\n"));
		return (False);
	}
	...
	if (as_root) {
		...
		if (strstr_m(passwordprogram, "%u") == NULL) {
			DEBUG0(("chgpasswd: Running as root the 'passwd program' parameter *MUST* contain \
...
BOOL chgpasswd(const char *name, const struct passwd *pass, 
	       const char *oldpass, const char *newpass, BOOL as_root)
{
	DEBUG0( ("chgpasswd: Unix Password changing not compiled in (user=%s)\n", name));
	return (False);
}
			   ...
			   uchar * pass2, struct samu **hnd)
{
	...
	if (ret == False) {
		DEBUG0(("check_lanman_password: getsampwnam returned NULL\n"));
		TALLOC_FREE(sampass);
		return False;
	}
	...
	if (acct_ctrl & ACB_DISABLED) {
		DEBUG0(("check_lanman_password: account %s disabled.\n", user));
		TALLOC_FREE(sampass);
		return False;
	}
	...
	if (lanman_pw == NULL) {
		if (acct_ctrl & ACB_PWNOTREQ) {
			...
			*hnd = sampass;
			...
			DEBUG0( ("check_lanman_password: no lanman password !\n"));
			TALLOC_FREE(sampass);
			return False;
		}
	}
	...
	if (memcmp(lanman_pw, unenc_old_pw, 16)) {
		DEBUG0(("check_lanman_password: old password doesn't match.\n"));
		TALLOC_FREE(sampass);
		return False;
	}
...
BOOL change_lanman_password(struct samu *sampass, uchar *pass2)
{
	...
	if (sampass == NULL) {
		DEBUG0(("change_lanman_password: no smb password entry.\n"));
		return False;
	}
	...
	if (acct_ctrl & ACB_DISABLED) {
		DEBUG0(("change_lanman_password: account %s disabled.\n",
	...
	if (pwd == NULL) { 
		if (acct_ctrl & ACB_PWNOTREQ) {
			...
			DEBUG0(("change_lanman_password: no lanman password !\n"));
			return False;
		}
	} else {
		/* Get the new lanman hash. */
				   ...
				   int new_passwd_size)
{
	...
	*hnd = NULL;
	...
	if (ret == False) {
		DEBUG0( ("check_oem_password: getsmbpwnam returned NULL\n"));
		TALLOC_FREE(sampass);
	...
	if (acct_ctrl & ACB_DISABLED) {
		DEBUG2(("check_lanman_password: account %s disabled.\n", user));
		TALLOC_FREE(sampass);
	...
	if (nt_pw && nt_pass_set) {
		...
		DEBUG1( ("NT password change supplied for user %s, but we have no NT password to check it with\n", 
			  user));
		TALLOC_FREE(sampass);
		return NT_STATUS_WRONG_PASSWORD;	
	} else if (lm_pass_set) {
		if (lp_lanman_auth()) {
			DEBUG1( ("LM password change supplied for user %s, but we have no LanMan password to check it with\n", 
				  user));
		} else {
			DEBUG1( ("LM password change supplied for user %s, but we have disabled LanMan authentication\n", 
				  user));
		}
		TALLOC_FREE(sampass);
		return NT_STATUS_WRONG_PASSWORD;
	} else {
		DEBUG1( ("password change requested for user %s, but no password supplied!\n", 
			  user));
		TALLOC_FREE(sampass);
		return NT_STATUS_WRONG_PASSWORD;
	}
	...
	if (nt_pass_set) {
		...
		if (nt_pw) {
			...
			if (memcmp(verifier, old_nt_hash_encrypted, 16)) {
				DEBUG0(("check_oem_password: old lm password doesn't match.\n"));
				TALLOC_FREE(sampass);
				return NT_STATUS_WRONG_PASSWORD;
			}
...
static BOOL check_passwd_history(struct samu *sampass, const char *plaintext)
{
	...
	if (!memcmp(nt_pw, new_nt_p16, NT_HASH_LEN)) {
		DEBUG10(("check_passwd_history: proposed new password for user %s is the same as the current password !\n",
			pdb_get_username(sampass) ));
		return True;
	}
	...
	for (i=0; i<pwHisLen; i++) {
		...
		if (!memcmp(new_nt_pw_salted_md5_hash, old_nt_pw_salted_md5_hash, SALTED_MD5_HASH_LEN)) {
			DEBUG1(("check_passwd_history: proposed new password for user %s found in history list !\n",
				pdb_get_username(sampass) ));
			found = True;
			break;
		}
...
NTSTATUS change_oem_password(struct samu *hnd, char *old_passwd, char *new_passwd, BOOL as_root, uint32 *samr_reject_reason)
{
	...
	if (!pdb_get_pass_can_change(hnd)) {
		DEBUG1( ("user %s does not have permissions to change password\n", username));
		if (samr_reject_reason) {
	...
	if ((can_change_time != 0) && (time(NULL) < can_change_time)) {
		DEBUG1( ("user %s cannot change password now, must "
			  "wait until %s\n", username,
	...
	if (pdb_get_account_policy(AP_MIN_PASSWORD_LEN, &min_len) && (str_charnum(new_passwd) < min_len)) {
		DEBUG1( ("user %s cannot change password - password too short\n", 
			  username));
...
/* 		return NT_STATUS_PWD_TOO_SHORT; */
	...
	if (!pass) {
		DEBUG1( ("change_oem_password: Username %s does not exist in system !?!\n", username));
	...
	if (lp_check_password_script() && *(lp_check_password_script())) {
		...
		DEBUG5( ("change_oem_password: check password script (%s) returned [%d]\n", lp_check_password_script(), check_ret));

		if (check_ret != 0) {
			DEBUG1( ("change_oem_password: check password script said new password is not good enough!\n"));
			if (samr_reject_reason) {
