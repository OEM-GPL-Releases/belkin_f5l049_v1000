				...
				unsigned int uLevel, char *id1, char *id2)
{
	...
	switch( uLevel ) {
		...
		case 52:
			...
			break;
		default:
			DEBUG0(("check_printq_info: invalid level %d\n",
				uLevel ));
			return False;
	}
	if (id1 == NULL || strcmp(desc->format,id1) != 0) {
		DEBUG0(("check_printq_info: invalid format %s\n",
			id1 ? id1 : "<NULL>" ));
		return False;
	}
	if (desc->subformat && (id2 == NULL || strcmp(desc->subformat,id2) != 0)) {
		DEBUG0(("check_printq_info: invalid subformat %s\n",
			id2 ? id2 : "<NULL>" ));
		return False;
	}
	return True;
				...
				struct pack_desc* desc,	int count )
{
	...
	if ( !W_ERROR_IS_OK(get_a_printer( NULL, &printer, 2, lp_servicename(snum))) ) {
		DEBUG3(("fill_printq_info_52: Failed to lookup printer [%s]\n", 
			lp_servicename(snum)));
		goto err;
	}
	...
	if ( !W_ERROR_IS_OK(get_a_printer_driver(&driver, 3, printer->info_2->drivername, 
		"Windows 4.0", 0)) )
	{
		DEBUG3(("fill_printq_info_52: Failed to lookup driver [%s]\n", 
	...
	DEBUG3(("Printer Driver Name: %s:\n",driver.info_3->name));
	DEBUG3(("Driver: %s:\n",driver.info_3->driverpath));
	DEBUG3(("Data File: %s:\n",driver.info_3->datafile));
	DEBUG3(("Language Monitor: %s:\n",driver.info_3->monitorname));
	DEBUG3(("Driver Location: %s:\n",location));
	...
	for ( i=0; i<count && driver.info_3->dependentfiles && *driver.info_3->dependentfiles[i]; i++) 
	{
		...
		DEBUG3(("Dependent File: %s:\n",driver.info_3->dependentfiles[i]));
	}

	/* sanity check */
	if ( i != count )
		DEBUG3(("fill_printq_info_52: file count specified by client [%d] != number of dependent files [%i]\n",
			count, i));

	DEBUG3(("fill_printq_info on <%s> gave %d entries\n", SERVICE(snum),i));

        desc->errcode=NERR_Success;
	goto done;

err:
	DEBUG3(("fill_printq_info: Can't supply driver files\n"));
...
static int get_printerdrivernumber(int snum)
{
	...
	if ( !W_ERROR_IS_OK(get_a_printer( NULL, &printer, 2, lp_servicename(snum))) ) {
		DEBUG3(("get_printerdrivernumber: Failed to lookup printer [%s]\n", 
			lp_servicename(snum)));
		goto done;
	}
	...
	if ( !W_ERROR_IS_OK(get_a_printer_driver(&driver, 3, printer->info_2->drivername, 
		"Windows 4.0", 0)) )
	{
		DEBUG3(("get_printerdrivernumber: Failed to lookup driver [%s]\n", 
			printer->info_2->drivername));
		goto done;
	}
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("api_DosPrintQGetInfo uLevel=%d name=%s\n",uLevel,QueueName));
	...
	if (uLevel==52) {
		...
		DEBUG3(("api_DosPrintQGetInfo: Driver files count: %d\n",count));
	} else {
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("printqgetinfo: errorcode %d\n",desc.errcode));
				...
				int *rdata_len, int *rparam_len)
{
	...
	if((queue = SMB_MALLOC_ARRAY(print_queue_struct*, queuecnt)) == NULL) {
		DEBUG0(("api_DosPrintQEnum: malloc fail !\n"));
		goto err;
	}
	...
	for (i=0;lines[i];i++) {
		...
		if (count == alloced) {
			...
			*servers = SMB_REALLOC_ARRAY(*servers,struct srv_info_struct, alloced);
			if (!*servers) {
				DEBUG0(("get_server_info: failed to enlarge servers info struct!\n"));
		...
		if (ok) {
			DEBUG4(("**SV** %20s %8x %25s %15s\n",
				s->name, s->type, s->comment, s->domain));
			s->server_added = True;
			count++;
		} else {
			DEBUG4(("%20s %8x %25s %15s\n",
				s->name, s->type, s->comment, s->domain));
		}
	}
				...
				char **rparam, int *rdata_len, int *rparam_len)
{
	...
	DEBUG4( ("server request level: %s %8x ", str2, servertype));
	DEBUG4( ("domains_req:%s ", BOOLSTR(domain_request)));
...

	{
		...
		for (i=0;i<total;i++) {
			...
			DEBUG4(("fill_srv_info %20s %8x %25s %15s\n",
				s->name, s->type, s->comment, s->domain));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG3(("NetServerEnum domain = %s uLevel=%d counted=%d total=%d\n",
				...
				char **rparam, int *rdata_len, int *rparam_len)
{
	...
	DEBUG5(("RNetGroupGetUsers: %s %s %s %d %d\n",
				...
				int               *rparam_len )
{
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG3(("RNetShareEnum gave %d entries of %d (%d %d %d %d)\n",
				...
				int *rdata_len,int *rparam_len)
{
	...
	if (command) {
		DEBUG10(("api_RNetShareAdd: Running [%s]\n", command ));

		if ((res = smbrun(command, NULL)) != 0) {
			DEBUG1(("api_RNetShareAdd: Running [%s] returned (%d)\n", command, res ));
			SAFE_FREE(command);
			res = ERRnoaccess;
			goto error_exit;
		} else {
				...
				int *rdata_len,int *rparam_len)
{
	...
	if (search == NULL) {
		DEBUG3(("api_RNetGroupEnum:failed to get group list"));
		return False;
	}

	resume_context = get_safe_SVAL(param,tpscnt,p,0,-1);
	cli_buf_size= get_safe_SVAL(param,tpscnt,p,2,0);
	DEBUG10(("api_RNetGroupEnum:resume context: %d, client buffer size: "
	...
	*rdata = SMB_REALLOC_LIMIT(*rdata,*rdata_len);
	...
	for(i=0; i<num_entries; i++) {
		...
		if( ((PTR_DIFF(p,*rdata)+21) <= *rdata_len) ) {
			...
			memcpy(p, name, 21); 
			DEBUG10(("adding entry %d group %s\n", i, p));
			p += 21;
			p += 5; /* Both NT4 and W2k3SP1 do padding here.
				   No idea why... */
		} else {
			/* set overflow error */
			DEBUG3(("overflow on entry %d group %s\n", i, name));
			errflags=234;
			break;
		}
	}
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rdata = SMB_REALLOC_LIMIT(*rdata,*rdata_len);
	...
	if (!lookup_name(mem_ctx, UserName, LOOKUP_NAME_ALL,
		...
		DEBUG10( ("lookup_name(%s) failed\n", UserName));
		goto done;
	}

	if (type != SID_NAME_USER) {
		DEBUG10( ("%s is a %s, not a user\n", UserName,
			   sid_type_lookup(type)));
		goto done;
	}

	if ( !pdb_getsampwsid(sampw, &user_sid) ) {
		DEBUG10( ("pdb_getsampwsid(%s) failed for user %s\n",
				...
				int *rdata_len,int *rparam_len)
{
	...
	resume_context = get_safe_SVAL(param,tpscnt,p,0,-1);
	cli_buf_size= get_safe_SVAL(param,tpscnt,p,2,0);
	DEBUG10(("api_RNetUserEnum:resume context: %d, client buffer size: %d\n",
			resume_context, cli_buf_size));
	...
	*rdata = SMB_REALLOC_LIMIT(*rdata,*rdata_len);
	...
	unbecome_root();
	if (search == NULL) {
		DEBUG0( ("api_RNetUserEnum:unable to open sam database.\n"));
		return False;
	}
	...
	for (i=0; i<num_users; i++) {
		...
		if(((PTR_DIFF(p,*rdata)+21)<=*rdata_len)&&(strlen(name)<=21)) {
			pstrcpy(p,name); 
			DEBUG10(("api_RNetUserEnum:adding entry %d username "
				  "%s\n",count_sent,p));
			p += 21; 
			count_sent++; 
		} else {
			/* set overflow error */
			DEBUG10(("api_RNetUserEnum:overflow on entry %d "
				  "username %s\n",count_sent,name));
			errflags=234;
			break;
		}
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	if(!strequal(p, "zsT")) {
		DEBUG0(("api_SamOEMChangePassword: Invalid parameter string %s\n", p));
		return False;
	}
	p = skip_string(param, tpscnt, p);
	if (!p) {
	...
	if(!strequal(p, "B516B16")) {
		DEBUG0(("api_SamOEMChangePassword: Invalid data parameter string %s\n", p));
		return False;
	}
	p = skip_string(param,tpscnt,p);
	if (!p) {
	...
	DEBUG3(("api_SamOEMChangePassword: Change password for <%s>\n",user));
			       ...
			       int uLevel, char* id)
{
	...
	default:
		DEBUG0(("check_printjob_info: invalid level %d\n",
			uLevel ));
		return False;
	}
	if (id == NULL || strcmp(desc->format,id) != 0) {
		DEBUG0(("check_printjob_info: invalid format %s\n",
			id ? id : "<NULL>" ));
		return False;
	}
	return True;
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	if (!share_defined(sharename)) {
		DEBUG0(("api_PrintJobInfo: sharen [%s] not defined\n",
			 sharename));
		return False;
	}
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG4(("NetServerGetInfo level %d\n",uLevel));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG4(("NetWkstaGetInfo level %d\n",level));
				...
				int *rdata_len,int *rparam_len)
{
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("RNetUserGetInfo level=%d\n", uLevel));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WWkstaUserLogon uLevel=%d name=%s\n",uLevel,name));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("WWkstaUserLogon: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WAccessGetUserPerms user=%s resource=%s\n",user,resource));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WPrintJobGetInfo uLevel=%d uJobId=0x%X\n",uLevel,SVAL(p,0)));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("WPrintJobGetInfo: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WPrintJobEnumerate uLevel=%d name=%s\n",uLevel,name));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("WPrintJobEnumerate: errorcode %d\n",desc.errcode));
				...
				int uLevel, char* id)
{
	...
	switch( uLevel ) {
		...
		case 3:
			...
			break;
		default:
			DEBUG0(("check_printdest_info: invalid level %d\n",
				uLevel));
			return False;
	}
	if (id == NULL || strcmp(desc->format,id) != 0) {
		DEBUG0(("check_printdest_info: invalid string %s\n", 
			id ? id : "<NULL>" ));
		return False;
	}
	return True;
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WPrintDestGetInfo uLevel=%d PrinterName=%s\n",uLevel,PrinterName));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("WPrintDestGetInfo: errorcode %d\n",desc.errcode));
	SAFE_FREE(tmpdata);
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WPrintDestEnum uLevel=%d\n",uLevel));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("WPrintDestEnumerate: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WPrintDriverEnum uLevel=%d\n",uLevel));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("WPrintDriverEnum: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WPrintQProcEnum uLevel=%d\n",uLevel));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("WPrintQProcEnum: errorcode %d\n",desc.errcode));
				...
				int *rdata_len,int *rparam_len)
{
	...
	DEBUG3(("WPrintPortEnum uLevel=%d\n",uLevel));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("WPrintPortEnum: errorcode %d\n",desc.errcode));
...

{
	...
	DEBUG3(("RNetSessionEnum uLevel=%d\n",uLevel));
	DEBUG7(("RNetSessionEnum req string=%s\n",str1));
	DEBUG7(("RNetSessionEnum ret string=%s\n",str2));
	...
	*rparam = SMB_REALLOC_LIMIT(*rparam,*rparam_len);
	...
	DEBUG4(("RNetSessionEnum: errorcode %d\n",desc.errcode));
			 ...
			 int *rdata_len, int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	DEBUG3(("Supplied buffer too small in API command\n"));
				...
				int *rdata_len, int *rparam_len)
{
	...
	*rdata_len = 0;
	...
	DEBUG3(("Unsupported API command\n"));
		     ...
		     int tdscnt,int tpscnt,int mdrcnt,int mprcnt)
{
	...
	if (!params) {
		DEBUG0(("ERROR: NULL params in api_reply()\n"));
		return 0;
	}
	...
	DEBUG3(("Got API command %d of form <%s> <%s> (tdscnt=%d,tpscnt=%d,mdrcnt=%d,mprcnt=%d)\n",
		api_command,
		name1,
		name2,
