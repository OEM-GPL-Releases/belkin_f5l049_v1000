...
static void mangled_map(char *s, const char *MangledMap)
{
	...
	DEBUG5( ("Mangled Mapping '%s' map '%s'\n", s, MangledMap) );
	while( *start ) {
		while( (*start) && (*start != '(') )
			start++;
		if( !*start )
		...
		DEBUG5( ("Start of first in pair '%s'\n", start) );
		while( (*end) && !((*end == ' ') || (*end == ')')) )
			end++;
		if( !*end ) {
			start = end;
		...
		DEBUG5( ("End of first in pair '%s'\n", end) );
		if( (match_string = map_filename( s, start, end-start )) ) {
			int size_left = sizeof(new_string) - 1;
			DEBUG5( ("Found a match\n") );
			/* Found a match. */
			start = end + 1; /* Point to start of what it is to become. */
			DEBUG5( ("Start of second in pair '%s'\n", start) );
			end = start;
			np = new_string;
			...
			DEBUG5(("End of second in pair '%s'\n", end));
