				...
				BOOL *ppath_contains_wcard)
{
	...
	if (*pathname != sepchar) {
		DEBUG10(("parse_dfs_path: path %s doesn't start with %c\n",
			pathname, sepchar ));
		/*
		...
		p = temp;
		DEBUG10(("parse_dfs_path: trying to convert %s to a local path\n",
			temp));
		goto local_path;
	}
	...
	DEBUG10(("parse_dfs_path: temp = |%s| after trimming %c's\n",
		temp, sepchar));
	...
	if(p == NULL) {
		DEBUG10(("parse_dfs_path: can't parse hostname from path %s\n",
			temp));
		/*
		 * Possibly client sent a local path by mistake.
		 * Try and convert to a local path.
	...
	*p = '\0';
	...
	DEBUG10(("parse_dfs_path: hostname: %s\n",pdp->hostname));
	...
	/* Is this really our servicename ? */
		...
		DEBUG10(("parse_dfs_path: %s is not our servicename\n",
			servicename));
		...
		*servicename = sepchar;
		...
		p = temp;
		DEBUG10(("parse_dfs_path: trying to convert %s "
			"to a local path\n",
			temp));
		goto local_path;
	}
	...
	DEBUG10(("parse_dfs_path: servicename: %s\n",pdp->servicename));
  ...
  local_path:
	...
	*ppath_contains_wcard = False;
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG10(("parse_dfs_path: '%s' failed with %s\n",
			p, nt_errstr(status) ));
		return status;
	}

	DEBUG10(("parse_dfs_path: rest of the path: %s\n",pdp->reqpath));
	return NT_STATUS_OK;
}
...
static NTSTATUS create_conn_struct(connection_struct *conn, int snum, const char *path)
{
	...
	if ((conn->mem_ctx=talloc_init("connection_struct")) == NULL) {
		DEBUG0(("talloc_init(connection_struct) failed!\n"));
	...
	if (!smbd_vfs_init(conn)) {
		...
		DEBUG0(("create_conn_struct: smbd_vfs_init failed.\n"));
	...
	if (vfs_ChDir(conn,conn->connectpath) != 0) {
		NTSTATUS status = map_nt_error_from_unix(errno);
		DEBUG3(("create_conn_struct: Can't ChDir to new conn path %s. Error was %s\n",
				...
				int *refcount)
{
	...
	prot = strtok(temp,":");
	if (!prot) {
		DEBUG0(("parse_msdfs_symlink: invalid path !\n"));
		return False;
	}
	...
	DEBUG10(("parse_msdfs_symlink: count=%d\n", count));

	if (count) {
		reflist = *preflist = TALLOC_ZERO_ARRAY(ctx, struct referral, count);
		if(reflist == NULL) {
			DEBUG0(("parse_msdfs_symlink: talloc failed!\n"));
			return False;
		}
	} else {
	...
	for(i=0;i<count;i++) {
		...
		DEBUG10( ("parse_msdfs_symlink: Created alt path: %s\n", reflist[i].alternate_path));
		*refcount += 1;
	}
			...
			SMB_STRUCT_STAT *sbufp)
{
	...
	if (SMB_VFS_LSTAT(conn, path, sbufp) != 0) {
		DEBUG5(("is_msdfs_link: %s does not exist.\n",path));
		return False;
	}
  
	if (!S_ISLNK(sbufp->st_mode)) {
		DEBUG5(("is_msdfs_link: %s is not a link.\n",path));
		return False;
	}
	...
	if (referral_len == -1) {
		DEBUG0(("is_msdfs_link: Error reading msdfs link %s: %s\n",
			path, strerror(errno)));
		return False;
	}
	link_target[referral_len] = '\0';

	DEBUG5(("is_msdfs_link: %s -> %s\n",path, link_target));
			...
			pstring targetpath)
{
	...
	DEBUG10(("dfs_path_lookup: Conn path = %s reqpath = %s\n",
	...
	if (is_msdfs_link(conn, localpath, targetpath, NULL)) {
		if (search_flag) {
			DEBUG6(("dfs_path_lookup (FindFirst) No redirection "
				 "for dfs link %s.\n", dfspath));
			return NT_STATUS_OK;
		}

		DEBUG6(("dfs_path_lookup: %s resolves to a "
			"valid dfs link %s.\n", dfspath, targetpath));
	...
	while (p) {
		*p = '\0';
		...
		if (is_msdfs_link(conn, localpath, targetpath, NULL)) {
			DEBUG4( ("dfs_path_lookup: Redirecting %s because "
				"parent %s is dfs link\n", dfspath, localpath));

			if (consumedcntp) {
				*consumedcntp = strlen(canon_dfspath);
				DEBUG10( ("dfs_path_lookup: Path consumed: %s "
			...
			BOOL *ppath_contains_wcard)
{
	...
	if (dp.reqpath[0] == '\0') {
		...
		DEBUG5(("dfs_redirect: self-referral.\n"));
		return NT_STATUS_OK;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		if (NT_STATUS_EQUAL(status, NT_STATUS_PATH_NOT_COVERED)) {
			DEBUG3(("dfs_redirect: Redirecting %s\n", dfs_path));
		} else {
			DEBUG10(("dfs_redirect: dfs_path_lookup failed for %s with %s\n",
				dfs_path, nt_errstr(status) ));
		}
		return status;
	}

	DEBUG3(("dfs_redirect: Not redirecting %s.\n", dfs_path));

	/* Form non-dfs tcon-relative path */
	pstrcpy(dfs_path, dp.reqpath);

	DEBUG3(("dfs_redirect: Path converted to non-dfs path %s\n", dfs_path));
	return NT_STATUS_OK;
}
			...
			BOOL *self_referralp)
{
	...
	*self_referralp = True;
	...
	if((ref = TALLOC_ZERO_P(ctx, struct referral)) == NULL) {
		DEBUG0(("self_ref: talloc failed for referral\n"));
		return NT_STATUS_NO_MEMORY;
	}
			...
			BOOL *self_referralp)
{
	...
	*self_referralp = False;
	...
	if (!lp_msdfs_root(snum)) {
		DEBUG3(("get_referred_path: |%s| in dfs path %s is not a dfs root.\n",
	...
	if (dp.reqpath[0] == '\0') {
		...
		jucn->referral_count = 1;
		if ((ref = TALLOC_ZERO_P(ctx, struct referral)) == NULL) {
			DEBUG0( ("malloc failed for referral\n"));
			return NT_STATUS_NO_MEMORY;
		}
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_PATH_NOT_COVERED)) {
		DEBUG3(("get_referred_path: No valid referrals for path %s\n",
			dfs_path));
		conn_free_internal(conn);
		return status;
	}
	...
	if (!parse_msdfs_symlink(ctx, targetpath,
				&jucn->referral_list,
				&jucn->referral_count)) {
		DEBUG3(("get_referred_path: failed to parse symlink "
			"target %s\n", targetpath ));
		conn_free_internal(conn);
		return NT_STATUS_NOT_FOUND;
	}
				...
				BOOL self_referral)
{
	...
	DEBUG10(("Setting up version2 referral\nRequested path:\n"));
				...
				BOOL self_referral)
{
	...
	DEBUG10(("setting up version3 referral\n"));
	...
	pdata = (char *)SMB_REALLOC(pdata,reply_size);
	if(pdata == NULL) {
		DEBUG0(("version3 referral setup: malloc failed for Realloc!\n"));
		return -1;
	}
	*ppdata = pdata;
			...
			char **ppdata, NTSTATUS *pstatus)
{
	...
	*pstatus = get_referred_path(ctx, pathnamep, &junction, &consumedcnt, &self_referral);
	...
	case 3:
		...
		break;
	default:
		DEBUG0(("setup_dfs_referral: Invalid dfs referral version: %d\n", max_referral_level));
		talloc_destroy(ctx);
...
BOOL create_junction(const char *dfs_path, struct junction_map *jucn)
{
	...
	if (!is_myname_or_ipaddr(dp.hostname)) {
		DEBUG4(("create_junction: Invalid hostname %s in dfs path %s\n",
	...
	if(snum < 0 || !lp_msdfs_root(snum)) {
		DEBUG4(("create_junction: %s is not an msdfs root.\n",
			dp.servicename));
		return False;
	}
...
BOOL create_msdfs_link(struct junction_map *jucn, BOOL exists)
{
	...
	DEBUG5(("create_msdfs_link: Creating new msdfs link: %s -> %s\n",
		path, msdfs_link));
	...
	if(SMB_VFS_SYMLINK(conn, msdfs_link, path) < 0) {
		DEBUG1(("create_msdfs_link: symlink failed %s -> %s\nError: %s\n", 
	...
	*/ 
	...
	while ((dname = vfs_readdirname(&conn, dirp)) != NULL) {
		...
		if (cnt >= jn_remain) {
			...
			DEBUG2( ("ran out of MSDFS junction slots"));
			goto out;
		}
