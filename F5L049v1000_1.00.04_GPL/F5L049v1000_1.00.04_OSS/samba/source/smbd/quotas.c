...
static BOOL nfs_quotas(char *nfspath, uid_t euser_id, SMB_BIG_UINT *bsize, SMB_BIG_UINT *dfree, SMB_BIG_UINT *dsize)
{
	...
	*bsize = *dfree = *dsize = (SMB_BIG_UINT)0;
	...
	DEBUG5(("nfs_quotas: looking for mount on \"%s\"\n", cutstr));
	DEBUG5(("nfs_quotas: of path \"%s\"\n", mnttype));
	testpath=strchr_m(mnttype, ':');
	args.gqa_pathp = testpath+1;
	args.gqa_uid = uid;

	DEBUG5(("nfs_quotas: Asking for host \"%s\" rpcprog \"%i\" rpcvers \"%i\" network \"%s\"\n", host, RQUOTAPROG, RQUOTAVERS, "udp"));
	...
	DEBUG9(("nfs_quotas: auth_success\n"));
	...
	if (clnt_stat != RPC_SUCCESS) {
		DEBUG9(("nfs_quotas: clnt_call fail\n"));
		ret = False;
		goto out;
	}
	...
	switch ( quotastat ) {
	case 0:
		DEBUG9(("nfs_quotas: Remote Quotas Failed!  Error \"%i\" \n", quotastat ));
		ret = False;
		goto out;

	case 1:
		DEBUG9(("nfs_quotas: Good quota data\n"));
	...
	case 3:
		D.dqb_bsoftlimit = 1;
		D.dqb_curblocks = 1;
		DEBUG9(("nfs_quotas: Remote Quotas returned \"%i\" \n", quotastat ));
		break;

	default:
		DEBUG9(("nfs_quotas: Remote Quotas Questionable!  Error \"%i\" \n", quotastat ));
		break;
	}

	DEBUG10(("nfs_quotas: Let`s look at D a bit closer... status \"%i\" bsize \"%i\" active? \"%i\" bhard \"%i\" bsoft \"%i\" curb \"%i\" \n",
			quotastat,
  ...
  out:
	...
	DEBUG5(("nfs_quotas: For path \"%s\" returning  bsize %.0f, dfree %.0f, dsize %.0f\n",args.gqa_pathp,(double)*bsize,(double)*dfree,(double)*dsize));

	SAFE_FREE(cutstr);
	DEBUG10(("nfs_quotas: End of nfs_quotas\n" ));
	return ret;
}
#endif
...
BOOL disk_quotas(const char *path, SMB_BIG_UINT *bsize, SMB_BIG_UINT *dfree, SMB_BIG_UINT *dsize)
{
	...
	devno = sbuf.st_dev ;
	DEBUG5(("disk_quotas: looking for path \"%s\" devno=%x\n",
	...
	if ( devno != devno_cached ) {
		...
		while (getmntent(fd, &mnt) == 0) {
			...
			DEBUG5(("disk_quotas: testing \"%s\" devno=%x\n",
	...
	if ( strcmp( mnt.mnt_fstype, "nfs" ) == 0) {
		BOOL retval;
		DEBUG5(("disk_quotas: looking for mountpath (NFS) \"%s\"\n", mnt.mnt_special));
		retval = nfs_quotas(mnt.mnt_special, euser_id, bsize, dfree, dsize);
		unbecome_root();
		return retval;
	}

	DEBUG5(("disk_quotas: looking for quotas file \"%s\"\n", name));
	...
	close(file);
#else
	DEBUG5(("disk_quotas: trying quotactl on device \"%s\"\n", name));
	...
	*dsize = D.dqb_bsoftlimit;
	...
	DEBUG5(("disk_quotas for path \"%s\" returning  bsize %.0f, dfree %.0f, dsize %.0f\n",
...
BOOL disk_quotas(const char *path, SMB_BIG_UINT *bsize, SMB_BIG_UINT *dfree, SMB_BIG_UINT *dsize)
{
  ...
  *bsize = 512;
  ...
  else if ( 0 == strcmp ( mnt->mnt_type, "xfs" ))
  {
    ...
    if (r==-1)
    {
      DEBUG5( ("quotactl for uid=%u: %s", euser_id, strerror(errno)));
      return(False);
    }
...
BOOL disk_quotas_vxfs(const pstring name, char *path, SMB_BIG_UINT *bsize, SMB_BIG_UINT *dfree, SMB_BIG_UINT *dsize)
{
  ...
  DEBUG5(("disk_quotas: looking for VxFS quotas file \"%s\"\n", qfname));
  ...
  if (ret < 0) {
    DEBUG5(("disk_quotas ioctl (VxFS) failed. Error = %s\n", strerror(errno) ));
    return(False);
  }
   ...
   * made of rubber latex and begins to expand to accommodate the user :-)
   */
  DEBUG5(("disk_quotas for path \"%s\" block c/s/h %ld/%ld/%ld; file c/s/h %ld/%ld/%ld\n",
