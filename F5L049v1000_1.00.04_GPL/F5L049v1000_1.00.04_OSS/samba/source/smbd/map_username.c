...
BOOL map_username(fstring user)
{
	...
	if (!f) {
		DEBUG0(("can't open username map %s. Error %s\n",mapfile, strerror(errno) ));
		return False;
	}

	DEBUG4(("Scanning username map %s\n",mapfile));
	...
	while((s=fgets_slash(buf,sizeof(buf),f))!=NULL) {
		...
		*dosname++ = 0;
		...
		if (!dosuserlist) {
			DEBUG0(("Bad username map entry.  Unable to build user list.  Ignoring.\n"));
			continue;
		}
