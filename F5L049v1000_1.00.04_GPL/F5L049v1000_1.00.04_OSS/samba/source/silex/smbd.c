...
{
	DEBUG10(("smb_conf_updated: Got message saying smb.conf was updated. Reloading.\n"));
				  ...
				  void *buf, size_t len, void *private_data)
{
	...
	DEBUG10(("smb_stat_cache_delete: delete name %s\n", name));
...
static void msg_sam_sync(int UNUSED(msg_type), struct process_id UNUSED(pid),
			 ...
			 void *private_data)
{
        DEBUG10( ("** sam sync message received, ignoring\n"));
}
			 ...
			 void *buf, size_t len, void *private_data)
{
        ...
        DEBUG3( ("received sam replication message, serial = 0x%04x\n",
                  low_serial));
}
			    ...
			    void *buf, size_t len, void *private_data)
{
	...
	if (len != sizeof(int)) {
		...
		DEBUG0( ("Process %llu sent bogus signal injection request\n",
...
#if HAVE_STRSIGNAL
	DEBUG0( ("Process %llu requested injection of signal %d (%s)\n",
		    (unsigned long long)src.pid, sig, strsignal(sig)));
#else
	DEBUG0( ("Process %llu requested injection of signal %d\n",
...
static void add_child_pid(pid_t pid)
{
	...
	if (child == NULL) {
		DEBUG0( ("Could not add child struct -- malloc failed\n"));
		return;
	}
	child->pid = pid;
...
static void remove_child_pid(pid_t pid)
{
	...
	DEBUG0( ("Could not find child %d -- ignoring\n", (int)pid));
}
...
static BOOL open_sockets_smbd(BOOL is_daemon, BOOL interactive, const char *smb_ports)
{
	...
	if (lp_interfaces() && lp_bind_interfaces_only()) {
		...
		*/
		...
		for(i = 0; i < num_interfaces; i++) {
			...
			if(ifip == NULL) {
				DEBUG0(("open_sockets_smbd: interface %d has NULL IP address !\n", i));
				continue;
			}
			...
			for (ptr=ports; next_token(&ptr, tok, " \t,", sizeof(tok)); ) {
				...
				if (num_sockets >= FD_SETSIZE) {
					DEBUG0(("open_sockets_smbd: Too many sockets to bind to\n"));
					return False;
				}
			}
		}
		...
		for (ptr=ports; next_token(&ptr, tok, " \t,", sizeof(tok)); ) {
			...
			if (listen(s, SMBD_LISTEN_BACKLOG) == -1) {
				DEBUG0(("open_sockets_smbd: listen: %s\n",
					 strerror(errno)));
				close(s);
				return False;
			}
	...
	DEBUG2(("waiting for a connection\n"));
	while (1) {
		fd_set lfds;
		int num;
		...
		for( ; num > 0; num--) {
			...
			if (smbd_server_fd() == -1) {
				DEBUG0(("open_sockets_smbd: accept: %s\n",
					 strerror(errno)));
				continue;
			}
...
void reload_printers(void)
{
	...
	for (snum = 0; snum < n_services; snum++) {
		...
		if (!pcap_printername_ok(pname)) {
			DEBUG3( ("removing stale printer %s\n", pname));
	...
	const char *const reason)
{
	...
	if (how != SERVER_EXIT_NORMAL) {
		...
		DEBUGSEP(0);
		DEBUG0(("Abnormal server exit: %s\n",
			reason ? reason : "no explanation provided"));
		DEBUGSEP(0);

		log_stack_trace();
		if (last_inbuf) {
			DEBUG0(("Last message was %s\n", LAST_MESSAGE()));
			show_msg(last_inbuf);
		}
 ...
 int smbd_main(int argc,const char *argv[])
{
	...
	if (log_stdout && Fork) {
		DEBUG0(("ERROR: Can't log to stdout (-S) unless daemon is in foreground (-F) or interactive (-i)\n"));
		exit(1);
	}
	...
	DEBUG0(( "smbd version %s started.\n", SAMBA_VERSION_STRING));
	DEBUGADD( 0, ( "%s\n", COPYRIGHT_STARTUP_MESSAGE ) );

	DEBUG2(("uid=%d gid=%d euid=%d egid=%d\n",
	...
	if (sizeof(uint16) < 2 || sizeof(uint32) < 4) {
		DEBUG0(("ERROR: Samba is not configured correctly for the word size on your machine\n"));
		exit(1);
	}
	...
	if (!profile_setup(False)) {
		DEBUG0(("ERROR: failed to setup profiling\n"));
		return -1;
	}
	if (profile_level != NULL) {
		...
		DEBUG1( ("setting profiling level: %s\n",profile_level));
		src.pid = getpid();
		set_profile_level(pl, src);
	}
#endif

	DEBUG3(( "loaded services\n"));

	if (!is_daemon && !is_a_socket(0)) {
		if (!interactive)
			DEBUG0(("standard input is not a socket, assuming -D option\n"));
	...
	if (!secrets_init()) {
		DEBUG0( ("ERROR: smbd can not open secrets.tdb\n"));
		exit(1);
	}

	if(!get_global_sam_sid()) {
		DEBUG0(("ERROR: Samba cannot create a SAM SID.\n"));
		exit(1);
	}
	...
	if (!init_guest_info()) {
		DEBUG0(("ERROR: failed to setup guest info.\n"));
		return -1;
	}
	...
	if (!init_account_policy()) {
		DEBUG0(("Could not open account policy tdb.\n"));
		exit(1);
	}

	if (*lp_rootdir()) {
		if (sys_chroot(lp_rootdir()) == 0)
			DEBUG2(("Changed root to %s\n", lp_rootdir()));
	}
