...
static void reload_interfaces(time_t t)
{
  ...
  try_again:
	...
	for (n=iface_count() - 1; n >= 0; n--) {
		...
		if (!iface) {
			DEBUG2(("reload_interfaces: failed to get interface %d\n", n));
			continue;
		}
		...
		if (ip_equal(iface->ip, loopback_ip)) {
			DEBUG2(("reload_interfaces: Ignoring loopback interface %s\n", inet_ntoa(iface->ip)));
			continue;
		}
	...
	for (subrec=subnetlist; subrec; subrec=subrec->next) {
		...
		if (n == -1) {
			/* oops, an interface has disapeared. This is
			 ...
			 freeing it */
			DEBUG2(("Deleting dead interface %s\n", 
				 ...
				 void *buf, size_t len, void *private_data)
{
	...
	DEBUG10( ("Received send_packet from %d\n", procid_to_pid(&src)));

	if (len != sizeof(struct packet_struct)) {
		DEBUG2( ("Discarding invalid packet length from %d\n",
			  procid_to_pid(&src)));
		return;
	}
	...
	if ((p->packet_type != NMB_PACKET) &&
		...
		DEBUG2( ("Discarding invalid packet type from %d: %d\n",
	...
	if (local_ip == NULL) {
		DEBUG2( ("Could not find ip for packet from %d\n",
			  procid_to_pid(&src)));
		return;
	}
...
static void process(void)
{
	...
	while( True ) {
		...
		if(reload_after_sighup) {
			DEBUG0( ( "Got SIGHUP dumping debug info.\n" ) );
...
static BOOL open_sockets(BOOL isdaemon, int port)
{
	...
	DEBUG3( ( "open_sockets: Broadcast sockets opened.\n" ) );
	return( True );
}
 ...
 int nmbd_main(int argc, const char *argv[])
{
	...
	DEBUG0( ( "Netbios nameserver version %s started.\n", SAMBA_VERSION_STRING) );
	...
	if (strequal(lp_workgroup(),"*")) {
		DEBUG0(("ERROR: a workgroup name of * is no longer supported\n"));
		exit(1);
	}
	...
	if( False == create_subnets() ) {
		DEBUG0(("ERROR: Failed when creating subnet lists. Exiting.\n"));
		kill_async_dns_child();
		exit(1);
	}
	...
	DEBUG3(("Loaded hosts file %s\n", p_lmhosts));

	/* If we are acting as a WINS server, initialise data structures. */
	if( !initialise_wins() ) {
		DEBUG0( ( "nmbd: Failed when initialising WINS server.\n" ) );
		kill_async_dns_child();
		exit(1);
	}
	...
	if( False == register_my_workgroup_and_names() ) {
		DEBUG0(("ERROR: Failed when creating my my workgroup. Exiting.\n"));
		kill_async_dns_child();
		exit(1);
	}
