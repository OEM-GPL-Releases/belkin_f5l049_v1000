...
static BOOL Section( myFILE *InFile, BOOL (*sfunc)(const char *) )
{
	...
	if (!FindSectionEnd(InFile)) {
		DEBUG0( ("%s No terminating ']' character in section.\n", func) );
		return False;
	}
	...
	while( (EOF != c) && (c > 0) ) {
		...
		if( i > (bSize - 2) ) {
			...
			if(!tb) {
				DEBUG0( ("%s Memory re-allocation failure.", func) );
				return False;
			}
			bufr = tb;
			bSize += BUFR_INC;
		...
		switch( c ) {
			case '\n': /* Got newline before closing ']'.    */
				...
				if( i < 0 ) {
					bufr[end] = '\0';
					DEBUG0( ("%s Badly formed line in configuration file: %s\n", func, bufr ));
					return False;
				}
		...
		if (AtSectionEnd(InFile)) {
			...
			if( 0 == end ) {
				...
				DEBUG0( ("%s Empty section name in configuration file.\n", func ));
				return False;
			}
	...
	DEBUG0( ("%s Unexpected EOF in the configuration file: %s\n", func, bufr ));
	return False;
}
	...
	while( 0 == vstart ) {
		...
		switch(c) {
			case '=': /* Equal sign marks end of param name. */
				if( 0 == end ) {
					/* Don't allow an empty name.      */
					DEBUG0( ("%s Invalid parameter name in config. file.\n", func ));
					return False;
				}
			...
			case '\n': /* Find continuation char, else error. */
				...
				if( i < 0 ) {
					bufr[end] = '\0';
					DEBUG1(("%s Ignoring badly formed line in configuration file: %s\n", func, bufr ));
					return True;
				}
				end = ( (i > 0) && (' ' == bufr[i - 1]) ) ? (i - 1) : (i);
			...
			case EOF:
				bufr[i] = '\0';
				DEBUG1(("%s Unexpected end-of-file at: %s\n", func, bufr ));
				return True;
		...
		BOOL (*pfunc)(const char *, const char *) )
{
	...
	DEBUG3( ("%s Processing configuration file \"%s\"\n", func, FileName) );
	...
	if( NULL != bufr ) {
		...
		if( NULL == bufr ) {
			DEBUG0(("%s memory allocation failure.\n", func));
			myfile_close(InFile);
			return False;
		}
	...
	if( !result ) {
		DEBUG0(("%s Failed.  Error returned from params.c:parse().\n", func));
		return False;
	}
