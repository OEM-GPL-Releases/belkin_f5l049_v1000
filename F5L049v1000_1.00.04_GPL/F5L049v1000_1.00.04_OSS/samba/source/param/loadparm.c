...
static void init_globals(BOOL first_time_only)
{
	...
	DEBUG3( ("Initialising global parameters\n"));
...
const char *lp_passdb_backend(void)
{
	...
	if (quote == NULL) {
		DEBUG0( ("WARNING: Your 'passdb backend' configuration is invalid due to a missing second \" char.\n"));
		goto out;
	} else if (*(quote+1) == '\0') {
...
warn:
	DEBUG0( ("WARNING: Your 'passdb backend' configuration includes multiple backends.  This\n"
...
#define MISSING_PARAMETER(name) \
    DEBUG0( ("%s(): value is NULL or empty!\n", #name))
...
static BOOL lp_bool(const char *s)
{
	...
	if (!set_boolean(&ret,s)) {
		DEBUG0(("lp_bool(%s): value is not boolean!\n",s));
		return False;
	}
...
static int lp_enum(const char *s,const struct enum_list *_enum)
{
	...
	DEBUG0(("lp_enum(%s,enum): value is not in enum_list!\n",s));
	return (-1);
}
...
static void free_service(service *pservice)
{
	...
	if (pservice->szService)
		DEBUG5( ("free_service: Freeing service %s\n",
		       pservice->szService));
...
static int add_a_service(const service *pservice, const char *name)
{
	...
	if (i == iNumServices) {
		...
		if (tsp == NULL) {
			DEBUG0(("add_a_service: failed to enlarge ServicePtrs!\n"));
			return (-1);
		}
		ServicePtrs = tsp;
		ServicePtrs[iNumServices] = SMB_MALLOC_P(service);
		if (!ServicePtrs[iNumServices]) {
			DEBUG0(("add_a_service: out of memory!\n"));
			return (-1);
		}
		iNumServices++;
		...
		tinvalid = SMB_REALLOC_ARRAY_KEEP_OLD_ON_ERROR(invalid_services, int,
					     num_to_alloc);
		if (tinvalid == NULL) {
			DEBUG0(("add_a_service: failed to enlarge "
				 "invalid_services!\n"));
			return (-1);
		}
	...
	DEBUG8(("add_a_service: Creating snum = %d for %s\n", 
...
static char *canonicalize_servicename(const char *src)
{
	...
	if ( !src ) {
		DEBUG0(("canonicalize_servicename: NULL source name!\n"));
		return NULL;
	}
...
static BOOL hash_a_service(const char *name, int idx)
{
	...
	if ( !ServiceHash ) {
		DEBUG10(("hash_a_service: creating tdb servicehash\n"));
		ServiceHash = tdb_open("servicehash", 1031, TDB_INTERNAL, 
                                        (O_RDWR|O_CREAT), 0600);
		if ( !ServiceHash ) {
			DEBUG0(("hash_a_service: open tdb servicehash failed!\n"));
			return False;
		}
	}

	DEBUG10(("hash_a_service: hashing index %d for service name %s\n",
		idx, name));
		 ...
		 const char *user, const char *pszHomedir)
{
	...
	DEBUG3( ("adding home's share [%s] for user '%s' at '%s'\n", pszHomename, 
...
BOOL lp_add_printer(const char *pszPrintername, int iDefaultService)
{
	...
	DEBUG3( ("adding printer service %s\n", pszPrintername));
...
static int map_parameter(const char *pszParmName)
{
	...
	if (strchr(pszParmName, ':') == NULL)
		DEBUG0( ("Unknown parameter encountered: \"%s\"\n", pszParmName));
...
static BOOL service_ok(int iService)
{
	...
	if (ServicePtrs[iService]->szService[0] == '\0') {
		DEBUG0( ("The following message indicates an internal error:\n"));
		DEBUG0( ("No service name in service entry.\n"));
		bRetval = False;
	}
	...
	if (strwicmp(ServicePtrs[iService]->szService, PRINTERS_NAME) == 0) {
		if (!ServicePtrs[iService]->bPrint_ok) {
			DEBUG0( ("WARNING: [%s] service MUST be printable!\n",
	...
	if (ServicePtrs[iService]->szPath[0] == '\0' &&
		...
		DEBUG0( ("WARNING: No path in service %s - making it unavailable!\n",
			ServicePtrs[iService]->szService));
	...
	if (!ServicePtrs[iService]->bAvailable)
		DEBUG1( ("NOTE: Service %s is flagged unavailable.\n",
			  ServicePtrs[iService]->szService));
...
static BOOL handle_netbios_name(int snum, const char *pszParmValue, char **ptr)
{
	...
	DEBUG4( ("handle_netbios_name: set global_myname to: %s\n",
	       global_myname()));
...
static BOOL handle_include(int snum, const char *pszParmValue, char **ptr)
{
	...
	DEBUG2( ("Can't find include file %s\n", fname));
...
static BOOL handle_copy(int snum, const char *pszParmValue, char **ptr)
{
	...
	DEBUG3( ("Copying service from service %s\n", pszParmValue));

	if ((iTemp = getservicebyname(pszParmValue, &serviceTemp)) >= 0) {
		if (iTemp == iServiceIndex) {
			DEBUG0( ("Can't copy service %s - unable to copy self!\n", pszParmValue));
		} else {
		...
		DEBUG0( ("Unable to copy service - source not found: %s\n", pszParmValue));
		bRetval = False;
	}
...
static const char *append_ldap_suffix( const char *str )
{
	...
	if ( !suffix_string ) {
		DEBUG0(("append_ldap_suffix: talloc_asprintf() failed!\n"));
		return "";
	}
...
BOOL lp_do_parameter(int snum, const char *pszParmName, const char *pszParmValue)
{
	...
	if (parmnum < 0) {
		...
		DEBUG0( ("Ignoring unknown parameter \"%s\"\n", pszParmName));
		return (True);
	}

	if (parm_table[parmnum].flags & FLAG_DEPRECATED) {
		DEBUG1( ("WARNING: The \"%s\" option is deprecated\n",
			  pszParmName));
	}
...
static BOOL do_section(const char *pszSectionName)
{
	...
	if (bRetval) {
		...
		DEBUG2( ("Processing section \"[%s]\"\n", pszSectionName));

		if ((iServiceIndex = add_a_service(&sDefault, pszSectionName))
		    < 0) {
			DEBUG0( ("Failed to add a new service\n"));
			return (False);
		}
	}
...
static void set_server_role(void)
{
	...
	switch (lp_security()) {
		case SEC_SHARE:
			if (lp_domain_logons())
				DEBUG0( ("Server's Role (logon server) conflicts with share-level security\n"));
			break;
		case SEC_SERVER:
			if (lp_domain_logons())
				DEBUG0( ("Server's Role (logon server) conflicts with server-level security\n"));
			/* this used to be considered ROLE_DOMAIN_MEMBER but that's just wrong */
			server_role = ROLE_STANDALONE;
			break;
		case SEC_DOMAIN:
			if (lp_domain_logons()) {
				DEBUG1( ("Server's Role (logon server) NOT ADVISED with domain-level security\n"));
		...
		case SEC_USER:
			...
			break;
		default:
			DEBUG0( ("Server's Role undefined due to unknown security mode\n"));
			break;
	}

	DEBUG10( ("set_server_role: role = %s\n", server_role_str(server_role)));
}
...
static BOOL check_usershare_stat(const char *fname, SMB_STRUCT_STAT *psbuf)
{
	if (!S_ISREG(psbuf->st_mode)) {
		DEBUG0(("check_usershare_stat: file %s owned by uid %u is "
			"not a regular file\n",
	...
	if (psbuf->st_mode & S_IWOTH) {
		DEBUG0(("check_usershare_stat: file %s owned by uid %u allows "
			...
			BOOL *pallow_guest)
{
	...
	*pallow_guest = False;
	...
	if (sharepath[0] != '/') {
		DEBUG2(("parse_usershare_file: share %s: path %s is not an absolute path.\n",
	...
	if (prefixdenylist) {
		...
		for ( i=0; prefixdenylist[i]; i++ ) {
			DEBUG10(("parse_usershare_file: share %s : checking prefixdenylist[%d]='%s' against %s\n",
				servicename, i, prefixdenylist[i], sharepath ));
			if (memcmp( sharepath, prefixdenylist[i], strlen(prefixdenylist[i])) == 0) {
				DEBUG2(("parse_usershare_file: share %s path %s starts with one of the "
	...
	if (prefixallowlist) {
		...
		for ( i=0; prefixallowlist[i]; i++ ) {
			DEBUG10(("parse_usershare_file: share %s checking prefixallowlist[%d]='%s' against %s\n",
		...
		if (prefixallowlist[i] == NULL) {
			DEBUG2(("parse_usershare_file: share %s path %s doesn't start with one of the "
	...
	if (!dp) {
		DEBUG2(("parse_usershare_file: share %s path %s is not a directory.\n",
			servicename, sharepath));
	...
	if (sys_stat(sharepath, &sbuf) == -1) {
		DEBUG2(("parse_usershare_file: share %s : stat failed on path %s. %s\n",
...
static int process_usershare_file(const char *dir_name, const char *file_name, int snum_template)
{
	...
	if (!validate_net_name(file_name, INVALID_SHARENAME_CHARS, strlen(file_name))) {
		DEBUG0(("process_usershare_file: share name %s contains "
	...
	if (sys_lstat(fname, &lsbuf) != 0) {
		DEBUG0(("process_usershare_file: stat of %s failed. %s\n",
	...
	if (iService != -1 && ServicePtrs[iService]->usershare_last_mod == lsbuf.st_mtime) {
		...
		DEBUG10(("process_usershare_file: service %s not changed.\n",
			service_name ));
	...
	if (fd == -1) {
		DEBUG0(("process_usershare_file: unable to open %s. %s\n",
			fname, strerror(errno) ));
		return -1;
	}
	...
	if (sys_fstat(fd, &sbuf) != 0) {
		close(fd);
		DEBUG0(("process_usershare_file: fstat of %s failed. %s\n",
			fname, strerror(errno) ));
		return -1;
	}
	...
	if (lsbuf.st_dev != sbuf.st_dev || lsbuf.st_ino != sbuf.st_ino) {
		close(fd);
		DEBUG0(("process_usershare_file: fstat of %s is a different file from lstat. "
	...
	close(fd);
	if (lines == NULL) {
		DEBUG0(("process_usershare_file: loading file %s owned by %u failed.\n",
	...
	if (iService < 0) {
		...
		if ((iService = add_a_service(sp, service_name)) < 0) {
			DEBUG0( ("process_usershare_file: Failed to add "
				"new service %s\n", service_name));
			talloc_destroy(ctx);
			return -1;
		}
	...
	if (!set_share_security(service_name, psd)) {
		 DEBUG0( ("process_usershare_file: Failed to set share "
...
int load_usershare_service(const char *servicename)
{
	...
	if (sys_stat(usersharepath, &sbuf) != 0) {
		DEBUG0(("load_usershare_service: stat of %s failed. %s\n",
			usersharepath, strerror(errno) ));
		return -1;
	}

	if (!S_ISDIR(sbuf.st_mode)) {
		DEBUG0(("load_usershare_service: %s is not a directory.\n",
			usersharepath ));
		return -1;
	}
...
#endif
		DEBUG0(("load_usershare_service: directory %s is not owned by root "
	...
	if (Globals.szUsershareTemplateShare[0]) {
		...
		if (snum_template == -1) {
			DEBUG0(("load_usershare_service: usershare template share %s "
				"does not exist.\n",
				Globals.szUsershareTemplateShare ));
			return -1;
		}
...
int load_usershare_shares(void)
{
	...
	if (sys_stat(usersharepath, &sbuf) != 0) {
		DEBUG0(("load_usershare_shares: stat of %s failed. %s\n",
			usersharepath, strerror(errno) ));
		return ret;
	}
	...
	if (sbuf.st_uid != 0 || (sbuf.st_mode & S_IWOTH)) {
#endif
		DEBUG0(("load_usershare_shares: directory %s is not owned by root "
			"or does not have the sticky bit 't' set or is writable by anyone.\n",
			usersharepath ));
		return ret;
	}
	...
	if (Globals.szUsershareTemplateShare[0]) {
		...
		if (snum_template == -1) {
			DEBUG0(("load_usershare_shares: usershare template share %s "
				"does not exist.\n",
				Globals.szUsershareTemplateShare ));
			return ret;
		}
	...
	if (!dp) {
		DEBUG0(("load_usershare_shares:: failed to open directory %s. %s\n",
			usersharepath, strerror(errno) ));
		return ret;
	}
	...
	for (num_dir_entries = 0, num_bad_dir_entries = 0, num_tmp_dir_entries = 0;
		...
		if (num_tmp_dir_entries > allowed_tmp_entries) {
			DEBUG0(("load_usershare_shares: too many temp entries (%u) "
				"in directory %s\n",
		...
		if (r == 0) {
			...
			if (num_usershares >= max_user_shares) {
				DEBUG0(("load_usershare_shares: max user shares reached "
		...
		if (num_bad_dir_entries > allowed_bad_entries) {
			DEBUG0(("load_usershare_shares: too many bad entries (%u) "
				"in directory %s\n",
		...
		if (num_dir_entries > max_user_shares + allowed_bad_entries) {
			DEBUG0(("load_usershare_shares: too many total entries (%u) "
			"in directory %s\n",
	...
	for (iService = iNumServices - 1; iService >= 0; iService--) {
		if (VALID(iService) && (ServicePtrs[iService]->usershare == USERSHARE_PENDING_DELETE)) {
			...
			DEBUG10(("load_usershare_shares: Removing deleted usershare %s\n",
             ...
             BOOL initialize_globals)
{
	...
	DEBUG3( ("lp_load: refreshing parameters\n"));
...
int lp_servicenumber(const char *pszServiceName)
{
	...
	if (iService < 0) {
		DEBUG7(("lp_servicenumber: couldn't find %s\n", pszServiceName));
		return GLOBAL_SECTION_SNUM;
	}
