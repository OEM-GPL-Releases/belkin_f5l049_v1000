...
static TREE_NODE* pathtree_find_child( TREE_NODE *node, char* key )
{
	...
	if ( !node ) {
		DEBUG0(("pathtree_find_child: NULL node passed into function!\n"));
		return NULL;
	}
	
	if ( !key ) {
		DEBUG0(("pathtree_find_child: NULL key string passed into function!\n"));
		return NULL;
	}
 ...
 BOOL pathtree_add( SORTED_TREE *tree, const char *path, void *data_p )
{
	...
	DEBUG8(("pathtree_add: Enter\n"));
		
	if ( !path || *path != '/' ) {
		DEBUG0(("pathtree_add: Attempt to add a node with a bad path [%s]\n",
			path ? path : "NULL" ));
		return False;
	}
	
	if ( !tree ) {
		DEBUG0(("pathtree_add: Attempt to add a node to an uninitialized tree!\n"));
		return False;
	}
	...
	if ( !path2 ) {
		DEBUG0(("pathtree_add: strdup() failed on string [%s]!?!?!\n", path));
		return False;
	}
	...
	do {
		...
		if ( !next ) {
			...
			if ( !next ) {
				DEBUG0(("pathtree_add: Failed to create new child!\n"));
				ret =  False;
				goto done;
			}
		}
	...
	DEBUG10(("pathtree_add: Successfully added node [%s] to tree\n",
		path ));

	DEBUG8(("pathtree_add: Exit\n"));
 ...
 void* pathtree_find( SORTED_TREE *tree, char *key )
{
	...
	DEBUG10(("pathtree_find: Enter [%s]\n", key ? key : "NULL" ));

	/* sanity checks first */
	
	if ( !key ) {
		DEBUG0(("pathtree_find: Attempt to search tree using NULL search string!\n"));
		return NULL;
	}
	
	if ( !tree ) {
		DEBUG0(("pathtree_find: Attempt to search an uninitialized tree using string [%s]!\n",
			key ? key : "NULL" ));
		return NULL;
	}
	...
	if ( !keystr ) {
		DEBUG0(("pathtree_find: strdup() failed on string [%s]!?!?!\n", key));
		return NULL;
	}
