...
BOOL init_names(void)
{
	...
	if (global_myname() == NULL || *global_myname() == '\0') {
		if (!set_global_myname(myhostname())) {
			DEBUG0( ( "init_structs: malloc fail.\n" ) );
			return False;
		}
	}
...
static const char *Atoic(const char *p, int *n, const char *c)
{
	...
	if (strchr_m(c, *p) == NULL) {
		DEBUG5( ("Atoic: no separator characters (%s) not found\n", c));
		return NULL;
	}
...
void close_low_fds(BOOL stderr_too)
{
	...
	for (i=0;i<3;i++) {
		...
		if (fd < 0) {
			DEBUG0(("Can't open /dev/null\n"));
			return;
		}
		if (fd != i) {
			DEBUG0(("Didn't get file descriptor %d\n",i));
			return;
		}
	}
#endif
...
ssize_t write_data_at_offset(int fd, const char *buffer, size_t N, SMB_OFF_T pos)
{
	...
	while (total < N) {
		...
		if (ret == -1) {
			DEBUG0(("write_data_at_offset: write failure. Error = %s\n", strerror(errno) ));
			return -1;
		}
		if (ret == 0) {
			return total;
						...
						ssize_t (*write_fn)(int, const void *, size_t))
{
	...
	while (total < n) {
		...
		if (read_ret == -1) {
			DEBUG0(("transfer_file_internal: read failure. Error = %s\n", strerror(errno) ));
			SAFE_FREE(buf);
			return -1;
		}
		if (read_ret == 0)
		...
		while (num_written < read_ret) {
			...
			if (write_ret == -1) {
				DEBUG0(("transfer_file_internal: write failure. Error = %s\n", strerror(errno) ));
				SAFE_FREE(buf);
				return -1;
			}
			if (write_ret == 0)
...
void *Realloc(void *p, size_t size, BOOL free_old_on_error)
{
	...
	if (size == 0) {
		...
		DEBUG2(("Realloc asked for 0 bytes\n"));
		return NULL;
	}
	...
	if (!ret) {
		...
		DEBUG0(("Memory allocation error: failed to expand to %d bytes\n",(int)size));
	}
...
int interpret_protocol(const char *str,int def)
{
	...
	DEBUG0(("Unrecognised protocol level %s\n",str));
  ...
  /* if it's in the form of an IP address then get the lib to interpret it */
	if (is_ipaddress(str)) {
		...
		if ((hp = sys_gethostbyname(str)) == 0) {
			DEBUG3(("sys_gethostbyname: Unknown host. %s\n",str));
			return 0;
		}

		if(hp->h_addr == NULL) {
			DEBUG3(("sys_gethostbyname: host address is invalid for host %s\n",str));
			return 0;
		}
...
char *automount_lookup(const char *user_name)
{
	...
	DEBUG4( ("NIS+ Lookup: %s resulted in %s\n", user_name, last_value));
	return last_value;
}
...
char *automount_lookup(const char *user_name)
{
	...
	if (!strcmp(user_name, last_key)) {
		...
		if ((nis_error = yp_match(nis_domain, nis_map, user_name, strlen(user_name),
			...
			last_value[0] = 0;
			DEBUG3( ("YP Key not found:  while looking up \"%s\" in map \"%s\"\n", 
					user_name, nis_map));
			DEBUG3( ("using defaults for server and home directory\n"));
		} else {
			DEBUG3( ("YP Error: \"%s\" while looking up \"%s\" in map \"%s\"\n", 
					yperr_string(nis_error), user_name, nis_map));
		}
	}

	DEBUG4( ("YP Lookup: %s resulted in %s\n", user_name, last_value));
	return last_value;
}
#endif /* WITH_NISPLUS_HOME */
#endif
...
void smb_panic(const char *const why)
{
...
#ifdef DEVELOPER
	{
		...
		if (global_clobber_region_function) {
			DEBUG0(("smb_panic: clobber_region() last called from [%s(%u)]\n",
	...
	DEBUG0(("PANIC (pid %llu): %s\n",
		    (unsigned long long)sys_getpid(), why));
	log_stack_trace();

	cmd = lp_panic_action();
	if (cmd && *cmd) {
		DEBUG0( ("smb_panic(): calling panic action [%s]\n", cmd));
		result = system(cmd);

		if (result == -1){
			DEBUG0( ("smb_panic(): fork failed in panic action: %s\n",
					  strerror(errno)));}
		else
			DEBUG0( ("smb_panic(): action returned status %d\n",
					  WEXITSTATUS(result)));
	}
...
libunwind_failed:
	DEBUG0( ("unable to produce a stack trace with libunwind\n"));
	...
	DEBUG0( ("BACKTRACE: %lu stack frames:\n", 
...
#define NAMESIZE 32 /* Arbitrary */
	...
	DEBUG0( ("BACKTRACE: %d stack frames:\n", levels));
	for (i = 0; i < levels; i++) {
...
#else
	DEBUG0( ("unable to produce a stack trace on this platform\n"));
#endif
}
	...
	for(; namelist->name != NULL; namelist++) {
		if(namelist->is_wild) {
			if (mask_match(last_component, namelist->name, case_sensitive)) {
				DEBUG8(("is_in_path: mask match succeeded\n"));
				return True;
			}
		} else {
			if((case_sensitive && (strcmp(last_component, namelist->name) == 0))||
						(!case_sensitive && (StrCaseCmp(last_component, namelist->name) == 0))) {
				DEBUG8(("is_in_path: match succeeded\n"));
				return True;
			}
		}
	}
	DEBUG8(("is_in_path: match not found\n"));
	return False;
}
...
void set_namearray(name_compare_entry **ppname_array, char *namelist)
{
	...
	*/
	...
	if(( (*ppname_array) = SMB_MALLOC_ARRAY(name_compare_entry, num_entries + 1)) == NULL) {
		DEBUG0(("set_namearray: malloc fail\n"));
		return;
	}
	...
	while(*nameptr) {
		...
		if(((*ppname_array)[i].name = SMB_STRDUP(nameptr)) == NULL) {
			DEBUG0(("set_namearray: malloc fail (1)\n"));
			return;
		}
...
BOOL fcntl_lock(int fd, int op, SMB_OFF_T offset, SMB_OFF_T count, int type)
{
	...
	DEBUG8(("fcntl_lock fd=%d op=%d offset=%.0f count=%.0f type=%d\n",
	...
	if (ret == -1) {
		int sav = errno;
		DEBUG3(("fcntl_lock: lock failed at offset %.0f count %.0f op %d type %d (%s)\n",
	...
	/* everything went OK */
	DEBUG8(("fcntl_lock: Lock call successful\n"));
...
BOOL fcntl_getlock(int fd, SMB_OFF_T *poffset, SMB_OFF_T *pcount, int *ptype, pid_t *ppid)
{
	...
	DEBUG8(("fcntl_getlock fd=%d offset=%.0f count=%.0f type=%d\n",
	...
	if (ret == -1) {
		int sav = errno;
		DEBUG3(("fcntl_getlock: lock request failed at offset %.0f count %.0f type %d (%s)\n",
	...
	*ppid = lock.l_pid;
	...
	DEBUG3(("fcntl_getlock: fd %d is returned info %d pid %u\n",
...
BOOL is_myworkgroup(const char *s)
{
	...
	DEBUG8( ("is_myworkgroup(\"%s\") returns %d\n", s, ret));
	return(ret);
}
...
void set_remote_arch(enum remote_arch_types type)
{
	...
	DEBUG10(("set_remote_arch: Client arch is \'%s\'\n", remote_arch));
}
...
int set_maxfiles(int requested_max)
{
	...
	if(getrlimit(RLIMIT_NOFILE, &rlp)) {
		DEBUG0(("set_maxfiles: getrlimit (1) for RLIMIT_NOFILE failed with error %s\n",
			strerror(errno) ));
		/* just guess... */
	...
	if (rlp.rlim_max != RLIM_INFINITY) {
		...
		if(setrlimit(RLIMIT_NOFILE, &rlp)) {
			DEBUG3(("set_maxfiles: setrlimit for RLIMIT_NOFILE for %d max files failed with error %s\n", 
	...
	if(setrlimit(RLIMIT_NOFILE, &rlp)) {
		DEBUG0(("set_maxfiles: setrlimit for RLIMIT_NOFILE for %d files failed with error %s\n", 
	...
	if(getrlimit(RLIMIT_NOFILE, &rlp)) {
		DEBUG0(("set_maxfiles: getrlimit (2) for RLIMIT_NOFILE failed with error %s\n",
			strerror(errno) ));
		/* just guess... */
		return saved_current_limit;
    }
...
void *smb_xmalloc_array(size_t size, unsigned int count)
{
	...
	if ((p = SMB_MALLOC(size*count)) == NULL) {
		DEBUG0( ("smb_xmalloc_array failed to allocate %lu * %lu bytes\n",
...
BOOL name_to_fqdn(fstring fqdn, const char *name)
{
	...
	if ( hp && hp->h_name && *hp->h_name ) {
		...
		if (full && (StrCaseCmp(full, "localhost.localdomain") == 0)) {
			DEBUG1( ("WARNING: your /etc/hosts file may be broken!\n"));
		...
		DEBUG10(("name_to_fqdn: lookup for %s -> %s.\n", name, full));
		fstrcpy(fqdn, full);
		return True;
	} else {
		DEBUG10(("name_to_fqdn: lookup for %s failed.\n", name));
		fstrcpy(fqdn, name);
		return False;
	}
}
...
void *talloc_check_name_abort(const void *ptr, const char *name)
{
	...
	DEBUG0( ("Talloc type mismatch, expected %s, got %s\n",
