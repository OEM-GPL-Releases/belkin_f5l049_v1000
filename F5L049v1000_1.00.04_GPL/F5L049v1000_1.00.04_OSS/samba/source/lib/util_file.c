	...
	*s = 0;
	...
	while (len < maxlen-1) {
		...
		if (!s2 && len > maxlen-3) {
			...
			if (!s) {
				DEBUG0(("fgets_slash: failed to expand buffer!\n"));
				return(NULL);
			}
		}
	}
...
char *file_pload(char *syscmd, size_t *size)
{
	...
	while ((n = read(fd, buf, sizeof(buf))) > 0) {
		...
		if (!p) {
		        DEBUG0(("file_pload: failed to expand buffer!\n"));
			close(fd);
			return NULL;
		}
...
BOOL unmap_file(void* start, size_t size)
{
	...
	if ( munmap( start, size ) != 0 ) {
		DEBUG1( ("map_file: Failed to unmap address %p "
			"of size %u - %s\n", 
...
void *map_file(char *fname, size_t size)
{
	...
	if (fd == -1) {
		DEBUG2(("map_file: Failed to load %s - %s\n", fname, strerror(errno)));
		return NULL;
	}
	p = mmap(NULL, size, PROT_READ, MAP_SHARED|MAP_FILE, fd, 0);
	close(fd);
	if (p == MAP_FAILED) {
		DEBUG1(("map_file: Failed to mmap %s - %s\n", fname, strerror(errno)));
		return NULL;
	}
#endif
	if (!p) {
		...
		if (s2 != size) {
			DEBUG1(("map_file: incorrect size for %s - got %lu expected %lu\n",
