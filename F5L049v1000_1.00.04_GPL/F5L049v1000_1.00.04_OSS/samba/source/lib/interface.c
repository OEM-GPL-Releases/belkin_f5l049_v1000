...
static void add_interface(struct in_addr ip, struct in_addr nmask)
{
	...
	if (iface_find(ip, False)) {
		DEBUG3(("not adding duplicate interface %s\n",inet_ntoa(ip)));
		return;
	}

#if !defined(__s390__)
	if (ip_equal(nmask, allones_ip)) {
		DEBUG3(("not adding non-broadcast interface %s\n",inet_ntoa(ip)));
		return;
	}
#endif
	...
	DEBUG2(("added interface ip=%s ",inet_ntoa(iface->ip)));
	DEBUG2(("bcast=%s ",inet_ntoa(iface->bcast)));
...
static void interpret_interface(char *token)
{
	...
	if (!p) {
		...
		DEBUG2(("can't determine netmask for %s\n", token));
		return;
	}
	...
	*p++ = '/';
	...
	if (ip.s_addr == MKBCADDR(ip.s_addr, nmask.s_addr) ||
		...
		DEBUG2(("Can't determine ip for broadcast address %s\n", token));
		return;
	}
...
void load_interfaces(void)
{
	...
	if (!ptr || !*ptr || !**ptr) {
		if (total_probed <= 0) {
			DEBUG0(("ERROR: Could not determine network interfaces, you must use a interfaces config line\n"));
			exit(1);
		}
		for (i=0;i<total_probed;i++) {
			if (
	...
	if (!local_interfaces) {
		DEBUG0(("WARNING: no network interfaces found\n"));
	}
}
