...
char *safe_strcpy_fn(const char *fn, int line, char *dest,const char *src, size_t maxlength)
{
	...
	if (!dest) {
		DEBUG0(("ERROR: NULL dest in safe_strcpy, called from [%s][%d]\n", fn, line));
		return NULL;
	}
	...
	if (len > maxlength) {
		DEBUG0(("ERROR: string overflow by %lu (%lu - %lu) in safe_strcpy [%.50s]\n",
...
char *safe_strcat_fn(const char *fn, int line, char *dest, const char *src, size_t maxlength)
{
	...
	if (!dest) {
		DEBUG0(("ERROR: NULL dest in safe_strcat, called from [%s][%d]\n", fn, line));
		return NULL;
	}
	...
	if (src_len + dest_len > maxlength) {
		DEBUG0(("ERROR: string overflow by %d in safe_strcat [%.50s]\n",
...
char *alpha_strcpy_fn(const char *fn, int line, char *dest, const char *src, const char *other_safe_chars, size_t maxlength)
{
	...
	if (!dest) {
		DEBUG0(("ERROR: NULL dest in alpha_strcpy, called from [%s][%d]\n", fn, line));
		return NULL;
	}
...
char *StrnCpy_fn(const char *fn, int line,char *dest,const char *src,size_t n)
{
	...
	if (!dest) {
		DEBUG0(("ERROR: NULL dest in StrnCpy, called from [%s][%d]\n", fn, line));
		return(NULL);
	}
...
static char *strncpyn(char *dest, const char *src, size_t n, char c)
{
	...
	p = strchr_m(src, c);
	if (p == NULL) {
		DEBUG5( ("strncpyn: separator character (%c) not found\n", c));
		return NULL;
	}
...
static BOOL string_init(char **dest,const char *src)
{
	...
	if (l == 0) {
		*dest = CONST_DISCARD(char*, null_string);
		...
		(*dest) = SMB_STRDUP(src);
		if ((*dest) == NULL) {
			DEBUG0(("Out of memory in string_init\n"));
			return False;
		}
	}
	return(True);
		 ...
		 BOOL remove_unsafe_characters, BOOL replace_once, BOOL allow_trailing_dollar)
{
	...
	while (lp <= ls && (p = strstr_m(s,pattern))) {
		if (ls + (li-lp) >= len) {
			DEBUG0(("ERROR: string overflow by %d in string_sub(%.50s, %d)\n", 
				 (int)(ls + (li-lp) - len),
				 pattern, (int)len));
			break;
		}
			 ...
			 const char *insert)
{
	...
	if (!in) {
		DEBUG0( ("realloc_string_sub: out of memory!\n"));
		return NULL;
	}
	ls = (ssize_t)strlen(s);
	lp = (ssize_t)strlen(pattern);
			...
			const char *pattern, const char *insert)
{
	...
	in = SMB_STRDUP(insert);
	if (!in) {
		DEBUG0( ("talloc_string_sub: out of memory!\n"));
		return NULL;
	}
	ls = (ssize_t)strlen(s);
	lp = (ssize_t)strlen(pattern);
	...
	while ((p = strstr_m(s,pattern))) {
		if (ld > 0) {
			...
			string = (char *)TALLOC_REALLOC(mem_ctx, string,
							ls + ld + 1);
			if (!string) {
				DEBUG0( ("talloc_string_sub: out of "
					  "memory!\n"));
				SAFE_FREE(in);
				return NULL;
			}
...
void all_string_sub(char *s,const char *pattern,const char *insert, size_t len)
{
	...
	while (lp <= ls && (p = strstr_m(s,pattern))) {
		if (ls + (li-lp) >= len) {
			DEBUG0(("ERROR: string overflow by %d in all_string_sub(%.50s, %d)\n", 
				 (int)(ls + (li-lp) - len),
				 pattern, (int)len));
			break;
		}
				...
				const smb_ucs2_t *insert)
{
	...
	if (!r) {
		DEBUG0( ("all_string_sub_w: out of memory!\n"));
		return NULL;
	}
...
char *strstr_m(const char *src, const char *findstr)
{
	...
	if (push_ucs2_allocate(&src_w, src) == (size_t)-1) {
		DEBUG0(("strstr_m: src malloc fail\n"));
		return NULL;
	}
	
	if (push_ucs2_allocate(&find_w, findstr) == (size_t)-1) {
		SAFE_FREE(src_w);
		DEBUG0(("strstr_m: find malloc fail\n"));
		return NULL;
	}
	...
	*p = 0;
	if (pull_ucs2_allocate(&s2, src_w) == (size_t)-1) {
		SAFE_FREE(src_w);
		SAFE_FREE(find_w);
		DEBUG0(("strstr_m: dest malloc fail\n"));
		return NULL;
	}
...
static char **str_list_make_internal(TALLOC_CTX *mem_ctx, const char *string, const char *sep)
{
	...
	if (!s) {
		DEBUG0(("str_list_make: Unable to allocate memory"));
		return NULL;
	}
	if (!sep) sep = LIST_SEP;
	...
	*dest = NULL;
	...
	while (src[num]) {
		if (num == lsize) {
			...
			rlist = SMB_REALLOC_ARRAY_KEEP_OLD_ON_ERROR(list, char *, lsize +1);
			if (!rlist) {
				DEBUG0(("str_list_copy: Unable to re-allocate memory"));
				str_list_free(&list);
				return False;
			} else {
				list = rlist;
		...
		if (!list[num]) {
			DEBUG0(("str_list_copy: Unable to allocate memory"));
			str_list_free(&list);
			return False;
		}
			 ...
			 const char *domain_name )
{
	...
	while ( *list ) {
		...
		if ( !tmpstr ) {
			DEBUG0(("str_list_sub_basic: alloc_sub_basic() return NULL!\n"));
			return False;
		}
	...
	while (*list) {
		...
		while ((p = strstr_m(s, pattern))) {
			...
			if (ld) {
				...
				if (!t) {
					DEBUG0(("str_list_substitute: Unable to allocate memory"));
					return False;
				}
				memcpy(t, *list, d);
...
int ipstr_list_parse(const char* ipstr_list, struct ip_service **ip_list)
{
	...
	if ( (*ip_list = SMB_MALLOC_ARRAY(struct ip_service, count)) == NULL ) {
		DEBUG0(("ipstr_list_parse: malloc failed for %lu entries\n", (unsigned long)count));
		return 0;
	}
...
char *sstring_sub(const char *src, char front, char back)
{
	...
	if (temp3 == NULL) {
		DEBUG1(("Malloc failure in sstring_sub\n"));
		return NULL;
	}
