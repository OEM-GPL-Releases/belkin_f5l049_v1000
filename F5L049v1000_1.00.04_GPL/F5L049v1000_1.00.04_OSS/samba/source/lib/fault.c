...
static void fault_report(int sig)
{
	...
	DEBUGSEP(0);
	DEBUG0(("INTERNAL ERROR: Signal %d in pid %d (%s)",sig,(int)sys_getpid(),SAMBA_VERSION_STRING));
...
void dump_core_setup(const char *progname)
{
...
#ifdef RLIMIT_CORE
	{
		...
		getrlimit(RLIMIT_CORE, &rlp);
		DEBUG3(("Maximum core file size limits now %d(soft) %d(hard)\n",
 ...
 void dump_core(void)
{
	...
	if (!lp_enable_core_files()) {
		DEBUG0( ("Exiting on internal error (core file administratively disabled)\n"));
		exit(1);
	}
	...
	if (*corepath != '\0') {
		 ...
		 */
		if (chdir(corepath) != 0) {
			DEBUG0( ("unable to change to %s\n", corepath));
			DEBUGADD(0, ("refusing to dump core\n"));
			exit(1);
		}

		DEBUG0(("dumping core in %s\n", corepath));
	}
