...
BOOL gencache_init(void)
{
	...
	DEBUG5( ("Opening cache file at %s\n", cache_fname));
	...
	if (!cache && (errno == EACCES)) {
		...
		if (cache) {
			cache_readonly = True;
			DEBUG5( ("gencache_init: Opening cache file %s read-only.\n", cache_fname));
		}
	}

	if (!cache) {
		DEBUG5( ("Attempt to open gencache.tdb has failed.\n"));
		return False;
	}
	return True;
}
...
BOOL gencache_set(const char *keystr, const char *value, time_t timeout)
{
	...
	DEBUG10( ("Adding cache entry with key = %s; value = %s and timeout ="
...
BOOL gencache_del(const char *keystr)
{
	...
	keybuf.dptr = CONST_DISCARD(char *, keystr);
	keybuf.dsize = strlen(keystr)+1;
	DEBUG10( ("Deleting cache entry (key = %s)\n", keystr));
...
BOOL gencache_get(const char *keystr, char **valstr, time_t *timeout)
{
	...
	if (databuf.dptr == NULL) {
		DEBUG10( ("Cache entry with key = %s couldn't be found\n",
			   keystr));
		return False;
	}
	...
	if ((endptr == NULL) || (*endptr != '/')) {
		DEBUG2( ("Invalid gencache data format: %s\n", databuf.dptr));
		SAFE_FREE(databuf.dptr);
		return False;
	}

	DEBUG10( ("Returning %s cache entry: key = %s, value = %s, "
                      ...
                      void* data, const char* keystr_pattern)
{
	...
	DEBUG5( ("Searching cache keys with pattern %s\n", keystr_pattern));
	...
	while (node) {
		...
		if ( status != 2 ) {
			DEBUG0(("gencache_iterate: invalid return from sscanf %d\n",status));
		}
		timeout = u;
		
		DEBUG10( ("Calling function with arguments (key = %s, value = %s, timeout = %s)\n",
