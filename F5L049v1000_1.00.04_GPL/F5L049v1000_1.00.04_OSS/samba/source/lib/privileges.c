...
static BOOL get_privileges( const DOM_SID *sid, SE_PRIV *mask )
{
	...
	if ( !data.dptr ) {
		DEBUG3(("get_privileges: No privileges assigned to SID [%s]\n",
...
static BOOL set_privileges( const DOM_SID *sid, SE_PRIV *mask )
{
	...
	if ( !sid || (sid->num_auths == 0) ) {
		DEBUG0(("set_privileges: Refusing to store empty SID!\n"));
		return False;
	}
				   ...
				   const SE_PRIV *check )
{
	...
	if ( se_priv_empty( check ) ) {
		DEBUG1(("is_privilege_assigned: no privileges in check_mask!\n"));
		return True;
	}
...
static BOOL is_any_privilege_assigned( SE_PRIV *privileges, const SE_PRIV *check )
{
	...
	if ( se_priv_empty( check ) ) {
		DEBUG1(("is_any_privilege_assigned: no privileges in check_mask!\n"));
		return True;
	}
...
static BOOL privilege_set_add(PRIVILEGE_SET *priv_set, LUID_ATTR set)
{
	...
	if ( !new_set ) {
		DEBUG0(("privilege_set_add: failed to allocate memory!\n"));
		return False;
	}	
...
BOOL get_privileges_for_sids(SE_PRIV *privileges, DOM_SID *slist, int scount)
{
	...
	for ( i=0; i<scount; i++ ) {
		...
		DEBUG5(("get_privileges_for_sids: sid = %s\nPrivilege set:\n", 
...
static int priv_traverse_fn(TDB_CONTEXT *t, TDB_DATA key, TDB_DATA data, void *state)
{
	...
	if ( !string_to_sid(&sid, sid_string) ) {
		DEBUG0(("travsersal_fn_enum__acct: Could not convert SID [%s]\n",
			sid_string));
		return 0;
	}
...
BOOL grant_privilege_by_name(DOM_SID *sid, const char *name)
{
	...
	for ( i=0; !se_priv_equal(&privs[i].se_priv, &se_priv_end); i++ ) {
        ...
        DEBUG3( ("grant_privilege_by_name: No Such Privilege Found (%s)\n", name));
...
BOOL revoke_privilege_by_name(DOM_SID *sid, const char *name)
{
	...
	for ( i=0; !se_priv_equal(&privs[i].se_priv, &se_priv_end); i++ ) {
        ...
        DEBUG3( ("revoke_privilege_by_name: No Such Privilege Found (%s)\n", name));
...
NTSTATUS privilege_set_init(PRIVILEGE_SET *priv_set)
{
	...
	if ( !mem_ctx ) {
		DEBUG0(("privilege_set_init: failed to initialize talloc ctx!\n"));
...
NTSTATUS dup_luid_attr(TALLOC_CTX *mem_ctx, LUID_ATTR **new_la, LUID_ATTR *old_la, int count)
{
	...
	if (count) {
		*new_la = TALLOC_ARRAY(mem_ctx, LUID_ATTR, count);
		if ( !*new_la ) {
			DEBUG0(("dup_luid_attr: failed to alloc new LUID_ATTR array [%d]\n", count));
			return NT_STATUS_NO_MEMORY;
		}
	} else {
		*new_la = NULL;
