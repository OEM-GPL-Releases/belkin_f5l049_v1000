	...
	if (ret && !strcmp(ret, "LOCALE")) {
		...
		if (ln) {
			...
			if (handle == (smb_iconv_t) -1) {
				DEBUG5(("Locale charset '%s' unsupported, using ASCII instead\n", ln));
				ln = NULL;
			} else {
				DEBUG5(("Substituting charset '%s' for LOCALE\n", ln));
	...
	for (c1=0;c1<NUM_CHARSETS;c1++) {
		for (c2=0;c2<NUM_CHARSETS;c2++) {
			...
			if (conv_handles[c1][c2] == (smb_iconv_t)-1) {
				DEBUG0(("init_iconv: Conversion from %s to %s not supported\n",
				...
				DEBUG0(("init_iconv: Attempting to replace with conversion from %s to %s\n",
					n1, n2 ));
				conv_handles[c1][c2] = smb_iconv_open(n2,n1);
				if (!conv_handles[c1][c2]) {
					DEBUG0(("init_iconv: Conversion from %s to %s failed", n1, n2));
		      ...
		      void *dest, size_t destlen, BOOL allow_bad_conv)
{
	...
	if (descriptor == (smb_iconv_t)-1 || descriptor == (smb_iconv_t)0) {
		if (!conv_silent)
			DEBUG0(("convert_string_internal: Conversion not supported.\n"));
		return (size_t)-1;
	}
	...
	if(retval==(size_t)-1) {
		...
		switch(errno) {
			case EINVAL:
				...
				if (!conv_silent)
					DEBUG3(("convert_string_internal: Conversion error: %s(%s)\n",reason,inbuf));
				if (allow_bad_conv)
					goto use_as_is;
				break;
			case E2BIG:
				...
				if (!conv_silent) {
					if (from == CH_UNIX) {
						DEBUG3(("E2BIG: convert_string(%s,%s): srclen=%u destlen=%u - '%s'\n",
							charset_name(from), charset_name(to),
							(unsigned int)srclen, (unsigned int)destlen, (const char *)src));
					} else {
						DEBUG3(("E2BIG: convert_string(%s,%s): srclen=%u destlen=%u\n",
							charset_name(from), charset_name(to),
			...
			default:
				if (!conv_silent)
					DEBUG0(("convert_string_internal: Conversion error: %s(%s)\n",reason,inbuf));
				break;
		}
		/* smb_panic(reason); */
	}
			       ...
			       void const *src, size_t srclen, void *dst, BOOL allow_bad_conv)
{
	...
	*dest = NULL;
	...
	if (descriptor == (smb_iconv_t)-1 || descriptor == (smb_iconv_t)0) {
		if (!conv_silent)
			DEBUG0(("convert_string_allocate: Conversion not supported.\n"));
		return (size_t)-1;
	}
  ...
  convert:
	...
	if ((destlen*2)+2 < destlen) {
		/* wrapped ! abort. */
		if (!conv_silent)
			DEBUG0( ("convert_string_allocate: destlen wrapped !\n"));
		if (!ctx)
			SAFE_FREE(outbuf);
		return (size_t)-1;
	} else {
	...
	if (!ob) {
		DEBUG0( ("convert_string_allocate: realloc failed!\n"));
		return (size_t)-1;
	}
	outbuf = ob;
	i_len = srclen;
 ...
 again:
	...
	if(retval == (size_t)-1) 		{
		...
		switch(errno) {
			case EINVAL:
				reason="Incomplete multibyte sequence";
				if (!conv_silent)
					DEBUG3(("convert_string_allocate: Conversion error: %s(%s)\n",reason,inbuf));
				if (allow_bad_conv)
					goto use_as_is;
				break;
			case E2BIG:
  ...
  out:
	...
	if (destlen && !ob) {
		DEBUG0( ("convert_string_allocate: out of memory!\n"));
		return (size_t)-1;
	}
