...
BOOL share_info_db_init(void)
{
	...
	if (!share_tdb) {
		DEBUG0(("Failed to open share info database %s (%s)\n",
...
SEC_DESC *get_share_security_default( TALLOC_CTX *ctx, size_t *psize, uint32 def_access)
{
	...
	if (!psd) {
		DEBUG0(("get_share_security: Failed to make SEC_DESC.\n"));
		return NULL;
	}
			      ...
			      size_t *psize)
{
	...
	*psize = 0;
	...
	if (tdb_prs_fetch(share_tdb, key, &ps, ctx)!=0 ||
		...
		DEBUG4( ("get_share_security: using default secdesc for %s\n",
			  servicename));
...
BOOL set_share_security(const char *share_name, SEC_DESC *psd)
{
	...
	if (tdb_prs_store(share_tdb, key, &ps)==0) {
		ret = True;
		DEBUG5(("set_share_security: stored secdesc for %s\n", share_name ));
	} else {
		DEBUG1(("set_share_security: Failed to store secdesc for %s\n", share_name ));
	} 
...
BOOL delete_share_security(const struct share_params *params)
{
	...
	if (tdb_trans_delete(share_tdb, kbuf) != 0) {
		DEBUG0(("delete_share_security: Failed to delete entry for share %s\n",
			 lp_servicename(params->service) ));
		return False;
	}
	...
	*ppsd = NULL;
	...
	for (i = 0; i < num_aces; i++) {
		...
		if (!next_token(&pacl, sidstr, ":", sizeof(sidstr))) {
			DEBUG0(("parse_usershare_acl: malformed usershare acl looking "
				"for ':' in string '%s'\n", pacl));
			return False;
		}

		if (!string_to_sid(&sid, sidstr)) {
			DEBUG0(("parse_usershare_acl: failed to convert %s to sid.\n",
				sidstr ));
			return False;
		}
		...
		switch (*pacl) {
			...
			case 'd': /* Deny all to this SID. */
				...
				break;
			default:
				DEBUG0(("parse_usershare_acl: unknown acl type at %s.\n",
					pacl ));
				return False;
		}
		...
		pacl++;
		if (*pacl && *pacl != ',') {
			DEBUG0(("parse_usershare_acl: bad acl string at %s.\n",
				pacl ));
			return False;
		}
		pacl++; /* Go past any ',' */
	...
	if (!psd) {
		DEBUG0(("parse_usershare_acl: Failed to make SEC_DESC.\n"));
		return False;
	}
