 ...
 const char** get_attr_list( TALLOC_CTX *mem_ctx, ATTRIB_MAP_ENTRY table[] )
{
	...
	if ( !names ) {
		DEBUG0(("get_attr_list: out of memory\n"));
		return NULL;
	}
				    ...
				    int max_len)
{
	...
	if (convert_string(CH_UTF8, CH_UNIX,values[0], -1, value, max_len, False) == (size_t)-1) {
		DEBUG1( ("smbldap_get_single_attribute: string conversion of [%s] = [%s] failed!\n", 
			  attribute, values[0]));
					...
					TALLOC_CTX *mem_ctx)
{
	...
	if (values == NULL) {
		DEBUG10( ("attribute %s does not exist\n", attribute));
		return NULL;
	}

	if (ldap_count_values(values) != 1) {
		DEBUG10( ("attribute %s has %d values, expected only one\n",
	...
	if (pull_utf8_talloc(mem_ctx, &result, values[0]) == (size_t)-1) {
		DEBUG10( ("pull_utf8_talloc failed\n"));
		ldap_value_free(values);
		return NULL;
	}
		      ...
		      const char *attribute, const char *newval)
{
	...
	if (existed && newval && (StrCaseCmp(oldval, newval) == 0)) {
		...
		/* Believe it or not, but LDAP will deny a delete and
		   ...
		   same... */
		DEBUG10(("smbldap_make_mod: attribute |%s| not changed.\n", attribute));
		return;
	}
	...
	if (existed) {
		...
		DEBUG10(("smbldap_make_mod: deleting attribute |%s| values |%s|\n", attribute, oldval));
	...
	if ((newval != NULL) && (strlen(newval) > 0)) {
		DEBUG10(("smbldap_make_mod: adding attribute |%s| value |%s|\n", attribute, newval));
...
{ 
	...
	if (version != LDAP_VERSION3) {
		DEBUG0( ("Need LDAPv3 for Start TLS\n"));
		return LDAP_OPERATIONS_ERROR;
	}

	if ((rc = ldap_start_tls_s (ldap_struct, NULL, NULL)) != LDAP_SUCCESS)	{
		DEBUG0(("Failed to issue the StartTLS instruction: %s\n",
			 ldap_err2string(rc)));
		return rc;
	}
	...
	return LDAP_SUCCESS;
#else
	DEBUG0(("StartTLS not supported by LDAP client libraries!\n"));
	return LDAP_OPERATIONS_ERROR;
#endif
}
...
int smb_ldap_setup_conn(LDAP **ldap_struct, const char *uri)
{
	...
	DEBUG10( ("smb_ldap_setup_connection: %s\n", uri));
	
#ifdef HAVE_LDAP_INITIALIZE
	
	rc = ldap_initialize(ldap_struct, uri);
	if (rc) {
		DEBUG0( ("ldap_initialize: %s\n", ldap_err2string(rc)));
	}
...

	{
		...
		if (port == 0) {
			if (strequal(protocol, "ldap")) {
				...
				DEBUG0( ("unrecognised protocol (%s)!\n", protocol));
			}
		}
		
		if ((*ldap_struct = ldap_init(host, port)) == NULL)	{
			DEBUG0( ("ldap_init failed !\n"));
			return LDAP_OPERATIONS_ERROR;
		}
			...
			{
				DEBUG0( ("Failed to setup a TLS session\n"));
			}
			
			DEBUG3(("LDAPS option set...!\n"));
#else
			DEBUG0(("smbldap_open_connection: Secure connection not supported by LDAP client libraries!\n"));
			return LDAP_OPERATIONS_ERROR;
...

{
	...
	DEBUG2( ("smbldap_open_connection: connection opened\n"));
	return rc;
}
				   ...
				   int *methodp, int freeit, void *arg)
{
	...
	if (freeit) {
		...
		DEBUG5(("rebind_proc_with_state: Rebinding as \"%s\"\n", 
					  ...
					  ber_int_t msgid, void *arg)
{
	...
	DEBUG5(("rebindproc_connect_with_state: Rebinding to %s as \"%s\"\n", 
	...
	switch (request) {
		...
		case LDAP_REQ_EXTENDED:
			DEBUG10(("rebindproc_connect_with_state: "
...
static int smbldap_connect_system(struct smbldap_state *ldap_state, LDAP * ldap_struct)
{
	...
	if (!ldap_state->anonymous && !ldap_state->bind_dn) {
		...
		if (!fetch_ldap_pw(&ldap_state->bind_dn, &ldap_state->bind_secret)) {
			DEBUG0( ("ldap_connect_system: Failed to retrieve password from secrets.tdb\n"));
	...
	DEBUG10(("ldap_connect_system: Binding to ldap server %s as \"%s\"\n",
...
# endif
	...
	DEBUG3( ("ldap_connect_system: successful connection to the LDAP server\n"));
...
static int smbldap_open(struct smbldap_state *ldap_state)
{
	...
	if (geteuid() != 0) {
		DEBUG0( ("smbldap_open: cannot access LDAP when not root..\n"));
	...
	DEBUG4(("The LDAP server is successfully connected\n"));
...
static NTSTATUS smbldap_close(struct smbldap_state *ldap_state)
{
	...
	DEBUG5(("The connection to the LDAP server was closed\n"));
			    ...
			    int *attempts, time_t endtime)
{
	...
	while (1) {
		...
		*attempts += 1;
		...
		if (open_rc != LDAP_SUCCESS) {
			DEBUG1( ("Connection to LDAP server failed for the "
				  "%d try!\n", *attempts));
		}
	}
			      ...
			      int sizelimit, LDAPMessage **res)
{
	...
	DEBUG5(("smbldap_search_ext: base => [%s], filter => [%s], "
		 "scope => [%d]\n", base, filter, scope));
	...
	if (ldap_state->last_rebind.tv_sec > 0) {
		...
		if (sleep_time > 0) {
			...
			DEBUG5(("smbldap_search_ext: waiting %d milliseconds "
				 "for LDAP replication.\n",sleep_time));
			smb_msleep(sleep_time);
			DEBUG5(("smbldap_search_ext: go on!\n"));
		}
		ZERO_STRUCT(ldap_state->last_rebind);
	}
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			DEBUG10( ("Failed search for base: %s, error: %d (%s) "
				   "(%s)\n", base, ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
			 ...
			 LDAPMessage **res, void **cookie)
{
	...
	*res = NULL;
	...
	DEBUG3(("smbldap_search_paged: base => [%s], filter => [%s],"
		 "scope => [%d], pagesize => [%d]\n",
		 base, filter, scope, pagesize));

	cookie_be = ber_alloc_t(LBER_USE_DER);
	if (cookie_be == NULL) {
		DEBUG0(("smbldap_create_page_control: ber_alloc_t returns "
			 "NULL\n"));
		return LDAP_NO_MEMORY;
	}
	...
	if (rc != 0) {
		DEBUG3(("smbldap_search_paged: smbldap_search_ext(%s) "
			 "failed with [%s]\n", filter, ldap_err2string(rc)));
		goto done;
	}

	DEBUG3(("smbldap_search_paged: search was successfull\n"));

	rc = ldap_parse_result(ldap_state->ldap_struct, *res, NULL, NULL, 
			       NULL, NULL, &rcontrols,  0);
	if (rc != 0) {
		DEBUG3(("smbldap_search_paged: ldap_parse_result failed " \
			 "with [%s]\n", ldap_err2string(rc)));
		goto done;
	}
...
int smbldap_modify(struct smbldap_state *ldap_state, const char *dn, LDAPMod *attrs[])
{
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->ldap_struct,
					LDAP_OPT_ERROR_STRING, &ld_error);
			DEBUG10( ("Failed to modify dn: %s, error: %d (%s) "
				   "(%s)\n", dn, ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
...
int smbldap_add(struct smbldap_state *ldap_state, const char *dn, LDAPMod *attrs[])
{
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->ldap_struct,
					LDAP_OPT_ERROR_STRING, &ld_error);
			DEBUG10( ("Failed to add dn: %s, error: %d (%s) "
				   "(%s)\n", dn, ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
...
int smbldap_delete(struct smbldap_state *ldap_state, const char *dn)
{
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->ldap_struct,
					LDAP_OPT_ERROR_STRING, &ld_error);
			DEBUG10( ("Failed to delete dn: %s, error: %d (%s) "
				   "(%s)\n", dn, ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
			       ...
			       char **retoidp, struct berval **retdatap)
{
	...
	while (another_ldap_try(ldap_state, &rc, &attempts, endtime)) {
		...
		if (rc != LDAP_SUCCESS) {
			...
			ldap_get_option(ldap_state->ldap_struct,
					LDAP_OPT_ERROR_STRING, &ld_error);
			DEBUG10( ("Extended operation failed with error: "
				   "%d (%s) (%s)\n", ld_errno,
				   ldap_err2string(rc),
				   ld_error ? ld_error : "unknown"));
			SAFE_FREE(ld_error);
...
static void smbldap_idle_fn(void **data, time_t *interval, time_t now)
{
	...
	if (state->ldap_struct == NULL) {
		DEBUG10(("ldap connection not connected...\n"));
		return;
	}
		
	if ((state->last_use+SMBLDAP_IDLE_TIME) > now) {
		DEBUG10(("ldap connection not idle...\n"));
		return;
	}
		
	DEBUG7(("ldap connection idle...closing connection\n"));
	smbldap_close(state);
}
...
NTSTATUS smbldap_init(TALLOC_CTX *mem_ctx, const char *location, struct smbldap_state **smbldap_state) 
{
	*smbldap_state = TALLOC_ZERO_P(mem_ctx, struct smbldap_state);
	if (!*smbldap_state) {
		DEBUG0( ("talloc() failed for ldapsam private_data!\n"));
	...
	if ((*smbldap_state)->event_id == SMB_EVENT_ID_INVALID) {
		DEBUG0(("Failed to register LDAP idle event!\n"));
...
static BOOL smbldap_check_root_dse(LDAP *ld, const char **attrs, const char *value) 
{
	...
	if (!attrs[0]) {
		DEBUG3(("smbldap_check_root_dse: nothing to look for\n"));
		return False;
	}
	...
	if (!strequal(attrs[0], "supportedExtension") && 
		...
		DEBUG3(("smbldap_check_root_dse: no idea what to query root-dse for: %s ?\n", attrs[0]));
		return False;
	}
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG3(("smbldap_check_root_dse: Could not search rootDSE\n"));
		return False;
	}
	...
	if (num_result != 1) {
		DEBUG3(("smbldap_check_root_dse: Expected one rootDSE, got %d\n", num_result));
		goto done;
	}
	...
	if (entry == NULL) {
		DEBUG3(("smbldap_check_root_dse: Could not retrieve rootDSE\n"));
		goto done;
	}
	...
	if (values == NULL) {
		DEBUG5(("smbldap_check_root_dse: LDAP Server does not support any %s\n", attrs[0]));
		goto done;
	}
	...
	if (num_values == 0) {
		DEBUG5(("smbldap_check_root_dse: LDAP Server does not have any %s\n", attrs[0]));
		goto done;
	}
