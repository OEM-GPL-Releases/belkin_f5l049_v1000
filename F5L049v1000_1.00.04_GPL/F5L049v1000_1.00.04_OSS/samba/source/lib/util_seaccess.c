...
void se_map_generic(uint32 *access_mask, struct generic_mapping *mapping)
{
	...
	if (old_mask != *access_mask) {
		DEBUG10( ("se_map_generic(): mapped mask 0x%08x to 0x%08x\n",
...
void se_map_standard(uint32 *access_mask, struct standard_mapping *mapping)
{
	...
	if (old_mask != *access_mask) {
		DEBUG10( ("se_map_standard(): mapped mask 0x%08x to 0x%08x\n",
			   old_mask, *access_mask));
	}
}
		     ...
		     NTSTATUS *status)
{
	...
	*acc_granted = 0;
	...
	DEBUG10(("se_access_check: requested access 0x%08x, for NT token with %u entries and first sid %s.\n",
	...
	if (!sd || (sd && (!(sd->type & SEC_DESC_DACL_PRESENT) || sd->dacl == NULL))) {
		...
		*acc_granted = acc_desired;
		DEBUG5( ("se_access_check: no sd or blank DACL, access allowed\n"));
		return True;
	}

	/* The user sid is the first in the token */
	if (DEBUGLVL(3)) {
		DEBUG3( ("se_access_check: user sid is %s\n", sid_to_string(sid_str, &token->user_sids[PRIMARY_USER_SID_INDEX]) ));
	...
	for ( i = 0 ; i < the_acl->num_aces && tmp_acc_desired != 0; i++) {
		...
		if (NT_STATUS_V(*status)) {
			*acc_granted = 0;
			DEBUG5(("se_access_check: ACE %u denied with status %s.\n", (unsigned int)i, nt_errstr(*status)));
			return False;
		}
	}
	...
	if (tmp_acc_desired == 0) {
		*acc_granted = acc_desired;
		*status = NT_STATUS_OK;
		DEBUG5(("se_access_check: access (%x) granted.\n", (unsigned int)acc_desired ));
		return True;
	}
		
	*acc_granted = 0;
	*status = NT_STATUS_ACCESS_DENIED;
	DEBUG5(("se_access_check: access (%x) denied.\n", (unsigned int)acc_desired ));
	return False;
}
