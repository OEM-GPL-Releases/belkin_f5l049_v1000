...
BOOL sec_desc_equal(SEC_DESC *s1, SEC_DESC *s2)
{
	...
	if (s1->revision != s2->revision) {
		DEBUG10( ("sec_desc_equal(): revision differs (%d != %d)\n",
			   s1->revision, s2->revision));
		return False;
	}

	if (s1->type!= s2->type) {
		DEBUG10( ("sec_desc_equal(): type differs (%d != %d)\n",
			   s1->type, s2->type));
		return False;
	}
	...
	if (!sid_equal(s1->owner_sid, s2->owner_sid)) {
		...
		DEBUG10( ("sec_desc_equal(): owner differs (%s != %s)\n",
			   str1, str2));
		return False;
	}
	...
	if (!sid_equal(s1->group_sid, s2->group_sid)) {
		...
		DEBUG10( ("sec_desc_equal(): group differs (%s != %s)\n",
			   str1, str2));
		return False;
	}
	...
	if ((s1->dacl && !s2->dacl) || (!s1->dacl && s2->dacl) ||
		...
		DEBUG10( ("sec_desc_equal(): dacl or sacl not present\n"));
		return False;
	}
	...
	if (!sec_acl_equal(s1->dacl, s2->dacl) ||
		...
		DEBUG10( ("sec_desc_equal(): dacl/sacl list not equal\n"));
		return False;
	}

 done:
	DEBUG10( ("sec_desc_equal(): secdescs are identical\n"));
	return True;
}
				      ...
				      BOOL child_container)
{
	...
	for (i = 0; i < the_acl->num_aces; i++) {
		...
		DEBUG5( ("se_create_child_secdesc(): %s:%d/0x%02x/0x%08x "
