			 ...
			 void *buf, size_t len, void *private_data)
{
	...
	DEBUG1(("INFO: Received PING message from PID %s [%s]\n",
...
BOOL message_init(void)
{
	...
	if (!tdb) {
		DEBUG0(("ERROR: Failed to initialise messages database\n"));
		return False;
	}
...
static NTSTATUS message_notify(struct process_id procid)
{
	...
	if (ret == -1) {
		if (errno == ESRCH) {
			DEBUG2(("pid %d doesn't exist - deleting messages record\n",
				 (int)pid));
		...
		DEBUG2(("message to process %d failed - %s\n", (int)pid,
			 strerror(errno)));
					  ...
					  unsigned int timeout)
{
	...
	if (duplicates_allowed) {
		...
		if (timeout) {
			if (tdb_chainlock_with_timeout(tdb, kbuf, timeout) == -1) {
				DEBUG0(("message_send_pid_internal: failed to get "
	...
	if (timeout) {
		if (tdb_chainlock_with_timeout(tdb, kbuf, timeout) == -1) {
			DEBUG0(("message_send_pid_internal: failed to get chainlock "
				 "with timeout %ul.\n", timeout));
			SAFE_FREE(dbuf.dptr);
			return NT_STATUS_IO_TIMEOUT;
		}
	...
	for(ptr = (char *)old_dbuf.dptr; ptr < old_dbuf.dptr + old_dbuf.dsize; ) {
		...
		if (!memcmp(ptr, &rec, sizeof(rec))) {
			if (!len || (len && !memcmp( ptr + sizeof(rec), buf, len))) {
				tdb_chainunlock(tdb, kbuf);
				DEBUG10(("message_send_pid_internal: discarding "
					  "duplicate message.\n"));
				SAFE_FREE(dbuf.dptr);
			 ...
			 struct process_id *src, char **buf, size_t *len)
{
	...
	*len = 0;
	...
	if (rec.msg_version != MESSAGE_VERSION) {
		DEBUG0(("message version %d received (expected %d)\n", rec.msg_version, MESSAGE_VERSION));
		return False;
	}
...
void message_dispatch(void)
{
	...
	DEBUG10(("message_dispatch: received_signal = %d\n", received_signal));
	...
	for (buf = msgs_buf; message_recv(msgs_buf, total_len, &msg_type, &src, &buf, &len); buf += len) {
		...
		DEBUG10(("message_dispatch: received msg_type=%d "
			  "src_pid=%u\n", msg_type,
		...
		for (dfn = dispatch_fns; dfn; dfn = dfn->next) {
			if (dfn->msg_type == msg_type) {
				DEBUG10(("message_dispatch: processing message of type %d.\n", msg_type));
				dfn->fn(msg_type, src,
		...
		if (!n_handled) {
			DEBUG5(("message_dispatch: warning: no handler registed for "
				 "msg_type %d in pid %u\n",
		      ...
		      void *private_data)
{
	...
	if (dfn != NULL) {
		...
		DEBUG0(("message_register: Not enough memory. malloc failed!\n"));
	}
}
...
static int traverse_fn(TDB_CONTEXT *the_tdb, TDB_DATA kbuf, TDB_DATA dbuf, void *state)
{
	...
	if (NT_STATUS_EQUAL(status, NT_STATUS_INVALID_HANDLE)) {
		...
		DEBUG2(("pid %s doesn't exist - deleting connections %d [%s]\n",
