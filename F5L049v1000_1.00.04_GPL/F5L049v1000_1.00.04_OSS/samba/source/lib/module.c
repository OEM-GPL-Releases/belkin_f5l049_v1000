...
static NTSTATUS do_smb_load_module(const char *module_name, BOOL is_probe)
{
	...
	error = sys_dlerror();
	if (error) {
		DEBUG0( ("Error trying to resolve symbol 'init_module' in %s: %s\n", 
			  module_name, error));
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG2( ("Module '%s' loaded\n", module_name));

	status = init();
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG0( ("Module '%s' initialization failed: %s\n",
...
int smb_load_modules(const char **modules)
{
	...
	DEBUG2( ("%d modules successfully loaded\n", success));
...
NTSTATUS smb_probe_module(const char *subsystem, const char *module)
{
	...
	DEBUG5( ("Probing module '%s': Trying to load from %s\n", module, full_path));
...
NTSTATUS smb_load_module(const char *module_name)
{
	DEBUG0(("This samba executable has not been built with plugin support\n"));
...
NTSTATUS smb_probe_module(const char *subsystem, const char *module)
{
	DEBUG0(("This samba executable has not been built with plugin support, not probing\n")); 
	return NT_STATUS_NOT_SUPPORTED;
}
