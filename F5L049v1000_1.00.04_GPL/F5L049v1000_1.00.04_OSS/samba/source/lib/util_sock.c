...
static char *get_socket_addr(int fd)
{
	...
	if (getsockname(fd, &sa, &length) < 0) {
		DEBUG0(("getsockname failed. Error was %s\n", strerror(errno) ));
		return addr_buf;
	}
...
static int get_socket_port(int fd)
{
	...
	if (getsockname(fd, &sa, &length) < 0) {
		DEBUG0(("getpeername failed. Error was %s\n", strerror(errno) ));
		return -1;
	}
...
static void print_socket_options(int s)
{
	...
	for (; p->name != NULL; p++) {
		if (getsockopt(s, p->level, p->option, (void *)&value, &vlen) == -1) {
			DEBUG5(("Could not test socket option %s.\n", p->name));
		} else {
			DEBUG5(("socket option %s = %d\n",p->name,value));
			}
		}
	}
 }
...
void set_socket_options(int fd, const char *options)
{
	...
	while (next_token(&options,tok," \t,", sizeof(tok))) {
		...
		if (!socket_options[i].name) {
			DEBUG0(("Unknown socket option %s\n",tok));
			continue;
		}
		...
		case OPT_ON:
			if (got_value)
				DEBUG0(("syntax error - %s does not take a value\n",tok));
		...
		if (ret != 0)
			DEBUG0(("Failed to set socket option %s (Error %s)\n",tok, strerror(errno) ));
	}
...
ssize_t read_udp_socket(int fd,char *buf,size_t len)
{
	...
	if (ret <= 0) {
		...
		if (errno == EAGAIN) {
			DEBUG10(("read socket returned EAGAIN. ERRNO=%s\n",strerror(errno)));
		} else {
			DEBUG2(("read socket failed. ERRNO=%s\n",strerror(errno)));
		}
		return(0);
	}
	...
	DEBUG10(("read_udp_socket: lastip %s lastport %d read: %lu\n",
	...
	while (total < N) {
		...
		if (endtime != NULL) {
			...
			if (!timeout_until(&timeout, endtime)) {
				DEBUG10(("read_data_until: read timed out\n"));
			...
			if (selrtn == -1) {
				...
				DEBUG0(("read_data_until: select error = %s.\n", strerror(errno) ));
			...
			/* Did we timeout ? */
			if (selrtn == 0) {
				DEBUG10(("read_data_until: select timed out.\n"));
				smb_read_error = READ_TIMEOUT;
				return -1;
			}
		}
		...
		if (ret == 0) {
			DEBUG10(("read_data_until: read of %d returned 0. Error = %s\n", (int)(N - total), strerror(errno) ));
		...
		if (ret == -1) {
			...
			DEBUG0(("read_data_until: read failure for %d. Error = %s\n", (int)(N - total), strerror(errno) ));
			smb_read_error = READ_ERROR;
			return -1;
		}
		total += ret;
...
ssize_t read_socket_with_timeout(int fd,char *buf,size_t mincnt,size_t maxcnt,unsigned int time_out)
{
	...
	if (time_out == 0) {
		...
		while (nread < mincnt) {
			...
			if (readret == 0) {
				DEBUG5(("read_socket_with_timeout: blocking read. EOF from client.\n"));
				smb_read_error = READ_EOF;
				return -1;
			}
			...
			if (readret == -1) {
				if (fd == client_fd) {
					...
					DEBUG0(("read_socket_with_timeout: client %s read error = %s.\n",
						client_ip_string, strerror(errno) ));
				} else {
					DEBUG0(("read_socket_with_timeout: read error = %s.\n", strerror(errno) ));
				}
				smb_read_error = READ_ERROR;
				return -1;
			}
	...
	for (nread=0; nread < mincnt; ) {      
		...
		if (selrtn == -1) {
			...
			if (fd == client_fd) {
				/* Try and give an error message saying what client failed. */
				DEBUG0(("read_socket_with_timeout: timeout read for client %s. select error = %s.\n",
					client_ip_string, strerror(errno) ));
			} else {
				DEBUG0(("read_socket_with_timeout: timeout read. select error = %s.\n", strerror(errno) ));
			}
			smb_read_error = READ_ERROR;
			return -1;
		}
		...
		/* Did we timeout ? */
		if (selrtn == 0) {
			DEBUG10(("read_socket_with_timeout: timeout read. select timed out.\n"));
			smb_read_error = READ_TIMEOUT;
			return -1;
		}
		...
		if (readret == 0) {
			...
			DEBUG5(("read_socket_with_timeout: timeout read. EOF from client.\n"));
			smb_read_error = READ_EOF;
			return -1;
		}
		...
		if (readret == -1) {
			...
			if (fd == client_fd) {
				/* Try and give an error message saying what client failed. */
				DEBUG0(("read_socket_with_timeout: timeout read to client %s. read error = %s.\n",
					client_ip_string, strerror(errno) ));
			} else {
				DEBUG0(("read_socket_with_timeout: timeout read. read error = %s.\n", strerror(errno) ));
			}
			smb_read_error = READ_ERROR;
			return -1;
		}
...
ssize_t read_data(int fd,char *buffer,size_t N)
{
	...
	while (total < N) {
		...
		if (ret == 0) {
			DEBUG10(("read_data: read of %d returned 0. Error = %s\n", (int)(N - total), strerror(errno) ));
			smb_read_error = READ_EOF;
			return 0;
		}
		...
		if (ret == -1) {
			if (fd == client_fd) {
				/* Try and give an error message saying what client failed. */
				DEBUG0(("read_data: read failure for %d bytes to client %s. Error = %s\n",
					(int)(N - total), client_ip_string, strerror(errno) ));
			} else {
				DEBUG0(("read_data: read failure for %d. Error = %s\n", (int)(N - total), strerror(errno) ));
			}
			smb_read_error = READ_ERROR;
			return -1;
		}
...
ssize_t write_data(int fd, const char *buffer, size_t N)
{
	...
	while (total < N) {
		...
		if (ret == -1) {
			if (fd == client_fd) {
				/* Try and give an error message saying what client failed. */
				DEBUG0(("write_data: write failure in writing to client %s. Error %s\n",
					client_ip_string, strerror(errno) ));
			} else {
				DEBUG0(("write_data: write failure. Error = %s\n", strerror(errno) ));
			}
			return -1;
		}
...
static ssize_t read_smb_length_return_keepalive(int fd, char *inbuf, unsigned int timeout)
{
	...
	while (!ok) {
		...
		if (msg_type == SMBkeepalive) 
			DEBUG5(("Got keepalive packet\n"));
	}

	DEBUG10(("got smb length of %lu\n",(unsigned long)len));
...
ssize_t read_smb_length(int fd, char *inbuf, unsigned int timeout)
{
	...
	DEBUG10(("read_smb_length: got smb length of %lu\n",
		  (unsigned long)len));
...
BOOL receive_smb_raw(int fd, char *buffer, size_t buflen, unsigned int timeout)
{
	...
	if (len < 0) {
		DEBUG10(("receive_smb_raw: length < 0!\n"));
	...
	if (len > buflen) {
		DEBUG0(("Invalid packet length! (%lu bytes).\n",(unsigned long)len));
...
BOOL receive_smb(int fd, char *buffer, size_t buflen, unsigned int timeout)
{
	...
	if (!srv_check_sign_mac(buffer, True)) {
		DEBUG0( ("receive_smb: SMB Signature verification failed on incoming packet!\n"));
		if (smb_read_error == 0)
...
BOOL send_smb(int fd, char *buffer)
{
	...
	while (nwritten < len) {
		...
		if (ret <= 0) {
			DEBUG0(("Error writing %d bytes to client. %d. (%s)\n",
...
int open_socket_in( int type, int port, int dlevel, uint32 socket_addr, BOOL rebind )
{
	...
	DEBUG10( ( "bind succeeded on port %d\n", port ) );
...
int open_socket_out(int type, struct in_addr *addr, int port ,int timeout)
{
	...
	DEBUG3(("Connecting to %s at port %d\n",inet_ntoa(*addr),port));
  ...
  connect_again:
	...
	if (ret < 0 && (errno == EINPROGRESS || errno == EALREADY ||
			errno == EAGAIN)) {
		DEBUG1(("timeout connecting to %s:%d\n",inet_ntoa(*addr),port));
		close(res);
		return -1;
	}
	...
	if (ret < 0) {
		DEBUG2(("error connecting to %s:%d (%s)\n",
...
static BOOL matchname(char *remotehost,struct in_addr  addr)
{
	...
	if ((hp = sys_gethostbyname(remotehost)) == 0) {
		DEBUG0(("sys_gethostbyname(%s): lookup failure.\n", remotehost));
		return False;
	} 
	...
	if (!strequal(remotehost, hp->h_name)
		...
		DEBUG0(("host name/name mismatch: %s != %s\n",
	...
	DEBUG0(("host name/address mismatch: %s != %s\n",
...
char *get_peer_name(int fd, BOOL force_lookup)
{
	...
	if ((hp = gethostbyaddr((char *)&addr.s_addr, sizeof(addr.s_addr), AF_INET)) == 0) {
		DEBUG1(("Gethostbyaddr failed for %s\n",p));
		pstrcpy(name_buf, p);
	} else {
		pstrcpy(name_buf,(char *)hp->h_name);
		if (!matchname(name_buf, addr)) {
			DEBUG0(("Matchname failed on %s %s\n",name_buf,p));
			pstrcpy(name_buf,"UNKNOWN");
		}
	}
		     ...
		     mode_t dir_perms)
{
	...
	if (lstat(socket_dir, &st) == -1) {
		if (errno == ENOENT) {
			...
			if (mkdir(socket_dir, dir_perms) == -1) {
				DEBUG0( ("error creating socket directory "
					"%s: %s\n", socket_dir, 
					strerror(errno)));
				goto out_umask;
			}
		} else {
			DEBUG0( ("lstat failed on socket directory %s: %s\n",
		...
		if (!S_ISDIR(st.st_mode)) {
			DEBUG0( ("socket directory %s isn't a directory\n",
				socket_dir));
			goto out_umask;
		}
		if ((st.st_uid != sec_initial_uid()) || 
				((st.st_mode & 0777) != dir_perms)) {
			DEBUG0( ("invalid permissions on socket directory "
				"%s\n", socket_dir));
			goto out_umask;
		}
	}
	...
	if (bind(sock, (struct sockaddr *)&sunaddr, sizeof(sunaddr)) == -1) {
		DEBUG0( ("bind failed on pipe socket %s: %s\n", path,
			strerror(errno)));
		goto out_close;
	}
        
	if (listen(sock, 5) == -1) {
		DEBUG0( ("listen failed on pipe socket %s: %s\n", path,
			strerror(errno)));
		goto out_close;
	}
...
#else
        DEBUG0( ("create_pipe_sock: No Unix sockets on this system\n"));
        return -1;
