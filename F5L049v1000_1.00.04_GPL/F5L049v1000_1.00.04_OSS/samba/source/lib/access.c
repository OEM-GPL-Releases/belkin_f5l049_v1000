...
static BOOL masked_match(const char *tok, const char *slash, const char *s)
{
	...
	if (net == INADDR_NONE || mask == INADDR_NONE) {
		DEBUG0(("access: bad net/mask access control: %s\n", tok));
		return (False);
	}
...
static BOOL string_match(const char *tok,const char *s, char *invalid_char)
{
	...
	*invalid_char = '\0';
	...
	if (tok[0] == '.') {			/* domain: match last fields */
		...
		if (!mydomain) {
			DEBUG0(("Unable to get default yp domain. Try without it.\n"));
		}
		if (!(hostname = SMB_STRDUP(s))) {
			DEBUG1(("out of memory for strdup!\n"));
			return False;
		}
		
		netgroup_ok = innetgr(tok + 1, hostname, (char *) 0, mydomain);
		
		DEBUG5(("looking for %s of domain %s in netgroup %s gave %s\n", 
			 hostname,
		...
		if (netgroup_ok)
			return(True);
#else
		DEBUG0(("access: netgroup support is not configured\n"));
		return (False);
#endif
...
static BOOL client_match(const char *tok, const char *item)
{
	...
	if ((match = string_match(tok, client[1], &invalid_char)) == 0) {
		if(invalid_char)
			DEBUG0(("client_match: address match failing due to invalid character '%c' found in \
	...
	for (; *list ; list++) {
		...
		if (!is_ipaddress(*list)) {
			...
			if ((strchr_m(*list, '/')) == NULL) {
				only_ip = False;
				DEBUG3(("only_ipaddrs_in_list: list has non-ip address (%s)\n", *list));
				break;
			}
		}
	}
...
BOOL check_access(int sock, const char **allow_list, const char **deny_list)
{
	...
	if (!ret) {
		...
		if (ret) {
			DEBUG2(("Allowed connection from %s (%s)\n",
				 only_ip ? "" : get_peer_name(sock,True),
				 get_peer_addr(sock)));
		} else {
			DEBUG0(("Denied connection from %s (%s)\n",
				 only_ip ? "" : get_peer_name(sock,True),
				 get_peer_addr(sock)));
		}
	}
