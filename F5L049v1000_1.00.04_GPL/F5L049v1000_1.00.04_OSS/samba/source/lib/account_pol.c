...
BOOL account_policy_get_default(int account_policy, uint32 *val)
{
	...
	DEBUG0(("no default for account_policy index %d found. This should never happen\n", 
		account_policy));
	return False;
}
...
BOOL init_account_policy(void)
{
	...
	if (!tdb) { /* the account policies files does not exist or open failed, try to create a new one */
		...
		if (!tdb) {
			DEBUG0(("Failed to open account policy database\n"));
			return False;
		}
	}
	...
	if (!tdb_fetch_uint32(tdb, vstring, &version) || version != DATABASE_VERSION) {
		...
		for (i=0; account_policy_names[i].field; i++) {
			...
			if (!account_policy_set_default_on_empty(account_policy_names[i].field)) {
				DEBUG0(("failed to set default value in account policy tdb\n"));
				return False;
			}
		}
	}
	...
	if ( lp_enable_privileges() ) {
		if ( !grant_all_privileges( &global_sid_Builtin_Administrators ) ) {
			DEBUG1(("init_account_policy: Failed to grant privileges "
...
BOOL account_policy_get(int field, uint32 *value)
{
	...
	if (name == NULL) {
		DEBUG1( ("account_policy_get: Field %d is not a valid account policy type!  Cannot get, returning 0.\n", field));
		return False;
	}
	
	if (!tdb_fetch_uint32(tdb, name, &regval)) {
		DEBUG1( ("account_policy_get: tdb_fetch_uint32 failed for field %d (%s), returning 0\n", field, name));
		return False;
	}
	...
	DEBUG10(("account_policy_get: name: %s, val: %d\n", name, regval));
	return True;
}
...
BOOL account_policy_set(int field, uint32 value)
{
	...
	name = decode_account_policy_name(field);
	if (name == NULL) {
		DEBUG1( ("Field %d is not a valid account policy type!  Cannot set.\n", field));
		return False;
	}

	if (!tdb_store_uint32(tdb, name, value)) {
		DEBUG1( ("tdb_store_uint32 failed for field %d (%s) on value %u\n", field, name, value));
		return False;
	}

	DEBUG10(("account_policy_set: name: %s, value: %d\n", name, value));
...
BOOL cache_account_policy_set(int field, uint32 value)
{
	...
	if (policy_name == NULL) {
		DEBUG0(("cache_account_policy_set: no policy found\n"));
		return False;
	}
	...
	DEBUG10(("cache_account_policy_set: updating account pol cache\n"));
