...
int debug_lookup_classname(const char *classname)
{
	...
	if (debug_warn_unknown_class) {
		DEBUG0( ("debug_lookup_classname(%s): Unknown class\n",
			  classname));
	}
...
static BOOL debug_parse_params(char **params)
{
	...
	for (; i < debug_num_classes && params[i]; i++) {
		if ((class_name=strtok(params[i],":")) &&
			...
			DEBUG0(("debug_parse_params: unrecognized debug class name or format [%s]\n", params[i]));
			return False;
		}
	}
			  ...
			  void *buf, size_t len, void *private_data)
{
	...
	if (params_str[len-1] != '\0') {
		DEBUG1( ("Invalid debug message from pid %u to pid %u\n",
			  (unsigned int)procid_to_pid(&src),
			  (unsigned int)getpid()));
		return;
	}

	DEBUG3( ("INFO: Remote set of debug to `%s'  (pid %u from pid %u)\n",
			       ...
			       void *buf, size_t len, void *private_data)
{
	...
	if (!message) {
		DEBUG0(("debuglevel_message - debug_list_class_names_and_levels returned NULL\n"));
		return;
	}

	DEBUG1(("INFO: Received REQ_DEBUGLEVEL message from PID %u\n",
		 (unsigned int)procid_to_pid(&src)));
...
BOOL reopen_logs( void )
{
	...
	if (!new_dbf) {
		log_overflow = True;
		DEBUG0( ("Unable to open new log file %s: %s\n", debugf, strerror(errno)));
		log_overflow = False;
		if (dbf)
			x_fflush(dbf);
		ret = False;
...
void check_log_size( void )
{
	...
	if(dbf == NULL) {
		...
		if(dbf) {
			DEBUG0(("check_log_size: open of debug file %s failed - using console.\n",
					debugf ));
		} else {
			/*
 ...
 int Debug1( const char *format_str, ... )
{
	...
	if( syslog_level < lp_syslog() ) {
		 ...
		 * note that not all DEBUG0( ...) calls are
		 * necessarily errors */
 ...
 BOOL dbgtext( const char *format_str, ... )
{
 ...
 static int sxloglevel = LOG_ERR;
 void sxsetloglebel(int level)
{
	sxloglevel = level;
}

 BOOL sxsyslog( const char *format_str, ... )
{
	va_list ap;
	pstring msgbuf;
