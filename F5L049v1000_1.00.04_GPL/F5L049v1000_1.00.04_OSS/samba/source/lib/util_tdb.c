...
static int tdb_chainlock_with_timeout_internal( TDB_CONTEXT *tdb, TDB_DATA key, unsigned int timeout, int rw_type)
{
	...
	if (timeout) {
		...
		if (gotalarm && (ret == -1)) {
			DEBUG0(("tdb_chainlock_with_timeout_internal: alarm (%u) timed out for key %s in tdb %s\n",
...
size_t tdb_pack_va(char *buf, int bufsize, const char *fmt, va_list ap)
{
	...
	while (*fmt) {
		...
		case 'B': /* fixed-length string */
			...
			break;
		default:
			DEBUG0(("Unknown tdb_pack format %c in %s\n", 
				 c, fmt));
			len = 0;
			break;
		}
	...
	while (*fmt) {
		...
		case 'B':
			...
			*b = (char *)SMB_MALLOC(*i);
			...
			break;
		default:
			DEBUG0(("Unknown tdb_unpack format %c in %s\n", 
				 c, fmt));
...
TDB_LIST_NODE *tdb_search_keys(TDB_CONTEXT *tdb, const char* pattern)
{
	...
	for (key = tdb_firstkey(tdb); key.dptr; key = next) {
		...
		if (!key_str) {
			DEBUG0( ("tdb_search_keys: strndup() failed!\n"));
		    ...
		    int flag)
{
	...
	if ((res = tdb_transaction_start(tdb)) != 0) {
		DEBUG5( ("tdb_transaction_start failed\n"));
		return res;
	}

	if ((res = tdb_store(tdb, key, dbuf, flag)) != 0) {
		DEBUG10( ("tdb_store failed\n"));
	...
	if ((res = tdb_transaction_commit(tdb)) != 0) {
		DEBUG5( ("tdb_transaction_commit failed\n"));
	}
