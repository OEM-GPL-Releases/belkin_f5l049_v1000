...
static BOOL upgrade_to_version_3(void)
{
	...
	DEBUG0(("upgrade_to_version_3: upgrading print tdb's to version 3\n"));
	...
	for (kbuf = tdb_firstkey(tdb_drivers); kbuf.dptr;
		...
		if (strncmp(kbuf.dptr, FORMS_PREFIX, strlen(FORMS_PREFIX)) == 0) {
			DEBUG0(("upgrade_to_version_3:moving form\n"));
			if (tdb_store(tdb_forms, kbuf, dbuf, TDB_REPLACE) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG0(("upgrade_to_version_3: failed to move form. Error (%s).\n", tdb_errorstr(tdb_forms)));
				return False;
			}
			if (tdb_delete(tdb_drivers, kbuf) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG0(("upgrade_to_version_3: failed to delete form. Error (%s)\n", tdb_errorstr(tdb_drivers)));
				return False;
			}
		}
 
		if (strncmp(kbuf.dptr, PRINTERS_PREFIX, strlen(PRINTERS_PREFIX)) == 0) {
			DEBUG0(("upgrade_to_version_3:moving printer\n"));
			if (tdb_store(tdb_printers, kbuf, dbuf, TDB_REPLACE) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG0(("upgrade_to_version_3: failed to move printer. Error (%s)\n", tdb_errorstr(tdb_printers)));
				return False;
			}
			if (tdb_delete(tdb_drivers, kbuf) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG0(("upgrade_to_version_3: failed to delete printer. Error (%s)\n", tdb_errorstr(tdb_drivers)));
				return False;
			}
		}
 
		if (strncmp(kbuf.dptr, SECDESC_PREFIX, strlen(SECDESC_PREFIX)) == 0) {
			DEBUG0(("upgrade_to_version_3:moving secdesc\n"));
			if (tdb_store(tdb_printers, kbuf, dbuf, TDB_REPLACE) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG0(("upgrade_to_version_3: failed to move secdesc. Error (%s)\n", tdb_errorstr(tdb_printers)));
				return False;
			}
			if (tdb_delete(tdb_drivers, kbuf) != 0) {
				SAFE_FREE(dbuf.dptr);
				DEBUG0(("upgrade_to_version_3: failed to delete secdesc. Error (%s)\n", tdb_errorstr(tdb_drivers)));
				return False;
			}
		}
                            ...
                            TDB_DATA data, void *state )
{
	...
	if ( !sec_io_desc_buf( "sec_desc_upg_fn", &sd_orig, &ps, 1 ) ) {
		/* delete bad entries */
		DEBUG0(("sec_desc_upg_fn: Failed to parse original sec_desc for %si.  Deleting....\n", key.dptr ));
	...
	if ( !(sd_store = sec_desc_merge( ctx, sd_new, sd_orig )) ) {
		DEBUG0(("sec_desc_upg_fn: Failed to update sec_desc for %s\n", key.dptr ));
		prs_mem_free( &ps );
		return 0;
	}
	...
	if ( !sec_io_desc_buf( "sec_desc_upg_fn", &sd_store, &ps, 1 ) ) {
		DEBUG0(("sec_desc_upg_fn: Failed to parse new sec_desc for %s\n", key.dptr ));
		prs_mem_free( &ps );
		return 0;
	}
...
static BOOL upgrade_to_version_4(void)
{
	...
	DEBUG0(("upgrade_to_version_4: upgrading printer security descriptors\n"));
                                  ...
                                  TDB_DATA data, void *state )
{
	...
	if ( tdb_delete( the_tdb, key ) != 0 ) {
		DEBUG0(("normalize_printers_fn: tdb_delete for [%s] failed!\n", 
			key.dptr));
		return 1;
	}
	
	if ( tdb_store( the_tdb, new_key, data, TDB_REPLACE) != 0 ) {
		DEBUG0(("normalize_printers_fn: failed to store new record for [%s]!\n",
			key.dptr));
		return 1;
	}
...
static BOOL upgrade_to_version_5(void)
{
	...
	DEBUG0(("upgrade_to_version_5: normalizing printer keys\n"));
...
BOOL nt_printing_init(void)
{
	...
	if (!tdb_drivers) {
		DEBUG0(("nt_printing_init: Failed to open nt drivers database %s (%s)\n",
	...
	if (!tdb_printers) {
		DEBUG0(("nt_printing_init: Failed to open nt printers database %s (%s)\n",
	...
	if (!tdb_forms) {
		DEBUG0(("nt_printing_init: Failed to open nt forms database %s (%s)\n",
	...
	if ( vers_id != NTDRIVERS_DATABASE_VERSION_5 ) {
		...
		if ( vers_id != NTDRIVERS_DATABASE_VERSION_5 ) {
			DEBUG0(("nt_printing_init: Unknown printer database version [%d]\n", vers_id));
			return False;
		}
	}
	...
	if ( lp_security() == SEC_ADS ) {
		...
		if (!W_ERROR_IS_OK(win_rc))
			DEBUG0( ("nt_printing_init: error checking published printers: %s\n", dos_errstr(win_rc)));
	}
                                      ...
                                      TDB_DATA data, void *context)
{
	...
	if (memcmp(PRINTERS_PREFIX, key.dptr, sizeof(PRINTERS_PREFIX)-1) == 0) {
		(*printer_count)++;
		DEBUG10(("traverse_counting_printers: printer = [%s]  printer_count = %d\n", key.dptr, *printer_count));
	}
...
uint32 update_c_setprinter(BOOL initialize)
{
	...
	DEBUG10(("update_c_setprinter: c_setprinter = %u\n", (unsigned int)c_setprinter));
...
uint32 get_c_setprinter(void)
{
	...
	DEBUG10(("get_c_setprinter: c_setprinter = %d\n", c_setprinter));
	...
	*list = NULL;
	     ...
	     newkey = tdb_nextkey(tdb_forms, kbuf), safe_free(kbuf.dptr), kbuf=newkey) 
	{
		...
		*list = SMB_REALLOC_ARRAY(*list, nt_forms_struct, n+1);
		if (!*list) {
			DEBUG0(("get_ntforms: Realloc fail.\n"));
			return 0;
		}
		(*list)[n] = form;
		n++;
...
BOOL add_a_form(nt_forms_struct **list, const FORM *form, int *count)
{
	...
	if (update==False) {
		if((*list=SMB_REALLOC_ARRAY(*list, nt_forms_struct, n+1)) == NULL) {
			DEBUG0(("add_a_form: failed to enlarge forms list!\n"));
			return False;
		}
	...
	DEBUG6(("add_a_form: Successfully %s form [%s]\n", 
...
BOOL delete_a_form(nt_forms_struct **list, UNISTR2 *del_name, int *count, WERROR *ret)
{
	...
	*ret = WERR_OK;
	...
	if (n == *count) {
		DEBUG10(("delete_a_form, [%s] not found\n", form_name));
...
int get_ntdrivers(fstring **list, const char *architecture, uint32 version)
{
	...
	for (kbuf = tdb_firstkey(tdb_drivers);
		...
		if((*list = SMB_REALLOC_ARRAY(*list, fstring, total+1)) == NULL) {
			DEBUG0(("get_ntdrivers: failed to enlarge list!\n"));
			return -1;
		}
...
static int get_file_version(files_struct *fsp, char *fname,uint32 *major, uint32 *minor)
{
	...
	if ((buf=(char *)SMB_MALLOC(DOS_HEADER_SIZE)) == NULL) {
		DEBUG0(("get_file_version: PE file [%s] DOS Header malloc failed bytes = %d\n",
				fname, DOS_HEADER_SIZE));
		goto error_exit;
	}

	if ((byte_count = vfs_read_data(fsp, buf, DOS_HEADER_SIZE)) < DOS_HEADER_SIZE) {
		DEBUG3(("get_file_version: File [%s] DOS header too short, bytes read = %lu\n",
	...
	if (SVAL(buf,DOS_HEADER_MAGIC_OFFSET) != DOS_HEADER_MAGIC) {
		DEBUG6(("get_file_version: File [%s] bad DOS magic = 0x%x\n",
	...
	if (SMB_VFS_LSEEK(fsp, fsp->fh->fd, SVAL(buf,DOS_HEADER_LFANEW_OFFSET), SEEK_SET) == (SMB_OFF_T)-1) {
		DEBUG3(("get_file_version: File [%s] too short, errno = %d\n",
				fname, errno));
	...
	if ((byte_count = vfs_read_data(fsp, buf, NE_HEADER_SIZE)) < NE_HEADER_SIZE) {
		DEBUG3(("get_file_version: File [%s] Windows header too short, bytes read = %lu\n",
			 fname, (unsigned long)byte_count));
		/* Assume this isn't an error... the file just looks sort of like a PE/NE file */
		goto no_version_info;
	}
	...
	if (IVAL(buf,PE_HEADER_SIGNATURE_OFFSET) == PE_HEADER_SIGNATURE) {
		...
		if (SMB_VFS_LSEEK(fsp, fsp->fh->fd,
			...
			DEBUG3(("get_file_version: File [%s] Windows optional header too short, errno = %d\n",
				fname, errno));
			goto error_exit;
		}
		...
		if ((buf=(char *)SMB_MALLOC(section_table_bytes)) == NULL) {
			DEBUG0(("get_file_version: PE file [%s] section table malloc failed bytes = %d\n",
					fname, section_table_bytes));
			goto error_exit;
		}

		if ((byte_count = vfs_read_data(fsp, buf, section_table_bytes)) < section_table_bytes) {
			DEBUG3(("get_file_version: PE file [%s] Section header too short, bytes read = %lu\n",
				 fname, (unsigned long)byte_count));
			goto error_exit;
		}
		...
		for (i = 0; i < num_sections; i++) {
			...
			if (strcmp(".rsrc", &buf[sec_offset+PE_HEADER_SECT_NAME_OFFSET]) == 0) {
				...
				if ((buf=(char *)SMB_MALLOC(section_bytes)) == NULL) {
					DEBUG0(("get_file_version: PE file [%s] version malloc failed bytes = %d\n",
							fname, section_bytes));
					goto error_exit;
				}
				...
				if (SMB_VFS_LSEEK(fsp, fsp->fh->fd, section_pos, SEEK_SET) == (SMB_OFF_T)-1) {
					DEBUG3(("get_file_version: PE file [%s] too short for section info, errno = %d\n",
							fname, errno));
					goto error_exit;
				}

				if ((byte_count = vfs_read_data(fsp, buf, section_bytes)) < section_bytes) {
					DEBUG3(("get_file_version: PE file [%s] .rsrc section too short, bytes read = %lu\n",
						 fname, (unsigned long)byte_count));
					goto error_exit;
				}
					...
					if (buf[i] == 'V' && buf[i+1] == '\0' && buf[i+2] == 'S') {
						...
						if (IVAL(buf,pos) == VS_MAGIC_VALUE) {
							...
							*minor = IVAL(buf,pos+VS_MINOR_OFFSET);
							...
							DEBUG6(("get_file_version: PE file [%s] Version = %08x:%08x (%d.%d.%d.%d)\n",
									  fname, *major, *minor,
		...
		DEBUG10(("get_file_version: PE file [%s] has no version info\n", fname));
		SAFE_FREE(buf);
		return 0;

	} else if (SVAL(buf,NE_HEADER_SIGNATURE_OFFSET) == NE_HEADER_SIGNATURE) {
		if (CVAL(buf,NE_HEADER_TARGET_OS_OFFSET) != NE_HEADER_TARGOS_WIN ) {
			DEBUG3(("get_file_version: NE file [%s] wrong target OS = 0x%x\n",
		...
		if ((buf=(char *)SMB_MALLOC(VS_NE_BUF_SIZE)) == NULL) {
			DEBUG0(("get_file_version: NE file [%s] malloc failed bytes  = %d\n",
			...
			for(i=0; i<byte_count; i++) {
				...
				if (i>byte_count-VS_VERSION_INFO_SIZE) {
					...
					if ((bc = vfs_read_data(fsp, &buf[byte_count-i], VS_NE_BUF_SIZE-
						...
						DEBUG0(("get_file_version: NE file [%s] Read error, errno=%d\n",
								 fname, errno));
						goto error_exit;
					}
				...
				if (strcmp(&buf[i], VS_SIGNATURE) == 0) {
					...
					*minor = IVAL(buf,i+sizeof(VS_SIGNATURE)+skip+VS_MINOR_OFFSET);
					DEBUG6(("get_file_version: NE file [%s] Version = %08x:%08x (%d.%d.%d.%d)\n",
							  fname, *major, *minor,
							  (*major>>16)&0xffff, *major&0xffff,
							  (*minor>>16)&0xffff, *minor&0xffff));
					SAFE_FREE(buf);
		...
		/* Version info not found, fall back to origin date/time */
		DEBUG0(("get_file_version: NE file [%s] Version info not found\n", fname));
		SAFE_FREE(buf);
		return 0;

	} else
		/* Assume this isn't an error... the file just looks sort of like a PE/NE file */
		DEBUG3(("get_file_version: File [%s] unknown file format, signature = 0x%x\n",
...
static int file_version_is_newer(connection_struct *conn, fstring new_file, fstring old_file)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG10(("file_version_is_newer: Can't open old file [%s], errno = %d\n",
				filepath, errno));
		return True;
		...
		if (!ret) {
			DEBUG6(("file_version_is_newer: Version info not found [%s], use mod time\n",
					 old_file));
			use_version = False;
	...
	if (!NT_STATUS_IS_OK(status)) {
		...
		DEBUG3(("file_version_is_newer: Can't open new file [%s], errno = %d\n",
				filepath, errno));
		goto error_exit;
	...
	if (use_version && (new_major != old_major || new_minor != old_minor)) {
		...
		if (new_major > old_major ||
			...
			DEBUG6(("file_version_is_newer: Replacing [%s] with [%s]\n", old_file, new_file));
			return True;
		}
		else {
			DEBUG6(("file_version_is_newer: Leaving [%s] unchanged\n", old_file));
			return False;
		}
				   ...
				   struct current_user *user, WERROR *perr)
{
	...
	*perr = WERR_INVALID_PARAM;
	...
	if (strcmp(architecture, SPL_ARCH_WIN40) == 0) {
		DEBUG10(("get_correct_cversion: Driver is Win9x, cversion = 0\n"));
		*perr = WERR_OK;
		return 0;
	}
	...
	if (strcmp(architecture, SPL_ARCH_X64) == 0) {
		DEBUG10(("get_correct_cversion: Driver is x64, cversion = 3\n"));
		*perr = WERR_OK;
		return 3;
	}
	...
	if (conn == NULL) {
		DEBUG0(("get_correct_cversion: Unable to connect\n"));
	...
	if (!become_user(conn, user->vuid)) {
		DEBUG0(("get_correct_cversion: Can't become user!\n"));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG3(("get_correct_cversion: Can't open file [%s], errno = %d\n",
				driverpath, errno));
		*perr = WERR_ACCESS_DENIED;
		goto error_exit;
	} else {
		...
		if (!ret) {
			DEBUG6(("get_correct_cversion: Version info not found [%s]\n", driverpath));
			goto error_exit;
		}
		...
		*/
		...
		switch (cversion) {
			...
			default:
				DEBUG6(("get_correct_cversion: cversion invalid [%s]  cversion = %d\n", 
					driverpath, cversion));
				goto error_exit;
		}

		DEBUG10(("get_correct_cversion: Version info found [%s]  major = 0x%x  minor = 0x%x\n",
				  driverpath, major, minor));
	}

	DEBUG10(("get_correct_cversion: Driver file [%s] cversion = %d\n",
		driverpath, cversion));
				  ...
				  struct current_user *user, WERROR *perr)
{
	...
	*perr = WERR_OK;
	...
	if (level==3)
		...
		DEBUG0(("move_driver_to_download_area: Unknown info level (%u)\n", (unsigned int)level ));
	...
	if (conn == NULL) {
		DEBUG0(("move_driver_to_download_area: Unable to connect\n"));
		*perr = ntstatus_to_werror(nt_status);
	...
	if (!become_user(conn, conn->vuid)) {
		DEBUG0(("move_driver_to_download_area: Can't become user!\n"));
	 ...
	 */
	DEBUG5(("Creating first directory\n"));
	...
	if (driver->driverpath && strlen(driver->driverpath)) {
		...
		if (ver != -1 && (ver=file_version_is_newer(conn, new_name, old_name)) > 0) {
			...
			if ( !NT_STATUS_IS_OK(copy_file(conn, new_name, old_name, OPENX_FILE_EXISTS_TRUNCATE|
				...
				DEBUG0(("move_driver_to_download_area: Unable to rename [%s] to [%s]\n",
						new_name, old_name));
				*perr = WERR_ACCESS_DENIED;
				ver = -1;
			}
...
static uint32 add_a_printer_driver_3(NT_PRINTER_DRIVER_INFO_LEVEL_3 *driver)
{
	...
	DEBUG5(("add_a_printer_driver_3: Adding driver with key %s\n", key ));
 ...
 again:
	...
	if (len != buflen) {
		...
		if (!buf) {
			DEBUG0(("add_a_printer_driver_3: failed to enlarge buffer\n!"));
			ret = -1;
			goto done;
		}
		buflen = len;
	...
	if (ret)
		DEBUG0(("add_a_printer_driver_3: Adding driver with key %s failed.\n", key ));
...
static WERROR get_a_printer_driver_3(NT_PRINTER_DRIVER_INFO_LEVEL_3 **info_ptr, fstring drivername, const char *arch, uint32 version)
{
	...
	DEBUG8(("get_a_printer_driver_3: [%s%s/%d/%s]\n", DRIVERS_PREFIX, architecture, version, drivername));
	...
	while (len < dbuf.dsize) {
		...
		if ( !driver.dependentfiles ) {
			DEBUG0(("get_a_printer_driver_3: failed to enlarge buffer!\n"));
			break;
		}
...
static WERROR update_a_printer_2(NT_PRINTER_INFO_LEVEL_2 *info)
{
 ...
 again:	
	...
	if (buflen != len) {
		buf = (char *)SMB_REALLOC(buf, len);
		if (!buf) {
			DEBUG0(("update_a_printer_2: failed to enlarge buffer!\n"));
			ret = WERR_NOMEM;
			goto done;
		}
		buflen = len;
	...
	if (!W_ERROR_IS_OK(ret))
		DEBUG8( ("error updating printer to tdb on disk\n"));

	SAFE_FREE(buf);

	DEBUG8(("packed printer [%s] with driver [%s] portname=[%s] len=%d\n",
...
NT_DEVICEMODE *construct_nt_devicemode(const fstring default_devicename)
{
	...
	if (nt_devmode == NULL) {
		DEBUG0(("construct_nt_devicemode: malloc fail.\n"));
		return NULL;
	}
...
NT_DEVICEMODE *dup_nt_devicemode(NT_DEVICEMODE *nt_devicemode)
{
	...
	if ((new_nt_devicemode = (NT_DEVICEMODE *)memdup(nt_devicemode, sizeof(NT_DEVICEMODE))) == NULL) {
		DEBUG0(("dup_nt_devicemode: malloc fail.\n"));
		return NULL;
	}
...
int unpack_devicemode(NT_DEVICEMODE **nt_devmode, char *buf, int buflen)
{
	...
	*nt_devmode = (NT_DEVICEMODE *)memdup(&devmode, sizeof(devmode));
	...
	DEBUG8(("Unpacked devicemode [%s](%s)\n", devmode.devicename, devmode.formname));
	if (devmode.nt_dev_private)
		DEBUG8(("with a private section of %d bytes\n", devmode.driverextra));
...
int add_new_printer_key( NT_PRINTER_DATA *data, const char *name )
{
	...
	if ( !(d = TALLOC_REALLOC_ARRAY( data, data->keys, NT_PRINTER_KEY, data->num_keys+1)) ) {
		DEBUG0(("add_new_printer_key: Realloc() failed!\n"));
		return -1;
	}
	...
	DEBUG10(("add_new_printer_key: Inserted new data key [%s]\n", name ));
	...
	if ( strlen(key) == 0 ) {
		for ( i=0; i<data->num_keys; i++ ) {
			...
			if ( !(subkeys_ptr = SMB_REALLOC_ARRAY( subkeys_ptr, fstring, num_subkeys+2)) ) {
				DEBUG0(("get_printer_subkeys: Realloc failed for [%d] entries!\n", 
					num_subkeys+1));
				return -1;
			}
...
WERROR nt_printer_publish(Printer_entry *print_hnd, int snum, int action)
{
	...
	win_rc = mod_a_printer(printer, 2);
	if (!W_ERROR_IS_OK(win_rc)) {
		DEBUG3( ("err %d saving data\n", W_ERROR_V(win_rc)));
		goto done;
	}

	ads = ads_init(lp_realm(), lp_workgroup(), NULL);
	if (!ads) {
		DEBUG3( ("ads_init() failed\n"));
			  ...
			  struct GUID *guid)
{
	...
	if ( guid ) {	
		...
		case REG_BINARY:
			...
			break;
		default:
			DEBUG0(("is_printer_published: GUID value stored as "
...
WERROR delete_all_printer_data( NT_PRINTER_INFO_LEVEL_2 *p2, const char *key )
{
	...
	if ( !strlen(key) ) {
		...
		DEBUG8(("delete_all_printer_data: Removed all Printer Data from printer [%s]\n",
			p2->printername ));
	...
	for ( i=0; i<data->num_keys; i++ ) {
		if ( StrnCaseCmp( data->keys[i].name, key, strlen(key)) == 0 ) {
			DEBUG8(("delete_all_printer_data: Removed all Printer Data from key [%s]\n",
				data->keys[i].name));
	...
	if ( !data->num_keys ) {
		DEBUG8(("delete_all_printer_data: No keys left for printer [%s]\n", p2->printername ));
...
WERROR delete_printer_data( NT_PRINTER_INFO_LEVEL_2 *p2, const char *key, const char *value )
{
	...
	DEBUG8(("delete_printer_data: Removed key => [%s], value => [%s]\n",
		key, value ));
                           ...
                           uint32 type, uint8 *data, int real_len )
{
	...
	DEBUG8(("add_printer_data: Added key => [%s], value => [%s], type=> [%d], size => [%d]\n",
...
REGISTRY_VALUE* get_printer_data( NT_PRINTER_INFO_LEVEL_2 *p2, const char *key, const char *value )
{
	...
	DEBUG8(("get_printer_data: Attempting to lookup key => [%s], value => [%s]\n",
		key, value ));
...
static int unpack_values(NT_PRINTER_DATA *printer_data, char *buf, int buflen)
{
	...
	while ( True ) {
		...
		if ( key_index == -1 ) {
			DEBUG0(("unpack_values: Failed to allocate a new key [%s]!\n",
				keyname));
			break;
		}
		
		DEBUG8(("specific: [%s:%s], len: %d\n", keyname, valuename, size));
...
static void map_to_os2_driver(fstring drivername)
{
	...
	if (strequal(drivername,last_from)) {
		DEBUG3(("Mapped Windows driver %s to OS/2 driver %s\n",drivername,last_to));
	...
	if (numlines == 0 || lines == NULL) {
		DEBUG0(("No entries in OS/2 driver map %s\n",mapfile));
		SAFE_FREE(lines);
		return;
	}

	DEBUG4(("Scanning OS/2 driver map %s\n",mapfile));
	...
	for( i = 0; i < numlines; i++) {
		...
		*os2_name++ = 0;
		...
		if (strequal(nt_name,drivername)) {
			DEBUG3(("Mapped windows driver %s to os2 driver%s\n",drivername,os2_name));
					...
					BOOL get_loc_com)
{
	...
	DEBUG10(("get_a_printer_2_default: driver name set to [%s]\n", info->drivername));
				...
				BOOL get_loc_com)
{
	...
	if (lp_default_devmode(snum) && !info->devmode) {
		DEBUG8(("get_a_printer_2: Constructing a default device mode for [%s]\n",
			printername));
	...
	if ( !(info->data = TALLOC_ZERO_P( info, NT_PRINTER_DATA )) ) {
		DEBUG0(("unpack_values: talloc() failed!\n"));
		SAFE_FREE(dbuf.dptr);
		return WERR_NOMEM;
	}
	...
	DEBUG9(("Unpacked printer [%s] name [%s] running driver [%s]\n",
...
static BOOL set_driver_init_2( NT_PRINTER_INFO_LEVEL_2 *info_ptr )
{
	...
	DEBUG10(("set_driver_init_2: Set printer [%s] init %s DEVMODE for driver [%s]\n",
	...
	if ( !(info_ptr->data = TALLOC_ZERO_P( info_ptr, NT_PRINTER_DATA )) ) {
		DEBUG0(("set_driver_init_2: talloc() failed!\n"));
		return False;
	}
...
BOOL set_driver_init(NT_PRINTER_INFO_LEVEL *printer, uint32 level)
{
	...
	switch (level) {
		...
		default:
			DEBUG0(("set_driver_init: Programmer's error!  Unknown driver_init level [%d]\n",
				level));
			break;
	}
...
BOOL del_driver_init(char *drivername)
{
	...
	if (!drivername || !*drivername) {
		DEBUG3(("del_driver_init: No drivername specified!\n"));
		return False;
	}
	...
	DEBUG6(("del_driver_init: Removing driver init data for [%s]\n", drivername));
...
static uint32 update_driver_init_2(NT_PRINTER_INFO_LEVEL_2 *info)
{
 ...
 again:	
	...
	if (buflen < len) {
		buf = (char *)SMB_REALLOC(buf, len);
		if (!buf) {
			DEBUG0( ("update_driver_init_2: failed to enlarge buffer!\n"));
			ret = -1;
			goto done;
		}
		buflen = len;
	...
	if (ret == -1)
		DEBUG8( ("update_driver_init_2: error updating printer init to tdb on disk\n"));

	SAFE_FREE(buf);

	DEBUG10(("update_driver_init_2: Saved printer [%s] init DEVMODE & values for driver [%s]\n",
...
static BOOL convert_driver_init( TALLOC_CTX *ctx, NT_DEVICEMODE *nt_devmode, uint8 *data, uint32 data_len )
{
	...
	if (spoolss_io_devmode("phantom DEVMODE", &ps, 0, &devmode))
		...
		DEBUG10(("convert_driver_init: error parsing DEVMODE\n"));
...
static WERROR save_driver_init_2(NT_PRINTER_INFO_LEVEL *printer, uint8 *data, uint32 data_len )
{
	 ...
	 */
	DEBUG8(("save_driver_init_2: Enter...\n"));
	...
	if ( !printer->info_2->devmode && data_len ) {
		...
		if ( !convert_driver_init( ctx, nt_devmode, data, data_len )) {
			DEBUG10(("save_driver_init_2: error converting DEVMODE\n"));
	...
	if ( update_driver_init(printer, 2) != 0 ) {
		DEBUG10(("save_driver_init_2: error updating DEVMODE\n"));
		status = WERR_NOMEM;
		goto done;
	}
	...
	if (!W_ERROR_IS_OK(status)) {
		DEBUG10(("save_driver_init_2: error setting DEVMODE on printer [%s]\n",
				  printer->info_2->printername));
	}
			...
			const char *sharename, BOOL get_loc_com)
{
	...
	DEBUG10(("get_a_printer: [%s] level %u\n", sharename, (unsigned int)level));

	if ( !(*pp_printer = TALLOC_ZERO_P(NULL, NT_PRINTER_INFO_LEVEL)) ) {
		DEBUG0(("get_a_printer: talloc() fail.\n"));
		return WERR_NOMEM;
	}
	...
	switch (level) {
		case 2:
			...
			if ( !W_ERROR_IS_OK(result) ) {
				TALLOC_FREE( *pp_printer );
				DEBUG10(("get_a_printer: [%s] level %u returning %s\n", 
...
uint32 free_a_printer(NT_PRINTER_INFO_LEVEL **pp_printer, uint32 level)
{
	...
	switch (level) {
		...
		default:
			DEBUG0(("free_a_printer: unknown level! [%d]\n", level ));
			return 1;
	}
...
BOOL printer_driver_in_use ( NT_PRINTER_DRIVER_INFO_LEVEL_3 *info_3 )
{
	...
	DEBUG10(("printer_driver_in_use: Beginning search through ntprinters.tdb...\n"));
	...
	DEBUG10(("printer_driver_in_use: Completed search through ntprinters.tdb...\n"));
	
	if ( in_use ) {
		NT_PRINTER_DRIVER_INFO_LEVEL d;
		WERROR werr;
		
		DEBUG5(("printer_driver_in_use: driver \"%s\" is currently in use\n", info_3->name));
		...
		if ( !strequal( "Windows NT x86", info_3->environment ) ) {
			...
			case 3:	
				...
				break;
			default:
				DEBUG0(("printer_driver_in_use: ERROR! unknown driver version (%d)\n", 
					info_3->cversion));
				       ...
				       NT_PRINTER_DRIVER_INFO_LEVEL_3 *drv )
{
	...
	while ( *src->dependentfiles[i] ) {
		if ( drv_file_in_use(src->dependentfiles[i], drv) ) {
			in_use = True;
			DEBUG10(("Removing [%s] from dependent file list\n", src->dependentfiles[i]));
...
BOOL printer_driver_files_in_use ( NT_PRINTER_DRIVER_INFO_LEVEL_3 *info )
{
	...
	DEBUG5(("printer_driver_files_in_use: Beginning search through ntdrivers.tdb...\n"));
	...
	DEBUG5(("printer_driver_files_in_use: Completed search through ntdrivers.tdb...\n"));
...
static BOOL delete_driver_files( NT_PRINTER_DRIVER_INFO_LEVEL_3 *info_3, struct current_user *user )
{
	...
	DEBUG6(("delete_driver_files: deleting driver [%s] - version [%d]\n", info_3->name, info_3->cversion));
	...
	if ( !conn ) {
		DEBUG0(("delete_driver_files: Unable to connect\n"));
		return False;
	}

	if ( !CAN_WRITE(conn) ) {
		DEBUG3(("delete_driver_files: Cannot delete print driver when [print$] is read-only\n"));
		return False;
	}
	...
	if ( !become_user(conn, conn->vuid) ) {
		DEBUG0(("delete_driver_files: Can't become user!\n"));
		return False;
	}
	...
	if ( info_3->dependentfiles ) {
		while ( info_3->dependentfiles[i][0] ) {
			...
			if ( (p = strchr( info_3->dependentfiles[i]+1, '\\' )) != NULL ) {
				pstrcpy( file, p );
				driver_unix_convert(file, conn, NULL, &st);
				DEBUG10(("deleting dependent file [%s]\n", file));
				unlink_internals(conn, 0, file, False, False);
			}
                              ...
                              uint32 version, BOOL delete_files )
{
	...
	DEBUG5(("delete_printer_driver: key = [%s] delete_files = %s\n",
	...
	dbuf = tdb_fetch( tdb_drivers, kbuf );
	if ( !dbuf.dptr ) {
		DEBUG8(("delete_printer_driver: Driver unknown [%s]\n", key));
		return WERR_UNKNOWN_PRINTER_DRIVER;
	}
	...
	DEBUG5(("delete_printer_driver: driver delete successful [%s]\n", key));
...
WERROR nt_printing_setsec(const char *sharename, SEC_DESC_BUF *secdesc_ctr)
{
	...
	if (tdb_prs_store(tdb_printers, key, &ps)==0) {
		...
		DEBUG1(("Failed to store secdesc for %s\n", sharename));
		status = WERR_BADFUNC;
	}
...
static SEC_DESC_BUF *construct_default_printer_sdb(TALLOC_CTX *ctx)
{
	...
	if (!psd) {
		DEBUG0(("construct_default_printer_sd: Failed to make SEC_DESC.\n"));
		return NULL;
	}

	sdb = make_sec_desc_buf(ctx, sd_size, psd);

	DEBUG4(("construct_default_printer_sdb: size = %u.\n",
...
BOOL nt_printing_getsec(TALLOC_CTX *ctx, const char *sharename, SEC_DESC_BUF **secdesc_ctr)
{
	...
	if (tdb_prs_fetch(tdb_printers, key, &ps, ctx)!=0 ||
		...
		DEBUG4(("using default secdesc for %s\n", sharename));
