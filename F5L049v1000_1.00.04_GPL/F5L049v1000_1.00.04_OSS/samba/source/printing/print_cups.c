...
static http_t *cups_connect(void)
{
	...
	DEBUG10( ("connecting to cups server %s:%d\n",
		   server, port));

	if ((http = httpConnect(server, port)) == NULL) {
		DEBUG0(("Unable to connect to CUPS server %s:%d - %s\n", 
...
BOOL cups_cache_reload(void)
{
	...
	DEBUG5( ("reloading cups printcap cache\n"));
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/")) == NULL) {
		DEBUG0(("Unable to get printer list - %s\n",
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/jobs")) != NULL) {
		if (response->request.status.status_code >= IPP_OK_CONFLICT) {
			DEBUG0(("Unable to cancel job %d - %s\n", pjob->sysjob,
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
		}
...
static int cups_job_resume(int snum, struct printjob *pjob)
{
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/jobs")) != NULL) {
		if (response->request.status.status_code >= IPP_OK_CONFLICT) {
			DEBUG0(("Unable to release job %d - %s\n", pjob->sysjob,
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
		}
...
static int cups_job_submit(int snum, struct printjob *pjob)
{
	...
	*/
	...
	if ((response = cupsDoFileRequest(http, request, uri, pjob->filename)) != NULL) {
		if (response->request.status.status_code >= IPP_OK_CONFLICT) {
			DEBUG0(("Unable to print file to %s - %s\n", PRINTERNAME(snum),
			         ippErrorString(cupsLastError())));
		} else {
			ret = 0;
		}
               ...
               print_status_struct *status)
{
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/")) == NULL) {
		DEBUG0(("Unable to get jobs for %s - %s\n", uri,
			 ippErrorString(cupsLastError())));
		goto out;
	}
	...
	queue  = NULL;
		...
		*/
		if (qcount >= qalloc) {
			...
			if (queue == NULL) {
				DEBUG0(("cups_queue_get: Not enough memory!"));
				qcount = 0;
				goto out;
			}
		}
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/")) == NULL) {
		DEBUG0(("Unable to get printer status for %s - %s\n", printername,
			 ippErrorString(cupsLastError())));
		*q = queue;
		goto out;
	}
...
static int cups_queue_pause(int snum)
{
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/admin/")) != NULL) {
		if (response->request.status.status_code >= IPP_OK_CONFLICT) {
			DEBUG0(("Unable to pause printer %s - %s\n", PRINTERNAME(snum),
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
		}
...
static int cups_queue_resume(int snum)
{
	...
	*/
	...
	if ((response = cupsDoRequest(http, request, "/admin/")) != NULL) {
		if (response->request.status.status_code >= IPP_OK_CONFLICT) {
			DEBUG0(("Unable to resume printer %s - %s\n", PRINTERNAME(snum),
				ippErrorString(cupsLastError())));
		} else {
			ret = 0;
		}
...
BOOL cups_pull_comment_location(NT_PRINTER_INFO_LEVEL_2 *printer)
{
	...
	if ((response = cupsDoRequest(http, request, "/")) == NULL) {
		DEBUG0(("Unable to get printer attributes - %s\n",
			 ippErrorString(cupsLastError())));
		goto out;
	}
	...
	for (attr = response->attrs; attr != NULL;) {
		...
		while ( attr && (attr->group_tag == IPP_TAG_PRINTER) ) {
			...
			{
				DEBUG5(("cups_pull_comment_location: Using cups comment: %s\n",
			...
			{
				DEBUG5(("cups_pull_comment_location: Using cups location: %s\n",
					 attr->values[0].string.text));				
