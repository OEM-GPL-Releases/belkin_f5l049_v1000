...
uint16 pjobid_to_rap(const char* sharename, uint32 jobid)
{
	...
	if (data.dptr && data.dsize == sizeof(uint16)) {
		...
		SAFE_FREE(data.dptr);
		DEBUG10(("pjobid_to_rap: jobid %u maps to RAP jobid %u\n",
	...
	DEBUG10(("pjobid_to_rap: created jobid %u maps to RAP jobid %u\n",
		(unsigned int)jobid, (unsigned int)rap_jobid));
	return rap_jobid;
}
...
BOOL rap_to_pjobid(uint16 rap_jobid, fstring sharename, uint32 *pjobid)
{
	...
	if ( data.dptr && data.dsize == sizeof(struct rap_jobid_key) ) 
	{
		...
		*pjobid = jinfo->jobid;
		DEBUG10(("rap_to_pjobid: jobid %u maps to RAP jobid %u\n",
			(unsigned int)*pjobid, (unsigned int)rap_jobid));
		SAFE_FREE(data.dptr);
		return True;
	}

	DEBUG10(("rap_to_pjobid: Failed to lookup RAP jobid %u\n",
...
static void rap_jobid_delete(const char* sharename, uint32 jobid)
{
	...
	DEBUG10(("rap_jobid_delete: called.\n"));
	...
	data = tdb_fetch(rap_tdb, key);
	if (!data.dptr || (data.dsize != sizeof(uint16))) {
		DEBUG10(("rap_jobid_delete: cannot find jobid %u\n",
			(unsigned int)jobid ));
		SAFE_FREE(data.dptr);
		return;
	}

	DEBUG10(("rap_jobid_delete: deleting jobid %u\n",
		(unsigned int)jobid ));
...
BOOL print_backend_init(void)
{
	...
	for (snum = 0; snum < services; snum++) {
		...
		if (tdb_lock_bystring(pdb->tdb, sversion) == -1) {
			DEBUG0(("print_backend_init: Failed to open printer %s database\n", lp_const_servicename(snum) ));
...
static struct printjob *print_job_find(const char *sharename, uint32 jobid)
{
	...
	DEBUG10(("print_job_find: looking up job %u for share %s\n",
	...
	if (!ret.dptr) {
		DEBUG10(("print_job_find: failed to find jobid %u.\n", (unsigned int)jobid ));
		return NULL;
	}
	...
	if ( unpack_pjob( ret.dptr, ret.dsize, &pjob ) == -1 ) {
		DEBUG10(("print_job_find: failed to unpack jobid %u.\n", (unsigned int)jobid ));
		SAFE_FREE(ret.dptr);
		return NULL;
	}
	...
	DEBUG10(("print_job_find: returning system job %d for jobid %u.\n",
...
static BOOL pjob_store(const char* sharename, uint32 jobid, struct printjob *pjob)
{
	...
	do {
		...
		if (buflen != len) {
			...
			if (!buf) {
				DEBUG0(("pjob_store: failed to enlarge buffer!\n"));
				goto done;
			}
			newlen = len;
		}
...
void pjob_delete(const char* sharename, uint32 jobid)
{
	...
	if (!pjob) {
		DEBUG5( ("pjob_delete: we were asked to delete nonexistent job %u\n",
					(unsigned int)jobid));
		release_print_db(pdb);
		return;
	}
...
static int traverse_fn_delete(TDB_CONTEXT *t, TDB_DATA key, TDB_DATA data, void *state)
{
	...
	if (!pjob.smbjob) {
		...
		if (i == ts->qcount) {
			DEBUG10(("traverse_fn_delete: pjob %u deleted due to !smbjob\n",
						(unsigned int)jobid ));
	...
	if (!pjob.spooled) {
		...
		if (!process_exists_by_pid(pjob.pid)) {
			DEBUG10(("traverse_fn_delete: pjob %u deleted due to !process_exists (%u)\n",
	...
	if (i == ts->qcount) {
		...
		if (pjob.starttime < ts->lpq_time) {
			DEBUG10(("traverse_fn_delete: pjob %u deleted due to pjob.starttime (%u) < ts->lpq_time (%u)\n",
						(unsigned int)jobid,
...
static void set_updating_pid(const fstring sharename, BOOL updating)
{
	...
	DEBUG5( ("set_updating_pid: %s updating lpq cache for print share %s\n", 
		updating ? "" : "not ",
		sharename ));
...
static BOOL print_cache_expired(const char *sharename, BOOL check_pending)
{
		...
		|| last_qscan_time > (time_now + MAX_CACHE_VALID_TIME)) 
	{
		...
		DEBUG4( ("print_cache_expired: cache expired for queue %s " 
		...
		{
			DEBUG4(("print_cache_expired: message already pending for %s.  Accepting cache\n",
				sharename));
			goto done;
		}
                                         ...
                                         char *lpq_command, char *lprm_command )
{
	...
	DEBUG5(("print_queue_update_internal: printer = %s, type = %d, lpq command = [%s]\n",
	...
	DEBUG3( ("print_queue_update_internal: %d job%s in queue for %s\n", 
	...
	*/
	...
	DEBUG10(("print_queue_update_internal: printer %s INFO/total_jobs = %d\n",
	...
	if (old_status.qcount != qcount)
		DEBUG10(("print_queue_update_internal: queue status change %d jobs -> %d jobs for printer %s\n",
	...
	if ( !tdb_store_uint32( pdb->tdb, keystr, 0 ) ) {
		...
		DEBUG0(("print_queue_update: failed to store MSG_PENDING flag for [%s]!\n",
			sharename));
	}
                                          ...
                                          char *lpq_command, char *lprm_command )
{
	...
	DEBUG5(("print_queue_update_with_lock: printer share = %s\n", sharename));
	pdb = get_print_db_byname(sharename);
	if (!pdb)
		return;

	if ( !print_cache_expired(sharename, False) ) {
		DEBUG5(("print_queue_update_with_lock: print cache for %s is still ok\n", sharename));
		release_print_db(pdb);
		return;
	}
	...
	if (tdb_lock_bystring_with_timeout(pdb->tdb, keystr, 10) == -1) {
		DEBUG0(("print_queue_update_with_lock: Failed to lock printer %s database\n", sharename));
		release_print_db(pdb);
		return;
	}
				...
				void *private_data)
{
	...
	if ( len == -1 ) {
		DEBUG0(("print_queue_receive: Got invalid print queue update message\n"));
		return;
	}
...
{
	DEBUG3(("start_background_queue: Starting background LPQ thread\n"));
	background_lpq_updater_pid = sys_fork();

	if (background_lpq_updater_pid == -1) {
		DEBUG5(("start_background_queue: background LPQ thread failed to start. %s\n", strerror(errno) ));
		exit(1);
	}

	if(background_lpq_updater_pid == 0) {
		/* Child. */
		DEBUG5(("start_background_queue: background LPQ thread started\n"));
		...
		DEBUG5(("start_background_queue: background LPQ thread waiting for messages\n"));
		while (1) {
			pause();
			...
			/* check for some essential signals first */
                        ...
                        if (reload_after_sighup) {
                                change_to_root_user();
                                DEBUG1(("Reloading services after SIGHUP\n"));
			...
			DEBUG10(("start_background_queue: background LPQ thread got a message\n"));
			message_dispatch();
...
static void print_queue_update(int snum, BOOL force)
{
	...
	if ( force || background_lpq_updater_pid == -1 ) {
		DEBUG4(("print_queue_update: updating queue [%s] myself\n", sharename));
	...
	DEBUG10(("print_queue_update: Sending message -> printer = %s, "
...
BOOL print_notify_register_pid(int snum)
{
	...
	if (tdb_lock_bystring_with_timeout(tdb, NOTIFY_PID_LIST_KEY, 10) == -1) {
		DEBUG0(("print_notify_register_pid: Failed to lock printer %s\n",
					printername));
		if (pdb)
			release_print_db(pdb);
		return False;
	...
	if (i == data.dsize) {
		...
		if (!data.dptr) {
			DEBUG0(("print_notify_register_pid: Relloc fail for printer %s\n",
						printername));
			goto done;
		}
		data.dsize += 8;
	...
	if (tdb_store_bystring(tdb, NOTIFY_PID_LIST_KEY, data, TDB_REPLACE) == -1) {
		DEBUG0(("print_notify_register_pid: Failed to update pid \
list for printer %s\n", printername));
		goto done;
	}
...
BOOL print_notify_deregister_pid(int snum)
{
	...
	if (tdb_lock_bystring_with_timeout(tdb, NOTIFY_PID_LIST_KEY, 10) == -1) {
		DEBUG0(("print_notify_register_pid: Failed to lock \
printer %s database\n", printername));
		if (pdb)
			release_print_db(pdb);
		return False;
...
{
	DEBUG2(("print_job_set_place not implemented yet\n"));
	return False;
}
...
static BOOL remove_from_jobs_changed(const char* sharename, uint32 jobid)
{
  ...
  out:
	...
	release_print_db(pdb);
	if (ret){
		DEBUG10(("remove_from_jobs_changed: removed jobid %u\n", (unsigned int)jobid ));}
	else
		DEBUG10(("remove_from_jobs_changed: Failed to remove jobid %u\n", (unsigned int)jobid ));
	return ret;
}
...
static BOOL print_job_delete1(int snum, uint32 jobid)
{
	...
	if (pjob->sysjob == -1) {
		DEBUG5( ("attempt to delete job %u not seen by lpr\n", (unsigned int)jobid));
	}
...
BOOL print_job_delete(struct current_user *user, int snum, uint32 jobid, WERROR *errcode)
{
	...
	*errcode = WERR_OK;
	...
	if (!owner && 
		...
		DEBUG3( ("delete denied by security descriptor\n"));
...
pause, or resume print job. User name: %s. Printer name: %s.",
	...
	if ( (fname = print_job_fname( sharename, jobid )) != NULL )
	{
		...
		DEBUG10(("print_job_delete: Removing spool file [%s]\n", fname ));
		if ( unlink( fname ) == -1 ) {
...
BOOL print_job_pause(struct current_user *user, int snum, uint32 jobid, WERROR *errcode)
{
	...
	if (!pjob || !user) {
		DEBUG10( ("print_job_pause: no pjob or user for jobid %u\n",
			(unsigned int)jobid ));
		return False;
	}

	if (!pjob->spooled || pjob->sysjob == -1) {
		DEBUG10( ("print_job_pause: not spooled or bad sysjob = %d for jobid %u\n",
			(int)pjob->sysjob, (unsigned int)jobid ));
		return False;
	}
	...
	if (!is_owner(user, lp_const_servicename(snum), jobid) &&
	    !print_access_check(user, snum, JOB_ACCESS_ADMINISTER)) {
		DEBUG3( ("pause denied by security descriptor\n"));
...
BOOL print_job_resume(struct current_user *user, int snum, uint32 jobid, WERROR *errcode)
{
	...
	if (!pjob || !user) {
		DEBUG10( ("print_job_resume: no pjob or user for jobid %u\n",
			(unsigned int)jobid ));
		return False;
	}

	if (!pjob->spooled || pjob->sysjob == -1) {
		DEBUG10( ("print_job_resume: not spooled or bad sysjob = %d for jobid %u\n",
			(int)pjob->sysjob, (unsigned int)jobid ));
		return False;
	}
	...
	if (!is_owner(user, lp_const_servicename(snum), jobid) &&
	    !print_access_check(user, snum, JOB_ACCESS_ADMINISTER)) {
		DEBUG3( ("resume denied by security descriptor\n"));
		*errcode = WERR_ACCESS_DENIED;
	...
	*pjobid = (uint32)-1;
	...
	for (i = 0; i < 3; i++) {
		...
		if (tdb_lock_bystring_with_timeout(pdb->tdb, "INFO/nextjob", 20) == -1) {
			DEBUG0(("allocate_print_jobid: failed to lock printing database %s\n", sharename));
			return False;
		}

		if (!tdb_fetch_uint32(pdb->tdb, "INFO/nextjob", &jobid)) {
			if (tdb_error(pdb->tdb) != TDB_ERR_NOEXIST) {
				DEBUG0( ("allocate_print_jobid: failed to fetch INFO/nextjob for print queue %s\n",
					sharename));
				return False;
			}
			jobid = 0;
		...
		if (tdb_store_int32(pdb->tdb, "INFO/nextjob", jobid)==-1) {
			DEBUG3( ("allocate_print_jobid: failed to store INFO/nextjob.\n"));
	...
	if (i > 2) {
		DEBUG0( ("allocate_print_jobid: failed to allocate a print job for queue %s\n",
			sharename));
		/* Probably full... */
		errno = ENOSPC;
		return False;
	...
	/* Store a dummy placeholder. */
	{
		...
		if (tdb_store(pdb->tdb, print_key(jobid), dum, TDB_INSERT) == -1) {
			DEBUG3( ("allocate_print_jobid: jobid (%d) failed to store placeholder.\n",
				jobid ));
			return False;
		}
	}
...
static BOOL add_to_jobs_changed(struct tdb_print_db *pdb, uint32 jobid)
{
	...
	DEBUG10(("add_to_jobs_changed: Added jobid %u\n", (unsigned int)jobid ));
...
uint32 print_job_start(struct current_user *user, int snum, char *jobname, NT_DEVICEMODE *nt_devmode )
{
	...
	if (!print_access_check(user, snum, PRINTER_ACCESS_USE)) {
		DEBUG3( ("print_job_start: job start denied by security descriptor\n"));
		release_print_db(pdb);
		return (uint32)-1;
	}

	if (!print_time_access_check(lp_servicename(snum))) {
		DEBUG3( ("print_job_start: job start denied by time check\n"));
		release_print_db(pdb);
		return (uint32)-1;
	}
	...
	if (lp_minprintspace(snum)) {
		...
		if (sys_fsusage(path, &dspace, &dsize) == 0 &&
			...
			DEBUG3( ("print_job_start: disk space check failed.\n"));
			release_print_db(pdb);
			errno = ENOSPC;
			return (uint32)-1;
		}
	...
	if (lp_autoloaded(snum) && !pcap_printername_ok(lp_const_servicename(snum))) {
		DEBUG3( ("print_job_start: printer name %s check failed.\n", lp_const_servicename(snum) ));
		release_print_db(pdb);
		errno = ENOENT;
		return (uint32)-1;
	}
	...
	if ((njobs = print_queue_length(snum,NULL)) > lp_maxprintjobs(snum)) {
		DEBUG3( ("print_job_start: Queue %s number of jobs (%d) larger than max printjobs per queue (%d).\n",
	...
	DEBUG10(("print_job_start: Queue %s number of jobs (%d), max printjobs = %d\n",
		sharename, njobs, lp_maxprintjobs(snum) ));
	...
	if (pjob.fd == -1) {
		if (errno == EACCES) {
			...
			DEBUG0( ("print_job_start: insufficient permissions \
to open spool file %s.\n", pjob.filename));
		} else {
			/* Normal case, report at level 3 and above. */
			DEBUG3( ("print_job_start: can't open spool file %s,\n", pjob.filename));
 ...
 fail:
	...
	DEBUG3( ("print_job_start: returning fail. Error = %s\n", strerror(errno) ));
	return (uint32)-1;
}
...
BOOL print_job_end(int snum, uint32 jobid, enum file_close_type close_type)
{
	...
	if ((close_type == NORMAL_CLOSE || close_type == SHUTDOWN_CLOSE) &&
		...
		close(pjob->fd);
		pjob->fd = -1;
		DEBUG3(("print_job_end: failed to stat file for jobid %d\n", jobid ));
		goto fail;
	}
	...
	if (pjob->size == 0 || pjob->status == LPQ_DELETING) {
		...
		DEBUG5(("print_job_end: canceling spool of %s (%s)\n",
...
static BOOL get_stored_queue_info(struct tdb_print_db *pdb, int snum, int *pcount, print_queue_struct **ppqueue)
{
	...
	*ppqueue = NULL;
	...
	DEBUG5(("get_stored_queue_info: qcount = %u, extra_count = %u\n", (unsigned int)qcount, (unsigned int)extra_count));
	...
	for( i  = 0; i < extra_count; i++) {
		...
		DEBUG5(("get_stored_queue_info: changed job = %u\n", (unsigned int)jobid));
		pjob = print_job_find(lp_const_servicename(snum), jobid);
		if (!pjob) {
			DEBUG5(("get_stored_queue_info: failed to find changed job = %u\n", (unsigned int)jobid));
			remove_from_jobs_changed(sharename, jobid);
			continue;
		}
	...
	DEBUG5(("get_stored_queue_info: total_count = %u\n", (unsigned int)total_count));
