				...
				DOM_CHAL *srv_chal_out)
{
	...
	DEBUG4(("cli_net_req_chal: LSA Request Challenge from %s to %s\n",
		       ...
		       uint32 *neg_flags, DOM_CHAL *srv_chal)
{
	...
	if ( sec_chan == SEC_CHAN_DOMAIN )
        ...
        DEBUG4(("cli_net_auth2: srv:%s acct:%s sc:%x mc: %s chal %s neg: %x\n",
	...
	CLI_DO_RPC(cli, mem_ctx, PI_NETLOGON, NET_AUTH2,
        ...
        if (NT_STATUS_IS_OK(result)) {
                ...
                if (cred_assert( &r.srv_chal, cli->sess_key, srv_chal, zerotime) == 0) {
                         ...
                         */
                        DEBUG0(("cli_net_auth2: server %s replied with bad credential (bad machine \
password ?).\n", cli->cli->desthost ));
			...
			DOM_CHAL *srv_chal_out)
{
        ...
        DEBUG4(("cli_net_auth2: srv:%s acct:%s sc:%x mc: %s neg: %x\n",
			...
			DOM_CHAL *srv_chal_out)
{
        ...
        DEBUG4(("cli_net_auth3: srv:%s acct:%s sc:%x mc: %s chal %s neg: %x\n",
		server_name, account_name, sec_chan_type, computer_name,
				...
				uint32 *neg_flags_inout)
{
 ...
 again:
	...
	if (!creds_client_check(dc, &srv_chal_recv)) {
		 ...
		 * Server replied with bad credential. Fail.
		 */
		DEBUG0(("rpccli_netlogon_setup_creds: server %s "
			"replied with bad credential\n",
			cli->cli->desthost ));
		return NT_STATUS_ACCESS_DENIED;
	}

	DEBUG5(("rpccli_netlogon_setup_creds: server %s credential "
		"chain established.\n",
		cli->cli->desthost ));
                               ...
                               SAM_DELTA_CTR **deltas)
{
	...
	if (!NT_STATUS_IS_ERR(result)) {
		...
		if (!creds_client_check(cli->dc, &r.srv_creds.challenge)) {
			DEBUG0(("cli_netlogon_sam_sync: credentials chain check failed\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	}
				   ...
				   int logon_type)
{
	...
	creds_client_step(cli->dc, &clnt_creds);
        ...
        default:
                DEBUG0( ("switch value %d not supported\n", 
                          ctr.switch_value));
	...
	if (r.buffer_creds) {
		...
		if (!creds_client_check(cli->dc, &r.srv_creds.challenge)) {
			DEBUG0(("rpccli_netlogon_sam_logon: credentials chain check failed\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	}
					   ...
					   NET_USER_INFO_3 *info3)
{
	...
	if (r.buffer_creds) {
		/* Check returned credentials if present. */
		if (!creds_client_check(cli->dc, &r.srv_creds.challenge)) {
			DEBUG0(("rpccli_netlogon_sam_network_logon: credentials chain check failed\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	}
			   ...
			   const char *machine_name, const uint8 hashed_mach_pwd[16])
{
	...
	DEBUG4(("cli_net_srv_pwset: srv:%s acct:%s sc: %d mc: %s\n",
	...
	if (!NT_STATUS_IS_OK(result)) {
		/* report error code */
		DEBUG0(("cli_net_srv_pwset: %s\n", nt_errstr(result)));
	}

	/* Always check returned credentials. */
	if (!creds_client_check(cli->dc, &r.srv_cred.challenge)) {
		DEBUG0(("rpccli_net_srv_pwset: credentials chain check failed\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
			       ...
			       const char *clear_text_mach_pwd)
{
	...
	DEBUG4(("cli_net_srv_pwset2: srv:%s acct:%s sc: %d mc: %s\n",
		 cli->dc->remote_machine, cli->dc->mach_acct, sec_chan_type, machine_name));
	...
	if (!NT_STATUS_IS_OK(result)) {
		/* report error code */
		DEBUG0(("cli_net_srv_pwset2: %s\n", nt_errstr(result)));
	}

	/* Always check returned credentials. */
	if (!creds_client_check(cli->dc, &r.srv_cred.challenge)) {
		DEBUG0(("rpccli_net_srv_pwset2: credentials chain check failed\n"));
		return NT_STATUS_ACCESS_DENIED;
	}
