...
static int map_pipe_auth_type_to_rpc_auth_type(enum pipe_auth_type auth_type)
{
	...
	default:
		DEBUG0(("map_pipe_auth_type_to_rpc_type: unknown pipe "
			"auth type %u\n",
			...
			uint32 *current_pdu_offset)
{
	...
	DEBUG5(("rpc_read: data_to_read: %u current_pdu offset: %u extra_data_size: %d\n",
	...
	if (extra_data_size > 0) {
		if(!prs_force_grow(current_pdu, (uint32)extra_data_size)) {
			DEBUG0(("rpc_read: Failed to grow parse struct by %d bytes.\n", (int)extra_data_size ));
			return NT_STATUS_NO_MEMORY;
		}
		DEBUG5(("rpc_read: grew buffer by %d bytes to %u\n", (int)extra_data_size, prs_data_size(current_pdu) ));
	}
	...
	do {
		...
		DEBUG5(("rpc_read: num_read = %d, read offset: %u, to read: %u\n",
		...
		if (cli_is_dos_error(cli->cli)) {
			...
			if (eclass != ERRDOS && ecode != ERRmoredata) {
				DEBUG0(("rpc_read: DOS Error %d/%u (%s) in cli_read on pipe %s\n",
		...
		if (cli_is_nt_error(cli->cli)) {
			if (!NT_STATUS_EQUAL(cli_nt_error(cli->cli), NT_STATUS_BUFFER_TOO_SMALL)) {
				DEBUG0(("rpc_read: Error (%s) in cli_read on pipe %s\n",
		...
		if (num_read == -1) {
			DEBUG0(("rpc_read: Error - cli_read on pipe %s returned -1\n",
				cli->pipe_name ));
...
static NTSTATUS cli_pipe_get_current_pdu(struct rpc_pipe_client *cli, RPC_HDR *prhdr, prs_struct *current_pdu)
{
	...
	if(!smb_io_rpc_hdr("rpc_hdr   ", prhdr, current_pdu, 0)) {
		DEBUG0(("cli_pipe_get_current_pdu: Failed to unmarshall RPC_HDR.\n"));
				...
				uint8 *p_ss_padding_len)
{
	...
	if ((auth_len > RPC_MAX_SIGN_SIZE) ||
		...
		DEBUG0(("cli_pipe_verify_ntlmssp: auth_len %u is too large.\n",
	...
	if(!prs_set_offset(current_pdu, RPC_HEADER_LEN + RPC_HDR_RESP_LEN + data_len)) {
		DEBUG0(("cli_pipe_verify_ntlmssp: cannot move offset to %u.\n",
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, current_pdu, 0)) {
		DEBUG0(("cli_pipe_verify_ntlmssp: failed to unmarshall RPC_HDR_AUTH.\n"));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
	...
	switch (cli->auth.auth_level) {
		case PIPE_AUTH_LEVEL_PRIVACY:
			...
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG0(("cli_pipe_verify_ntlmssp: failed to unseal "
		...
		case PIPE_AUTH_LEVEL_INTEGRITY:
			...
			status = ntlmssp_check_packet(ntlmssp_state,
							...
							&auth_blob);
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG0(("cli_pipe_verify_ntlmssp: check signing failed on "
					"packet from remote machine %s on pipe %s "
					"fnum 0x%x. Error was %s.\n",
					cli->cli->desthost,
					cli->pipe_name,
			...
			break;
		default:
			DEBUG0(("cli_pipe_verify_ntlmssp: unknown internal auth level %d\n",
				cli->auth.auth_level ));
				...
				uint8 *p_ss_padding_len)
{
	...
	if (auth_len != RPC_AUTH_SCHANNEL_SIGN_OR_SEAL_CHK_LEN) {
		DEBUG0(("cli_pipe_verify_schannel: auth_len %u.\n", (unsigned int)auth_len ));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if ((auth_len > RPC_MAX_SIGN_SIZE) ||
			(RPC_HEADER_LEN + RPC_HDR_RESP_LEN + RPC_HDR_AUTH_LEN + auth_len > prhdr->frag_len)) {
		DEBUG0(("cli_pipe_verify_schannel: auth_len %u is too large.\n",
			(unsigned int)auth_len ));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if(!prs_set_offset(current_pdu, RPC_HEADER_LEN + RPC_HDR_RESP_LEN + data_len)) {
		DEBUG0(("cli_pipe_verify_schannel: cannot move offset to %u.\n",
			(unsigned int)RPC_HEADER_LEN + RPC_HDR_RESP_LEN + data_len ));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
                                                                                                                             
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, current_pdu, 0)) {
		DEBUG0(("cli_pipe_verify_schannel: failed to unmarshall RPC_HDR_AUTH.\n"));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}

	if (auth_info.auth_type != RPC_SCHANNEL_AUTH_TYPE) {
		DEBUG0(("cli_pipe_verify_schannel: Invalid auth info %d on schannel\n",
			auth_info.auth_type));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
	...
	if(!smb_io_rpc_auth_schannel_chk("", RPC_AUTH_SCHANNEL_SIGN_OR_SEAL_CHK_LEN,
		...
		DEBUG0(("cli_pipe_verify_schannel: failed to unmarshal RPC_AUTH_SCHANNEL_CHK.\n"));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
	...
	if (!schannel_decode(schannel_auth,
			...
			data_len)) {
		DEBUG3(("cli_pipe_verify_schannel: failed to decode PDU "
				...
				uint8 *p_ss_padding_len)
{
	...
	switch(cli->auth.auth_type) {
		case PIPE_AUTH_TYPE_NONE:
			if (prhdr->auth_len) {
				DEBUG3( ("cli_pipe_validate_rpc_response: Connection to remote machine %s "
			...
			prs_struct *return_data)
{
	...
	if (current_pdu_len != prhdr->frag_len) {
		DEBUG5(("cli_pipe_validate_current_pdu: incorrect pdu length %u, expected %u\n",
	...
	*pdata_len = current_pdu_len;
	...
	switch (prhdr->pkt_type) {
		...
		case RPC_RESPONSE:
		{
			...
			if(!smb_io_rpc_hdr_resp("rpc_hdr_resp", &rhdr_resp, current_pdu, 0)) {
				DEBUG5(("cli_pipe_validate_current_pdu: failed to unmarshal RPC_HDR_RESP.\n"));
				return NT_STATUS_BUFFER_TOO_SMALL;
			}
			...
			*pdata_len = current_pdu_len - RPC_HEADER_LEN - RPC_HDR_RESP_LEN - ss_padding_len;
			...
			DEBUG10(("cli_pipe_validate_current_pdu: got pdu len %u, data_len %u, ss_len %u\n",
			...
			if ((prs_data_size(return_data) == 0) && rhdr_resp.alloc_hint && (rhdr_resp.alloc_hint < 15*1024*1024)) {
				if (!prs_set_buffer_size(return_data, rhdr_resp.alloc_hint)) {
					DEBUG0(("cli_pipe_validate_current_pdu: reply alloc hint %u "
		...
		case RPC_BINDNACK:
			DEBUG1( ("cli_pipe_validate_current_pdu: Bind NACK received from remote machine %s "
				"pipe %s fnum 0x%x!\n",
				cli->cli->desthost,
				cli->pipe_name,
				(unsigned int)cli->fnum));
		...
		case RPC_FAULT:
		{
			...
			if(!smb_io_rpc_hdr_fault("fault", &fault_resp, current_pdu, 0)) {
				DEBUG5(("cli_pipe_validate_current_pdu: failed to unmarshal RPC_HDR_FAULT.\n"));
				return NT_STATUS_BUFFER_TOO_SMALL;
			}

			DEBUG1( ("cli_pipe_validate_current_pdu: RPC fault code %s received from remote machine %s "
				"pipe %s fnum 0x%x!\n",
		...
		default:
			DEBUG0( ("cli_pipe_validate_current_pdu: unknown packet type %u received "
	...
	if (prhdr->pkt_type != expected_pkt_type) {
		DEBUG3( ("cli_pipe_validate_current_pdu: Connection to remote machine %s "
	...
	if ((prhdr->pkt_type == RPC_BINDACK) && !(prhdr->flags & RPC_FLG_LAST)) {
		DEBUG5(("cli_pipe_validate_current_pdu: bug in server (AS/U?), "
			...
			uint8 expected_pkt_type)
{
	...
	DEBUG5(("rpc_api_pipe: Remote machine %s pipe %s fnum 0x%x\n",
		cli->cli->desthost,
		cli->pipe_name,
		(unsigned int)cli->fnum ));
	...
	{
		DEBUG0( ("rpc_api_pipe: Remote machine %s pipe %s fnum 0x%x"
	...
	if (prdata == NULL) {
		DEBUG3(("rpc_api_pipe: Remote machine %s pipe %s "
	...
	while(1) {
		...
		DEBUG10(("rpc_api_pipe: got PDU len of %u at offset %u\n",
		...
		if ((rhdr.flags & RPC_FLG_FIRST)) {
			if (rhdr.pack_type[0] == 0) {
				...
				DEBUG10(("rpc_api_pipe: On machine %s pipe %s fnum 0x%x "
				...
				if (current_pdu.bigendian_data != rbuf->bigendian_data) {
					DEBUG0(("rpc_api_pipe: Error : Endianness changed from %s to %s\n",
	...
	DEBUG10(("rpc_api_pipe: Remote machine %s pipe %s fnum 0x%x returned %u bytes.\n",
		cli->cli->desthost,
		cli->pipe_name,
		(unsigned int)cli->fnum,
						...
						prs_struct *auth_data)
{
	...
	DEBUG5( ("create_krb5_auth_bind_req: creating a service ticket for principal %s\n",
		a->service_principal ));
	...
	if (ret) {
		DEBUG1(("create_krb5_auth_bind_req: cli_krb5_get_ticket for principal %s "
			"failed with %s\n",
			a->service_principal,
			error_message(ret) ));
	...
	DEBUG5( ("create_krb5_auth_bind_req: Created krb5 GSS blob :\n"));
						...
						prs_struct *auth_data)
{
	...
	DEBUG5( ("create_spnego_ntlmssp_auth_rpc_bind_req: Processing NTLMSSP Negotiate\n"));
	...
	DEBUG5( ("create_spnego_ntlmssp_auth_rpc_bind_req: NTLMSSP Negotiate:\n"));
						...
						prs_struct *auth_data)
{
	...
	DEBUG5( ("create_ntlmssp_auth_rpc_bind_req: Processing NTLMSSP Negotiate\n"));
	nt_status = ntlmssp_update(cli->auth.a_u.ntlmssp_state,
					null_blob,
					&request);
	...
	DEBUG5( ("create_ntlmssp_auth_rpc_bind_req: NTLMSSP Negotiate:\n"));
						...
						prs_struct *auth_data)
{
	...
	if(!smb_io_rpc_auth_schannel_neg("schannel_neg",
		...
		DEBUG0(("Failed to marshall RPC_AUTH_SCHANNEL_NEG.\n"));
		prs_mem_free(auth_data);
		return NT_STATUS_NO_MEMORY;
	}
						...
						prs_struct *pauth_info)
{
	...
	if(!smb_io_rpc_hdr("hdr"   , &hdr, rpc_out, 0)) {
		DEBUG0(("create_bind_or_alt_ctx_internal: failed to marshall RPC_HDR.\n"));
		return NT_STATUS_NO_MEMORY;
	}

	/* Marshall the bind request data */
	if(!smb_io_rpc_hdr_rb("", &hdr_rb, rpc_out, 0)) {
		DEBUG0(("create_bind_or_alt_ctx_internal: failed to marshall RPC_HDR_RB.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if(auth_len != 0) {
		if (ss_padding_len) {
			...
			if (!prs_copy_data_in(rpc_out, pad, ss_padding_len)) {
				DEBUG0(("create_bind_or_alt_ctx_internal: failed to marshall padding.\n"));
				return NT_STATUS_NO_MEMORY;
			}
		}

		if(!smb_io_rpc_hdr_auth("hdr_auth", phdr_auth, rpc_out, 0)) {
			DEBUG0(("create_bind_or_alt_ctx_internal: failed to marshall RPC_HDR_AUTH.\n"));
			return NT_STATUS_NO_MEMORY;
		}


		if(!prs_append_prs_data( rpc_out, pauth_info)) {
			DEBUG0(("create_bind_or_alt_ctx_internal: failed to grow parse struct to add auth.\n"));
			return NT_STATUS_NO_MEMORY;
		}
	}
					...
					prs_struct *outgoing_pdu)
{
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, outgoing_pdu, 0)) {
		DEBUG0(("add_ntlmssp_auth_footer: failed to marshall RPC_HDR_AUTH.\n"));
	...
	if (!prs_copy_data_in(outgoing_pdu, (const char *)auth_blob.data, NTLMSSP_SIG_SIZE)) {
		DEBUG0(("add_ntlmssp_auth_footer: failed to add %u bytes auth blob.\n",
					...
					prs_struct *outgoing_pdu)
{
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, outgoing_pdu, 0)) {
		DEBUG0(("add_schannel_auth_footer: failed to marshall RPC_HDR_AUTH.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	switch (cli->auth.auth_level) {
		case PIPE_AUTH_LEVEL_PRIVACY:
		case PIPE_AUTH_LEVEL_INTEGRITY:
			DEBUG10(("add_schannel_auth_footer: SCHANNEL seq_num=%d\n",
				sas->seq_num));
				   ...
				   const char *pipe_name, uint16 device_state)
{
	...
	DEBUG5(("Set Handle state Pipe[%x]: %s - device state:%x\n",
	...
	{
		DEBUG5( ("Set Handle state: return OK\n"));
		state_set = True;
	}
...
static BOOL valid_pipe_name(const int pipe_idx, RPC_IFACE *abstract, RPC_IFACE *transfer)
{
	if ( pipe_idx >= PI_MAX_PIPES ) {
		DEBUG0(("valid_pipe_name: Programmer error!  Invalid pipe index [%d]\n",
			pipe_idx));
		return False;
	}

	DEBUG5(("Bind Abstract Syntax: "));	
	dump_data(5, (char*)&pipe_names[pipe_idx].abstr_syntax, 
	          sizeof(pipe_names[pipe_idx].abstr_syntax));
	DEBUG5(("Bind Transfer Syntax: "));
...
static BOOL check_bind_response(RPC_HDR_BA *hdr_ba, const int pipe_idx, RPC_IFACE *transfer)
{
	if ( hdr_ba->addr.len == 0) {
		DEBUG4(("Ignoring length check -- ASU bug (server didn't fill in the pipe name correctly)"));
	}
...
# if 0	/* JERRY -- apparently ASU forgets to fill in the server pipe name sometimes */
	...
	{
		DEBUG4(("bind_rpc_pipe: pipe_name %s != expected pipe %s.  oh well!\n",
		         pipe_names[i].server_pipe ,hdr_ba->addr.str));
		return False;
	}
	
	DEBUG5(("bind_rpc_pipe: server pipe_name found: %s\n", pipe_names[i].server_pipe ));

	if (pipe_names[pipe_idx].server_pipe == NULL) {
		DEBUG2(("bind_rpc_pipe: pipe name %s unsupported\n", hdr_ba->addr.str));
		return False;
	}
#endif 	/* JERRY */
	...
	if ((hdr_ba->transfer.version != transfer->version) ||
		...
		DEBUG2(("bind_rpc_pipe: transfer syntax differs\n"));
		return False;
	}

	if (hdr_ba->res.num_results != 0x1 || hdr_ba->res.result != 0) {
		DEBUG2(("bind_rpc_pipe: bind denied results: %d reason: %x\n",
		          hdr_ba->res.num_results, hdr_ba->res.reason));
	}

	DEBUG5(("check_bind_response: accepted!\n"));
	return True;
}
				...
				prs_struct *rpc_out)
{
	...
	/* Marshall it. */
	if(!smb_io_rpc_hdr("hdr", &hdr, rpc_out, 0)) {
		DEBUG0(("create_rpc_bind_auth3: failed to marshall RPC_HDR.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	*/
	...
	if (!prs_uint32("pad", rpc_out, 0, &pad)) {
		DEBUG0(("create_rpc_bind_auth3: failed to marshall 4 byte pad.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &hdr_auth, rpc_out, 0)) {
		DEBUG0(("create_rpc_bind_auth3: failed to marshall RPC_HDR_AUTHA.\n"));
		return NT_STATUS_NO_MEMORY;
	}
	...
	if(!prs_copy_data_in(rpc_out, (char *)pauth_blob->data, pauth_blob->length)) {
		DEBUG0(("create_rpc_bind_auth3: failed to marshall auth blob.\n"));
		return NT_STATUS_NO_MEMORY;
	}
				...
				enum pipe_auth_level auth_level)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("rpc_finish_auth3_bind: NTLMSSP update using server blob failed.\n"));
	...
	if (ret != (ssize_t)prs_offset(&rpc_out)) {
		DEBUG0(("rpc_send_auth_auth3: cli_write failed. Return was %d\n", (int)ret));
		prs_mem_free(&rpc_out);
		data_blob_free(&client_reply);
		data_blob_free(&server_response);
		return cli_get_nt_error(cli->cli);
	...
	DEBUG5(("rpc_send_auth_auth3: Remote machine %s pipe %s "
                                ...
                                enum pipe_auth_level auth_level)
{
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG0(("rpc_finish_spnego_ntlmssp_bind: NTLMSSP update using server blob failed.\n"));
		data_blob_free(&client_reply);
		return nt_status;
	}
	...
	if(!smb_io_rpc_hdr("rpc_hdr   ", phdr, rbuf, 0)) {
		DEBUG0(("rpc_finish_spnego_ntlmssp_bind: Failed to unmarshall RPC_HDR.\n"));
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
	...
	DEBUG5(("rpc_finish_spnego_ntlmssp_bind: alter context request to "
			...
			enum pipe_auth_level auth_level)
{
	...
	DEBUG5(("Bind RPC Pipe[%x]: %s auth_type %u, auth_level %u\n",
		(unsigned int)cli->fnum,
		cli->pipe_name,
		(unsigned int)auth_type,
	...
	DEBUG3(("rpc_pipe_bind: Remote machine %s pipe %s "
	...
	if(!smb_io_rpc_hdr("hdr"   , &hdr, &rbuf, 0)) {
		DEBUG0(("rpc_pipe_bind: failed to unmarshall RPC_HDR.\n"));
		prs_mem_free(&rbuf);
		return NT_STATUS_BUFFER_TOO_SMALL;
	}

	if(!smb_io_rpc_hdr_ba("", &hdr_ba, &rbuf, 0)) {
		DEBUG0(("rpc_pipe_bind: Failed to unmarshall RPC_HDR_BA.\n"));
		prs_mem_free(&rbuf);
		return NT_STATUS_BUFFER_TOO_SMALL;
	}

	if(!check_bind_response(&hdr_ba, cli->pipe_idx, &transfer)) {
		DEBUG2(("rpc_pipe_bind: check_bind_response failed.\n"));
		prs_mem_free(&rbuf);
		return NT_STATUS_BUFFER_TOO_SMALL;
	}
	...
	switch(auth_type) {
		...
		default:
			DEBUG0(("cli_finish_bind_auth: unknown auth type %u\n",
				(unsigned int)auth_type ));
			prs_mem_free(&rbuf);
			return NT_STATUS_INVALID_INFO_CLASS;
	}
	...
	if (auth_type == PIPE_AUTH_TYPE_NTLMSSP || auth_type == PIPE_AUTH_TYPE_SPNEGO_NTLMSSP) {
		if (auth_level == PIPE_AUTH_LEVEL_INTEGRITY) {
			if (!(cli->auth.a_u.ntlmssp_state->neg_flags & NTLMSSP_NEGOTIATE_SIGN)) {
				DEBUG0(("cli_finish_bind_auth: requested NTLMSSSP signing and server refused.\n"));
				prs_mem_free(&rbuf);
				return NT_STATUS_INVALID_PARAMETER;
			}
		}
		if (auth_level == PIPE_AUTH_LEVEL_INTEGRITY) {
			if (!(cli->auth.a_u.ntlmssp_state->neg_flags & NTLMSSP_NEGOTIATE_SEAL)) {
				DEBUG0(("cli_finish_bind_auth: requested NTLMSSSP sealing and server refused.\n"));
				prs_mem_free(&rbuf);
				return NT_STATUS_INVALID_PARAMETER;
			}
		}
...
static struct rpc_pipe_client *cli_rpc_pipe_open(struct cli_state *cli, int pipe_idx, NTSTATUS *perr)
{
	...
	*perr = NT_STATUS_NO_MEMORY;
	...
	if (fnum == -1) {
		DEBUG1(("cli_rpc_pipe_open: cli_nt_create failed on pipe %s "
...
struct rpc_pipe_client *cli_rpc_pipe_open_noauth(struct cli_state *cli, int pipe_idx, NTSTATUS *perr)
{
	...
	*perr = rpc_pipe_bind(result, PIPE_AUTH_TYPE_NONE, PIPE_AUTH_LEVEL_NONE);
	...
	DEBUG10(("cli_rpc_pipe_open_noauth: opened pipe %s to machine %s and bound anonymously.\n",
			result->pipe_name, cli->desthost ));
						...
						NTSTATUS *perr)
{
	...
	*perr = rpc_pipe_bind(result, auth_type, auth_level);
	if (!NT_STATUS_IS_OK(*perr)) {
		DEBUG0( ("cli_rpc_pipe_open_ntlmssp_internal: cli_rpc_pipe_bind failed with error %s\n",
			nt_errstr(*perr) ));
		goto err;
	}

	DEBUG10(("cli_rpc_pipe_open_ntlmssp_internal: opened pipe %s to "
					    ...
					    NTSTATUS *perr)
{
	...
	if (!get_trust_pw_hash(domain, machine_pwd, &machine_account,
			       &sec_chan_type))
	{
		DEBUG0( ("get_schannel_session_key: could not fetch "
	...
	*perr = rpccli_netlogon_setup_creds(netlogon_pipe,
	...
	if (!NT_STATUS_IS_OK(*perr)) {
		DEBUG3(("get_schannel_session_key_common: rpccli_netlogon_setup_creds "
	...
	if (((*pneg_flags) & NETLOGON_NEG_SCHANNEL) == 0) {
		DEBUG3( ("get_schannel_session_key: Server %s did not offer schannel\n",
			cli->desthost));
					...
					NTSTATUS *perr)
{
	...
	*perr = rpc_pipe_bind(result, PIPE_AUTH_TYPE_SCHANNEL, auth_level);
	if (!NT_STATUS_IS_OK(*perr)) {
		DEBUG0( ("cli_rpc_pipe_open_schannel_with_key: cli_rpc_pipe_bind failed with error %s\n",
			nt_errstr(*perr) ));
		cli_rpc_pipe_close(result);
		return NULL;
	}
	...
	DEBUG10(("cli_rpc_pipe_open_schannel_with_key: opened pipe %s to machine %s "
		"for domain %s "
						...
						NTSTATUS *perr)
{
	...
	if (!netlogon_pipe) {
		DEBUG0(("cli_rpc_pipe_open_ntlmssp_auth_schannel: failed to get schannel session "
						...
						NTSTATUS *perr)
{
	...
	if (!netlogon_pipe) {
		DEBUG0(("cli_rpc_pipe_open_schannel: failed to get schannel session "
			"key from server %s for domain %s.\n",
			cli->desthost, domain ));
		return NULL;
	}
						...
						NTSTATUS *perr)
{
	...
	*perr = rpc_pipe_bind(result, PIPE_AUTH_TYPE_KRB5, auth_level);
	if (!NT_STATUS_IS_OK(*perr)) {
		DEBUG0( ("cli_rpc_pipe_open_krb5: cli_rpc_pipe_bind failed with error %s\n",
			nt_errstr(*perr) ));
		cli_rpc_pipe_close(result);
		return NULL;
	}
	...
	return result;
#else
	DEBUG0(("cli_rpc_pipe_open_krb5: kerberos not found at compile time.\n"));
	return NULL;
#endif
}
 ...
 void cli_rpc_pipe_close(struct rpc_pipe_client *cli)
{
	if (!cli_close(cli->cli, cli->fnum)) {
		DEBUG0(("cli_rpc_pipe_close: cli_close failed on pipe %s "
			 "to machine %s.  Error was %s\n",
			 cli->pipe_name),
			 cli->cli->desthost,
			 cli_errstr(cli->cli)));
	...
	DEBUG10(("cli_rpc_pipe_close: closed pipe %s to machine %s\n",
