				...
				struct winbindd_pw *pw)
{
	...
	if (!NT_STATUS_IS_OK(idmap_sid_to_uid(user_sid, &pw->pw_uid))) {
		DEBUG1( ("error getting user id for sid %s\n", sid_to_string(sid_string, user_sid)));
		return False;
	}
	...
	if (!NT_STATUS_IS_OK(idmap_sid_to_gid(group_sid, &pw->pw_gid))) {
		DEBUG1( ("error getting group id for sid %s\n", sid_to_string(sid_string, group_sid)));
		return False;
	}
					    ...
					    struct winbindd_cli_state *state)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG1( ("error getting user info for sid %s\n",
	...
	if (!sid_peek_check_rid(&domain->sid, &user_info.group_sid,
		...
		DEBUG1( ("Could not extract group rid out of %s\n",
			  sid_string_static(&sid)));
		return WINBINDD_ERROR;
	}
				    ...
				    uint32 group_rid)
{
	...
	if (!success) {
		DEBUG5( ("Could not query domain %s SID %s\n", s->domain->name,
...
static void getpwsid_sid2uid_recv(void *private_data, BOOL success, uid_t uid)
{
	...
	if (!success) {
		DEBUG5( ("Could not query uid for user %s\\%s\n",
...
static void getpwsid_sid2gid_recv(void *private_data, BOOL success, gid_t gid)
{
	...
	if ( s->gid == (gid_t)-1 ) {
		...
		if (!success) {
			DEBUG5( ("Could not query gid for user %s\\%s\n",
				  s->domain->name, s->username));
			goto failed;
		}
...
void winbindd_getpwnam(struct winbindd_cli_state *state)
{
	...
	if ( strequal(domname, lp_workgroup()) && lp_winbind_trusted_domains_only() ) {
		DEBUG7(("winbindd_getpwnam: My domain -- rejecting getpwnam() for %s\\%s.\n", 
			domname, username));
		request_error(state);
		return;
	}	
				   ...
				   const DOM_SID *sid, enum lsa_SidType type)
{
	...
	if (!success) {
		DEBUG5( ("Could not lookup name for user %s\n",
			  state->request.data.username));
		request_error(state);
		return;
	}
...
static void getpwuid_recv(void *private_data, BOOL success, const char *sid)
{
	...
	if (!success) {
		DEBUG10(("uid2sid_recv: uid [%lu] to sid mapping failed\n.",
			  (unsigned long)(state->request.data.uid)));
		request_error(state);
		return;
	}
	
	DEBUG10(("uid2sid_recv: uid %lu has sid %s\n",
...
static BOOL get_sam_user_entries(struct getent_state *ent, TALLOC_CTX *mem_ctx)
{
	...
	if (!(domain = find_domain_from_name(ent->domain_name))) {
		DEBUG3( ("no such domain %s in get_sam_user_entries\n",
			  ent->domain_name));
		return False;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG10(("get_sam_user_entries: query_user_list failed with %s\n",
			nt_errstr(status) ));
		return False;
	}
	...
	if (num_entries) {
		...
		if (!name_list) {
			DEBUG0(("get_sam_user_entries realloc failed.\n"));
			return False;
		}
	}
...
void winbindd_getpwent(struct winbindd_cli_state *state)
{
	...
	for (user_list_ndx = 0; user_list_ndx < num_users; ) {
		...
		if (result) {
			...
			DEBUG1( ("could not lookup domain user %s\n",
				  name_list[ent->sam_entry_index].name));
