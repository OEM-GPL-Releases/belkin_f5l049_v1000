 ...
 NTSTATUS smb_register_idmap_nss(int version, const char *name, struct nss_info_methods *methods)
{
	struct nss_function_entry *entry;
 	...
 	if ((version != SMB_NSS_INFO_INTERFACE_VERSION)) {
		DEBUG0( ("smb_register_idmap_nss: Failed to register idmap_nss module.\n"
	...
	if (!name || !name[0] || !methods) {
		DEBUG0(("smb_register_idmap_nss: called with NULL pointer or empty name!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if ( nss_get_backend(name) ) {
		DEBUG0(("smb_register_idmap_nss: idmap module %s "
			 "already registered!\n", name));
	...
	DEBUG5( ("smb_register_idmap_nss: Successfully added idmap "
		  "nss backend '%s'\n", name));
 ...
 NTSTATUS nss_init( const char **nss_list )
{
	...
	for ( i=0; nss_list && nss_list[i]; i++ ) {
		...
		if ( !parse_nss_parm(nss_list[i], &backend, &domain) ) 	{
			DEBUG0(("nss_init: failed to parse \"%s\"!\n",
				 nss_list[i]));
			continue;			
		}
		...
		if ( (nss_backend = nss_get_backend( backend )) == NULL ) {
			...
			/* try again */
			if ( (nss_backend = nss_get_backend( backend )) == NULL ) {
				DEBUG0(("nss_init: unregistered backend %s!.  Skipping\n",
					 backend));
				continue;
			}
		...
		if ( !nss_domain ) {
			DEBUG0(("nss_init: talloc() failure!\n"));
		...
		if ( NT_STATUS_IS_OK( status ) ) {
			...
			DEBUG0(("nss_init: Failed to init backend for %s domain!\n", 
				 nss_domain->domain));
		}
	...
	if ( !nss_domain_list ) {
		DEBUG3(("nss_init: no nss backends configured.  "
...
static struct nss_domain_entry *find_nss_domain( const char *domain )
{
	...
	if ( !NT_STATUS_IS_OK(status) ) {
		DEBUG4(("nss_get_info: Failed to init nss_info API (%s)!\n",
			 nt_errstr(status)));
		return NULL;
	}
		       ...
		       gid_t *p_gid)
{
	...
	if ( (p = find_nss_domain( domain )) == NULL ) {
		DEBUG4(("nss_get_info: Failed to find nss domain pointer for %s\n",
			 domain ));
