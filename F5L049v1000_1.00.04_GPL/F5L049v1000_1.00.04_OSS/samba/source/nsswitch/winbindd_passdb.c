			    ...
			    enum lsa_SidType *type)
{
	...
	DEBUG10( ("Converting SID %s\n", sid_string_static(sid)));

	/* Paranoia check */
	if (!sid_check_is_in_builtin(sid) &&
	    !sid_check_is_in_our_domain(sid)) {
		DEBUG0( ("Possible deadlock: Trying to lookup SID %s with "
			  "passdb backend\n", sid_string_static(sid)));
		return NT_STATUS_NONE_MAPPED;
	}
			   ...
			   WINBIND_USERINFO *user_info)
{
	...
	DEBUG10(("sam_query_user: getting samu info for sid %s\n",
		sidstr ));
	...
	DEBUG10(("sam_query_user: group sid %s\n", sidstr ));
	...
	*name_types = TALLOC_ARRAY(mem_ctx, uint32, num_members);
	...
	for (i=0; i<num_members; i++) {
		if (lsa_names[i].type != SID_NAME_USER) {
			DEBUG2( ("Got %s as group member -- ignoring\n",
