...
static ADS_STRUCT *ad_idmap_cached_connection_internal(void)
{
	...
	if (!ADS_ERR_OK(status)) {
		DEBUG1( ("ad_idmap_init: failed to connect to AD\n"));
		ads_destroy(&ads);
		return NULL;
	}
...
static ADS_STRUCT *ad_idmap_cached_connection(void)
{
	     ...
	     (ad_map_type ==  WB_POSIX_MAP_RFC2307) ) 
	{
		...
		if ( !ADS_ERR_OK(schema_status) ) {
			DEBUG2(("ad_idmap_cached_connection: Failed to obtain schema details!\n"));
			return NULL;			
		}
	}
...
static NTSTATUS idmap_ad_initialize(struct idmap_domain *dom)
{
	...
	if ( schema_mode && schema_mode[0] ) {
		if ( strequal(schema_mode, "sfu") )
			...
			DEBUG0(("idmap_ad_initialize: Unknown schema_mode (%s)\n",
				 schema_mode));
	}
...
again:
	...
	for (i = 0; (i < IDMAP_AD_MAX_IDS) && ids[idx]; i++, idx++) {
		...
		default:
			DEBUG3( ("Error: mapping requested but Unknown ID type\n"));
	...
	if (!ADS_ERR_OK(rc)) {
		DEBUG1( ("ERROR: ads search returned: %s\n", ads_errstr(rc)));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}

	if ( (count = ads_count_replies(ads, res)) == 0 ) {
		DEBUG10( ("No IDs found\n"));
	}
	...
	for (i = 0; (i < count) && entry; i++) {
		...
		if ( !entry ) {
			DEBUG2( ("ERROR: Unable to fetch ldap entries from results\n"));
			break;
		}

		/* first check if the SID is present */
		if (!ads_pull_sid(ads, entry, "objectSid", &sid)) {
			DEBUG2( ("Could not retrieve SID from entry\n"));
			continue;
		}

		/* get type */
		if (!ads_pull_uint32(ads, entry, "sAMAccountType", &atype)) {
			DEBUG1( ("could not get SAM account type\n"));
			continue;
		}
		...
		case ATYPE_INTERDOMAIN_TRUST:
			...
			break;
		default:
			DEBUG1( ("unrecognized SAM account type %08x\n", atype));
			continue;
		}
		...
		if (!map) {
			DEBUG2( ("WARNING: couldn't match result with requested ID\n"));
			continue;
		}
...
again:
	...
	for (i = 0; (i < count) && entry; i++) {
		...
		if (!map) {
			DEBUG2( ("WARNING: couldn't match result with requested SID\n"));
			continue;
		}
...
static NTSTATUS nss_sfu_init( struct nss_domain_entry *e )
{
	...
	if ( (ad_map_type != WB_POSIX_MAP_UNKNOWN) &&
	     (ad_map_type != WB_POSIX_MAP_SFU) ) 
	{
		DEBUG0(("nss_sfu_init: Posix Map type has already been set.  "
...
static NTSTATUS nss_rfc2307_init( struct nss_domain_entry *e )
{
	...
	if ( (ad_map_type != WB_POSIX_MAP_UNKNOWN) &&
	     (ad_map_type != WB_POSIX_MAP_RFC2307) ) 
	{
		DEBUG0(("nss_rfc2307_init: Posix Map type has already been set.  "
			 "Mixed schema models not supported!\n"));
		return NT_STATUS_NOT_SUPPORTED;
	}
