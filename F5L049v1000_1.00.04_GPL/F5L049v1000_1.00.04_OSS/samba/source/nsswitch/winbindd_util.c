...
static void trustdom_recv(void *private_data, BOOL success)
{
	...
	if ((!success) || (response->result != WINBINDD_OK)) {
		DEBUG1( ("Could not receive trustdoms\n"));
	...
	while ((p != NULL) && (*p != '\0')) {
		...
		alt_name = strchr(p, '\\');
		if (alt_name == NULL) {
			DEBUG0( ("Got invalid trustdom response\n"));
			break;
		}
...
static void init_child_getdc_recv(void *private_data, BOOL success)
{
	...
	DEBUG10( ("Received getdcname response\n"));
...
static void init_child_recv(void *private_data, BOOL success)
{
	...
	DEBUG5( ("Received child initialization response for domain %s\n",
		  state->domain->name));

	if ((!success) || (state->response->result != WINBINDD_OK)) {
		DEBUG3( ("Could not init child\n"));
						   ...
						   struct winbindd_cli_state *state)
{
	...
	if (!domain->initialized) {
		...
		*/
		DEBUG5( ("winbindd_dual_init_connection: %s returning without initialization "
...
BOOL init_domain_list(void)
{
	...
	if ( role == ROLE_DOMAIN_MEMBER ) {
		...
		if (!secrets_fetch_domain_sid(lp_workgroup(), &our_sid)) {
			DEBUG0( ("Could not fetch our SID - did we join?\n"));
			return False;
		}
...
struct winbindd_domain *find_lookup_domain_from_sid(const DOM_SID *sid)
{
	...
	DEBUG10( ("find_lookup_domain_from_sid(%s)\n",
		   sid_string_static(sid)));

	if (IS_DC || is_internal_domain(sid) || is_in_internal_domain(sid)) {
		DEBUG10( ("calling find_domain_from_sid\n"));
	...
	DEBUG10( ("calling find_our_domain\n"));
...
int open_winbindd_socket(void)
{
	if (_winbindd_socket == -1) {
		...
		DEBUG10( ("open_winbindd_socket: opened socket fd %d\n",
			   _winbindd_socket));
	}
...
int open_winbindd_priv_socket(void)
{
	if (_winbindd_priv_socket == -1) {
		...
		DEBUG10( ("open_winbindd_priv_socket: opened socket fd %d\n",
...
{
	if (_winbindd_socket != -1) {
		DEBUG10( ("close_winbindd_socket: closing socket fd %d\n",
			   _winbindd_socket));
...
void winbindd_kill_all_clients(void)
{
	...
	DEBUG10( ("winbindd_kill_all_clients: going postal\n"));
				  ...
				  uint32 *p_num_groups, DOM_SID **user_sids)
{
	...
	DEBUG3((": lookup_usergroups_cached\n"));
	...
	*p_num_groups = num_groups;
	...
	DEBUG3((": lookup_usergroups_cached succeeded\n"));
