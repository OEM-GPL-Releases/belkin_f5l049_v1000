 ...
 */
{
	...
	if (num_require_membership_of_sid > 1) {
		...
		while ( (comma = strstr(search_location, ",")) != NULL ) {
			...
			if (!string_to_sid(&require_membership_of_sid[i++], single_group_sid)) {
				DEBUG0( ("check_info3_in_group: could not parse %s as a SID!", 
					  single_group_sid));
	...
	if (!sid_append_rid(&all_sids[0], info3->user_rid)) {
		DEBUG3(("could not append user's primary RID 0x%x\n",
			 info3->user_rid));			
	...
	if (!sid_append_rid(&all_sids[1], info3->group_rid)) {
		DEBUG3(("could not append additional group rid 0x%x\n",
			 info3->group_rid));			
	...
	for (i = 0; i < j; i++) {
		...
		for (k = 0; k < num_require_membership_of_sid; k++) {
			if (sid_equal(&require_membership_of_sid[k], &all_sids[i])) {
				DEBUG10( ("SID %s matches %s - user permitted to authenticate!\n", 
					...
					const char *domain_name)
{
	...
	if (IS_DC) {
		...
		if (domain == NULL) {
			DEBUG3( ("Authentication for domain [%s] refused"
	...
	if (is_myname(domain_name)) {
		DEBUG3( ("Authentication for domain %s (local domain "
	...
	if (state->request.flags & WBFLAG_PAM_CONTACT_TRUSTDOM) {
		domain = find_domain_from_name_noinit(domain_name);
		if (domain == NULL) {
			DEBUG3( ("Authentication for domain [%s] skipped " 
				  "as it is not a trusted domain\n", 
				  domain_name));
		} else {
			return domain;
					...
					BOOL *internal_ccache)
{
	...
	*internal_ccache = True;
	...
	if (strequal(type, "FILE")) {
		...
		DEBUG10(("we don't allow to set a %s type ccache\n", type));
		goto memory_ccache;
	}
...
static void setup_return_cc_name(struct winbindd_cli_state *state, const char *cc)
{
	...
	if (!strequal(type, "FILE") &&
	    !strequal(type, "WRFILE")) {
	    	DEBUG10(("won't return krbccname for a %s type ccache\n", 
			type));
		return;
	}
					    ...
					    NET_USER_INFO_3 **info3)
{
	...
	*info3 = NULL;
	...
	if (!internal_ccache) {
		...
		DEBUG10(("winbindd_raw_kerberos_login: uid is %d\n", uid));
	}
	...
	if (krb5_ret) {
		DEBUG1(("winbindd_raw_kerberos_login: kinit failed for '%s' with: %s (%d)\n", 
	...
	DEBUG10(("got TGT for %s in %s (valid until: %s (%d), renewable till: %s (%d))\n", 
		principal_s, cc, 
	...
	if (local_service == NULL) {
		DEBUG0(("winbindd_raw_kerberos_login: out of memory\n"));
		result = NT_STATUS_NO_MEMORY;
		goto failed;
	}
	...
	krb5_ret = cli_krb5_get_ticket(local_service, 
				       ...
				       NULL);
	if (krb5_ret) {
		DEBUG1(("winbindd_raw_kerberos_login: failed to get ticket for %s: %s\n", 
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG0(("winbindd_raw_kerberos_login: ads_verify_ticket failed: %s\n", 
			nt_errstr(result)));
		goto failed;
	}

	if (!pac_data) {
		DEBUG3(("winbindd_raw_kerberos_login: no pac data\n"));
	...
	if (logon_info == NULL) {
		DEBUG1(("winbindd_raw_kerberos_login: no logon info\n"));
		result = NT_STATUS_INVALID_PARAMETER;
		goto failed;
	}

	DEBUG10(("winbindd_raw_kerberos_login: winbindd validated ticket of %s\n", 
		local_service));
	...
	*info3 = &logon_info->info3;
	...
	if (!internal_ccache) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG10(("winbindd_raw_kerberos_login: failed to add ccache to list: %s\n", 
				nt_errstr(result)));
		}
	} else {
		...
		if (krb5_ret) {
			DEBUG3(("winbindd_raw_kerberos_login: "
...
void winbindd_pam_auth(struct winbindd_cli_state *state)
{
	...
	if (!canonicalize_username(state->request.data.auth.user,
		...
		DEBUG5( ("Plain text authentication for %s returned %s "
			  "(PAM: %d)\n",
			  state->request.data.auth.user, 
				       ...
				       NET_USER_INFO_3 **info3)
{
	...
	*info3 = NULL;
	...
	DEBUG10(("winbindd_dual_pam_auth_cached\n"));
	...
	if (!lookup_cached_name(state->mem_ctx,
				...
				&sid,
				&type)) {
		DEBUG10(("winbindd_dual_pam_auth_cached: no such user in the cache\n"));
		return NT_STATUS_NO_SUCH_USER;
	}

	if (type != SID_NAME_USER) {
		DEBUG10(("winbindd_dual_pam_auth_cached: not a user (%s)\n", sid_type_lookup(type)));
		return NT_STATUS_LOGON_FAILURE;
	}
	...
	result = winbindd_get_creds(domain, 
				    ...
				    &cached_salt);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG10(("winbindd_dual_pam_auth_cached: failed to get creds: %s\n", nt_errstr(result)));
		return result;
	}
	...
	*info3 = my_info3;
	...
	if (password_good) {
		...
		if (!(my_info3->acct_flags & ACB_NORMAL)) {
			DEBUG0(("winbindd_dual_pam_auth_cached: whats wrong with that one?: 0x%08x\n", 
				my_info3->acct_flags));
			return NT_STATUS_LOGON_FAILURE;
		}
		...
		if (!strequal(domain->name, domain->alt_name) &&
			...
			uid = get_uid_from_state(state);
			if (uid == -1) {
				DEBUG0(("winbindd_dual_pam_auth_cached: invalid uid\n"));
				return NT_STATUS_INVALID_PARAMETER;
			}
			...
			if (!internal_ccache) {
				...
				if (!NT_STATUS_IS_OK(result)) {
					DEBUG10(("winbindd_dual_pam_auth_cached: failed "
 ...
 success:
		...
		result = winbindd_update_creds_by_info3(domain,
							...
							my_info3);
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG1(("winbindd_dual_pam_auth_cached: failed to update creds: %s\n",
				nt_errstr(result)));
			return result;
		}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG10(("winbindd_dual_pam_auth_cached: failed to get max_allowed_bad_attempts. "
	...
	if (my_info3->bad_pw_count >= max_allowed_bad_attempts) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG10(("winbindd_dual_pam_auth_cached: failed to get password properties.\n"));
		}
...
failed:
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG0(("winbindd_dual_pam_auth_cached: failed to update creds %s\n", 
			nt_errstr(result)));
	}
					 ...
					 NET_USER_INFO_3 **info3)
{
	...
	DEBUG10(("winbindd_dual_pam_auth_kerberos\n"));
	...
	if ( IS_DC ) {
		if (!(contact_domain = find_domain_from_name(name_domain))) {
			DEBUG3( ("Authentication for domain for [%s] -> [%s]\\[%s] failed as %s is not a trusted domain\n", 
		...
		if (is_myname(name_domain)) {
			DEBUG3( ("Authentication for domain %s (local domain to this server) not supported at this stage\n", name_domain));
			result =  NT_STATUS_NO_SUCH_USER;
			goto done;
		}
	...
	if (!contact_domain->active_directory) {
		DEBUG3(("krb5 auth requested but domain is not Active Directory\n"));
					 ...
					 NET_USER_INFO_3 **info3)
{
	...
	*info3 = NULL;
	...
	DEBUG10(("winbindd_dual_pam_auth_samlogon\n"));
	...
	if (lp_client_ntlmv2_auth()) {
		...
		*/
		...
		if (!SMBNTLMv2encrypt(name_user, name_domain, 
			...
			DEBUG0( ("winbindd_pam_auth: SMBNTLMv2encrypt() failed!\n"));
			result = NT_STATUS_NO_MEMORY;
			goto done;
		}
		data_blob_free(&names_blob);
	...
	do {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG3( ("could not open handle to NETLOGON pipe\n"));
			goto done;
		}
		...
		if ( NT_STATUS_EQUAL(result, NT_STATUS_ACCESS_DENIED) ) {
			DEBUG3(("winbindd_pam_auth: sam_logon returned "
	...
	if ((state->request.flags & WBFLAG_PAM_INFO3_TEXT) && 
		...
		if (!NT_STATUS_IS_OK(status_tmp)) {
			DEBUG3( ("could not open handle to SAMR pipe: %s\n", 
		...
		if (!NT_STATUS_IS_OK(status_tmp)) {
			DEBUG3( ("could not open user handle on SAMR pipe: %s\n",
				nt_errstr(status_tmp)));
			goto done;
		}
		...
		if (!NT_STATUS_IS_OK(status_tmp)) {
			DEBUG3( ("could not query user info on SAMR pipe: %s\n",
				nt_errstr(status_tmp)));
		...
		DEBUG10(("successfully retrieved acct_flags 0x%x\n", acct_flags));
					    ...
					    struct winbindd_cli_state *state) 
{
	...
	if (domain->online == False) {
		...
		if (domain->startup) {
			...
			DEBUG10(("winbindd_dual_pam_auth: domain: %s offline and auth "
	...
	DEBUG10(("winbindd_dual_pam_auth: domain: %s last was %s\n", domain->name, domain->online ? "online":"offline"));
	...
	if (domain->online && (state->request.flags & WBFLAG_PAM_KRB5)) {
		...
		if (NT_STATUS_IS_OK(result)) {
			DEBUG10(("winbindd_dual_pam_auth_kerberos succeeded\n"));
			goto process_result;
		} else {
			DEBUG10(("winbindd_dual_pam_auth_kerberos failed: %s\n", nt_errstr(result)));
		}

		if (NT_STATUS_EQUAL(result, NT_STATUS_NO_LOGON_SERVERS) ||
		    NT_STATUS_EQUAL(result, NT_STATUS_IO_TIMEOUT) ||
		    NT_STATUS_EQUAL(result, NT_STATUS_DOMAIN_CONTROLLER_NOT_FOUND)) {
			DEBUG10(("winbindd_dual_pam_auth_kerberos setting domain to offline\n"));
		...
		if (state->request.flags & WBFLAG_PAM_FALLBACK_AFTER_KRB5) {
			DEBUG3(("falling back to samlogon\n"));
			goto sam_logon;
		} else {
			goto cached_logon;
		}
...
sam_logon:
	...
	if (domain->online) {
		...
		if (NT_STATUS_IS_OK(result)) {
			DEBUG10(("winbindd_dual_pam_auth_samlogon succeeded\n"));
			goto process_result;
		} else {
			DEBUG10(("winbindd_dual_pam_auth_samlogon failed: %s\n", nt_errstr(result)));
			if (domain->online) {
...
cached_logon:
	...
	if (!domain->online && (state->request.flags & WBFLAG_PAM_CACHED_LOGIN) && 
		...
		if (NT_STATUS_IS_OK(result)) {
			DEBUG10(("winbindd_dual_pam_auth_cached succeeded\n"));
			goto process_result;
		} else {
			DEBUG10(("winbindd_dual_pam_auth_cached failed: %s\n", nt_errstr(result)));
			goto done;
		}
	}
...
process_result:
	...
	if (NT_STATUS_IS_OK(result)) {
		...
		if (!NT_STATUS_IS_OK(result = check_info3_in_group(state->mem_ctx, info3,
			...
			DEBUG3( ("User %s is not in the required group (%s), so plaintext authentication is rejected\n",
				  state->request.data.auth.user, 
				  state->request.data.auth.require_membership_of_sid));
			goto done;
		}
		...
		if (state->request.flags & WBFLAG_PAM_INFO3_NDR) {
			...
			if (!NT_STATUS_IS_OK(result)) {
				DEBUG10(("Failed to append INFO3 (NDR): %s\n", nt_errstr(result)));
				goto done;
			}
		}
		...
		if (state->request.flags & WBFLAG_PAM_INFO3_TEXT) {
			...
			if (!NT_STATUS_IS_OK(result)) {
				DEBUG10(("Failed to append INFO3 (TXT): %s\n", nt_errstr(result)));
				goto done;
			}
		...
		if ((state->request.flags & WBFLAG_PAM_CACHED_LOGIN)) {
			...
			if (!NT_STATUS_IS_OK(result)) {
				DEBUG10(("Failed to store memory creds: %s\n", nt_errstr(result)));
				goto done;
			}
			...
			if (lp_winbind_offline_logon()) {
				...
				if (!NT_STATUS_IS_OK(result)) {
					...
					DEBUG10(("Failed to store creds: %s\n", nt_errstr(result)));
					goto done;
				}
			}
		}
		...
		if (state->request.flags & WBFLAG_PAM_GET_PWD_POLICY) {
			...
			if (!NT_STATUS_IS_OK(result)) {
				DEBUG10(("Failed to get password policies: %s\n", nt_errstr(result)));
				goto done;
			}
		}
		...
		if (state->request.flags & WBFLAG_PAM_UNIX_NAME) {
			...
			DEBUG5( ("Setting unix username to [%s]\n", username_out));
	...
	if ( NT_STATUS_IS_OK(result) && info3 &&
		...
		DEBUG10( ("Generating token for user %s\n", afsname));
...
void winbindd_pam_auth_crap(struct winbindd_cli_state *state)
{
	...
	if (!state->privileged) {
		...
		DEBUG2( ("winbindd_pam_auth_crap: non-privileged access "
			  "denied.  !\n"));
	...
	DEBUG3( ("[%5lu]: pam auth crap domain: [%s] user: %s\n",
		  (unsigned long)state->pid,
		  state->request.data.auth_crap.domain,
		  state->request.data.auth_crap.user));
	...
	DEBUG5( ("CRAP authentication for %s\\%s returned %s (PAM: %d)\n",
		  state->request.data.auth_crap.domain,
		  state->request.data.auth_crap.user, 
		  state->response.data.auth.nt_status_string,
		  state->response.data.auth.pam_error));
						 ...
						 struct winbindd_cli_state *state) 
{
	...
	if (*state->request.data.auth_crap.domain) {
		...
		DEBUG5(("no domain specified with username (%s) - failing auth\n", 
			 name_user));
		result = NT_STATUS_NO_SUCH_USER;
		goto done;
	}
	...
	if (state->request.data.auth_crap.lm_resp_len > sizeof(state->request.data.auth_crap.lm_resp)
		...
		if (!state->request.flags & WBFLAG_BIG_NTLMV2_BLOB ||
			...
			DEBUG0( ("winbindd_pam_auth_crap: invalid password length %u/%u\n",
	...
	do {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG3( ("could not open handle to NETLOGON pipe (error: %s)\n",
				  nt_errstr(result)));
			goto done;
		}
...
void winbindd_pam_chauthtok(struct winbindd_cli_state *state)
{
	...
	if (!canonicalize_username(state->request.data.chauthtok.user, domain, user)) {
		set_auth_errors(&state->response, NT_STATUS_NO_SUCH_USER);
		DEBUG5( ("winbindd_pam_chauthtok: canonicalize_username %s failed with %s"
			  "(PAM: %d)\n",
			  state->request.data.auth.user, 
			  state->response.data.auth.nt_status_string,
			  state->response.data.auth.pam_error));
	...
	if (!contact_domain) {
		set_auth_errors(&state->response, NT_STATUS_NO_SUCH_USER);
		DEBUG3( ("Cannot change password for [%s] -> [%s]\\[%s] as %s is not a trusted domain\n", 
						 ...
						 struct winbindd_cli_state *state)
{
	...
	DEBUG3( ("[%5lu]: dual pam chauthtok %s\n", (unsigned long)state->pid,
		  state->request.data.auth.user));
	...
	result = cm_connect_sam(contact_domain, state->mem_ctx, &cli,
				&dom_pol);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG1( ("could not get SAM handle on DC for %s\n", domain));
		goto done;
	}
	...
	if ((NT_STATUS_EQUAL(result, NT_STATUS(DCERPC_FAULT_OP_RNG_ERROR))) ||
		...
		DEBUG10(("Password change with chgpasswd3 failed with: %s, retrying chgpasswd_user\n", 
			nt_errstr(result)));
	...
	if (NT_STATUS_IS_OK(result) && (state->request.flags & WBFLAG_PAM_CACHED_LOGIN)) {
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG10(("Failed to replace memory creds: %s\n", nt_errstr(result)));
			goto process_result;
		}
...
void winbindd_pam_logoff(struct winbindd_cli_state *state)
{
	...
	if ((sys_getpeereid(state->sock, &caller_uid)) != 0) {
		DEBUG1(("winbindd_pam_logoff: failed to check peerid: %s\n", 
			strerror(errno)));
		goto failed;
	}
	...
	switch (caller_uid) {
		...
		default:
			if (caller_uid != request_uid) {
				DEBUG1(("winbindd_pam_logoff: caller requested invalid uid\n"));
				goto failed;
			}
 ...
 failed:
	set_auth_errors(&state->response, NT_STATUS_NO_SUCH_USER);
	DEBUG5( ("Pam Logoff for %s returned %s "
		  "(PAM: %d)\n",
		  state->request.data.logoff.user,
		  state->response.data.auth.nt_status_string,
		  state->response.data.auth.pam_error));
					      ...
					      struct winbindd_cli_state *state) 
{
	...
	if (state->request.data.logoff.uid < 0) {
		DEBUG0(("winbindd_pam_logoff: invalid uid\n"));
		goto process_result;
	}
	...
	if (!ccache_entry_exists(state->request.data.logoff.user)) {
		result = NT_STATUS_OK;
		DEBUG10(("winbindd_pam_logoff: no entry found.\n"));
		goto process_result;
	}
	...
	if (!ccache_entry_identical(state->request.data.logoff.user, 
					...
					state->request.data.logoff.krb5ccname)) {
		DEBUG0(("winbindd_pam_logoff: cached entry differs.\n"));
		goto process_result;
	}

	result = remove_ccache(state->request.data.logoff.user);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG0(("winbindd_pam_logoff: failed to remove ccache: %s\n",
			nt_errstr(result)));
		goto process_result;
	}
...
void winbindd_pam_chng_pswd_auth_crap(struct winbindd_cli_state *state)
{
	...
	DEBUG3( ("[%5lu]: pam change pswd auth crap domain: %s user: %s\n",
		  (unsigned long)state->pid,
		  state->request.data.chng_pswd_auth_crap.domain,
		  state->request.data.chng_pswd_auth_crap.user));
	...
	if (domain != NULL) {
		DEBUG7( ("[%5lu]: pam auth crap changing pswd in domain: "
	...
	set_auth_errors(&state->response, NT_STATUS_NO_SUCH_USER);
	DEBUG5( ("CRAP change password  for %s\\%s returned %s (PAM: %d)\n",
		  state->request.data.chng_pswd_auth_crap.domain,
		  state->request.data.chng_pswd_auth_crap.user, 
		  state->response.data.auth.nt_status_string,
		  state->response.data.auth.pam_error));
...
enum winbindd_result winbindd_dual_pam_chng_pswd_auth_crap(struct winbindd_domain *domainSt, struct winbindd_cli_state *state)
{
	...
	*user = 0;
	...
	if (lp_winbind_offline_logon()) {
		DEBUG0(("Refusing password change as winbind offline logons are enabled. "));
	...
	if (*state->request.data.chng_pswd_auth_crap.domain) {
		...
		if(!*domain) {
			DEBUG3(("no domain specified with username (%s) - "
				 "failing auth\n",
				 state->request.data.chng_pswd_auth_crap.user));
			result = NT_STATUS_NO_SUCH_USER;
			goto done;
