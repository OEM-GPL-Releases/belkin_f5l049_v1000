				   ...
				   void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger idmap_set_mapping\n"));
		cont(private_data, False);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("idmap_set_mapping returned an error\n"));
		cont(private_data, False);
		return;
	}
				   ...
				   void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger idmap_set_hwm\n"));
		cont(private_data, False);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("idmap_set_hwm returned an error\n"));
		cont(private_data, False);
		return;
	}
			       ...
			       void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger sids2xids\n"));
		cont(private_data, False, NULL, 0);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("sids2xids returned an error\n"));
		cont(private_data, False, NULL, 0);
		return;
	}
					   ...
					   struct winbindd_cli_state *state)
{
	...
	DEBUG3( ("[%5lu]: sids to unix ids\n", (unsigned long)state->pid));

	if (state->request.extra_len == 0) {
		DEBUG0( ("Invalid buffer size!\n"));
		return WINBINDD_ERROR;
	}
			       ...
			       void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger sid2uid\n"));
		cont(private_data, False, 0);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("sid2uid returned an error\n"));
		cont(private_data, False, 0);
		return;
	}
					    ...
					    struct winbindd_cli_state *state)
{
	...
	DEBUG3( ("[%5lu]: uid2name %lu\n", (unsigned long)state->pid, 
		  (unsigned long)state->request.data.uid));

	pw = getpwuid(state->request.data.uid);
	if (pw == NULL) {
		DEBUG5( ("User %lu not found\n",
			  (unsigned long)state->request.data.uid));
		return WINBINDD_ERROR;
	}
			  ...
			  void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger uid2name\n"));
		cont(private_data, False, NULL);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("uid2name returned an error\n"));
		cont(private_data, False, NULL);
		return;
	}
			  ...
			  void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger name2uid\n"));
		cont(private_data, False, 0);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("name2uid returned an error\n"));
		cont(private_data, False, 0);
		return;
	}
			       ...
			       void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger sid2gid\n"));
		cont(private_data, False, 0);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("sid2gid returned an error\n"));
		cont(private_data, False, 0);
		return;
	}
			 ...
			 void *private_data)
{
	...
	DEBUG7(("winbindd_sid2gid_async: Resolving %s to a gid\n", 
			  ...
			  void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger gid2name\n"));
		cont(private_data, False, NULL);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("gid2name returned an error\n"));
		cont(private_data, False, NULL);
		return;
	}
			  ...
			  void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger name2gid\n"));
		cont(private_data, False, 0);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("name2gid returned an error\n"));
		cont(private_data, False, 0);
		return;
	}
			   ...
			   void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger lookupsid\n"));
		cont(private_data, False, NULL, NULL, SID_NAME_UNKNOWN);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("lookupsid returned an error\n"));
		cont(private_data, False, NULL, NULL, SID_NAME_UNKNOWN);
		return;
	}
			      ...
			      void *private_data)
{
	...
	if (domain == NULL) {
		DEBUG5( ("Could not find domain for sid %s\n",
			    ...
			    void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger lookup_name\n"));
		cont(s->caller_private_data, False, NULL, SID_NAME_UNKNOWN);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("lookup_name returned an error\n"));
		cont(s->caller_private_data, False, NULL, SID_NAME_UNKNOWN);
		return;
	}

	if (!string_to_sid(&sid, response->data.sid.sid)) {
		DEBUG0( ("Could not convert string %s to sid\n",
			    ...
			    void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("lookupname_recv: lookup_name() failed!\n"));
		cont(s->caller_private_data, False, NULL, SID_NAME_UNKNOWN);
		return;
	}
	...
	if (response->result != WINBINDD_OK) {
		...
		if ( !root_domain ) {
			DEBUG5(("lookupname_recv: unable to determine forest root\n"));
			cont(s->caller_private_data, False, NULL, SID_NAME_UNKNOWN);
			return;
		}
			       ...
			       void *private_data)
{
	...
	if ( (domain = find_lookup_domain_from_name(dom_name)) == NULL ) {
		DEBUG5( ("Could not find domain for name %s\n", dom_name));
	...
	if ( (s = TALLOC_ZERO_P(mem_ctx, struct lookupname_state)) == NULL ) {
		DEBUG0( ("winbindd_lookupname_async: talloc failed\n"));
		cont(private_data, False, NULL, SID_NAME_UNKNOWN);
		return;
	}
			  ...
			  DOM_SID **sids, size_t *num_sids)
{
	...
	while (p[0] != '\0') {
		...
		q = strchr(p, '\n');
		if (q == NULL) {
			DEBUG0( ("Got invalid sidstr: %s\n", p));
			return False;
		}
		*q = '\0';
		q += 1;
		if (!string_to_sid(&sid, p)) {
			DEBUG0( ("Could not parse sid %s\n", p));
			return False;
		}
			       ...
			       void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger getsidaliases\n"));
		cont(private_data, success, NULL, 0);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("getsidaliases returned an error\n"));
		cont(private_data, False, NULL, 0);
		return;
	}
						 ...
						 struct winbindd_cli_state *state)
{
	...
	if (!parse_sidlist(state->mem_ctx, sidstr, &sids, &num_sids)) {
		DEBUG0( ("Could not parse SID list: %s\n", sidstr));
		return WINBINDD_ERROR;
	}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG3( ("Could not lookup_useraliases: %s\n",
			  nt_errstr(result)));
		return WINBINDD_ERROR;
	}
	...
	if (!print_sidlist(state->mem_ctx, sids, num_sids, &sidstr, &len)) {
		DEBUG0( ("Could not print_sidlist\n"));
			     ...
			     void *private_data)
{
	...
	if (domain == NULL) {
		DEBUG5( ("Could not find domain from SID %s\n",
				   ...
				   void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG10( ("Could not get domain groups\n"));
	...
	if (sids_str == NULL) {
		...
		if ( !sid_check_is_in_our_domain( &state->user_sid ) ) {
			DEBUG10( ("Received no domain groups\n"));
	...
	if (state->alias_domain == NULL) {
		DEBUG10( ("Don't expand domain local groups\n"));
				 ...
				 size_t num_aliases)
{
	...
	if (!success) {
		DEBUG10( ("Could not receive domain local groups\n"));
		state->cont(state->private_data, False, NULL, 0);
		return;
	}
	...
	if (state->local_alias_domain != NULL) {
		...
		DEBUG10( ("Expanding our own local groups\n"));
	...
	if (state->builtin_domain != NULL) {
		...
		DEBUG10( ("Expanding our own BUILTIN groups\n"));
			    ...
			    void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger query_user\n"));
		cont(private_data, False, NULL, NULL, NULL, NULL, -1, -1);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("query_user returned an error\n"));
		cont(private_data, False, NULL, NULL, NULL, NULL, -1, -1);
		return;
	}
				  ...
				  void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger uid2sid\n"));
		cont(private_data, False, NULL);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("uid2sid returned an error\n"));
		cont(private_data, False, NULL);
		return;
	}
				  ...
				  void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger gid2sid\n"));
		cont(private_data, False, NULL);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("gid2sid returned an error\n"));
		cont(private_data, False, NULL);
		return;
	}
			       ...
			       void *c, void *private_data)
{
	...
	if (!success) {
		DEBUG5( ("Could not trigger a map dump\n"));
		cont(private_data, False);
		return;
	}

	if (response->result != WINBINDD_OK) {
		DEBUG5( ("idmap dump maps returned an error\n"));
		cont(private_data, False);
		return;
	}
