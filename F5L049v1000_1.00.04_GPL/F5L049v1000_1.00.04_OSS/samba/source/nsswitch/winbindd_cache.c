...
void winbindd_check_cache_size(time_t t)
{
	...
	if (wcache == NULL || wcache->tdb == NULL) {
		DEBUG0( ("Unable to check size of tdb cache - cache not open !\n"));
		return;
	}

	if (fstat(tdb_fd(wcache->tdb), &st) == -1) {
		DEBUG0( ("Unable to check size of tdb cache %s!\n", strerror(errno) ));
		return;
	}

	if (st.st_size > WINBINDD_MAX_CACHE_SIZE) {
		DEBUG10(("flushing cache due to size (%lu) > (%lu)\n",
...
static struct winbind_cache *get_cache(struct winbindd_domain *domain)
{
	...
	if (!domain->backend) {
		...
		if ( (our_domain->active_directory || IS_DC) && domain->active_directory ) {
			DEBUG5(("get_cache: Setting ADS methods for domain %s\n", domain->name));
			domain->backend = &ads_methods;
		} else {
#endif	/* HAVE_ADS */
			DEBUG5(("get_cache: Setting MS-RPC methods for domain %s\n", domain->name));
...
static uint32 centry_uint32(struct cache_entry *centry)
{
	...
	if (centry->len - centry->ofs < 4) {
		DEBUG0(("centry corruption? needed 4 bytes, have %d\n", 
...
static uint16 centry_uint16(struct cache_entry *centry)
{
	...
	if (centry->len - centry->ofs < 2) {
		DEBUG0(("centry corruption? needed 2 bytes, have %d\n", 
			 centry->len - centry->ofs));
...
static uint8 centry_uint8(struct cache_entry *centry)
{
	...
	if (centry->len - centry->ofs < 1) {
		DEBUG0(("centry corruption? needed 1 bytes, have %d\n", 
			 centry->len - centry->ofs));
		smb_panic("centry_uint32");
	}
	ret = CVAL(centry->data, centry->ofs);
...
static NTTIME centry_nttime(struct cache_entry *centry)
{
	...
	if (centry->len - centry->ofs < 8) {
		DEBUG0(("centry corruption? needed 8 bytes, have %d\n", 
			 centry->len - centry->ofs));
...
static char *centry_string(struct cache_entry *centry, TALLOC_CTX *mem_ctx)
{
	...
	if (centry->len - centry->ofs < len) {
		DEBUG0(("centry corruption? needed %d bytes, have %d\n", 
...
static char *centry_hash16(struct cache_entry *centry, TALLOC_CTX *mem_ctx)
{
	...
	if (len != 16) {
		DEBUG0(("centry corruption? hash len (%u) != 16\n", 
			len ));
		return NULL;
	}

	if (centry->len - centry->ofs < 16) {
		DEBUG0(("centry corruption? needed 16 bytes, have %d\n", 
			 centry->len - centry->ofs));
		return NULL;
	}
...
static BOOL wcache_server_down(struct winbindd_domain *domain)
{
	...
	if (ret)
		DEBUG10(("wcache_server_down: server for Domain %s down\n", 
			domain->name ));
	return ret;
}
...
static NTSTATUS fetch_cache_seqnum( struct winbindd_domain *domain, time_t now )
{
	...
	if (!wcache->tdb) {
		DEBUG10(("fetch_cache_seqnum: tdb == NULL\n"));
	...
	if ( !data.dptr || data.dsize!=8 ) {
		DEBUG10(("fetch_cache_seqnum: invalid data size key [%s]\n", key ));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if ( time_diff > lp_winbind_cache_time() ) {
		DEBUG10(("fetch_cache_seqnum: timeout [%s][%u @ %u]\n",
			domain->name, domain->sequence_number,
			(uint32)domain->last_seq_check));
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG10(("fetch_cache_seqnum: success [%s][%u @ %u]\n", 
		domain->name, domain->sequence_number, 
		(uint32)domain->last_seq_check));
...
static NTSTATUS store_cache_seqnum( struct winbindd_domain *domain )
{
	...
	if (!wcache->tdb) {
		DEBUG10(("store_cache_seqnum: tdb == NULL\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	if ( tdb_store_bystring( wcache->tdb, key_str, data, TDB_REPLACE) == -1 ) {
		DEBUG10(("store_cache_seqnum: tdb_store fail key [%s]\n", key_str ));
		return NT_STATUS_UNSUCCESSFUL;
	}

	DEBUG10(("store_cache_seqnum: success [%s][%u @ %u]\n", 
		domain->name, domain->sequence_number, 
		(uint32)domain->last_seq_check));
...
static void refresh_sequence_number(struct winbindd_domain *domain, BOOL force)
{
	...
	if (!force && (time_diff < cache_time)) {
		DEBUG10( ("refresh_sequence_number: %s time ok\n", domain->name));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG10(("refresh_sequence_number: failed with %s\n", nt_errstr(status)));
	...
	DEBUG10( ("refresh_sequence_number: %s seq number is now %d\n", 
		   domain->name, domain->sequence_number));
...
static BOOL centry_expired(struct winbindd_domain *domain, const char *keystr, struct cache_entry *centry)
{
	...
	if (lp_winbind_offline_logon() && global_winbindd_offline_state) {
		DEBUG10(("centry_expired: Key %s for domain %s valid as winbindd is globally offline.\n",
			keystr, domain->name ));
		return False;
	}
	...
	if (!domain->online) {
		DEBUG10(("centry_expired: Key %s for domain %s valid as domain is offline.\n",
			keystr, domain->name ));
		return False;
	}
	...
	if ((domain->sequence_number != DOM_SEQUENCE_NONE) &&  
		...
		DEBUG10(("centry_expired: Key %s for domain %s invalid sequence.\n",
			keystr, domain->name ));
		return True;
	}
	...
	if (wcache_server_down(domain) || 
		...
		DEBUG10(("centry_expired: Key %s for domain %s is good.\n",
			keystr, domain->name ));
		return False;
	}

	DEBUG10(("centry_expired: Key %s for domain %s expired\n",
		keystr, domain->name ));
...
static struct cache_entry *wcache_fetch_raw(char *kstr)
{
	...
	if (centry->len < 8) {
		/* huh? corrupt cache? */
		DEBUG10(("wcache_fetch_raw: Corrupt cache for key %s (len < 8) ?\n", kstr));
		centry_free(centry);
		return NULL;
	}
					...
					const char *format, ...)
{
	...
	if (centry_expired(domain, kstr, centry)) {
		...
		DEBUG10(("wcache_fetch: entry %s expired for domain %s\n",
			 kstr, domain->name ));
	...
	DEBUG10(("wcache_fetch: returning entry %s for domain %s\n",
		 kstr, domain->name ));
...
static void centry_expand(struct cache_entry *centry, uint32 len)
{
	...
	centry->data = SMB_REALLOC_ARRAY(centry->data, unsigned char,
					 centry->len);
	if (!centry->data) {
		DEBUG0(("out of memory: needed %d bytes in centry_expand\n", centry->len));
...
static void centry_put_string(struct cache_entry *centry, const char *s)
{
	...
	if (len > 254) {
		DEBUG10(("centry_put_string: truncating len (%d) to: 254\n", len));
		len = 254;
	}
				    ...
				    enum lsa_SidType type)
{
	...
	DEBUG10(("wcache_save_name_to_sid: %s\\%s -> %s\n", domain_name, uname,
				    ...
				    const DOM_SID *sid, const char *domain_name, const char *name, enum lsa_SidType type)
{
	...
	DEBUG10(("wcache_save_sid_to_name: %s -> %s\n", sid_string, name));
	centry_free(centry);
}
...
static void wcache_save_user(struct winbindd_domain *domain, NTSTATUS status, WINBIND_USERINFO *info)
{
	...
	DEBUG10(("wcache_save_user: %s (acct_name %s)\n", sid_string, info->acct_name));
	centry_free(centry);
}
...
static void wcache_save_lockout_policy(struct winbindd_domain *domain, NTSTATUS status, SAM_UNK_INFO_12 *lockout_policy)
{
	...
	DEBUG10(("wcache_save_lockout_policy: %s\n", domain->name));
...
static void wcache_save_password_policy(struct winbindd_domain *domain, NTSTATUS status, SAM_UNK_INFO_1 *policy)
{
	...
	DEBUG10(("wcache_save_password_policy: %s\n", domain->name));
			  ...
			  const uint8 **cached_salt)
{
	...
	if (!centry) {
		DEBUG10(("wcache_get_creds: entry for [CRED/%s] not found\n", 
				sid_string_static(sid)));
		return NT_STATUS_OBJECT_NAME_NOT_FOUND;
	}
	...
	*cached_nt_pass = (const uint8 *)centry_hash16(centry, mem_ctx);
	if (*cached_nt_pass == NULL) {
		...
		/* Bad (old) cred cache. Delete and pretend we
		   don't have it. */
		DEBUG0(("wcache_get_creds: bad entry for [CRED/%s] - deleting\n", 
				sidstr));
	...
	DEBUG10(("wcache_get_creds: [Cached] - cached creds for user %s status: %s\n",
...
do_cached:	
	...
	DEBUG10(("query_user_list: [Cached] - cached list for domain %s status: %s\n",
...
do_query:
	...
	*info = NULL;
	...
	do {
		...
		DEBUG10(("query_user_list: [Cached] - doing backend query for list for domain %s\n",
			domain->name ));

		status = domain->backend->query_user_list(domain, mem_ctx, num_entries, info);
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG3( ("query_user_list: returned 0x%08x, "
				  "retrying\n", NT_STATUS_V(status)));
		}
		if (NT_STATUS_EQUAL(status, NT_STATUS_UNSUCCESSFUL)) {
			DEBUG3( ("query_user_list: flushing "
				  "connection cache\n"));
...
do_cached:	
	...
	DEBUG10(("enum_dom_groups: [Cached] - cached list for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	*info = NULL;
	...
	DEBUG10(("enum_dom_groups: [Cached] - doing backend query for list for domain %s\n",
		domain->name ));
...
do_cached:	
	...
	if (wcache_server_down(domain)) {
		DEBUG10( ("enum_local_groups: returning cached user list and server was down\n"));
		status = NT_STATUS_MORE_PROCESSING_REQUIRED;
	} else
		status = centry->status;

	DEBUG10(("enum_local_groups: [Cached] - cached list for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	*info = NULL;
	...
	DEBUG10(("enum_local_groups: [Cached] - doing backend query for list for domain %s\n",
		domain->name ));
			    ...
			    enum lsa_SidType *type)
{
	...
	*type = (enum lsa_SidType)centry_uint32(centry);
	...
	DEBUG10(("name_to_sid: [Cached] - cached name for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	DEBUG10(("name_to_sid: [Cached] - doing backend query for name for domain %s\n",
		domain->name ));
			    ...
			    enum lsa_SidType *type)
{
	...
	DEBUG10(("sid_to_name: [Cached] - cached name for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	*domain_name = NULL;
	...
	DEBUG10(("sid_to_name: [Cached] - doing backend query for name for domain %s\n",
		domain->name ));
			   ...
			   WINBIND_USERINFO *info)
{
	...
	if (NT_STATUS_V(domain->last_status) == NT_STATUS_V(NT_STATUS_ACCESS_DENIED) &&
		...
		DEBUG10( ("query_user: cached access denied and have cached info3\n"));
	...
	DEBUG10(("query_user: [Cached] - cached info for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	DEBUG10(("query_user: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
				  ...
				  uint32 *num_groups, DOM_SID **user_gids)
{
	...
	if (NT_STATUS_V(domain->last_status) == NT_STATUS_V(NT_STATUS_ACCESS_DENIED) &&
	    netsamlogon_cache_have(user_sid)) {
		DEBUG10( ("lookup_usergroups: cached access denied and have cached info3\n"));
		domain->last_status = NT_STATUS_OK;
		centry_free(centry);
		goto do_query;
	}
...
do_cached:	
	...
	DEBUG10(("lookup_usergroups: [Cached] - cached info for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	DEBUG10(("lookup_usergroups: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
				   ...
				   uint32 *num_aliases, uint32 **alias_rids)
{
	...
	*alias_rids = NULL;
	...
	DEBUG10(("lookup_useraliases: [Cached] - cached info for domain: %s "
		  "status %s\n", domain->name, nt_errstr(status)));
 ...
 do_query:
	...
	DEBUG10(("lookup_usergroups: [Cached] - doing backend query for info "
		  "for domain %s\n", domain->name ));
...
do_cached:	
	...
	DEBUG10(("lookup_groupmem: [Cached] - cached info for domain %s status: %s\n",
		domain->name, nt_errstr(status)));
...
do_query:
	...
	DEBUG10(("lookup_groupmem: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
				...
				DOM_SID **dom_sids)
{
	...
	*num_domains = centry_uint32(centry);
	...
	DEBUG10(("trusted_domains: [Cached] - cached info for domain %s (%d trusts) status: %s\n",
...
do_query:
	...
	DEBUG10(("trusted_domains: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
...
static NTSTATUS lockout_policy(struct winbindd_domain *domain,
	...
	DEBUG10(("lockout_policy: [Cached] - cached info for domain %s status: %s\n",
		domain->name, nt_errstr(status) ));
...
do_query:
	...
	DEBUG10(("lockout_policy: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
...
do_query:
	...
	DEBUG10(("password_policy: [Cached] - doing backend query for info for domain %s\n",
		domain->name ));
...
void wcache_invalidate_cache(void)
{
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		DEBUG10( ("wcache_invalidate_cache: invalidating cache "
			   "entries for %s\n", domain->name));
		if (cache)
...
static BOOL init_wcache(void)
{
	...
	if (wcache->tdb == NULL) {
		DEBUG0(("Failed to open winbindd_cache.tdb!\n"));
		return False;
	}
...
BOOL initialize_winbindd_cache(void)
{
	...
	if (!init_wcache()) {
		DEBUG0(("initialize_winbindd_cache: init_wcache failed.\n"));
		return False;
	}
	...
	if (cache_bad) {
		DEBUG0(("initialize_winbindd_cache: clearing cache "
		...
		if (unlink(lock_path("winbindd_cache.tdb")) == -1) {
			DEBUG0(("initialize_winbindd_cache: unlink %s failed %s ",
				lock_path("winbindd_cache.tdb"),
				strerror(errno) ));
			return False;
		}
		if (!init_wcache()) {
			DEBUG0(("initialize_winbindd_cache: re-initialization "
					"init_wcache failed.\n"));
			return False;
		}
		...
		if (!tdb_store_uint32(wcache->tdb, WINBINDD_CACHE_VERSION_KEYSTR, WINBINDD_CACHE_VERSION)) {
			DEBUG0(("initialize_winbindd_cache: version number store failed %s\n",
...
void cache_store_response(pid_t pid, struct winbindd_response *response)
{
	...
	DEBUG10( ("Storing response for pid %d, len %d\n",
		   pid, response->length));
	...
	DEBUG10( ("Storing extra data: len=%d\n",
...
BOOL cache_retrieve_response(pid_t pid, struct winbindd_response * response)
{
	...
	DEBUG10( ("Retrieving response for pid %d\n", pid));
	...
	DEBUG10( ("Retrieving extra data length=%d\n",
		   (int)(response->length - sizeof(*response))));
	...
	if (data.dptr == NULL) {
		DEBUG0( ("Did not find extra data\n"));
		return False;
	}

	if (data.dsize != (response->length - sizeof(*response))) {
		DEBUG0( ("Invalid extra data length: %d\n", (int)data.dsize));
		SAFE_FREE(data.dptr);
		return False;
	}
...
void wcache_flush_cache(void)
{
	...
	DEBUG10(("wcache_flush_cache success\n"));
}
				    ...
				    void *state)
{
	...
	if (strncmp(kbuf.dptr, "CRED/", 5) == 0) {
		...
		if (cred == NULL) {
			DEBUG0(("traverse_fn_remove_first_creds: failed to malloc new entry for list\n"));
			return -1;
		}
...
NTSTATUS wcache_remove_oldest_cached_creds(struct winbindd_domain *domain, const DOM_SID *sid) 
{
	...
	for (cred = wcache_cred_list; cred; cred = cred->next) {
		...
		if (!data.dptr) {
			DEBUG10(("wcache_remove_oldest_cached_creds: entry for [%s] not found\n", 
				cred->name));
...
BOOL set_global_winbindd_state_offline(void)
{
	...
	DEBUG10(("set_global_winbindd_state_offline: offline requested.\n"));

	/* Only go offline if someone has created
	   the key "WINBINDD_OFFLINE" in the cache tdb. */

	if (wcache == NULL || wcache->tdb == NULL) {
		DEBUG10(("set_global_winbindd_state_offline: wcache not open yet.\n"));
		return False;
	}

	if (!lp_winbind_offline_logon()) {
		DEBUG10(("set_global_winbindd_state_offline: rejecting.\n"));
		return False;
	}
	...
	if (!data.dptr || data.dsize != 4) {
		DEBUG10(("set_global_winbindd_state_offline: offline state not set.\n"));
		SAFE_FREE(data.dptr);
		return False;
	} else {
		DEBUG10(("set_global_winbindd_state_offline: offline state set.\n"));
...
{
	DEBUG10(("set_global_winbindd_state_online: online requested.\n"));

	if (!lp_winbind_offline_logon()) {
		DEBUG10(("set_global_winbindd_state_online: rejecting.\n"));
		return;
	}
