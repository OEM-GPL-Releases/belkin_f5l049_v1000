					...
					struct WINBINDD_MEMORY_CREDS *entry)
{
	if (client_uid == entry->uid || client_uid == 0) {
		DEBUG10( ("Access granted to uid %d\n", client_uid));
		return True;
	}

	DEBUG1( ("Access denied to uid %d (expected %d)\n", client_uid, entry->uid));
	return False;
}
					...
					DATA_BLOB *auth_msg)
{
	...
	*/
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED)) {
		DEBUG1( ("Failed to create initial message! [%s]\n",
			nt_errstr(status)));
		goto done;
	}
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_OK)) {
		DEBUG1( ("We didn't get a response to the challenge! [%s]\n",
			nt_errstr(status)));
		data_blob_free(&reply);
		goto done;
	}
...
static BOOL check_client_uid(struct winbindd_cli_state *state, uid_t uid)
{
	...
	if (ret != 0) {
		DEBUG1( ("check_client_uid: Could not get socket peer uid: %s; "
			"denying access\n", strerror(errno)));
		return False;
	}

	if (uid != ret_uid) {
		DEBUG1( ("check_client_uid: Client lied about its uid: said %d, "
...
void winbindd_ccache_ntlm_auth(struct winbindd_cli_state *state)
{
	...
	DEBUG3( ("[%5lu]: perform NTLM auth on behalf of user %s\n", (unsigned long)state->pid,
		state->request.data.ccache_ntlm_auth.user));
	...
	if (!canonicalize_username(state->request.data.ccache_ntlm_auth.user,
		...
		DEBUG5(("winbindd_ccache_ntlm_auth: cannot parse domain and user from name [%s]\n",
			state->request.data.ccache_ntlm_auth.user));
		request_error(state);
		return;
	}
	...
	if (domain == NULL) {
		DEBUG5(("winbindd_ccache_ntlm_auth: can't get domain [%s]\n",
			name_domain));
		request_error(state);
		return;
	}
						...
						struct winbindd_cli_state *state)
{
	...
	DEBUG3( ("winbindd_dual_ccache_ntlm_auth: [%5lu]: perform NTLM auth on "
	...
	if (initial_blob_len > extra_len || challenge_blob_len > extra_len ||
		...
		DEBUG10(("winbindd_dual_ccache_ntlm_auth: blob lengths overrun "
	...
	if (!parse_domain_user(state->request.data.ccache_ntlm_auth.user, name_domain, name_user)) {
		DEBUG10(("winbindd_dual_ccache_ntlm_auth: cannot parse "
	...
	if (entry == NULL || entry->nt_hash == NULL || entry->lm_hash == NULL) {
		DEBUG10(("winbindd_dual_ccache_ntlm_auth: could not find "
			"credentials for user %s\n", 
			state->request.data.ccache_ntlm_auth.user));
		goto process_result;
	}

	DEBUG10(("winbindd_dual_ccache_ntlm_auth: found ccache [%s]\n", entry->username));
