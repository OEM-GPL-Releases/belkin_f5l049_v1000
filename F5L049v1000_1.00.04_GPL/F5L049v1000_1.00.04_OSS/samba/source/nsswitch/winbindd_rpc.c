				  ...
				  enum lsa_SidType *type)
{
	...
	POLICY_HND lsa_policy;
        ...
        if(name == NULL || *name=='\0') {
                DEBUG3(("rpc: name_to_sid name=%s\n", domain_name));
                full_name = talloc_asprintf(mem_ctx, "%s", domain_name);
        } else {
                DEBUG3(("rpc: name_to_sid name=%s\\%s\n", domain_name, name));
                full_name = talloc_asprintf(mem_ctx, "%s\\%s", domain_name, name);
        }
	if (!full_name) {
		DEBUG0( ("talloc_asprintf failed!\n"));
		return NT_STATUS_NO_MEMORY;
	}

	ws_name_return( full_name, WB_REPLACE_CHAR );

	DEBUG3(("name_to_sid [rpc] %s for domain %s\n", full_name?full_name:"", domain_name ));
				  ...
				  enum lsa_SidType *type)
{
	...
	DEBUG3(("sid_to_name [rpc] %s for domain %s\n", sid_string_static(sid),
			domain->name ));
				    ...
				    enum lsa_SidType **types)
{
	...
	DEBUG3( ("rids_to_names [rpc] for domain %s\n", domain->name ));
			   ...
			   WINBIND_USERINFO *user_info)
{
	...
	if ( (user = netsamlogon_cache_get( mem_ctx, user_sid )) != NULL ) 
	{
		...
		DEBUG5(("query_user: Cache lookup succeeded for %s\n", 
				  ...
				  uint32 *num_groups, DOM_SID **user_grpsids)
{
	...
	DEBUG3(("rpc: lookup_usergroups sid=%s\n",
		 sid_to_string(sid_string, user_sid)));
					 ...
					 uint32 **alias_rids)
{
	...
	*alias_rids = NULL;
	...
	do {
		...
		DEBUG10(("rpc: lookup_useraliases: entering query %d for %d sids\n", 
	...
	DEBUG10(("rpc: lookup_useraliases: got %d aliases in %d queries "
				...
				uint32 **name_types)
{
	...
	DEBUG10(("rpc: lookup_groupmem %s sid=%s\n", domain->name,
...
static int get_ldap_sequence_number(struct winbindd_domain *domain, uint32 *seq)
{
	...
	if ((ret = get_ldap_seq( ipstr, LDAP_PORT, seq)) == 0) {
		DEBUG3( ("get_ldap_sequence_number: Retrieved sequence "
...
static NTSTATUS sequence_number(struct winbindd_domain *domain, uint32 *seq)
{
	...
	DEBUG10(("rpc: fetch sequence_number for %s\n", domain->name));
	...
	*seq = DOM_SEQUENCE_NONE;
	...
	if ( domain->native_mode ) 
	{
		...
		DEBUG8(("using get_ldap_seq() to retrieve the "
			 "sequence number\n"));
		...
		{			
			result = NT_STATUS_OK;
			DEBUG10(("domain_sequence_number: LDAP for "
				  "domain %s is %u\n",
				  domain->name, *seq));
			goto done;
		}

		DEBUG10(("domain_sequence_number: failed to get LDAP "
 ...
 seq_num:
	if (got_seq_num) {
		DEBUG10(("domain_sequence_number: for domain %s is %u\n",
			  domain->name, (unsigned)*seq));
	} else {
		DEBUG10(("domain_sequence_number: failed to get sequence "
				     ...
				     SAM_UNK_INFO_12 *lockout_policy)
{
	...
	DEBUG10(("rpc: fetch lockout policy for %s\n", domain->name));
	...
	*lockout_policy = ctr.info.inf12;
	...
	DEBUG10(("msrpc_lockout_policy: bad_attempt_lockout %d\n", 
				      ...
				      SAM_UNK_INFO_1 *password_policy)
{
	...
	DEBUG10(("rpc: fetch password policy for %s\n", domain->name));
	...
	*password_policy = ctr.info.inf1;
	...
	DEBUG10(("msrpc_password_policy: min_length_password %d\n", 
