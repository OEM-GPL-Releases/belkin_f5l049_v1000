					...
					void *private_data)
{
	...
	DEBUG10(("krb5_ticket_refresh_handler called\n"));
...
rekinit:
		if (cred_ptr && cred_ptr->pass) {
			...
			if (ret) {
				DEBUG3(("krb5_ticket_refresh_handler: "
			...
			DEBUG10(("krb5_ticket_refresh_handler: successful re-kinit "
				"for: %s in ccache: %s\n",
	...
	if (ret) {
		DEBUG3(("krb5_ticket_refresh_handler: could not renew tickets: %s\n",
			error_message(ret)));
					...
					void *private_data)
{
	...
	DEBUG10(("krb5_ticket_gain_handler called\n"));
	DEBUGADD(10,("event called for: %s, %s\n", entry->ccname, entry->username));
	...
	if (!cred_ptr || !cred_ptr->pass) {
		DEBUG10(("krb5_ticket_gain_handler: no memory creds\n"));
		return;
	}

	if ((domain = find_domain_from_name(entry->realm)) == NULL) {
		DEBUG0(("krb5_ticket_gain_handler: unknown domain\n"));
		return;
	}
	...
	if (domain->online) {
		...
		if (ret) {
			DEBUG3(("krb5_ticket_gain_handler: could not kinit: %s\n",
				error_message(ret)));
		...
		DEBUG10(("krb5_ticket_gain_handler: successful kinit for: %s in ccache: %s\n",
			entry->principal_name, entry->ccname));
...
BOOL ccache_entry_identical(const char *username, uid_t uid, const char *ccname)
{
	...
	if (entry->uid != uid) {
		DEBUG0(("cache_entry_identical: uid's differ: %u != %u\n",
			(unsigned int)entry->uid, (unsigned int)uid ));
		return False;
	}
	if (!strcsequal(entry->ccname, ccname)) {
		DEBUG0(("cache_entry_identical: ccnames differ: (cache) %s != (client) %s\n",
                        entry->ccname, ccname));
		return False;
	}
	return True;
			    ...
			    BOOL postponed_request)
{
	...
	if (ccache_entry_count() + 1 > MAX_CCACHES) {
		DEBUG10(("add_ccache_to_list: max number of ccaches reached\n"));
	...
	if (postponed_request) {
		...
		if (ret) {
			DEBUG0( ("add_ccache_to_list: failed to destroy "
				   "user krb5 ccache %s with %s\n", ccname,
				   error_message(ret)));
			return krb5_to_nt_status(ret);
		} else {
			DEBUG10( ("add_ccache_to_list: successfully destroyed "
	...
	if (entry) {
		...
		entry->ref_count++;
		DEBUG10(("add_ccache_to_list: ref count on entry %s is now %d\n",
		...
		if (!entry->event) {
			...
			if (!entry->event) {
				...
				if (!NT_STATUS_IS_OK(ntret)) {
					DEBUG0( ("add_ccache_to_list: Failed to remove krb5 "
						  "ccache %s for user %s\n", entry->ccname,
						  entry->username));
					DEBUG0( ("add_ccache_to_list: error is %s\n",
						  nt_errstr(ntret)));
					return ntret;
				}
			...
			DEBUG10(("add_ccache_to_list: added krb5_ticket handler\n"));
		}
	...
	DEBUG10(("add_ccache_to_list: added ccache [%s] for user [%s] to the list\n", ccname, username));
...
NTSTATUS remove_ccache(const char *username)
{
	...
	if (entry->ref_count <= 0) {
		DEBUG0(("remove_ccache: logic error. ref count for user %s = %d\n",
			username, entry->ref_count));
	...
	if (entry->ref_count > 0) {
		DEBUG10(("remove_ccache: entry %s ref count now %d\n",
			username, entry->ref_count ));
		return NT_STATUS_OK;
	}
	...
	if (ret == KRB5_FCC_NOFILE) {
		...
		DEBUG0(("remove_ccache: failed to destroy user krb5 ccache %s with: %s\n",
			entry->ccname, error_message(ret)));
	} else {
		DEBUG10(("remove_ccache: successfully destroyed krb5 ccache %s for user %s\n",
	...
	TALLOC_FREE(entry);
 	DEBUG10(("remove_ccache: removed ccache for user %s\n", username));
...
static NTSTATUS store_memory_creds(struct WINBINDD_MEMORY_CREDS *memcredp, const char *pass)
{
	...
	DEBUG10(("mlocking memory: %p\n", memcredp->nt_hash));
#endif		
	if ((mlock(memcredp->nt_hash, memcredp->len)) == -1) {
		DEBUG0(("failed to mlock memory: %s (%d)\n", 
...
static NTSTATUS delete_memory_creds(struct WINBINDD_MEMORY_CREDS *memcredp)
{
	...
	if (munlock(memcredp->nt_hash, memcredp->len) == -1) {
		DEBUG0(("failed to munlock memory: %s (%d)\n", 
			strerror(errno), errno));
		return map_nt_error_from_unix(errno);
	}
...
static NTSTATUS winbindd_add_memory_creds_internal(const char *username, uid_t uid, const char *pass)
{
	...
	if (uid == (uid_t)-1) {
		DEBUG0(("winbindd_add_memory_creds_internal: invalid uid for user %s.\n",
			username ));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	if (memcredp) {
		...
		if (uid != memcredp->uid) {
			DEBUG0(("winbindd_add_memory_creds_internal: uid %u for user %s doesn't "
				"match stored uid %u. Replacing.\n",
				(unsigned int)uid, username, (unsigned int)memcredp->uid ));
			memcredp->uid = uid;
		}
		memcredp->ref_count++;
		DEBUG10(("winbindd_add_memory_creds_internal: ref count for user %s is now %d\n",
	...
	DEBUG10(("winbindd_add_memory_creds_internal: added entry for user %s\n",
		username ));
...
NTSTATUS winbindd_delete_memory_creds(const char *username)
{
	...
	if (!memcredp) {
		DEBUG10(("winbindd_delete_memory_creds: unknown user %s\n",
			username ));
		return NT_STATUS_OBJECT_NAME_NOT_FOUND;
	}

	if (memcredp->ref_count <= 0) {
		DEBUG0(("winbindd_delete_memory_creds: logic error. ref count for user %s = %d\n",
			username, memcredp->ref_count));
	...
	if (memcredp->ref_count <= 0) {
		...
		talloc_destroy(memcredp);
		DEBUG10(("winbindd_delete_memory_creds: deleted entry for user %s\n",
			username));
	} else {
		DEBUG10(("winbindd_delete_memory_creds: entry for user %s ref_count now %d\n",
			username, memcredp->ref_count));
	}
...
NTSTATUS winbindd_replace_memory_creds(const char *username, const char *pass)
{
	...
	if (!memcredp) {
		DEBUG10(("winbindd_replace_memory_creds: unknown user %s\n",
			username ));
		return NT_STATUS_OBJECT_NAME_NOT_FOUND;
	}

	DEBUG10(("winbindd_replace_memory_creds: replaced creds for user %s\n",
		username ));
