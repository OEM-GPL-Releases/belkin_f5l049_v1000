			    ...
			    struct idmap_methods *methods)
{
	...
	if (!idmap_ctx) {
 	...
 	if ((version != SMB_IDMAP_INTERFACE_VERSION)) {
		DEBUG0( ("Failed to register idmap module.\n"
	...
	if (!name || !name[0] || !methods) {
		DEBUG0(("Called with NULL pointer or empty name!\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}

	test = get_methods(backends, name);
	if (test) {
		DEBUG0(("Idmap module %s already registered!\n", name));
	...
	DEBUG5( ("Successfully added idmap backend '%s'\n", name));
	return NT_STATUS_OK;
}
				  ...
				  struct idmap_alloc_methods *methods)
{
	...
	if (!idmap_ctx) {
 	...
 	if ((version != SMB_IDMAP_INTERFACE_VERSION)) {
		DEBUG0( ("Failed to register idmap alloc module.\n"
		          "The module was compiled against "
			  "SMB_IDMAP_INTERFACE_VERSION %d,\n"
		          "current SMB_IDMAP_INTERFACE_VERSION is %d.\n"
		          "Please recompile against the current version "
	...
	if (test) {
		DEBUG0(("idmap_alloc module %s already registered!\n", name));
		return NT_STATUS_OBJECT_NAME_COLLISION;
	}
	...
	DEBUG5( ("Successfully added idmap alloc backend '%s'\n", name));
	return NT_STATUS_OK;
}
...
static int close_domain_destructor(struct idmap_domain *dom)
{
	...
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG3( ("Failed to close idmap domain [%s]!\n", dom->name));
	}
...
NTSTATUS idmap_init(void)
{
	...
	if ( lp_idmap_backend() ) {
		...
		if (dom_list) {
			DEBUG0( ("WARNING: idmap backend and idmap domains "
			...
			if (strncmp(*compat_list, "idmap_", 6) == 0 ) {
				q = *compat_list += 6;
				DEBUG0( ("WARNING: idmap backend uses obsolete"
	 ...
	 */
	DEBUG1( ("Initializing idmap domains\n"));
	...
	for (i=0, num_domains=0; dom_list[i]; i++) {
		...
		{
			DEBUG0(("idmap_init: Ignoring domain %s\n",
				 dom_list[i]));
			continue;
		}
		...
		if (dom->default_domain ||
			...
			if (default_already_defined) {
				DEBUG1( ("ERROR: Multiple domains defined as"
					  " default!\n"));
		...
		if ( ! dom->methods) {
			DEBUG0( ("ERROR: Could not get methods for "
				  "backend %s\n", parm_backend));
		...
		if ( ! dom->methods->set_mapping) {
			DEBUG5( ("Forcing to readonly, as this module can't"
		...
		if ( ! NT_STATUS_IS_OK(ret)) {
			DEBUG0( ("ERROR: Initialization failed for backend "
				  "%s (domain %s), deferred!\n",
		...
		DEBUG10( ("Domain %s - Backend %s - %sdefault - %sreadonly\n",
	...
	if ((lp_server_role() == ROLE_DOMAIN_MEMBER) &&
		...
		if ( ! dom->methods) {
			DEBUG0( ("ERROR: No methods for idmap_nss ?!\n"));
			ret = NT_STATUS_UNSUCCESSFUL;
			goto done;
		}
		...
		ret = dom->methods->init(dom);
		if ( ! NT_STATUS_IS_OK(ret)) {
			DEBUG0( ("ERROR: Init. failed for idmap_nss ?!\n"));
			ret = NT_STATUS_UNSUCCESSFUL;
			goto done;
		}
		...
		DEBUG10( ("Domain %s - Backend nss - not default - readonly\n",
			   dom->name ));
	...
	if ( ! dom->methods) {
		DEBUG0( ("ERROR: No methods for idmap_passdb ?!\n"));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	ret = dom->methods->init(dom);
	if ( ! NT_STATUS_IS_OK(ret)) {
		DEBUG0( ("ERROR: Init. failed for idmap_passdb ?!\n"));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	DEBUG10( ("Domain %s - Backend passdb - not default - readonly\n",
		   dom->name));
	...
	DEBUG3( ("Initializing idmap alloc module\n"));
	...
	if ( alloc_backend ) {
		...
		if (idmap_alloc_ctx->methods) {
			...
			if ( ! NT_STATUS_IS_OK(ret)) {
				DEBUG0( ("ERROR: Initialization failed for "
			...
			DEBUG2( ("idmap_init: Unable to get methods for "
				  "alloc backend %s\n",
				  alloc_backend));
	...
	DEBUG0( ("Aborting IDMAP Initialization ...\n"));
	idmap_close();
...
static NTSTATUS idmap_alloc_init(void)
{
	...
	if ( ! idmap_alloc_ctx->initialized) {
		ret = idmap_alloc_ctx->methods->init(idmap_alloc_ctx->params);
		if ( ! NT_STATUS_IS_OK(ret)) {
			DEBUG0( ("ERROR: Initialization failed for alloc "
				  "backend, deferred!\n"));
			return ret;
		} else {
			idmap_alloc_ctx->initialized = True;
			      ...
			      struct idmap_domain **ret_dom)
{
	...
	if ((sid_compare_domain(map->sid, get_global_sam_sid()) == 0) ||
		...
		DEBUG10( ("We are not supposed to create mappings for "
	...
	if ( (dom = find_idmap_domain_from_sid( map->sid )) == NULL ) {
		...
		DEBUG10( ("Could not find idmap backend for SID %s\n",
			   sid_string_static(map->sid)));
	...
	if (dom->readonly) {
		 ...
		 *  let's just leave it unmapped */
		DEBUG10( ("idmap backend for SID %s is READONLY!\n",
			   sid_string_static(map->sid)));
		return NT_STATUS_UNSUCCESSFUL;
	}
...
static NTSTATUS idmap_new_mapping(TALLOC_CTX *ctx, struct id_map *map)
{
	...
	default:
		...
		DEBUG3(("idmap_new_mapping: Refusing to create a "
	...
	DEBUG10( ("Setting mapping: %s <-> %s %lu\n",
		   sid_string_static(map->sid),
	...
	if ( ! NT_STATUS_IS_OK(ret)) {
		...
		DEBUG2( ("Failed to commit mapping\n!"));
...
static NTSTATUS idmap_backends_set_mapping(const struct id_map *map)
{
	...
	DEBUG10(("set_mapping for domain %s\n", dom->name ));
...
static NTSTATUS idmap_backends_unixids_to_sids(struct id_map **ids)
{
	...
	DEBUG10( ("Query backends to map ids->sids\n"));
	...
	for (n = num_domains-1; n >= 0; n--) { /* cycle backwards */
		...
		DEBUG10( ("Query sids from domain %s\n", dom->name));
...
static NTSTATUS idmap_backends_sids_to_unixids(struct id_map **ids)
{
	...
	if ( (ctx = talloc_named_const(NULL, 0, "be_sids_to_ids")) == NULL ) {
		DEBUG1( ("failed to allocate talloc context, OOM?\n"));
		return NT_STATUS_NO_MEMORY;
	}

	DEBUG10( ("Query backends to map sids->ids\n"));

	/* split list per domain */
	if (num_domains == 0) {
		DEBUG1( ("No domains available?\n"));
		return NT_STATUS_UNSUCCESSFUL;
	}
	...
	for (i = 0; ids[i]; i++) {
		...
		DEBUG10(("SID %s is being handled by %s\n",
	...
	for (i = 0; i < num_domains; i++) {
		if (dom_ids[i]) {
			dom = idmap_domains[i];
			DEBUG10( ("Query ids from domain %s\n", dom->name));
...
NTSTATUS idmap_unixids_to_sids(struct id_map **ids)
{
	...
	for (i = 0; ids[i]; i++) {
		...
		if ( ! ids[i]->sid) {
			DEBUG1( ("invalid null SID in id_map array"));
			talloc_free(ctx);
			return NT_STATUS_INVALID_PARAMETER;
		}
...
NTSTATUS idmap_set_mapping(const struct id_map *id)
{
	...
	if ((id->sid == NULL) || (id->status != ID_MAPPED)) {
		DEBUG1( ("NULL SID or unmapped entry\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
void idmap_dump_maps(char *logfile)
{
	...
	if ( ! dump) {
		DEBUG0( ("Unable to open open stream for file [%s], "
			  "errno: %d\n", logfile, errno));
		return;
	}
