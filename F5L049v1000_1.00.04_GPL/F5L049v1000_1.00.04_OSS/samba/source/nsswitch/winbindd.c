...
static void winbindd_status(void)
{
	...
	DEBUG0( ("winbindd status:\n"));

	/* Print client state information */
	
	DEBUG0( ("\t%d clients currently active\n", winbindd_num_clients()));
...
static void request_len_recv(void *private_data, BOOL success)
{
	...
	if (*(uint32 *)(&state->request) != sizeof(state->request)) {
		DEBUG0(("request_len_recv: Invalid request size received: %d (expected %d)\n",
...
static void request_main_recv(void *private_data, BOOL success)
{
	...
	if ((!state->privileged) &&
		...
		DEBUG3( ("Got request with %d bytes extra data on "
...
static BOOL remove_idle_client(void)
{
	...
	if (remove_state) {
		DEBUG5(("Found %d idle client connections, shutting down sock %d, pid %u\n",
...
static void process_loop(void)
{
	...
	if (FD_ISSET(listen_sock, &r_fds)) {
		while (winbindd_num_clients() >
			...
			DEBUG5(("winbindd: Exceeding %d client "
				 "connections, removing idle "
				 "connection.\n",
				 WINBINDD_MAX_SIMULTANEOUS_CLIENTS));
			if (!remove_idle_client()) {
				DEBUG0(("winbindd: Exceeding %d "
...
int main(int argc, char **argv, char **envp)
{
	...
	if ( !winbind_off() ) {
		DEBUG0(("Failed to disable recusive winbindd calls.  Exiting.\n"));
		exit(1);
	}
	...
	DEBUG1( ("winbindd version %s started.\n%s\n", 
		  SAMBA_VERSION_STRING, 
		  COPYRIGHT_STARTUP_MESSAGE) );

	if (!reload_services_file(NULL)) {
		DEBUG0( ("error opening config file\n"));
		exit(1);
	}
	...
	if (!secrets_init()) {
		...
		DEBUG0(("Could not initialize domain trust account secrets. Giving up\n"));
		return False;
	}
	...
	if ( ! NT_STATUS_IS_OK(idmap_init_cache()) ) {
		DEBUG1( ("Could not init idmap cache!\n"));		
	}
	...
	if (!message_init()) {
		DEBUG0( ("unable to initialize messaging system\n"));
		exit(1);
	}
	...
	if (!init_domain_list()) {
		DEBUG0(("unable to initalize domain list\n"));
		exit(1);
	}
