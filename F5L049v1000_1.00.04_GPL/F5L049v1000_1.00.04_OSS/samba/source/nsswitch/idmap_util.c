...
NTSTATUS idmap_uid_to_sid(DOM_SID *sid, uid_t uid)
{
	...
	if ( ! NT_STATUS_IS_OK(ret)) {
		DEBUG10( ("error mapping uid [%lu]\n", (unsigned long)uid));
		return ret;
	}

	if (map.status != ID_MAPPED) {
		DEBUG10( ("uid [%lu] not mapped\n", (unsigned long)uid));
...
NTSTATUS idmap_gid_to_sid(DOM_SID *sid, gid_t gid)
{
	...
	ret = idmap_unixids_to_sids(maps);
	if ( ! NT_STATUS_IS_OK(ret)) {
		DEBUG10( ("error mapping gid [%lu]\n", (unsigned long)gid));
		return ret;
	}

	if (map.status != ID_MAPPED) {
		DEBUG10( ("gid [%lu] not mapped\n", (unsigned long)gid));
		return NT_STATUS_NONE_MAPPED;
	}
...
NTSTATUS idmap_sid_to_uid(DOM_SID *sid, uid_t *uid)
{
	...
	DEBUG10(("idmap_sid_to_uid: sid = [%s]\n", sid_string_static(sid)));
	...
	if ( ! NT_STATUS_IS_OK(ret)) {
		DEBUG10( ("error mapping sid [%s] to uid\n", 
			   sid_string_static(sid)));
		return ret;
	}

	if ((map.status != ID_MAPPED) || (map.xid.type != ID_TYPE_UID)) {
		DEBUG10( ("sid [%s] not mapped to an uid [%u,%u,%u]\n", 
			   sid_string_static(sid), 
			   map.status, 
			   map.xid.type, 
			   map.xid.id));
...
NTSTATUS idmap_sid_to_gid(DOM_SID *sid, gid_t *gid)
{
	...
	DEBUG10(("idmap_sid_to_gid: sid = [%s]\n", sid_string_static(sid)));
	...
	ret = idmap_sids_to_unixids(maps);
	if ( ! NT_STATUS_IS_OK(ret)) {
		DEBUG10( ("error mapping sid [%s] to gid\n", 
			   sid_string_static(sid)));
		return ret;
	}

	if ((map.status != ID_MAPPED) || (map.xid.type != ID_TYPE_GID)) {
		DEBUG10( ("sid [%s] not mapped to an gid [%u,%u]\n", 
			   sid_string_static(sid), 
			   map.status, 
			   map.xid.type));
		return NT_STATUS_NONE_MAPPED;
