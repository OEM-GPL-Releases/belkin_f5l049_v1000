...
static void child_read_request(struct winbindd_cli_state *state)
{
	...
	DEBUG10( ("Need to read %d extra bytes\n", (int)state->request.extra_len));
	...
	if (len != state->request.extra_len) {
		DEBUG0( ("Could not read extra data\n"));
		state->finished = True;
		return;
	}
}
...
static void async_main_request_sent(void *private_data, BOOL success)
{
	...
	if (!success) {
		DEBUG5( ("Could not send async request\n"));
					...
					void *private_data)
{
	...
	DEBUG0(("async_request_timeout_handler: child pid %u is not responding. "
...
static void async_request_sent(void *private_data_data, BOOL success)
{
	...
	if (!success) {
		DEBUG5( ("Could not send async request to child pid %u\n",
...
static void async_reply_recv(void *private_data, BOOL success)
{
	...
	if (!success) {
		DEBUG5( ("Could not receive async reply from child pid %u\n",
			(unsigned int)state->child_pid ));
...
static void domain_init_recv(void *private_data_data, BOOL success)
{
	...
	if (!success) {
		DEBUG5( ("Domain init returned an error\n"));
				  ...
				  struct winbindd_cli_state *state)
{
	...
	for (table = child_dispatch_table; table->fn; table++) {
		if (state->request.cmd == table->cmd) {
			DEBUG10(("process_request: request fn %s\n",
	...
	if (!table->fn) {
		DEBUG10(("process_request: unknown request fn number %d\n",
...
void winbind_child_died(pid_t pid)
{
	...
	if (child == NULL) {
		DEBUG5( ("Already reaped child %u died\n", (unsigned int)pid));
		return;
	}
			 ...
			 void *buf, size_t len, void *private_data)
{
	...
	DEBUG10(("winbind_msg_offline: got offline message.\n"));

	if (!lp_winbind_offline_logon()) {
		DEBUG10(("winbind_msg_offline: rejecting offline message.\n"));
		return;
	}

	/* Set our global state as offline. */
	if (!set_global_winbindd_state_offline()) {
		DEBUG10(("winbind_msg_offline: offline request failed.\n"));
		return;
	}
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		DEBUG5(("winbind_msg_offline: marking %s offline.\n", domain->name));
	...
	for (child = children; child != NULL; child = child->next) {
		...
		DEBUG10(("winbind_msg_offline: sending message to pid %u for domain %s.\n",
			...
			void *buf, size_t len, void *private_data)
{
	...
	DEBUG10(("winbind_msg_online: got online message.\n"));

	if (!lp_winbind_offline_logon()) {
		DEBUG10(("winbind_msg_online: rejecting online message.\n"));
		return;
	}
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		DEBUG5(("winbind_msg_online: requesting %s to go online.\n", domain->name));
	...
	for (child = children; child != NULL; child = child->next) {
		...
		DEBUG10(("winbind_msg_online: sending message to pid %u for domain %s.\n",
			      ...
			      void *buf, size_t len, void *private_data)
{
	...
	DEBUG10(("winbind_msg_onlinestatus: got onlinestatus message.\n"));

	for (child = children; child != NULL; child = child->next) {
		if (child->domain && child->domain->primary) {
			DEBUG10(("winbind_msg_onlinestatus: "
					   ...
					   void *private_data)
{
	...
	DEBUG10(("account_lockout_policy_handler called\n"));
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG10(("account_lockout_policy_handler: lockout_policy failed error %s\n",
			 nt_errstr(result)));
	}
			      ...
			      void *buf, size_t len, void *private_data)
{
	...
	DEBUG5(("child_msg_offline received for domain %s.\n", domainname));

	if (!lp_winbind_offline_logon()) {
		DEBUG10(("child_msg_offline: rejecting offline message.\n"));
		return;
	}

	/* Set our global state as offline. */
	if (!set_global_winbindd_state_offline()) {
		DEBUG10(("child_msg_offline: offline request failed.\n"));
		return;
	}
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (strequal(domain->name, domainname)) {
			DEBUG5(("child_msg_offline: marking %s offline.\n", domain->name));
			set_domain_offline(domain);
			     ...
			     void *buf, size_t len, void *private_data)
{
	...
	DEBUG5(("child_msg_online received for domain %s.\n", domainname));

	if (!lp_winbind_offline_logon()) {
		DEBUG10(("child_msg_online: rejecting online message.\n"));
		return;
	}
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (strequal(domain->name, domainname)) {
			DEBUG5(("child_msg_online: requesting %s to go online.\n", domain->name));
			winbindd_flush_negative_conn_cache(domain);
			set_domain_online_request(domain);
...
static const char *collect_onlinestatus(TALLOC_CTX *mem_ctx)
{
	...
	DEBUG5(("collect_onlinestatus: %s", buf));
				   ...
				   void *buf, size_t len, void *private_data)
{
	...
	DEBUG5(("winbind_msg_onlinestatus received.\n"));
...
bool winbindd_reinit_after_fork(const char *logfile)
{
	...
	if (!reinit_after_fork(NULL,
		...
		DEBUG0( ("reinit_after_fork failed.\n"));
		return false;
	}
...
static BOOL fork_domain_child(struct winbindd_child *child)
{
	...
	if (socketpair(AF_UNIX, SOCK_STREAM, 0, fdpair) != 0) {
		DEBUG0( ("Could not open child pipe: %s\n",
			  strerror(errno)));
		return False;
	}
	...
	while (1) {
		...
		if (child->domain && child->domain->startup &&
			...
			DEBUG10(("fork_domain_child: domain %s no longer in 'startup' mode.\n",
				child->domain->name ));
