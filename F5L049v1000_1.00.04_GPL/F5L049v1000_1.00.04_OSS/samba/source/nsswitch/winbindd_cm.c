				    ...
				    void *buf, size_t len, void *private_data)
{
	...
	DEBUG5(("msg_fail_to_go_online: received for domain %s.\n", domainname));
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (strequal(domain->name, domainname)) {
			if (domain->online) {
				...
				DEBUG5(("msg_fail_to_go_online: domain %s "
					"already online.\n", domainname));
				continue;
			}
				 ...
				 void *buf, size_t len, void *private_data)
{
	...
	DEBUG5(("msg_try_to_go_online: received for domain %s.\n", domainname));
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (strequal(domain->name, domainname)) {
			...
			if (domain->online) {
				/* We're already online, ignore. */
				DEBUG5(("msg_try_to_go_online: domain %s "
					"already online.\n", domainname));
				continue;
			}
...
static BOOL fork_child_dc_connect(struct winbindd_domain *domain)
{
	...
	if (child_pid == -1) {
		DEBUG0( ("fork_child_dc_connect: Could not fork: %s\n", strerror(errno)));
		message_unblock();
		return False;
	}
	...
	if (!winbindd_reinit_after_fork(logfile)) {
		DEBUG0(("winbindd_reinit_after_fork failed.\n"));
					...
					void *private_data)
{
	...
	DEBUG10(("check_domain_online_handler: called for domain %s\n",
		domain->name ));
	...
	if (domain->startup && (now->tv_sec > domain->startup_time + 30)) {
		...
		DEBUG10(("check_domain_online_handler: domain %s no longer in 'startup' mode.\n",
			domain->name ));
	...
	if (get_global_winbindd_state_offline()) {
		DEBUG10(("check_domain_online_handler: domain %s remaining globally offline\n",
			domain->name ));
		return;
	}
...
{
	DEBUG10(("set_domain_offline: called for domain %s\n",
		domain->name ));

	TALLOC_FREE(domain->check_online_event);

	if (domain->internal) {
		DEBUG3(("set_domain_offline: domain %s is internal - logic error.\n",
			domain->name ));
		return;
	}
	...
	if (get_global_winbindd_state_offline()) {
		DEBUG10(("set_domain_offline: domain %s remaining globally offline\n",
			domain->name ));
		return;
	}
	...
	DEBUG10(("set_domain_offline: added event handler for domain %s\n",
		domain->name ));
}
...
{
	DEBUG10(("set_domain_online: called for domain %s\n",
		domain->name ));

	if (domain->internal) {
		DEBUG3(("set_domain_online: domain %s is internal - logic error.\n",
			domain->name ));
		return;
	}

	if (get_global_winbindd_state_offline()) {
		DEBUG10(("set_domain_online: domain %s remaining globally offline\n",
			domain->name ));
		return;
	}
...
void set_domain_online_request(struct winbindd_domain *domain)
{
	...
	DEBUG10(("set_domain_online_request: called for domain %s\n",
		domain->name ));

	if (get_global_winbindd_state_offline()) {
		DEBUG10(("set_domain_online_request: domain %s remaining globally offline\n",
			domain->name ));
		return;
	}
	...
	if (!domain->check_online_event) {
		...
		DEBUG10(("set_domain_online_request: domain %s was globally offline.\n",
			domain->name ));
...
static void cm_get_ipc_userpass(char **username, char **domain, char **password)
{
	...
	*password = (char *)secrets_fetch(SECRETS_AUTH_PASSWORD, NULL);
	...
	if (*username && **username) {
		...
		DEBUG3( ("cm_get_ipc_userpass: Retrieved auth-user from secrets.tdb [%s\\%s]\n", 
			  *domain, *username));

	} else {
		DEBUG3( ("cm_get_ipc_userpass: No auth-user defined\n"));
				     ...
				     fstring dcname, struct in_addr *dc_ip)
{
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG10( ("rpccli_netlogon_getanydcname failed: %s\n",
			   dos_errstr(werr)));
		return False;
	}
	...
	DEBUG10( ("rpccli_netlogon_getanydcname returned %s\n", dcname));
				      ...
				      BOOL *retry)
{
	...
	DEBUG10(("cm_prepare_connection: connecting to DC %s for domain %s\n",
	...
	*retry = True;
	...
	if (!got_mutex) {
		DEBUG0(("cm_prepare_connection: mutex grab failed for %s\n",
			 controller));
	...
	if ((*cli = cli_initialise()) == NULL) {
		DEBUG1( ("Could not cli_initialize\n"));
	...
	{
		DEBUG0(("cm_prepare_connection: %s\n", strerror(errno)));
	...
	if (ntohs(peeraddr_in->sin_port) == 139) {
		...
		if (!cli_session_request(*cli, &calling, &called)) {
			DEBUG8( ("cli_session_request failed for %s\n",
				  controller));
			result = NT_STATUS_UNSUCCESSFUL;
			goto done;
		}
	    ...
	    (*cli)->capabilities & CAP_EXTENDED_SECURITY)
	{
		...
		if (lp_security() == SEC_ADS) {
			...
			(*cli)->use_kerberos = True;
			DEBUG5( ("connecting to %s from %s with kerberos principal "
			...
			if (!ADS_ERR_OK(ads_status)) {
				DEBUG4(("failed kerberos session setup with %s\n",
		...
		DEBUG5( ("connecting to %s from %s with username "
			  "[%s]\\[%s]\n",  controller, global_myname(),
		...
		ads_status = cli_session_setup_spnego(*cli,
						      ...
						      NULL);
		if (!ADS_ERR_OK(ads_status)) {
			DEBUG4( ("authenticated session setup failed with %s\n",
				ads_errstr(ads_status)));
		}
	...
	if ((((*cli)->sec_mode & NEGOTIATE_SECURITY_CHALLENGE_RESPONSE) != 0) &&
		...
		if (NT_STATUS_IS_OK(cli_session_setup(
			...
			DEBUG4( ("authenticated session setup with user %s\\%s failed.\n",
				ipc_domain, ipc_username ));
		}
	}
			      ...
			      struct dc_name_ip **dcs, int *num)
{
	if (!NT_STATUS_IS_OK(check_negative_conn_cache(domain_name, dcname))) {
		DEBUG10( ("DC %s was in the negative conn cache\n", dcname));
		return False;
	}
				   ...
				   fstring dc_name)
{
	...
	if (packet == NULL) {
		DEBUG5( ("Did not receive packet for %s\n", my_mailslot));
		return False;
	}

	DEBUG5( ("Received packet for %s\n", my_mailslot));
	...
	if (len < 70) {
		...
		DEBUG3( ("GetDC got short response\n"));
		return False;
	}
	...
	if (CVAL(p,0) != SAMLOGON_R) {
		DEBUG8( ("GetDC got invalid response type %d\n", CVAL(p, 0)));
		return False;
	}
	...
	if (!strequal(domain, domain_name)) {
		DEBUG3( ("GetDC: Expected domain %s, got %s\n",
			  domain_name, domain));
		return False;
	}
	...
	DEBUG10( ("GetDC gave name %s for domain %s\n",
		   dc_name, domain));
...
static BOOL dcip_to_name(const struct winbindd_domain *domain, struct in_addr ip, fstring name )
{
	...
	if (lp_security() == SEC_ADS) {
		...
		if (ads_try_connect( ads, inet_ntoa(ip) ) )  {
			...
			DEBUG10(("dcip_to_name: flags = 0x%x\n", (unsigned int)ads->config.flags));
		    ...
		    struct dc_name_ip **dcs, int *num_dcs)
{
	...
	{
		DEBUG10( ("Retrieved DC %s at %s via netlogon\n",
			   dcname, inet_ntoa(ip)));
		return True;
	}
			...
			fstring dcname, struct sockaddr_in *addr, int *fd)
{
 ...
 again:
	...
	{
		for (i=0; i<num_dcs; i++) {
			DEBUG10( ("find_new_dc: open_any_socket_out failed for "
				   ...
				   struct winbindd_cm_conn *new_conn)
{
	...
	if ( saf_servername && NT_STATUS_IS_OK(check_negative_conn_cache( domain->name, saf_servername))) {
		...
		DEBUG10(("cm_open_connection: saf_servername is '%s' for domain %s\n",
	...
	for (retries = 0; retries < 3; retries++) {
		...
		DEBUG10(("cm_open_connection: dcname is '%s' for domain %s\n",
...
{
	if (domain->conn.cli == NULL) {
		DEBUG8( ("connection_ok: Connection to %s for domain %s has NULL "
			  "cli!\n", domain->dcname, domain->name));
		return False;
	}

	if (!domain->conn.cli->initialised) {
		DEBUG3( ("connection_ok: Connection to %s for domain %s was never "
			  "initialised!\n", domain->dcname, domain->name));
		return False;
	}

	if (domain->conn.cli->fd == -1) {
		DEBUG3( ("connection_ok: Connection to %s for domain %s has died or was "
			  "never started (fd == -1)\n", 
			  domain->dcname, domain->name));
		return False;
	}
	...
	if (domain->online == False) {
		DEBUG3( ("connection_ok: Domain %s is offline\n", domain->name));
		return False;
	}
...
static void set_dc_type_and_flags( struct winbindd_domain *domain )
{
	...
	DEBUG5( ("set_dc_type_and_flags: domain %s\n", domain->name ));

	cli = cli_rpc_pipe_open_noauth(domain->conn.cli, PI_LSARPC_DS,
				       &result);

	if (cli == NULL) {
		DEBUG5( ("set_dc_type_and_flags: Could not bind to "
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG5( ("set_dc_type_and_flags: rpccli_ds_getprimarydominfo "
...
no_lsarpc_ds:
	...
	mem_ctx = talloc_init("set_dc_type_and_flags on domain %s\n",
			      domain->name);
	if (!mem_ctx) {
		DEBUG1( ("set_dc_type_and_flags: talloc_init() failed\n"));
		cli_rpc_pipe_close(cli);
		return;
	}
	...
	DEBUG5( ("set_dc_type_and_flags: domain %s is %sin native mode.\n",
		  domain->name, domain->native_mode ? "" : "NOT "));

	DEBUG5(("set_dc_type_and_flags: domain %s is %srunning active directory.\n",
			...
			struct rpc_pipe_client **cli, POLICY_HND *sam_handle)
{
	    ...
	    (conn_pwd[0] == '\0'))
	{
		...
		if (!NT_STATUS_IS_OK(result)) {
			DEBUG10( ("cm_connect_sam: No no user available for "
	...
	if (conn->samr_pipe == NULL) {
		DEBUG10(("cm_connect_sam: failed to connect to SAMR "
	...
	DEBUG10(("cm_connect_sam: connected to SAMR pipe for "
	...
	DEBUG10(("cm_connect_sam: ntlmssp-sealed rpccli_samr_connect "
 ...
 schannel:
	...
	if (!cm_get_schannel_dcinfo(domain, &p_dcinfo)) {
		...
		DEBUG10( ("cm_connect_sam: Could not get schannel auth info "
	...
	if (conn->samr_pipe == NULL) {
		DEBUG10(("cm_connect_sam: failed to connect to SAMR pipe for "
			  "domain %s using schannel. Error was %s\n",
			  domain->name, nt_errstr(result) ));
		goto anonymous;
	}
	DEBUG10(("cm_connect_sam: connected to SAMR pipe for domain %s using "
		  "schannel.\n", domain->name ));
	...
	DEBUG10(("cm_connect_sam: schannel-sealed rpccli_samr_connect failed "
 ...
 anonymous:
	...
	result = rpccli_samr_connect(conn->samr_pipe, mem_ctx,
				     ...
				     &conn->sam_connect_handle);
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG10(("cm_connect_sam: rpccli_samr_connect failed "
			...
			struct rpc_pipe_client **cli, POLICY_HND *lsa_policy)
{
	...
	if ((conn->cli->user_name[0] == '\0') ||
	    (conn->cli->domain[0] == '\0') || 
	    (conn_pwd[0] == '\0')) {
		DEBUG10( ("cm_connect_lsa: No no user available for "
			   "domain %s, trying schannel\n", conn->cli->domain));
		goto schannel;
	}
	...
	if (conn->lsa_pipe == NULL) {
		DEBUG10(("cm_connect_lsa: failed to connect to LSA pipe for "
	...
	DEBUG10(("cm_connect_lsa: connected to LSA pipe for domain %s using "
	...
	DEBUG10(("cm_connect_lsa: rpccli_lsa_open_policy failed, trying "
		  "schannel\n"));
 ...
 schannel:
	...
	if (!cm_get_schannel_dcinfo(domain, &p_dcinfo)) {
		/* If this call fails - conn->cli can now be NULL ! */
		DEBUG10( ("cm_connect_lsa: Could not get schannel auth info "
			   "for domain %s, trying anon\n", domain->name));
		goto anonymous;
	}
			     ...
			     struct rpc_pipe_client **cli)
{
	...
	*cli = NULL;
	...
	if ((lp_client_schannel() == True) &&
		...
		DEBUG3( ("Server did not offer schannel\n"));
		cli_rpc_pipe_close(netlogon_pipe);
 ...
 no_schannel:
	...
	*/
	...
	if (conn->netlogon_pipe == NULL) {
		DEBUG3( ("Could not open schannel'ed NETLOGON pipe. Error "
			  "was %s\n", nt_errstr(result)));
