				 ...
				 char **dn )
{
	...
	if ( tmp ) {
		...
		if (!secret) {
			DEBUG0( ("get_credentials: Unable to fetch "
		...
		*dn = talloc_strdup(mem_ctx, tmp);
		...
		if (!fetch_ldap_pw(&user_dn, &secret)) {
			DEBUG2( ("get_credentials: Failed to lookup ldap "
...
static NTSTATUS verify_idpool(void)
{
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG1( ("Unable to verify the idpool, "
	...
	if ( count > 1 ) {
		DEBUG0(("Multiple entries returned from %s (base == %s)\n",
...
static NTSTATUS idmap_ldap_alloc_init(const char *params)
{
	...
	if (range && range[0]) {
		...
		if (sscanf(range, "%u - %u", &low_id, &high_id) == 2) {
			if (low_id < high_id) {
				...
				DEBUG1( ("ERROR: invalid idmap alloc range "
					  "[%s]", range));
			}
		} else {
			DEBUG1( ("ERROR: invalid syntax for idmap alloc "
	...
	if (params && *params) {
		...
		if ( ! tmp) {
			DEBUG1( ("ERROR: missing idmap ldap url\n"));
			ret = NT_STATUS_UNSUCCESSFUL;
			goto done;
		}
	...
	if ( ! tmp || ! *tmp) {
		...
		if ( ! tmp) {
			DEBUG1( ("ERROR: missing idmap ldap suffix\n"));
			ret = NT_STATUS_UNSUCCESSFUL;
			goto done;
		}
	}
	...
	if (!NT_STATUS_IS_OK(ret)) {
		DEBUG1( ("ERROR: smbldap_init (%s) failed!\n",
	...
	if ( !NT_STATUS_IS_OK(ret) ) {
		DEBUG1(("idmap_ldap_alloc_init: Failed to get connection "
			 "credentials (%s)\n", nt_errstr(ret)));
		goto done;
	}
...
static NTSTATUS idmap_ldap_allocate_id(struct unixid *xid)
{
	...
	DEBUG10( ("Search of the id pool (filter: %s)\n", filter));
	...
	count = ldap_count_entries(idmap_alloc_ldap->smbldap_state->ldap_struct,
				   result);
	if (count != 1) {
		DEBUG0(("Single %s object not found\n", LDAP_OBJ_IDPOOL));
		goto done;
	}
	...
	case ID_TYPE_UID:
		if (xid->id > idmap_alloc_ldap->high_uid) {
			DEBUG0(("Cannot allocate uid above %lu!\n",
	...
	case ID_TYPE_GID:
		if (xid->id > idmap_alloc_ldap->high_gid) {
			DEBUG0(("Cannot allocate gid above %lu!\n",
				 (unsigned long)idmap_alloc_ldap->high_uid));
			goto done;
		}
		break;
	...
	if (mods == NULL) {
		DEBUG0(("smbldap_set_mod() failed.\n"));
		goto done;
	}

	DEBUG10( ("Try to atomically increment the id (%s -> %s)\n",
		   id_str, new_id_str));
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG1(("Failed to allocate new %s. "
...
static NTSTATUS idmap_ldap_db_init(struct idmap_domain *dom)
{
	...
	if (range && range[0]) {
		if ((sscanf(range, "%u - %u", &ctx->filter_low_id,
			...
			DEBUG1( ("ERROR: invalid filter range [%s]", range));
			ctx->filter_low_id = 0;
	...
	if (!NT_STATUS_IS_OK(ret)) {
        ...
        ret = get_credentials( ctx, ctx->smbldap_state, config_option,
			       dom, &ctx->user_dn );
	if ( !NT_STATUS_IS_OK(ret) ) {
		DEBUG1(("idmap_ldap_db_init: Failed to get connection "
			 "credentials (%s)\n", nt_errstr(ret)));
		goto done;
	}
...
again:
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG3(("Failure looking up ids (%s)\n", ldap_err2string(rc)));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	for (i = 0; i < count; i++) {
		...
		if ( ! entry) {
			DEBUG2( ("ERROR: Unable to fetch ldap entries "
				  "from results\n"));
			break;
		}
		...
		if ( ! sidstr) { /* no sid, skip entry */
			DEBUG2( ("WARNING SID not found on entry\n"));
			continue;
		}
		...
		if ( ! tmp) { /* wow very strange entry, how did it match ? */
			DEBUG5( ("Unprobable match on (%s), no uidNumber, "
		...
		if ((id == 0) ||
			...
			DEBUG5( ("Requested id (%u) out of range (%u - %u). "
				  "Filtered!\n", id,
				  ctx->filter_low_id, ctx->filter_high_id));
			TALLOC_FREE(sidstr);
			TALLOC_FREE(tmp);
		...
		if (!map) {
			DEBUG2( ("WARNING: couldn't match sid (%s) "
				  "with requested ids\n", sidstr));
			TALLOC_FREE(sidstr);
			continue;
		}
		...
		if ( ! string_to_sid(map->sid, sidstr)) {
			DEBUG2( ("ERROR: Invalid SID on entry\n"));
			TALLOC_FREE(sidstr);
			continue;
		}
		TALLOC_FREE(sidstr);
...
again:
	...
	if (rc != LDAP_SUCCESS) {
		DEBUG3(("Failure looking up sids (%s)\n",
			 ldap_err2string(rc)));
		ret = NT_STATUS_UNSUCCESSFUL;
		goto done;
	}
	...
	for (i = 0; i < count; i++) {
		...
		if (!map) {
			DEBUG2( ("WARNING: couldn't find entry sid (%s) "
				  "in ids", sidstr));
			TALLOC_FREE(sidstr);
			continue;
		}
				       ...
				       const struct id_map *map)
{
	...
	if (rc != LDAP_SUCCESS) {
		...
		DEBUG0(("ldap_set_mapping_internals: Failed to add %s to %lu "
			 "mapping [%s]\n", sid,
			 (unsigned long)map->xid.id, type));
		DEBUG0( ("ldap_set_mapping_internals: Error was: %s (%s)\n",
	...
	DEBUG10(("ldap_set_mapping: Successfully created mapping from %s to "
