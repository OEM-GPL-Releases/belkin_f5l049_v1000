...
static int convert_fn(TDB_CONTEXT *tdb, TDB_DATA key, TDB_DATA data, void *state)
{
	...
	*p++ = '/';
	...
	if (domain == NULL) {
		...
		DEBUG0(("Unable to find domain %s\n", dom_name ));
		DEBUG0(("deleting record %s\n", key.dptr ));

		if (tdb_delete(tdb, key) != 0) {
			DEBUG0( ("Unable to delete record %s\n", key.dptr));
			*failed = True;
			return -1;
		}
	...
	if (tdb_store(tdb, key2, data, TDB_INSERT) != 0) {
		DEBUG0(("Unable to add record %s\n", key2.dptr ));
		*failed = True;
		return -1;
	}

	if (tdb_store(tdb, data, key2, TDB_REPLACE) != 0) {
		DEBUG0(("Unable to update record %s\n", data.dptr ));
		*failed = True;
		return -1;
	}
...
static BOOL idmap_tdb_convert(const char *idmap_name)
{
	...
	if (!(idmap_tdb = tdb_open_log(idmap_name, 0,
					TDB_DEFAULT, O_RDWR,
					0600))) {
		DEBUG0( ("Unable to open idmap database\n"));
		return False;
	}
	...
	if (((vers == -1) && bigendianheader) || (IREV(vers) == IDMAP_VERSION)) {
		...
		if (tdb_store_int32(idmap_tdb, HWM_USER, wm) == -1) {
			DEBUG0( ("Unable to byteswap user hwm in idmap database\n"));
			tdb_close(idmap_tdb);
			return False;
		}
		...
		if (tdb_store_int32(idmap_tdb, HWM_GROUP, wm) == -1) {
			DEBUG0( ("Unable to byteswap group hwm in idmap database\n"));
			tdb_close(idmap_tdb);
			return False;
		}
	}
	...
	if (failed) {
		DEBUG0( ("Problem during conversion\n"));
		tdb_close(idmap_tdb);
		return False;
	}

	if (tdb_store_int32(idmap_tdb, "IDMAP_VERSION", IDMAP_VERSION) == -1) {
		DEBUG0( ("Unable to dtore idmap version in databse\n"));
		tdb_close(idmap_tdb);
		return False;
	}
...
BOOL idmap_tdb_upgrade(TALLOC_CTX *ctx, const char *tdbfile)
{
	...
	DEBUG0( ("Upgrading winbindd_idmap.tdb from an old version\n"));
	...
	if (backup_tdb(tdbfile, backup_name, 0) != 0) {
		DEBUG0( ("Could not backup idmap database\n"));
...
static NTSTATUS idmap_tdb_open_db(TALLOC_CTX *memctx, TDB_CONTEXT **tdbctx)
{
	...
	if (version != IDMAP_VERSION) {
		...
		if ( ! idmap_tdb_upgrade(ctx, tdbfile)) {
			...
			DEBUG0( ("Unable to open idmap database, it's in an old formati, and upgrade failed!\n"));
...
BOOL idmap_tdb_tdb_close(TDB_CONTEXT *tdbctx)
{
	if (tdbctx != idmap_tdb_common_ctx) {
		DEBUG0( ("ERROR: Invalid tdb context!"));
		return False;
	}
...
static NTSTATUS idmap_tdb_alloc_init( const char *params )
{
	...
	if (range && range[0]) {
		...
		if (sscanf(range, "%u - %u", &low_id, &high_id) == 2) {
			if (low_id < high_id) {
				...
				DEBUG1( ("ERROR: invalid idmap alloc range [%s]", range));
			}
		} else {
			DEBUG1( ("ERROR: invalid syntax for idmap alloc config:range [%s]", range));
		}
	}
	...
	if (idmap_tdb_state.high_uid <= idmap_tdb_state.low_uid) {
		DEBUG1( ("idmap uid range missing or invalid\n"));
		...
		if (((low_id = tdb_fetch_int32(idmap_alloc_tdb, HWM_USER)) == -1) ||
			...
			if (tdb_store_int32(idmap_alloc_tdb, HWM_USER, idmap_tdb_state.low_uid) == -1) {
				DEBUG0( ("Unable to initialise user hwm in idmap database\n"));
	...
	if (idmap_tdb_state.high_gid <= idmap_tdb_state.low_gid) {
		DEBUG1( ("idmap gid range missing or invalid\n"));
		DEBUGADD(1, ("idmap will be unable to map foreign SIDs\n"));
		return NT_STATUS_UNSUCCESSFUL;
	} else {
		uint32 low_id;
		...
		if (((low_id = tdb_fetch_int32(idmap_alloc_tdb, HWM_GROUP)) == -1) ||
			...
			if (tdb_store_int32(idmap_alloc_tdb, HWM_GROUP, idmap_tdb_state.low_gid) == -1) {
				DEBUG0( ("Unable to initialise group hwm in idmap database\n"));
				return NT_STATUS_INTERNAL_DB_ERROR;
			}
		}
	}
...
static NTSTATUS idmap_tdb_allocate_id(struct unixid *xid)
{
	...
	if (hwm > high_hwm) {
		DEBUG1( ("Fatal Error: %s range full!! (max: %lu)\n", 
	...
	if (!ret) {
		DEBUG1( ("Fatal error while fetching a new %s value\n!", hwmtype));
		return NT_STATUS_UNSUCCESSFUL;
	}
...
static NTSTATUS idmap_tdb_get_hwm(struct unixid *xid)
{
	...
	if (hwm >= high_hwm) {
		DEBUG0( ("Warning: %s range full!! (max: %lu)\n", 
			  hwmtype, (unsigned long)high_hwm));
	}
...
static NTSTATUS idmap_tdb_id_to_sid(struct idmap_tdb_context *ctx, struct id_map *map)
{
	...
	if ((ctx->filter_low_id && (map->xid.id < ctx->filter_low_id)) ||
		...
		DEBUG5( ("Requested id (%u) out of range (%u - %u). Filtered!\n",
	...
	default:
		DEBUG2( ("INVALID unix ID type: 0x02%x\n", map->xid.type));
		return NT_STATUS_INVALID_PARAMETER;
	}
	...
	DEBUG10(("Fetching record %s\n", key.dptr));

	/* Check if the mapping exists */
	data = tdb_fetch(ctx->tdb, key);

	if (!data.dptr) {
		DEBUG10(("Record %s not found\n", key.dptr));
		ret = NT_STATUS_NONE_MAPPED;
		goto done;
	}
		
	if (!string_to_sid(map->sid, data.dptr)) {
		DEBUG10(("INVALID SID (%s) in record %s\n",
				data.dptr, key.dptr));
		ret = NT_STATUS_INTERNAL_DB_ERROR;
		goto done;
	}

	DEBUG10(("Found record %s -> %s\n", key.dptr, data.dptr));
	ret = NT_STATUS_OK;
...
static NTSTATUS idmap_tdb_sid_to_id(struct idmap_tdb_context *ctx, struct id_map *map)
{
	...
	if (sscanf(data.dptr, "UID %lu", &rec_id) == 1) { /* Try a UID record. */
		...
		DEBUG10(("Found uid record %s -> %s \n", key.dptr, data.dptr ));
		ret = NT_STATUS_OK;

	} else if (sscanf(data.dptr, "GID %lu", &rec_id) == 1) { /* Try a GID record. */
		map->xid.id = rec_id;
		map->xid.type = ID_TYPE_GID;
		DEBUG10(("Found gid record %s -> %s \n", key.dptr, data.dptr ));
		ret = NT_STATUS_OK;

	} else { /* Unknown record type ! */
		DEBUG2( ("Found INVALID record %s -> %s\n", key.dptr, data.dptr));
		ret = NT_STATUS_INTERNAL_DB_ERROR;
	}
...
static NTSTATUS idmap_tdb_set_mapping(struct idmap_domain *dom, const struct id_map *map)
{
	...
	if (tdb_chainlock(ctx->tdb, ksid) != 0) {
		DEBUG10(("Failed to lock record %s. Error %s\n",
	...
	if (data.dptr) {
		DEBUG10( ("Deleting existing mapping %s <-> %s\n", data.dptr, ksid.dptr ));
...
static NTSTATUS idmap_tdb_remove_mapping(struct idmap_domain *dom, const struct id_map *map)
{
	...
	if ((data.dsize != kid.dsize) ||
		...
		DEBUG10(("Specified SID does not map to specified ID\n"));
