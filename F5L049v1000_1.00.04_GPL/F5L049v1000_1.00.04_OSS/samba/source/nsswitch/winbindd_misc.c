...
{
	DEBUG3( ("[%5lu]: check machine account\n",
...
{
	DEBUG3( ("[%5lu]: list trusted domains\n",
		  (unsigned long)state->pid));
							...
							struct winbindd_cli_state *state)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG3( ("winbindd_dual_list_trusted_domains: trusted_domains returned %s\n",
			nt_errstr(result) ));
					     ...
					     struct winbindd_cli_state *state)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG1( ("Can't contact the NETLOGON pipe\n"));
		return WINBINDD_ERROR;
	}
	...
	if (!W_ERROR_IS_OK(werr)) {
		DEBUG5( ("Error requesting DCname: %s\n", dos_errstr(werr)));
		return WINBINDD_ERROR;
	}
...
static void domain_info_init_recv(void *private_data, BOOL success)
{
	...
	DEBUG10( ("Got back from child init: %d\n", success));

	if ((!success) || (!domain->initialized)) {
		DEBUG5( ("Could not init child for domain %s\n",
			  domain->name));
		request_error(state);
		return;
	}
...
{
	DEBUG3( ("[%5lu]: request interface version\n",
		  (unsigned long)state->pid));
...
{
	DEBUG3( ("[%5lu]: request domain name\n", (unsigned long)state->pid));
...
{
	DEBUG3( ("[%5lu]: request netbios name\n",
		  (unsigned long)state->pid));
...
void winbindd_priv_pipe_dir(struct winbindd_cli_state *state)
{
	...
	DEBUG3( ("[%5lu]: request location of privileged pipe\n",
		  (unsigned long)state->pid));
