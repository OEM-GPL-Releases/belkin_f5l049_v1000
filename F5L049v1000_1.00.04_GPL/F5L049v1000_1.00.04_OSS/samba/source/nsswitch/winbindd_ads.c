...
static ADS_STRUCT *ads_cached_connection(struct winbindd_domain *domain)
{
	...
	if (domain->private_data) {
		...
		DEBUG7( ("Current tickets expire in %d seconds (at %d, time is now %d)\n",
		...
		if ( ads->config.realm && (expire > now)) {
			...
			DEBUG7(("Deleting expired krb5 credential cache\n"));
			ads->is_mine = True;
			ads_destroy( &ads );
	...
	if (!ads) {
		DEBUG1(("ads_init for domain %s failed\n", domain->name));
		return NULL;
	}
	...
	if (!ADS_ERR_OK(status) || !ads->config.realm) {
		DEBUG1(("ads_connect for domain %s failed: %s\n", 
			       ...
			       WINBIND_USERINFO **info)
{
	...
	*num_entries = 0;
	...
	if (!ADS_ERR_OK(rc) || !res) {
		DEBUG1(("query_user_list ads_search: %s\n", ads_errstr(rc)));
		goto done;
	}

	count = ads_count_replies(ads, res);
	if (count == 0) {
		DEBUG1(("query_user_list: No users found\n"));
		goto done;
	}
	...
	for (msg = ads_first_entry(ads, res); msg; msg = ads_next_entry(ads, msg)) {
		...
		if (!ads_pull_uint32(ads, msg, "sAMAccountType", &atype) ||
			...
			DEBUG1(("Not a user account? atype=0x%x\n", atype));
			continue;
		}
	...
	DEBUG3(("ads query_user_list gave %d entries\n", (*num_entries)));
				...
				struct acct_info **info)
{
	...
	*num_entries = 0;
	...
	if (!ADS_ERR_OK(rc) || !res) {
		DEBUG1(("enum_dom_groups ads_search: %s\n", ads_errstr(rc)));
		goto done;
	}

	count = ads_count_replies(ads, res);
	if (count == 0) {
		DEBUG1(("enum_dom_groups: No groups found\n"));
		goto done;
	}
	...
	DEBUG3(("ads enum_dom_groups gave %d entries\n", (*num_entries)));
			   ...
			   WINBIND_USERINFO *info)
{
	...
	if (!ADS_ERR_OK(rc) || !msg) {
		DEBUG1(("query_user(sid=%s) ads_search: %s\n",
	...
	if (count != 1) {
		DEBUG1(("query_user(sid=%s): Not found\n",
					 ...
					 size_t *p_num_groups, DOM_SID **user_sids)
{
	...
	DEBUG3(("ads: lookup_usergroups_member\n"));
	...
	if (!(ldap_exp = talloc_asprintf(mem_ctx, "(&(member=%s)(objectCategory=group))", escaped_dn))) {
		DEBUG1(("lookup_usergroups(dn=%s) asprintf failed!\n", user_dn));
	...
	if (!ADS_ERR_OK(rc) || !res) {
		DEBUG1(("lookup_usergroups ads_search member=%s: %s\n", user_dn, ads_errstr(rc)));
	...
	*p_num_groups = num_groups;
	...
	DEBUG3(("ads lookup_usergroups (member) succeeded for dn=%s\n", user_dn));
done:
	if (res) 
		ads_msgfree(ads, res);
					   ...
					   size_t *p_num_groups, DOM_SID **user_sids)
{
	...
	DEBUG3(("ads: lookup_usergroups_memberof\n"));
	...
	if (!ADS_ERR_OK(rc) || !res) {
		DEBUG1(("lookup_usergroups_memberof ads_search member=%s: %s\n", 
			user_dn, ads_errstr(rc)));
		return ads_ntstatus(rc);
	}
	...
	*user_sids = NULL;
	...
	count = ads_pull_sids_from_extendeddn(ads, mem_ctx, res, "memberOf", 
					      ...
					      &group_sids);
	if (count == 0) {
		DEBUG1(("No memberOf for this user?!?\n"));
		status = NT_STATUS_NO_MEMORY;
		goto done;
	}
	...
	*p_num_groups = num_groups;
	...
	DEBUG3(("ads lookup_usergroups (memberof) succeeded for dn=%s\n", user_dn));
done:
				  ...
				  uint32 *p_num_groups, DOM_SID **user_sids)
{
	...
	*p_num_groups = 0;
	...
	if (!ADS_ERR_OK(rc)) {
		...
		DEBUG1(("lookup_usergroups(sid=%s) ads_search tokenGroups: %s\n", 
	...
	if (count != 1) {
		...
		DEBUG1(("lookup_usergroups(sid=%s) ads_search tokenGroups: "
	...
	if (!msg) {
		DEBUG1(("lookup_usergroups(sid=%s) ads_search tokenGroups: NULL msg\n", 
	...
	if (!ads_pull_uint32(ads, msg, "primaryGroupID", &primary_group_rid)) {
		DEBUG1(("%s: No primary group for sid=%s !?\n", 
	...
	*p_num_groups = (uint32)num_groups;
	...
	DEBUG3(("ads lookup_usergroups (tokenGroups) succeeded for sid=%s\n",
		 sid_to_string(sid_string, sid)));
done:
				...
				uint32 **name_types)
{
	...
	DEBUG10(("ads: lookup_groupmem %s sid=%s\n", domain->name, 
	...
	*num_names = 0;
	...
	if (!(ldap_exp = talloc_asprintf(mem_ctx, "(objectSid=%s)",sidstr))) {
		SAFE_FREE(sidstr);
		DEBUG1( ("ads: lookup_groupmem: tallloc_asprintf for ldap_exp failed!\n"));
		status = NT_STATUS_NO_MEMORY;
		goto done;
	}
	SAFE_FREE(sidstr);
	...
	do {
		...
		DEBUG10( ("Searching for attrs[0] = %s, attrs[1] = %s\n", attrs[0], attrs[1]));

		rc = ads_search_retry(ads, &res, ldap_exp, attrs);

		if (!ADS_ERR_OK(rc) || !res) {
			DEBUG1(("ads: lookup_groupmem ads_search: %s\n",
				 ads_errstr(rc)));
			status = ads_ntstatus(rc);
			goto done;
		}
		...
		if (num_members == 0) {
			if (!ads_pull_uint32(ads, res, "usnChanged", &first_usn)) {
				DEBUG1( ("ads: lookup_groupmem could not pull usnChanged!\n"));
				goto done;
			}
		}
		...
		if (first_usn != current_usn) {
			DEBUG5( ("ads: lookup_groupmem USN on this record changed"
				  " - restarting search\n"));
			if (num_retries < 5) {
				num_retries++;
				num_members = 0;
	...
	*/
	...
	status = NT_STATUS_OK;
	DEBUG3(("ads lookup_groupmem for sid=%s\n", sid_to_string(sid_string, group_sid)));
done:
...
static NTSTATUS sequence_number(struct winbindd_domain *domain, uint32 *seq)
{
	...
	DEBUG3(("ads: fetch sequence_number for %s\n", domain->name));
				...
				DOM_SID **dom_sids)
{
	...
	*dom_sids    = NULL;
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG5( ("trusted_domains: Could not open a connection to %s "
			  "for PIPE_NETLOGON (%s)\n", 
	...
	if ( NT_STATUS_IS_OK(result) && count) {
		...
		if ( !(*names = TALLOC_ARRAY(mem_ctx, char *, count)) ) {
			DEBUG0( ("trusted_domains: out of memory\n"));
