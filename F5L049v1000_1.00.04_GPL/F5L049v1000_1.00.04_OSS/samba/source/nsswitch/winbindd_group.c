...
static void add_expanded_sid(const DOM_SID *sid, char **pp_members, size_t *p_num_members)
{
	...
	if (domain == NULL) {
		DEBUG3( ("Could not find domain for sid %s\n",
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG3( ("sid_to_name failed for sid %s\n",
			  sid_string_static(sid)));
		goto done;
	}

	DEBUG10( ("Found name %s, type %d\n", name, type));
	...
	if (type != SID_NAME_DOM_GRP) {
		DEBUG10( ("Alias member %s neither user nor group, ignore\n",
			   name));
		goto done;
	}
	...
	if (domain == NULL) {
		DEBUG3( ("Could not find domain from SID %s\n",
			  sid_string_static(sid)));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG10( ("Could not lookup group members for %s: %s\n",
			   name, nt_errstr(result)));
		goto done;
	}

	for (i=0; i<num_names; i++) {
		DEBUG10( ("Adding group member SID %s\n",
			   sid_string_static(&sid_mem[i])));

		if (types[i] != SID_NAME_USER) {
			DEBUG1( ("Hmmm. Member %s of group %s is no user. "
				  "Ignoring.\n", names[i], name));
			continue;
		}
			   ...
			   size_t *num_gr_mem, char **gr_mem, size_t *gr_mem_len)
{
	...
	*num_gr_mem = 0;
	...
	{
		DEBUG1( ("SID %s in domain %s isn't a domain group (%d)\n", 
	...
	if (!lp_winbind_enum_users() && group_rid == DOMAIN_GROUP_RID_USERS) {
		...
		DEBUG10(("fill_grent_mem: optimized lookup for sid %s domain %s\n",
			sid_to_string(sid_string, group_sid), domain->name ));
		...
		if (state) {
			...
			if (sys_getpeereid(state->sock, &ret_uid)==0) {
				...
				if (NT_STATUS_IS_OK(status)) {
					...
					DEBUG10(("fill_grent_mem: querying uid %u -> %s\n",
		...
		if (pquerying_user_sid &&
			...
			DEBUG10(("fill_grent_mem: querying user = %s\n",
				sid_to_string(sid_string, pquerying_user_sid) ));
			...
			status = domain->methods->lookup_usergroups(domain,
							...
							&user_sids);
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG1( ("fill_grent_mem: lookup_usergroups failed "
		...
		if (u_in_group) {
			...
			DEBUG10(("fill_grent_mem: sid %s in 'Domain Users' in domain %s\n",
			...
			status = domain->methods->sid_to_name(domain, mem_ctx,
								...
								&type);
			if (!NT_STATUS_IS_OK(status)) {
				DEBUG1( ("could not lookup username for user "
			...
			DEBUG10(("fill_grent_mem: user %s in 'Domain Users' in domain %s\n",
				name, domain->name ));
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG1( ("could not lookup membership for group sid %s in domain %s (error: %s)\n", 
			  sid_to_string(sid_string, group_sid), domain->name, nt_errstr(status)));
		goto done;
	}

	DEBUG10( ("looked up %d names\n", num_names));
 ...
 again:
	...
	*gr_mem_len = buf_len;
	...
	DEBUG10( ("fill_grent_mem returning %d\n", result));
...
void winbindd_getgrnam(struct winbindd_cli_state *state)
{
	...
	if ((domain = find_domain_from_name(name_domain)) == NULL) {
		DEBUG3( ("could not get domain sid for domain %s\n",
			  name_domain));
		request_error(state);
		return;
	}
	...
	if ( lp_winbind_trusted_domains_only() && domain->primary) {
		DEBUG7(("winbindd_getgrnam: My domain -- rejecting "
	...
	if (!winbindd_lookup_sid_by_name(state->mem_ctx, domain, domain->name,
		...
		DEBUG1( ("group %s in domain %s does not exist\n", 
	...
	{
		DEBUG1( ("name '%s' is not a local, domain or builtin "
	...
	if ( !sid_equal( &tmp_sid, &domain->sid ) ) {
		DEBUG3(("winbindd_getgrnam: group %s resolves to a SID in the wrong domain [%s]\n", 
	...
	DEBUG1( ("error converting unix gid to sid\n"));
	request_error(state);
	return;
...
static void getgrgid_got_sid(struct winbindd_cli_state *state, DOM_SID group_sid)
{
	...
	if (!domain) {
		DEBUG1(("Can't find domain from sid\n"));
		request_error(state);
		TALLOC_FREE(group_name);
		TALLOC_FREE(dom_name);
		return;
	...
	if ( !((name_type==SID_NAME_DOM_GRP) ||
	       ...
	       ((name_type==SID_NAME_ALIAS) && domain->internal)) )
	{
		DEBUG1( ("name '%s' is not a local or domain group: %d\n", 
			  group_name, name_type));
		request_error(state);
		TALLOC_FREE(group_name);
		TALLOC_FREE(dom_name);
...
static void getgrgid_recv(void *private_data, BOOL success, const char *sid)
{
	...
	if (success) {
		DEBUG10(("getgrgid_recv: gid %lu has sid %s\n",
	...
	if (pdb_gid_to_sid(state->request.data.gid, &group_sid) &&
	    ...
	    (name_type == SID_NAME_ALIAS)) {
		/* Hey, got an alias */
		DEBUG10(("getgrgid_recv: we have an alias with gid %lu and sid %s\n",
			  (unsigned long)(state->request.data.gid), sid));
		getgrgid_got_sid(state, group_sid);
		return;
	}

	DEBUG1( ("could not convert gid %lu to sid\n", 
...
static BOOL winbindd_setgrent_internal(struct winbindd_cli_state *state)
{
	...
	for (domain = domain_list(); domain != NULL; domain = domain->next) {
		...
		if ((domain_state = SMB_MALLOC_P(struct getent_state)) == NULL) {
			DEBUG1( ("winbindd_setgrent: malloc failed for domain_state!\n"));
			return False;
		}
...
static BOOL get_sam_group_entries(struct getent_state *ent)
{
	...
	if (!(mem_ctx = talloc_init("get_sam_group_entries(%s)",
					  ent->domain_name))) {
		DEBUG1( ("get_sam_group_entries: could not create talloc context!\n")); 
		return False;
	}
	...
	if (!(domain = find_domain_from_name(ent->domain_name))) {
		DEBUG3( ("no such domain %s in get_sam_group_entries\n", ent->domain_name));
		goto done;
	}
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG3( ("get_sam_group_entries: could not enumerate domain groups! Error: %s\n", nt_errstr(status)));
		result = False;
		goto done;
	}
	...
	if (num_entries) {
		if ( !(name_list = SMB_MALLOC_ARRAY(struct acct_info, num_entries)) ) {
			DEBUG0(("get_sam_group_entries: Failed to malloc memory for %d domain groups!\n", 
				num_entries));
			result = False;
			goto done;
		}
	...
	{
		DEBUG4(("get_sam_group_entries: %s domain; enumerating local groups as well\n", 
		...
		if ( !NT_STATUS_IS_OK(status) ) { 
			DEBUG3(("get_sam_group_entries: Failed to enumerate domain local groups!\n"));
			num_entries = 0;
		}
		else
			DEBUG4(("get_sam_group_entries: Returned %d local groups\n", num_entries));
		...
		if ( num_entries ) {
			...
			{
				DEBUG0(("get_sam_group_entries: Failed to realloc more memory for %d local groups!\n", 
					num_entries));
				result = False;
				goto done;
			}
...
void winbindd_getgrent(struct winbindd_cli_state *state)
{
	...
	tryagain:
		...
		DEBUG10( ("entry_index = %d, num_entries = %d\n",
		...
		if (ent->num_sam_entries == ent->sam_entry_index) {
			...
			while(ent && !get_sam_group_entries(ent)) {
				...
				DEBUG10( ("freeing state info for domain %s\n", ent->domain_name)); 
		...
		if (!(domain = 
			...
			DEBUG3( ("No such domain %s in winbindd_getgrent\n", ent->domain_name));
			result = False;
			goto done;
		}
		...
		if (!NT_STATUS_IS_OK(idmap_sid_to_gid(&group_sid, &group_gid))) {
			...
			DEBUG10( ("SID %s not in idmap\n",
				   sid_string_static(&group_sid)));

			if (!pdb_sid_to_id(&group_sid, &id, &type)) {
				DEBUG1( ("could not look up gid for group "
					  "%s\n", 
			...
			if ((type != SID_NAME_DOM_GRP) &&
				...
				DEBUG1( ("Group %s is a %s, not a group\n",
		...
		DEBUG10( ("got gid %lu for group %lu\n", (unsigned long)group_gid,
		...
		if (result) {
			...
			DEBUG10( ("adding group num_entries = %d\n",
			...
			DEBUG0( ("could not lookup domain group %s\n", 
				  domain_group_name));
		}
	}
	...
	DEBUG10( ("returning %d groups, length = %d\n",
...
void winbindd_list_groups(struct winbindd_cli_state *state)
{
	...
	for (domain = domain_list(); domain; domain = domain->next) {
		...
		if (!extra_data) {
			DEBUG0(("failed to enlarge buffer!\n"));
			request_error(state);
			return;
		}
...
void winbindd_getgroups(struct winbindd_cli_state *state)
{
	...
	if (!parse_domain_user_talloc(state->mem_ctx,
		...
		DEBUG5( ("Could not parse domain user: %s\n",
			  state->request.data.username));
	...
	if (s->domain == NULL) {
		DEBUG7( ("could not find domain entry for domain %s\n", 
			  s->domname));
		request_error(state);
		return;
	}
	...
	if ( s->domain->primary && lp_winbind_trusted_domains_only()) {
		DEBUG7(("winbindd_getgroups: My domain -- rejecting "
			 "getgroups() for %s\\%s.\n", s->domname,
			 s->username));
		request_error(state);
		return;
...
void winbindd_getusersids(struct winbindd_cli_state *state)
{
	...
	if (user_sid == NULL) {
		DEBUG1( ("talloc failed\n"));
		request_error(state);
		return;
	}

	if (!string_to_sid(user_sid, state->request.data.sid)) {
		DEBUG1( ("Could not get convert sid %s from string\n",
...
void winbindd_getuserdomgroups(struct winbindd_cli_state *state)
{
	...
	if ((domain = find_domain_from_sid_noinit(&user_sid)) == NULL) {
		DEBUG0(("could not find domain entry for sid %s\n", 
