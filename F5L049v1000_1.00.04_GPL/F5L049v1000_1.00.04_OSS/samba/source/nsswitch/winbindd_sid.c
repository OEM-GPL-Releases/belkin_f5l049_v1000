			    ...
			    const DOM_SID *sid, enum lsa_SidType type)
{
	...
	if (!success) {
		DEBUG5( ("lookupname returned an error\n"));
		request_error(state);
		return;
	}
...
void winbindd_lookuprids(struct winbindd_cli_state *state)
{
	...
	DEBUG10( ("lookup_rids: %s\n", state->request.data.sid));

	if (!string_to_sid(&domain_sid, state->request.data.sid)) {
		DEBUG5( ("Could not convert %s to SID\n",
			  state->request.data.sid));
		request_error(state);
		return;
	}
	...
	if (domain == NULL) {
		DEBUG10( ("Could not find domain for name %s\n",
...
static void sid2uid_recv(void *private_data, BOOL success, uid_t uid)
{
	...
	if (!success) {
		DEBUG5( ("Could not convert sid %s\n",
			  state->request.data.sid));
		request_error(state);
		return;
	}
				    ...
				    enum lsa_SidType type)
{
	...
	if (!success) {
		DEBUG5( ("sid2uid_lookupsid_recv Could not convert get sid type for %s\n",
			  state->request.data.sid));
		request_error(state);
		return;
	}
	...
	if ( (type!=SID_NAME_USER) && (type!=SID_NAME_COMPUTER) ) {
		DEBUG5(("sid2uid_lookupsid_recv: Sid %s is not a user or a computer.\n", 
			 state->request.data.sid));
		request_error(state);
		return;		
	}
	...
	if (!string_to_sid(&sid, state->request.data.sid)) {
		DEBUG1( ("sid2uid_lookupsid_recv: Could not get convert sid %s from string\n",
			  state->request.data.sid));
		request_error(state);
		return;
	}
				    ...
				    enum lsa_SidType type)
{
	...
	if (!success) {
		DEBUG5( ("sid2gid_lookupsid_recv: Could not convert get sid type for %s\n",
			  state->request.data.sid));
		request_error(state);
		return;
	}
	...
	{
		DEBUG5(("sid2gid_lookupsid_recv: Sid %s is not a group.\n", 
			 state->request.data.sid));
		request_error(state);
		return;		
	}
	...
	if (!string_to_sid(&sid, state->request.data.sid)) {
		DEBUG1( ("sid2gid_lookupsid_recv: Could not get convert sid %s from string\n",
			  state->request.data.sid));
		request_error(state);
		return;
	}
...
static void sids2xids_recv(void *private_data, BOOL success, void *data, int len)
{
	...
	if (!success) {
		DEBUG5( ("Could not convert sids to xids\n"));
		request_error(state);
		return;
	}
...
static void set_mapping_recv(void *private_data, BOOL success)
{
	...
	if (!success) {
		DEBUG5( ("Could not set sid mapping\n"));
		request_error(state);
		return;
	}
...
void winbindd_set_mapping(struct winbindd_cli_state *state)
{
	...
	DEBUG3( ("[%5lu]: set id map\n", (unsigned long)state->pid));

	if ( ! state->privileged) {
		DEBUG0( ("Only root is allowed to set mappings!\n"));
		request_error(state);
		return;
	}
...
static void uid2sid_recv(void *private_data, BOOL success, const char *sid)
{
	...
	if (success) {
		DEBUG10(("uid2sid: uid %lu has sid %s\n",
...
static void gid2sid_recv(void *private_data, BOOL success, const char *sid)
{
	...
	if (success) {
		DEBUG10(("gid2sid: gid %lu has sid %s\n",
...
{
	if ( !state->privileged ) {
		DEBUG2( ("winbindd_allocate_uid: non-privileged access "
			  "denied!\n"));
		request_error(state);
		return;
	}
...
{
	if ( !state->privileged ) {
		DEBUG2( ("winbindd_allocate_gid: non-privileged access "
			  "denied!\n"));
		request_error(state);
		return;
	}
...
{
	if ( ! state->privileged) {
		DEBUG0( ("Only root is allowed to ask for an idmap dump!\n"));
		request_error(state);
		return;
	}

	DEBUG3( ("[%5lu]: dump maps\n", (unsigned long)state->pid));
