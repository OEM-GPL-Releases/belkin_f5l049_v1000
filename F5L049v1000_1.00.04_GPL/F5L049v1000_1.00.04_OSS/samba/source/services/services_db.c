...
static BOOL read_init_file( const char *servicename, struct rcinit_file_information **service_info )
{
	...
	if ( !(f = x_fopen( filepath, O_RDONLY, 0 )) ) {
		DEBUG0(("read_init_file: failed to open [%s]\n", filepath));
		TALLOC_FREE(info);
		return False;
	}
                              ...
                              const char *name )
{
	...
	if ( !W_ERROR_IS_OK(wresult) ) {
		DEBUG0(("add_new_svc_name: key lookup failed! [%s] (%s)\n", 
	...
	if ( !(svc_subkeys = TALLOC_ZERO_P( key_service, REGSUBKEY_CTR )) ) {
		DEBUG0(("add_new_svc_name: talloc() failed!\n"));
	...
	if ( !(sd = construct_service_sd(key_secdesc)) ) {
		DEBUG0(("add_new_svc_name: Failed to create default sec_desc!\n"));
		regkey_close_internal( key_secdesc );
		return;
	}
...
void svcctl_init_keys( void )
{
	...
	if ( !W_ERROR_IS_OK(wresult) ) {
		DEBUG0(("svcctl_init_keys: key lookup failed! (%s)\n", 
			dos_errstr(wresult)));
		return;
	}
	...
	if ( !(subkeys = TALLOC_ZERO_P( key, REGSUBKEY_CTR )) ) {
		DEBUG0(("svcctl_init_keys: talloc() failed!\n"));
...
SEC_DESC* svcctl_get_secdesc( TALLOC_CTX *ctx, const char *name, NT_USER_TOKEN *token )
{
	...
	if ( !W_ERROR_IS_OK(wresult) ) {
		DEBUG0(("svcctl_get_secdesc: key lookup failed! [%s] (%s)\n", 
			path, dos_errstr(wresult)));
		return NULL;
	}
	...
	if ( !(val = regval_ctr_getvalue( values, "Security" )) ) {
		DEBUG6(("svcctl_get_secdesc: constructing default secdesc for service [%s]\n", 
			name));
		regkey_close_internal( key );
...
char* svcctl_lookup_dispname( const char *name, NT_USER_TOKEN *token )
{
	...
	if ( !W_ERROR_IS_OK(wresult) ) {
		DEBUG0(("svcctl_lookup_dispname: key lookup failed! [%s] (%s)\n", 
			path, dos_errstr(wresult)));
		goto fail;
	}

	if ( !(values = TALLOC_ZERO_P( key, REGVAL_CTR )) ) {
		DEBUG0(("svcctl_lookup_dispname: talloc() failed!\n"));
		regkey_close_internal( key );
		goto fail;
	}
...
REGVAL_CTR* svcctl_fetch_regvalues( const char *name, NT_USER_TOKEN *token )
{
	...
	wresult = regkey_open_internal( &key, path, token, REG_KEY_READ );
	if ( !W_ERROR_IS_OK(wresult) ) {
		DEBUG0(("svcctl_fetch_regvalues: key lookup failed! [%s] (%s)\n", 
			path, dos_errstr(wresult)));
		return NULL;
	}

	if ( !(values = TALLOC_ZERO_P( NULL, REGVAL_CTR )) ) {
		DEBUG0(("svcctl_fetch_regvalues: talloc() failed!\n"));
		regkey_close_internal( key );
		return NULL;
	}
