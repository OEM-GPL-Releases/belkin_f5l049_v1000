...
static int pipe_enum_fn( TDB_CONTEXT *the_tdb, TDB_DATA kbuf, TDB_DATA dbuf, void *p)
{
	...
	if ( process_exists(prec.pid) ) {
		...
		if ( !f ) {
			DEBUG0(("conn_enum_fn: realloc failed for %d items\n", i+1));
			return 1;
		}
		fenum->info = f;
                              ...
                              uint32 *count, uint32 resume )
{
	...
	if ( !conn_tdb ) {
		DEBUG0(("net_enum_pipes: Failed to retrieve the connections tdb handle!\n"));
	...
	if (tdb_traverse(conn_tdb, pipe_enum_fn, &fenum) == -1) {
		DEBUG0(("net_enum_pipes: traverse of connections.tdb failed with error %s.\n",
	       ...
	       uint32 info_level, uint32 *resume_hnd, uint32 *total_entries, BOOL all_shares)
{
	...
	DEBUG5(("init_srv_share_info_ctr\n"));
	...
	*total_entries = num_entries;
	...
	default:
		DEBUG5(("init_srv_share_info_ctr: unsupported switch value %d\n", info_level));
		return False;
	}
...
{
	DEBUG5(("init_srv_r_net_share_enum: %d\n", __LINE__));
				  ...
				  char *share_name, uint32 info_level)
{
	...
	DEBUG5(("init_srv_r_net_share_get_info: %d\n", __LINE__));
	...
	if (snum >= 0) {
		...
		case 1501:
			...
			break;
		default:
			DEBUG5(("init_srv_net_share_get_info: unsupported switch value %d\n", info_level));
...
static void init_srv_sess_info_1(SRV_SESS_INFO_1 *ss1, uint32 *snum, uint32 *stot)
{
	...
	for (; (*snum) < (*stot) && num_entries < MAX_SESS_ENTRIES; (*snum)++) {
		...
		if ( !pw ) {
			DEBUG10(("init_srv_sess_info_1: failed to find owner: %s\n",
...
{
	WERROR status = WERR_OK;
	DEBUG5(("init_srv_sess_info_ctr: %d\n", __LINE__));
	...
	case 1:
		...
		break;
	default:
		DEBUG5(("init_srv_sess_info_ctr: unsupported switch value %d\n", switch_value));
		(*resume_hnd) = 0;
		(*total_entries) = 0;
		ctr->ptr_sess_ctr = 0;
		status = WERR_UNKNOWN_LEVEL;
...
{
	DEBUG5(("init_srv_r_net_sess_enum: %d\n", __LINE__));
...
{
	WERROR status = WERR_OK;
	DEBUG5(("init_srv_conn_info_ctr: %d\n", __LINE__));
	...
	case 1:
		...
		break;
	default:
		DEBUG5(("init_srv_conn_info_ctr: unsupported switch value %d\n", switch_value));
		(*resume_hnd = 0);
		(*total_entries) = 0;
		ctr->ptr_conn_ctr = 0;
		status = WERR_UNKNOWN_LEVEL;
...
{
	DEBUG5(("init_srv_r_net_conn_enum: %d\n", __LINE__));
...
WERROR _srv_net_srv_get_info(pipes_struct *p, SRV_Q_NET_SRV_GET_INFO *q_u, SRV_R_NET_SRV_GET_INFO *r_u)
{
	...
	DEBUG5(("srv_net_srv_get_info: %d\n", __LINE__));

	if (!pipe_access_check(p)) {
		DEBUG3( ("access denied to srv_net_srv_get_info\n"));
		return WERR_ACCESS_DENIED;
	}
...
{
	DEBUG5(("_srv_net_share_enum: %d\n", __LINE__));

	if (!pipe_access_check(p)) {
		DEBUG3( ("access denied to srv_net_share_enum_all\n"));
		return WERR_ACCESS_DENIED;
	}
...
WERROR _srv_net_share_enum(pipes_struct *p, SRV_Q_NET_SHARE_ENUM *q_u, SRV_R_NET_SHARE_ENUM *r_u)
{
	...
	if (!pipe_access_check(p)) {
		DEBUG3( ("access denied to srv_net_share_enum\n"));
		return WERR_ACCESS_DENIED;
	}
...
WERROR _srv_net_share_get_info(pipes_struct *p, SRV_Q_NET_SHARE_GET_INFO *q_u, SRV_R_NET_SHARE_GET_INFO *r_u)
{
	...
	DEBUG5(("_srv_net_share_get_info: %d\n", __LINE__));
...
WERROR _srv_net_share_set_info(pipes_struct *p, SRV_Q_NET_SHARE_SET_INFO *q_u, SRV_R_NET_SHARE_SET_INFO *r_u)
{
	...
	DEBUG5(("_srv_net_share_set_info: %d\n", __LINE__));
	...
	case 1005:
		...
		if (((q_u->info.share.info1005.share_info_flags &
			...
			DEBUG3( ("_srv_net_share_set_info: client is trying to change csc policy from the network; must be done with smb.conf\n"));
			return WERR_ACCESS_DENIED;
		}
	case 1006:
	case 1007:
	...
	case 1501:
		...
		break;
	default:
		DEBUG5(("_srv_net_share_set_info: unsupported switch value %d\n", q_u->info_level));
		return WERR_UNKNOWN_LEVEL;
	}
	...
	DEBUG10(("_srv_net_share_set_info: change share command = %s\n",
		...
		|| (lp_max_connections(snum) != max_connections) ) 
	{
		if (!lp_change_share_cmd() || !*lp_change_share_cmd()) {
			DEBUG10(("_srv_net_share_set_info: No change share command\n"));
			return WERR_ACCESS_DENIED;
		}
		...
		DEBUG10(("_srv_net_share_set_info: Running [%s]\n", command ));
		...
		DEBUG3(("_srv_net_share_set_info: Running [%s] returned (%d)\n", command, ret ));		
	
		if ( ret != 0 )
			return WERR_ACCESS_DENIED;
	} else {
		DEBUG10(("_srv_net_share_set_info: No change to share name (%s)\n", share_name ));
	}
	...
	if (psd) {
		...
		if (old_sd && !sec_desc_equal(old_sd, psd)) {
			if (!set_share_security(share_name, psd))
				DEBUG0(("_srv_net_share_set_info: Failed to change security info in share %s.\n",
					share_name ));
		}
	}
...
WERROR _srv_net_share_add(pipes_struct *p, SRV_Q_NET_SHARE_ADD *q_u, SRV_R_NET_SHARE_ADD *r_u)
{
	...
	if (!lp_add_share_cmd() || !*lp_add_share_cmd()) {
		DEBUG10(("_srv_net_share_add: No add share command\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	case 1501:
		...
		return WERR_ACCESS_DENIED;
	default:
		DEBUG5(("_srv_net_share_add: unsupported switch value %d\n", q_u->info_level));
		return WERR_UNKNOWN_LEVEL;
	}
	...
	if ( !validate_net_name( share_name, INVALID_SHARENAME_CHARS, sizeof(share_name) ) ) {
		DEBUG5(("_srv_net_name_validate: Bad sharename \"%s\"\n", share_name));
	...
	DEBUG10(("_srv_net_share_add: Running [%s]\n", command ));
	...
	DEBUG3(("_srv_net_share_add: Running [%s] returned (%d)\n", command, ret ));
	...
	if (psd) {
		if (!set_share_security(share_name, psd)) {
			DEBUG0(("_srv_net_share_add: Failed to add security info to share %s.\n", share_name ));
		}
	}
...
WERROR _srv_net_share_del(pipes_struct *p, SRV_Q_NET_SHARE_DEL *q_u, SRV_R_NET_SHARE_DEL *r_u)
{
	...
	if (!lp_delete_share_cmd() || !*lp_delete_share_cmd()) {
		DEBUG10(("_srv_net_share_del: No delete share command\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	DEBUG10(("_srv_net_share_del: Running [%s]\n", command ));
	...
	DEBUG3(("_srv_net_share_del: Running [%s] returned (%d)\n", command, ret ));
...
{
	DEBUG5(("_srv_net_share_del_stick: %d\n", __LINE__));
			...
			SRV_R_NET_FILE_QUERY_SECDESC *r_u)
{
	...
	if (conn == NULL) {
		DEBUG3(("_srv_net_file_query_secdesc: Unable to connect to %s\n", qualname));
		r_u->status = ntstatus_to_werror(nt_status);
		goto error_exit;
	}

	if (!become_user(conn, conn->vuid)) {
		DEBUG0(("_srv_net_file_query_secdesc: Can't become connected user!\n"));
		r_u->status = WERR_ACCESS_DENIED;
		goto error_exit;
	}
	became_user = True;
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG3(("_srv_net_file_query_secdesc: bad pathname %s\n", filename));
		r_u->status = WERR_ACCESS_DENIED;
		goto error_exit;
	}
	...
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG3(("_srv_net_file_query_secdesc: can't access %s\n", filename));
		r_u->status = WERR_ACCESS_DENIED;
		goto error_exit;
	}
	...
	if ( !NT_STATUS_IS_OK(nt_status)) {
		...
		if (!NT_STATUS_IS_OK(nt_status)) {
			DEBUG3(("_srv_net_file_query_secdesc: Unable to open file %s\n", filename));
			r_u->status = ntstatus_to_werror(nt_status);
			goto error_exit;
		}
	}
	...
	if (sd_size == 0) {
		DEBUG3(("_srv_net_file_query_secdesc: Unable to get NT ACL for file %s\n", filename));
		r_u->status = WERR_ACCESS_DENIED;
		goto error_exit;
	}
									...
									SRV_R_NET_FILE_SET_SECDESC *r_u)
{
	...
	if (conn == NULL) {
		DEBUG3(("_srv_net_file_set_secdesc: Unable to connect to %s\n", qualname));
		r_u->status = ntstatus_to_werror(nt_status);
		goto error_exit;
	}

	if (!become_user(conn, conn->vuid)) {
		DEBUG0(("_srv_net_file_set_secdesc: Can't become connected user!\n"));
		r_u->status = WERR_ACCESS_DENIED;
		goto error_exit;
	}
	became_user = True;
	...
	nt_status = unix_convert(conn, filename, False, NULL, &st);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG3(("_srv_net_file_set_secdesc: bad pathname %s\n", filename));
		r_u->status = WERR_ACCESS_DENIED;
		goto error_exit;
	}
	...
	nt_status = check_name(conn, filename);
	if (!NT_STATUS_IS_OK(nt_status)) {
		DEBUG3(("_srv_net_file_set_secdesc: can't access %s\n", filename));
		r_u->status = WERR_ACCESS_DENIED;
		goto error_exit;
	}
	...
	if ( !NT_STATUS_IS_OK(nt_status) ) {
		...
		if ( !NT_STATUS_IS_OK(nt_status) ) {
			DEBUG3(("_srv_net_file_set_secdesc: Unable to open file %s\n", filename));
			r_u->status = ntstatus_to_werror(nt_status);
			goto error_exit;
		}
	}
	...
	if (ret == False) {
		DEBUG3(("_srv_net_file_set_secdesc: Unable to set NT ACL on file %s\n", filename));
		r_u->status = WERR_ACCESS_DENIED;
		goto error_exit;
	}
...
static const char *next_server_disk_enum(uint32 *resume)
{
	...
	DEBUG10( ("next_server_disk_enum: reporting disk %s. resume handle %d.\n", disk, *resume));
