...
static BOOL pipe_init_outgoing_data(pipes_struct *p)
{
	...
	if(!prs_init(&o_data->rdata, RPC_MAX_PDU_FRAG_LEN, p->mem_ctx, MARSHALL)) {
		DEBUG0(("pipe_init_outgoing_data: malloc fail.\n"));
		return False;
	}
			      ...
			      connection_struct *conn, uint16 vuid)
{
	...
	DEBUG4(("Open pipe requested %s (pipes_open=%d)\n",
	...
	if (is_spoolss_pipe && current_spoolss_pipes_open >= MAX_OPEN_SPOOLSS_PIPES) {
		DEBUG10(("open_rpc_pipe_p: spooler bug workaround. Denying open on pipe %s\n",
			pipe_name ));
		return NULL;
	}
	...
	if (i == -1) {
		DEBUG0(("ERROR! Out of pipe structures\n"));
		return NULL;
	}
	...
	for (p = Pipes; p; p = p->next) {
		DEBUG5(("open_rpc_pipe_p: name %s pnum=%x\n", p->name, p->pnum));  
	}

	p = SMB_MALLOC_P(smb_np_struct);
	if (!p) {
		DEBUG0(("ERROR! no memory for pipes_struct!\n"));
		return NULL;
	}
	...
	if (p->np_state == NULL) {
		DEBUG0(("open_rpc_pipe_p: make_internal_rpc_pipe_p failed.\n"));
		SAFE_FREE(p);
		return NULL;
	}
	...
	DEBUG4(("Opened pipe %s with handle %x (pipes_open=%d)\n",
	...
	for (p_it = Pipes; p_it; p_it = p_it->next) {
		DEBUG5(("open pipes: name %s pnum=%x\n", p_it->name, p_it->pnum));  
	}
			      ...
			      connection_struct *conn, uint16 vuid)
{
	...
	DEBUG4(("Create pipe requested %s\n", pipe_name));

	if (!vuser && vuid != UID_FIELD_INVALID) {
		DEBUG0(("ERROR! vuid %d did not map to a valid vuser struct!\n", vuid));
		return NULL;
	}
	...
	if ((p->mem_ctx = talloc_init("pipe %s %p", pipe_name, p)) == NULL) {
		DEBUG0(("open_rpc_pipe_p: talloc_init failed.\n"));
		SAFE_FREE(p);
		return NULL;
	}
	...
	if (!init_pipe_handle_list(p, pipe_name)) {
		DEBUG0(("open_rpc_pipe_p: init_pipe_handles failed.\n"));
		talloc_destroy(p->mem_ctx);
	...
	if(!prs_init(&p->in_data.data, RPC_MAX_PDU_FRAG_LEN, p->mem_ctx, MARSHALL)) {
		DEBUG0(("open_rpc_pipe_p: malloc fail for in_data struct.\n"));
		talloc_destroy(p->mem_ctx);
		talloc_destroy(p->pipe_state_mem_ctx);
	...
	DEBUG4(("Created internal pipe %s (pipes_open=%d)\n",
		 pipe_name, pipes_open));
...
static void set_incoming_fault(pipes_struct *p)
{
	...
	p->fault_state = True;
	DEBUG10(("set_incoming_fault: Setting fault state on pipe %s : vuid = 0x%x\n",
		p->name, p->vuid ));
}
...
static ssize_t fill_rpc_header(pipes_struct *p, char *data, size_t data_to_copy)
{
	...
	DEBUG10(("fill_rpc_header: data_to_copy = %u, len_needed_to_complete_hdr = %u, receive_len = %u\n",
...
static ssize_t unmarshall_rpc_header(pipes_struct *p)
{
	...
	if(p->in_data.pdu_received_len != RPC_HEADER_LEN) {
		DEBUG0(("unmarshall_rpc_header: assert on rpc header length failed.\n"));
	...
	if(!smb_io_rpc_hdr("", &p->hdr, &rpc_in, 0)) {
		DEBUG0(("unmarshall_rpc_header: failed to unmarshall RPC_HDR.\n"));
		set_incoming_fault(p);
		prs_mem_free(&rpc_in);
		return -1;
	}
	...
	if(p->hdr.major != 5 && p->hdr.minor != 0) {
		DEBUG0(("unmarshall_rpc_header: invalid major/minor numbers in RPC_HDR.\n"));
		set_incoming_fault(p);
		prs_mem_free(&rpc_in);
		return -1;
	}
	...
	if(prs_offset(&p->in_data.data) == 0) {
		...
		if ((p->hdr.pkt_type == RPC_REQUEST) && !(p->hdr.flags & RPC_FLG_FIRST)) {
			...
			DEBUG0(("unmarshall_rpc_header: FIRST flag not set in first PDU !\n"));
			set_incoming_fault(p);
			prs_mem_free(&rpc_in);
			return -1;
		}
		...
		DEBUG5(("unmarshall_rpc_header: using %sendian RPC\n",
		...
		if (p->endian != rpc_in.bigendian_data) {
			DEBUG0(("unmarshall_rpc_header: FIRST endianness flag (%d) different in next PDU !\n", (int)p->endian));
			set_incoming_fault(p);
			prs_mem_free(&rpc_in);
			return -1;
		}
	...
	if((p->hdr.frag_len < RPC_HEADER_LEN) || (p->hdr.frag_len > RPC_MAX_PDU_FRAG_LEN)) {
		DEBUG0(("unmarshall_rpc_header: assert on frag length failed.\n"));
		set_incoming_fault(p);
		prs_mem_free(&rpc_in);
		return -1;
	}

	DEBUG10(("unmarshall_rpc_header: type = %u, flags = %u\n", (unsigned int)p->hdr.pkt_type,
...
{
	if (p->mem_ctx) {
		DEBUG3(("free_pipe_context: destroying talloc pool of size "
...
static BOOL process_request_pdu(pipes_struct *p, prs_struct *rpc_in_p)
{
	...
	if(!p->pipe_bound) {
		DEBUG0(("process_request_pdu: rpc request with no bind.\n"));
		set_incoming_fault(p);
		return False;
	}
	...
	if(!smb_io_rpc_hdr_req("req", &p->hdr_req, rpc_in_p, 0)) {
		DEBUG0(("process_request_pdu: failed to unmarshall RPC_HDR_REQ.\n"));
		set_incoming_fault(p);
		return False;
	}
	...
	switch(p->auth.auth_type) {
		...
		case PIPE_AUTH_TYPE_NTLMSSP:
		{
			...
			if(!api_pipe_ntlmssp_auth_process(p, rpc_in_p, &ss_padding_len, &status)) {
				DEBUG0(("process_request_pdu: failed to do auth processing.\n"));
				DEBUG0(("process_request_pdu: error was %s.\n", nt_errstr(status) ));
				set_incoming_fault(p);
				return False;
			}
			break;
		...
		case PIPE_AUTH_TYPE_SCHANNEL:
			if (!api_pipe_schannel_process(p, rpc_in_p, &ss_padding_len)) {
				DEBUG3(("process_request_pdu: failed to do schannel processing.\n"));
				set_incoming_fault(p);
				return False;
			}
			break;
		...
		default:
			DEBUG0(("process_request_pdu: unknown auth type %u set.\n", (unsigned int)p->auth.auth_type ));
			set_incoming_fault(p);
			return False;
	}
	...
	if(prs_offset(&p->in_data.data) + data_len > 15*1024*1024) {
		DEBUG0(("process_request_pdu: rpc data buffer too large (%u) + (%u)\n",
	...
	if(!prs_append_some_prs_data(&p->in_data.data, rpc_in_p, prs_offset(rpc_in_p), data_len)) {
		DEBUG0(("process_request_pdu: Unable to append data size %u to parse buffer of size %u.\n",
	...
	if(p->hdr.flags & RPC_FLG_LAST) {
		 ...
		 */
 		...
 		if(!prs_set_buffer_size(&p->in_data.data, prs_offset(&p->in_data.data))) {
			DEBUG0(("process_request_pdu: Call to prs_set_buffer_size failed!\n"));
			set_incoming_fault(p);
			return False;
		}
...
static void process_complete_pdu(pipes_struct *p)
{
	...
	if(p->fault_state) {
		DEBUG10(("process_complete_pdu: pipe %s in fault state.\n",
			p->name ));
		set_incoming_fault(p);
	...
	DEBUG10(("process_complete_pdu: processing packet type %u\n",
			(unsigned int)p->hdr.pkt_type ));
	...
	switch (p->hdr.pkt_type) {
		...
		case RPC_PING: /* CL request - ignore... */
			DEBUG0(("process_complete_pdu: Error. Connectionless packet type %u received on pipe %s.\n",
				(unsigned int)p->hdr.pkt_type, p->name));
			break;

		case RPC_RESPONSE: /* No responses here. */
			DEBUG0(("process_complete_pdu: Error. RPC_RESPONSE received from client on pipe %s.\n",
				p->name ));
			break;
		...
		case RPC_BINDACK:
		case RPC_BINDNACK:
			DEBUG0(("process_complete_pdu: Error. RPC_BINDACK/RPC_BINDNACK packet type %u received on pipe %s.\n",
				(unsigned int)p->hdr.pkt_type, p->name));
			break;
		...
		case RPC_ALTCONTRESP:
			DEBUG0(("process_complete_pdu: Error. RPC_ALTCONTRESP on pipe %s: Should only be server -> client.\n",
				p->name));
			break;
		...
		case RPC_SHUTDOWN:
			DEBUG0(("process_complete_pdu: Error. RPC_SHUTDOWN on pipe %s: Should only be server -> client.\n",
				p->name));
			break;

		case RPC_CO_CANCEL:
			/* For now just free all client data and continue processing. */
			DEBUG3(("process_complete_pdu: RPC_ORPHANED. Abandoning rpc call.\n"));
		...
		default:
			DEBUG0(("process_complete_pdu: Unknown rpc type = %u received.\n", (unsigned int)p->hdr.pkt_type ));
			break;
	}
	...
	if (!reply) {
		DEBUG3(("process_complete_pdu: DCE/RPC fault sent on pipe %s\n", p->pipe_srv_name));
		set_incoming_fault(p);
		setup_fault_pdu(p, NT_STATUS(DCERPC_FAULT_OP_RNG_ERROR));
		prs_mem_free(&rpc_in);
	} else {
...
static ssize_t process_incoming_data(pipes_struct *p, char *data, size_t n)
{
	...
	DEBUG10(("process_incoming_data: Start: pdu_received_len = %u, pdu_needed_len = %u, incoming data = %u\n",
	...
	if(data_to_copy == 0) {
		 ...
		 */
		DEBUG0(("process_incoming_data: No space in incoming pdu buffer. Current size = %u \
incoming data size = %u\n", (unsigned int)p->in_data.pdu_received_len, (unsigned int)n ));
	...
	DEBUG10(("process_incoming_data: not a complete PDU yet. pdu_received_len = %u, pdu_needed_len = %u\n",
		(unsigned int)p->in_data.pdu_received_len, (unsigned int)p->in_data.pdu_needed_len ));
...
{
	DEBUG6(("write_to_pipe: %x", p->pnum));

	DEBUG6((" name: %s open: %s len: %d\n",
...
static ssize_t write_to_internal_pipe(void *np_conn, char *data, size_t n)
{
	...
	while(data_left) {
		...
		DEBUG10(("write_to_pipe: data_left = %u\n", (unsigned int)data_left ));

		data_used = process_incoming_data(p, data, data_left);

		DEBUG10(("write_to_pipe: data_used = %d\n", (int)data_used ));
...
{
	if (!p || !p->open) {
		DEBUG0(("read_from_pipe: pipe not open\n"));
		return -1;		
	}

	DEBUG6(("read_from_pipe: %x", p->pnum));
		...
		BOOL *is_data_outstanding)
{
	...
	if(n > RPC_MAX_PDU_FRAG_LEN) {
                DEBUG5(("read_from_pipe: too large read (%u) requested on \
pipe %s. We can only service %d sized reads.\n", (unsigned int)n, p->name, RPC_MAX_PDU_FRAG_LEN ));
	...
	if((pdu_remaining = p->out_data.current_pdu_len - p->out_data.current_pdu_sent) > 0) {
		...
		DEBUG10(("read_from_pipe: %s: current_pdu_len = %u, current_pdu_sent = %u \
returning %d bytes.\n", p->name, (unsigned int)p->out_data.current_pdu_len, 
	...
	DEBUG10(("read_from_pipe: %s: fault_state = %d : data_sent_length \
= %u, prs_offset(&p->out_data.rdata) = %u.\n",
	...
	if(!create_next_pdu(p)) {
		DEBUG0(("read_from_pipe: %s: create_next_pdu failed.\n", p->name));
		return -1;
	}
...
BOOL wait_rpc_pipe_hnd_state(smb_np_struct *p, uint16 priority)
{
	...
	if (p->open) {
		DEBUG3(("wait_rpc_pipe_hnd_state: Setting pipe wait state priority=%x on pipe (name=%s)\n",
		         priority, p->name));
	...
	DEBUG3(("wait_rpc_pipe_hnd_state: Error setting pipe wait state priority=%x (name=%s)\n",
		 priority, p->name));
	return False;
}
...
BOOL set_rpc_pipe_hnd_state(smb_np_struct *p, uint16 device_state)
{
	...
	if (p->open) {
		DEBUG3(("set_rpc_pipe_hnd_state: Setting pipe device state=%x on pipe (name=%s)\n",
		         device_state, p->name));
	...
	DEBUG3(("set_rpc_pipe_hnd_state: Error setting pipe device state=%x (name=%s)\n",
		 device_state, p->name));
	return False;
}
...
{
	if (!p) {
		DEBUG0(("Invalid pipe in close_rpc_pipe_hnd\n"));
		return False;
	}
	...
	DEBUG4(("closed pipe name %s pnum=%x (pipes_open=%d)\n", 
	...
	if ( !delete_pipe_opendb( p ) ) {
		DEBUG3(("close_rpc_pipe_hnd: failed to delete %s "
			"pipe from open db.\n", p->name));
	}
...
static BOOL close_internal_rpc_pipe_hnd(void *np_conn)
{
	pipes_struct *p = (pipes_struct *)np_conn;
	if (!p) {
		DEBUG0(("Invalid pipe in close_internal_rpc_pipe_hnd\n"));
		return False;
	}
...
smb_np_struct *get_rpc_pipe(int pnum)
{
	...
	DEBUG4(("search for pipe pnum=%x\n", pnum));

	for (p=Pipes;p;p=p->next) {
		DEBUG5(("pipe name %s pnum=%x (pipes_open=%d)\n", 
		          p->name, p->pnum, pipes_open));  
	}
