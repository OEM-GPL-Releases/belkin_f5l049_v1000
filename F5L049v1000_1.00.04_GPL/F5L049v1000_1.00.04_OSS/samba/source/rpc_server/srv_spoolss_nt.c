...
static void srv_spoolss_replycloseprinter(int snum, POLICY_HND *handle)
{
	...
	if (!print_notify_deregister_pid(snum))
		DEBUG0(("print_notify_register_pid: Failed to register our pid for printer %s\n", lp_const_servicename(snum) ));

	/* weird if the test succeds !!! */
	if (smb_connections==0) {
		DEBUG0(("srv_spoolss_replycloseprinter:Trying to close non-existant notify backchannel !\n"));
		return;
	}
	...
	if (!W_ERROR_IS_OK(result))
		DEBUG0(("srv_spoolss_replycloseprinter: reply_close_printer failed [%s].\n",
			dos_errstr(result)));
...
static Printer_entry *find_printer_index_by_hnd(pipes_struct *p, POLICY_HND *hnd)
{
	...
	if(!find_policy_by_hnd(p,hnd,(void **)(void *)&find_printer)) {
		DEBUG2(("find_printer_index_by_hnd: Printer handle not found: "));
		return NULL;
	}
...
static BOOL close_printer_handle(pipes_struct *p, POLICY_HND *hnd)
{
	...
	if (!Printer) {
		DEBUG2(("close_printer_handle: Invalid handle (%s:%u:%u)\n", OUR_HANDLE(hnd)));
		return False;
	}
...
static WERROR delete_printer_handle(pipes_struct *p, POLICY_HND *hnd)
{
	...
	if (!Printer) {
		DEBUG2(("delete_printer_handle: Invalid handle (%s:%u:%u)\n", OUR_HANDLE(hnd)));
		return WERR_BADFID;
	}
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG3( ("delete_printer_handle: denied by handle\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	if (del_a_printer( Printer->sharename ) != 0) {
		DEBUG3(("Error deleting printer %s\n", Printer->sharename));
		return WERR_BADFID;
	}
...
static BOOL get_printer_snum(pipes_struct *p, POLICY_HND *hnd, int *number)
{
	...
	if (!Printer) {
		DEBUG2(("get_printer_snum: Invalid handle (%s:%u:%u)\n", OUR_HANDLE(hnd)));
		return False;
	}
...
static BOOL set_printer_hnd_name(Printer_entry *Printer, char *handlename)
{
	...
	DEBUG4(("Setting printer name=%s (len=%lu)\n", handlename, (unsigned long)strlen(handlename)));
	...
	for (snum=0; !found && snum<n_services; snum++) {
		...
		if ( !W_ERROR_IS_OK(result) ) {
			DEBUG0(("set_printer_hnd_name: failed to lookup printer [%s] -- result [%s]\n",
		...
		if ( !(printername = strchr_m(&printer->info_2->printername[2], '\\')) ) {
			DEBUG0(("set_printer_hnd_name: info2->printername in wrong format! [%s]\n",
...
static BOOL open_printer_hnd(pipes_struct *p, POLICY_HND *hnd, char *name, uint32 access_granted)
{
	...
	DEBUG10(("open_printer_hnd: name [%s]\n", name));
	...
	if ( !(new_printer->ctx = talloc_init("Printer Entry [%p]", hnd)) ) {
		DEBUG0(("open_printer_hnd: talloc_init() failed!\n"));
	...
	DEBUG5( ("%d printer handles active\n", (int)p->pipe_handles->count ));
				...
				uint16 notify_field)
{
	...
	DEBUG10( ("Open handle for \\\\%s\\%s is not monitoring 0x%02x/0x%02x\n",
			       ...
			       TALLOC_CTX *mem_ctx)
{
	...
	if (msg->len != sizeof(time_t)) {
		DEBUG5( ("notify_system_time: received wrong sized message (%d)\n",
			  msg->len));
		return;
	}

	if (!prs_init(&ps, RPC_MAX_PDU_FRAG_LEN, mem_ctx, MARSHALL)) {
		DEBUG5( ("notify_system_time: prs_init() failed\n"));
		return;
	}

	if (!make_systemtime(&systime, gmtime((time_t *)msg->notify.data))) {
		DEBUG5( ("notify_system_time: unable to make systemtime\n"));
		prs_mem_free(&ps);
		return;
	}
...
static int notify_msg_ctr_addmsg( SPOOLSS_NOTIFY_MSG_CTR *ctr, SPOOLSS_NOTIFY_MSG *msg )
{
	...
	if ( i == ctr->num_groups ) {
		...
		if ( !(groups = TALLOC_REALLOC_ARRAY( ctr->ctx, ctr->msg_groups, SPOOLSS_NOTIFY_MSG_GROUP, ctr->num_groups)) ) {
			DEBUG0(("notify_msg_ctr_addmsg: talloc_realloc() failed!\n"));
			return 0;
		}
	...
	if ( !(msg_list = TALLOC_REALLOC_ARRAY( ctr->ctx, msg_grp->msgs, SPOOLSS_NOTIFY_MSG, msg_grp->num_msgs )) ) {
		DEBUG0(("notify_msg_ctr_addmsg: talloc_realloc() failed for new message [%d]!\n", msg_grp->num_msgs));
		return 0;
	}
...
static void send_notify2_changes( SPOOLSS_NOTIFY_MSG_CTR *ctr, uint32 idx )
{
	...
	if ( !msg_group ) {
		DEBUG5(("send_notify2_changes() called with no msg group!\n"));
		return;
	}
	...
	if ( !messages ) {
		DEBUG5(("send_notify2_changes() called with no messages!\n"));
		return;
	}
	
	DEBUG8(("send_notify2_changes: Enter...[%s]\n", msg_group->printername));
	...
	for (p = printers_list; p; p = p->next) {
		...
		for ( i=0; i<msg_group->num_msgs; i++ ) {
			...
			DEBUG10(("process_notify2_message: Sending message type [0x%x] field [0x%2x] for printer [%s]\n",
			...
			default:
				DEBUG5( ("Unknown notification type %d\n", msg->type));
				goto done;
			}
	...
	DEBUG8(("send_notify2_changes: Exit...\n"));
	return;
}
...
static BOOL notify2_unpack_msg( SPOOLSS_NOTIFY_MSG *msg, struct timeval *tv, void *buf, size_t len )
{
	...
	DEBUG3( ("notify2_unpack_msg: got NOTIFY2 message for printer %s, jobid %u type %d, field 0x%02x, flags 0x%04x\n",
	...
	if (msg->len == 0){
		DEBUG3( ("notify2_unpack_msg: value1 = %d, value2 = %d\n", msg->notify.value[0],
			  msg->notify.value[1]));}
	else
					 ...
					 void *private_data)
{
	...
	if (len < 4) {
		DEBUG0(("receive_notify2_message_list: bad message format (len < 4)!\n"));
		return;
	}
	...
	DEBUG5( ("receive_notify2_message_list: got %lu messages in list\n", (unsigned long)msg_count));

	if (msg_count == 0) {
		DEBUG0(("receive_notify2_message_list: bad message format (msg_count == 0) !\n"));
		return;
	}
	...
	for ( i=0; i<msg_count; i++ ) {
		...
		if (msg_ptr + 4 - buf > len) {
			DEBUG0(("receive_notify2_message_list: bad message format (len > buf_size) !\n"));
			return;
		}
		...
		if (msg_ptr + msg_len - buf > len) {
			DEBUG0(("receive_notify2_message_list: bad message format (bad len) !\n"));
			return;
		}
	...
	DEBUG10(("receive_notify2_message_list: processed %u messages\n", (uint32)msg_count ));
...
static BOOL srv_spoolss_drv_upgrade_printer(char* drivername)
{
	...
	DEBUG10(("srv_spoolss_drv_upgrade_printer: Sending message about driver upgrade [%s]\n",
		drivername));
			    ...
			    void *buf, size_t len, void *private_data)
{
	...
	DEBUG10(("do_drv_upgrade_printer: Got message for new driver [%s]\n", drivername ));
	...
	for (snum=0; snum<n_services; snum++)
	{
		if (lp_snum_ok(snum) && lp_print_ok(snum) ) 
		{
			...
			{
				DEBUG6(("Updating printer [%s]\n", printer->info_2->printername));
				
				/* all we care about currently is the change_id */
				
				result = mod_a_printer(printer, 2);
				if (!W_ERROR_IS_OK(result)) {
					DEBUG3(("do_drv_upgrade_printer: mod_a_printer() failed with status [%s]\n", 
						dos_errstr(result)));
				}
			}
...
static BOOL srv_spoolss_reset_printerdata(char* drivername)
{
	...
	DEBUG10(("srv_spoolss_reset_printerdata: Sending message about resetting printerdata [%s]\n",
		drivername));
			   ...
			   void *buf, size_t len, void *private_data)
{
	...
	DEBUG10(("reset_all_printerdata: Got message for new driver [%s]\n", drivername ));
	...
	for ( snum=0; snum<n_services; snum++ )
	{
		if ( lp_snum_ok(snum) && lp_print_ok(snum) ) 
		{
			...
			if ( printer && printer->info_2 && !strcmp(drivername, printer->info_2->drivername) ) 
			{
				DEBUG6(("reset_all_printerdata: Updating printer [%s]\n", printer->info_2->printername));
				
				if ( !set_driver_init(printer, 2) ) {
					DEBUG5(("reset_all_printerdata: Error resetting printer data for printer [%s], driver [%s]!\n",
						printer->info_2->printername, printer->info_2->drivername));
				}	
				
				result = mod_a_printer( printer, 2 );
				if ( !W_ERROR_IS_OK(result) ) {
					DEBUG3(("reset_all_printerdata: mod_a_printer() failed!  (%s)\n", 
...
static WERROR convert_to_openprinterex(TALLOC_CTX *ctx, SPOOL_Q_OPEN_PRINTER_EX *q_u_ex, SPOOL_Q_OPEN_PRINTER *q_u)
{
	...
	DEBUG8(("convert_to_openprinterex\n"));
...
WERROR _spoolss_open_printer_ex( pipes_struct *p, SPOOL_Q_OPEN_PRINTER_EX *q_u, SPOOL_R_OPEN_PRINTER_EX *r_u)
{
	...
	if ( !Printer ) {
		DEBUG0((" _spoolss_open_printer_ex: logic error.  Can't find printer "
	...
	case SPLHND_PORTMON_LOCAL:
		...
		if (printer_default->access_required &
			...
			DEBUG3( ("access DENIED for non-printserver bits\n"));
			close_printer_handle(p, handle);
			return WERR_ACCESS_DENIED;
		}
		...
		DEBUG4(("Setting print server access = %s\n", (printer_default->access_required == SERVER_ACCESS_ADMINISTER) 
			? "SERVER_ACCESS_ADMINISTER" : "SERVER_ACCESS_ENUMERATE" ));
	...
	case SPLHND_PRINTER:
		...
		if ( !check_access(smbd_server_fd(), lp_hostsallow(snum), lp_hostsdeny(snum)) ) {    
			DEBUG3( ("access DENIED (hosts allow/deny) for printer open\n"));
			return WERR_ACCESS_DENIED;
		}
		...
		DEBUG4(("Setting printer access = %s\n", (printer_default->access_required == PRINTER_ACCESS_ADMINISTER) 
	...
	{
		DEBUG10(("_spoolss_open_printer_ex: Enabling LAN/WAN hack for Win2k clients.\n"));
		sys_usleep( 500000 );
	}
#endif
	...
	switch (level) {
		case 2:
			...
			if ( !printer->info_2 ) {
				...
				if ( !printer->info_2 ) {
					DEBUG0(("convert_printer_info: talloc() failed!\n"));
					return False;
				}
			}
				...
				NT_DEVICEMODE **pp_nt_devmode)
{
	...
	if (nt_devmode == NULL) {
		DEBUG5( ("convert_devicemode: allocating a generic devmode\n"));
...
static WERROR _spoolss_enddocprinter_internal(pipes_struct *p, POLICY_HND *handle)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_enddocprinter_internal: Invalid handle (%s:%u:%u)\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
				  ...
				  uint32 *needed, uint32 in_size  )
{
	...
	*type = regval_type( val );
	...
	DEBUG5(("get_printer_dataex: allocating %d\n", in_size));
	...
	*needed = size;
	...
	DEBUG5(("get_printer_dataex: copy done\n"));
...
{		
	...
	DEBUG8(("getprinterdata_printer_server:%s\n", value));
...
WERROR _spoolss_getprinterdata(pipes_struct *p, SPOOL_Q_GETPRINTERDATA *q_u, SPOOL_R_GETPRINTERDATA *r_u)
{
	...
	*type   = 0;
	...
	DEBUG4(("_spoolss_getprinterdata\n"));
	
	if ( !Printer ) {
		DEBUG2(("_spoolss_getprinterdata: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		status = WERR_BADFID;
		goto done;
	}
			...
			struct in_addr *client_ip, const char *remote_machine)
{
	...
	if ( is_zero_ip(*client_ip) ) {
		if ( !resolve_name( remote_machine, &rm_addr, 0x20) ) {
			DEBUG2(("spoolss_connect_to_client: Can't resolve address for %s\n", remote_machine));
			return False;
		}

		if ( ismyip( rm_addr )) {
			DEBUG0(("spoolss_connect_to_client: Machine %s is one of our addresses. Cannot add to ourselves.\n", remote_machine));
			return False;
		}
	} else {
		rm_addr.s_addr = client_ip->s_addr;
		DEBUG5(("spoolss_connect_to_client: Using address %s (no name resolution necessary)\n",
			inet_ntoa(*client_ip) ));
	}
	...
	if ( !NT_STATUS_IS_OK( ret ) ) {
		DEBUG2(("spoolss_connect_to_client: connection to [%s] failed!\n", 
			remote_machine ));
		return False;
	}	
		
	if ( the_cli->protocol != PROTOCOL_NT1 ) {
		DEBUG0(("spoolss_connect_to_client: machine %s didn't negotiate NT protocol.\n", remote_machine));
	...
	if ( !(*pp_pipe = cli_rpc_pipe_open_noauth(the_cli, PI_SPOOLSS, &ret)) ) {
		DEBUG2(("spoolss_connect_to_client: unable to open the spoolss pipe on machine %s. Error was : %s.\n",
					...
					POLICY_HND *handle, struct in_addr *client_ip)
{
	...
	if (!W_ERROR_IS_OK(result))
		DEBUG5(("srv_spoolss_reply_open_printer: Client RPC returned [%s]\n",
			dos_errstr(result)));
...
WERROR _spoolss_rffpcnex(pipes_struct *p, SPOOL_Q_RFFPCNEX *q_u, SPOOL_R_RFFPCNEX *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_rffpcnex: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
...
static uint32 size_of_notify_info_data(uint16 type, uint16 field)
{
	...
	DEBUG5( ("invalid notify data type %d/%d\n", type, field));
					  ...
					  TALLOC_CTX *mem_ctx) 
{
	...
	DEBUG4(("construct_notify_printer_info: Notify type: [%s], number of notify info: [%d] on printer: [%s]\n",
	...
	for(field_num=0; field_num<option_type->count; field_num++) {
		...
		DEBUG4(("construct_notify_printer_info: notify [%d]: type [%x], field [%x]\n", field_num, type, field));

		if (!search_notify(type, field, &j) )
			continue;

		if((info->data=SMB_REALLOC_ARRAY(info->data, SPOOL_NOTIFY_INFO_DATA, info->count+1)) == NULL) {
			DEBUG2(("construct_notify_printer_info: failed to enlarge buffer info->data!\n"));
			free_a_printer(&printer, 2);
			return False;
		}
		...
		DEBUG10(("construct_notify_printer_info: calling [%s]  snum=%d  printername=[%s])\n",
				       ...
				       TALLOC_CTX *mem_ctx) 
{
	...
	DEBUG4(("construct_notify_jobs_info\n"));
	...
	for(field_num=0; field_num<option_type->count; field_num++) {
		...
		if((info->data=SMB_REALLOC_ARRAY(info->data, SPOOL_NOTIFY_INFO_DATA, info->count+1)) == NULL) {
			DEBUG2(("construct_notify_jobs_info: failed to enlarg buffer info->data!\n"));
			return False;
		}
				      ...
				      TALLOC_CTX *mem_ctx)
{
	...
	DEBUG4(("printserver_notify_info\n"));
	...
	DEBUG1(("dumping the NOTIFY_INFO\n"));
...
WERROR _spoolss_rfnpcnex( pipes_struct *p, SPOOL_Q_RFNPCNEX *q_u, SPOOL_R_RFNPCNEX *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_rfnpcnex: Invalid handle (%s:%u:%u).\n",
			 OUR_HANDLE(handle)));
		goto done;
	}

	DEBUG4(("Printer type %x\n",Printer->printer_type));
	...
	if (Printer->notify.client_connected) {
		DEBUG10(("_spoolss_rfnpcnex: Saving change value in request [%x]\n", q_u->change));
...
DEVICEMODE *construct_dev_mode(const char *servicename)
{
	...
	if ( !printer->info_2->devmode ) {
		DEBUG5( ("BONG! There was no device mode!\n"));
		goto done;
	}

	if ((devmode = SMB_MALLOC_P(DEVICEMODE)) == NULL) {
		DEBUG2(("construct_dev_mode: malloc fail.\n"));
		goto done;
	}
...
static BOOL construct_printer_info_2(Printer_entry *print_hnd, PRINTER_INFO_2 *printer, int snum)
{
	...
	if ( !(printer->devmode = construct_dev_mode(
		       lp_const_servicename(snum))) )
		DEBUG8( ("Returning NULL Devicemode!\n"));
...
static BOOL construct_printer_info_3(Printer_entry *print_hnd, PRINTER_INFO_3 **pp_printer, int snum)
{
	...
	*pp_printer = NULL;
	if ((printer = SMB_MALLOC_P(PRINTER_INFO_3)) == NULL) {
		DEBUG2(("construct_printer_info_3: malloc fail.\n"));
		free_a_printer(&ntprinter, 2);
		return False;
	}
...
static WERROR enum_all_printers_info_1(uint32 flags, RPC_BUFFER *buffer, uint32 offered, uint32 *needed, uint32 *returned)
{
	...
	DEBUG4(("enum_all_printers_info_1\n"));	

	for (snum=0; snum<n_services; snum++) {
		if (lp_browseable(snum) && lp_snum_ok(snum) && lp_print_ok(snum) ) {
			DEBUG4(("Found a printer in smb.conf: %s[%x]\n", lp_servicename(snum), snum));
			...
			if (construct_printer_info_1(NULL, flags, &current_prt, snum)) {
				if((printers=SMB_REALLOC_ARRAY(printers, PRINTER_INFO_1, *returned +1)) == NULL) {
					DEBUG2(("enum_all_printers_info_1: failed to enlarge printers buffer!\n"));
					*returned=0;
					return WERR_NOMEM;
				}
				DEBUG4(("ReAlloced memory for [%d] PRINTER_INFO_1\n", *returned));		
...
{
	DEBUG4(("enum_all_printers_info_1_local\n"));	
...
static WERROR enum_all_printers_info_1_name(fstring name, RPC_BUFFER *buffer, uint32 offered, uint32 *needed, uint32 *returned)
{
	...
	DEBUG4(("enum_all_printers_info_1_name\n"));	
...
static WERROR enum_all_printers_info_1_remote(fstring name, RPC_BUFFER *buffer, uint32 offered, uint32 *needed, uint32 *returned)
{
	...
	fstring desc;
	fstring comment;
	DEBUG4(("enum_all_printers_info_1_remote\n"));	
	WERROR result = WERR_OK;
...
static WERROR enum_all_printers_info_1_network(fstring name, RPC_BUFFER *buffer, uint32 offered, uint32 *needed, uint32 *returned)
{
	...
	DEBUG4(("enum_all_printers_info_1_network\n"));	
		...
		if (lp_browseable(snum) && lp_snum_ok(snum) && lp_print_ok(snum) ) {
			...
			if (construct_printer_info_2(NULL, &current_prt, snum)) {
				if ( !(printers=SMB_REALLOC_ARRAY(printers, PRINTER_INFO_2, *returned +1)) ) {
					DEBUG2(("enum_all_printers_info_2: failed to enlarge printers buffer!\n"));
					*returned = 0;
					return WERR_NOMEM;
				}

				DEBUG4(("ReAlloced memory for [%d] PRINTER_INFO_2\n", *returned + 1));		
	...
	*uni_array=NULL;
	...
	while (True) 
	{
		...
		if ( (*uni_array=SMB_REALLOC_ARRAY(*uni_array, uint16, j+1+strlen(line)+2)) == NULL ) {
			DEBUG2(("init_unistr_array: Realloc error\n" ));
			return 0;
		}
...
{	
	...
	DEBUG8(("construct_printer_driver_info_3: status: %s\n", dos_errstr(status)));
	if (!W_ERROR_IS_OK(status))
		return WERR_INVALID_PRINTER_NAME;
...
{	
	...
	DEBUG8(("construct_printer_driver_info_6: status: %s\n", dos_errstr(status)));
...
WERROR _spoolss_getprinterdriver2(pipes_struct *p, SPOOL_Q_GETPRINTERDRIVER2 *q_u, SPOOL_R_GETPRINTERDRIVER2 *r_u)
{
	...
	DEBUG4(("_spoolss_getprinterdriver2\n"));

	if ( !(printer = find_printer_index_by_hnd( p, handle )) ) {
		DEBUG0(("_spoolss_getprinterdriver2: invalid printer handle!\n"));
		return WERR_INVALID_PRINTER_NAME;
	}
...
WERROR _spoolss_startpageprinter(pipes_struct *p, SPOOL_Q_STARTPAGEPRINTER *q_u, SPOOL_R_STARTPAGEPRINTER *r_u)
{
	...
	if (!Printer) {
		DEBUG3(("Error in startpageprinter printer handle\n"));
		return WERR_BADFID;
	}
...
WERROR _spoolss_endpageprinter(pipes_struct *p, SPOOL_Q_ENDPAGEPRINTER *q_u, SPOOL_R_ENDPAGEPRINTER *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_endpageprinter: Invalid handle (%s:%u:%u).\n",OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
...
WERROR _spoolss_startdocprinter(pipes_struct *p, SPOOL_Q_STARTDOCPRINTER *q_u, SPOOL_R_STARTDOCPRINTER *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_startdocprinter: Invalid handle (%s:%u:%u)\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
...
WERROR _spoolss_writeprinter(pipes_struct *p, SPOOL_Q_WRITEPRINTER *q_u, SPOOL_R_WRITEPRINTER *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_writeprinter: Invalid handle (%s:%u:%u)\n",OUR_HANDLE(handle)));
			      ...
			      pipes_struct *p)
{
	...
	if (!Printer) {
		DEBUG2(("control_printer: Invalid handle (%s:%u:%u)\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
...
WERROR _spoolss_abortprinter(pipes_struct *p, SPOOL_Q_ABORTPRINTER *q_u, SPOOL_R_ABORTPRINTER *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_abortprinter: Invalid handle (%s:%u:%u)\n",OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
				 ...
				 pipes_struct *p, SEC_DESC_BUF *secdesc_ctr)
{
	...
	if (!Printer || !get_printer_snum(p, handle, &snum)) {
		DEBUG2(("update_printer_sec: Invalid handle (%s:%u:%u)\n",
			 OUR_HANDLE(handle)));
	...
	if (!secdesc_ctr) {
		DEBUG10(("update_printer_sec: secdesc_ctr is NULL !\n"));
	...
	if ( Printer->access_granted != PRINTER_ACCESS_ADMINISTER ) {
		DEBUG4(("update_printer_sec: updated denied by printer permissions\n"));
	...
	if (DEBUGLEVEL >= 10) {
		...
		DEBUG10( ("old_secdesc_ctr for %s has %d aces:\n", 
		...
		if (the_acl) {
			DEBUG10( ("secdesc_ctr for %s has %d aces:\n", 
				   PRINTERNAME(snum), the_acl->num_aces));
			...
			DEBUG10( ("dacl for secdesc_ctr is NULL\n"));
		}
	}
...
static BOOL check_printer_ok(NT_PRINTER_INFO_LEVEL_2 *info, int snum)
{
	...
	DEBUG5(("check_printer_ok: servername=%s printername=%s sharename=%s "
                           ...
                           DEVICEMODE *devmode)
{
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG3( ("update_printer: printer property change denied by handle\n"));
		result = WERR_ACCESS_DENIED;
		goto done;
	}
	...
	if (!strequal(printer->info_2->drivername, old_printer->info_2->drivername))
	{
		if (!set_driver_init(printer, 2)) 
		{
			DEBUG5(("update_printer: Error restoring driver initialization data for driver [%s]!\n",
				printer->info_2->drivername));
		}
		
		DEBUG10(("update_printer: changing driver [%s]!  Sending event!\n",
			printer->info_2->drivername));
				   ...
				   const SPOOL_PRINTER_INFO_LEVEL *info)
{
	...
	DEBUG5(("publish_or_unpublish_printer, action = %d\n",info7->action));
...
WERROR _spoolss_setprinter(pipes_struct *p, SPOOL_Q_SETPRINTER *q_u, SPOOL_R_SETPRINTER *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_setprinter: Invalid handle (%s:%u:%u)\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
...
WERROR _spoolss_fcpn(pipes_struct *p, SPOOL_Q_FCPN *q_u, SPOOL_R_FCPN *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_fcpn: Invalid handle (%s:%u:%u)\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
	...
	for (version=0; version<DRIVER_MAX_VERSION; version++) {
		...
		if(ndrivers != 0) {
			if((driver_info_1=SMB_REALLOC_ARRAY(driver_info_1, DRIVER_INFO_1, *returned+ndrivers )) == NULL) {
				DEBUG0(("enumprinterdrivers_level1: failed to enlarge driver info buffer!\n"));
				SAFE_FREE(list);
				return WERR_NOMEM;
			}
		}
	...
	for (version=0; version<DRIVER_MAX_VERSION; version++) {
		...
		if(ndrivers != 0) {
			if((driver_info_2=SMB_REALLOC_ARRAY(driver_info_2, DRIVER_INFO_2, *returned+ndrivers )) == NULL) {
				DEBUG0(("enumprinterdrivers_level2: failed to enlarge driver info buffer!\n"));
				SAFE_FREE(list);
				return WERR_NOMEM;
			}
		}
	...
	for (version=0; version<DRIVER_MAX_VERSION; version++) {
		...
		if(ndrivers != 0) {
			if((driver_info_3=SMB_REALLOC_ARRAY(driver_info_3, DRIVER_INFO_3, *returned+ndrivers )) == NULL) {
				DEBUG0(("enumprinterdrivers_level3: failed to enlarge driver info buffer!\n"));
				SAFE_FREE(list);
				return WERR_NOMEM;
			}
		}
...
WERROR _spoolss_enumprinterdrivers( pipes_struct *p, SPOOL_Q_ENUMPRINTERDRIVERS *q_u, SPOOL_R_ENUMPRINTERDRIVERS *r_u)
{
	...
	DEBUG4(("_spoolss_enumprinterdrivers\n"));
	...
	case 1:
		if (foundBuiltin) {
			...
			for (i=0; i<numofforms; i++) {
				...
				DEBUG4(("_spoolss_getform: checking form %s (want %s)\n", list[i].name, form_name));
...
static WERROR enumports_level_1(RPC_BUFFER *buffer, uint32 offered, uint32 *needed, uint32 *returned)
{
	...
	if(numlines) {
		if((ports=SMB_MALLOC_ARRAY( PORT_INFO_1, numlines )) == NULL) {
			DEBUG10(("Returning WERR_NOMEM [%s]\n", 
		...
		for (i=0; i<numlines; i++) {
			DEBUG6(("Filling port number [%d] with port [%s]\n", i, qlines[i]));
				...
				POLICY_HND *handle)
{
	...
	if ( !(printer = TALLOC_ZERO_P(NULL, NT_PRINTER_INFO_LEVEL)) ) {
		DEBUG0(("spoolss_addprinterex_level_2: malloc fail.\n"));
		return WERR_NOMEM;
	}
	...
	if ((snum = print_queue_snum(printer->info_2->sharename)) != -1) {
		DEBUG5( ("spoolss_addprinterex_level_2: Attempted to add a printer named [%s] when one already existed!\n", 
	...
	if (*lp_addprinter_cmd() ) {
		...
		DEBUG0(("spoolss_addprinterex_level_2: add printer for printer %s called and no"
...
WERROR _spoolss_addprinterdriver(pipes_struct *p, SPOOL_Q_ADDPRINTERDRIVER *q_u, SPOOL_R_ADDPRINTERDRIVER *r_u)
{
	...
	DEBUG5(("Cleaning driver's information\n"));
	err = clean_up_driver_struct(driver, level, &user);
	if (!W_ERROR_IS_OK(err))
		goto done;

	DEBUG5(("Moving driver to final destination\n"));
	...
	if (!srv_spoolss_drv_upgrade_printer(driver_name)) {
		DEBUG0(("_spoolss_addprinterdriver: Failed to send message about upgrading driver [%s]!\n",
			driver_name));
	}
	...
	*/
	...
	switch (version) {
		...
		*/
		case 0: 
			DEBUG10(("_spoolss_addprinterdriver: init data not deleted for 9x driver [%s]\n",
					driver_name));
			break;
			...
			if (!W_ERROR_IS_OK(get_a_printer_driver(&driver1, 3, driver_name, "Windows NT x86", 3))) {
				...
				*/
				if (!del_driver_init(driver_name))
					DEBUG6(("_spoolss_addprinterdriver: del_driver_init(%s) Nt failed!\n", driver_name));
			} else {
				/*
				 * a 2k/Xp driver was found, don't delete init data because Nt driver will use it.
				*/
				free_a_printer_driver(driver1,3);
				DEBUG10(("_spoolss_addprinterdriver: init data not deleted for Nt driver [%s]\n", 
						driver_name));
			}
		}
		break;
		...
		*/
		case 3:	
			if (!del_driver_init(driver_name))
				DEBUG6(("_spoolss_addprinterdriver: del_driver_init(%s) 2k/Xp failed!\n", driver_name));
			break;

		default:
			DEBUG0(("_spoolss_addprinterdriver: invalid level=%d\n", level));
			break;
 	}
...
static WERROR getprinterdriverdir_level_1(UNISTR2 *name, UNISTR2 *uni_environment, RPC_BUFFER *buffer, uint32 offered, uint32 *needed)
{
	...
	DEBUG4(("printer driver directory: [%s]\n", path));
...
WERROR _spoolss_getprinterdriverdirectory(pipes_struct *p, SPOOL_Q_GETPRINTERDRIVERDIR *q_u, SPOOL_R_GETPRINTERDRIVERDIR *r_u)
{
	...
	DEBUG4(("_spoolss_getprinterdriverdirectory\n"));
...
WERROR _spoolss_enumprinterdata(pipes_struct *p, SPOOL_Q_ENUMPRINTERDATA *q_u, SPOOL_R_ENUMPRINTERDATA *r_u)
{
	...
	*out_data_len     = 0;
	...
	DEBUG5(("spoolss_enumprinterdata\n"));

	if (!Printer) {
		DEBUG2(("_spoolss_enumprinterdata: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
...
WERROR _spoolss_setprinterdata( pipes_struct *p, SPOOL_Q_SETPRINTERDATA *q_u, SPOOL_R_SETPRINTERDATA *r_u)
{
	...
	DEBUG5(("spoolss_setprinterdata\n"));

	if (!Printer) {
		DEBUG2(("_spoolss_setprinterdata: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}

	if ( Printer->printer_type == SPLHND_SERVER ) {
		DEBUG10(("_spoolss_setprinterdata: Not implemented for server handles yet\n"));
		return WERR_INVALID_PARAM;
	}
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) 
	{
		DEBUG3( ("_spoolss_setprinterdata: change denied by handle access permissions\n"));
		status = WERR_ACCESS_DENIED;
		goto done;
	}
...
WERROR _spoolss_resetprinter(pipes_struct *p, SPOOL_Q_RESETPRINTER *q_u, SPOOL_R_RESETPRINTER *r_u)
{
	...
	if (!Printer) {
		DEBUG2(("_spoolss_resetprinter: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
...
WERROR _spoolss_deleteprinterdata(pipes_struct *p, SPOOL_Q_DELETEPRINTERDATA *q_u, SPOOL_R_DELETEPRINTERDATA *r_u)
{
	...
	DEBUG5(("spoolss_deleteprinterdata\n"));
	
	if (!Printer) {
		DEBUG2(("_spoolss_deleteprinterdata: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG3( ("_spoolss_deleteprinterdata: printer properties change denied by handle\n"));
		return WERR_ACCESS_DENIED;
	}
...
WERROR _spoolss_addform( pipes_struct *p, SPOOL_Q_ADDFORM *q_u, SPOOL_R_ADDFORM *r_u)
{
	...
	DEBUG5(("spoolss_addform\n"));

	if (!Printer) {
		DEBUG2(("_spoolss_addform: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
	...
	if ( !(Printer->access_granted & (PRINTER_ACCESS_ADMINISTER|SERVER_ACCESS_ADMINISTER)) ) {
		DEBUG2(("_spoolss_addform: denied by handle permissions.\n"));
		status = WERR_ACCESS_DENIED;
		goto done;
	}
...
WERROR _spoolss_deleteform( pipes_struct *p, SPOOL_Q_DELETEFORM *q_u, SPOOL_R_DELETEFORM *r_u)
{
	...
	DEBUG5(("spoolss_deleteform\n"));

	if (!Printer) {
		DEBUG2(("_spoolss_deleteform: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
	...
	if ( !(Printer->access_granted & (PRINTER_ACCESS_ADMINISTER|SERVER_ACCESS_ADMINISTER)) ) {
		DEBUG2(("_spoolss_deleteform: denied by handle permissions.\n"));
		status = WERR_ACCESS_DENIED;
		goto done;
	}
...
WERROR _spoolss_setform(pipes_struct *p, SPOOL_Q_SETFORM *q_u, SPOOL_R_SETFORM *r_u)
{
	...
	Printer_entry *Printer = find_printer_index_by_hnd(p, handle);
 	...
 	DEBUG5(("spoolss_setform\n"));

	if (!Printer) {
		DEBUG2(("_spoolss_setform: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
	...
	if ( !(Printer->access_granted & (PRINTER_ACCESS_ADMINISTER|SERVER_ACCESS_ADMINISTER)) ) {
		DEBUG2(("_spoolss_setform: denied by handle permissions\n"));
		status = WERR_ACCESS_DENIED;
		goto done;
	}
...
WERROR _spoolss_enumprintprocessors(pipes_struct *p, SPOOL_Q_ENUMPRINTPROCESSORS *q_u, SPOOL_R_ENUMPRINTPROCESSORS *r_u)
{
	...
	buffer = r_u->buffer;
 	...
 	DEBUG5(("spoolss_enumprintprocessors\n"));
...
WERROR _spoolss_enumprintprocdatatypes(pipes_struct *p, SPOOL_Q_ENUMPRINTPROCDATATYPES *q_u, SPOOL_R_ENUMPRINTPROCDATATYPES *r_u)
{
	...
	buffer = r_u->buffer;
 	...
 	DEBUG5(("_spoolss_enumprintprocdatatypes\n"));
...
WERROR _spoolss_enumprintmonitors(pipes_struct *p, SPOOL_Q_ENUMPRINTMONITORS *q_u, SPOOL_R_ENUMPRINTMONITORS *r_u)
{
	...
	buffer = r_u->buffer;
 	...
 	DEBUG5(("spoolss_enumprintmonitors\n"));
...
WERROR _spoolss_getprinterdataex(pipes_struct *p, SPOOL_Q_GETPRINTERDATAEX *q_u, SPOOL_R_GETPRINTERDATAEX *r_u)
{
	...
	*out_size = in_size;
	...
	if (!Printer) {
		DEBUG2(("_spoolss_getprinterdataex: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		status = WERR_BADFID;
		goto done;
	}
	...
	if (Printer->printer_type == SPLHND_SERVER) {
		DEBUG10(("_spoolss_getprinterdataex: Not implemented for server handles yet\n"));
		status = WERR_INVALID_PARAM;
		goto done;
	}
	...
	if ( lookup_printerkey( printer->info_2->data, keyname ) == -1 ) {
		DEBUG4(("_spoolss_getprinterdataex: Invalid keyname [%s]\n", keyname ));
		free_a_printer( &printer, 2 );
		status = WERR_BADFILE;
		goto done;
	}
...
WERROR _spoolss_setprinterdataex(pipes_struct *p, SPOOL_Q_SETPRINTERDATAEX *q_u, SPOOL_R_SETPRINTERDATAEX *r_u)
{
	...
	DEBUG4(("_spoolss_setprinterdataex\n"));

        /* From MSDN documentation of SetPrinterDataEx: pass request to
           SetPrinterData if key is "PrinterDriverData" */

	if (!Printer) {
		DEBUG2(("_spoolss_setprinterdataex: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}

	if ( Printer->printer_type == SPLHND_SERVER ) {
		DEBUG10(("_spoolss_setprinterdataex: Not implemented for server handles yet\n"));
		return WERR_INVALID_PARAM;
	}
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) 
	{
		DEBUG3( ("_spoolss_setprinterdataex: change denied by handle access permissions\n"));
		return WERR_ACCESS_DENIED;
	}
...
WERROR _spoolss_deleteprinterdataex(pipes_struct *p, SPOOL_Q_DELETEPRINTERDATAEX *q_u, SPOOL_R_DELETEPRINTERDATAEX *r_u)
{
	...
	DEBUG5(("spoolss_deleteprinterdataex\n"));
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG3( ("_spoolss_deleteprinterdataex: printer properties change denied by handle\n"));
		return WERR_ACCESS_DENIED;
	}
...
WERROR _spoolss_enumprinterkey(pipes_struct *p, SPOOL_Q_ENUMPRINTERKEY *q_u, SPOOL_R_ENUMPRINTERKEY *r_u)
{
	...
	DEBUG4(("_spoolss_enumprinterkey\n"));

	if (!Printer) {
		DEBUG2(("_spoolss_enumprinterkey: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
...
WERROR _spoolss_deleteprinterkey(pipes_struct *p, SPOOL_Q_DELETEPRINTERKEY *q_u, SPOOL_R_DELETEPRINTERKEY *r_u)
{
	...
	DEBUG5(("spoolss_deleteprinterkey\n"));
	
	if (!Printer) {
		DEBUG2(("_spoolss_deleteprinterkey: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
	...
	if (Printer->access_granted != PRINTER_ACCESS_ADMINISTER) {
		DEBUG3( ("_spoolss_deleteprinterkey: printer properties change denied by handle\n"));
		return WERR_ACCESS_DENIED;
	}
...
WERROR _spoolss_enumprinterdataex(pipes_struct *p, SPOOL_Q_ENUMPRINTERDATAEX *q_u, SPOOL_R_ENUMPRINTERDATAEX *r_u)
{
	...
	DEBUG4(("_spoolss_enumprinterdataex\n"));

	if (!Printer) {
		DEBUG2(("_spoolss_enumprinterdataex: Invalid handle (%s:%u:%u1<).\n", OUR_HANDLE(handle)));
		return WERR_BADFID;
	}
	...
	{
		DEBUG10(("_spoolss_enumprinterdataex: Unknown keyname [%s]\n", key));
		result = WERR_INVALID_PARAM;
		goto done;
	}
	...
	if ( num_entries )
	{
		...
		{
			DEBUG0(("_spoolss_enumprinterdataex: talloc() failed to allocate memory for [%lu] bytes!\n",
	...
	for ( i=0; i<num_entries; i++ )
	{
		...
		val = regval_ctr_specific_value( p_data->keys[key_index].values, i );
		DEBUG10(("retrieved value number [%d] [%s]\n", i, regval_name(val) ));
		...
		if ( data_len ) {
			...
			{
				DEBUG0(("TALLOC_MEMDUP failed to allocate memory [data_len=%d] for data!\n", 
					data_len ));
				result = WERR_NOMEM;
				goto done;
			}
...
WERROR _spoolss_getprintprocessordirectory(pipes_struct *p, SPOOL_Q_GETPRINTPROCESSORDIRECTORY *q_u, SPOOL_R_GETPRINTPROCESSORDIRECTORY *r_u)
{
	...
	buffer = r_u->buffer;
 	...
 	DEBUG5(("_spoolss_getprintprocessordirectory\n"));
                                      ...
                                      uint32 *needed )
{
	...
	DEBUG10(("process_xcvtcp_command: Received command \"%s\"\n", command));
					...
					uint32 *needed )
{
	...
	DEBUG10(("process_xcvlocal_command: Received command \"%s\"\n", command));
...
{	
	...
	if (!Printer) {
		DEBUG2(("_spoolss_xcvdataport: Invalid handle (%s:%u:%u).\n", OUR_HANDLE(&q_u->handle)));
		return WERR_BADFID;
	}
	...
	if ( !(Printer->printer_type & (SPLHND_PORTMON_LOCAL|SPLHND_PORTMON_TCP)) ) {
		DEBUG2(("_spoolss_xcvdataport: Call only valid for Port Monitors\n"));
		return WERR_BADFID;
	}
	...
	if ( !(Printer->access_granted & SERVER_ACCESS_ADMINISTER) ) {
		DEBUG2(("_spoolss_xcvdataport: denied by handle permissions.\n"));
		return WERR_ACCESS_DENIED;
	}
