...
BOOL init_pipe_handle_list(pipes_struct *p, char *pipe_name)
{
	...
	if (!hl) {
		...
		DEBUG10(("init_pipe_handles: created handle list for pipe %s\n", pipe_name ));
	}
	...
	DEBUG10(("init_pipe_handles: pipe_handles ref count = %lu for pipe %s\n",
...
BOOL create_policy_hnd(pipes_struct *p, POLICY_HND *hnd, void (*free_fn)(void *), void *data_ptr)
{
	...
	if (p->pipe_handles->count > MAX_OPEN_POLS) {
		DEBUG0(("create_policy_hnd: ERROR: too many handles (%d) on this pipe.\n",
	...
	if (!pol) {
		DEBUG0(("create_policy_hnd: ERROR: out of memory!\n"));
		return False;
	}
	...
	*hnd = pol->pol_hnd;
	...
	DEBUG4(("Opened policy hnd[%d] ", (int)p->pipe_handles->count));
...
static struct policy *find_policy_by_hnd_internal(pipes_struct *p, POLICY_HND *hnd, void **data_p)
{
	...
	for (i = 0, pol=p->pipe_handles->Policy;pol;pol=pol->next, i++) {
		if (memcmp(&pol->pol_hnd, hnd, sizeof(*hnd)) == 0) {
			DEBUG4(("Found policy hnd[%d] ", (int)i));
			dump_data(4, (char *)hnd, sizeof(*hnd));
			if (data_p)
				*data_p = pol->data_ptr;
			return pol;
...
BOOL close_policy_hnd(pipes_struct *p, POLICY_HND *hnd)
{
	...
	if (!pol) {
		DEBUG3(("Error closing policy\n"));
		return False;
	}

	DEBUG3(("Closed policy\n"));
...
void close_policy_by_pipe(pipes_struct *p)
{
	...
	if (p->pipe_handles->pipe_ref_count == 0) {
		...
		DEBUG10(("close_policy_by_pipe: deleted handle list for pipe %s\n", p->name ));
	}
}
