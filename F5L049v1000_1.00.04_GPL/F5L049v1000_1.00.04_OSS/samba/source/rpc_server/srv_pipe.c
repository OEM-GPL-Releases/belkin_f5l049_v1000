...
static BOOL create_next_pdu_ntlmssp(pipes_struct *p)
{
	...
	if(!data_len_left) {
		DEBUG0(("create_next_pdu_ntlmssp: no data left to send !\n"));
		return False;
	}
	...
	if(p->out_data.data_sent_length + data_len >= prs_offset(&p->out_data.rdata)) {
		...
		if (data_len_left % 8) {
			...
			DEBUG10(("create_next_pdu_ntlmssp: adding sign/seal padding of %u\n",
				ss_padding_len ));
		}
	}
	...
	if(!smb_io_rpc_hdr("hdr", &p->hdr, &outgoing_pdu, 0)) {
		DEBUG0(("create_next_pdu_ntlmssp: failed to marshall RPC_HDR.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}

	if(!smb_io_rpc_hdr_resp("resp", &hdr_resp, &outgoing_pdu, 0)) {
		DEBUG0(("create_next_pdu_ntlmssp: failed to marshall RPC_HDR_RESP.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
	...
	if(!prs_append_some_prs_data(&outgoing_pdu, &p->out_data.rdata, p->out_data.data_sent_length, data_len)) {
		DEBUG0(("create_next_pdu_ntlmssp: failed to copy %u bytes of data.\n", (unsigned int)data_len));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
	...
	if (ss_padding_len) {
		...
		if (!prs_copy_data_in(&outgoing_pdu, pad, ss_padding_len)) {
			DEBUG0(("create_next_pdu_ntlmssp: failed to add %u bytes of pad data.\n",
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, &outgoing_pdu, 0)) {
		DEBUG0(("create_next_pdu_ntlmssp: failed to marshall RPC_HDR_AUTH.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
	...
	if (!prs_copy_data_in(&outgoing_pdu, (char *)auth_blob.data, NTLMSSP_SIG_SIZE)) {
		DEBUG0(("create_next_pdu_ntlmssp: failed to add %u bytes auth blob.\n",
...
static BOOL create_next_pdu_schannel(pipes_struct *p)
{
	...
	if(!data_len_left) {
		DEBUG0(("create_next_pdu_schannel: no data left to send !\n"));
		return False;
	}
	...
	if(p->out_data.data_sent_length + data_len >= prs_offset(&p->out_data.rdata)) {
		...
		if (data_len_left % 8) {
			ss_padding_len = 8 - (data_len_left % 8);
			DEBUG10(("create_next_pdu_schannel: adding sign/seal padding of %u\n",
				ss_padding_len ));
		}
	}
	...
	/* Store the header in the data stream. */
	if(!smb_io_rpc_hdr("hdr", &p->hdr, &outgoing_pdu, 0)) {
		DEBUG0(("create_next_pdu_schannel: failed to marshall RPC_HDR.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}

	if(!smb_io_rpc_hdr_resp("resp", &hdr_resp, &outgoing_pdu, 0)) {
		DEBUG0(("create_next_pdu_schannel: failed to marshall RPC_HDR_RESP.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
	...
	if(!prs_append_some_prs_data(&outgoing_pdu, &p->out_data.rdata, p->out_data.data_sent_length, data_len)) {
		DEBUG0(("create_next_pdu_schannel: failed to copy %u bytes of data.\n", (unsigned int)data_len));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
	...
	if (ss_padding_len) {
		...
		memset(pad, '\0', 8);
		if (!prs_copy_data_in(&outgoing_pdu, pad, ss_padding_len)) {
			DEBUG0(("create_next_pdu_schannel: failed to add %u bytes of pad data.\n", (unsigned int)ss_padding_len));
			prs_mem_free(&outgoing_pdu);
			return False;
		}
	}

	{
		...
		if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, &outgoing_pdu, 0)) {
			DEBUG0(("create_next_pdu_schannel: failed to marshall RPC_HDR_AUTH.\n"));
			prs_mem_free(&outgoing_pdu);
			return False;
		}
...
static BOOL create_next_pdu_noauth(pipes_struct *p)
{
	...
	if(!data_len_left) {
		DEBUG0(("create_next_pdu_noath: no data left to send !\n"));
		return False;
	}
	...
	/* Store the header in the data stream. */
	if(!smb_io_rpc_hdr("hdr", &p->hdr, &outgoing_pdu, 0)) {
		DEBUG0(("create_next_pdu_noath: failed to marshall RPC_HDR.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}

	if(!smb_io_rpc_hdr_resp("resp", &hdr_resp, &outgoing_pdu, 0)) {
		DEBUG0(("create_next_pdu_noath: failed to marshall RPC_HDR_RESP.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
	...
	if(!prs_append_some_prs_data(&outgoing_pdu, &p->out_data.rdata, p->out_data.data_sent_length, data_len)) {
		DEBUG0(("create_next_pdu_noauth: failed to copy %u bytes of data.\n", (unsigned int)data_len));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
...
BOOL create_next_pdu(pipes_struct *p)
{
	...
	DEBUG0(("create_next_pdu: invalid internal auth level %u / type %u",
...
static BOOL pipe_ntlmssp_verify_final(pipes_struct *p, DATA_BLOB *p_resp_blob)
{
	...
	DEBUG5(("pipe_ntlmssp_verify_final: pipe %s checking user details\n", p->name));
	...
	if (p->auth.auth_level == PIPE_AUTH_LEVEL_INTEGRITY) {
		if (!(a->ntlmssp_state->neg_flags & NTLMSSP_NEGOTIATE_SIGN)) {
			DEBUG0(("pipe_ntlmssp_verify_final: pipe %s : packet integrity requested "
	...
	if (p->auth.auth_level == PIPE_AUTH_LEVEL_PRIVACY) {
		if (!(a->ntlmssp_state->neg_flags & NTLMSSP_NEGOTIATE_SEAL)) {
			DEBUG0(("pipe_ntlmssp_verify_final: pipe %s : packet privacy requested "
	...
	DEBUG5(("pipe_ntlmssp_verify_final: OK: user: %s domain: %s workstation: %s\n",
	...
	if (p->pipe_user.ut.ngroups) {
		if (!(p->pipe_user.ut.groups = memdup(a->server_info->groups,
			...
			DEBUG0(("failed to memdup group list to p->pipe_user.groups\n"));
			return False;
		}
	}
	...
	if (a->server_info->ptok) {
		...
		DEBUG1(("Error: Authmodule failed to provide nt_user_token\n"));
...
BOOL api_pipe_bind_auth3(pipes_struct *p, prs_struct *rpc_in_p)
{
	...
	DEBUG5(("api_pipe_bind_auth3: decode request. %d\n", __LINE__));

	if (p->hdr.auth_len == 0) {
		DEBUG0(("api_pipe_bind_auth3: No auth field sent !\n"));
		goto err;
	}

	/* 4 bytes padding. */
	if (!prs_uint32("pad", rpc_in_p, 0, &pad)) {
		DEBUG0(("api_pipe_bind_auth3: unmarshall of 4 byte pad failed.\n"));
		goto err;
	}
	...
	if(!smb_io_rpc_hdr_auth("", &auth_info, rpc_in_p, 0)) {
		DEBUG0(("api_pipe_bind_auth3: unmarshall of RPC_HDR_AUTH failed.\n"));
		goto err;
	}

	if (auth_info.auth_type != RPC_NTLMSSP_AUTH_TYPE) {
		DEBUG0(("api_pipe_bind_auth3: incorrect auth type (%u).\n",
	...
	if (!prs_copy_data_out((char *)blob.data, rpc_in_p, p->hdr.auth_len)) {
		DEBUG0(("api_pipe_bind_auth3: Failed to pull %u bytes - the response blob.\n",
...
static BOOL setup_bind_nak(pipes_struct *p)
{
	...
	if(!smb_io_rpc_hdr("", &nak_hdr, &outgoing_rpc, 0)) {
		DEBUG0(("setup_bind_nak: marshalling of RPC_HDR failed.\n"));
...
BOOL setup_fault_pdu(pipes_struct *p, NTSTATUS status)
{
	...
	if(!smb_io_rpc_hdr("", &fault_hdr, &outgoing_pdu, 0)) {
		DEBUG0(("setup_fault_pdu: marshalling of RPC_HDR failed.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}

	if(!smb_io_rpc_hdr_resp("resp", &hdr_resp, &outgoing_pdu, 0)) {
		DEBUG0(("setup_fault_pdu: failed to marshall RPC_HDR_RESP.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}

	if(!smb_io_rpc_hdr_fault("fault", &fault_resp, &outgoing_pdu, 0)) {
		DEBUG0(("setup_fault_pdu: failed to marshall RPC_HDR_FAULT.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
...
BOOL setup_cancel_ack_reply(pipes_struct *p, prs_struct *rpc_in_p)
{
	...
	if(!smb_io_rpc_hdr("", &ack_reply_hdr, &outgoing_pdu, 0)) {
		DEBUG0(("setup_cancel_ack_reply: marshalling of RPC_HDR failed.\n"));
		prs_mem_free(&outgoing_pdu);
		return False;
	}
	...
	for ( i=0; pipe_names[i].client_pipe; i++ ) {
		...
		if ( strequal(pipe_names[i].client_pipe, pname)
			...
			if ( !(context_fns = SMB_MALLOC_P(PIPE_RPC_FNS)) ) {
				DEBUG0(("check_bind_req: malloc() failed!\n"));
				return False;
			}
...
NTSTATUS rpc_pipe_register_commands(int version, const char *clnt, const char *srv, const struct api_struct *cmds, int size)
{
	...
	if (version != SMB_RPC_INTERFACE_VERSION) {
		DEBUG0(("Can't register rpc commands!\n"
	...
	rpc_lookup_size++;
        ...
        if (NULL == rpc_entry) {
                rpc_lookup_size--;
                DEBUG0( ("rpc_pipe_register_commands: memory allocation failed\n"));
					...
					RPC_HDR_AUTH *pauth_info, prs_struct *pout_auth)
{
	...
	if (!prs_copy_data_out((char *)blob.data, rpc_in_p, p->hdr.auth_len)) {
		DEBUG0(("pipe_spnego_auth_bind_negotiate: Failed to pull %u bytes - the SPNEGO auth header.\n",
			(unsigned int)p->hdr.auth_len ));
		goto err;
	}
	...
	if (!parse_negTokenTarg(blob, OIDs, &secblob)) {
		DEBUG0(("pipe_spnego_auth_bind_negotiate: Failed to parse the security blob.\n"));
		goto err;
        }
	...
	for (i=0;OIDs[i];i++) {
		DEBUG3(("pipe_spnego_auth_bind_negotiate: Got OID %s\n", OIDs[i]));
		SAFE_FREE(OIDs[i]);
	}
	DEBUG3(("pipe_spnego_auth_bind_negotiate: Got secblob of size %lu\n", (unsigned long)secblob.length));
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED)) {
		DEBUG3(("pipe_spnego_auth_bind_negotiate: auth_ntlmssp_update failed.\n"));
		goto err;
	}
	...
	if(!smb_io_rpc_hdr_auth("", &auth_info, pout_auth, 0)) {
		DEBUG0(("pipe_spnego_auth_bind_negotiate: marshalling of RPC_HDR_AUTH failed.\n"));
		goto err;
	}

	if (!prs_copy_data_in(pout_auth, (char *)response.data, response.length)) {
		DEBUG0(("pipe_spnego_auth_bind_negotiate: marshalling of data blob failed.\n"));
		goto err;
	}
					...
					RPC_HDR_AUTH *pauth_info, prs_struct *pout_auth)
{
	...
	if (p->auth.auth_type != PIPE_AUTH_TYPE_SPNEGO_NTLMSSP || !a) {
		DEBUG0(("pipe_spnego_auth_bind_continue: not in NTLMSSP auth state.\n"));
		goto err;
	}
	...
	if (!prs_copy_data_out((char *)spnego_blob.data, rpc_in_p, p->hdr.auth_len)) {
		DEBUG0(("pipe_spnego_auth_bind_continue: Failed to pull %u bytes - the SPNEGO auth header.\n",
			(unsigned int)p->hdr.auth_len ));
		goto err;
	}

	if (spnego_blob.data[0] != ASN1_CONTEXT(1)) {
		DEBUG0(("pipe_spnego_auth_bind_continue: invalid SPNEGO blob type.\n"));
		goto err;
	}

	if (!spnego_parse_auth(spnego_blob, &auth_blob)) {
		DEBUG0(("pipe_spnego_auth_bind_continue: invalid SPNEGO blob.\n"));
		goto err;
	}
	...
	init_rpc_hdr_auth(&auth_info, RPC_SPNEGO_AUTH_TYPE, pauth_info->auth_level, RPC_HDR_AUTH_LEN, 1);
	if(!smb_io_rpc_hdr_auth("", &auth_info, pout_auth, 0)) {
		DEBUG0(("pipe_spnego_auth_bind_continue: marshalling of RPC_HDR_AUTH failed.\n"));
		goto err;
	}

	if (!prs_copy_data_in(pout_auth, (char *)response.data, response.length)) {
		DEBUG0(("pipe_spnego_auth_bind_continue: marshalling of data blob failed.\n"));
		goto err;
	}
					...
					RPC_HDR_AUTH *pauth_info, prs_struct *pout_auth)
{
	...
	if (!smb_io_rpc_auth_schannel_neg("", &neg, rpc_in_p, 0)) {
		DEBUG0(("pipe_schannel_auth_bind: Could not unmarshal SCHANNEL auth neg\n"));
		return False;
	}
	...
	if (!ret) {
		DEBUG0( ("pipe_schannel_auth_bind: Attempt to bind using schannel without successful serverauth2\n"));
		return False;
	}
	...
	if(!smb_io_rpc_hdr_auth("", &auth_info, pout_auth, 0)) {
		DEBUG0(("pipe_schannel_auth_bind: marshalling of RPC_HDR_AUTH failed.\n"));
		return False;
	}
	...
	if(!smb_io_rpc_schannel_verifier("", &auth_verifier, pout_auth, 0)) {
		DEBUG0(("pipe_schannel_auth_bind: marshalling of RPC_AUTH_VERIFIER failed.\n"));
		return False;
	}
	...
	DEBUG10(("pipe_schannel_auth_bind: schannel auth: domain [%s] myname [%s]\n",
					...
					RPC_HDR_AUTH *pauth_info, prs_struct *pout_auth)
{
	...
	if (!prs_copy_data_out((char *)blob.data, rpc_in_p, p->hdr.auth_len)) {
		DEBUG0(("pipe_ntlmssp_auth_bind: Failed to pull %u bytes - the NTLM auth header.\n",
			(unsigned int)p->hdr.auth_len ));
		goto err;
	}

	if (strncmp((char *)blob.data, "NTLMSSP", 7) != 0) {
		DEBUG0(("pipe_ntlmssp_auth_bind: Failed to read NTLMSSP in blob\n"));
                goto err;
        }
	...
	status = auth_ntlmssp_start(&a);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG0(("pipe_ntlmssp_auth_bind: auth_ntlmssp_start failed: %s\n",
			nt_errstr(status) ));
		goto err;
	}
	...
	if (!NT_STATUS_EQUAL(status, NT_STATUS_MORE_PROCESSING_REQUIRED)) {
		DEBUG0(("pipe_ntlmssp_auth_bind: auth_ntlmssp_update failed: %s\n",
			nt_errstr(status) ));
		goto err;
	}
	...
	if(!smb_io_rpc_hdr_auth("", &auth_info, pout_auth, 0)) {
		DEBUG0(("pipe_ntlmssp_auth_bind: marshalling of RPC_HDR_AUTH failed.\n"));
		goto err;
	}

	if (!prs_copy_data_in(pout_auth, (char *)response.data, response.length)) {
		DEBUG0(("pipe_ntlmssp_auth_bind: marshalling of data blob failed.\n"));
		goto err;
	}
	...
	DEBUG10(("pipe_ntlmssp_auth_bind: NTLMSSP auth started\n"));
...
BOOL api_pipe_bind_req(pipes_struct *p, prs_struct *rpc_in_p)
{
	...
	if (p->pipe_bound) {
		DEBUG2(("api_pipe_bind_req: rejecting bind request on bound pipe %s.\n", p->pipe_srv_name));
	...
	if(!prs_init(&out_hdr_ba, 1024, p->mem_ctx, MARSHALL)) {
		DEBUG0(("api_pipe_bind_req: malloc out_hdr_ba failed.\n"));
		prs_mem_free(&outgoing_rpc);
		return False;
	}

	if(!prs_init(&out_auth, 1024, p->mem_ctx, MARSHALL)) {
		DEBUG0(("api_pipe_bind_req: malloc out_auth failed.\n"));
		prs_mem_free(&outgoing_rpc);
		prs_mem_free(&out_hdr_ba);
		return False;
	}

	DEBUG5(("api_pipe_bind_req: decode request. %d\n", __LINE__));
	...
	for (i = 0; i < rpc_lookup_size; i++) {
			...
			DEBUG3( ("api_pipe_bind_req: \\PIPE\\%s -> \\PIPE\\%s\n",
	...
	if (i == rpc_lookup_size) {
		if (NT_STATUS_IS_ERR(smb_probe_module("rpc", p->name))) {
                       DEBUG3(("api_pipe_bind_req: Unknown pipe name %s in bind request.\n",
                                p->name ));
			prs_mem_free(&outgoing_rpc);
			prs_mem_free(&out_hdr_ba);
		...
		if (i == rpc_lookup_size) {
			DEBUG0( ("module %s doesn't provide functions for pipe %s!\n", p->name, p->name));
			goto err_exit;
		}
	}
	...
	if(!smb_io_rpc_hdr_rb("", &hdr_rb, rpc_in_p, 0))  {
		DEBUG0(("api_pipe_bind_req: unable to unmarshall RPC_HDR_RB struct.\n"));
		goto err_exit;
	}
	...
	DEBUG5(("api_pipe_bind_req: make response. %d\n", __LINE__));
	...
	if (p->hdr.auth_len) {
		...
		if(!smb_io_rpc_hdr_auth("", &auth_info, rpc_in_p, 0)) {
			DEBUG0(("api_pipe_bind_req: unable to unmarshall RPC_HDR_AUTH struct.\n"));
			goto err_exit;
		}
		...
		switch (auth_info.auth_level) {
			...
			case RPC_AUTH_LEVEL_PRIVACY:
				...
				break;
			default:
				DEBUG0(("api_pipe_bind_req: unexpected auth level (%u).\n",
	...
	switch(auth_type) {
		...
		default:
			DEBUG0(("api_pipe_bind_req: unknown auth type %x requested.\n", auth_type ));
			goto err_exit;
	}
	...
	if(!smb_io_rpc_hdr_ba("", &hdr_ba, &out_hdr_ba, 0)) {
		DEBUG0(("api_pipe_bind_req: marshalling of RPC_HDR_BA failed.\n"));
		goto err_exit;
	}
	...
	if(!smb_io_rpc_hdr("", &p->hdr, &outgoing_rpc, 0)) {
		DEBUG0(("api_pipe_bind_req: marshalling of RPC_HDR failed.\n"));
		goto err_exit;
	}
	...
	if(!prs_append_prs_data( &outgoing_rpc, &out_hdr_ba)) {
		DEBUG0(("api_pipe_bind_req: append of RPC_HDR_BA failed.\n"));
		goto err_exit;
	}

	if (auth_len && !prs_append_prs_data( &outgoing_rpc, &out_auth)) {
		DEBUG0(("api_pipe_bind_req: append of auth info failed.\n"));
		goto err_exit;
	}
...
BOOL api_pipe_alter_context(pipes_struct *p, prs_struct *rpc_in_p)
{
	...
	if(!prs_init(&out_hdr_ba, 1024, p->mem_ctx, MARSHALL)) {
		DEBUG0(("api_pipe_alter_context: malloc out_hdr_ba failed.\n"));
		prs_mem_free(&outgoing_rpc);
		return False;
	}

	if(!prs_init(&out_auth, 1024, p->mem_ctx, MARSHALL)) {
		DEBUG0(("api_pipe_alter_context: malloc out_auth failed.\n"));
		prs_mem_free(&outgoing_rpc);
		prs_mem_free(&out_hdr_ba);
		return False;
	}

	DEBUG5(("api_pipe_alter_context: decode request. %d\n", __LINE__));

	/* decode the alter context request */
	if(!smb_io_rpc_hdr_rb("", &hdr_rb, rpc_in_p, 0))  {
		DEBUG0(("api_pipe_alter_context: unable to unmarshall RPC_HDR_RB struct.\n"));
		goto err_exit;
	}
	...
	DEBUG5(("api_pipe_alter_context: make response. %d\n", __LINE__));
	...
	if (p->hdr.auth_len != 0) {
		...
		if(!smb_io_rpc_hdr_auth("", &auth_info, rpc_in_p, 0)) {
			DEBUG0(("api_pipe_alter_context: unable to unmarshall RPC_HDR_AUTH struct.\n"));
			goto err_exit;
		}
	...
	if(!smb_io_rpc_hdr_ba("", &hdr_ba, &out_hdr_ba, 0)) {
		DEBUG0(("api_pipe_alter_context: marshalling of RPC_HDR_BA failed.\n"));
		goto err_exit;
	}
	...
	if(!smb_io_rpc_hdr("", &p->hdr, &outgoing_rpc, 0)) {
		DEBUG0(("api_pipe_alter_context: marshalling of RPC_HDR failed.\n"));
		goto err_exit;
	}
	...
	if(!prs_append_prs_data( &outgoing_rpc, &out_hdr_ba)) {
		DEBUG0(("api_pipe_alter_context: append of RPC_HDR_BA failed.\n"));
		goto err_exit;
	}

	if (auth_len && !prs_append_prs_data( &outgoing_rpc, &out_auth)) {
		DEBUG0(("api_pipe_alter_context: append of auth info failed.\n"));
		goto err_exit;
	}
					...
					uint32 *p_ss_padding_len, NTSTATUS *pstatus)
{
	...
	*pstatus = NT_STATUS_OK;
	...
	if ((auth_len > RPC_MAX_SIGN_SIZE) ||
		...
		DEBUG0(("api_pipe_ntlmssp_auth_process: auth_len %u is too large.\n",
	...
	if(!prs_set_offset(rpc_in, RPC_HDR_REQ_LEN + data_len)) {
		DEBUG0(("api_pipe_ntlmssp_auth_process: cannot move offset to %u.\n",
	...
	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, rpc_in, 0)) {
		DEBUG0(("api_pipe_ntlmssp_auth_process: failed to unmarshall RPC_HDR_AUTH.\n"));
		*pstatus = NT_STATUS_INVALID_PARAMETER;
		return False;
	}
	...
	if(!prs_set_offset(rpc_in, save_offset)) {
		DEBUG0(("api_pipe_auth_process: failed to set offset back to %u\n",
...
BOOL api_pipe_schannel_process(pipes_struct *p, prs_struct *rpc_in, uint32 *p_ss_padding_len)
{
	...
	if (p->hdr.frag_len < RPC_HEADER_LEN + RPC_HDR_REQ_LEN + RPC_HDR_AUTH_LEN + auth_len) {
		DEBUG0(("Incorrect frag %u, auth %u.\n",
	...
	DEBUG5(("data %d auth %d\n", data_len, auth_len));

	if(!prs_set_offset(rpc_in, RPC_HDR_REQ_LEN + data_len)) {
		DEBUG0(("cannot move offset to %u.\n",
			 (unsigned int)RPC_HDR_REQ_LEN + data_len ));
		return False;
	}

	if(!smb_io_rpc_hdr_auth("hdr_auth", &auth_info, rpc_in, 0)) {
		DEBUG0(("failed to unmarshall RPC_HDR_AUTH.\n"));
		return False;
	}

	if (auth_info.auth_type != RPC_SCHANNEL_AUTH_TYPE) {
		DEBUG0(("Invalid auth info %d on schannel\n",
			 auth_info.auth_type));
		return False;
	}

	if(!smb_io_rpc_auth_schannel_chk("", RPC_AUTH_SCHANNEL_SIGN_OR_SEAL_CHK_LEN, &schannel_chk, rpc_in, 0)) {
		DEBUG0(("failed to unmarshal RPC_AUTH_SCHANNEL_CHK.\n"));
		return False;
	}
	...
	if(!prs_set_offset(rpc_in, save_offset)) {
		DEBUG0(("failed to set offset back to %u\n",
			 (unsigned int)save_offset ));
		return False;
	}
...
static PIPE_RPC_FNS* find_pipe_fns_by_context( PIPE_RPC_FNS *list, uint32 context_id )
{
	...
	if ( !list ) {
		DEBUG0(("find_pipe_fns_by_context: ERROR!  No context list for pipe!\n"));
		return NULL;
	}
...
BOOL api_pipe_request(pipes_struct *p)
{
	...
	if ( pipe_fns ) {
		...
		DEBUG0(("api_pipe_request: No rpc function table associated with context [%d] on pipe [%s]\n",
		...
		const struct api_struct *api_rpc_cmds, int n_cmds)
{
	...
	DEBUG4(("api_rpcTNP: %s op 0x%x - ", rpc_name, p->hdr_req.opnum));
	...
	for (fn_num = 0; fn_num < n_cmds; fn_num++) {
		if (api_rpc_cmds[fn_num].opnum == p->hdr_req.opnum && api_rpc_cmds[fn_num].fn != NULL) {
			DEBUG3(("api_rpcTNP: rpc command: %s\n", api_rpc_cmds[fn_num].name));
			break;
		}
	}
	...
	offset1 = prs_offset(&p->out_data.rdata);
        ...
        DEBUG6( ("api_rpc_cmds[%d].fn == %p\n", 
                fn_num, api_rpc_cmds[fn_num].fn));
	/* do the actual command */
	if(!api_rpc_cmds[fn_num].fn(p)) {
		DEBUG0(("api_rpcTNP: %s: %s failed.\n", rpc_name, api_rpc_cmds[fn_num].name));
		prs_mem_free(&p->out_data.rdata);
		return False;
	}

	if (p->bad_handle_fault_state) {
		DEBUG4(("api_rpcTNP: bad handle fault return.\n"));
	...
	DEBUG5(("api_rpcTNP: called %s successfully\n", rpc_name));
	...
	if ((DEBUGLEVEL >= 10) && 
		...
		DEBUG10( ("api_rpcTNP: rpc input buffer underflow (parse error?)\n"));
		if (data) {
...
#endif
		default:
			DEBUG0(("get_pipe_fns: Unknown pipe index! [%d]\n", idx));
	}
