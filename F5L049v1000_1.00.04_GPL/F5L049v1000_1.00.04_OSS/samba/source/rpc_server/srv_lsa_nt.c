			...
			uint32 *pmapped_count)
{
	...
	*pmapped_count = 0;
	...
	for (i = 0; i < num_entries; i++) {
		...
		if (full_name == NULL) {
			DEBUG0( ("pull_ucs2_talloc failed\n"));
			return NT_STATUS_NO_MEMORY;
		}

		DEBUG5( ("lookup_lsa_rids: looking up name %s\n", full_name));
		...
		case SID_NAME_WKN_GRP:
			DEBUG5( ("init_lsa_rids: %s found\n", full_name));
			/* Leave these unchanged */
			break;
		default:
			/* Don't hand out anything but the list above */
			DEBUG5( ("init_lsa_rids: %s not found\n", full_name));
			...
			uint32 *pmapped_count)
{
	...
	*pmapped_count = 0;
	...
	for (i = 0; i < num_entries; i++) {
		...
		DEBUG5( ("init_lsa_sids: looking up name %s\n", full_name));
		...
		case SID_NAME_ALIAS:
		case SID_NAME_WKN_GRP:
			DEBUG5( ("init_lsa_sids: %s found\n", full_name));
			/* Leave these unchanged */
			break;
		default:
			/* Don't hand out anything but the list above */
			DEBUG5( ("init_lsa_sids: %s not found\n", full_name));
			type = SID_NAME_UNKNOWN;
			break;
		}
...
NTSTATUS _lsa_open_policy2(pipes_struct *p, LSA_Q_OPEN_POL2 *q_u, LSA_R_OPEN_POL2 *r_u)
{
	...
	if(!se_access_check(psd, p->pipe_user.nt_user_token, des_access, &acc_granted, &status)) {
		...
		DEBUG4(("ACCESS should be DENIED (granted: %#010x;  required: %#010x)\n",
...
NTSTATUS _lsa_query_info(pipes_struct *p, LSA_Q_QUERY_INFO *q_u, LSA_R_QUERY_INFO *r_u)
{
	...
	case 0x02:
		...
		if (!(handle->access & POLICY_VIEW_AUDIT_INFORMATION)) {
			DEBUG10(("_lsa_query_info: insufficient access rights\n"));
			return NT_STATUS_ACCESS_DENIED;
		}
	...
	case 0x06:
		...
		break;
	default:
		DEBUG0(("_lsa_query_info: unknown info level in Lsa Query: %d\n", q_u->info_class));
	...
	*pp_ref = NULL;
	...
	for (i=0; i<MAX_REF_DOMAINS; i++) {
		...
		if (init_dom_ref(ref, dom_infos[i].name,
				 &dom_infos[i].sid) != i) {
			DEBUG0( ("Domain %s mentioned twice??\n",
				  dom_infos[i].name));
	...
	DEBUG10( ("num_sids %d, mapped_count %d, status %s\n",
			  ...
			  LSA_R_LOOKUP_SIDS *r_u)
{
	...
	if (num_sids >  MAX_LOOKUP_SIDS) {
		DEBUG5(("_lsa_lookup_sids: limit of %d exceeded, requested %d\n",
			  ...
			  LSA_R_LOOKUP_SIDS2 *r_u)
{
	...
	if (num_sids >  MAX_LOOKUP_SIDS) {
		DEBUG5(("_lsa_lookup_sids2: limit of %d exceeded, requested %d\n",
			 MAX_LOOKUP_SIDS, num_sids));
		return NT_STATUS_NONE_MAPPED;
	}
			  ...
			  LSA_R_LOOKUP_SIDS3 *r_u)
{
	...
	if (p->auth.auth_type != PIPE_AUTH_TYPE_SCHANNEL) {
		DEBUG0(("_lsa_lookup_sids3: client %s not using schannel for netlogon\n",
			get_remote_machine_name() ));
		return NT_STATUS_INVALID_PARAMETER;
	}

	if (num_sids >  MAX_LOOKUP_SIDS) {
		DEBUG5(("_lsa_lookup_sids3: limit of %d exceeded, requested %d\n",
			 MAX_LOOKUP_SIDS, num_sids));
		return NT_STATUS_NONE_MAPPED;
	}
...
NTSTATUS _lsa_lookup_names(pipes_struct *p,LSA_Q_LOOKUP_NAMES *q_u, LSA_R_LOOKUP_NAMES *r_u)
{
	...
	if (num_entries >  MAX_LOOKUP_SIDS) {
		...
		DEBUG5(("_lsa_lookup_names: truncating name lookup list to %d\n", num_entries));
	}
...
NTSTATUS _lsa_lookup_names2(pipes_struct *p, LSA_Q_LOOKUP_NAMES2 *q_u, LSA_R_LOOKUP_NAMES2 *r_u)
{
	...
	if (num_entries >  MAX_LOOKUP_SIDS) {
		num_entries = MAX_LOOKUP_SIDS;
		DEBUG5(("_lsa_lookup_names2: truncating name lookup list to %d\n", num_entries));
	}
...
NTSTATUS _lsa_lookup_names3(pipes_struct *p, LSA_Q_LOOKUP_NAMES3 *q_u, LSA_R_LOOKUP_NAMES3 *r_u)
{
	...
	if (num_entries >  MAX_LOOKUP_SIDS) {
		num_entries = MAX_LOOKUP_SIDS;
		DEBUG5(("_lsa_lookup_names3: truncating name lookup list to %d\n", num_entries));
	}
...
NTSTATUS _lsa_lookup_names4(pipes_struct *p, LSA_Q_LOOKUP_NAMES4 *q_u, LSA_R_LOOKUP_NAMES4 *r_u)
{
	...
	if (num_entries >  MAX_LOOKUP_SIDS) {
		num_entries = MAX_LOOKUP_SIDS;
		DEBUG5(("_lsa_lookup_names4: truncating name lookup list to %d\n", num_entries));
	}
	...
	/* No policy handle on this call. Restrict to crypto connections. */
	if (p->auth.auth_type != PIPE_AUTH_TYPE_SCHANNEL) {
		DEBUG0(("_lsa_lookup_names4: client %s not using schannel for netlogon\n",
			get_remote_machine_name() ));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
NTSTATUS _lsa_enum_privs(pipes_struct *p, LSA_Q_ENUM_PRIVS *q_u, LSA_R_ENUM_PRIVS *r_u)
{
	...
	DEBUG10(("_lsa_enum_privs: enum_context:%d total entries:%d\n", 
...
NTSTATUS _lsa_priv_get_dispname(pipes_struct *p, LSA_Q_PRIV_GET_DISPNAME *q_u, LSA_R_PRIV_GET_DISPNAME *r_u)
{
	...
	DEBUG10(("_lsa_priv_get_dispname: name = %s\n", name_asc));

	description = get_privilege_dispname( name_asc );
	
	if ( description ) {
		DEBUG10(("_lsa_priv_get_dispname: display name = %s\n", description));
		...
		DEBUG10(("_lsa_priv_get_dispname: doesn't exist\n"));
...
NTSTATUS _lsa_enum_privsaccount(pipes_struct *p, prs_struct *ps, LSA_Q_ENUMPRIVSACCOUNT *q_u, LSA_R_ENUMPRIVSACCOUNT *r_u)
{
	...
	if ( se_priv_to_privilege_set( &privileges, &mask ) ) {
		...
		DEBUG10(("_lsa_enum_privsaccount: %s has %d privileges\n", 
...
NTSTATUS _lsa_addprivs(pipes_struct *p, LSA_Q_ADDPRIVS *q_u, LSA_R_ADDPRIVS *r_u)
{
	...
	if ( !grant_privilege( &info->sid, &mask ) ) {
		DEBUG3(("_lsa_addprivs: grant_privilege(%s) failed!\n",
			sid_string_static(&info->sid) ));
		DEBUG3(("Privilege mask:\n"));
...
NTSTATUS _lsa_removeprivs(pipes_struct *p, LSA_Q_REMOVEPRIVS *q_u, LSA_R_REMOVEPRIVS *r_u)
{
	...
	if ( !revoke_privilege( &info->sid, &mask ) ) {
		DEBUG3(("_lsa_removeprivs: revoke_privilege(%s) failed!\n",
			sid_string_static(&info->sid) ));
		DEBUG3(("Privilege mask:\n"));
		dump_se_priv( DBGC_ALL, 3, &mask );
		return NT_STATUS_NO_SUCH_PRIVILEGE;
...
NTSTATUS _lsa_query_info2(pipes_struct *p, LSA_Q_QUERY_INFO2 *q_u, LSA_R_QUERY_INFO2 *r_u)
{
	...
	case 0x0c:
		...
		break;
	default:
		DEBUG0(("_lsa_query_info2: unknown info level in Lsa Query: %d\n", q_u->info_class));
		r_u->status = NT_STATUS_INVALID_INFO_CLASS;
		break;
	}
...
NTSTATUS _lsa_add_acct_rights(pipes_struct *p, LSA_Q_ADD_ACCT_RIGHTS *q_u, LSA_R_ADD_ACCT_RIGHTS *r_u)
{
	...
	if ( q_u->count != uni_privnames->count ) {
		DEBUG0(("_lsa_add_acct_rights: count != number of UNISTR2 elements!\n"));
		return NT_STATUS_INVALID_HANDLE;	
	}
	...
	for ( i=0; i<q_u->count; i++ ) {
		...
		if ( !grant_privilege_by_name( &sid, privname ) ) {
			DEBUG2(("_lsa_add_acct_rights: Failed to add privilege [%s]\n", privname ));
			return NT_STATUS_NO_SUCH_PRIVILEGE;
		}
	}
...
NTSTATUS _lsa_remove_acct_rights(pipes_struct *p, LSA_Q_REMOVE_ACCT_RIGHTS *q_u, LSA_R_REMOVE_ACCT_RIGHTS *r_u)
{
	...
	for ( i=0; i<q_u->count; i++ ) {
		...
		if ( !revoke_privilege_by_name( &sid, privname ) ) {
			DEBUG2(("_lsa_remove_acct_rights: Failed to revoke privilege [%s]\n", privname ));
			return NT_STATUS_NO_SUCH_PRIVILEGE;
		}
	}
...
NTSTATUS _lsa_enum_acct_rights(pipes_struct *p, LSA_Q_ENUM_ACCT_RIGHTS *q_u, LSA_R_ENUM_ACCT_RIGHTS *r_u)
{
	...
	if ( se_priv_to_privilege_set( &privileges, &mask ) ) {
		...
		DEBUG10(("_lsa_enum_acct_rights: %s has %d privileges\n", 
...
NTSTATUS _lsa_lookup_priv_value(pipes_struct *p, LSA_Q_LOOKUP_PRIV_VALUE *q_u, LSA_R_LOOKUP_PRIV_VALUE *r_u)
{
	...
	DEBUG10(("_lsa_lookup_priv_value: name = %s\n", name));
