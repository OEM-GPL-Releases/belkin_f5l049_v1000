...
WERROR _dfs_Add(pipes_struct *p, NETDFS_Q_DFS_ADD* q_u, NETDFS_R_DFS_ADD *r_u)
{
	...
	if (user.ut.uid != 0) {
		DEBUG10(("_dfs_add: uid != 0. Access denied.\n"));
	...
	DEBUG5(("init_reply_dfs_add: Request to add %s -> %s\\%s.\n",
	...
	if(jn.referral_list == NULL) {
		DEBUG0(("init_reply_dfs_add: talloc failed for referral list!\n"));
                   ...
                   NETDFS_R_DFS_REMOVE *r_u)
{
	...
	if (user.ut.uid != 0) {
		DEBUG10(("_dfs_remove: uid != 0. Access denied.\n"));
		return WERR_ACCESS_DENIED;
	}
	...
	DEBUG5(("init_reply_dfs_remove: Request to remove %s -> %s\\%s.\n",
		dfspath, servername, sharename));
	...
	if(!q_u->ptr0_server && !q_u->ptr0_share) {
		...
		DEBUG10(("altpath: .%s. refcnt: %d\n", altpath, jn.referral_count));
		for(i=0;i<jn.referral_count;i++) {
			pstring refpath;
			pstrcpy(refpath,jn.referral_list[i].alternate_path);
			trim_char(refpath, '\\', '\\');
			DEBUG10(("_dfs_remove:  refpath: .%s.\n", refpath));
			if(strequal(refpath, altpath)) {
				*(jn.referral_list[i].alternate_path)='\0';
				DEBUG10(("_dfs_remove: Removal request matches referral %s\n",
					refpath));
				found = True;
			}
		}
...
static BOOL init_reply_dfs_info_1(struct junction_map* j, NETDFS_DFS_INFO1* dfs1)
{
	...
	DEBUG5(("init_reply_dfs_info_1: initing entrypath: %s\n",str));
...
static BOOL init_reply_dfs_info_3(TALLOC_CTX *ctx, struct junction_map* j, NETDFS_DFS_INFO3* dfs3)
{
	...
	for(ii=0;ii<j->referral_count;ii++) {
		...
		p = strrchr_m(path,'\\');
		if(p==NULL) {
			DEBUG4(("init_reply_dfs_info_3: invalid path: no \\ found in %s\n",path));
			continue;
		}
		*p = '\0';
		DEBUG5(("storage %d: %s.%s\n",ii,path,p+1));
...
WERROR _dfs_Enum(pipes_struct *p, NETDFS_Q_DFS_ENUM *q_u, NETDFS_R_DFS_ENUM *r_u)
{
	...
	DEBUG5(("_dfs_Enum: %d junctions found in Dfs, doing level %d\n", num_jn, level));
