...
static void send_sync_message(void)
{
        ...
        if (!tdb) {
                DEBUG3( ("send_sync_message(): failed to open connections "
                          "database\n"));
                return;
        }

        DEBUG3( ("sending sam synchronisation message\n"));
...
NTSTATUS _net_logon_ctrl2(pipes_struct *p, NET_Q_LOGON_CTRL2 *q_u, NET_R_LOGON_CTRL2 *r_u)
{
	...
	switch ( q_u->function_code ) {
		...
		default:
			...
			DEBUG0(("_net_logon_ctrl2: unimplemented function level [%d]\n",
				q_u->function_code));
	}
...
static NTSTATUS get_md4pw(char *md4pw, char *mach_acct, uint16 sec_chan_type)
{
	...
	{
		DEBUG0(("get_md4pw: Workstation %s denied access to domain\n", mach_acct));
		return False;
	}
#endif /* 0 */
	...
	unbecome_root();
 	...
 	if (!ret) {
 		DEBUG0(("get_md4pw: Workstation %s: no account in domain\n", mach_acct));
		TALLOC_FREE(sampass);
	...
	if (acct_ctrl & ACB_DISABLED) {
		DEBUG0(("get_md4pw: Workstation %s: account is disabled\n", mach_acct));
		TALLOC_FREE(sampass);
	...
	{
		DEBUG0(("get_md4pw: Workstation %s: account is not a trust account\n", mach_acct));
		TALLOC_FREE(sampass);
	...
	switch (sec_chan_type) {
		case SEC_CHAN_BDC:
			if (!(acct_ctrl & ACB_SVRTRUST)) {
				DEBUG0(("get_md4pw: Workstation %s: BDC secure channel requested "
		...
		case SEC_CHAN_WKSTA:
			if (!(acct_ctrl & ACB_WSTRUST)) {
				DEBUG0(("get_md4pw: Workstation %s: WORKSTATION secure channel requested "
		...
		case SEC_CHAN_DOMAIN:
			if (!(acct_ctrl & ACB_DOMTRUST)) {
				DEBUG0(("get_md4pw: Workstation %s: DOMAIN secure channel requested "
	...
	if ((pass = pdb_get_nt_passwd(sampass)) == NULL) {
		DEBUG0(("get_md4pw: Workstation %s: account does not have a password\n", mach_acct));
		TALLOC_FREE(sampass);
...
NTSTATUS _net_req_chal(pipes_struct *p, NET_Q_REQ_CHAL *q_u, NET_R_REQ_CHAL *r_u)
{
	if (!p->dc) {
		...
		DEBUG10(("_net_req_chal: new challenge requested. Clearing old state.\n"));
		ZERO_STRUCTP(p->dc);
	}
...
NTSTATUS _net_auth(pipes_struct *p, NET_Q_AUTH *q_u, NET_R_AUTH *r_u)
{
	...
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG0(("_net_auth: creds_server_check failed. Failed to "
	...
	if (!creds_server_check(p->dc, &q_u->clnt_chal)) {
		DEBUG0(("_net_auth: creds_server_check failed. Rejecting auth "
...
NTSTATUS _net_auth_2(pipes_struct *p, NET_Q_AUTH_2 *q_u, NET_R_AUTH_2 *r_u)
{
	...
	if (!p->dc || !p->dc->challenge_sent) {
		DEBUG0(("_net_auth2: no challenge sent to client %s\n",
			remote_machine ));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	if ( (lp_server_schannel() == True) &&
		...
		DEBUG0(("_net_auth2: schannel required but client failed "
	...
	status = get_md4pw((char *)p->dc->mach_pw, mach_acct, q_u->clnt_id.sec_chan);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG0(("_net_auth2: failed to get machine password for "
			"account %s: %s\n",
	...
	/* Check client credentials are valid. */
	if (!creds_server_check(p->dc, &q_u->clnt_chal)) {
		DEBUG0(("_net_auth2: creds_server_check failed. Rejecting auth "
			"request from client %s machine account %s\n",
			remote_machine, mach_acct ));
		return NT_STATUS_ACCESS_DENIED;
	}
...
NTSTATUS _net_srv_pwset(pipes_struct *p, NET_Q_SRV_PWSET *q_u, NET_R_SRV_PWSET *r_u)
{
	...
	if ( (lp_server_schannel() == True) && (p->auth.auth_type != PIPE_AUTH_TYPE_SCHANNEL) ) {
		...
		DEBUG0(("_net_srv_pwset: client %s not using schannel for netlogon\n",
			remote_machine ));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	DEBUG3(("_net_srv_pwset: Server Password Set by remote machine:[%s] on account [%s]\n",
			remote_machine, p->dc->mach_acct));
	
	/* Step the creds chain forward. */
	if (!creds_server_step(p->dc, &q_u->clnt_id.cred, &cred_out)) {
		DEBUG2(("_net_srv_pwset: creds_server_step failed. Rejecting auth "
			"request from client %s machine account %s\n",
			remote_machine, p->dc->mach_acct ));
...
NTSTATUS _net_sam_logoff(pipes_struct *p, NET_Q_SAM_LOGOFF *q_u, NET_R_SAM_LOGOFF *r_u)
{
	...
	if ( (lp_server_schannel() == True) && (p->auth.auth_type != PIPE_AUTH_TYPE_SCHANNEL) ) {
		/* 'server schannel = yes' should enforce use of
		   schannel, the client did offer it in auth2, but
		   obviously did not use it. */
		DEBUG0(("_net_sam_logoff: client %s not using schannel for netlogon\n",
	...
	if (!creds_server_step(p->dc, &q_u->sam_id.client.cred, &r_u->srv_creds)) {
		DEBUG2(("_net_sam_logoff: creds_server_step failed. Rejecting auth "
			"request from client %s machine account %s\n",
			remote_machine, p->dc->mach_acct ));
		return NT_STATUS_INVALID_PARAMETER;
	}
					...
					BOOL process_creds)
{
	...
	if ( (lp_server_schannel() == True) && (p->auth.auth_type != PIPE_AUTH_TYPE_SCHANNEL) ) {
		/* 'server schannel = yes' should enforce use of
		   schannel, the client did offer it in auth2, but
		   obviously did not use it. */
		DEBUG0(("_net_sam_logon_internal: client %s not using schannel for netlogon\n",
			get_remote_machine_name() ));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	if (q_u->validation_level != 2 && q_u->validation_level != 3) {
		DEBUG0(("_net_sam_logon: bad validation_level value %d.\n", (int)q_u->validation_level ));
		return NT_STATUS_ACCESS_DENIED;
	}
	...
	if (process_creds) {
		...
		*/
		...
		/* checks and updates credentials.  creates reply credentials */
		if (!creds_server_step(p->dc, &q_u->sam_id.client.cred,  &r_u->srv_creds)) {
			DEBUG2(("_net_sam_logon: creds_server_step failed. Rejecting auth "
				"request from client %s machine account %s\n",
				remote_machine, p->dc->mach_acct ));
			return NT_STATUS_INVALID_PARAMETER;
		}
	...
	case INTERACTIVE_LOGON_TYPE:
		...
		DEBUG3(("SAM Logon (Interactive). Domain:[%s].  ", lp_workgroup()));
		break;
	case NET_LOGON_TYPE:
		...
		DEBUG3(("SAM Logon (Network). Domain:[%s].  ", lp_workgroup()));
		break;
	default:
		DEBUG2(("SAM Logon: unsupported switch value\n"));
	...
	DEBUG3(("User:[%s@%s] Requested Domain:[%s]\n", nt_username, nt_workstation, nt_domain));
	fstrcpy(current_user_info.smb_name, nt_username);
	sub_set_smb_name(nt_username);
     
	DEBUG5(("Attempting validation level %d for unmapped username %s.\n", q_u->sam_id.ctr->switch_value, nt_username));
	...
	DEBUG5( ("_net_sam_logon: check_password returned status %s\n", 
		  nt_errstr(status)));
	...
	if (server_info->guest) {
		...
		DEBUG5(("_net_sam_logon: Attempted domain logon as GUEST "
			 "denied.\n"));
		TALLOC_FREE(server_info);
		return NT_STATUS_LOGON_FAILURE;
	}
    ...
    
	{
		...
		if ((user_sid == NULL) || (group_sid == NULL)) {
			DEBUG1( ("_net_sam_logon: User without group or user SID\n"));
			return NT_STATUS_UNSUCCESSFUL;
		}
		...
		if (!sid_peek_check_rid(&domain_sid, group_sid, &group_rid)) {
			DEBUG1( ("_net_sam_logon: user %s\\%s has user sid "
				  "%s\n but group sid %s.\n"
...
NTSTATUS _net_sam_logon_ex(pipes_struct *p, NET_Q_SAM_LOGON_EX *q_u, NET_R_SAM_LOGON_EX *r_u)
{
	...
	if (p->auth.auth_type != PIPE_AUTH_TYPE_SCHANNEL) {
		DEBUG0(("_net_sam_logon_ex: client %s not using schannel for netlogon\n",
			get_remote_machine_name() ));
		return NT_STATUS_INVALID_PARAMETER;
        }
