...
static BOOL elog_check_access( EVENTLOG_INFO *info, NT_USER_TOKEN *token )
{
	...
	if ( !sec_desc ) {
		DEBUG5(("elog_check_access: Unable to get NT ACL for %s\n", 
			tdbname));
		return False;
	}
	...
	if ( geteuid() == sec_initial_uid() ) {
		DEBUG5(("elog_check_access: using root's token\n"));
	...
	if ( !ret ) {
		DEBUG8(("elog_check_access: se_access_check() return %s\n",
			nt_errstr( ntstatus)));
		return False;
	}
...
static NTSTATUS elog_open( pipes_struct * p, const char *logname, POLICY_HND *hnd )
{
	...
	if ( !get_oldest_entry_hook( elog ) ) {
		DEBUG3(("elog_open: Successfully opened eventlog but can't "
...
{
	if ( !info || !info->etdb ) {
		DEBUG0(("elog_size: Invalid info* structure!\n"));
		return 0;
	}
				     ...
				     int recno, Eventlog_entry * ee )
{
	...
	DEBUG10( ( "Unpacking record %d, size is %d\n", srecno, len ) );
	...
	DEBUG10( ( "get_eventlog_record: read back %d\n", len ) );
	DEBUG10(
...
static BOOL sync_eventlog_params( EVENTLOG_INFO *info )
{
	...
	DEBUG4( ( "sync_eventlog_params with %s\n", elogname ) );

	if ( !info->etdb ) {
		DEBUG4( ( "No open tdb! (%s)\n", info->logname ) );
		return False;
	}
	...
	if ( !( values = TALLOC_ZERO_P( keyinfo, REGVAL_CTR ) ) ) {
		TALLOC_FREE( keyinfo );
		DEBUG0( ( "control_eventlog_hook: talloc() failed!\n" ) );
				...
				EVENTLOG_R_OPEN_EVENTLOG * r_u )
{
	...
	DEBUG10(("_eventlog_open_eventlog: Server [%s], Log [%s]\n",
		servername, logname ));
	...
	if ( !(info = find_eventlog_info_by_hnd( p, &r_u->handle )) ) {
		DEBUG0(("_eventlog_open_eventlog: eventlog (%s) opened but unable to find handle!\n",
			logname ));
		elog_close( p, &r_u->handle );
		return NT_STATUS_INVALID_HANDLE;
	}

	DEBUG10(("_eventlog_open_eventlog: Size [%d]\n", elog_size( info )));
				 ...
				 EVENTLOG_R_CLEAR_EVENTLOG * r_u )
{
	...
	if ( q_u->backupfile.string ) {
		...
		DEBUG8(( "_eventlog_clear_eventlog: Using [%s] as the backup "
			"file name for log [%s].",
				...
				EVENTLOG_R_READ_EVENTLOG * r_u )
{
	...
	{
		DEBUG3(("_eventlog_read_eventlog: Invalid flags [0x%x] for ReadEventLog\n", q_u->flags));
	...
	while ( bytes_left > 0 ) {
		...
		DEBUG10( ( "_eventlog_read_eventlog: read [%d] records for a total "
