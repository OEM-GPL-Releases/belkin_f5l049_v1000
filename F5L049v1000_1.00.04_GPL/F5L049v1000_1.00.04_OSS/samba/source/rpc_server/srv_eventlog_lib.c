...
TDB_CONTEXT *elog_init_tdb( char *tdbfilename )
{
	...
	DEBUG10(("elog_init_tdb: Initializing eventlog tdb (%s)\n",
		tdbfilename));
			     ...
			     BOOL whack_by_date )
{
	...
	for ( i = start_record; i < end_record; i++ ) {
		...
		if (len == -1) {
			DEBUG10(("make_way_for_eventlogs: tdb_unpack failed.\n"));
			tdb_unlock_bystring( the_tdb, EVT_NEXT_RECORD );
			return False;
		}
...
ELOG_TDB *elog_open_tdb( char *logname, BOOL force_clear )
{
	...
	DEBUG7(("elog_open_tdb: Opening %s...(force_clear == %s)\n", 
	...
	if ( !force_clear ) {
		...
		if ( tdb ) {
			...
			if ( vers_id != EVENTLOG_DATABASE_VERSION_V1 ) {
				DEBUG1(("elog_open_tdb: Invalid version [%d] on file [%s].\n",
					vers_id, tdbpath));
				tdb_close( tdb );
	...
	if ( tdb ) {
		...
		if ( !(tdb_node = TALLOC_ZERO_P( NULL, ELOG_TDB)) ) {
			DEBUG0(("elog_open_tdb: talloc() failure!\n"));
			tdb_close( tdb );
			return NULL;
		}
...
int write_eventlog_tdb( TDB_CONTEXT * the_tdb, Eventlog_entry * ee )
{
	...
	if ( !can_write_to_eventlog( the_tdb, ee->record.length ) ) {
		DEBUG3( ( "Can't write to Eventlog, no room \n" ) );
	...
	/*DEBUG3(("write_eventlog_tdb: packed into  %d\n",n_packed)); */
	...
	if ( tdb_store( the_tdb, kbuf, ebuf, 0 ) ) {
		/* DEBUG1(("write_eventlog_tdb: Can't write record %d to eventlog\n",next_record)); */
		tdb_unlock_bystring( the_tdb, EVT_NEXT_RECORD );
		talloc_destroy( mem_ctx );
		return 0;
	}
...
void fixup_eventlog_entry( Eventlog_entry * ee )
{
	...
	/* DEBUG10( ("sid_padding is [%d].\n", ee->data_record.sid_padding)); */
	/* DEBUG10( ("data_padding is [%d].\n", ee->data_record.data_padding)); */
...
BOOL parse_logentry( char *line, Eventlog_entry * entry, BOOL * eor )
{
	...
	DEBUG6( ( "parse_logentry: trying to parse [%s].\n", line ) );
...
*/
		...
		DEBUG10( ( "Unknown entry [%s]. Ignoring.\n", line ) );
