					  ...
					  const char *debug )
{
	...
	if ( rights && user_has_any_privilege( token, rights ) ) {
		...
		DEBUG4(("access_check_samr_object: user rights access mask [0x%x]\n",
			rights_mask));
	}
	...
	if ( geteuid() == sec_initial_uid() ) {
		...
		DEBUG4(("%s: ACCESS should be DENIED  (requested: %#010x)\n", debug, des_access));
	...
	*acc_granted |= rights_mask;
	...
	DEBUG4(("%s: access %s (requested: 0x%08x, granted: 0x%08x)\n", 
...
{
	DEBUG5(("%s: access check ((granted: %#010x;  required: %#010x)\n",  
	...
	if (geteuid() == sec_initial_uid()) {
		...
		DEBUG4(("%s: ACCESS should be DENIED (granted: %#010x;  required: %#010x)\n",
			debug, acc_granted, acc_required));
	...
	DEBUG2(("%s: ACCESS DENIED (granted: %#010x;  required: %#010x)\n", 
		debug, acc_granted, acc_required));
...
static struct samr_info *get_samr_info_by_sid(DOM_SID *psid)
{
	...
	DEBUG10(("get_samr_info_by_sid: created new info for sid %s\n", sid_str));
	if (psid) {
		sid_copy( &info->sid, psid);
		info->builtin_domain = sid_check_is_builtin(psid);
	} else {
		DEBUG10(("get_samr_info_by_sid: created new info for NULL sid.\n"));
...
{
	DEBUG10(("free_samr_cache: deleting cache for SID %s\n", sid_str));
					...
					time_t ev_now)
{
	...
	DEBUG10(("disp_info_cache_idle_timeout_handler: caching timed out for SID %s at %u\n",
...
static void set_disp_info_cache_timeout(DISP_INFO *disp_info, time_t secs_fromnow)
{
	...
	DEBUG10(("set_disp_info_cache_timeout: caching enumeration for SID %s for %u seconds\n",
...
static void force_flush_samr_cache(DISP_INFO *disp_info)
{
	if (disp_info) {
		...
		if (disp_info->di_cache_timeout_event) {
			smb_unregister_idle_event(disp_info->di_cache_timeout_event);
			disp_info->di_cache_timeout_event = (smb_event_id_t)0;
			DEBUG10(("force_flush_samr_cache: clearing idle event for SID %s\n",
				sid_str));
		}
		free_samr_cache(disp_info, sid_str);
	}
...
NTSTATUS _samr_get_usrdom_pwinfo(pipes_struct *p, SAMR_Q_GET_USRDOM_PWINFO *q_u, SAMR_R_GET_USRDOM_PWINFO *r_u)
{
	...
	DEBUG5(("_samr_get_usrdom_pwinfo: %d\n", __LINE__));
...
NTSTATUS _samr_query_sec_obj(pipes_struct *p, SAMR_Q_QUERY_SEC_OBJ *q_u, SAMR_R_QUERY_SEC_OBJ *r_u)
{
	...
	DEBUG10(("_samr_query_sec_obj: querying security on SID: %s\n", sid_to_string(str_sid, &pol_sid)));
	...
	if (pol_sid.sid_rev_num == 0) {
		DEBUG5(("_samr_query_sec_obj: querying security on SAM\n"));
		...
		DEBUG5(("_samr_query_sec_obj: querying security on Domain with SID: %s\n", sid_to_string(str_sid, &pol_sid)));
		...
		DEBUG5(("_samr_query_sec_obj: querying security on Builtin Domain with SID: %s\n", sid_to_string(str_sid, &pol_sid)));
		r_u->status = make_samr_object_sd(p->mem_ctx, &psd, &sd_size, &dom_generic_mapping, NULL, 0);
		...
		DEBUG10(("_samr_query_sec_obj: querying security on Object with SID: %s\n", sid_to_string(str_sid, &pol_sid)));
					 ...
					 struct samr_displayentry *entries)
{
	...
	*uni_name_pp = NULL;
	...
	if (sam == NULL || uni_name == NULL) {
		DEBUG0( ("make_user_sam_entry_list: TALLOC_ZERO failed!\n"));
		return NT_STATUS_NO_MEMORY;
	}
			      ...
			      SAMR_R_ENUM_DOM_USERS *r_u)
{
	...
	DEBUG5(("_samr_enum_dom_users: %d\n", __LINE__));

	if (info->builtin_domain) {
		/* No users in builtin. */
		init_samr_r_enum_dom_users(r_u, q_u->start_idx, 0);
		DEBUG5(("_samr_enum_dom_users: No users in BUILTIN\n"));
		return r_u->status;
	}
	...
	if (num_account == 0) {
		DEBUG5( ("_samr_enum_dom_users: enumeration handle over "
			  "total entries\n"));
		init_samr_r_enum_dom_users(r_u, q_u->start_idx, 0);
		return NT_STATUS_OK;
	}
				      ...
				      struct samr_displayentry *entries)
{
	...
	*uni_name_pp = NULL;
	...
	if (sam == NULL || uni_name == NULL) {
		DEBUG0( ("NULL pointers in SAMR_R_QUERY_DISPINFO\n"));
		return;
	}
...
NTSTATUS _samr_enum_dom_groups(pipes_struct *p, SAMR_Q_ENUM_DOM_GROUPS *q_u, SAMR_R_ENUM_DOM_GROUPS *r_u)
{
	...
	DEBUG5(("samr_reply_enum_dom_groups: %d\n", __LINE__));

	if (info->builtin_domain) {
		/* No groups in builtin. */
		init_samr_r_enum_dom_groups(r_u, q_u->start_idx, 0);
		DEBUG5(("_samr_enum_dom_users: No groups in BUILTIN\n"));
		return r_u->status;
	}
...
NTSTATUS _samr_enum_dom_aliases(pipes_struct *p, SAMR_Q_ENUM_DOM_ALIASES *q_u, SAMR_R_ENUM_DOM_ALIASES *r_u)
{
	...
	DEBUG5(("samr_reply_enum_dom_aliases: sid %s\n",
	...
	DEBUG5(("samr_enum_dom_aliases: %d\n", __LINE__));
			      ...
			      SAMR_R_QUERY_DISPINFO *r_u)
{
	...
	DEBUG5( ("samr_reply_query_dispinfo: %d\n", __LINE__));
	...
	if ((q_u->switch_level < 1) || (q_u->switch_level > 5)) {
		DEBUG0(("_samr_query_dispinfo: Unknown info level (%u)\n",
	...
	if(max_entries > max_sam_entries) {
		DEBUG5( ("samr_reply_query_dispinfo: client requested %d "
	...
	if (temp_size>max_size) {
		...
		DEBUG5( ("samr_reply_query_dispinfo: buffer size limits to "
			  "only %d entries\n", max_entries));
	}
	...
	case 0x4:
		if (info->disp_info->users == NULL) {
			...
			DEBUG10(("samr_reply_query_dispinfo: starting user enumeration at index %u\n",
				(unsigned  int)enum_context ));
		} else {
			DEBUG10(("samr_reply_query_dispinfo: using cached user enumeration at index %u\n",
				(unsigned  int)enum_context ));
		}
	...
	case 0x2:
		if (info->disp_info->machines == NULL) {
			...
			DEBUG10(("samr_reply_query_dispinfo: starting machine enumeration at index %u\n",
				(unsigned  int)enum_context ));
		} else {
			DEBUG10(("samr_reply_query_dispinfo: using cached machine enumeration at index %u\n",
				(unsigned  int)enum_context ));
		}
	...
	case 0x5:
		if (info->disp_info->groups == NULL) {
			...
			DEBUG10(("samr_reply_query_dispinfo: starting group enumeration at index %u\n",
				(unsigned  int)enum_context ));
		} else {
			DEBUG10(("samr_reply_query_dispinfo: using cached group enumeration at index %u\n",
				(unsigned  int)enum_context ));
		}
...
NTSTATUS _samr_query_aliasinfo(pipes_struct *p, SAMR_Q_QUERY_ALIASINFO *q_u, SAMR_R_QUERY_ALIASINFO *r_u)
{
	...
	DEBUG5(("_samr_query_aliasinfo: %d\n", __LINE__));
 ...
 uint32 _samr_lookup_ids(pipes_struct *p, SAMR_Q_LOOKUP_IDS *q_u, SAMR_R_LOOKUP_IDS *r_u)
{
    ...
    DEBUG5(("_samr_lookup_ids: %d\n", __LINE__));

    if (num_rids > MAX_SAM_ENTRIES) {
        num_rids = MAX_SAM_ENTRIES;
        DEBUG5(("_samr_lookup_ids: truncating entries to %d\n", num_rids));
    }
...
NTSTATUS _samr_lookup_names(pipes_struct *p, SAMR_Q_LOOKUP_NAMES *q_u, SAMR_R_LOOKUP_NAMES *r_u)
{
	...
	if (num_rids > MAX_SAM_ENTRIES) {
		num_rids = MAX_SAM_ENTRIES;
		DEBUG5(("_samr_lookup_names: truncating entries to %d\n", num_rids));
	}

	DEBUG5(("_samr_lookup_names: looking name on SID %s\n", sid_to_string(sid_str, &pol_sid)));
...
NTSTATUS _samr_chgpasswd_user(pipes_struct *p, SAMR_Q_CHGPASSWD_USER *q_u, SAMR_R_CHGPASSWD_USER *r_u)
{
	...
	DEBUG5(("samr_chgpasswd_user: user: %s wks: %s\n", user_name, wks));
...
NTSTATUS _samr_chgpasswd_user3(pipes_struct *p, SAMR_Q_CHGPASSWD_USER3 *q_u, SAMR_R_CHGPASSWD_USER3 *r_u)
{
	...
	DEBUG5(("_samr_chgpasswd_user3: %d\n", __LINE__));

	rpcstr_pull(user_name, q_u->uni_user_name.buffer, sizeof(user_name), q_u->uni_user_name.uni_str_len*2, 0);
	rpcstr_pull(wks, q_u->uni_dest_host.buffer, sizeof(wks), q_u->uni_dest_host.uni_str_len*2,0);

	DEBUG5(("_samr_chgpasswd_user3: user: %s wks: %s\n", user_name, wks));
...
NTSTATUS _samr_lookup_rids(pipes_struct *p, SAMR_Q_LOOKUP_RIDS *q_u, SAMR_R_LOOKUP_RIDS *r_u)
{
	...
	if (num_rids > 1000) {
		DEBUG0( ("Got asked for %d rids (more than 1000) -- according "
...
NTSTATUS _samr_query_userinfo(pipes_struct *p, SAMR_Q_QUERY_USERINFO *q_u, SAMR_R_QUERY_USERINFO *r_u)
{
	...
	DEBUG5(("_samr_query_userinfo: sid:%s\n", sid_string_static(&info->sid)));
	...
	DEBUG5(("_samr_query_userinfo: user info level: %d\n", q_u->switch_value));
...
NTSTATUS _samr_query_usergroups(pipes_struct *p, SAMR_Q_QUERY_USERGROUPS *q_u, SAMR_R_QUERY_USERGROUPS *r_u)
{
	...
	DEBUG5(("_samr_query_usergroups: %d\n", __LINE__));
	...
	if (!ret) {
		DEBUG10( ("pdb_getsampwsid failed for %s\n",
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG10( ("pdb_enum_group_memberships failed for %s\n",
			   sid_string_static(&sid)));
		return result;
	}

	if ( !success ) {
		DEBUG5( ("Group sid %s for user %s not in our domain\n",
	...
	for (i=0; i<num_groups; i++) {
		...
		if (!sid_peek_check_rid(get_global_sam_sid(),
			...
			DEBUG10( ("Found sid %s not in our domain\n",
				 ...
				 SAMR_R_QUERY_DOMAIN_INFO *r_u)
{
	...
	DEBUG5(("_samr_query_domain_info: %d\n", __LINE__));
...
static NTSTATUS can_create(TALLOC_CTX *mem_ctx, const char *new_name)
{
	...
	DEBUG10( ("Checking whether [%s] can be created\n", new_name));
	...
	if (!result) {
		DEBUG10( ("%s does not exist, can create it\n", new_name));
		return NT_STATUS_OK;
	}

	DEBUG5( ("trying to create %s, exists as %s\n",
			   ...
			   SAMR_R_CREATE_USER *r_u)
{
	...
	DEBUG5( ("_samr_create_user: %s can add this account : %s\n",
...
NTSTATUS _samr_connect_anon(pipes_struct *p, SAMR_Q_CONNECT_ANON *q_u, SAMR_R_CONNECT_ANON *r_u)
{
	...
	if (!pipe_access_check(p)) {
		DEBUG3( ("access denied to samr_connect_anon\n"));
...
NTSTATUS _samr_connect(pipes_struct *p, SAMR_Q_CONNECT *q_u, SAMR_R_CONNECT *r_u)
{
	...
	DEBUG5(("_samr_connect: %d\n", __LINE__));

	/* Access check */

	if (!pipe_access_check(p)) {
		DEBUG3( ("access denied to samr_connect\n"));
		r_u->status = NT_STATUS_ACCESS_DENIED;
		return r_u->status;
	}
...
NTSTATUS _samr_connect4(pipes_struct *p, SAMR_Q_CONNECT4 *q_u, SAMR_R_CONNECT4 *r_u)
{
	...
	DEBUG5(("_samr_connect4: %d\n", __LINE__));

	/* Access check */

	if (!pipe_access_check(p)) {
		DEBUG3( ("access denied to samr_connect4\n"));
		r_u->status = NT_STATUS_ACCESS_DENIED;
		return r_u->status;
	}
...
NTSTATUS _samr_connect5(pipes_struct *p, SAMR_Q_CONNECT5 *q_u, SAMR_R_CONNECT5 *r_u)
{
	...
	if (!pipe_access_check(p)) {
		DEBUG3( ("access denied to samr_connect5\n"));
		r_u->status = NT_STATUS_ACCESS_DENIED;
		return r_u->status;
	}
...
NTSTATUS _samr_lookup_domain(pipes_struct *p, SAMR_Q_LOOKUP_DOMAIN *q_u, SAMR_R_LOOKUP_DOMAIN *r_u)
{
	...
	DEBUG2(("Returning domain sid for domain %s -> %s\n", domain_name, sid_string_static(&sid)));
...
{
	if (id16 == NULL) {
		DEBUG5( ("set_user_info_16: NULL id16\n"));
		TALLOC_FREE(pwd);
		return False;
	}
...
static BOOL set_user_info_18(SAM_USER_INFO_18 *id18, struct samu *pwd)
{
	...
	if (id18 == NULL) {
		DEBUG2( ("set_user_info_18: id18 is NULL\n"));
		TALLOC_FREE(pwd);
		return False;
	}
...
{
	if (id20 == NULL) {
		DEBUG5( ("set_user_info_20: NULL id20\n"));
		return False;
	}
				 ...
				 struct samu *pwd)
{
	...
	if (id21 == NULL) {
		DEBUG5( ("set_user_info_21: NULL id21\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
		...
		&& (!strequal(new_name, pdb_get_username(pwd)))) 
	{
		...
		if (!NT_STATUS_IS_OK(status)) {
			DEBUG0(("set_user_info_21: failed to rename account: %s\n", 
				nt_errstr(status)));
			TALLOC_FREE(pwd);
			return status;
		}
				 ...
				 struct samu *pwd)
{
	...
	if (id23 == NULL) {
		DEBUG5( ("set_user_info_23: NULL id23\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
 
	DEBUG5( ("Attempting administrator password change (level 23) for user %s\n",
	...
	if (    ( (acct_ctrl &  ACB_DOMTRUST) == ACB_DOMTRUST ) ||
		...
		DEBUG5( ("Changing trust account.  Not updating /etc/passwd\n"));
	} else  {
		/* update the UNIX password */
		if (lp_unix_password_sync() ) {
			struct passwd *passwd;
			if (pdb_get_username(pwd) == NULL) {
				DEBUG1( ("chgpasswd: User without name???\n"));
				TALLOC_FREE(pwd);
				return NT_STATUS_ACCESS_DENIED;
			}

			if ((passwd = Get_Pwnam(pdb_get_username(pwd))) == NULL) {
				DEBUG1( ("chgpasswd: Username does not exist in system !?!\n"));
			}
			    ...
			    int level)
{
	...
	DEBUG5( ("Attempting administrator password change for user %s\n",
		  pdb_get_username(pwd)));
	...
	if ( ( (acct_ctrl &  ACB_DOMTRUST) == ACB_DOMTRUST ) ||
		( (acct_ctrl &  ACB_WSTRUST) ==  ACB_WSTRUST) ||
		( (acct_ctrl &  ACB_SVRTRUST) ==  ACB_SVRTRUST) ) {
		DEBUG5( ("Changing trust account or non-unix-user password, not updating /etc/passwd\n"));
	} else {
		/* update the UNIX password */
		if (lp_unix_password_sync()) {
			struct passwd *passwd;
	...
	DEBUG5(("set_user_info_pw: pdb_update_pwd()\n"));
				 ...
				 struct samu *pwd)
{
	...
	if (id25 == NULL) {
		DEBUG5( ("set_user_info_25: NULL id25\n"));
		return NT_STATUS_INVALID_PARAMETER;
	}
...
NTSTATUS _samr_set_userinfo(pipes_struct *p, SAMR_Q_SET_USERINFO *q_u, SAMR_R_SET_USERINFO *r_u)
{
	...
	DEBUG5( ("_samr_set_userinfo: sid:%s, level:%d\n", sid_string_static(&sid), switch_value));

	if (ctr == NULL) {
		DEBUG5( ("_samr_set_userinfo: NULL info level\n"));
		return NT_STATUS_INVALID_INFO_CLASS;
	}
	...
	DEBUG5( ("_samr_set_userinfo: %s does%s possess sufficient rights\n",
		  uidtoname(p->pipe_user.ut.uid),
...
NTSTATUS _samr_set_userinfo2(pipes_struct *p, SAMR_Q_SET_USERINFO2 *q_u, SAMR_R_SET_USERINFO2 *r_u)
{
	...
	DEBUG5( ("samr_reply_set_userinfo2: %d\n", __LINE__));
	...
	DEBUG5( ("samr_reply_set_userinfo2: sid:%s\n", sid_string_static(&sid)));

	if (ctr == NULL) {
		DEBUG5( ("samr_reply_set_userinfo2: NULL info level\n"));
		return NT_STATUS_INVALID_INFO_CLASS;
	}
	...
	DEBUG5( ("_samr_set_userinfo2: %s does%s possess sufficient rights\n",
		  uidtoname(p->pipe_user.ut.uid),
		  has_enough_rights ? "" : " not"));
...
NTSTATUS _samr_query_useraliases(pipes_struct *p, SAMR_Q_QUERY_USERALIASES *q_u, SAMR_R_QUERY_USERALIASES *r_u)
{
	...
	DEBUG5(("_samr_query_useraliases: %d\n", __LINE__));
...
NTSTATUS _samr_query_groupmem(pipes_struct *p, SAMR_Q_QUERY_GROUPMEM *q_u, SAMR_R_QUERY_GROUPMEM *r_u)
{
	...
	if (!sid_check_is_in_our_domain(&group_sid)) {
		DEBUG3( ("sid %s is not in our domain\n", group_sid_str));
		return NT_STATUS_NO_SUCH_GROUP;
	}

	DEBUG10( ("lookup on Domain SID\n"));
...
NTSTATUS _samr_del_aliasmem(pipes_struct *p, SAMR_Q_DEL_ALIASMEM *q_u, SAMR_R_DEL_ALIASMEM *r_u)
{
	...
	DEBUG10( ("_samr_del_aliasmem:sid is %s\n",
		   sid_string_static(&alias_sid)));
...
NTSTATUS _samr_delete_dom_user(pipes_struct *p, SAMR_Q_DELETE_DOM_USER *q_u, SAMR_R_DELETE_DOM_USER *r_u )
{
	...
	DEBUG5( ("_samr_delete_dom_user: %d\n", __LINE__));
	...
	if( !ret ) {
		DEBUG5(("_samr_delete_dom_user:User %s doesn't exist.\n", 
			sid_string_static(&user_sid)));
		TALLOC_FREE(sam_pass);
		return NT_STATUS_NO_SUCH_USER;
	}
	...
	if ( !NT_STATUS_IS_OK(r_u->status) ) {
		DEBUG5(("_samr_delete_dom_user: Failed to delete entry for "
			 "user %s: %s.\n", pdb_get_username(sam_pass),
...
NTSTATUS _samr_delete_dom_group(pipes_struct *p, SAMR_Q_DELETE_DOM_GROUP *q_u, SAMR_R_DELETE_DOM_GROUP *r_u)
{
	...
	DEBUG5( ("samr_delete_dom_group: %d\n", __LINE__));
	...
	if ( !NT_STATUS_IS_OK(r_u->status) ) {
		DEBUG5(("_samr_delete_dom_group: Failed to delete mapping "
			 "entry for group %s: %s\n",
			 sid_string_static(&group_sid),
			 nt_errstr(r_u->status)));
		return r_u->status;
...
NTSTATUS _samr_delete_dom_alias(pipes_struct *p, SAMR_Q_DELETE_DOM_ALIAS *q_u, SAMR_R_DELETE_DOM_ALIAS *r_u)
{
	...
	DEBUG5( ("_samr_delete_dom_alias: %d\n", __LINE__));
...
NTSTATUS _samr_create_dom_alias(pipes_struct *p, SAMR_Q_CREATE_DOM_ALIAS *q_u, SAMR_R_CREATE_DOM_ALIAS *r_u)
{
	...
	if (!NT_STATUS_IS_OK(result)) {
		DEBUG10( ("pdb_create_alias failed: %s\n",
			   nt_errstr(result)));
		return result;
	}
	...
	if (!sid_to_gid(&info_sid, &gid)) {
		DEBUG10( ("Could not find alias just created\n"));
		return NT_STATUS_ACCESS_DENIED;
	}

	/* check if the group has been successfully created */
	if ( getgrgid(gid) == NULL ) {
		DEBUG10( ("getgrgid(%d) of just created alias failed\n",
			   gid));
		return NT_STATUS_ACCESS_DENIED;
	}
...
NTSTATUS _samr_get_dom_pwinfo(pipes_struct *p, SAMR_Q_GET_DOM_PWINFO *q_u, SAMR_R_GET_DOM_PWINFO *r_u)
{
	...
	if (!pipe_access_check(p)) {
		DEBUG3( ("access denied to samr_get_dom_pwinfo\n"));
		r_u->status = NT_STATUS_ACCESS_DENIED;
		return r_u->status;
	}
...
NTSTATUS _samr_open_group(pipes_struct *p, SAMR_Q_OPEN_GROUP *q_u, SAMR_R_OPEN_GROUP *r_u)
{
	...
	DEBUG10( ("_samr_open_group:Opening SID: %s\n", sid_string));
                                          ...
                                          SAMR_R_REMOVE_SID_FOREIGN_DOMAIN *r_u)
{
	...
	DEBUG5(("_samr_remove_sid_foreign_domain: removing SID [%s]\n",
	...
	DEBUG8( ("_samr_remove_sid_foreign_domain:sid is %s\n", 
	...
	if (!sid_check_is_builtin(&domain_sid)) {
		DEBUG1(("_samr_remove_sid_foreign_domain: domain_sid = %s, "
			 "global_sam_sid() = %s\n",
			 sid_string_static(&domain_sid),
				  ...
				  SAMR_R_QUERY_DOMAIN_INFO2 *r_u)
{
	...
	DEBUG5(("_samr_query_domain_info2: %d\n", __LINE__));
...
NTSTATUS _samr_set_dom_info(pipes_struct *p, SAMR_Q_SET_DOMAIN_INFO *q_u, SAMR_R_SET_DOMAIN_INFO *r_u)
{
	...
	DEBUG5(("_samr_set_dom_info: %d\n", __LINE__));

	/* find the policy handle.  open a policy on it. */
	if (!find_policy_by_hnd(p, &q_u->domain_pol, NULL))
		return NT_STATUS_INVALID_HANDLE;

	DEBUG5(("_samr_set_dom_info: switch_value: %d\n", q_u->switch_value));
...
NTSTATUS _samr_get_dispenum_index(pipes_struct *p, SAMR_Q_GET_DISPENUM_INDEX *q_u, SAMR_R_GET_DISPENUM_INDEX *r_u)
{
	...
	DEBUG5( ("_samr_get_dispenum_index: %d\n", __LINE__));
	...
	if ((q_u->switch_level < 1) || (q_u->switch_level > 3)) {
		DEBUG0(("_samr_get_dispenum_index: Unknown info level (%u)\n",
			 (unsigned int)q_u->switch_level ));
		return NT_STATUS_INVALID_INFO_CLASS;
	}
	...
	case 0x1:
		if (info->disp_info->users == NULL) {
			...
			DEBUG10(("_samr_get_dispenum_index: starting user enumeration at index %u\n",
				(unsigned  int)enum_context ));
		} else {
			DEBUG10(("_samr_get_dispenum_index: using cached user enumeration at index %u\n",
				(unsigned  int)enum_context ));
		}
	...
	case 0x2:
		if (info->disp_info->machines == NULL) {
			...
			DEBUG10(("_samr_get_dispenum_index: starting machine enumeration at index %u\n",
				(unsigned  int)enum_context ));
		} else {
			DEBUG10(("_samr_get_dispenum_index: using cached machine enumeration at index %u\n",
				(unsigned  int)enum_context ));
		}
	...
	case 0x3:
		if (info->disp_info->groups == NULL) {
			...
			DEBUG10(("_samr_get_dispenum_index: starting group enumeration at index %u\n",
				(unsigned  int)enum_context ));
		} else {
			DEBUG10(("_samr_get_dispenum_index: using cached group enumeration at index %u\n",
				(unsigned  int)enum_context ));
		}
	...
	DEBUG10(("_samr_get_dispenum_index: looking for :%s\n", account_name));

	for (i=0; i<num_account; i++) {
		if (strequal(entries[i].account_name, account_name)) {
			DEBUG10(("_samr_get_dispenum_index: found %s at idx %d\n",
				account_name, i));
