...
static BOOL init_registry_data( void )
{
	...
	if ( tdb_transaction_start( tdb_reg ) == -1 ) {
		DEBUG0( ("init_registry_data: tdb_transaction_start "
			  "failed\n"));
		return False;
	}
	...
	for ( i=0; builtin_registry_paths[i] != NULL; i++ ) {
		...
		DEBUG6(("init_registry_data: Adding [%s]\n", builtin_registry_paths[i]));
		...
		while ( next_token(&p, keyname, "\\", sizeof(keyname)) ) {
			...
			*subkeyname = '\0';
			...
			DEBUG10(("init_registry_data: Storing key [%s] with subkey [%s]\n",
	...
	for ( i=0; builtin_registry_values[i].path != NULL; i++ ) {
		...
		if ( !regval_ctr_key_exists( values, builtin_registry_values[i].valuename ) ) 
		{
			...
			default:
				DEBUG0(("init_registry_data: invalid value type in builtin_registry_values [%d]\n",
	...
	if (tdb_transaction_commit( tdb_reg ) == -1) {
		DEBUG0( ("init_registry_data: Could not commit "
			  "transaction\n"));
		return False;
	}
...
BOOL regdb_init( void )
{
	...
	if ( !(tdb_reg = tdb_open_log(lock_path("registry.tdb"), 0, TDB_DEFAULT, O_RDWR, 0600)) )
	{
		...
		if ( !tdb_reg ) {
			DEBUG0(("regdb_init: Failed to open registry %s (%s)\n",
				lock_path("registry.tdb"), strerror(errno) ));
			return False;
		}
		
		DEBUG10(("regdb_init: Successfully created registry tdb\n"));
	}
	...
	if ( !init_registry_data() ) {
		DEBUG0(("init_registry: Failed to initialize data in registry!\n"));
		return False;
	}
...
WERROR regdb_open( void )
{
	...
	if ( tdb_reg ) {
		DEBUG10(("regdb_open: incrementing refcount (%d)\n", tdb_refcount));
		tdb_refcount++;
		return WERR_OK;
	}
	...
	if ( !tdb_reg ) {
		...
		DEBUG0(("regdb_open: Failed to open %s! (%s)\n", 
			lock_path("registry.tdb"), strerror(errno) ));
	}
	...
	tdb_refcount = 1;
	DEBUG10(("regdb_open: refcount reset (%d)\n", tdb_refcount));
...
int regdb_close( void )
{
	...
	DEBUG10(("regdb_close: decrementing refcount (%d)\n", tdb_refcount));
	...
	for (i=0; i<num_subkeys; i++) {
		...
		if ( len > buflen ) {
			...
			if ((buffer = (char *)SMB_REALLOC( buffer, len*2 )) == NULL) {
				DEBUG0(("regdb_store_keys: Failed to realloc memory of size [%d]\n", len*2));
				ret = False;
				goto done;
			}
			buflen = len*2;
...
BOOL regdb_store_keys( const char *key, REGSUBKEY_CTR *ctr )
{
	...
	if ( !(old_subkeys = TALLOC_ZERO_P( ctr, REGSUBKEY_CTR )) ) {
		DEBUG0(("regdb_store_keys: talloc() failure!\n"));
		return False;
	}
	...
	if ( !regdb_store_keys_internal( key, ctr ) ) {
		DEBUG0(("regdb_store_keys: Failed to store new subkey list for parent [%s}\n", key ));
		return False;
	}
	...
	for ( i=0; i<num_subkeys; i++ ) {
		...
		if ( regdb_fetch_keys( path, subkeys ) == -1 ) {
			...
			if ( !regdb_store_keys_internal( path, subkeys ) ) {
				DEBUG0(("regdb_store_keys: Failed to store new record for key [%s}\n", path ));
				TALLOC_FREE( subkeys );
				return False;
			}
		}
...
int regdb_fetch_keys( const char* key, REGSUBKEY_CTR *ctr )
{
	...
	if ( !buf ) {
		DEBUG5(("regdb_fetch_keys: tdb lookup failed to locate key [%s]\n", key));
		return -1;
	}
...
int regdb_fetch_values( const char* key, REGVAL_CTR *values )
{
	...
	DEBUG10(("regdb_fetch_values: Looking for value of key [%s] \n", key));
...
BOOL regdb_store_values( const char *key, REGVAL_CTR *values )
{
	...
	DEBUG10(("regdb_store_values: Looking for value of key [%s] \n", key));
	
	ZERO_STRUCT( data );
	
	len = regdb_pack_values( values, data.dptr, data.dsize );
	if ( len <= 0 ) {
		DEBUG0(("regdb_store_values: unable to pack values. len <= 0\n"));
		return False;
	}
