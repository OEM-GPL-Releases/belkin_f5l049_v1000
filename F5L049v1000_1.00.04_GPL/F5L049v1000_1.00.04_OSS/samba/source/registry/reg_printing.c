...
static int key_forms_fetch_values( const char *key, REGVAL_CTR *values )
{
	...
	DEBUG10(("print_values_forms: key=>[%s]\n", key ? key : "NULL" ));
	
	num_values = get_ntforms( &forms_list );
		
	DEBUG10(("hive_forms_fetch_values: [%d] user defined forms returned\n",
		num_values));
	...
	DEBUG10(("print_subpath_values_forms: [%d] built-in forms returned\n",
		num_values));
...
static int key_printers_fetch_keys( const char *key, REGSUBKEY_CTR *subkeys )
{
	...
	DEBUG10(("key_printers_fetch_keys: key=>[%s]\n", key ? key : "NULL" ));
	...
	for ( i=0; i<num_keys; i++ ) {
		...
		if ( snum == -1 ) {
			...
			if ( !add_printer_hook( NULL, &printer ) ) {
				DEBUG0(("add_printers_by_registry: Failed to add printer [%s]\n",
					printername));
			}	
		}
	}
...
static BOOL key_printers_store_keys( const char *key, REGSUBKEY_CTR *subkeys )
{
	...
	if ( !W_ERROR_IS_OK(get_a_printer(NULL, &printer, 2, printername)) ) {
		DEBUG0(("key_printers_store_keys: Tried to store subkey for bad printername %s\n", 
			printername));
		return False;
	}
	...
	for ( i=0; i<num_existing_keys; i++ ) {
		...
		if ( !regsubkey_ctr_key_exists( subkeys, existing_subkeys[i] ) ) {
			DEBUG5(("key_printers_store_keys: deleting key %s\n", 
				existing_subkeys[i]));
	...
	for ( i=0; i<num_subkeys; i++ ) {
		...
		if ( lookup_printerkey(printer->info_2->data, subkeyname) == -1 ) {
			DEBUG5(("key_printers_store_keys: adding key %s\n", 
				existing_subkeys[i]));
...
static int key_printers_fetch_values( const char *key, REGVAL_CTR *values )
{
	...
	if ( (key_index = lookup_printerkey( p_data, printerdatakey )) == -1  ) {
		...
		DEBUG10(("key_printers_fetch_values: Unknown keyname [%s]\n", printerdatakey));
		if ( printer )
			free_a_printer( &printer, 2 );
		return -1;
	}
	...
	for ( i=0; i<num_values; i++ ) {
		...
		switch( value_index ) {
			...
			default:
				...
				DEBUG8(("convert_values_to_printer_info_2: Unsupported registry value [%s]\n", 
					regval_name( val ) ));
		}
	}
	...
	if ( !keyname ) {
		...
		for ( i=0; i<num_values; i++ ) {
			...
			if ( !W_ERROR_IS_OK(result) ) {
				DEBUG0(("key_printers_store_values: failed to set printer data [%s]!\n",
					keyname));
				free_a_printer( &printer, 2 );
				return False;
			}
...
static int key_driver_fetch_keys( const char *key, REGSUBKEY_CTR *subkeys )
{
	...
	DEBUG10(("key_driver_fetch_keys key=>[%s]\n", key ? key : "NULL" ));
	...
	DEBUG1(("key_driver_fetch_keys unhandled key [%s] (subkey == %s\n", 
		key, subkeypath ));
...
static int driver_arch_fetch_values( char *key, REGVAL_CTR *values )
{
	...
	DEBUG8(("key_driver_fetch_values: Exit\n"));
...
static int key_driver_fetch_values( const char *key, REGVAL_CTR *values )
{
	...
	DEBUG8(("key_driver_fetch_values: Enter key => [%s]\n", key ? key : "NULL"));
