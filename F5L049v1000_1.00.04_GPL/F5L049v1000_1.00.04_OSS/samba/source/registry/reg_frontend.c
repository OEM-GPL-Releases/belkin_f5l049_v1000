                                     ...
                                     uint32 access_desired, uint32 *access_granted )
{
	...
	if ( geteuid() == sec_initial_uid() ) {
		DEBUG5(("registry_access_check: using root's token\n"));
...
BOOL init_registry( void )
{
	...
	if ( !regdb_init() ) {
		DEBUG0(("init_registry: failed to initialize the registry tdb!\n"));
		return False;
	}
...
BOOL fetch_reg_keys_specific( REGISTRY_KEY *key, char** subkey, uint32 key_index )
{
	...
	*subkey = NULL;
	...
	DEBUG8(("fetch_reg_keys_specific: Looking for key [%d] of  [%s]\n", key_index, key->name));
	
	if ( !ctr ) {
		DEBUG8(("fetch_reg_keys_specific: Initializing cache of subkeys for [%s]\n", key->name));

		if ( !(ctr = TALLOC_ZERO_P( NULL, REGSUBKEY_CTR )) ) {
			DEBUG0(("fetch_reg_keys_specific: talloc() failed!\n"));
			return False;
		}
	...
	/* clear the cache when key_index == 0 or the path has changed */
		...
		DEBUG8(("fetch_reg_keys_specific: Updating cache of subkeys for [%s]\n", key->name));
...
BOOL fetch_reg_values_specific( REGISTRY_KEY *key, REGISTRY_VALUE **val, uint32 val_index )
{
	...
	*val = NULL;
	...
	if ( !ctr ) {
		DEBUG8(("fetch_reg_values_specific: Initializing cache of values for [%s]\n", key->name));

		if ( !(ctr = TALLOC_ZERO_P( NULL, REGVAL_CTR )) ) {
			DEBUG0(("fetch_reg_values_specific: talloc() failed!\n"));
			return False;
		}
	...
	/* clear the cache when val_index == 0 or the path has changed */
		...
		DEBUG8(("fetch_reg_values_specific: Updating cache of values for [%s]\n", key->name));		
                             ...
                             NT_USER_TOKEN *token, uint32 access_desired )
{
	...
	DEBUG7(("regkey_open_internal: name = [%s]\n", path));
	...
	if ( !(keyinfo->hook = reghook_cache_find( keyinfo->name )) ) {
		DEBUG0(("open_registry_key: Failed to assigned a REGISTRY_HOOK to [%s]\n",
			keyinfo->name ));
		result = WERR_BADFILE;
		goto done;
	}
