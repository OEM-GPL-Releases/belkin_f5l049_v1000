...
static struct cli_state *connect_one(char *share)
{
 ...
 again:
	...
	if (!(c=cli_initialise())) {
		DEBUG0(("Connection to %s failed\n", server_n));
		return NULL;
	}

	status = cli_connect(c, server_n, &ip);
	if (!NT_STATUS_IS_OK(status)) {
		DEBUG0(("Connection to %s failed. Error %s\n", server_n, nt_errstr(status) ));
		return NULL;
	}
	...
	if (!cli_session_request(c, &calling, &called)) {
		DEBUG0(("session request to %s failed\n", called.name));
		cli_shutdown(c);
	...
	if (!cli_negprot(c)) {
		DEBUG0(("protocol negotiation failed\n"));
		cli_shutdown(c);
		return NULL;
	}
...
static void testpair(struct cli_state *cli, char *mask, char *file)
{
	...
	if (showall || 
		...
		DEBUG0(("%s %s %d mask=[%s] file=[%s] rfile=[%s/%s]\n",
