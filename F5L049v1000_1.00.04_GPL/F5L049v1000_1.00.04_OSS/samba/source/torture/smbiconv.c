  ...
  while (len > 0)
      ...
      if (errno != E2BIG)
	{
	  ...
	  switch (errno)
	    {
	    case EILSEQ:
	      DEBUG0(("illegal input sequence at position %ld", 
		     (long) (addr - start)));
	      break;
	    case EINVAL:
	      DEBUG0( ("\
incomplete character or shift sequence at end of buffer"));
	      break;
	    case EBADF:
	      DEBUG0( ("internal error (illegal descriptor)"));
	      break;
	    default:
	      DEBUG0( ("unknown iconv() error %d", errno));
	      break;
	    }
...
process_fd (iconv_t cd, int fd, FILE *output)
{
  ...
  while (actlen < maxlen)
	...
	{
	  /* Error while reading.  */
	  DEBUG0( ("error while reading the input"));
	  return -1;
	}
  ...
  if (actlen == maxlen)
	...
	if (new_inbuf == NULL)
	  {
	    DEBUG0( ("unable to allocate buffer for input"));
	    return -1;
	  }
	inbuf = new_inbuf;
	maxlen += 32768;
...
int main(int argc, char *argv[])
{
	...
	if(output) {
		...
		if(!out) {
			DEBUG0( ("Can't open output file '%s': %s, exiting...\n", output, strerror(errno)));
			return 1;
		}
	}
	...
	if((int)cd == -1) {
		DEBUG0(("unable to find from or to encoding, exiting...\n"));
		return 1;
	}
	...
	while((file = poptGetArg(pc))) {
		if(strcmp(file, "-") == 0) fd = 0;
			...
			if(!fd) {
				DEBUG0( ("Can't open input file '%s': %s, ignoring...\n", file, strerror(errno)));
				continue;
			}
		}
