 ...
 * $Id: //depot/sw/releases/7.1_AP/apps/wireless_tools.28/ifrename.c#1 $
 * -------------------------------------------------------
 *
		 ...
		 const char *		victimname)
{
  ...
  strncpy(ifr.ifr_name, victimname, IFNAMSIZ -1);
  strncpy(ifr.ifr_newname, autoname, IFNAMSIZ -1);
	    ...
	    char *		retname)
{
  ...
  strncpy(ifr.ifr_newname, newname, IFNAMSIZ -1);
		   ...
		   int			flag)
{
  ...
  strncpy(target->iwproto, wrq.u.name, IFNAMSIZ -1);
...
probe_mappings(int		skfd)
{
  ...
  for(ifnode = mapping_list; ifnode != NULL; ifnode = ifnode->next)
	...
	fprintf(stderr, "Probing : Trying to load interface [%s]\n",
       ...
       */
      strncpy(ifr.ifr_name, ifnode->ifname, IFNAMSIZ -1);
