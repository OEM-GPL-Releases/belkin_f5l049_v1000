...
#if 0 
	/* BUG 090502: Segmentation fault
	 *    STI's drivers has over 256(IW_CUSTOM_MAX) bytes result !!!
	 */
...
#else
		if((event->u.data.pointer) && (event->u.data.length)) {
			if( event->u.data.length < IW_CUSTOM_MAX ) {
				memcpy(custom, event->u.data.pointer, event->u.data.length);
				custom[event->u.data.length] = '\0';
				printf("                    Extra:%s\n", custom);
			}
			else {
				memcpy(custom, event->u.data.pointer, IW_CUSTOM_MAX);
				custom[IW_CUSTOM_MAX] = '\0';
				printf("                    Extra:%s...\n", custom);
			}
		}
#endif
