...
ushare_new (void)
{
  ...
  ut->model_name = strdup (DEFAULT_MODEL_NAME);
  ut->friendly_name = strdup (DEFAULT_MODEL_NAME);
  ut->manufact_name = NULL;
  ut->manufact_url = NULL;
  ut->name = NULL;
  ut->interface = strdup (DEFAULT_USHARE_IFACE);
  ut->serialnum = strdup (DEFAULT_SERIAL_NUM);
  memset(ut->content_rootdir, 0x00, MAX_DIR_PATH_LEN + 1);
  ut->contentlist = NULL;
  ...
  ut->udn = NULL;
  ut->ip = NULL;
  ut->port = DEFAULT_USHARE_PORT; /* Randomly attributed by libupnp */
#ifdef HAVE_DLNA
...
init_upnp (struct ushare_t *ut)
{
  ...
  if (ut->dlna_enabled)
  {
    ...
    if((model_url = (char *)malloc(sizeof(char) * (strlen("http:///") +
    	 strlen(ut->name) + 1))) == NULL) {
    	 return -1;
    }
    sprintf(model_url, "http://%s/", ut->name);

    if((fname = (char *)malloc(sizeof(char) * (strlen(ut->model_name) +
    	 strlen(ut->name) + 1))) == NULL) {
    	 return -1;
    }
    sprintf(fname, "%s: %s", ut->model_name, ut->name);
  ...
  else
  {
	...
	len = strlen (UPNP_DESCRIPTION) + (strlen (ut->name) * 2) + (strlen(ut->friendly_name))
	 + strlen(ut->serialnum) + (strlen (ut->model_name) * 1) + strlen (ut->udn) + 1;

	if(ut->manufact_name != NULL) { 
		len += strlen(ut->manufact_name);
	}
	else {
		++len;
	}

	if(ut->manufact_url != NULL) {
		len += strlen(ut->manufact_url) * 2;
	}
	else {
		len += 2;
	}
  ...
  sprintf (description, UPNP_DESCRIPTION, ut->name, ut->model_name, ut->manufact_name,
    ut->manufact_url, ut->friendly_name, ut->manufact_url, ut->serialnum, ut->udn, ut->name);
...
has_iface (char *interface)
{
  ...
  for (i = n - 1 ; i >= 0 ; i--)
  {
    ...
    {
      /* interface is down */
      log_error (_("(2)Interface %s is down.\n"), interface);
      log_error (_("(2)Recheck uShare's configuration and try again !\n"));
      close (sock);
      return false;
    }
...
get_iface_address (char *interface)
{
  ...
  if (sock < 0)
  {
    log_error (_("Error: create socket(%d)\n"), errno);
    perror ("socket");
    return NULL;
  }
  ...
  { 
      log_error (_("Error: ioctl socket(%d)\n"), errno);
      perror ("ioctl");
      close (sock);
      return NULL;
  }
...
main (int argc, char **argv)
{
  ...
  {
    /* fprintf here, because syslog not yet ready */
    fprintf (stderr, ("Warning: can't command line.\n"));
    ushare_free (ut);
    return EXIT_SUCCESS;
  }
  ...
  if (ut->on == 1) {
	...
	/*  signal (SIGHUP, reload_config); */
	  ...
	  {
	    log_error (_("Error: no content directory to be shared.\n"));
	    log_error (_("Directory: %s\n"), ut->content_rootdir);
	    log_error (_("ConfFile : %s\n"), DEFAULT_CONFIG_FILE);
	  }
	  else {

		  if (!has_iface (ut->interface))
		  {
	        log_error (_("ERROR: IF ERR(main)\n"));
		    ushare_free (ut);
		    return EXIT_FAILURE;
		  }
		  ...
		  if (!ut->ip)
		  {
            ...
            ut->interface = strdup (DEFAULT_USHARE_IFACE_LOCAL);
		    if (!has_iface (ut->interface))
		    {
	          log_error (_("ERROR: IF ERR(main)\n"));
		      ushare_free (ut);
		    ...
		    if (!ut->ip)
            {
	          log_error (_("Error: interface address.\n"));
		      ushare_free (ut);
		      return EXIT_FAILURE;
		    }
		  }
