...
extern int set_fuse_dev_path( const char *path );
extern const char *get_fuse_dev_path( void );
				...
				char *value, size_t size)
{
	...
	/* NTFS attribute */
	if(!strcmp(name,"/attribute/")&&value&&size>=sizeof(FILE_ATTR_FLAGS)) {
		FILE_ATTR_FLAGS attr;
		ni = ntfs_pathname_to_inode(ctx->vol, NULL, path);
		attr = __le32_to_cpu(ni->flags);
		if(memcpy(value,&attr,sizeof(FILE_ATTR_FLAGS)))
			res = sizeof(FILE_ATTR_FLAGS);
		else
			res = -1;
		goto exit;
	}
				...
				const char *value, size_t size, int flags)
{
	...
	/* NTFS attribute */
	if(!strcmp(name,"/attribute/")&&value&&size==sizeof(FILE_ATTR_FLAGS)) {
		const FILE_ATTR_FLAGS cngattr = 
				FILE_ATTR_READONLY | FILE_ATTR_HIDDEN | FILE_ATTR_ARCHIVE;
		FILE_ATTR_FLAGS attr;
		res = -1;

		ni = ntfs_pathname_to_inode(ctx->vol, NULL, path);
		if(ni) {
			if(!memcpy(&attr,value,sizeof(FILE_ATTR_FLAGS)))
				goto exit;
			attr = __cpu_to_le32(attr);
			attr = (ni->flags&~cngattr) | (attr&cngattr);
			ni->flags = attr;
...
static int parse_options(int argc, char *argv[])
{
	...
	static const char *sopt = "-d:o:hv";
	static const struct option lopt[] = {
		{ "options",	 required_argument,	NULL, 'o' },
		{ "help",	 no_argument,		NULL, 'h' },
		{ "verbose",	 no_argument,		NULL, 'v' },
		{ "devdir",	 required_argument,	NULL, 'd' },
		{ NULL,		 0,			NULL,  0  }
	};
...
static void create_dev_fuse( const char *devdir )
{
	if( devdir ) {
		mknod_dev_fuse(devdir);
	}
	else {
...
int ntfs3g_main(int argc, char *argv[])
{
	...
	create_dev_fuse( get_fuse_dev_path() );
 ...
 * @retval -1 Failde.
**/
static void vlabel_usage( int argc, char **argv )
{
	...
	printf("\n");
	printf("       -d  FUSE device path.\n");
	printf("       -c  convert UTF-16 -> UTF8\n");
	printf("\n");
}

static int vlabel_parse_options(int argc, char *argv[])
{
	int c;

	static const char *sopt = "-d:hc";
	static const struct option lopt[] = {
		{ "help",    no_argument,      NULL, 'h' },
		{ "devdir",  required_argument,NULL, 'd' },
	...
	while ((c = getopt_long(argc, argv, sopt, lopt, NULL)) != -1) {
		...
		case 1:	/* A non-option argument */
			if (!opts.device) {
				...
				/* Canonicalize device name (mtab, etc) */
				if (!realpath(optarg, opts.device)) {
					ntfs_log_perror("%s: Failed to access volume '%s'", EXEC_NAME, optarg);
					free(opts.device);
					opts.device = NULL;
					return -1;
				}
			} else {
				ntfs_log_error("%s: You must specify exactly one device.\n", EXEC_NAME);
				return -1;
			}
			break;
		case 'h':
			vlabel_usage(argc,argv);
			exit(9);
		case 'd':
			set_fuse_dev_path( optarg );
			break;
	...
	if (strappend(&opts.options, "force"))
		return -1;
...
int vlabel_main(int argc, char *argv[])
{
	...
	if (vlabel_parse_options(argc, argv)) {
		ntfs_log_error("Please type '%s --help' for more "
			       "information.\n", argv[0]);
		return NTFS_VOLUME_SYNTAX_ERROR;
	}
	...
	if(ctx->vol->vol_name) {
		err = 0;
		fprintf(stdout,"%s",ctx->vol->vol_name);
		fflush(stdout);
	}
...
{
	if(PROGNAME("ntfs-3g"))    return ntfs3g_main(argc,argv);
	if(PROGNAME("ntfsvlabel")) return vlabel_main(argc,argv);

	return ntfs3g_main(argc,argv);
}
