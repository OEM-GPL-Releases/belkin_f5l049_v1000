                          ...
                          const char *cds_event_url)
{
  ...
  if (!friendly_name || !manufacturer || !manufacturer_url ||
      ...
      !cds_scpd_url || !cds_control_url || !cds_event_url || !model_name )
    return NULL;
  
  len = strlen (UPNP_DMS_DESCRIPTION) + strlen (friendly_name) /* + strlen (model_name)*/
...
#if 0
  sprintf (desc, UPNP_DMS_DESCRIPTION, friendly_name, model_name, 
